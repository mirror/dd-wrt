 ...
 * @ingroup IxEthMii
 *
 * @fn  ixEthMiiPhyPresent(UINT32 phyAddr, BOOL *phyPresentPtr)
 *
 * @brief Scan the MDIO bus for given PHY.
 *  This function looks for the PHY with the given address, and sets phyPresent to 
 *  TRUE if a phy is discovered at that address. 
 *
 * - Reentrant    - no
 * - ISR Callable - no
 *
 ...
 *
 * @param phyAddr UINT32 [in] - address of PHY to search for.
 * @param phyPresentPtr BOOL* [in] - pointer to boolean
 *
 * @return IX_STATUS
 * - IX_ETH_ACC_SUCCESS
 ...
 * <hr>
 */
PUBLIC IX_STATUS ixEthMiiPhyPresent(UINT32 phyAddr, BOOL *phyPresentPtr);
