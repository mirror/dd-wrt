...
int
hal_plf_pci_ignore_bar(void *dev_info, int bar)
{
    cyg_pci_device *dev = dev_info;

    // Here is where a decision can be made whether or not to
    // ignore a particular BAR on a given device. Return non-zero
    // skip configuration of the BAR.
    return 0;
}
