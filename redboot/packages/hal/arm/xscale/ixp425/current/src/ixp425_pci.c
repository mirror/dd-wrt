...
hal_pci_reset_all_devs(void)
{
	...
	if (dev_info.command & CYG_PCI_CFG_COMMAND_ACTIVE) {
	    dev_info.command &= ~CYG_PCI_CFG_COMMAND_ACTIVE;
	    cyg_pci_write_config_uint16(devid, CYG_PCI_CFG_COMMAND, dev_info.command);
	}
    }
}
