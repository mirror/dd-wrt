...
#define CYGHWR_REDBOOT_LINUX_ATAG_MEMSIZE(_sz_)                                                 \
    CYG_MACRO_START                                                                             \
    (_sz_) = (u32)HAL_MEM_REAL_REGION_TOP((u8 *)CYGMEM_REGION_ram + CYGMEM_REGION_ram_SIZE);    \
    (_sz_) -= (u32)CYGMEM_REGION_ram;                                                           \
    CYG_MACRO_END
#else
...
#define CYGHWR_REDBOOT_LINUX_ATAG_MEMSIZE(_sz_)                                                 \
    CYG_MACRO_START                                                                             \
    (_sz_) = 1<<hal_msbindex(CYGMEM_REGION_ram_SIZE);                                           \
    if ((_sz_) < CYGMEM_REGION_ram_SIZE)                                                        \
	    (_sz_) <<= 1;                                                                       \
    CYG_MACRO_END
#endif
#define CYGHWR_REDBOOT_LINUX_ATAG_MEM(_p_)                                                      \
    ...
    CYGHWR_REDBOOT_LINUX_ATAG_MEMSIZE(_p_->u.mem.size);                                         \
...
do_exec(int argc, char *argv[])
{
    ...
    CYGHWR_REDBOOT_LINUX_ATAG_MEM(params);
		...
		mem_size = params->u.mem.size;
		mem_start = params->u.mem.start;
...
#ifdef CYGHWR_REDBOOT_LINUX_EXEC_X_SWITCH
#ifdef CYGPKG_HAL_ARM_XSCALE_IXP425
	if (swap_endian) {
	    int i;
	    cyg_uint32 *s;
	    for (i = 0, s = params->u.cmdline.cmdline; i < (params->hdr.size - 2); i++, s++)
		*s = CYG_SWAP32(*s);
	}
#endif
#endif
