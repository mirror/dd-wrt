...
static void
redir_termination (int signum)
{
  if (optionsdebug)
    printf ("Terminating redir client!\n");
  keep_going = 0;
}

/* Alarm handler for ensured shutdown */
static void
redir_alarm (int signum)
{
...
static int
redir_challenge (unsigned char *dst)
{
  FILE *file;
...
static int
redir_hextochar (char *src, unsigned char *dst)
{
...
static int
redir_chartohex (unsigned char *src, char *dst)
{
...
static int
redir_urlencode (char *src, int srclen, char *dst, int dstsize)
{
...
static int
redir_urldecode (char *src, int srclen, char *dst, int dstsize)
{
...
static int
redir_stradd (char *dst, int dstsize, char *fmt, ...)
{
  va_list args;
  ...
  switch (res)
      ...
      if (redir->radiuslocationname)
	{
      ...
      redir_stradd (dst, dstsize,
		    ...
		    redir->url, inet_ntoa (redir->addr), redir->port,
		    hexchal);
      redir_stradd (dst, dstsize,
...
int
redir_new (struct redir_t **redir, struct in_addr *addr, int port)
{
...
int
redir_free (struct redir_t *redir)
{
  if (close (redir->fd))
    {
...
void
redir_set (struct redir_t *redir, int debug,
...
static int
redir_getpath (struct redir_t *redir, char *src, char *dst, int dstsize)
{
...
static int
redir_geturl (struct redir_t *redir, char *src, char *dst, int dstsize)
{
...
static int
redir_getparam (struct redir_t *redir, char *src,
		char *param, char *dst, int dstsize)
{
...
static int
redir_getreq (struct redir_t *redir, int fd, struct redir_conn_t *conn)
{
  ...
  while ((redir->starttime + REDIR_HTTP_MAX_TIME) > time (NULL))
	...
	{
	  if ((recvlen =
	       recv (fd, buffer + buflen, sizeof (buffer) - 1 - buflen,
		     0)) < 0)
	    {
	      if (errno != ECONNRESET)
		sys_err (LOG_ERR, __FILE__, __LINE__, errno,
  ...
  if ((!strcmp (path, "logon")) || (!strcmp (path, "login")))
			  ...
			  conn->username, sizeof (conn->username)))
	{
      ...
      if (!redir_getparam (redir, buffer, "response", resp, sizeof (resp)))
	{
...
static int
redir_cb_radius_auth_conf (struct radius_t *radius,
			   ...
			   struct radius_packet_t *pack_req, void *cbp)
{
  ...
  if (!radius_getattr (pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
      ...
      else
	{
	  ...
	  sys_err (LOG_WARNING, __FILE__, __LINE__, 0,
		   "Illegal WISPr-Session-Terminate-Time received: %s",
		   attrs);
	}
...
static int
redir_radius (struct redir_t *redir, struct in_addr *addr,
	      struct redir_conn_t *conn)
{
  ...
  if (radius_new (&radius,
		  &redir->radiuslisten, 0, 0, NULL, 0, NULL, NULL, NULL))
    {
  ...
  if (conn->chap == 0)
	...
	user_password[n] = conn->password[n] ^ chap_challenge[n];
      ...
      radius_addattr (radius, &radius_pack, RADIUS_ATTR_USER_PASSWORD, 0, 0,
		      0, user_password, strlen ((char *) user_password));
    }
  else if (conn->chap == 1)
    {
      chap_password[0] = 0;	/* Chap ident */
      memcpy (chap_password + 1, conn->chappassword, REDIR_MD5LEN);
      radius_addattr (radius, &radius_pack, RADIUS_ATTR_CHAP_CHALLENGE, 0, 0,
		      0, chap_challenge, REDIR_MD5LEN);
      radius_addattr (radius, &radius_pack, RADIUS_ATTR_CHAP_PASSWORD, 0, 0,
		      0, chap_password, REDIR_MD5LEN + 1);
    }

  radius_addattr (radius, &radius_pack, RADIUS_ATTR_NAS_IP_ADDRESS, 0, 0, ntohl (conn->nasip.s_addr), NULL, 0);	/* WISPr_V1.0 */
  radius_addattr (radius, &radius_pack, RADIUS_ATTR_SERVICE_TYPE, 0, 0, RADIUS_SERVICE_TYPE_LOGIN, NULL, 0);	/* WISPr_V1.0 */
  radius_addattr (radius, &radius_pack, RADIUS_ATTR_FRAMED_IP_ADDRESS, 0, 0, ntohl (conn->hisip.s_addr), NULL, 0);	/* WISPr_V1.0 */
  ...
  radius_addattr (radius, &radius_pack, RADIUS_ATTR_CALLING_STATION_ID, 0, 0,
		  0, (uint8_t *) mac, REDIR_MACSTRLEN);
  ...
  radius_addattr (radius, &radius_pack, RADIUS_ATTR_CALLED_STATION_ID, 0, 0, 0, (uint8_t *) mac, REDIR_MACSTRLEN);	/* WISPr_V1.0 */

  if (redir->radiusnasid)
    radius_addattr (radius, &radius_pack, RADIUS_ATTR_NAS_IDENTIFIER, 0, 0, 0, (uint8_t *) redir->radiusnasid, strlen (redir->radiusnasid));	/* WISPr_V1.0 */
...
int
redir_accept (struct redir_t *redir)
{
  ...
  if (optionsdebug)
    printf ("Calling redir_getreq()\n");

  if (redir_getreq (redir, new_socket, &conn))
    {
      if (optionsdebug)
	printf ("Error calling get_req. Terminating\n");
      exit (0);
    }

  termstate = REDIR_TERM_GETSTATE;
  if (optionsdebug)
    printf ("Calling cb_getstate()\n");
  ...
  if (optionsdebug)
    printf ("Processing received request\n");
...
/*JAC-KYN */
	...
	int n;
/*
      printf("type:     %d\n",conn.type); // REDIR_LOGOUT, LOGIN, PRELOGIN, CHALLENGE, MSDOWNLOAD 
      printf("username: %s\n",conn.username);
      for(n=0;n<20;n++) printf(" %x", conn.username[n]);
      printf("\n");
      printf("userurl: %s\n",conn.userurl);
      for(n=0;n<20;n++) printf(" %x", conn.userurl[n]);
      printf("\n");
      printf("chap:     %d\n",conn.chap);  // 0 if using normal password; 1 if using CHAP 
      
      printf("chappwd:  %s\n",(char *)conn.chappassword);
      for(n=0;n<20;n++) printf(" %x", conn.chappassword[n]);
      printf("\n");

      printf("pwd:      %s\n",(char *)conn.password);
      for(n=0;n<20;n++) printf(" %x", conn.password[n]);
      printf("\n");
  
      printf("uamchal:  %s\n",(char *)conn.uamchal);

      printf("uamtime:  %d\n",conn.uamtime);
      printf("authentd: %d\n",conn.authenticated); // 1 if user was authenticated 
...
/*endJAC-KYN*/
	  ...
	  if (optionsdebug)
	    printf ("Calling radius\n");

	  if (optionsdebug)
	    printf ("redir_accept: Sending redius request\n");
  ...
  if (optionsdebug)
    printf ("redir_accept: Original request\n");
...
int
redir_set_cb_getstate (struct redir_t *redir,
