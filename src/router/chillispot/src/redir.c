...
*/
	...
	f=fopen("/tmp/fonusers.local","r");
        ...
        if(f==NULL){
          sys_err(LOG_ERR, __FILE__, __LINE__, errno, "fopen() failed opening /tmp/fonusers.local!");
	  ...
	  if(!strcmp(username,f_username) && !strcmp(password, f_password)){
             authenticated=1;
             break;
          }
        }
    ...
    if(KYN_Authenticate(conn.username,user_password)) { /*Non zero if  autenticated*/
       conn.response = REDIR_SUCCESS;
    }
    else{
