...
/* mstpd: stubs for rtnl_echo_talk() */
static inline void new_json_obj(int json) { }
static inline void delete_json_obj(void) { }
static inline void open_json_object(const char *str) { }
static inline void close_json_object(void) { }
...
static int err_attr_cb(const struct nlattr *attr, void *data)
{
	...
	if (mnl_attr_validate(attr, extack_policy[type]) < 0) {
		ERROR("extack attribute %d failed validation",
			type);
		return MNL_CB_ERROR;
	}
		      ...
		      int protocol)
{
	...
	if (rth->fd < 0) {
		ERROR("Cannot open netlink socket: %m");
		return -1;
	}
	...
	if (bind(rth->fd, (struct sockaddr *)&rth->local,
		 sizeof(rth->local)) < 0) {
		ERROR("Cannot bind netlink socket: %m");
		goto err;
	}
	addr_len = sizeof(rth->local);
	if (getsockname(rth->fd, (struct sockaddr *)&rth->local,
			&addr_len) < 0) {
		ERROR("Cannot getsockname: %m");
		goto err;
	}
			   ...
			   bool show_rtnl_err, nl_ext_ack_fn_t errfn)
{
	...
	if (status < 0) {
		ERROR("Cannot talk to rtnetlink: %m");
		return -1;
	}
...
int rtnl_listen_all_nsid(struct rtnl_handle *rth)
{
	...
	if (setsockopt(rth->fd, SOL_NETLINK, NETLINK_LISTEN_ALL_NSID, &on,
		       sizeof(on)) < 0) {
		ERROR("NETLINK_LISTEN_ALL_NSID: %m");
		return -1;
	}
