...
static bool ppe_drv_v4_bind_acl_policer(struct ppe_drv_v4_rule_create *create, struct ppe_drv_v4_conn *cn)
{
	...
	case PPE_DRV_RULE_TYPE_FLOW_POLICER:
		...
		if (ap_rule->rule_id.policer.flags & PPE_DRV_VALID_FLAG_FLOW_POLICER) {
			...
			ppe_drv_v4_conn_flow_flags_set(&cn->pcf, PPE_DRV_V4_CONN_FLAG_FLOW_POLICER_VALID);
			ppe_drv_trace("%p: using sc: %d for rule_id: %d and hw_id: %d", p, policer_info.sc, policer_info.id, cn->pcf.policer_hw_id);
		}
		...
		if (ap_rule->rule_id.policer.flags & PPE_DRV_VALID_FLAG_RETURN_POLICER) {
			...
			ppe_drv_v4_conn_flow_flags_set(&cn->pcr, PPE_DRV_V4_CONN_FLAG_FLOW_POLICER_VALID);
			ppe_drv_trace("%p: using sc: %d for rule_id: %d and hw_id: %d", p, policer_info.sc, policer_info.id, cn->pcr.policer_hw_id);
		}
	break;
	}
...
static bool ppe_drv_v4_unbind_acl_policer(struct ppe_drv_v4_conn *cn)
{
	...
	if (ppe_drv_v4_conn_flow_flags_check(&cn->pcf, PPE_DRV_V4_CONN_FLAG_FLOW_POLICER_VALID)) {
		...
		cn->pcf.acl_sc = 0;
		ppe_drv_v4_conn_flow_flags_clear(&cn->pcf, PPE_DRV_V4_CONN_FLAG_FLOW_POLICER_VALID);
		ppe_drv_trace("%p: unlinking flow from POLICER rule_id: %d", p, policer_info.id);
	}

	if (ppe_drv_v4_conn_flow_flags_check(&cn->pcr, PPE_DRV_V4_CONN_FLAG_FLOW_POLICER_VALID)) {
		...
		cn->pcr.acl_sc = 0;
		ppe_drv_v4_conn_flow_flags_clear(&cn->pcr, PPE_DRV_V4_CONN_FLAG_FLOW_POLICER_VALID);
		ppe_drv_trace("%p: Unlinking flow from POLICER rule_id: %d", p, policer_info.id);
	}
				       ...
				       struct ppe_drv_v4_conn *cn, enum ppe_drv_conn_type flow_type)
{
	...
	/*
	 * Check if the PPE offload is disabled on the physical Rx port
	 */
	if ((pp_tx->user_type == PPE_DRV_PORT_USER_TYPE_ACTIVE_VP) ||
		...
		if (!ppe_drv_port_check_flow_offload_enabled(pp_rx)) {
			...
			ppe_drv_v4_conn_flow_flags_set(pcf,
...
/*
 * ppe_drv_v4_policer_conn_fill()
 *	Populate single direction flow object rule.
 */
ppe_drv_ret_t ppe_drv_v4_policer_conn_fill(struct ppe_drv_v4_rule_create *create, struct ppe_drv_top_if_rule *top_if,
				       struct ppe_drv_v4_conn *cn, enum ppe_drv_conn_type flow_type)
{
	struct ppe_drv_v4_connection_rule *conn = &create->conn_rule;
	struct ppe_drv_v4_5tuple *tuple = &create->tuple;
...
/*
 * ppe_drv_v4_priority_conn_fill()
 * 	Populate single direction flow object rule.
 */
ppe_drv_ret_t ppe_drv_v4_priority_conn_fill(struct ppe_drv_v4_rule_create *create, struct ppe_drv_v4_conn *cn,
						enum ppe_drv_conn_type flow_type)
{
	...
	ppe_drv_v4_conn_flow_match_src_ip_set(pcf, tuple->flow_ip);
	ppe_drv_v4_conn_flow_match_src_ident_set(pcf, tuple->flow_ident);
	ppe_drv_v4_conn_flow_xlate_src_ip_set(pcf, tuple->flow_ip);
	...
	ppe_drv_v4_conn_flow_int_pri_set(pcf, create->qos_rule.flow_qos_tag);
	ppe_drv_v4_conn_flow_flags_set(pcf, PPE_DRV_V4_CONN_FLOW_FLAG_QOS_VALID);
	ppe_drv_v4_conn_flow_flags_set(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_PRIORITY_PPE_ASSIST);
 ...
 */
static inline void ppe_drv_v4_conn_flow_metadata_set(struct ppe_drv_v4_conn_flow *pcf, struct ppe_drv_flow_cookie_metadata *fc_metadata, ppe_drv_tree_id_type_t tree_id_type)
{
	switch (tree_id_type) {
				   ...
				   enum ppe_drv_conn_type flow_type)
{
	...
	if (rule_flags & PPE_DRV_V4_RULE_FLAG_FLOW_VALID) {
		...
		if (valid_flags & PPE_DRV_V4_VALID_FLAG_QOS) {
			...
			ppe_drv_v4_conn_flow_flags_set(pcf, PPE_DRV_V4_CONN_FLOW_FLAG_QOS_VALID);
                 ...
                 */
		if ((valid_flags & PPE_DRV_V4_VALID_FLAG_WIFI_TID) &&
				(ppe_drv_port_flags_check(pp_tx, PPE_DRV_PORT_FLAG_WIFI_DEV))) {
			fc_metadata.type.mark = qos_rule->flow_qos_tag;
			ppe_drv_v4_conn_flow_metadata_set(pcf, &fc_metadata, PPE_DRV_TREE_ID_TYPE_WIFI_TID);
		...
		/*
		 * Check if Legacy SCS info is valid in this direction and if the
		 * interface is a wifi VP.
		 */
		if ((valid_flags & PPE_DRV_V4_VALID_FLAG_SCS) &&
					(ppe_drv_port_flags_check(pp_tx, PPE_DRV_PORT_FLAG_WIFI_DEV))) {
			memset(&fc_metadata, 0, sizeof(fc_metadata));
			fc_metadata.type.scs.scs_mark = sawf_rule->flow_mark;
			ppe_drv_v4_conn_flow_metadata_set(pcf, &fc_metadata, PPE_DRV_TREE_ID_TYPE_SCS);
			ppe_drv_v4_conn_flow_flags_set(pcf, PPE_DRV_V4_CONN_FLOW_METADATA_TYPE_WIFI_INFO);
		}
		...
		if ((rule_flags & PPE_DRV_V4_RULE_FLAG_BRIDGE_FLOW) && !is_wanif) {
			if (!ppe_drv_iface_parent_get(top_if_rx) || !ppe_drv_iface_parent_get(top_if_tx)) {
				...
				ppe_drv_trace("%p: one of top's parent interface is null: top_rx_if : %d tx_if: %d",
						create, top_rule->rx_if, top_rule->tx_if);
				ppe_drv_trace("top_if_rx_dev: %s top_if_tx_dev: %s top_if_tx_index: %d top_if_rx_index: %d\n", top_if_rx->dev->name,
						top_if_tx->dev->name, top_if_rx->index, top_if_tx->index);
	...
	if (rule_flags & PPE_DRV_V4_RULE_FLAG_RETURN_VALID) {
		...
		if ((valid_flags & PPE_DRV_V4_VALID_FLAG_WIFI_TID) &&
			...
			ppe_drv_v4_conn_flow_metadata_set(pcr, &fc_metadata, PPE_DRV_TREE_ID_TYPE_WIFI_TID);
		...
		if ((rule_flags & PPE_DRV_V4_RULE_FLAG_BRIDGE_FLOW) && !is_wanif) {
			...
			if (ppe_drv_v4_conn_flow_ingress_vlan_cnt_get(pcr) || ppe_drv_v4_conn_flow_egress_vlan_cnt_get(pcr)) {
				if (ppe_drv_iface_parent_get(top_if_rx) != ppe_drv_iface_parent_get(top_if_tx)) {
					ppe_drv_stats_inc(&comm_stats->v4_create_fail_vlan_filter);
					ppe_drv_warn("%p: IF not part of same bridge rx_if: %d tx_if: %d",
							create, top_rule->rx_if, top_rule->tx_if);
...
static struct ppe_drv_flow *ppe_drv_v4_flow_add(struct ppe_drv_v4_conn_flow *pcf)
{
	 ...
	 */
	if (!(ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLOW_FLAG_BRIDGE_FLOW) ||
				ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_RFS_PPE_ASSIST) ||
				ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_PRIORITY_PPE_ASSIST))) {
 ...
 * This function is deprecated, Please use "ppe_drv_v4_assist_rule_destroy()"
 * API for destroy of RFS flows
 */
...
ppe_drv_ret_t ppe_drv_v4_assist_rule_destroy(struct ppe_drv_v4_rule_destroy *destroy)
{
	 ...
	 * Update stats
	 */
	ppe_drv_stats_inc(&comm_stats->v4_assist_rule_destroy_req);
	...
	if (!flow) {
		spin_unlock_bh(&p->lock);
		ppe_drv_stats_inc(&comm_stats->v4_assist_rule_destroy_conn_not_found);
		ppe_drv_warn("%p: flow entry not found", p);
		return PPE_DRV_RET_FAILURE_DESTROY_NO_CONN;
	}
	...
	if (!ppe_drv_v4_flow_del(pcf)) {
		spin_unlock_bh(&p->lock);
		ppe_drv_stats_inc(&comm_stats->v4_assist_rule_destroy_fail);
		ppe_drv_warn("%p: deletion of flow failed: %p", p, pcf);
		return PPE_DRV_RET_FAILURE_DESTROY_FAIL;
	}
...
EXPORT_SYMBOL(ppe_drv_v4_assist_rule_destroy);

/*
 * ppe_drv_v4_policer_flow_destroy()
...
EXPORT_SYMBOL(ppe_drv_v4_policer_flow_destroy);
...
/*
 * ppe_drv_v4_policer_flow_create()
...
ppe_drv_ret_t ppe_drv_v4_policer_flow_create(struct ppe_drv_v4_rule_create *create)
{
	...
	if (create->ap_rule.rule_id.policer.flags & PPE_DRV_VALID_FLAG_RETURN_POLICER) {
		pcr = &cn->pcr;
		ppe_drv_v4_conn_flow_flags_set(pcr, PPE_DRV_V4_CONN_FLAG_FLOW_PPE_POLICER_ASSIST);
...
EXPORT_SYMBOL(ppe_drv_v4_policer_flow_create);

/*
 * ppe_drv_v4_rfs_create()
 *      Adds a connection entry in PPE.
 *
 * This function is deprecated, Please use "ppe_drv_v4_assist_rule_create()"
...
ppe_drv_ret_t ppe_drv_v4_rfs_create(struct ppe_drv_v4_rule_create *create)
{
	...
	pcf = &cn->pcf;
	ppe_drv_v4_conn_flow_flags_set(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_RFS_PPE_ASSIST);
...
ppe_drv_ret_t ppe_drv_v4_assist_rule_create(struct ppe_drv_v4_rule_create *create, uint32_t feature)
{
	...
	cn = ppe_drv_v4_conn_alloc();
	if (!cn) {
		ppe_drv_stats_inc(&comm_stats->v4_assist_rule_create_fail_mem);
		ppe_drv_warn("%p: failed to allocate connection memory: %p", p, create);
		return PPE_DRV_RET_FAILURE_CREATE_OOM;
	}
	...
	if (ppe_drv_assist_feature_type_check(feature, PPE_DRV_ASSIST_FEATURE_RFS)) {
		...
		ppe_drv_stats_inc(&comm_stats->v4_create_priority_req);
		ret = ppe_drv_v4_priority_conn_fill(create, cn, PPE_DRV_CONN_TYPE_FLOW);
		if (ret != PPE_DRV_RET_SUCCESS) {
			ppe_drv_stats_inc(&comm_stats->v4_assist_rule_create_priority_fail_conn);
			ppe_drv_warn("%p: failed to fill connection object: %p", p, create);
			goto fail;
		}
	}
	 ...
	 */
	if (ppe_drv_v4_flow_check(&cn->pcf)) {
		ppe_drv_stats_inc(&comm_stats->v4_assist_rule_create_fail_collision);
		ppe_drv_warn("%p: create collision detected: %p", p, create);
		ret = PPE_DRV_RET_FAILURE_CREATE_COLLISSION;
		goto fail;
	}
	...
	pcf->pf = ppe_drv_v4_flow_add(pcf);
	if (!pcf->pf) {
		ppe_drv_stats_inc(&comm_stats->v4_assist_rule_create_fail);
		ppe_drv_warn("%p: acceleration of flow failed: %p", p, pcf);
		ret = PPE_DRV_RET_FAILURE_FLOW_ADD_FAIL;
		goto fail;
	}
...
fail:
	...
	if (ppe_drv_assist_feature_type_check(feature, PPE_DRV_ASSIST_FEATURE_RFS)) {
		ppe_drv_v4_conn_flow_flags_clear(&cn->pcf, PPE_DRV_V4_CONN_FLAG_FLOW_RFS_PPE_ASSIST);
	} else if (ppe_drv_assist_feature_type_check(feature, PPE_DRV_ASSIST_FEATURE_PRIORITY)) {
		ppe_drv_v4_conn_flow_flags_clear(&cn->pcf, PPE_DRV_V4_CONN_FLAG_FLOW_PRIORITY_PPE_ASSIST);
	}
...
EXPORT_SYMBOL(ppe_drv_v4_assist_rule_create);
					...
					struct ppe_drv_v4_conn_flow *pcr)
{
	...
	if (!inter_vap) {
		...
		if (!fse_cn) {
			ppe_drv_trace("Failed to get v4 FSE connection\n");
			return false;
		}
