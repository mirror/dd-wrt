  ...
  do
    ...
    then
      if [ -z "$(nvram get transparent_proxy)" ]; then
        $IPTABLES -t nat -A PREROUTING -i br0 -s $ROUTER_IP/$ROUTER_NETMASK \
          -d $ROUTER_IP/$ROUTER_NETMASK -p tcp --dport 80 -j ACCEPT
        $IPTABLES -t nat -A PREROUTING -i br0 ! -s $PROXY_SERVER -p tcp --dport 80 \
          -j DNAT --to $PROXY_SERVER:$PROXY_PORT
        $IPTABLES -t nat -A POSTROUTING -o br0 -s $ROUTER_IP/$ROUTER_NETMASK -p tcp -d \
          $PROXY_SERVER -j SNAT --to $ROUTER_IP
        $IPTABLES -t filter -I FORWARD -s $ROUTER_IP/$ROUTER_NETMASK -d $PROXY_SERVER -i br0 \
      ...
      if [ "$(/usr/sbin/nvram get transparent_proxy)" = "1" ]; then
        $IPTABLES -t nat -D PREROUTING -i br0 -s $ROUTER_IP/$ROUTER_NETMASK \
          -d $ROUTER_IP/$ROUTER_NETMASK -p tcp --dport 80 -j ACCEPT
        $IPTABLES -t nat -D PREROUTING -i br0 ! -s $PROXY_SERVER -p tcp --dport 80 \
          -j DNAT --to $PROXY_SERVER:$PROXY_PORT
        $IPTABLES -t nat -D POSTROUTING -o br0 -s $ROUTER_IP/$ROUTER_NETMASK -p tcp -d \
          $PROXY_SERVER -j SNAT --to $ROUTER_IP
        $IPTABLES -t filter -D FORWARD -s $ROUTER_IP/$ROUTER_NETMASK -d $PROXY_SERVER -i br0 \
          -o br0 -p tcp --dport $PROXY_PORT -j ACCEPT
