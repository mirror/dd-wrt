...
{
    (uint8_t *) "II*|00|", /* pattern (now in snort content format) */
    4, /* depth */
    0, /* offset */
    CONTENT_BUF_NORMALIZED, /* flags */
...
{
    (uint8_t *) "MM|00|*", /* pattern (now in snort content format) */
    4, /* depth */
    0, /* offset */
    CONTENT_BUF_NORMALIZED, /* flags */
    NULL, /* holder for boyer/moore PTR */
...
Rule rule15975 = {
   ...
   { 
       ...
       "FILE-IMAGE OpenOffice TIFF file in little endian format parsing integer overflow attempt",     /* message */
...
Rule rule15976 = {
   ...
   { 
       ...
       "attempted-user", /* classification */
       0,  /* hardcoded priority XXX NOT PROVIDED BY GRAMMAR YET! */
       "FILE-IMAGE OpenOffice TIFF file in big endian format parsing integer overflow attempt",     /* message */
       ruleCVE_2007_2834refs /* ptr to references */
       ,ruleCVE_2007_2834metadata
   },
...
int ruleCVE_2007_2834eval(void *p) {
    ...
    const uint8_t *cursor_start;          // pointer to the beginning of the TIFF file
    const uint8_t *cursor_IFD_table;      // pointer to the current IFD table 
    const uint8_t *cursor_IFD_entry;      // pointer to IFD entry to inspect
    const uint8_t *cursor_temp;
    uint32_t       offset_IFD_table;      // offset to the next IFD table    
    uint16_t       num_IFD_entry;         // number of IFD entries in a table
    uint16_t       i;
    uint32_t       data_count;            // the size of buffer = data_count * 4. if data_count >= 0x400000, malicious packet    
    uint16_t       tag_ID;
    
    uint32_t (* fn_read32)(const uint8_t *p);
