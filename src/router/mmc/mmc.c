...
MODULE_AUTHOR("Madsuk/Rohde (speedup Cyril CATTIAUX v1.2,GPIO detection BrainSlayer)");
...
static  inline void mmc_spi_io_v(unsigned char data_out) {
    unsigned char di;
...
static int mmc_card_init(void)
{
    ...
    for (j = 0; j < 10000; j++)
    {
        ...
        /* CMD1 - init */
        mmc_spi_io(0x41);
        for (i = 0; i < 4; i++) mmc_spi_io(0x00); /* dummy params */
