...
#if defined(AVM_VERY_SMALL) || defined(AVM_WITHOUT_ERROR_CODES)

#ifdef AVM_VERY_SMALL_LOGFILE
	#define AVM_VERY_SMALL_LOG { FILE* logfile = fopen("/var/log/smb_very_small_log", "a"); \
		if(!logfile) return; fprintf(logfile, "%s: %i\n", __FILE__, __LINE__); fclose(logfile); }
#else
	#define AVM_VERY_SMALL_LOG
#endif
