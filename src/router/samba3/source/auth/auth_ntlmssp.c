...
NTSTATUS auth_ntlmssp_start(AUTH_NTLMSSP_STATE **auth_ntlmssp_state) { AVM_VERY_SMALL_LOG return NT_STATUS_OK; }
void auth_ntlmssp_end(AUTH_NTLMSSP_STATE **auth_ntlmssp_state) {AVM_VERY_SMALL_LOG  }
NTSTATUS auth_ntlmssp_update(AUTH_NTLMSSP_STATE *auth_ntlmssp_state,
		const DATA_BLOB request, DATA_BLOB *reply) { AVM_VERY_SMALL_LOG return NT_STATUS_OK; }
#else
...
#ifdef AVM_FRITZBOX // AVM for TCOM
	// NOTE that we only user mapping for FRITZBOX
	if (username_was_mapped && user_info->smb_name && *user_info->smb_name) {
Log(("AVM for TCOM %s password check  nt_status=0x%x smb_name=%s", __FUNCTION__, nt_status, user_info->smb_name));
		if (NT_STATUS_IS_OK(nt_status)) {
			if (!LoginControlIsAllowed(user_info->smb_name)) {
				nt_status = NT_STATUS_ACCESS_DENIED;
			} else {
				LoginControlPasswordOk(user_info->smb_name, 0);
			}
		} else {
			LoginControlWrongPassword(user_info->smb_name, 0);
		}
	}
#endif
