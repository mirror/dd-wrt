...
#ifdef AVM_FIX // AVM_USERNAME_MAP_FIX
again:
DEBUGC(DBGC_CLASS, 0, ("%s smb_name=%s encrypted=%d\n", __FUNCTION__, smb_name, encrypted));
#endif
...
#ifdef AVM_FIX // AVM_USERNAME_MAP_FIX
	if (get_session_client_username() && smb_name != get_session_client_username()) {
		smb_name = get_session_client_username();
DEBUGC(DBGC_CLASS, 0, ("%s retry with smb_name=%s\n", __FUNCTION__, smb_name));
		goto again;
	}
#endif
