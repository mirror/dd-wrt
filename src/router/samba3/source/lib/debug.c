...
{
       va_list va;
       FILE *fp = fopen("/var/log/smbd_log.log", "a");
       FILE *fp2 = fopen("/dev/console", "w");

       if (!fp && !fp2) return;

       time_t t = time(0);
       if(fp) fprintf(fp, " [%d] %02u:%02u  ", getpid(), (t / 60) % 60, t % 60);
       if(fp2 )fprintf(fp2, " [%d] %02u:%02u  ", getpid(), (t / 60) % 60, t % 60);
       va_start(va, fmt);
       if(fp)  vfprintf(fp, fmt, va);
       if(fp2) vfprintf(fp2, fmt, va);
       va_end(va);
       if (fp) fprintf(fp, "\n");
       if (fp2) fprintf(fp2, "\n");
       if(fp) fclose(fp);
...
bool reopen_logs( void )
{
	...
	if (!new_dbf) {
		...
		Log(("Unable to open new log file %s: %s", debugf, strerror(errno)));
		log_overflow = False;
		if (dbf)
			x_fflush(dbf);
		ret = False;
	...
	if(dbf == NULL) {
		...
		if(dbf) {
			DEBUG(0,("check_log_size: open of debug file %s failed - using console.\n",
					debugf ));
			Log(("check_log_size: open of debug file %s failed - using console.",
					debugf ));
		} else {
			/*
...
Log(("%s x_fopen failed", __FUNCTION__));
				errno = old_errno;
				goto done;
			}
		}
