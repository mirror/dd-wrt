...
#ifdef AVM_FRITZBOX
/* Get the free and total space of the partition with path
 * plus all other partitions mounted below this partition
 *
 * results are returned in *dfree and *dsize, in 512 byte units
 */
int sys_avm_fsusage(const char *path, SMB_BIG_UINT *dfree, SMB_BIG_UINT *dsize)
{
	SMB_BIG_UINT free;
	SMB_BIG_UINT size;
	int ret;
	...
	*dfree = *dsize = 0;
	...
	if (fp) {
		char real[PATH_MAX];
		char buf[2048];
		if (0 != realpath(path , real)) {
			int real_len = strlen(real);
			while(fgets(buf, sizeof(buf), fp)) {
				char *part = buf;
				while(*part != ' ' && *part != '\0') part++;
				if (*part != ' ') continue;
				part++;
				char *p = part;
				while(*p != ' ' && *p != '\0') p++;
				if (*p != ' ') continue;
				*p = '\0';
				// check if part is sub of real
				int part_len = p - part;
				if (part_len <= real_len) continue;
				if (memcmp(real, part, real_len)) continue;
				if (part[real_len] != '/') continue;
				ret = sys_fsusage(part, &free, &size);
				if (0 == ret) {
					*dfree += free;
					*dsize += size;
				}
	...
	ret = sys_fsusage(path, &free, &size);
	if (0 == ret) {
		*dfree += free;
		*dsize += size;
	}
