...
DATA_BLOB spnego_gen_auth_response(DATA_BLOB *reply, NTSTATUS nt_status,
				   const char *mechOID) { AVM_VERY_SMALL_LOG DATA_BLOB a; return a; }
bool spnego_parse_auth(DATA_BLOB blob, DATA_BLOB *auth) { AVM_VERY_SMALL_LOG return True; }
bool parse_negTokenTarg(DATA_BLOB blob, char *OIDs[ASN1_MAX_OIDS], DATA_BLOB *secblob) {AVM_VERY_SMALL_LOG  return True; }
DATA_BLOB spnego_gen_negTokenInit(char guid[16], 
				  const char *OIDs[], 
				  const char *principal) { AVM_VERY_SMALL_LOG DATA_BLOB a; return a; }; 
#else
