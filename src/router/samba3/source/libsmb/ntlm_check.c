				 ...
				 DATA_BLOB *user_sess_key)
{
	...
	DEBUG(0,("Part password (P16) was |\n"));
	dump_data(0, part_passwd, 16);
	DEBUGADD(0,("Password from client was |\n"));
	dump_data(0, nt_response->data, nt_response->length);
	DEBUGADD(0,("Given challenge was |\n"));
	dump_data(0, sec_blob->data, sec_blob->length);
	DEBUGADD(0,("Value from encryption was |\n"));
	dump_data(0, p24, 24);
#endif
				 ...
				 DATA_BLOB *user_sess_key)
{
	...
	*/
	...
	dump_data(0, part_passwd, 16);
	DEBUGADD(0,("Password from client was |\n"));
	dump_data(0, ntv2_response->data, ntv2_response->length);
	DEBUGADD(0,("Variable data from client was |\n"));
	dump_data(0, client_key_data.data, client_key_data.length);
	DEBUGADD(0,("Given challenge was |\n"));
	dump_data(0, sec_blob->data, sec_blob->length);
	DEBUGADD(0,("Value from encryption was |\n"));
	dump_data(0, value_from_encryption, 16);
#endif
...
#ifdef AVM_FRITZBOX
if (res) {
	DEBUG(0, ("*** NTLMv2 Auth OK ****\n"));
} else {
	DEBUG(0, ("!!! NTLMv2 Auth FAILED !!!\n"));
}
#endif
	return res;
}
