...
bool unmap_username(fstring user)
{
	XFILE *f;
	char *mapfile = lp_username_map();
	char *s;
	...
	f = x_fopen(mapfile,O_RDONLY, 0);
	if (!f) {
		DEBUG(4,("can't open username map %s. Error %s\n", mapfile, strerror(errno) ));
		return False;
	}
	...
	while((s=fgets_slash(buf,sizeof(buf),f))!=NULL) {
		...
		*dosname++ = 0;
		...
		if (strcmp(unixname, user)) continue;
		...
		if (*dosuserlist) {
DEBUG(4,("unmap_username %s -> %s\n", user, *dosuserlist));
			fstrcpy(user, *dosuserlist); // NOTE: no @ + ! * etc. supported here!
			TALLOC_FREE(dosuserlist);
			x_fclose(f);
			return True;
		}
	...
	x_fclose(f);
DEBUG(4,("unmap_username not found for %s\n", user));
	return False;
}
#endif // AVM for TCOM
