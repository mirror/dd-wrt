...
/*
 * 24.9.09
 * We have to remember the real username from the client to use
 * with NTLMv2 if we use security=SHARE and "username map" option
 */
static char *session_username = NULL;
#endif
...
#ifdef AVM_FIX // AVM_USERNAME_MAP_FIX
/****************************************************************************
 In security=share mode we need to store the client username, as that's
 what Vista/Windows7 uses for the NTLMv2 calculation.
 The name would get lost if uername map is used.
****************************************************************************/
void add_session_client_username(const char *username)
{
	if (session_username) {
		SAFE_FREE(session_username);
	}
	session_username= smb_xstrdup(username);
}

/****************************************************************************
 In security=share mode we need to return the client username, as that's
 what Vista/Windows7 uses for the NTLMv2 calculation.
****************************************************************************/
const char *get_session_client_username(void)
{
	...
	if (!ok) {
		...
		for (auser = strtok_r(user_list, LIST_SEP, &saveptr);
			...
			if (!user_ok(user2,snum))
				continue;
#ifdef AVM_FRITZBOX // AVM for TCOM
			if (*my_user == '\0') fstrcpy(my_user, user2);
#endif
...
#ifdef AVM_FRITZBOX // AVM for TCOM
		if (*my_user == '\0') fstrcpy(my_user, guestname);
#endif
...
#ifdef AVM_FRITZBOX // AVM for TCOM
	if (!ok && *my_user) fstrcpy(user, my_user);
#endif
