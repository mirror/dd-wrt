		...
		bool guest;
                ...
                if (!authorise_login(snum,user,password,&guest)) {
                        DEBUG( 2, ( "Invalid username/password for [%s]\n",
                                    lp_servicename(snum)) );
#ifdef AVM_FRITZBOX // AVM for TCOM
			if (!is_ipc && *user && unmap_username(user)) {
				if (!LoginControlIsAllowed(user)) {
					return NT_STATUS_ACCESS_DENIED;
				} else {
#if 0 // we cant do this cause we dont know the real username in case of wrong password!
					if (*user) LoginControlWrongPassword(user, 0);
#endif
				}
			}
...
// TODO !!
			...
			fstring display_user;
			fstrcpy(display_user, user);
			if (*display_user && unmap_username(display_user)) {
				if (!LoginControlIsAllowed(display_user)) {
					//blockiert
					return NT_STATUS_ACCESS_DENIED;
				}
				LoginControlPasswordOk(display_user, 0);
			}
		}
#endif
...
#ifdef AVM_FRITZBOX // AVM for TCOM
	if (!IS_IPC(conn) && conn->server_info && conn->server_info->unix_name && *conn->server_info->unix_name) {
		fstring display_user;
		fstrcpy(display_user, conn->server_info->unix_name);
		if (unmap_username(display_user)) LoginControlLogout(display_user, 0);
	}
#endif
