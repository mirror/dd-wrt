...
echo "	linux /boot/vmlinuz root=/dev/hda2 nohz_full=all pci=pcie_bus_perf rootfstype=ext4 noinitrd video=vga16fb:off nofb console=ttyS0,115200n8 initcall_blacklist=acpi_cpufreq_init rootdelay=5" >> tmp/boot/grub/grub.cfg
...
echo "}" >> tmp/boot/grub/grub.cfg
PADDING="1" SIGNATURE="$IMG_PART_SIGNATURE" GUID="$IMG_PART_DISKGUID" ./scripts/gen_image_generic.sh efi.img 32 tmp 2147483648 ../x86_64-uclibc/root.fs 256 128
...
PADDING="1" SIGNATURE="$IMG_PART_SIGNATURE" GUID="$IMG_PART_DISKGUID" ./scripts/gen_image_generic.sh efi-big.img 32 tmp 2147483648 ../x86_64-uclibc/root.fs 256 1780
...
echo "	linux /boot/vmlinuz root=/dev/hda2 nohz_full=all pci=pcie_bus_perf rootfstype=ext4 noinitrd fbcon=nodefer vga=0x305 video=efifb:1024x768x32 initcall_blacklist=acpi_cpufreq_init rootdelay=5" >> tmp/boot/grub/grub.cfg
echo "}" >> tmp/boot/grub/grub.cfg
...
PADDING="1" SIGNATURE="$IMG_PART_SIGNATURE" GUID="$IMG_PART_DISKGUID" ./scripts/gen_image_generic.sh efi-vga.img 32 tmp 400 ../x86_64-uclibc/root.fs 256 128
rm -rf imp
mkdir -p img
cp boot/grub/boot.img img
cp boot/grub/core.img img
...
PADDING="1" SIGNATURE="$IMG_PART_SIGNATURE" GUID="$IMG_PART_DISKGUID" ./scripts/gen_image_generic.sh efi-vga-big.img 32 tmp 400 ../x86_64-uclibc/root.fs 256 1780
rm -rf imp
mkdir -p img
cp boot/grub/boot.img img
cp boot/grub/core.img img
