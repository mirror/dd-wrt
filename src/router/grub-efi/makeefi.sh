...
echo "	linux /boot/vmlinuz root=/dev/hda2 nohz_full=all rootfstype=squashfs noinitrd video=vga16fb:off nofb console=ttyS0,115200n8 initcall_blacklist=acpi_cpufreq_init rootdelay=5" >> tmp/boot/grub/grub.cfg
...
echo "	linux /boot/vmlinuz root=/dev/hda2 nohz_full=all rootfstype=squashfs noinitrd fbcon=nodefer vga=0x305 video=efifb:1024x768x32 initcall_blacklist=acpi_cpufreq_init rootdelay=5" >> tmp/boot/grub/grub.cfg
echo "}" >> tmp/boot/grub/grub.cfg
