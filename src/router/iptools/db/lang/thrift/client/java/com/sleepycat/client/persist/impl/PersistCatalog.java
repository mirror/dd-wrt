...
import com.sleepycat.client.persist.model.ModelInternal;
...
public class PersistCatalog implements Catalog {
     ...
     * database.
     */
    public PersistCatalog(final SEnvironment env,
                          final String storePrefix,
                          final String dbName,
                          final SDatabaseConfig dbConfig,
    ...
    private void init(final STransaction txn,
                      final String storePrefix,
                      final EntityModel modelParam,
                      final Mutations mutationsParam)
        throws SDatabaseException {
     ...
     * auto-commit.
     */
    public synchronized void flush(STransaction txn)
        throws SDatabaseException {
     ...
     * never returned.
     */
    private Data readData(STransaction txn)
        throws SDatabaseException {

        Data oldData;
        SDatabaseEntry key = new SDatabaseEntry(DATA_KEY);
        SDatabaseEntry data = new SDatabaseEntry();
        SOperationStatus status = db.get(txn, key, data, null);
     ...
     */
    boolean isMetadataStale(STransaction txn)
        throws SDatabaseException {
     ...
     */
    private void writeData(STransaction txn, Data newData)
        throws SDatabaseException {
        ...
        SDatabaseEntry key = new SDatabaseEntry(DATA_KEY);
        SDatabaseEntry data = new SDatabaseEntry(baos.toByteArray());
        db.put(txn, key, data);
