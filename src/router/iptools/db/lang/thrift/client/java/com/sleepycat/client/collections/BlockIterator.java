...
class BlockIterator<E> extends BaseIterator<E> {
    ...
    private void makeDataObject() {
        ...
        int i = dataIndex;
        SDatabaseEntry keyEntry = new SDatabaseEntry(keys[i]);
        SDatabaseEntry priKeyEntry = (keys != priKeys)
                                    ? (new SDatabaseEntry(priKeys[i]))
                                    : keyEntry;
        SDatabaseEntry valuesEntry = new SDatabaseEntry(values[i]);
        ...
        try {
            ...
            if (found) {
                ...
                while (!done) {
                    ...
                    i += 1;
                    if (i < keys.length) {
                        SOperationStatus status = coll.iterateDuplicates() ?
    ...
    public void add(E value) {
        ...
        try {
            ...
            if (status == SOperationStatus.KEYEXIST) {
                throw new IllegalArgumentException("Duplicate value");
            } else if (status != SOperationStatus.SUCCESS) {
