...
#
# TEST	test152
# TEST	Test database compaction with blobs.  
# TEST  This is basically test111 with blobs. 
# TEST
# TEST	Populate a database with a mixture of blob and non-blob
# TEST	items. Remove a high proportion of entries.
...
proc test152 { method {nentries 10000} {tnum "152"} args } {
	...
	# Blobs are not supported by all access methods and
	# configuration options. 
	if { [can_support_blobs $method $args] == 1 } {
		set bflags " -blob_threshold 10"
	} else { 
		puts "Skipping test$tnum ($method $args) for blobs."
		return
	}
	...
	if { $eindex == -1 } {
		...
		set env NULL
		set blobrootdir $testdir/__db_bl
		append bflags " -blob_dir $blobrootdir"
	} else {
		...
		set blobdir [$env get_blob_dir]
		if { $blobdir == "" } {
			set blobdir __db_bl
		}
		set blobrootdir $testdir/$blobdir
	}
	puts "Test$tnum: ($method $args) Database compaction with blobs."
	...
	foreach splitopt $splitopts {
		...
		set db [eval {berkdb_open -create \
		    -mode 0644} $bflags $splitopt $args $omethod $testfile]
		...
		# size, for example hash with large pagesizes.  Hash 
		# with blobs also doesn't reduce much.
		#
		#### We should look at the partitioned files #####
		set test_filesize 1
