		    ...
		    struct blob_attr *msg)
{
	...
	u8 chwidth = iconf->vht_oper_chwidth;
	u8 seg0 = 0, seg1 = 0;
...
#define SET_CSA_SETTING(name, field, type) \
	...
	case 160:
		...
		break;
	case 80:
		chwidth = css.freq_params.center_freq2 ? VHT_CHANWIDTH_80P80MHZ : VHT_CHANWIDTH_80MHZ;
		break;
	default:
		    ...
		    struct blob_attr *neighbors, u8 mbo_reason, u8 cell_pref, u8 reassoc_delay)
{
	...
	*mbo_pos++ = cell_pref;
	...
	if (wnm_send_bss_tm_req(hapd, sta, req_mode, disassoc_timer, validity_period, NULL,
				NULL, nr, nr_len, mbo_len ? mbo : NULL, mbo_len))
		return UBUS_STATUS_UNKNOWN_ERROR;
