...
{      
  ...
  switch (option)
	...
	if (!(dir_stream = opendir(directory)))
	  die2(_("cannot access directory %s: %s"), directory);
...
static void one_file(struct daemon *daemon, char *file, int nest)	
{
  ...
  if (nest > 20)
    die2(_("files nested too deep in %s"), file);
...
#endif  
	  if (errmess)
	    die2(_("bad command line options: %s"), errmess);
	}
    }
