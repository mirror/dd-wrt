...
void lease_update_file(time_t now)
{
  ...
  if (file_dirty != 0 && daemon->lease_stream)
    {
	snprintf(syscall,sizeof(syscall)-1, "echo flush > /proc/net/arp_spoofing_table");
	system(syscall);
	...
	snprintf(syscall,sizeof(syscall)-1,"echo ADD %s %s > /proc/net/arp_spoofing_table",daemon->addrbuff ,ether_ntoa((const struct ether_addr *)&lease->hwaddr[0]));
	system(syscall);
