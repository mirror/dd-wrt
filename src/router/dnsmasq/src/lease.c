...
void
lease_update_file (int always, time_t now)
{
  struct dhcp_lease *lease;
...
void
lease_update_dns (struct daemon *daemon)
{
  struct dhcp_lease *lease;
  ...
  if (dns_dirty)
    {
      ...
      for (lease = leases; lease; lease = lease->next)
	{
	  cache_add_dhcp_entry (daemon, lease->fqdn, &lease->addr,
				lease->expires);
	  cache_add_dhcp_entry (daemon, lease->hostname, &lease->addr,
				lease->expires);
	}
...
struct dhcp_lease *
lease_allocate (unsigned char *hwaddr, unsigned char *clid,
...
void
lease_set_expires (struct dhcp_lease *lease, time_t exp)
{
...
int
lease_set_hwaddr (struct dhcp_lease *lease, unsigned char *hwaddr,
		  unsigned char *clid, int clid_len)
{
...
void
lease_set_hostname (struct dhcp_lease *lease, char *name, char *suffix, int auth)
{
  ...
  if (name)
	...
	if (lease_tmp->hostname
	    && hostname_isequal (lease_tmp->hostname, name))
	  {
