        ...
        ZSTD_CCtx_params* CCtxParams, ZSTD_cParameter param, unsigned value)
{
    ...
    case ZSTD_p_compressionLevel : {
        ...
        if (cLevel) {  /* 0 : does not change c_current level */
...
/*! ZSTD_toFlushNow()
 *  Only useful for multithreading scenarios c_currently (nbWorkers >= 1).
 */
...
#endif
    (void)cctx;
    return 0;   /* over-simplification; could also check if context is c_currently running in streaming mode, and in which case, report how many bytes are left to be flushed within output buffer */
}
                                   ...
                                   ZSTD_EndDirective const flushMode)
{
    ...
    while (someMoreWork) {
        ...
        case zcss_load:
            ...
            /* compress c_current block (note : this stage cannot be stopped in the middle) */
