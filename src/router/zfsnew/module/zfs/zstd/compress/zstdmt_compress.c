...
typedef struct {
  ...
  size_t pos;       /* The position of the c_current inBuff in the round
 ...
 *  Updates a selected set of compression parameters, remaining compatible with c_currently active frame.
 ...
 * tells how much data has been consumed (input) and produced (output) for c_current frame.
 ...
 *  flush whatever data has been produced but not yet flushed in c_current job.
 *  move to next job if c_current one is fully flushed.
...
static size_t ZSTDMT_flushProduced(ZSTDMT_CCtx* mtctx, ZSTD_outBuffer* output, unsigned blockToFlush, ZSTD_EndDirective end)
{
    ...
    if (  blockToFlush
      && (mtctx->doneJobID < mtctx->nextJobID) ) {
        ...
        while (mtctx->jobs[wJobID].dstFlushed == mtctx->jobs[wJobID].cSize) {  /* nothing to flush */
            ...
            DEBUGLOG(5, "waiting for something to flush from job %u (c_currently flushed: %u bytes)",
    ...
    {   size_t cSize = mtctx->jobs[wJobID].cSize;                  /* shared */
        ...
        if (srcSize > srcConsumed) return 1;   /* c_current job not completely compressed */
    }
                                     ...
                                     ZSTD_EndDirective endOp)
{
    ...
    if ((mtctx->frameEnded) && (endOp==ZSTD_e_continue)) {
        /* c_current frame being ended. Only flush/end are allowed */
...
size_t ZSTDMT_compressStream(ZSTDMT_CCtx* mtctx, ZSTD_outBuffer* output, ZSTD_inBuffer* input)
{
    ...
    /* recommended next input size : fill c_current input buffer */
