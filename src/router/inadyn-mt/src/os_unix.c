...
/** The actual handler
	...
	05/08 -- Interpret USR1 as winmodem connect.
*/
static void unix_signal_handler(int signo)
{
	...
	case SIGUSR1:
		DBG_PRINTF((LOG_DEBUG,MODULE_TAG "Signal '0x%x' received. Sending update cmd.\n", signo));

		p_self->cmd = CMD_UPDT;
		break;

	case SIGPIPE:
		DBG_PRINTF((LOG_DEBUG,MODULE_TAG "Signal '0x%x' (broken pipe) received...\n", signo));
