 ...
 *
 * $Id: main.c,v 1.1.1.6 2004/04/12 04:34:05 honor Exp $
 */
...
#define valid_ports(min, max) (!((min) == 0 && (max) == 0) && !((min) == 0 && (max) == 0xffff))

static void print_fw(netconf_fw_t *fw)
{
	...
	if (fw->match.src.ipaddr.s_addr & fw->match.src.netmask.s_addr) {
		printf(" src %s%s", (fw->match.flags & NETCONF_INV_SRCIP) ? "!" : "", inet_ntoa(fw->match.src.ipaddr));
	...
	if (fw->match.ipproto && valid_ports(fw->match.src.ports[0], fw->match.src.ports[1])) {
		printf(":%s%d", (fw->match.flags & NETCONF_INV_SRCPT) ? "!" : "", ntohs(fw->match.src.ports[0]));
	...
	if (fw->match.dst.ipaddr.s_addr & fw->match.dst.netmask.s_addr) {
		printf(" dst %s%s", (fw->match.flags & NETCONF_INV_DSTIP) ? "!" : "", inet_ntoa(fw->match.dst.ipaddr));
	...
	if (fw->match.ipproto && valid_ports(fw->match.dst.ports[0], fw->match.dst.ports[1] != 0xffff)) {
		printf(":%s%d", (fw->match.flags & NETCONF_INV_DSTPT) ? "!" : "", ntohs(fw->match.dst.ports[0]));
	...
	if (!ETHER_ISNULLADDR(fw->match.mac.octet)) {
		printf(" mac %s%02X:%02X:%02X:%02X:%02X:%02X", (fw->match.flags & NETCONF_INV_MAC) ? "! " : "",
		       fw->match.mac.octet[0] & 0xff, fw->match.mac.octet[1] & 0xff, fw->match.mac.octet[2] & 0xff,
		       fw->match.mac.octet[3] & 0xff, fw->match.mac.octet[4] & 0xff, fw->match.mac.octet[5] & 0xff);
	}
	...
	if (fw->match.secs[0] || fw->match.secs[1]) {
		...
		if (fw->match.days[0] < 7 && fw->match.days[1] < 7)
			printf(" %s-%s", days[fw->match.days[0]], days[fw->match.days[1]]);
		if (fw->match.secs[0] < (24 * 60 * 60) && fw->match.days[1] < (24 * 60 * 60))
			printf(" %02d:%02d:%02d-%02d:%02d:%02d", fw->match.secs[0] / (60 * 60), (fw->match.secs[0] / 60) % 60,
			       fw->match.secs[0] % 60, fw->match.secs[1] / (60 * 60), (fw->match.secs[1] / 60) % 60,
			       fw->match.secs[1] % 60);
	}

	/* Match interface name */
	if (strlen(fw->match.in.name))
		printf(" in %s%s", (fw->match.flags & NETCONF_INV_IN) ? "!" : "", fw->match.in.name);
	if (strlen(fw->match.out.name))
		printf(" out %s%s", (fw->match.flags & NETCONF_INV_OUT) ? "!" : "", fw->match.out.name);
