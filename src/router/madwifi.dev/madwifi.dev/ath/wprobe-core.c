...
int wprobe_add_link(struct wprobe_iface *s, struct wprobe_link *l, const char *addr)
{
	unsigned long flags;
...
void wprobe_remove_link(struct wprobe_iface *s, struct wprobe_link *l)
{
	unsigned long flags;
...
static void wprobe_measure_timer(unsigned long data)
{
	struct wprobe_iface *dev = (struct wprobe_iface *)data;

	/* set next measurement interval */
	mod_timer(&dev->measure_timer, jiffies + msecs_to_jiffies(dev->measure_interval));

	/* perform measurement */
	wprobe_sync_data(dev, NULL, false);
}

int wprobe_add_iface(struct wprobe_iface *s)
{
	unsigned long flags;
	int vsize;
...
void wprobe_remove_iface(struct wprobe_iface *s)
{
	unsigned long flags;
...
static struct wprobe_iface *wprobe_get_dev(struct nlattr *attr)
{
...
int wprobe_add_frame(struct wprobe_iface *dev, const struct wprobe_wlan_hdr *hdr, void *data, int len)
{
...
static void wprobe_scale_stats(struct wprobe_iface *dev, const struct wprobe_item *item, struct wprobe_value *val, int n)
{
...
void wprobe_update_stats(struct wprobe_iface *dev, struct wprobe_link *l)
{
	...
	for (i = 0; i < n; i++) {
		...
		val[i].n++;
		if ((item[i].flags & WPROBE_F_KEEPSTAT) && (dev->scale_max > 0) && (val[i].n > dev->scale_max)) {
			scale_stats = true;
		}
...
static bool wprobe_check_ptr(struct list_head *list, struct list_head *ptr)
{
	struct list_head *p;
...
static bool wprobe_send_item_value(struct sk_buff *msg, struct netlink_callback *cb, struct wprobe_iface *dev, struct wprobe_link *l, const struct wprobe_item *item, int i, u32 flags)
{
	struct genlmsghdr *hdr;
	struct wprobe_value *val = dev->query_val;
	u64 time = val[i].last - val[i].first;

	hdr = genlmsg_put(msg, NETLINK_CB(cb->skb).pid, cb->nlh->nlmsg_seq, &wprobe_fam, NLM_F_MULTI, WPROBE_CMD_GET_INFO);
...
static bool wprobe_send_item_info(struct sk_buff *msg, struct netlink_callback *cb, struct wprobe_iface *dev, const struct wprobe_item *item, int i)
{
	struct genlmsghdr *hdr;

	hdr = genlmsg_put(msg, NETLINK_CB(cb->skb).pid, cb->nlh->nlmsg_seq, &wprobe_fam, NLM_F_MULTI, WPROBE_CMD_GET_LIST);
...
static struct wprobe_link *wprobe_find_link(struct wprobe_iface *dev, const char *mac)
{
...
static bool wprobe_dump_filter_group(struct sk_buff *msg, struct wprobe_filter_group *fg, struct netlink_callback *cb)
{
	struct genlmsghdr *hdr;
	struct nlattr *group, *item;
	int i;

	hdr = genlmsg_put(msg, NETLINK_CB(cb->skb).pid, cb->nlh->nlmsg_seq, &wprobe_fam, NLM_F_MULTI, WPROBE_CMD_GET_FILTER);
	if (!hdr)
		return false;
...
static int wprobe_dump_filters(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	if (!dev) {
		err = nlmsg_parse(cb->nlh, GENL_HDRLEN + wprobe_fam.hdrsize, wprobe_fam.attrbuf, wprobe_fam.maxattr, wprobe_policy);
		if (err)
			goto done;
...
static bool wprobe_dump_link(struct sk_buff *msg, struct wprobe_link *l, struct netlink_callback *cb)
{
	struct genlmsghdr *hdr;

	hdr = genlmsg_put(msg, NETLINK_CB(cb->skb).pid, cb->nlh->nlmsg_seq, &wprobe_fam, NLM_F_MULTI, WPROBE_CMD_GET_LINKS);
	if (!hdr)
		return false;
...
static int wprobe_dump_links(struct sk_buff *skb, struct netlink_callback *cb)
{
	struct wprobe_iface *dev = (struct wprobe_iface *)cb->args[0];
	struct wprobe_link *l;
	int err = 0;
...
static int wprobe_dump_info(struct sk_buff *skb, struct netlink_callback *cb)
{
	struct wprobe_iface *dev = (struct wprobe_iface *)cb->args[0];
...
static int wprobe_update_auto_measurement(struct wprobe_iface *dev, u32 interval)
{
...
static int wprobe_measure(struct sk_buff *skb, struct genl_info *info)
{
...
static int wprobe_check_filter(void *data, int datalen, int gs)
{
...
static int wprobe_set_filter(struct wprobe_iface *dev, void *data, int len)
{
	...
	f = kzalloc(sizeof(struct wprobe_filter) + gs * sizeof(struct wprobe_filter_group), GFP_ATOMIC);
	if (!f)
		return -ENOMEM;
...
static int wprobe_set_config(struct sk_buff *skb, struct genl_info *info)
{
	struct wprobe_iface *dev;
	unsigned long flags;
	int err = -ENOENT;
	...
	if (info->attrs[WPROBE_ATTR_SAMPLES_MIN] || info->attrs[WPROBE_ATTR_SAMPLES_MAX]) {
		...
		if ((!scale_min && !scale_max) || (scale_min && scale_max && (scale_min < scale_max))) {
	...
	if (info->attrs[WPROBE_ATTR_SAMPLES_SCALE_M] && info->attrs[WPROBE_ATTR_SAMPLES_SCALE_D]) {
	...
	if (info->attrs[WPROBE_ATTR_INTERVAL]) {
		...
		err = wprobe_update_auto_measurement(dev, (u32)nla_get_u64(info->attrs[WPROBE_ATTR_INTERVAL]));
	}
