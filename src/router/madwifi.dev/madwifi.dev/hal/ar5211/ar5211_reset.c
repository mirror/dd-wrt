 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ar5211/ar5211_reset.c#5 $
 */
#include "opt_ah.h"
...
#undef IS
	...
	for (i = 0; i < N(ar5211Common); i++) {
		...
		if ((resetType != HAL_RESET_FULL) && (0x8000 <= reg && reg < 0x9000))
	 ...
	 * only when operating in station mode.  Text from bug 20450:
	 *
 ...
 */
HAL_BOOL
ar5211PerCalibration(struct ath_hal *ah, HAL_CHANNEL *chan, HAL_BOOL longCal, HAL_BOOL *isIQdone)
{
	struct ath_hal_5211 *ahp = AH5211(ah);
	HAL_CHANNEL_INTERNAL *ichan;
