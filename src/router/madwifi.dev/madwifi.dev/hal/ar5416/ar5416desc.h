 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ar5416/ar5416desc.h#1 $
 */
 ...
 */
#define _get_index(_ah) ( AR_SREV_OWL_20_OR_LATER(_ah) ? 0 : -4 )
#define AR5416_DS_TXSTATUS(_ah, _ads) \
	((u_int32_t*)(&(_ads)->u.tx.status[_get_index(_ah)]))
#define AR5416_DS_TXSTATUS_CONST(_ah, _ads) \
	((const u_int32_t*)(&(_ads)->u.tx.status[_get_index(_ah)]))
...
struct ar5416_tx_desc { /* tx desc has 12 control words + 10 status words */
	...
	u_int32_t	ctl10;
	u_int32_t	ctl11;
	u_int32_t	status[AR5416_NUM_TX_STATUS];
};
...
#define AR_TxStatusRsvd10       0xfff80000
...
#define TXCTL_NUMWORDS(ah) \
	((ah->ah_macRev >= AR_SREV_REVISION_OWL_20) ? 12 : 8)
#define TXSTATUS_OFFSET(ah) \
	((ah->ah_macRev >= AR_SREV_REVISION_OWL_20) ? 14 : 10)
...
#define RXSTATUS_RATE(ah, ads) \
	((ah->ah_macRev >= AR_SREV_REVISION_OWL_20) ? \
	  MS(ads->ds_rxstatus0, AR_RxRate) : (ads->ds_rxstatus3 >> 2) & 0xFF)
#define RXSTATUS_DUPLICATE(ah, ads) \
	((ah->ah_macRev >= AR_SREV_REVISION_OWL_20) ?	\
	 MS(ads->ds_rxstatus3, AR_Parallel40) : (ads->ds_rxstatus3 >> 10) & 0x1)
