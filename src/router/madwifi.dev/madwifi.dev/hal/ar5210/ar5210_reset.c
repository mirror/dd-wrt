 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ar5210/ar5210_reset.c#3 $
 */
#include "opt_ah.h"
 ...
 */
HAL_BOOL
ar5210PerCalibration(struct ath_hal *ah, HAL_CHANNEL *chan, HAL_BOOL longCal, HAL_BOOL *isIQdone)
{
	u_int32_t regBeacon;
...
#ifdef AH_DEBUG
	HALDEBUG(ah, "%s: empty transmit power table?\n", __func__);
#endif
	return 1;
}
...
#define	N(a)	(sizeof (a) / sizeof (a[0]))
...
#ifdef AH_DEBUG
		HALDEBUG(ah, "%s: unable to setup power settings\n",
			__func__);
#endif
		return AH_FALSE;
	}
	if (cp[15] < 1 || cp[15] > 5) {
#ifdef AH_DEBUG
		HALDEBUG(ah, "%s: OB out of range (%u)\n",
			__func__, cp[15]);
#endif
		return AH_FALSE;
	}
	if (cp[16] < 1 || cp[16] > 5) {
#ifdef AH_DEBUG
		HALDEBUG(ah, "%s: DB out of range (%u)\n",
			__func__, cp[16]);
#endif
		return AH_FALSE;
	}
...
ar5210CalNoiseFloor(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
	...
	if (!ath_hal_wait(ah, AR_PHY_AGCCTL, AR_PHY_AGC_NF, 0)) {
		...
		HALDEBUG(ah, " -MAC Reset Reg state: 0x%x\n",
