 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ar5212/ar5212_ani.c#17 $
 */
#include "opt_ah.h"
...
enableAniMIBCounters(struct ath_hal *ah, const struct ar5212AniParams *params)
{
	...
	HALDEBUG(ah, "%s: Enable mib counters: "
...
disableAniMIBCounters(struct ath_hal *ah)
{
	...
	HALDEBUG(ah, "Disable MIB counters\n");
...
#define N(a)     (sizeof(a) / sizeof(a[0]))
	...
	for (i = 0; i < HAL_CHANNELS; i++) {
		struct ar5212AniState *asp = &ahp->ah_ani[i];
		if (asp->c.channel == chan->channel) // sollte man hier nicht auch isSetup checken?
			return i;
		if (asp->c.channel == 0) {
	...
	const struct ar5212AniParams *params5, HAL_BOOL enable)
{
	...
	OS_MEMZERO(ahp->ah_ani, HAL_CHANNELS*sizeof(struct ar5212AniState));
	ahp->ah_curani = &ahp->ah_ani[0];
...
ar5212AniControl(struct ath_hal *ah, HAL_ANI_CMD cmd, int param)
{
	...
	case HAL_ANI_PRESENT:
		break;
	case HAL_ANI_MODE:
		if ((param & HAL_ANI_ENA) == 0) {
			ahp->ah_procPhyErr &= ~(HAL_ANI_ENA|HAL_RSSI_ANI_ENA);
	...
	HAL_OPMODE opmode, int restore)
{
	...
	if (!ahp->ah_curani->params) { // das hier ist unsinn, wird von getanichannelindex erledigt
		if (IS_CHAN_2GHZ(chan))
			ahp->ah_curani->params = &ahp->ah_aniParams24;
		else
			ahp->ah_curani->params = &ahp->ah_aniParams5;
	}
#if 0
...
#else
	HALDEBUG(ah,"%s: chan %u/0x%x restore %d setup %d opmode %u\n",
	    __func__, chan->channel, chan->channelFlags, restore,
	    aniState->isSetup, opmode);
#endif
	 ...
	 */
	if ((opmode == HAL_M_STA) || (opmode == HAL_M_IBSS))
	...
	if (restore && aniState->isSetup) {
		...
		ar5212AniControl(ah, HAL_ANI_NOISE_IMMUNITY_LEVEL, ANI_ENA(ah) ? 4 : 0);
...
ar5212ProcessMibIntr(struct ath_hal *ah, const HAL_NODE_STATS *stats)
{
	...
	HALDEBUG(ah, "%s: mibc 0x%x phyCnt1 0x%x phyCnt2 0x%x "
...
updateMIBStats(struct ath_hal *ah, struct ar5212AniState *aniState)
{
	...
	ofdmPhyErrCnt = phyCnt1 - params->ofdmPhyErrBase;
	if (ofdmPhyErrCnt < 0) {
		HALDEBUG(ah, "OFDM phyErrCnt %d phyCnt1 0x%x\n",
	...
	cckPhyErrCnt = phyCnt2 - params->cckPhyErrBase;
	if (cckPhyErrCnt < 0) {
		HALDEBUG(ah, "CCK phyErrCnt %d phyCnt2 0x%x\n",
