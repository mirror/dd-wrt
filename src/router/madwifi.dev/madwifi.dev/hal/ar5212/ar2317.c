 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ar5212/ar2317.c#5 $
 * $File: //depot/sw/branches/sam_hal/ar5212/ar2317.c $
 * $Author: sam $
...
{
	struct ar2317state *priv = AH5212(ah)->ah_rfHal.priv;
...
ar2317RfAttach(struct ath_hal *ah, HAL_STATUS *status)
{
	...
	HALASSERT(ahp->ah_analogBanks == AH_NULL);
	ahp->ah_analogBanks = ath_hal_malloc(sizeof(AR5212_RF_BANKS_2317));
	if (ahp->ah_analogBanks == AH_NULL) {
	...
	ahp->ah_rfHal.priv = ath_hal_malloc(sizeof(struct ar2317state));
	if (ahp->ah_rfHal.priv == AH_NULL) {
