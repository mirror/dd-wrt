 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ar5212/ar2413.c#4 $
 * $File: //depot/sw/branches/sam_hal/ar5212/ar2413.c $
 * $Author: sam $
...
ar2413SetChannel(struct ath_hal *ah,  HAL_CHANNEL_INTERNAL *chan)
{
	...
	OS_MARK(ah, AH_MARK_SETCHANNEL, chan->channel);
		...
		if (chan->channel>2500 && chan->channel<2700)
		    {
		    ar5212ModifyRfBuffer(pRfBanks->Bank6Data, 1,   1, 214, 2);
		    ar5212ModifyRfBuffer(pRfBanks->Bank6Data, 0x1e,   8, 206, 2);
		    ar5212ModifyRfBuffer(pRfBanks->Bank6Data, chan->channel>>4,   11, 195, 2);
		    ar5212ModifyRfBuffer(pRfBanks->Bank6Data, chan->channel%16,   4, 191, 2);	    
...
{
	struct ar2413state *priv = AH5212(ah)->ah_rfHal.priv;
...
ar2413RfAttach(struct ath_hal *ah, HAL_STATUS *status)
{
	...
	ahp->ah_rfHal.priv = ath_hal_malloc(sizeof(struct ar2413state));
	if (ahp->ah_rfHal.priv == AH_NULL) {
