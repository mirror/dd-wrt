 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ar5212/ar5212_reset.c#27 $
 */
#include "opt_ah.h"
...
void		ar5212Set11nCompat(struct ath_hal *);
...
static u_int32_t ar5212GetGainFCorrection(struct ath_hal *ah);
...
#undef IS
	...
	if (IS_2413(ah) || IS_5413(ah)) {
		 ...
		 */
		if ((resetType == HAL_RESET_CHANNEL_FAST) &&
	...
	if (IS_5413(ah) || IS_2417(ah) || IS_2425(ah)) {
		u_int32_t newReg=1;
...
update_flags:
	...
	if (ahp->ah_eifstime != (u_int) -1)
		ar5212SetEifsTime(ah, ahp->ah_eifstime);
 ...
 * only when operating in station mode.  Refer to bug 20450
 * for complete details.
 */
void
ar5212Set11nCompat(struct ath_hal *ah)
{
	u_int32_t compat = AH_PRIVATE(ah)->ah_11nCompat;

	/*
	 * On all Atheros legacy devices (AP and client), in order to
	 * reduce the likelihood of false detects for OFDM frames, we
	 * check that all 16 SERVICE bits (between the L-SIG and the
	 * beginning of the MAC header) are zero.
	 * 
	 * In the Mixed Mode (MM) preamble, instead of SERVICE bits,
	 * a legacy device sees the HT SIGNAL (HT-SIG) field.  Try as
	 * it might to decode the rotated BPSK symbol, it decodes garbage.
	 * While there is a chance that the garbage could come up all
	 * zeros, it is highly improbable.  Thus legacy devices should
	 * treat this frame as a PHY error and discard it but just in
	 * case we provide a tunable to explicitly enable/disable the
	 * SERVICE bits check.
	 *
	 * Note that by default we use whatever settings are in the ini
	 * file; these controls are here to enable workarounds and/or
	 * experiments once 11n is more widely deployed.
	 */
	if ((compat & HAL_DIAG_11N_SERVICES) != 0) {
		/* NB: override the .ini setting */
		OS_REG_RMW_FIELD(ah, AR_PHY_FRAME_CTL,
			AR_PHY_FRAME_CTL_ERR_SERV,
			MS(compat, HAL_DIAG_11N_SERVICES)&1);
	}
	/*
	 * When the SERVICE bits check is disabled transmit
	 * stomp can also have an effect so provide a knob
	 * to control that too.
	 */
	if ((compat & HAL_DIAG_11N_TXSTOMP) != 0) {
#define	TXSTOMP	(AR_XRSTMP_TX_RSSI | AR_XRSTMP_TX_BSSID | AR_XRSTMP_TX_DATA)
		if ((MS(compat, HAL_DIAG_11N_TXSTOMP)&1) == 0)
			OS_REG_CLR_BIT(ah, AR_XRSTMP, TXSTOMP);
		else
			OS_REG_SET_BIT(ah, AR_XRSTMP, TXSTOMP);
#undef TXSTOMP
	}
}
	...
	if (chan != AH_NULL) {		/* NB: can be null during attach */
		...
		if (IS_5413(ah)) {
			...
			if (IS_CHAN_HALF_RATE(chan))
				...
				{
				if (IS_CHAN_CCK(chan) || IS_CHAN_G(chan))
					phyPLL = AR_PHY_PLL_CTL_44_5112_SUBQUARTER;
				else
					phyPLL = AR_PHY_PLL_CTL_40_5112_SUBQUARTER;
				}
			...
			if (IS_CHAN_CCK(chan) || IS_CHAN_G(chan)) {
				...
				if (IS_CHAN_HALF_RATE(chan)) {
					...
					phyPLL = AR_PHY_PLL_CTL_44_SUBQUARTER;
				}
			} else {
				if (IS_CHAN_HALF_RATE(chan)) {
					...
					phyPLL = AR_PHY_PLL_CTL_40_SUBQUARTER;
				} else {
 ...
 */
HAL_BOOL
ar5212PerCalibration(struct ath_hal *ah,  HAL_CHANNEL *chan, HAL_BOOL longCal, HAL_BOOL *isIQdone)
{
	...
	if (longCal) {
		...
		if (!(chan->channelFlags&CHANNEL_CW_INT)) {
			...
			if ((IS_CHAN_5GHZ(chan) ||
			    ...
			    !(IS_2413(ah) || IS_5413(ah) || IS_2417(ah) || IS_2425(ah)))
...
int16_t
ar5212GetNoiseFloor(struct ath_hal *ah,u_int16_t nfarray[NUM_NOISEFLOOR_READINGS])
{
    int16_t nf;
...
/*
 * Return the medium noise floor value in the ring buffer.
 */
static int16_t 
getNfHistMid(const int16_t calData[AR512_NF_CAL_HIST_MAX])
{
...
ar5212GetNf(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
	 ...
	 */
	nf = ar5212GetNoiseFloor(ah,nfarray);
	...
	if (!h->first_run) {
		if (nf < AR5212_CCA_MIN_BAD_VALUE ||
			...
			h->first_run = 1;
			h->privNF = nf = getNfHistMid(h->nfCalBuffer);
		} else {
			nf = AR5212_CCA_MAX_GOOD_VALUE;
		}
	} else {
    ...
    if (i == 1000) {
         ...
         */
		HALDEBUG(ah, "%s: AGC not ready AGC_CONTROL 0x%x\n",
...
#define	ANT_SWITCH_TABLE2	AR_PHY(89)
	...
	OS_MEMCPY(&ichan, chan, sizeof(ichan));
	...
	if (antSwitchB == antSwitchA) {
		HALDEBUG(ah, "%s: Setting fast diversity off.\n",
			 __func__);
		OS_REG_CLR_BIT(ah,AR_PHY_CCK_DETECT, 
			       AR_PHY_CCK_DETECT_BB_ENABLE_ANT_FAST_DIV);
		OS_REG_CLR_BIT(ah, AR_PHY_RESTART, AR_PHY_RESTART_DIV_GC);
	} else {
		HALDEBUG(ah, "%s: Setting fast diversity on.\n",
		    __func__);
		OS_REG_SET_BIT(ah,AR_PHY_CCK_DETECT, 
			       AR_PHY_CCK_DETECT_BB_ENABLE_ANT_FAST_DIV);
		OS_REG_SET_BIT(ah, AR_PHY_RESTART, AR_PHY_RESTART_DIV_GC);
	}
...
{
    u_int32_t clockFreq = ((IS_5413(ah) || IS_2413(ah) || IS_5112(ah) || IS_2417(ah) || IS_2425(ah)) ? 40 : 32);
...
#define	AR_PHY_BIS(_ah, _reg, _mask, _val) \
	...
	if (ahp->ah_eeversion >= AR_EEPROM_VER5_1) {
		OS_REG_WRITE(ah,  AR_PHY_HEAVY_CLIP_ENABLE,
			(pCap->halPhyHeavyClippingSupport ? 0x1ff : 0));
	}
...
#define CHAN_TO_SPUR(_f, _freq)   ( ((_freq) - ((_f) ? 2300 : 4900)) * 10 )
	...
	if (IS_2417(ah) || IS_2425(ah)) {
    	    HALDEBUG(ah, "%s no spur mitigation\n", __func__);
    	    return;
	}
	...
	if (finalSpur == AR_NO_SPUR) {
		...
		if (OS_REG_READ(ah, AR_PHY_TIMING_CTRL4) & AR_PHY_TIMING_CTRL4_ENABLE_SPUR_MASK) {
			...
			val &= ~AR_PHY_TIMING_CTRL4_ENABLE_SPUR_MASK;
		...
		OS_REG_RMW_FIELD(ah, AR_PHY_TIMING8, AR_PHY_TIMING8_RSSI_THRESH, rssiThresh);
		...
		OS_REG_RMW_FIELD(ah, AR_PHY_MASK_CTL, AR_PHY_MASK_CTL_SELECT, 0xF0);
...
ar5212SetTxPowerLimit(struct ath_hal *ah, u_int32_t limit)
{
	...
	AH_PRIVATE(ah)->ah_extra_txpow = 0;// not yet
...
#define	N(a)			(sizeof (a) / sizeof (a[0]))
	...
	OS_REG_WRITE(ah, AR_PHY_POWER_TX_RATE1, 
		POW_OFDM(ahp->ah_ratesArray[3] + ahpriv->ah_extra_txpow, 24)
	      | POW_OFDM(ahp->ah_ratesArray[2] + ahpriv->ah_extra_txpow, 16)
	      | POW_OFDM(ahp->ah_ratesArray[1] + ahpriv->ah_extra_txpow,  8)
	      | POW_OFDM(ahp->ah_ratesArray[0] + ahpriv->ah_extra_txpow,  0)
	);
	OS_REG_WRITE(ah, AR_PHY_POWER_TX_RATE2, 
		POW_OFDM(ahp->ah_ratesArray[7] + ahpriv->ah_extra_txpow, 24)
	      | POW_OFDM(ahp->ah_ratesArray[6] + ahpriv->ah_extra_txpow, 16)
	      | POW_OFDM(ahp->ah_ratesArray[5] + ahpriv->ah_extra_txpow,  8)
	      | POW_OFDM(ahp->ah_ratesArray[4] + ahpriv->ah_extra_txpow,  0)
	);

	/* Write the CCK power per rate set */
	OS_REG_WRITE(ah, AR_PHY_POWER_TX_RATE3,
		POW_CCK(ahp->ah_ratesArray[10] + ahpriv->ah_extra_txpow, 24)
	      | POW_CCK(ahp->ah_ratesArray[9] + ahpriv->ah_extra_txpow,  16)
	      | POW_CCK(ahp->ah_ratesArray[15] + ahpriv->ah_extra_txpow,  8)	/* XR target power */
	      | POW_CCK(ahp->ah_ratesArray[8] + ahpriv->ah_extra_txpow,   0)
	);
	OS_REG_WRITE(ah, AR_PHY_POWER_TX_RATE4,
		POW_CCK(ahp->ah_ratesArray[14] + ahpriv->ah_extra_txpow, 24)
	      | POW_CCK(ahp->ah_ratesArray[13] + ahpriv->ah_extra_txpow, 16)
	      | POW_CCK(ahp->ah_ratesArray[12] + ahpriv->ah_extra_txpow,  8)
	      | POW_CCK(ahp->ah_ratesArray[11] + ahpriv->ah_extra_txpow,  0)
	);
...
ar5212GetRfgain(struct ath_hal *ah)
{
	...
	if (ahp->ah_rfgainState == HAL_RFGAIN_READ_REQUESTED) {
		...
		if ((rddata & AR_PHY_PAPD_PROBE_NEXT_TX) == 0) {
			...
			if (IS_5112(ah)) {
				u_int32_t correct = ar5212GetGainFCorrection(ah);
...
ar5212GetGainFCorrection(struct ath_hal *ah)
{
	...
	if (ar5212GetRfField(ar5212GetRfBank(ah, 7), 1, 36, 0) == 1) {
		...
		u_int32_t gainStep = ar5212GetRfField(ar5212GetRfBank(ah, 7), 4, 32, 0);
		switch (mixGain) {
		case 0 :
			correction = 0;
			break;
...
#define	WLAN_CTRL_FRAME_SIZE	(2+2+6+4)	/* ACK+FCS */
	...
	if (IS_CHAN_HALF_RATE(chan)) {
		...
		rt = ar5212GetRateTable(ah, HAL_MODE_11A_SUBQUARTER_RATE);
	} else {
...
{
	u_int32_t txLat, rxLat, usec, refClock, eifs, init_usec;
	...
	if (IS_CHAN_HALF_RATE(chan)) {
		...
		rxLat = RX_NON_FULL_RATE_LATENCY << AR5212_USEC_RX_LAT_S;
		txLat = TX_SUBQUARTER_RATE_LATENCY << AR5212_USEC_TX_LAT_S;
		usec = SUBQUARTER_RATE_USEC;
		eifs = IFS_EIFS_SUBQUARTER_RATE;
		init_usec = INIT_USEC >> 3;
	} else { /* quarter rate */
		rxLat = RX_NON_FULL_RATE_LATENCY << AR5212_USEC_RX_LAT_S;
