 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/branches/sam_hal/ah_eeprom.c#3 $
 */
#include "opt_ah.h"
...
#define	EEREAD(_off) do {				\
	...
	for (mode = headerInfo11A; mode <= headerInfo11G; mode++) {
		...
		for (i = 0; i < numPiers; i++) {
			...
			int powerfix = 0;
			if (freq[i]<4900)
			    powerfix = AH_PRIVATE(ah)->ah_powerfix;

			EEREAD(off++);
			eePower.pDataPerChannel[i].pwr1_xg0 = (int16_t)
				((eeval & dbmmask) - ((eeval >> 7) & 0x1)*256)+powerfix;
			eePower.pDataPerChannel[i].pwr2_xg0 = (int16_t)
				(((eeval >> 8) & dbmmask) - ((eeval >> 15) & 0x1)*256)+powerfix;
		...
		for (i = 0; i < nchan; i++) {
			...
			for (j = 0; j < pChannelData->numPcdacValues; j++) {
				pChannelData->PwrValues[j] = (u_int16_t)(
					PWR_STEP * (pChannelData->PwrValues[j]+AH_PRIVATE(ah)->ah_powerfix));
		...
		*pNumTrgtChannels = 0;
		for (i = 0; i < nchan; i++) {
			...
			if (pPowerInfo->testChannel != 0) {
				...
				if (mode != headerInfo11A)
				{
				pPowerInfo->twicePwr36 += AH_PRIVATE(ah)->ah_powerfix*2;
				pPowerInfo->twicePwr48 += AH_PRIVATE(ah)->ah_powerfix*2;
				pPowerInfo->twicePwr54 += AH_PRIVATE(ah)->ah_powerfix*2;
				pPowerInfo->twicePwr6_24 += AH_PRIVATE(ah)->ah_powerfix*2;
				}
				(*pNumTrgtChannels)++;
			}
			pPowerInfo++;
...
#define	EEREAD(_off) do {				\
	...
	ee->ee_turbo2Disable	= 0;
	ee->ee_turbo5Disable	= 0;
