	...
	if (croute != NULL) {
		...
		if (originAddr > 0) {
			if (croute->originAddr > 0 && croute->originAddr != originAddr) {
				my_log(LOG_INFO, 0, "The origin for route %s changed from %s to %s", inetFmt(croute->group, s1), inetFmt(croute->originAddr, s2), inetFmt(originAddr, s3));
			}
	...
	if (croute == NULL) {
		...
		do {
			...
			my_log(LOG_DEBUG, 0, "#%d: Src: %s, Dst: %s, Age:%d, St: %s, OutVifs: 0x%08x",
			       rcount, inetFmt(croute->originAddr, s1), inetFmt(croute->group, s2), croute->ageValue, (croute->originAddr > 0 ? "A" : "I"), croute->vifBits);
