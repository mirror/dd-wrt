...
void initRouteTable(void)
{
	...
	for (Ix = 0; (Dp = getIfByIx(Ix)); Ix++) {
		...
		if (Dp->InAdr.s_addr && !(Dp->Flags & IFF_LOOPBACK) && Dp->state == IF_STATE_DOWNSTREAM) {
			my_log(LOG_DEBUG, 0, "Joining all-routers group %s on vif %s", inetFmt(allrouters_group, s1), inetFmt(Dp->InAdr.s_addr, s2));

			//k_join(allrouters_group, Dp->InAdr.s_addr);
			joinMcGroup(getMcGroupSock(), Dp, allrouters_group);

			my_log(LOG_DEBUG, 0, "Joining all igmpv3 multicast routers group %s on vif %s", inetFmt(alligmp3_group, s1), inetFmt(Dp->InAdr.s_addr, s2));
	...
	if (croute != NULL) {
		...
		if (originAddr > 0) {
			...
			if (i == MAX_ORIGINS) {
				...
				my_log(LOG_WARNING, 0, "Too many origins for route %s; replacing %s with %s", inetFmt(croute->group, s1), inetFmt(croute->originAddrs[i], s2), inetFmt(originAddr, s3));
			}
			// set origin
...
void logRouteTable(const char *header)
{
	...
	if (croute == NULL) {
		...
		do {
			...
			my_log(LOG_DEBUG, 0, "#%d: %sDst: %s, Age:%d, St: %c, OutVifs: 0x%08x", rcount, src, inetFmt(croute->group, s2), croute->ageValue, st, croute->vifBits);
