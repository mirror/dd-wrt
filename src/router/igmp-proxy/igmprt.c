...
void igmp_add_membership(igmp_router_t *igmprt, igmp_group_t *newgrp)
{
	...
	mreq.imr_address.s_addr   = htonl(igmprt->upstream_interface->ip_addr.s_addr);
	if (setsockopt(igmprt->upstream_interface->socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&mreq, sizeof(mreq)) < 0) {
		log(LOG_INFO, "setsockopt error: IP_ADD_MEMBERSHIP\n");
		return;
	}

	ifp = (igmp_interface_t *)list_get_head((set_t **)&igmprt->network_if_list);
	while (ifp != NULL) {
		...
		while (grp != NULL) {
			if (grp->group_addr== newgrp->group_addr) { //group exist
	...
	if (setsockopt(igmprt->igmp_socket, IPPROTO_IP, MRT_ADD_MFC, (char *)&mfcctrl, sizeof(mfcctrl)) < 0) {
...
void igmp_drop_membership(igmp_router_t *igmprt, igmp_group_t *igmpgrp, igmp_interface_t *ptr_add_net, int  both_flag)
{
	igmp_interface_t	*ifp;
	igmp_group_t		*grp;
	...
	while (ifp != NULL) {
		...
		grp = (igmp_group_t *)list_get_head((set_t **)&ifp->group_database);
		while (grp != NULL) {
			if (grp->group_addr == igmpgrp->group_addr) { //group exist
				network_list[ifp->vif_index] = 1;
				drop_membership = 0;
                                /* ******* zg 2007-01-04 porting WAG54Gv3 Source code ******* */
                                /* ******* To fix cdrouter v3.4 item 295(cdrouter_mcast_11) bug ******* */
                                if(strcmp(ifp->name, igmprt->ptr_bothstream_interface[0]->name) == 0)
                                {
                                    no_drop_num_0 = 0;
                                }
                                else
                                if(strcmp(ifp->name, igmprt->ptr_bothstream_interface[1]->name) == 0)
                                {
                                    no_drop_num_1 = 0;
                                }
	...
	if (drop_membership) {
                ...
                if(both_flag)
                {
                    if(no_drop_num_0 && no_drop_num_1)
                    {
                        ...
                        {
                            mreq.imr_ifindex                = igmprt->ptr_bothstream_interface[1]->if_index;
                            mreq.imr_multiaddr.s_addr       = igmpgrp->group_addr;
                            mreq.imr_address.s_addr         = htonl(igmprt->ptr_bothstream_interface[1]->ip_addr.s_addr);
                            if(setsockopt(igmprt->ptr_bothstream_interface[1]->socket, IPPROTO_IP, IP_DROP_MEMBERSHIP,
                        ...
                        {
                            mreq.imr_ifindex                = igmprt->ptr_bothstream_interface[0]->if_index;
                            mreq.imr_multiaddr.s_addr       = igmpgrp->group_addr;
                            mreq.imr_address.s_addr         = htonl(igmprt->ptr_bothstream_interface[0]->ip_addr.s_addr);
                            if(setsockopt(igmprt->ptr_bothstream_interface[0]->socket, IPPROTO_IP, IP_DROP_MEMBERSHIP,
                                      (void*)&mreq, sizeof(mreq)) < 0)
                            {
                                 return;
                            }                                                               
		...
		mreq.imr_multiaddr.s_addr = igmpgrp->group_addr;
		mreq.imr_address.s_addr   = htonl(igmprt->upstream_interface->ip_addr.s_addr);
		if (setsockopt(igmprt->upstream_interface->socket, IPPROTO_IP, IP_DROP_MEMBERSHIP,
	    	 			(void *)&mreq, sizeof(mreq)) < 0) {
		...
		if (setsockopt(igmprt->igmp_socket, IPPROTO_IP, MRT_DEL_MFC, (char *)&mfcctrl, sizeof(mfcctrl)) < 0) {
		...
		if (setsockopt(igmprt->igmp_socket, IPPROTO_IP, MRT_ADD_MFC, (char *)&mfcctrl, sizeof(mfcctrl)) < 0) {
			...
			return;
                ...
                if(both_flag)
                {
                    if(ptr_add_net == igmprt->ptr_bothstream_interface[0])
                    {
                        mfcctrl.mfcc_parent = igmprt->ptr_bothstream_interface[1]->vif_index;
                    }
                    else
                    if(ptr_add_net == igmprt->ptr_bothstream_interface[1])
                    {
                        mfcctrl.mfcc_parent = igmprt->ptr_bothstream_interface[0]->vif_index;
                    }
                }
                else
                {
 ...
 */
void igmprt_send_query(igmp_router_t *igmprt, igmp_interface_t *ifp, unsigned char max_resp_time, unsigned int gaddr)
{
	...
	num_bytes = sendto(ifp->socket, (char *)&igmphdr, sizeof(igmphdr), 0, (struct sockaddr *)&to, sizeof(to));
	if (num_bytes < 0)
 ...
 */
void igmprt_received_v1_report(igmp_router_t *igmprt, igmp_interface_t *ifp, struct igmphdr *igmp)
{
	igmp_group_t *grp;
...
#else
		grp->group_timer	= ifp->general_query_timer + igmprt->config.igmp_gmi;
		grp->v1_host_timer	= ifp->general_query_timer + igmprt->config.igmp_gmi;
#endif
		grp->rtx_timer = 0;
	...
	if (grp->state == IGMP_CHECKING_MEMBERSHIP) {
		list_cat((set_t **)&igmprt->free_query_list,(set_t **)&grp->query_list);
		grp->query_list = NULL;
	}
 ...
 */
void igmprt_received_v2_report(igmp_router_t *igmprt, igmp_interface_t *ifp, struct igmphdr *igmp)
{
	igmp_group_t *grp;
        /* ******* zg 2007-01-04 porting WAG54Gv3 Source code ******* */
        /* ******* To fix cdrouter v3.4 item 295(cdrouter_mcast_11) bug ********/
...
#if 1 //fixed by crazy 20070614 - bug id unknown(20070614)
//Maybe we must return here...
	if (LOCAL_MCAST(igmp->group))
		return;
#ifdef IGMP_BLOCK_SOME_SPEC_ADDRS
	if(is_spec_addr(igmp->group))
        ...
        if( (ifp->mode == BOTHSTREAM_INTERFACE || ifp->mode == UPSTREAM_INTERFACE)
          && ((igmp->group) & htonl(0xf0000000)) == htonl(0xe0000000)
          && ((igmp->group & htonl(0xf0ffff00)) != htonl(0xe0ffff00)))
        {
            ...
            if(ifp == igmprt->ptr_bothstream_interface[0])                                                                   
            {
                 ...
                 ptr_net=(igmp_interface_t *)list_get_head((set_t **)&igmprt->network_if_list);
                 while (ptr_net != NULL)
                 {
                     ...
                     ptr_net = (igmp_interface_t *) list_get_next((set_t *)ptr_net);
                     continue;
                 }
...
#if 0 //moved by crazy 20070614 - Please see above...
	if (LOCAL_MCAST(igmp->group))
		return;
#ifdef IGMP_BLOCK_SOME_SPEC_ADDRS
	if(is_spec_addr(igmp->group))
	...
	if (grp->state == IGMP_CHECKING_MEMBERSHIP) {
		...
		if(1 == grp->sent_packets)
		{// it has sent one query packet...
			list_cat((set_t **)&igmprt->free_query_list, (set_t **)&grp->query_list);
			grp->receive_state = RECEIVE_FIRST;
			return;
		}
		else if(IGMP_DEF_RV <= grp->sent_packets)
		{// it has sent two or more than two query packets...
			if(RECEIVE_NONE == grp->receive_state)
			{
				list_cat((set_t **)&igmprt->free_query_list, (set_t **)&grp->query_list);
				grp->receive_state = RECEIVE_SECOND;
			}
			else 
			{
				grp->receive_state = RECEIVE_BOTH;
			}
		}
		else 
		{// no query packet has been sent...
#endif
		list_cat((set_t **)&igmprt->free_query_list, (set_t **)&grp->query_list);
#ifdef IGMP_SEND_THE_SECOND_QUERY_PACKET_AFTER_RECEIVE_ONE
		}
 ...
 */
void igmprt_received_leave(igmp_router_t *igmprt, igmp_interface_t *ifp, struct igmphdr *igmp)
{
	igmp_group_t *grp;
	igmpq_t *q;
	int index = 0;
	...
	for (index = 0; index < (igmprt->config.igmp_lmqc); index++) {
#else
	...
	//grp->group_timer = igmprt->config.igmp_lmqi * igmprt->config.igmp_lmqc;
        grp->group_timer = igmprt->config.igmp_lmqi * igmprt->config.igmp_lmqc - 1;
#ifdef IGMP_SEND_THE_SECOND_QUERY_PACKET_AFTER_RECEIVE_ONE
	grp->receive_state = RECEIVE_NONE;
	grp->sent_packets = 0;
#endif
	return;
}
 ...
 */
void igmprt_received_general_query(igmp_router_t *igmprt, igmp_interface_t *ifp, struct iphdr *ip)
{
	if (ip->saddr < ifp->ip_addr.s_addr) { //non-querier
		ifp->is_querier = 0;
#if 0
		ifp->querier_present_timer = igmprt->config.igmp_oqp;
#else
		ifp->querier_present_timer = ifp->general_query_timer + igmprt->config.igmp_oqp;
#endif
 ...
 */
void igmp_read_data(igmp_router_t *igmprt)
{
	int			num, len;
	struct iphdr		*ip;
	struct igmphdr		*igmp;
...
igmp_group_t* igmp_group_timer_expire(igmp_router_t *igmprt, igmp_interface_t *ifp, igmp_group_t* grp, int both_flag)
{
 ...
 */
void igmp_v1_host_timer_expire(igmp_router_t *igmprt, igmp_interface_t *ifp, igmp_group_t *grp)
{
	if (grp->state != IGMP_V1_MEMBERS_PRESENT)
		return;
...
igmp_group_t* igmp_rtx_timer_expire(igmp_router_t *igmprt, igmp_interface_t *ifp, igmp_group_t *grp, int both_flag)
{
	igmpq_t *q;
	igmp_group_t *nextgrp;
...
#ifdef IGMP_SEND_THE_SECOND_QUERY_PACKET_AFTER_RECEIVE_ONE
		if(IGMP_DEF_RV > grp->sent_packets)
		{
			grp->sent_packets++;
		}
#endif
...
#ifdef IGMP_SEND_THE_SECOND_QUERY_PACKET_AFTER_RECEIVE_ONE
		if(IGMP_DEF_RV <= grp->sent_packets 
			&& RECEIVE_NONE != grp->receive_state)
		{
			grp->state = IGMP_MEMBERS_PRESENT;
			return grp;
		}
 ...
 */
void igmp_timer_expire(igmp_router_t *igmprt)
{
	igmp_interface_t *ifp;
	igmp_group_t *grp;	
        /* ******* zg 2007-01-04 porting WAG54Gv3 Source code ******* */
	...
	while (ifp != NULL) {
		...
		if (ifp->is_querier == 1) {
			if (ifp->general_query_timer <= 0) {
				igmprt_send_query(igmprt, ifp, igmprt->config.igmp_qri*10, 0);
		...
		grp = (igmp_group_t*)list_get_head((set_t**)&ifp->group_database);
		while (grp != NULL) {
			if (grp->state < IGMP_NO_MEMBERS_PRESENT || grp->state > IGMP_CHECKING_MEMBERSHIP)
				return;
			...
			if (grp->state == IGMP_MEMBERS_PRESENT  || grp->state == IGMP_V1_MEMBERS_PRESENT) {
				if (grp->group_timer <= 0) {
                                        /* ******* zg 2007-01-04 porting WAG54Gv3 Source code ******* */
                                        /* ******* To fix cdrouter v3.4 item 295(cdrouter_mcast_11) bug ******* */
					//grp = igmp_group_timer_expire(igmprt, ifp, grp);
                                        grp = igmp_group_timer_expire(igmprt, ifp, grp, both_flag);
				}
				else {
...
int main(int argc, char *argv[])
{
	...
	if (upsname == NULL)
		show_usage();
#else
        if(ptr_bothstream_ifaceName[0] == NULL &&  ptr_bothstream_ifaceName[1] == NULL)
	...
	strcpy (router.config.bothstream_ifaceName[0], ptr_bothstream_ifaceName[0]);
        strcpy (router.config.bothstream_ifaceName[1], ptr_bothstream_ifaceName[1]);
	...
	i = 1;
   	if (setsockopt(router.igmp_socket, IPPROTO_IP, MRT_INIT, (char *)&i, sizeof(int)) < 0) {
		log(LOG_INFO, "setsockopt error: MRT_INIT\n");
		return -1;
	}
	i = 1;
	setsockopt(router.igmp_socket, SOL_SOCKET, SO_REUSEADDR, (void *)&i, sizeof(i));
	i = 1;
	setsockopt(router.igmp_socket, IPPROTO_IP, IP_PKTINFO, (void *)&i, sizeof(i));
	i = 1;
	setsockopt(router.igmp_socket, IPPROTO_IP, IP_MULTICAST_TTL, (void *)&i, sizeof(i));
	i = 0;
	setsockopt(router.igmp_socket, IPPROTO_IP, IP_MULTICAST_LOOP, (void *)&i, sizeof(i));
...
int igmprt_init(igmp_router_t *igmprt)
{
	...
	for (cp = buffer; cp < limit; cp = cp + sizeof(ifr->ifr_name) + sizeof(ifr->ifr_ifru)) {
		ifr = (struct ifreq *)cp;
		sin = (struct sockaddr_in *)&ifr->ifr_ifru.ifru_addr;
                /* ******* zg 2007-01-04 porting WAG54Gv3 Source code ******* */
                /* ******* To fix cdrouter v3.4 item 295(cdrouter_mcast_11) bug ******* */
                if(strcmp(ifr->ifr_name, igmprt->config.bothstream_ifaceName[0]) !=0
                   && strcmp(ifr->ifr_name, igmprt->config.bothstream_ifaceName[1]) != 0)
                {
		...
		if (ioctl(sock, SIOCGIFNETMASK, (void *)&ifreq) < 0) //can't get netmask
			continue;
		memcpy((void *)&netmask, (void *)&ifreq.ifr_netmask, sizeof(netmask));
		...
		if (flags & IFF_POINTOPOINT) {
			...
			if (ioctl(sock, SIOCGIFDSTADDR,(void *)&ifreq) < 0)
				...
				memcpy((void *)&dstaddr, (void *)&ifreq.ifr_dstaddr, sizeof(struct sockaddr));
		}
		else {
			strcpy(ifreq.ifr_name, ifr->ifr_name);
		...
		ifp->if_index = index;
		memcpy((void *)&ifp->ip_addr, (void *)&sin->sin_addr, sizeof(struct in_addr));
		ifp->flags = flags;
		ifp->is_querier = 1;
		ifp->general_query_timer = 0;
		...
		memcpy((void *)&ifaddr, (void *)&ifp->ip_addr, sizeof(struct in_addr));
		setsockopt(ifp->socket, IPPROTO_IP, IP_MULTICAST_IF, (void *)&ifp->ip_addr, sizeof(ifaddr));
                /* ******* zg 2007-01-04 porting WAG54Gv3 Source code ******* */
                /* ******* To fix cdrouter v3.4 item 295(cdrouter_mcast_11) bug******* */
#if 0
		...
		if (upstream == 1) {
			...
			if (setsockopt(ifp->socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&mreq, sizeof(mreq)) < 0)
				log(LOG_INFO, "setsockopt error: IP_ADD_MEMBERSHIP\n"); 
		}
#else
                if(bothstream[0])
...
the ALL router club. */
		...
		if (setsockopt(igmprt->igmp_socket, IPPROTO_IP, MRT_ADD_VIF, (char *)&vifctl, sizeof(vifctl)) < 0) {
 ...
 */
void igmp_interface_cleanup(igmp_router_t *igmprt, igmp_interface_t *ifp)
{
	igmp_group_t *grp;
	struct ifreq ifr;
	struct vifctl vifctl;
	...
	if (setsockopt(igmprt->igmp_socket, IPPROTO_IP, MRT_DEL_VIF, (char *)&vifctl, sizeof(vifctl)) < 0)
...
void igmprt_stop(igmp_router_t *igmprt)
{
	...
	ifp = (igmp_interface_t *)list_remove((set_t **)&igmprt->network_if_list);
	while (ifp != NULL) {
		if (ifp->mode == UPSTREAM_INTERFACE) {
			ifp = (igmp_interface_t *)list_remove((set_t **)&igmprt->network_if_list);
			continue;
	...
	if (setsockopt(igmprt->igmp_socket, IPPROTO_IP, MRT_DONE, (char *)NULL, 0) < 0)
 ...
 */
igmp_group_t* igmprt_group_lookup(igmp_interface_t *ifp, unsigned int gaddr)
{
	igmp_group_t *grp;
