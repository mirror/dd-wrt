 ...
 */
igmp_interface_t *
igmp_interface_create(struct in_addr ifaddr, char *ifname, vifi_t index, int itype)
{
    igmp_interface_t *ifp;
    struct ip_mreq  mreq;
    u_char i;
			 ...
			 igmp_interface_t * ifp, struct in_addr groupaddr)
{
	...
	for (ifp = router->igmprt_interfaces; ifp; ifp = ifp->igmpi_next) {
		...
		if (VALID_ADDR(mreq.imr_multiaddr)) {	/* only if the
			...
			if (!interface_lo) {
#ifdef HND_FIX
			    mcast_filter_add(ifp->igmpi_name, groupaddr);
#else /* HND_FIX */
			    sprintf(cmd,
...
igmprt_interface_add(igmp_router_t * igmprt,
		     struct in_addr ifaddr, char *ifname, vifi_t index,int itype)
{
    igmp_interface_t *ifp;
     ...
     */
    if ((ifp = igmp_interface_create(ifaddr, ifname, index, itype))) {
...
#endif
{
	...
	if (ifp->type==DOWNSTREAM)
	    fprintf(stderr,"adding downstream interface %s\n",ifp->ifl_name);
	else
	    fprintf(stderr,"adding upstream interface %s\n",ifp->ifl_name);
	    
	igmprt_interface_add(&router, psin->sin_addr, ifp->ifl_name,
			     ifp->ifl_index,ifp->type);
