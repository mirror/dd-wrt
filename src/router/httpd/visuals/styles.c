...
EJ_VISIBLE void ej_show_styles(webs_t wp, int argc, char_t **argv)
{
	...
	while ((entry = readdir(directory)) != NULL) {
		...
		websWrite(wp, "<option value=\"%s\" %s>%s</option>\n", entry->d_name,
			  nvram_match("router_style", entry->d_name) ? "selected=\"selected\"" : "", entry->d_name);
	}
	closedir(directory);
	return;
}
...
{
	if (nvram_match("router_style", "kromo") || nvram_match("router_style", "brainslayer") ||
	    nvram_match("router_style", "wikar") || nvram_match("router_style", "xirian"))
		return;
	char path[128];
	size_t len = 0;
