...
EJ_VISIBLE void ej_getwirelessmode(webs_t wp, int argc, char_t **argv)
{
	...
	if (nvram_match(mode, "wdsap_mtik"))
		websWrite(wp, "Capture(wl_basic.wdsap_mtik)");
...
EJ_VISIBLE void ej_getwirelessnetmode(webs_t wp, int argc, char_t **argv)
{
	...
	if (nvram_match(netmode, "disabled") || nvram_match(mode, "disabled"))
		...
		if (is_morse_micro(ifname)) {
			...
			if (nvram_match(netmode, "be6-only"))
				websWrite(wp, "Capture(wl_basic.be6)");
			if (nvram_match(netmode, "be5-only"))
				websWrite(wp, "Capture(wl_basic.be5)");
			if (nvram_match(netmode, "beax6-only"))
				websWrite(wp, "Capture(wl_basic.beax6)");
			if (nvram_match(netmode, "beax5-only"))
				websWrite(wp, "Capture(wl_basic.beax5)");
			if (nvram_match(netmode, "mixed5"))
				websWrite(wp, "Capture(wl_basic.mixed5)");
