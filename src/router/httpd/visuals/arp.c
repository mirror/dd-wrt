...
EJ_VISIBLE void ej_dumparptable(webs_t wp, int argc, char_t **argv)
{
	...
	if ((f = fopen("/proc/net/arp", "r")) != NULL) {
		while (fgets(buf, sizeof(buf), f)) {
			...
			if (!strcmp(hostname, "*") && nvram_matchi("dhcpd_usenvram", 0)) {
			...
			if (!strcmp(hostname, "*") && nvram_matchi("dhcpd_usenvram", 1)) {
			...
			for (i = 0; i < len; i++)
				mac[i] = toupper(mac[i]);
			websWrite(wp, "%c'%s','%s','%s','%d', '%s','0','0','0'", (count ? ',' : ' '), hostname, ip, mac, conn_count,
				  landev);
			++count;
			conn_count = 0;
		}
...
static int addtable(struct arptable **tbl, char *mac, char *ip, char *ifname, long long in, long long out, long long total,
		    int *tablelen)
{
...
EJ_VISIBLE void ej_dumparptable(webs_t wp, int argc, char_t **argv)
{
	...
	if ((f = fopen("/tmp/report.tmp", "r")) != NULL) {
		...
		for (i = 0; i < tablelen; i++) {
			if (!table[i].mark) {
				websWrite(wp, "%c'%s','%s','%s','%d', '%s','%lld','%lld','%lld'", (count ? ',' : ' '),
					  table[i].hostname, table[i].ip, table[i].mac, table[i].conncount, table[i].ifname,
