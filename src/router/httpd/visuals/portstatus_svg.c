...
static void show_svg(webs_t wp, int speed, int fd, char *color, char *shadow)
{
	char spd[32];
	if (!speed)
		sprintf(spd, "Down");
	...
	websWrite(wp, "<img src=\"/port.svg?%s,%s,%s\" width=\"%d%%\" heigth=\"%d%%\">", spd, color, shadow, 100, 25 / MAXCOL);
}
