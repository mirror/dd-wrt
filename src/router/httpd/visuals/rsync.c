...
EJ_VISIBLE void ej_rsync_sharepaths(webs_t wp, int argc, char_t **argv)
{
	...
	websWrite(
		wp,
		"<input type=\"hidden\" name=\"rsync_shares_count\" id=\"rsync_shares_count\" value=\"%d\">\n",
		rows);
	rows = 5;

	websWrite(
		wp,
		"<input type=\"hidden\" name=\"rsync_shares_count_limit\" id=\"rsync_shares_count_limit\" value=\"%d\">\n",
		rows);
	rows = 0;
	...
	websWrite(
		wp,
		"<table id=\"rsync_shares\" class=\"table\" summary=\"rsync share table\">\n");
	...
	for (cs = rsyncshares; cs; cs = csnext) {
		if (rows == 0) {
			// dummy entry
			sprintf(buffer, "%s",
				"id=\"rsync_shares_row_template\" style=\"display: none;\"");
			//sprintf(number, '\0');
			number[0] = '\0';
		} else {
		...
		websWrite(
			wp,
			"<td id=\"n_rsync_mp%s\" style=\"width: 17.816em;\"><select name=\"rsyncshare_mp%s\" id=\"rsyncshare_mp%s\" style=\"width: 100%%;\" >\n",
