		...
		if (si->isi_rates && ((si->isi_rates[si->isi_txrate] & IEEE80211_RATE_VAL) != 0)
			...
			websWrite(wp,
				  ...
				  ((si->isi_rates[si->isi_txrate] &
				    IEEE80211_RATE_VAL) / 2) * turbo, ((si->isi_rates[si->isi_rxrate] & IEEE80211_RATE_VAL) / 2) * turbo, si->isi_noise + si->isi_rssi + bias, si->isi_noise + bias, si->isi_rssi, qual, nvram_nget("%s_label", ifname),ifname);
		} else {
			websWrite(wp, "'%s','%s','%s%s','%s','N/A','N/A','N/A','%d','%d','%d','%d','0','0','0','0','%s','%s'", mac, si->radioname, type, ifname, UPTIME(si->isi_uptime, str), si->isi_noise + si->isi_rssi + bias,
				  si->isi_noise + bias, si->isi_rssi, qual, nvram_nget("%s_label", ifname),ifname);
		}
		bufcount += si->isi_len;
		cp += si->isi_len;
		len -= si->isi_len;
...
extern int active_wireless_if_ath9k(webs_t wp, int argc, char_t ** argv, char *ifname, int *cnt, int globalcnt, int turbo, int macmask);
#endif
			...
			foreach(var, vifs, next) {
				if (!is_mac80211(devs)) {
					...
					if (nvram_nmatch("1", "%s_owe", var)) {
						...
						global = active_wireless_if_ath9k(wp, argc, argv, var, &assoc_count[cnt], global, t, macmask);
				}
				cnt++;
			}
		}
