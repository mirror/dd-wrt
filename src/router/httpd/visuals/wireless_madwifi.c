	...
	do {
		...
		*cnt = (*cnt) + 1;
		...
		if (si->isi_rates && ((si->isi_rates[si->isi_txrate] & IEEE80211_RATE_VAL) != 0) &&
			...
			websWrite(wp, "'%s','%s','%s%s','%s','%3dM','%3dM','N/A','%d','%d','%d','%d','0','0','0','0','0','0','0','0','%s','%s'",
			...
			websWrite(wp, "'%s','%s','%s%s','%s','N/A','N/A','N/A','%d','%d','%d','%d','0','0','0','0','0','0','0','0','%s','%s'", mac,
				  si->radioname, type, ifname, UPTIME(si->isi_uptime, str, sizeof(str)),
				  si->isi_noise + si->isi_rssi + bias, si->isi_noise + bias, si->isi_rssi, qual,
				  nvram_nget("%s_label", ifname), ifname);
		}
		bufcount += si->isi_len;
