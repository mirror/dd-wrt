...
void show_cidr(webs_t wp, char *prefix, char *var, int nm, char *type,
	       char *nmtype, char *nmname);
void show_ip(webs_t wp, char *prefix, char *var, int nm, int invalid,
	     char *type);
void show_caption(webs_t wp, const char *class, const char *cap,
		  const char *ext);
void show_caption_simple(webs_t wp, const char *cap);

static void show_oet_checkbox(webs_t wp, const char *label, const char *fmt,
			      int id, int def, const char *ext)
{
	char temp[64];
	...
	websWrite(wp, "<div class=\"setting\">\n");
	{
		...
		websWrite(
			wp,
			"<input class=\"spaceradio\" type=\"checkbox\" name=\"%s\" value=\"1\" %s %s/>\n",
			temp,
			(nvram_default_matchi(temp, 1, def) ?
				 "checked=\"checked\"" :
				 ""),
			attrib);
	}
...
static void show_oet_checkbox_peer(webs_t wp, const char *label,
				   const char *fmt, int id, int peer, int def,
				   const char *ext)
{
	char temp[64];
...
static void show_oet_radio(webs_t wp, const char *label, const char *fmt,
			   int id, int def, const char *exttrue,
			   const char *extfalse)
{
	char temp[64];
	...
	websWrite(wp, "<div class=\"setting\">\n");
	{
		...
		websWrite(
			wp,
			"<input class=\"spaceradio\" type=\"radio\" value=\"1\" name=\"%s\" %s %s />",
			temp, attrib1,
			(nvram_default_matchi(temp, 1, def) ?
				 "checked=\"checked\"" :
				 ""));
		...
		websWrite(
			wp,
			"<input class=\"spaceradio\" type=\"radio\" value=\"0\" name=\"%s\" %s %s />",
			temp, attrib2,
			(nvram_default_matchi(temp, 0, def) ?
				 "checked=\"checked\"" :
				 ""));
...
static void show_oet_radio_peer(webs_t wp, const char *label, const char *fmt,
				int id, int peer, int def, const char *exttrue,
				const char *extfalse)
{
	char temp[64];
...
static void show_oet_num(webs_t wp, const char *label, int size, int maxlength,
			 int def, int id, const char *ext, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	char *buf;
	...
	websWrite(wp, "<div class=\"setting\">\n");
	{
		...
		if (def != -1)
			websWrite(
				wp,
				"<input size=\"%d\" maxlength=\"%d\" name=\"%s\" class=\"num\" value=\"%d\" %s/>\n",
				size, maxlength, buf,
				nvram_default_geti(buf, def), attrib);
		else
			websWrite(
				wp,
				"<input size=\"%d\" maxlength=\"%d\" name=\"%s\" class=\"num\" value=\"%s\" %s/>\n",
				size, maxlength, buf, nvram_safe_get(buf),
				attrib);
		free(buf);
	}
	websWrite(wp, "</div>\n");
...
static void show_oet_textfield(webs_t wp, int pass, const char *label, int size,
			       int maxlength, const char *def, int id,
			       const char *ext, const char *fmt, ...)
{
	...
	websWrite(wp, "<div class=\"setting\">\n");
	{
		...
		if (pass)
			websWrite(
				wp,
				"<input type=\"password\" size=\"%d\" maxlength=\"%d\" name=\"%s\" value=\"%s\" %s/>\n",
				size, maxlength, buf,
				nvram_default_get(buf, def), attrib);
		else
			websWrite(
				wp,
				"<input size=\"%d\" maxlength=\"%d\" name=\"%s\" value=\"%s\" %s/>\n",
				size, maxlength, buf,
				nvram_default_get(buf, def), attrib);
		free(buf);
	}
...
EJ_VISIBLE void ej_show_eop_tunnels(webs_t wp, int argc, char_t **argv)
{
	...
	for (tun = 1; tun < tunnels; tun++) {
		...
		snprintf(temp, sizeof(temp), "oet%d_label", tun);
		{
			...
			websWrite(
				wp,
				" %s - <input type=\"text\" size=\"14\" id=\"%s\" name=\"%s\" value=\"%s\" ></legend>\n",
				oet, temp, temp, nvram_safe_get(temp));
		}
		show_oet_radio(
			wp, "eoip.srv", "oet%d_en", tun, 0,
			"onclick=\"show_layer_ext(this, 'idoet%d', true)\"",
		...
		websWrite(wp, "<div id=\"idoet%d\">\n", tun);
		{
			...
			websWrite(wp, "<div class=\"setting\">\n");
			{
				...
				websWrite(
					wp,
					"<select name=\"%s\" onclick=\"changeproto(this, %d, this.value, %s, %s, %s)\">\n",
					temp, tun,
					nvram_nget("oet%d_bridged", tun),
					nvram_nget("vxlan%d_bridged", tun),
				...
				websWrite(
					wp,
					"document.write(\"<option value=\\\"0\\\" %s >\"  + eoip.etherip + \"</option>\");\n",
					nvram_matchi(temp, 0) ?
						"selected=\\\"selected\\\"" :
						"");
				websWrite(
					wp,
					"document.write(\"<option value=\\\"1\\\" %s >\"  + eoip.mtik + \"</option>\");\n",
					nvram_matchi(temp, 1) ?
						"selected=\\\"selected\\\"" :
						"");
				...
				websWrite(
					wp,
					"document.write(\"<option value=\\\"3\\\" %s >\"  + eoip.vxlan + \"</option>\");\n",
					nvram_matchi(temp, 3) ?
						"selected=\\\"selected\\\"" :
						"");
#endif
			...
			{
				show_oet_num(
					wp, "eoip.tunnelID", 4, 3, -1, tun,
					"onblur=\"valid_range(this,0,65535,eoip.tunnelID)\"",
					"oet%d_id", tun);
			}
			websWrite(wp, "</div>\n");
			...
			{
				show_oet_num(
					wp, "eoip.tunnelID", 8, 8, -1, tun,
					"onblur=\"valid_range(this,0,16777215,eoip.tunnelID)\"",
					"oet%d_id", tun);
			}
			websWrite(wp, "</div>\n");
			...
			{
#ifdef HAVE_WIREGUARD
				show_oet_checkbox(wp, "service.vpn_mit",
						  "oet%d_mit", tun, 0, NULL);
				show_oet_checkbox(wp,
						  "eoip.wireguard_oet_natout",
						  "oet%d_natout", tun, 0, NULL);
				show_oet_radio(
					wp, "eoip.wireguard_obfuscation",
					"oet%d_obf", tun, 0,
					"onclick=\"show_layer_ext(this, 'idoet%d_showobf', true)\"",
					"onclick=\"show_layer_ext(this, 'idoet%d_showobf', false)\"");
				websWrite(
					wp, "<div id=\"idoet%d_showobf\">\n",
					tun); //for show or hide advanced options
				{
					show_oet_textfield(
						wp, 1, "share.key", 32, 32, "",
						tun,
				...
				show_oet_num(wp, "eoip.wireguard_localport", 5,
					     5, 51820, tun, NULL, "oet%d_port",
					     tun);

				{
					...
					mtu -= nvram_matchi("ipv6_enable", 1) ?
						       80 :
						       60;
					show_oet_num(wp, "idx.mtu", 5, 5, mtu,
						     tun, NULL, "oet%d_mtu",
						     tun);
				}
				...
				//public key show
				show_oet_textfield(wp, 0,
						   "eoip.wireguard_localkey",
						   48, 48, "", tun,
						   "readonly=\"readonly\"",
						   "oet%d_public", tun);
					...
					tun); //for show or hide advanced options
				{
					...
					show_oet_textfield(
						wp, 1,
						"eoip.wireguard_localprivatekey",
						48, 48, "", tun,
						"onmouseover=\"this.type=\'text\'\" onmouseout=\"this.type=\'password\'\"",
						"oet%d_private", tun);
						  ...
						  "<div class=\"setting\">\n");
					{
						...
						websWrite(
							wp,
							"<select name=\"%s\" onclick=\"changespbr(this, %d, this.value)\">\n",
							temp, tun);
						websWrite(
							wp,
							"<script type=\"text/javascript\">\n//<![CDATA[\n");
					...
					{ //egc: SPBR input box
						show_oet_textfield(
							wp, 0,
							"eoip.wireguard_oet_spbr_ip",
							78, 1024, "", tun, NULL,
							"oet%d_spbr_ip", tun);
						show_oet_checkbox(
							wp,
							"eoip.wireguard_dnspbr",
							"oet%d_dnspbr", tun, 0,
							"onclick=\"changedns46(this, %d)\"");
						websWrite(
							wp,
							"<div id=\"idoet%d_dns46\">\n",
						  ...
						  "<div class=\"setting\">\n");
					{
						...
						websWrite(
							wp,
							"<select name=\"%s\" onclick=\"changedpbr(this, %d, this.value)\">\n",
							temp, tun);
						websWrite(
							wp,
							"<script type=\"text/javascript\">\n//<![CDATA[\n");
					...
					websWrite(
						wp,
						"<div id=\"idoet%d_failgrp\">\n",
						tun); //show/hide failgrp
					{
						show_oet_radio(
							wp,
							"eoip.wireguard_failgrp",
							"oet%d_failgrp", tun, 0,
				...
				snprintf(temp, sizeof(temp), "oet%d_peers",
					 tun);
				...
				for (peer = 0; peer < peers; peer++) {
					...
					show_caption_legend(
						wp, "eoip.wireguard_cllegend");
					//Show/Hide Client Config
					show_oet_radio_peer(
						wp, "eoip.wireguard_cllegend",
						"oet%d_clconfig%d", tun, peer,
						0,
					...
					websWrite(
						wp,
						"<div id=\"idclconfig%d_peer%d\">\n",
						tun, peer);
					{
						show_oet_textfield(
							wp, 0,
							"eoip.wireguard_peerip",
							48, 128, "0.0.0.0/0",
							tun, NULL,
							"oet%d_clip%d", tun,
							peer);
						show_oet_textfield(
							wp, 0,
							"eoip.wireguard_peerdns",
							48, 128, "0.0.0.0", tun,
							NULL, "oet%d_cldns%d",
							tun, peer);
						show_oet_textfield(
							wp, 0,
							"eoip.wireguard_clend",
							40, 48, "", tun, NULL,
							"oet%d_clend%d", tun,
							peer);
						show_oet_num(
					...
					websWrite(wp, "</div>\n");
					snprintf(temp, sizeof(temp),
						 "oet%d_aip%d", tun, peer);
					websWrite(wp,
						  "<div class=\"setting\">\n");
					{
						show_caption(
							wp, "label",
							"eoip.wireguard_allowedips",
							NULL);
						websWrite(
							wp,
							"<input size=\"72\" maxlength=\"1024\" name=\"%s\" value=\"%s\" />\n",
							temp,
							nvram_default_get(
								temp,
								"0.0.0.0/0"));
					...
					show_oet_checkbox_peer(
						wp,
						"eoip.wireguard_route_allowedip",
						"oet%d_aip_rten%d", tun, peer,
						1, NULL);

					show_oet_num(wp, "eoip.wireguard_ka", 5,
						     5, -1, tun, NULL,
						     "oet%d_ka%d", tun, peer);
					//public key peer input
					show_oet_textfield(
						wp, 0, "eoip.wireguard_peerkey",
						48, 48, "", tun, NULL,
						"oet%d_peerkey%d", tun, peer);

					show_oet_radio_peer(
						wp, "eoip.wireguard_usepsk",
						"oet%d_usepsk%d", tun, peer, 0,
						"onclick=\"show_layer_ext(this, 'idpsk%d_peer%d', true)\"",
						"onclick=\"show_layer_ext(this, 'idpsk%d_peer%d', false)\"");

					websWrite(
						wp,
						"<div id=\"idpsk%d_peer%d\">\n",
						tun, peer);
					{
						websWrite(
							wp,
					...
					websWrite(wp, "</div>\n");
					int hasqr = 0;
					if (nvram_nexists("oet%d_peerpk%d", tun,
							  peer)) {
						char svgpath[64];
						char s_tun[32];
						char s_peer[32];
						...
						sprintf(confpath,
							"/tmp/wireguard/oet%d_peer%d_conf",
							tun, peer);
						eval("makewgconfig", s_tun,
						     s_peer);
						sprintf(svgpath,
							"/tmp/wireguard/oet%d_peer%d_svg",
							tun, peer);
						eval("qrencode", "-t", "svg",
						     "--rle", "-r", confpath,
						     "-o", svgpath);
						...
						tun); //naming for periodic refresh need also peer
					{
							...
							"<div class=\"setting\">\n");
						{
							...
							snprintf(
								command,
								sizeof(command),
								"/usr/bin/wireguard-state.sh %d %s 2>/dev/null",
								tun,
								nvram_safe_get(
									temp));
							FILE *in = popen(
							...
							if (in != NULL) {
								...
								if (len == 0) {
									...
									snprintf(
										...
										peer);
									sprintf(buffer,
										"No connection present for %s ",
										nvram_safe_get(
											temp3));
								}
							} else {
								sprintf(buffer,
									"Unable to retrieve WireGuardStatus oet%d",
									tun);
							}
							show_caption(
								wp, "label",
								"eoip.wireguard_oet_status",
								NULL);
							websWrite(
								wp,
					...
					websWrite(
						wp,
						"show_layer_ext(this, 'idclconfig%d_peer%d',%s);\n",
						tun, peer,
						nvram_nmatchi(
							1, "oet%d_clconfig%d",
							tun, peer) ?
					...
					websWrite(
						wp,
						"show_layer_ext(this, 'idpsk%d_peer%d',%s);\n",
						tun, peer,
						nvram_nmatchi(1,
							      "oet%d_usepsk%d",
							      tun, peer) ?
					...
					websWrite(
						wp,
						"show_layer_ext(this, 'idendpoint%d_peer%d',%s);\n",
						tun, peer,
						nvram_nmatchi(
							1, "oet%d_endpoint%d",
							tun, peer) ?
					...
					websWrite(
						wp,
						"show_layer_ext(this, 'idshowobf%d_peer%d',%s);\n",
						tun, peer,
						nvram_nmatchi(1, "oet%d_obf%d",
							      tun, peer) ?
							"true" :
							"false");
					websWrite(
			...
			websWrite(wp, "<div id=\"idlocalip%d\">\n", tun);
			{
				websWrite(wp, "<div class=\"setting\">\n");
				{
					...
					websWrite(
						wp,
						"<input type=\"hidden\" name=\"oet%d_local\" value=\"0.0.0.0\"/>\n",
						tun);
					snprintf(temp, sizeof(temp),
						 "oet%d_local", tun);
					show_ip(wp, NULL, temp, 1, 0,
						"eoip.localIP");
				}
				websWrite(wp, "</div>\n");
				websWrite(wp, "<div class=\"setting\">\n");
			...
			{
				show_oet_radio(
					wp, "eoip.mcast", "oet%d_mcast", tun, 0,
			...
			{
				show_oet_textfield(wp, 0, "eoip.mcast_group",
						   48, 128, "", tun, NULL,
						   "oet%d_group", tun);
			}
			websWrite(wp, "</div>\n");
			...
			{
				show_oet_textfield(wp, 0, "eoip.remoteIP", 48,
						   128, "", tun, NULL,
						   "oet%d_remip6", tun);
			}
			websWrite(wp, "</div>\n");
			...
			{
				show_oet_textfield(wp, 0, "eoip.localIP", 48,
						   128, "", tun, NULL,
						   "oet%d_localip6", tun);
			}
			websWrite(wp, "</div>\n");
			websWrite(wp, "<div id=\"idvxlansettings%d\">\n", tun);
			{
				...
				websWrite(wp, "<div class=\"setting\">\n");
				{
					...
					websWrite(
						wp,
						"<option value=\"%s\" %s >LAN &amp; WLAN</option>\n",
						nvram_safe_get("lan_ifname"),
						nvram_match("lan_ifname", cmp) ?
							"selected=\"selected\"" :
							"");
					if (strcmp(wanface, "br0")) {
						websWrite(
							wp,
							"<option value=\"%s\" %s >WAN</option>\n",
							wanface,
							!strcmp(wanface, cmp) ?
								"selected=\"selected\"" :
								"");
				...
				websWrite(wp, "</div>\n");
				show_oet_num(wp, "eoip.dport", 5, 5, 4789, tun,
					     NULL, "oet%d_dstport", tun);
				...
				{
					show_caption(wp, "label",
						     "eoip.sportrange", NULL);
					websWrite(
						wp,
						"<input name=\"%s\" size=\"5\" maxlength=\"5\" class=\"num\" value=\"%s\" /> ~ <input name=\"%s\" size=\"5\" maxlength=\"5\" class=\"num\" value=\"%s\" />\n",
						min,
						nvram_default_get(min, "0"),
						max,
						nvram_default_get(max, "0"));
				}
				websWrite(wp, "</div>\n");
				show_oet_num(wp, "eoip.ttl", 5, 5, 0, tun, NULL,
					     "oet%d_ttl", tun);
				show_oet_num(wp, "eoip.tos", 5, 5, 0, tun, NULL,
					     "oet%d_tos", tun);

				show_oet_checkbox(wp, "eoip.lrn", "oet%d_lrn",
						  tun, 1, NULL);
				show_oet_checkbox(wp, "eoip.proxy",
						  "oet%d_proxy", tun, 0, NULL);
				show_oet_checkbox(wp, "eoip.rsc", "oet%d_rsc",
						  tun, 0, NULL);
				show_oet_checkbox(wp, "eoip.l2miss",
						  "oet%d_l2miss", tun, 0, NULL);
				show_oet_checkbox(wp, "eoip.l3miss",
						  "oet%d_l3miss", tun, 0, NULL);
				/* kernel 4.4 or higher */
				show_oet_checkbox(wp, "eoip.udpcsum",
						  "oet%d_udpcsum", tun, 0,
						  NULL);
				show_oet_checkbox(wp, "eoip.udp6zerocsumtx",
						  "oet%d_udp6zerocsumtx", tun,
						  0, NULL);
				show_oet_checkbox(wp, "eoip.udp6zerocsumrx",
						  "oet%d_udp6zerocsumrx", tun,
						  0, NULL);
				/* kernel 3.10 or higher */
				show_oet_checkbox(wp, "eoip.df", "oet%d_df",
						  tun, 0, NULL);

				show_oet_num(wp, "eoip.ageing", 5, 5, 300, tun,
					     NULL, "oet%d_ageing", tun);
				show_oet_num(wp, "eoip.flowlabel", 5, 5, -1,
					     tun, NULL, "oet%d_fl", tun);
				show_oet_radio(
					wp, "eoip.bridging", "vxlan%d_bridged",
					tun, 0,
			...
			{
				show_oet_radio(
					wp, "eoip.bridging", "oet%d_bridged",
					tun, 0,
			...
			websWrite(wp, "<div id=\"idbridged%d\">\n", tun);
			{
				websWrite(wp, "<div class=\"setting\">\n");
				{
					show_caption(wp, "label", "share.ip",
						     NULL);
					websWrite(
						wp,
			...
			websWrite(wp, "<div id=\"idvxlanbridged%d\">\n", tun);
			{
				websWrite(wp, "<div class=\"setting\">\n");
				{
					...
					websWrite(
						wp,
						"<input type=\"hidden\" name=\"vxlan%d_ipaddr\" value=\"0.0.0.0\"/>\n",
						tun);
					snprintf(temp, sizeof(temp),
						 "vxlan%d_ipaddr", tun);
					snprintf(temp2, sizeof(temp2),
			...
			{
				show_oet_textfield(wp, 0, "share.ipaddrmask",
						   48, 128, "", tun, NULL,
						   "oet%d_ipaddrmask", tun);
		...
		websWrite(
			wp,
			"toggle_layer_ext(document.eop.oet%d_en, 'idmcast%d', 'idremoteip6%d', %s);\n",
			tun, tun, tun,
			nvram_nmatchi(1, "oet%d_mcast", tun) ? "true" :
							       "false");
		websWrite(
			wp,
			"changeproto(document.eop.oet%d_proto, %d, %s, %s, %s, %s);\n",
			tun, tun, nvram_nget("oet%d_proto", tun),
			nvram_nget("oet%d_bridged", tun),
			nvram_nget("vxlan%d_bridged", tun),
			nvram_nmatchi(1, "oet%d_mcast", tun));
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d', %s);\n",
			tun, tun,
			nvram_nmatchi(1, "oet%d_en", tun) ? "true" : "false");
		//hide or show advanced settings
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_showadvanced',%s);\n",
			tun, tun,
			nvram_nmatchi(1, "oet%d_showadvanced", tun) ? "true" :
								      "false");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idshowmcast%d',%s);\n",
			tun, tun,
			nvram_nmatchi(3, "oet%d_proto", tun) ? "true" :
							       "false");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idremoteip6%d',%s);\n",
			tun, tun,
			nvram_nmatchi(3, "oet%d_proto", tun) ? "true" :
							       "false");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idlocalip6%d',%s);\n",
			tun, tun,
			nvram_nmatchi(3, "oet%d_proto", tun) ? "true" :
							       "false");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idvxlansettings%d',%s);\n",
			tun, tun,
			nvram_nmatchi(3, "oet%d_proto", tun) ? "true" :
							       "false");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_showobf',%s);\n",
			tun, tun,
			nvram_nmatchi(1, "oet%d_obf", tun) ? "true" : "false");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_tunnelstate', %s);\n",
			tun, tun,
			nvram_nmatchi(1, "oet%d_failgrp", tun) ? "true" :
								 "false");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_wdog', %s);\n",
			tun, tun,
			(nvram_nmatchi(1, "oet%d_failgrp", tun) ||
			 nvram_nmatchi(1, "oet%d_wdog", tun)) ?
				"true" :
				"false");
		...
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_wdog2', %s);\n",
			tun, tun,
			nvram_nmatchi(1, "oet%d_failgrp", tun) ? "false" :
								 "true");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_failgrp', %s);\n",
			tun, tun,
			nvram_nmatchi(1, "oet%d_wdog", tun) ? "false" : "true");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_spbr',%s);\n",
			tun, tun,
			nvram_nmatchi(0, "oet%d_spbr", tun) ? "false" : "true");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_dpbr',%s);\n",
			tun, tun,
			nvram_nmatchi(0, "oet%d_dpbr", tun) ? "false" : "true");
		websWrite(
			wp,
			"show_layer_ext(document.eop.oet%d_en, 'idoet%d_dns46',%s);\n",
			tun, tun,
			nvram_nmatchi(0, "oet%d_dnspbr", tun) ? "false" :
								"true");
		websWrite(wp, "//]]>\n</script>\n");
		websWrite(wp, "</fieldset><br>\n");
	}
