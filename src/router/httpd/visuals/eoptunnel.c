...
static void show_oet_textfield(webs_t wp, int pass, const char *label, int size, int maxlength, const char *def, int id, const char *ext, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	char *buf;
	...
	for (tun = 1; tun < tunnels; tun++) {
		...
		websWrite(wp, "<div id=\"idoet%d\">\n", tun);
		{
			...
			websWrite(wp, "<div class=\"setting\">\n");
			{
				...
				websWrite(wp, "<select name=\"%s\" onclick=\"changeproto(this, %d, this.value, %s, %s, %s)\">\n", temp, tun, nvram_nget("oet%d_bridged", tun), nvram_nget("vxlan%d_bridged", tun), nvram_nmatchi(1, "oet%d_mcast", tun));
			...
			{
				show_oet_num(wp, "eoip.tunnelID", 4, 3, -1, tun, "onblur=\"valid_range(this,0,65535,eoip.tunnelID)\"", "oet%d_id", tun);
			}
			websWrite(wp, "</div>\n");
			websWrite(wp, "<div id=\"idvxlan%d\">\n", tun);
			{
				show_oet_num(wp, "eoip.tunnelID", 8, 8, -1, tun, "onblur=\"valid_range(this,0,16777215,eoip.tunnelID)\"", "oet%d_id", tun);
			}
			websWrite(wp, "</div>\n");
			websWrite(wp, "<div id=\"idwireguard%d\">\n", tun);
			{
				...
				websWrite(wp, "<div id=\"idoet%d_showadvanced\">\n", tun);	//for show or hide advanced options
				{
					...
					//egc set private key
					show_oet_textfield(wp, 1, "eoip.wireguard_localprivatekey", 48, 48, "", tun, "onmouseover=\"this.type=\'text\'\" onmouseout=\"this.type=\'password\'\"", "oet%d_private", tun);
					...
					websWrite(wp, "<div id=\"idoet%d_failgrp\">\n", tun);	//show/hide failgrp
					{
						...
						websWrite(wp, "<div id=\"idoet%d_tunnelstate\">\n", tun);	//show or hide tunnel state only if failgrp is checked
						{
							...
							websWrite(wp, "<script type=\"text/javascript\">\n//<![CDATA[\n");
							websWrite(wp, "document.write(\"<option value=\\\"0\\\" %s >\"  + eoip.wireguard_standby + \"</option>\");\n",
								  nvram_matchi(temp, 0) ? "selected=\\\"selected\\\"" : "");
							websWrite(wp, "document.write(\"<option value=\\\"2\\\" %s >\"  + eoip.wireguard_running + \"</option>\");\n",
								  nvram_matchi(temp, 2) ? "selected=\\\"selected\\\"" : "");
							websWrite(wp, "document.write(\"<option value=\\\"1\\\" %s >\"  + eoip.wireguard_failed + \"</option>\");\n",
								  nvram_matchi(temp, 1) ? "selected=\\\"selected\\\"" : "");
							websWrite(wp, "//]]>\n</script>\n");
							websWrite(wp, "</select>\n");
							websWrite(wp, "</div>\n");
				...
				for (peer = 0; peer < peers; peer++) {
					...
					websWrite(wp, "<div id=\"idpsk%d_peer%d\">\n", tun, peer);
					{
						...
						websWrite(wp, "</div>\n");
						show_oet_textfield(wp, 0, "eoip.wireguard_psk", 48, 48, "", tun, NULL, "oet%d_psk%d", tun, peer);
		...
		websWrite(wp, "changeproto(document.eop.oet%d_proto, %d, %s, %s, %s, %s);\n", tun, tun, nvram_nget("oet%d_proto", tun), nvram_nget("oet%d_bridged", tun), nvram_nget("vxlan%d_bridged", tun), nvram_nmatchi(1, "oet%d_mcast", tun));
