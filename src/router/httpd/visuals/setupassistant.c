...
EJ_VISIBLE void ej_sas_make_time_list(webs_t wp, int argc, char_t **argv)
{
	...
	for (i = st; i <= en; i++) {
		...
		websWrite(wp, "<option value=\"%d\" %s >%02d</option>\n", i,
			  nvram_selmatch(wp, argv[0], ic) ? "selected=\"selected\"" : "", i);
	}
...
{
	int i;
	static char labels[5][32] = { "", "sas.internet_connection", "sas.network_settings", "sas.wireless_settings",
				      "sas.other_settings" };
	char *stage;
...
// RELAYD OPTIONAL SETTINGS
	...
	if (nvram_selmatch(wp, wl_mode, "wet")) {
		...
		websWrite(
			...
			"</div>\n",
			prefix, nvram_selmatch(wp, wl_relayd, "1") ? " style=\"display: none; visibility: hidden;\"" : "", prefix,
			prefix, ip[0], prefix, ip[1], prefix, ip[2], prefix, ip[3]);
	}
#endif
	...
	if (is_mac80211(prefix)) {
		if ((nvram_selnmatch(wp, "n-only", "%s_net_mode", prefix) ||
		     ...
		     nvram_selnmatch(wp, "n2-only", "%s_net_mode", prefix) || nvram_selnmatch(wp, "mixed", "%s_net_mode", prefix) ||
	...
	if (!is_mac80211(prefix) ||
	    (is_mac80211(prefix) &&
	     (nvram_selnmatch(wp, "n-only", "%s_net_mode", prefix) || nvram_selnmatch(wp, "ng-only", "%s_net_mode", prefix) ||
	      nvram_selnmatch(wp, "n2-only", "%s_net_mode", prefix) || nvram_selnmatch(wp, "n5-only", "%s_net_mode", prefix) ||
	      nvram_selnmatch(wp, "mixed", "%s_net_mode", prefix) || nvram_selnmatch(wp, "na-only", "%s_net_mode", prefix))))
#endif
	{
#if defined(HAVE_ATH9K)
		if (!is_mac80211(prefix) || can_ht40(prefix))
...
#ifdef HAVE_RT2880
	if (nvram_selmatch(wp, wl_mode, "ap") || nvram_selmatch(wp, wl_mode, "wdsap") || nvram_selmatch(wp, wl_mode, "infra") ||
	    nvram_selmatch(wp, wl_mode, "apsta") || nvram_selmatch(wp, wl_mode, "apstawet"))
#else
	if (nvram_selmatch(wp, wl_mode, "ap") || nvram_selmatch(wp, wl_mode, "wdsap") || nvram_selmatch(wp, wl_mode, "infra"))
#endif
	{
		...
		if (has_mimo(prefix) &&
		    (nvram_selnmatch(wp, "n-only", "%s_net_mode", prefix) ||
		     nvram_selnmatch(wp, "ng-only", "%s_net_mode", prefix) || nvram_selnmatch(wp, "mixed", "%s_net_mode", prefix) ||
...
#if defined(HAVE_ATH9K)
			if (is_mac80211(prefix) && (nvram_selmatch(wp, wl_width, "40") || nvram_selmatch(wp, wl_width, "2040"))) {
				websWrite(wp, "<div class=\"setting\">\n");
				websWrite(
					wp,
					"<div class=\"label\"><script type=\"text/javascript\">Capture(wl_basic.channel_wide)</script></div>\n");
...
void sas_show_channel(webs_t wp, char *dev, char *prefix, int type)
{
	...
	if (nvram_selmatch(wp, wl_mode, "ap") || nvram_selmatch(wp, wl_mode, "wdsap") || nvram_selmatch(wp, wl_mode, "infra")) {
			...
			while (chan[i].freq != -1) {
				...
				if (freq != -1) {
					websWrite(wp, "document.write(\"<option value=\\\"%s\\\" %s>%s - %d MHz</option>\");\n", fr,
						  nvram_selmatch(wp, wl_channel, fr) ? "selected=\\\"selected\\\"" : "", cn,
						  (freq + offset));
				}
				// free (chan[i].freq);
				i++;
		...
		for (i = 0; i < chancount; i++) {
			...
			if (nvram_selmatch(wp, wl_net_mode, "a-only") || nvram_selmatch(wp, wl_net_mode, "na-only") ||
			    nvram_selmatch(wp, wl_net_mode, "n5-only") || nvram_selmatch(wp, wl_net_mode, "acn-mixed") ||
			    nvram_selmatch(wp, wl_net_mode, "ac-only") || (net_is_a && nvram_selmatch(wp, wl_net_mode, "mixed"))) {
				if (chanlist[i] < 25)
					showit = 0;
			} else {
				if (chanlist[i] > 25)
			...
			if ((nvram_selmatch(wp, wl_net_mode, "n-only") || nvram_selmatch(wp, wl_net_mode, "n2-only") ||
			...
			if (showit) {
				websWrite(wp, "document.write(\"<option value=\\\"%d\\\" %s>%d - %0.3f GHz</option>\");\n",
					  chanlist[i],
					  nvram_selnmatch(wp, channelstring, "%s_channel", prefix) ? "selected=\\\"selected\\\"" :
												     "",
					  chanlist[i], ofs);
			}
		}
...
void sas_show_security_prefix(webs_t wp, int argc, char_t **argv, char *prefix, int primary)
{
	...
	if (!primary || nvram_selmatch(wp, sta, "ap") || nvram_selmatch(wp, sta, "wdsap")) {
...
#ifndef HAVE_MICRO
#if !defined(HAVE_RT2880) || defined(HAVE_MT76)
	if (nvram_selmatch(wp, sta, "sta") || nvram_selmatch(wp, sta, "wdssta") || nvram_match(sta, "apsta") ||
	    nvram_match(sta, "wet")) {
...
void sas_show_80211X(webs_t wp, char *prefix)
{
	...
	websWrite(wp, "<input name=\"%s_ttls8021xuser\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "ttls8021xuser", prefix));
	...
	websWrite(wp, "<input name=\"%s_ttls8021xanon\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "ttls8021xanon", prefix));
	...
	websWrite(wp, "<input name=\"%s_ttls8021xphase2\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "ttls8021xphase2", prefix));
	...
	websWrite(wp, "<input name=\"%s_peap8021xuser\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "peap8021xuser", prefix));

	websWrite(wp, "<div class=\"setting\">\n");
	websWrite(wp, "<div class=\"label\"><script type=\"text/javascript\">Capture(sec80211x.anon)</script></div>\n");
	websWrite(wp, "<input name=\"%s_peap8021xanon\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "peap8021xanon", prefix));
	...
	websWrite(wp, "<input name=\"%s_peap8021xphase2\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "peap8021xphase2", prefix));
	...
	websWrite(wp, "<input name=\"%s_leap8021xuser\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "leap8021xuser", prefix));

	websWrite(wp, "<div class=\"setting\">\n");
	websWrite(wp, "<div class=\"label\"><script type=\"text/javascript\">Capture(sec80211x.anon)</script></div>\n");
	websWrite(wp, "<input name=\"%s_leap8021xanon\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "leap8021xanon", prefix));
	...
	websWrite(wp, "<input name=\"%s_leap8021xphase2\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "leap8021xphase2", prefix));
	...
	websWrite(wp, "<input name=\"%s_tls8021xanon\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "tls8021xanon", prefix));
	...
	websWrite(wp, "<input name=\"%s_tls8021xphase2\" size=\"20\" maxlength=\"79\" value=\"%s\" /></div>\n", prefix,
		  sas_nvram_prefix_get(wp, "tls8021xphase2", prefix));
...
void sas_show_wpa_setting(webs_t wp, int argc, char_t **argv, char *prefix, char *security_prefix)
{
	...
	else if (!strcmp(security_mode, "wpa") || !strcmp(security_mode, "wpa2") || !strcmp(security_mode, "wpa wpa2"))
...
EJ_VISIBLE void ej_sas_show_dhcpd_settings(webs_t wp, int argc, char_t **argv)
{
	...
	if (nvram_selmatch(wp, "dhcpfwd_enable", "1")) {
		...
		websWrite(
			...
			sas_get_single_ip(wp, ipfwd, 0), sas_get_single_ip(wp, ipfwd, 1), sas_get_single_ip(wp, ipfwd, 2),
		...
		websWrite(wp, "<div class=\"label\"><script type=\"text/javascript\">Capture(idx.dhcp_start)</script></div>%s",
			  buf);
		websWrite(
			wp,
			"<input class=\"num\" name=\"dhcp_start\" size=\"3\" maxlength=\"3\" onblur=\"valid_range(this,1,254,idx.dhcp_start)\" value=\"%s\" %s />",
			nvram_selget(wp, "dhcp_start"), nvram_selmatch(wp, "lan_proto", "static") ? "disabled class=\"off\"" : "");
		websWrite(wp, "</div>\n");
