...
//    sprintf( wl_comp, "%s_compression", prefix );
...
#ifdef HAVE_RT2880
	if (nvram_selmatch(wp, wl_mode, "ap") || nvram_selmatch(wp, wl_mode, "wdsap")
	    || nvram_selmatch(wp, wl_mode, "infra") || nvram_selmatch(wp, wl_mode, "apsta")
...
#endif
	{
		...
		if (has_mimo(prefix)
			...
			if (nvram_selnmatch(wp, "40", "%s_nbw", prefix)) {
				...
				websWrite(wp,
					  ...
					  nvram_selnmatch(wp, "upper", "%s_nctrlsb",
						       prefix) ?
					  "selected=\\\"selected\\\"" : "");
				websWrite(wp,
					  "<option value=\"lower\" %s>lower</option>\n",
					  nvram_selnmatch(wp, "lower", "%s_nctrlsb",
						       prefix) ?
					  "selected=\\\"selected\\\"" : "");
				websWrite(wp, "</select>\n");
		...
		websWrite(wp,
			  ...
			  wl_closed, nvram_selmatch(wp, wl_closed,
						    "0") ? "checked=\"checked\""
			  : "");
		websWrite(wp,
...
void sas_show_netmode(webs_t wp, char *prefix)
{
	...
	if (nvram_selnmatch(wp, "n-only", "%s_net_mode", prefix)) {
		...
		websWrite(wp,
			  ...
			  sas_nvram_default_match(wp, wl_greenfield, "0",
					      "0") ? "selected=\\\"selected\\\""
			  : "");
		websWrite(wp,
			  "document.write(\"<option value=\\\"1\\\" %s>\" + wl_basic.greenfield + \"</option>\");\n",
			  sas_nvram_default_match(wp, wl_greenfield, "1",
					      "0") ? "selected=\\\"selected\\\""
			  : "");
		websWrite(wp, "//]]>\n</script>\n");
		websWrite(wp, "</select>\n");
...
void sas_show_channel(webs_t wp, char *dev, char *prefix, int type) {
...
#endif
	{
		...
		if (chan != NULL) {
			...
			while (chan[i].freq != -1) {
				...
				if (freq != -1)
					websWrite(wp,
						...
						fr, nvram_selmatch(wp, wl_channel,
								fr) ?
						"selected=\\\"selected\\\"" :
						"", cn, (freq));
					// free (chan[i].freq);
			...
			websWrite(wp,
				"document.write(\"<option value=\\\"0\\\" %s>\" + share.auto + \"</option>\");\n",
				nvram_selnmatch(wp, "0", "%s_channel",
						prefix) ?
				"selected=\\\"selected\\\"" : "");
...
#ifndef HAVE_MICRO
#ifndef HAVE_RT2880
	if (!primary || nvram_selmatch(wp, sta, "sta") || nvram_selmatch(wp, sta, "wdssta")
...
void sas_show_80211X(webs_t wp, char *prefix) {
	...
	websWrite(wp,
		...
		prefix, prefix, sas_nvram_prefix_match(wp, "8021xtype", prefix,
			"peap") ?
		"checked=\"checked\"" : "");
	websWrite(wp,
...
char *sas_get_wep_value(char *temp, char *type, char *_bit, char *prefix)
{
	...
	if(nvram_match("gozila_action", "1") && !nvram_match("generate_key", "1")) {
		char label[32];
...
void sas_show_wep(webs_t wp, char *prefix) {
	...
	sprintf(p_temp, "%s", sas_get_wep_value(temp, "passphrase", bit, prefix));
...
void sas_show_radius(webs_t wp, char *prefix, int showmacformat, int backup) {
	...
	if (!showmacformat) {
		...
		vvar, prefix, sas_nvram_default_match(wp, acct, "1",
				"0") ?
			"checked=\"checked\"" : "");
		websWrite(wp,
		...
		vvar, prefix, sas_nvram_default_match(wp, acct, "0",
				"0") ?
			"checked=\"checked\"" : "");
		websWrite(wp, "</div>\n");
		...
		websWrite(wp,
			...
			prefix, vvar, nvram_selmatch(wp, acct,
				"1") ? "true" : "false");
		websWrite(wp, "//]]>\n</script>\n");
	}
#endif
...
void ej_sas_show_dhcpd_settings(webs_t wp, int argc, char_t ** argv) {
	...
	websWrite(wp,
		...
		nvram_selmatch(wp, "dhcpfwd_enable",
			"0") ? "selected=\\\"selected\\\"" : "");
	websWrite(wp,
	...
	if (nvram_selmatch(wp, "dhcpfwd_enable", "1")) {
		...
		websWrite(wp,
			...
			sas_get_single_ip(wp, ipfwd, 0), sas_get_single_ip(wp, ipfwd, 1),
			sas_get_single_ip(wp, ipfwd, 2), sas_get_single_ip(wp, ipfwd, 3));
		...
		websWrite(wp,
			...
			nvram_selmatch(wp, "lan_proto", 
				"static") ? "disabled style=\"background: #e0e0e0\"" : "");
		websWrite(wp, "</div>\n");
