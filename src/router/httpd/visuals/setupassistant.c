		...
		if (i + 1 < 5) {
			if (atoi(stage) == i) {
				websWrite(wp,
					  ...
					  stage,
					  live_translate(labels[atoi(stage)]));
			} else {
				websWrite(wp,
...
static void sas_init_80211x_layers(webs_t wp, char *prefix)
{
	...
	if (!strcmp(nvram_selget(wp, var), "8021X")) {
		if (sas_nvram_prefix_match(wp, "8021xtype", prefix, "tls")) {
		    ...
		    if (sas_nvram_prefix_match(wp, "8021xtype", prefix, "leap"))
		{
			websWrite(wp, "enable_idleap(\"%s\");\n", prefix);
		} else
		    if (sas_nvram_prefix_match(wp, "8021xtype", prefix, "ttls"))
		{
...
void sas_show_80211X(webs_t wp, char *prefix)
{
	...
	websWrite(wp,
		  ...
		  prefix, prefix, sas_nvram_prefix_match(wp, "8021xtype",
							 prefix,
							 "peap") ?
		  "checked=\"checked\"" : "");
	websWrite(wp,
	...
	websWrite(wp,
		  ...
		  prefix, prefix, nvram_selmatch(wp, peap,
						 "peap") ? "true" : "false");
	websWrite(wp,
...
void sas_show_preshared(webs_t wp, char *prefix)
{
	...
	websWrite(wp,
		  "<input type=\"password\" id=\"%s_wpa_psk\" name=\"%s_wpa_psk\" onblur=\"valid_psk_length(this)\" maxlength=\"63\" size=\"32\" value=\"",
		  prefix, prefix);
...
void ej_sas_show_dhcpd_settings(webs_t wp, int argc, char_t ** argv)
{
	...
	if (nvram_selmatch(wp, "dhcpfwd_enable", "1")) {
		...
		websWrite(wp,
			  ...
			  sas_get_single_ip(wp, ipfwd, 0), sas_get_single_ip(wp,
									     ipfwd,
									     1),
			  sas_get_single_ip(wp, ipfwd, 2), sas_get_single_ip(wp,
									     ipfwd,
									     3));
		...
		websWrite(wp,
			  ...
			  nvram_selget(wp, "dhcp_start"), nvram_selmatch(wp,
									 "lan_proto",
									 "static")
