...
EJ_VISIBLE void ej_show_bridgenames(webs_t wp, int argc, char_t ** argv)
{
	...
	if (!br0found) {
		...
		websWrite(wp, "<td>");
		showOptions_trans(wp, bridge_name, "On Off", (char *[]) { "share.on", "share.off" }, nvram_default_matchi("br0_mcast", 1, 0) ? "On" : "Off");
		websWrite(wp, "</td>");
	...
	foreach(word, wordlist, next) {
		...
		websWrite(wp, "<td>");
		showOptions_trans(wp, bridge_name, "On Off", (char *[]) { "share.on", "share.off" }, nvram_default_matchi(mcast, 1, 0) ? "On" : "Off");
		websWrite(wp, "</td>");
		sprintf(bridge_name, "bridgeprio%d", count);
		websWrite(wp, "<td>");
...
EJ_VISIBLE void ej_show_bridgeifnames(webs_t wp, int argc, char_t ** argv)
{
	...
	websWrite(wp, "<td class=\"center\">\n");
	websWrite(wp,
		  "<script type=\"text/javascript\">\n//<![CDATA[\n document.write(\"<input class=\\\"button\\\" type=\\\"add\\\" aria-label=\\\"\" + sbutton.add + \"\\\" onclick=\\\"bridgeif_add_submit(this.form)\\\" />\");\n//]]>\n</script>\n");
	websWrite(wp, "</td>\n");
	websWrite(wp, "</tr>\n");
	char var[32];
