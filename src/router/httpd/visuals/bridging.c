...
EJ_VISIBLE void ej_show_bridgenames(webs_t wp, int argc, char_t **argv)
{
	...
	websWrite(
		wp,
		"<table cellspacing=\"5\" summary=\"bridges\" id=\"Bridge_table\" class=\"table\"><thead><tr>\n");
	websWrite(
		wp,
	...
	char *stpoptions_trans[] = { "share.stp", "share.rstp", "share.mstp",
				     "share.off" };
#else
	...
	show_caption_pp(wp, NULL, "networking.forward_delay", "<th>",
			"</th>\n");
	...
	if (!br0found) {
		...
		websWrite(
			wp,
			"<tbody><tr><td><input class=\"num\" name=\"%s\" size=\"3\" value=\"br0\" /></td>\n",
			bridge_name);

		sprintf(bridge_name, "bridgestp%d", count);
		websWrite(wp, "<td>");
		showOptions_trans(wp, bridge_name, stpoptions, stpoptions_trans,
				  "Off");
		websWrite(wp, "</td>");
		sprintf(bridge_name, "bridgemcastbr%d", count);
		websWrite(wp, "<td>");
		showOptions_trans(
			wp, bridge_name, "On Off",
			(char *[]){ "share.on", "share.off" },
		...
		websWrite(
			wp,
			"<td><input class=\"num\" name=\"%s\" size=\"2\" value=\"15\" /></td>\n",
			bridge_name);
		sprintf(bridge_name, "bridgemax_age%d", count);
		websWrite(
			wp,
			"<td><input class=\"num\" name=\"%s\" size=\"2\" value=\"20\" /></td>\n",
			bridge_name);
		...
		websWrite(
			wp,
			"<td><input class=\"num\" name=\"%s\" size=\"3\" value=\"1500\" /></td>\n",
			bridge_name);

		sprintf(bridge_name, "lan_hwaddr");
		websWrite(
			wp,
			"<td class=\"center\"><input class=\"num\" name=\"%s\" size=\"16\" value=\"%s\" /></td>\n",
	...
	foreach(word, wordlist, next)
	{
		...
		websWrite(wp, "<td>");
		showOptions_trans(wp, bridge_name, "On Off",
				  (char *[]){ "share.on", "share.off" },
				  nvram_default_matchi(mcast, 1, 0) ? "On" :
								      "Off");
		websWrite(wp, "</td>");
		sprintf(bridge_name, "bridgeprio%d", count);
		websWrite(wp, "<td>");
		...
		websWrite(
			wp,
			"<td><input class=\"num\" name=\"%s\" size=\"2\" value=\"%s\" /></td>\n",
		...
		websWrite(
			wp,
			"<td><input class=\"num\" name=\"%s\" size=\"3\" value=\"%s\" /></td>\n",
...
static void show_bridgeifname(webs_t wp, char *bridges, char *devs, int count,
			      char *bridge, char *port, char *stp, char *prio,
			      char *hairpin, char *cost)
{
	...
	if (hasstp) {
		showOptions_trans(wp, vlan_name, "On Off",
				  (char *[]){ "share.on", "share.off" },
				  stp ? (!strcmp(stp, "1") ? "On" : "Off") :
					"On");
	} else {
		showOptions_ext_trans(wp, vlan_name, "Off",
				      (char *[]){ "share.off" }, "Off", 1);
	}
	websWrite(wp, "</td>");
#endif
	sprintf(vlan_name, "bridgeifprio%d", count);
	websWrite(wp, "<td>");
	showOptions(wp, vlan_name,
	...
	websWrite(
		wp,
		"<td><input class=\"num\" name=\"%s\" size=\"5\" value=\"%s\" /></td>\n",
...
EJ_VISIBLE void ej_show_bridgeifnames(webs_t wp, int argc, char_t **argv)
{
	...
	{
		if (!isbridge(word) && strcmp(word, "lo") &&
		    !strchr(word, ':')) {
			strcat(finalbuffer, " ");
	...
	show_caption_pp(wp, NULL, "networking.hairpin", "<th class=\"center\">",
			"</th>\n");
	show_caption_pp(wp, NULL, "share.actiontbl",
			"<th class=\"center\" width=\"10%%\">",
			"</th></thead>\n");
	...
	foreach(word, wordlist, next)
	{
		...
		show_bridgeifname(wp, finalbuffer, bufferif, count, tag, port,
				  stp, prio, hairpin, pathcost);
		count++;
	}
	int totalcount = count;

	for (i = count; i < realcount; i++) {
		show_bridgeifname(wp, finalbuffer, bufferif, i, "", "", NULL,
				  NULL, NULL, NULL);
		totalcount++;
	}
