...
static int cantkip(const char *prefix)
{
	...
	return (strcmp(netmode, "ng-only") && strcmp(netmode, "na-only") && strcmp(netmode, "n2-only") &&
		...
		strcmp(netmode, "be-only") && strcmp(netmode, "acn-mixed") && strcmp(netmode, "ax-only") &&
		strcmp(netmode, "beax6-only") && strcmp(netmode, "beax5-only") && strcmp(netmode, "be6-only") &&
		strcmp(netmode, "be5-only") && strcmp(netmode, "ax6-only") && strcmp(netmode, "ax5-only") &&
		strcmp(netmode, "xacn-mixed") && strcmp(netmode, "bexacn-mixed") && (strcmp(netmode, "mixed") || !ncapable));
}
...
static void show_security_prefix(webs_t wp, int argc, char_t **argv, char *prefix, int primary)
{
	...
	if (!has_qtn(prefix)) {
		...
		if (!nvram_match(sta, "mesh") && cantkip(prefix) && !has_6ghz(prefix))
...
static void show_bgscan_options(webs_t wp, char *prefix)
{
	char signal[32];
	...
	if (is_ap(prefix) || nvram_match(wl_mode, "infra") || nvram_match(wl_mode, "mesh")) {
		...
		if (is_mac80211(prefix)) {
			...
			if (nvram_nmatch("160", "%s_channelbw", prefix))
				channelbw = 160;
			if (nvram_nmatch("320", "%s_channelbw", prefix))
				channelbw = 320;
		...
		if (chan) {
			...
			while (chan[i].freq != -1) {
				...
				if (!is_morse_micro(prefix)) {
					...
					if (channelbw == 40 && chan[i].ht40 == false) {
						i++;
				...
				char eirp[32] = { 0 };
				if (chan[i].hw_eirp)
					snprintf(eirp, sizeof(eirp), " (%d dBm%s)", chan[i].max_eirp + poffset,
						 is_6ghz_psc_frequency(freq) ? ", PSC" : "");
				if (freq != -1) {
					if (is_morse_micro(prefix)) {
						websWrite(
							wp,
							"document.write(\"<option value=\\\"%s\\\" %s>%d - %d.%d \"+wl_basic.mhz+\"%s</option>\");\n",
			...
			instance = 2;
		{
			...
			for (i = 0; i < chancount; i++) {
				...
				if (nvram_match(wl_net_mode, "a-only") || nvram_match(wl_net_mode, "na-only") ||
				    ...
				    nvram_match(wl_net_mode, "be-only") || nvram_match(wl_net_mode, "bexacn-mixed") ||
				...
				if ((nvram_match(wl_net_mode, "na-only") || nvram_match(wl_net_mode, "ac-only") ||
				     nvram_match(wl_net_mode, "acn-mixed") || nvram_match(wl_net_mode, "ax-only") ||
				     nvram_match(wl_net_mode, "be-only") || nvram_match(wl_net_mode, "xacn-mixed") ||
				     nvram_match(wl_net_mode, "bexacn-mixed") || (net_is_a && nvram_match(wl_net_mode, "mixed")) ||
	...
	if (is_morse_micro(prefix)) {
		...
		if (!has_ad(prefix)) {
			websWrite(wp, "document.write(\"<option value=\\\"mixed\\\" %s>\" + wl_basic.mixed + \"</option>\");\n",
				  nvram_match(wl_net_mode, "mixed") ? "selected=\\\"selected\\\"" : "");
			if (is_mac80211(prefix) && has_5ghz(prefix) && has_2ghz(prefix))
				websWrite(
					wp,
					"document.write(\"<option value=\\\"mixed5\\\" %s>\" + wl_basic.mixed5 + \"</option>\");\n",
					nvram_match(wl_net_mode, "mixed5") ? "selected=\\\"selected\\\"" : "");
		}
		...
		if (has_ax(prefix) && !is_ath10k(prefix)) {
			...
			if (has_5ghz(prefix) && (has_6ghz(prefix) || has_2ghz(prefix))) {
				websWrite(
					wp,
...
static void show_droplowsignal(webs_t wp, char *prefix)
{
	if (is_mac80211(prefix)) {
		if (is_ap(prefix)) {
			websWrite(
...
static void internal_ej_show_wireless_single(webs_t wp, char *prefix)
{
	...
	if (has_ah(prefix)) {
		...
		sprintf(frequencies,
			...
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
	} else if (has_2ghz(prefix) && has_6ghz(prefix) && has_5ghz(prefix) && has_be(prefix)) {
		sprintf(frequencies,
			" <script type=\"text/javascript\">document.write(\"[2.4\"+wl_basic.ghz+\"/5 \"+wl_basic.ghz+\"/6 \"+wl_basic.ghz+\"/802.11be]%s%s - Max Vaps(%d)\");</script>",
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
		...
		sprintf(frequencies,
			...
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
	} else if (has_6ghz(prefix) && has_5ghz(prefix) && has_be(prefix)) {
		sprintf(frequencies,
			" <script type=\"text/javascript\">document.write(\"[5 \"+wl_basic.ghz+\"/6 \"+wl_basic.ghz+\"/802.11be]%s%s - Max Vaps(%d)\");</script>",
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
		...
		sprintf(frequencies,
			...
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
	} else if (has_6ghz(prefix) && has_be(prefix)) {
		sprintf(frequencies,
			" <script type=\"text/javascript\">document.write(\"[6 \"+wl_basic.ghz+\"/802.11be]%s%s - Max Vaps(%d)\");</script>",
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
		...
		sprintf(frequencies,
			...
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
	} else if (has_2ghz(prefix) && has_5ghz(prefix) && has_be(prefix)) {
		sprintf(frequencies,
			" <script type=\"text/javascript\">document.write(\"[2.4\"+wl_basic.ghz+\"/5 \"+wl_basic.ghz+\"/802.11be]%s%s - Max Vaps(%d)\");</script>",
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
		...
		sprintf(frequencies,
			...
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
	} else if (has_5ghz(prefix) && has_be(prefix)) {
		sprintf(frequencies,
			" <script type=\"text/javascript\">document.write(\"[5 \"+wl_basic.ghz+\"/802.11be]%s%s - Max Vaps(%d)\");</script>",
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
		...
		sprintf(frequencies,
			...
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
	} else if (has_2ghz(prefix) && has_be(prefix)) {
		sprintf(frequencies,
			" <script type=\"text/javascript\">document.write(\"[2.4 \"+wl_basic.ghz+\"/802.11be]%s%s - Max Vaps(%d)\");</script>",
			chipset ? " - " : "", chipset ? chipset : "", maxvaps);
	...
	if (has_be(prefix)) {
		...
		if (!strcmp(netmode, "mixed") || !strcmp(netmode, "bexacn-mixed") || !strcmp(netmode, "be-only") ||
			...
			showRadio(wp, "wl_basic.allow_mcs15", color);
		}
	}
	if (has_ax(prefix)) {
		char *netmode = nvram_nget("%s_net_mode", prefix);
		if (!strcmp(netmode, "mixed") || !strcmp(netmode, "mixed5") || !strcmp(netmode, "xacn-mixed") ||
		    !strcmp(netmode, "ax-only") || !strcmp(netmode, "bexacn-mixed") || !strcmp(netmode, "be-only") ||
		    !strcmp(netmode, "ax6-only") || !strcmp(netmode, "be6-only") || !strcmp(netmode, "be5-only") ||
		    !strcmp(netmode, "beax6-only") || !strcmp(netmode, "beax5-only") || !strcmp(netmode, "ax5-only") ||
	...
	if (has_ldpc(prefix)) {
		...
		if ((strcmp(netmode, "mixed") && //
		     ...
		     strcmp(netmode, "be5-only") && strcmp(netmode, "be5-only") && strcmp(netmode, "be-only") &&
		     strcmp(netmode, "ax6-only") && strcmp(netmode, "ax5-only") && strcmp(netmode, "beax6-only") &&
		     strcmp(netmode, "beax5-only") && strcmp(netmode, "mixed5") && strcmp(netmode, "axg-only") &&
		     strcmp(netmode, "bexacn-mixed") && strcmp(netmode, "xacn-mixed")))
	...
	if (is_ath10k(prefix) || is_ath11k(prefix)) {
		char wl_txbeacon[32];
		sprintf(wl_txbeacon, "%s_beacon_tx_mode", prefix);
		websWrite(wp, "<div class=\"setting\">\n");
		show_caption(wp, "label", "wl_basic.beacon_tx_mode", NULL);
		websWrite(wp, "<select name=\"%s\">\n", wl_txbeacon);
		websWrite(wp, "<script type=\"text/javascript\">\n//<![CDATA[\n");
	...
	if (((has_5ghz(prefix) || has_6ghz(prefix)) && has_ac(prefix)) || has_ax(prefix)) {
		...
		if (has_mubeamforming(prefix) || has_ax(prefix)) {
			nvram_default_geti(wl_mubf, DEFAULT_BF);
	...
	if (is_morse_micro(prefix)) {
		...
		if (!is_mac80211(prefix) || canht40)
			if ((nvram_nmatch("n-only", "%s_net_mode", prefix) || nvram_nmatch("ng-only", "%s_net_mode", prefix) ||
			     ...
			     nvram_nmatch("mixed5", "%s_net_mode", prefix) || nvram_nmatch("n5-only", "%s_net_mode", prefix) ||
			     nvram_nmatch("ac-only", "%s_net_mode", prefix) || nvram_nmatch("acn-mixed", "%s_net_mode", prefix) ||
			     nvram_nmatch("ax-only", "%s_net_mode", prefix) || nvram_nmatch("be-only", "%s_net_mode", prefix) ||
			     nvram_nmatch("ax6-only", "%s_net_mode", prefix) || nvram_nmatch("be6-only", "%s_net_mode", prefix) ||
			     nvram_nmatch("be5-only", "%s_net_mode", prefix) || nvram_nmatch("beax6-only", "%s_net_mode", prefix) ||
			     nvram_nmatch("beax5-only", "%s_net_mode", prefix) nvram_nmatch("ax5-only", "%s_net_mode", prefix) ||
			     nvram_nmatch("axg-only", "%s_net_mode", prefix) || nvram_nmatch("xacn-mixed", "%s_net_mode", prefix) ||
			     nvram_nmatch("bexacn-mixed", "%s_net_mode", prefix) ||
		...
		if (is_mac80211(prefix) &&
		    (nvram_nmatch("n-only", "%s_net_mode", prefix) || nvram_nmatch("ng-only", "%s_net_mode", prefix) ||
		     ...
		     nvram_nmatch("beax5-only", "%s_net_mode", prefix) || nvram_nmatch("ax5-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("axg-only", "%s_net_mode", prefix) || nvram_nmatch("xacn-mixed", "%s_net_mode", prefix) ||
		     nvram_nmatch("bexacn-mixed", "%s_net_mode", prefix) || nvram_nmatch("na-only", "%s_net_mode", prefix))) {
			if (!is_mac80211(prefix) || is_ath5k(prefix))
				...
				websWrite(wp, "document.write(\"<option value=\\\"40\\\" %s >\" + share.ht40 + \"</option>\");\n",
					  nvram_matchi(wl_width, 40) ? "selected=\\\"selected\\\"" : "");
		if ((is_ath11k(prefix) ||is_ath10k(prefix) || is_mvebu(prefix) || has_vht80(prefix)) && (((has_5ghz(prefix) || has_6ghz(prefix)) || (cansuperchannel(wp, prefix) && nvram_nmatch("1", "%s_turbo_qam",prefix))) 
		    && nvram_nmatch("mixed", "%s_net_mode", prefix) || 
		    nvram_nmatch("mixed5", "%s_net_mode", prefix) || 
		    nvram_nmatch("ac-only", "%s_net_mode", prefix) || 
		    nvram_nmatch("ax-only", "%s_net_mode", prefix) || 
		    nvram_nmatch("be-only", "%s_net_mode", prefix) || 
		    nvram_nmatch("ax6-only", "%s_net_mode", prefix) || 
		    nvram_nmatch("ax5-only", "%s_net_mode", prefix) ||
		    nvram_nmatch("beax5-only", "%s_net_mode", prefix) ||
		    nvram_nmatch("beax6-only", "%s_net_mode", prefix) ||
		    nvram_nmatch("be6-only", "%s_net_mode", prefix) || 
		    nvram_nmatch("be5-only", "%s_net_mode", prefix) 
		    || nvram_nmatch("acn-mixed", "%s_net_mode", prefix) || nvram_nmatch("xacn-mixed", "%s_net_mode", prefix) || nvram_nmatch("bexacn-mixed", "%s_net_mode", prefix) || (cansuperchannel(wp, prefix) && nvram_nmatch("1", "%s_turbo_qam",prefix))) {
				if (canvht80)
					websWrite(
						wp,
				...
				if (has_eht320(prefix) && has_6ghz(prefix) && can_eht320(prefix))
					websWrite(
						wp,
						"document.write(\"<option value=\\\"320\\\" %s >\" + share.eht320 + \"</option>\");\n",
						nvram_matchi(wl_width, 320) ? "selected=\\\"selected\\\"" : "");
	...
	#endif
	{
		if (has_mimo(prefix) &&
		    (nvram_nmatch("n-only", "%s_net_mode", prefix) || nvram_nmatch("ng-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("mixed", "%s_net_mode", prefix) || nvram_nmatch("mixed5", "%s_net_mode", prefix) ||
		     nvram_nmatch("n2-only", "%s_net_mode", prefix) || nvram_nmatch("n5-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("acn-mixed", "%s_net_mode", prefix) || nvram_nmatch("ac-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("xacn-mixed", "%s_net_mode", prefix) || nvram_nmatch("bexacn-mixed", "%s_net_mode", prefix) ||
		     nvram_nmatch("be-only", "%s_net_mode", prefix) || nvram_nmatch("ax-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("ax6-only", "%s_net_mode", prefix) || nvram_nmatch("be6-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("be5-only", "%s_net_mode", prefix) || nvram_nmatch("beax6-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("beax5-only", "%s_net_mode", prefix) || nvram_nmatch("ax5-only", "%s_net_mode", prefix) ||
		     nvram_nmatch("na-only", "%s_net_mode", prefix) || nvram_nmatch("axg-only", "%s_net_mode", prefix))) {
			...
			if (is_morse_micro(prefix)) {
				...
				if ((has_ac(prefix) || has_ax(prefix)) && (has_5ghz(prefix) || has_6ghz(prefix)) &&
					...
					if (has_eht320(prefix) && has_6ghz(prefix)) {
						websWrite(
							wp,
							"<option value=\"320\" %s><script type=\"text/javascript\">Capture(share.eht320);</script></option>\n",
							nvram_nmatch("320", "%s_nbw", prefix) ? "selected=\"selected\"" : "");
					}
				}
			}
			websWrite(wp, "</select>\n");
	...
	if (has_acktiming(prefix)) {
		...
		websWrite(
			...
			(int)get_length((float)500), nvram_match("feet", "1") ? "feet" : "meters");
		websWrite(wp, "</div>\n");
	}
	#ifdef HAVE_MADWIFI
			...
			if (!is_mac80211(prefix) ||
			    (is_mac80211(prefix) &&
			     (nvram_nmatch("n-only", "%s_net_mode", prefix) || nvram_nmatch("ng-only", "%s_net_mode", prefix) ||
			      ...
			      nvram_nmatch("beax6-only", "%s_net_mode", prefix) ||
			      nvram_nmatch("beax5-only", "%s_net_mode", prefix) ||
			      nvram_nmatch("ax5-only", "%s_net_mode", prefix) || nvram_nmatch("axg-only", "%s_net_mode", prefix) ||
			      nvram_nmatch("xacn-mixed", "%s_net_mode", prefix) ||
			      nvram_nmatch("bexacn-mixed", "%s_net_mode", prefix) ||
			      nvram_nmatch("na-only", "%s_net_mode", prefix)))) {
				if (!is_mac80211(prefix) || is_ath5k(prefix))
				...
				if ((is_ath11k(prefix) || is_ath10k(prefix) || is_mvebu(prefix) || has_vht80(prefix)) &&
					...
					if (has_vht80plus80(prefix) && canvht80 && (has_5ghz(prefix) || has_6ghz(prefix)))
						websWrite(
							...
							"document.write(\"<option value=\\\"80+80\\\" %s >\" + share.vht80plus80 + \"</option>\");\n",
							nvram_match(wl_width, "80+80") ? "selected=\\\"selected\\\"" : "");
					if (has_eht320(prefix) && can_eht320(prefix) && has_6ghz(prefix))
						websWrite(
							wp,
							"document.write(\"<option value=\\\"320\\\" %s >\" + share.eht320 + \"</option>\");\n",
							nvram_matchi(wl_width, 320) ? "selected=\\\"selected\\\"" : "");
	...
	if (has_be(prefix)) {
		char *netmode = nvram_nget("%s_net_mode", prefix);
		if (!strcmp(netmode, "mixed") || !strcmp(netmode, "mixed5") || !strcmp(netmode, "bexacn-mixed") ||
	...
	if (has_ax(prefix)) {
		char *netmode = nvram_nget("%s_net_mode", prefix);
		if (!strcmp(netmode, "mixed") || !strcmp(netmode, "mixed5") || !strcmp(netmode, "xacn-mixed") ||
		    !strcmp(netmode, "bexacn-mixed") || !strcmp(netmode, "ax-only") || !strcmp(netmode, "be-only") ||
		    !strcmp(netmode, "ax6-only") || !strcmp(netmode, "be6-only") || !strcmp(netmode, "be5-only") ||
		    !strcmp(netmode, "beax6-only") || !strcmp(netmode, "beax5-only") || !strcmp(netmode, "ax5-only") ||
		    !strcmp(netmode, "axg-only")) {
			char color[32];
	...
	if (has_ldpc(prefix)) {
		...
		if ((strcmp(netmode, "mixed") && strcmp(netmode, "mixed5") && strcmp(netmode, "ac-only") &&
		     strcmp(netmode, "acn-mixed") && strcmp(netmode, "ax-only") && strcmp(netmode, "be-only") &&
		     strcmp(netmode, "ax6-only") && strcmp(netmode, "ax5-only") && strcmp(netmode, "be5-only") &&
		     strcmp(netmode, "be6-only") && strcmp(netmode, "beax5-only") && strcmp(netmode, "beax6-only") &&
		     strcmp(netmode, "axg-only") && strcmp(netmode, "xacn-mixed") && strcmp(netmode, "bexacn-mixed")))
			showRadioInv(wp, "wl_basic.ldpc", wl_ldpc);
	}
	if (has_uapsd(prefix)) {
		showRadio(wp, "wl_basic.uapsd", wl_uapsd);
...
typedef struct pair {
	...
	int (*valid5)(const char *prefix);
	int (*valid6)(const char *prefix);
...
{
	if (!has_be(prefix) || has_5ghz(prefix))
		return 1;
	char *netmode = nvram_nget("%s_net_mode", prefix);
...
void show_authtable(webs_t wp, char *prefix, int show80211x)
{
	struct pair s_cryptopair[] = {
		{ "wpa.ccmp", "ccmp", noad, wpaauth, alwaystrue, alwaystrue, alwaystrue, alwaystrue },
	...
	struct pair s_authpair_wpa[] = {
		...
		{ "wpa.wpa", "wpa", aponly, alwaystrue, nomesh, noad, disable_if_6ghz, disable_if_be },
		{ "wpa.wpa2", "wpa2", aponly, alwaystrue, nomesh, noad, disable_if_6ghz, disable_if_be },
		{ "wpa.wpa2_sha256", "wpa2-sha256", aponly_wpa3, is_mac80211, nomesh, noad, disable_if_6ghz, disable_if_be },
		{ "wpa.wpa2_sha384", "wpa2-sha384", aponly_wpa3, is_mac80211, nomesh, noad, disable_if_6ghz, disable_if_be },
		{ "wpa.wpa3", "wpa3", aponly_wpa3, is_mac80211, nomesh, noad, alwaystrue, alwaystrue },
		{ "wpa.wpa3_128", "wpa3-128", aponly_wpa3_gcmp128, has_gmac_128, nomesh, noad, alwaystrue, alwaystrue },
		{ "wpa.wpa3_192", "wpa3-192", aponly_wpa3_gcmp256, has_gmac_256, nomesh, noad, alwaystrue, alwaystrue },
		{ "wpa.owe", "owe", aponly_wpa3, is_mac80211, owe_possible, noad, alwaystrue, alwaystrue }
	};
	struct pair s_authpair_80211x[] = {
		{ "wpa.wpa", "wpa", alwaystrue, alwaystrue, alwaystrue, alwaystrue, disable_if_6ghz, disable_if_be },
		{ "wpa.wpa2", "wpa2", alwaystrue, alwaystrue, alwaystrue, alwaystrue, disable_if_6ghz, disable_if_be },
		{ "wpa.wpa2_sha256", "wpa2-sha256", has_wpa3, alwaystrue, alwaystrue, alwaystrue, disable_if_6ghz, disable_if_be },
		{ "wpa.wpa2_sha384", "wpa2-sha384", has_wpa3, alwaystrue, alwaystrue, alwaystrue, disable_if_6ghz, disable_if_be },
		...
		{ "wpa.wep_8021x", "802.1x", alwaystrue, alwaystrue, alwaystrue, alwaystrue, disable_if_6ghz, disable_if_be }
	};
	struct pair s_authmethod[] = {
		{ "wpa.peap", "peap", alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue },
		{ "wpa.leap", "leap", alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue },
		{ "wpa.tls", "tls", alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue },
		{ "wpa.ttls", "ttls", alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue, alwaystrue },
	};
	...
	for (i = 0; i < sizeof(s_cryptopair) / sizeof(struct pair); i++) {
		if (s_cryptopair[i].valid(prefix) && s_cryptopair[i].valid2(prefix) && s_cryptopair[i].valid3(prefix) &&
		    s_cryptopair[i].valid4(prefix) && s_cryptopair[i].valid5(prefix) && s_cryptopair[i].valid6(prefix))
	...
	for (i = 0; i < sizeof(s_authmethod) / sizeof(struct pair); i++) {
		if (s_authmethod[i].valid(prefix) && s_authmethod[i].valid2(prefix) && s_authmethod[i].valid3(prefix) &&
		    s_authmethod[i].valid4(prefix) && s_authmethod[i].valid5(prefix) && s_authmethod[i].valid6(prefix))
	...
	if (!show80211x) {
		for (i = 0; i < sizeof(s_authpair_wpa) / sizeof(struct pair); i++) {
			if (s_authpair_wpa[i].valid(prefix) && s_authpair_wpa[i].valid2(prefix) &&
			    ...
			    s_authpair_wpa[i].valid5(prefix) && s_authpair_wpa[i].valid6(prefix))
		...
		for (i = 0; i < sizeof(s_authpair_80211x) / sizeof(struct pair); i++) {
			if (s_authpair_80211x[i].valid(prefix) && s_authpair_80211x[i].valid2(prefix) &&
			    ...
			    s_authpair_80211x[i].valid5(prefix) && s_authpair_80211x[i].valid6(prefix))
