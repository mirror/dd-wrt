...
void ej_show_connectiontype(webs_t wp, int argc, char_t ** argv)
{
	...
	websWrite(wp,
		  "<option value=\"bridge\" %s >DSL Modem Bridge</option>\n",
		  nvram_selmatch(wp, "wan_proto",
...
void ej_show_olsrd(webs_t wp, int argc, char_t ** argv)
{
	...
	if (!strcmp(var, "olsr")) {
		...
		websWrite(wp,
			  ...
			  nvram_default_match("olsrd_gateway", "1",
					      "0") ? "checked=\"checked\"" :
			  "");
		websWrite(wp,
			  ...
			  nvram_default_match("olsrd_gateway", "0",
					      "0") ? "checked=\"checked\"" :
			  "");
		websWrite(wp, "</div>\n");
...
#endif
#if defined(HAVE_EASY_WIRELESS_CONFIG) && !defined(HAVE_MODERN)
		websWrite(wp, "</div>\n");
#endif
		websWrite(wp, "</fieldset><br />\n");
		count++;
