...
static void show_channel( webs_t wp, char *dev, char *prefix, int type )
{
	...
	return;
#ifdef HAVE_RT2880
    if( nvram_match( wl_mode, "ap" ) || nvram_match( wl_mode, "wdsap" ) || nvram_match( wl_mode, "apsta" ) || nvram_match( wl_mode, "apstawet" )
...
void ej_show_wireless_single( webs_t wp, char *prefix )
{
	...
	websWrite( wp,"document.write(\"<option value=\\\"4\\\" %s >jjPlus JA24MP0 (MakSat OEM)</option>\");\n",nvram_default_match( wl_cardtype, "4","0" ) ? "selected=\\\"selected\\\"" : "" );
	websWrite( wp, "//]]>\n</script>\n</select>\n</div>\n" );	
	}
...
#ifdef HAVE_RT2880
    if( nvram_match( wl_mode, "ap" ) || nvram_match( wl_mode, "wdsap" )
	|| nvram_match( wl_mode, "infra" ) || nvram_match( wl_mode, "apsta" ) || nvram_match( wl_mode, "apstawet" ) )
#else
    if( nvram_match( wl_mode, "ap" ) || nvram_match( wl_mode, "wdsap" )
	|| nvram_match( wl_mode, "infra" ) )
#endif
