			...
			if (strlen(line) > 2) {
				...
				if (sscanf(line, "%s %s %s %s", used, avail, per, mp) == 4) {
					if (!strncmp(mp, "/dev", 4))
						continue;
					websWrite(wp, "<div class=\"setting\">");
					websWrite(wp, "<div class=\"label\">%s %s</div>", live_translate(wp, "usb.usb_diskspace"), mp);
...
//wl_basic.vht80p80chan="Wireless Channel 2 (80+80)";
		...
		if (is_mac80211(prefix))
			websWrite(wp, "<select name=\"%s\" rel=\"mac80211\" onfocus=\"check_action(this,0)\" onchange=\"setChannelProperties(this);\"><script type=\"text/javascript\">\n//<![CDATA[\n", wl_channel);
		else
		...
		if (chan) {
			...
			while (chan[i].freq != -1) {
				...
				if (channelbw == 160 && !chan[i].lll && !chan[i].llu && !chan[i].lul && !chan[i].luu && !chan[i].ull && !chan[i].ulu && !chan[i].uul && !chan[i].uuu) {
					i++;
					continue;	// do not show channels where bandwidth is not available
				}
...
//              websWrite( wp, ", \"%0.3f\"", ofs );
				...
				if ((nvram_match(wl_net_mode, "na-only") || nvram_match(wl_net_mode, "ac-only") || nvram_match(wl_net_mode, "acn-mixed") || nvram_match(wl_net_mode, "ax-only") || nvram_match(wl_net_mode, "xacn-mixed")
				     || (net_is_a && nvram_match(wl_net_mode, "mixed"))
				     || nvram_match(wl_net_mode, "n5-only"))
				...
				if (showit) {
					websWrite(wp, "document.write(\"<option value=\\\"%d\\\" %s>%d - %d.%d \"+wl_basic.ghz+\"</option>\");\n",
...
//      show_chanshift( wp, wl_chanshift );
		...
		sprintf(wnm, "%s_time_advertisement", var);
...
//                        nvram_nmatch("0", "%s_nbw", prefix) ? "selected=\\\"selected\\\"" : "");
			...
			if (has_ac(prefix) && has_5ghz(prefix)
			    && (nvram_nmatch("mixed", "%s_net_mode", prefix) || nvram_nmatch("ac-only", "%s_net_mode", prefix) || nvram_nmatch("acn-mixed", "%s_net_mode", prefix) || nvram_nmatch("ax-only", "%s_net_mode", prefix) || nvram_nmatch("xacn-mixed", "%s_net_mode", prefix))) {
			...
			if (nvram_nmatch("80", "%s_nbw", prefix) || nvram_nmatch("80+80", "%s_nbw", prefix)) {	// 802.11ac
				websWrite(wp, "<div class=\"setting\">\n");
				show_caption(wp, "label", "wl_basic.channel_wide", NULL);
				websWrite(wp, "<select name=\"%s_nctrlsb\" >\n", prefix);
			...
			if (is_mac80211(prefix)) {
				if (!is_ath5k(prefix) && (nvram_matchi(wl_width, 40) || nvram_matchi(wl_width, 2040))) {
					...
					show_caption(wp, "label", "wl_basic.channel_wide", NULL);
					websWrite(wp, "<select name=\"%s_nctrlsb\" >\n", prefix);
					websWrite(wp, "<option value=\"ull\" %s><script type=\"text/javascript\">Capture(wl_basic.ch_pos_upr);</script></option>\n",
						  (nvram_nmatch("ull", "%s_nctrlsb", prefix) || nvram_nmatch("upper", "%s_nctrlsb", prefix)) ? "selected=\"selected\"" : "");
					websWrite(wp, "<option value=\"luu\" %s><script type=\"text/javascript\">Capture(wl_basic.ch_pos_lwr);</script></option>\n",
						  (nvram_nmatch("luu", "%s_nctrlsb", prefix) || nvram_nmatch("lower", "%s_nctrlsb", prefix)) ? "selected=\"selected\"" : "");
					websWrite(wp, "</select>\n");
					websWrite(wp, "</div>\n");
				}
				if (nvram_matchi(wl_width, 80) || nvram_match(wl_width, "80+80")) {
					...
					show_caption(wp, "label", "wl_basic.channel_wide", NULL);
					websWrite(wp, "<select name=\"%s_nctrlsb\" >\n", prefix);
					websWrite(wp, "<option value=\"lul\" %s><script type=\"text/javascript\">Capture(wl_basic.ch_pos_ll);</script></option>\n",
						  (nvram_nmatch("lul", "%s_nctrlsb", prefix) || nvram_nmatch("lower", "%s_nctrlsb", prefix)) ? "selected=\"selected\"" : "");
					websWrite(wp, "<option value=\"luu\" %s><script type=\"text/javascript\">Capture(wl_basic.ch_pos_lu);</script></option>\n",
						  nvram_nmatch("luu", "%s_nctrlsb", prefix) ? "selected=\"selected\"" : "");
					websWrite(wp, "<option value=\"ull\" %s><script type=\"text/javascript\">Capture(wl_basic.ch_pos_ul);</script></option>\n",
						  nvram_nmatch("ull", "%s_nctrlsb", prefix) ? "selected=\"selected\"" : "");
					websWrite(wp, "<option value=\"ulu\" %s><script type=\"text/javascript\">Capture(wl_basic.ch_pos_uu);</script></option>\n",
						  (nvram_nmatch("ulu", "%s_nctrlsb", prefix) || nvram_nmatch("upper", "%s_nctrlsb", prefix)) ? "selected=\"selected\"" : "");
					websWrite(wp, "</select>\n");
					websWrite(wp, "</div>\n");
				}
	...
	if (is_mac80211(prefix)) {
		...
		sprintf(wnm, "%s_time_advertisement", prefix);
		showRadio(wp, "wpa.time_advertisement", wnm);
...
// RELAYD OPTIONAL SETTINGS
	...
	if (nvram_match(wl_mode, "wet")) {
		...
		websWrite(wp, "<div id=\"%s_relayd_gw_ipaddr\" class=\"setting\"%s>\n"
			  ...
			  "</div>\n", prefix, nvram_default_matchi(wl_relayd, 1, 0) ? " style=\"display: none; visibility: hidden;\"" : "", prefix, prefix, ip[0], prefix, ip[1], prefix, ip[2], prefix, ip[3]);
	}
#endif
...
/* limit channel options by mode */
		...
		if (!is_mac80211(prefix)
			...
			if (!is_mac80211(prefix) || is_ath5k(prefix))
				...
				websWrite(wp, "document.write(\"<option value=\\\"40\\\" %s >\" + share.ht40 + \"</option>\");\n", nvram_matchi(wl_width, 40) ? "selected=\\\"selected\\\"" : "");
			if ((is_ath11k(prefix) ||is_ath10k(prefix) || is_mvebu(prefix) || has_vht80(prefix)) && (has_5ghz(prefix) || (cansuperchannel(wp, prefix) && nvram_nmatch("1", "%s_turbo_qam",prefix)))
			    && (nvram_nmatch("mixed", "%s_net_mode", prefix) || nvram_nmatch("ac-only", "%s_net_mode", prefix) || nvram_nmatch("acn-mixed", "%s_net_mode", prefix)|| nvram_nmatch("ax-only", "%s_net_mode", prefix) || nvram_nmatch("xacn-mixed", "%s_net_mode", prefix) || (cansuperchannel(wp, prefix) && nvram_nmatch("1", "%s_turbo_qam",prefix)))) {
				if (canvht80)
					websWrite(wp, "document.write(\"<option value=\\\"80\\\" %s >\" + share.vht80 + \"</option>\");\n", nvram_matchi(wl_width, 80) ? "selected=\\\"selected\\\"" : "");
				if ((has_vht160(prefix) || has_he160(prefix)) && can_vht160(prefix) && has_5ghz(prefix))
					websWrite(wp, "document.write(\"<option value=\\\"160\\\" %s >\" + share.vht160 + \"</option>\");\n", nvram_matchi(wl_width, 160) ? "selected=\\\"selected\\\"" : "");
...
//              { "wpa.gcmp_128", "gcmp", has_ad, wpaauth, alwaystrue },
	...
	if (!show80211x) {
		...
		for (i = 0; i < sizeof(s_authpair_80211x) / sizeof(struct pair); i++) {
			if (s_authpair_80211x[i].valid(prefix) && s_authpair_80211x[i].valid2(prefix) && s_authpair_80211x[i].valid3(prefix))
