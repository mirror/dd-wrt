...
static struct menucontext *init_menu(webs_t wp)
{
	...
	static char *menu_t[8][13] = {
		{ "index.asp", "IPV6.asp", "DDNS.asp", "WanMAC.asp", "Routing.asp", "Vlan.asp", "Networking.asp", "eop-tunnel.asp",
		  "", "", "", "", "" }, //
		{ "Wireless_Basic.asp", "SuperChannel.asp", "WiMAX.asp", "Wireless_radauth.asp", "WL_WPATable.asp", "Roaming.asp",
		  "Wireless_MAC.asp", "Wireless_Advanced.asp", "Wireless_WDS.asp", "", "", "" }, //
		{ "Services.asp", "FreeRadius.asp", "PPPoE_Server.asp", "PPTP.asp", "USB.asp", "NAS.asp", "Hotspot.asp",
		  "Nintendo.asp", "Milkfish.asp", "Privoxy.asp", "Speedchecker.asp", "", "" }, //
		...
		{ "Status_Router.asp", "Status_Internet.asp", "Status_Lan.asp", "Status_Wireless.asp", "Status_SputnikAPD.asp",
		  "Status_OpenVPN.asp", "Status_Bandwidth.asp", "Syslog.asp", "Info.htm", "register.asp", "MyPage.asp", "Gpio.asp",
		  "Status_CWMP.asp" } //
	};
	/*
	 * real name is bmenu.menuname[i][j]
	...
	static char *menuname_t[8][14] = {
		{ "setup", "setupbasic", "setupipv6", "setupddns", "setupmacclone", "setuprouting", "setupvlan", "networking",
		  "setupeop", "", "", "", "", "" }, //
		{ "wireless", "wirelessBasic", "wirelessSuperchannel", "wimax", "wirelessRadius", "wirelessSecurity",
		  "wirelessRoaming", "wirelessMac", "wirelessAdvanced", "wirelessWds", "", "", "" }, //
		{ "services", "servicesServices", "servicesRadius", "servicesPppoesrv", "servicesPptp", "servicesUSB",
		  "servicesNAS", "servicesHotspot", "servicesNintendo", "servicesMilkfish", "servicesPrivoxy",
		  "servicesSpeedchecker", "", "" }, //
		{ "security", "firwall", "vpn", "", "", "", "", "", "", "", "", "", "", "" }, //
		{ "accrestriction", "webaccess", "", "", "", "", "", "", "", "", "", "", "", "" }, //
		{ "applications", "applicationspforwarding", "applicationsprforwarding", "applicationsipforwarding",
		  "applicationsptriggering", "applicationsUpnp", "applicationsDMZ", "applicationsQoS", "applicationsP2P", "", "",
		  "", "", "" }, //
		...
		{ "statu", "statuRouter", "statuInet", "statuLAN", "statuWLAN", "statuSputnik", "statuVPN", "statuBand",
		  "statuSyslog", "statuSysInfo", "statuActivate", "statuMyPage", "statuGpio", "statuCWMP" } //
	};
	int x, y;
	for (x = 0; x < 8; x++) {
		for (y = 0; y < 14; y++) {
	...
	for (a = 0; a < ifcount; a++) {
		...
		if (has_ad(check) || is_ath10k(check) || is_ath11k(check) || is_brcmfmac(check) || is_mt76(check) || is_mvebu(check))
			continue;
	...
	if (wp->userid) {
		...
		int a;
		static char *wdsmenu[4] = { "Wireless_WDS-wl0.asp", "Wireless_WDS-wl1.asp", "Wireless_WDS-wl2.asp",
					    "Wireless_WDS-wl3.asp" };
		static char *tran_wdsmenu[4] = { "wirelessWdswl0", "wirelessWdswl1", "wirelessWdswl2", "wirelessWdswl3" };

		static char *advmenu[4] = { "Wireless_Advanced-wl0.asp", "Wireless_Advanced-wl1.asp", "Wireless_Advanced-wl2.asp",
					    "Wireless_Advanced-wl3.asp" };
		static char *tran_advmenu[4] = { "wirelessAdvancedwl0", "wirelessAdvancedwl1", "wirelessAdvancedwl2",
						 "wirelessAdvancedwl3" };
...
#endif
#ifdef HAVE_CORENET
		if (!strcmp_pnt(m->menu[i][0], "Firewall.asp") || !strcmp_pnt(m->menu[i][0], "Filters.asp") ||
...
//fprintf(stderr,"%s->%d\n",__func__,__LINE__);
			...
			for (j = 0; j < MAXSUBMENU; j++) {
				...
				if (!strcmp_pnt(m->menu[i][j],
					...
					goto skip;
#else
				if (!wimaxwifi && !strcmp_pnt(m->menu[i][j],
							      "WiMAX.asp")) // jump
					// over
					// WiMAX
					goto skip;
				...
				if (!strncmp(m->menu[i][j], "Wireless_Advanced", 17))
					goto skip;
				if ((!wifi || cpeonly) && !strncmp(m->menu[i][j], "Wireless_WDS", 12))
					goto skip;
...
//#endif
				...
				if (!strcmp_pnt(m->menu[i][j], "P2P.asp"))
					goto skip;
#endif
				if ((!sputnik) && !strcmp_pnt(m->menu[i][j],
							      "Status_SputnikAPD.asp")) // jump
					// over
					// Sputnik
					goto skip;
				if ((!openvpn) && !strcmp_pnt(m->menu[i][j],
				...
				if ((!auth) && !strcmp_pnt(m->menu[i][j],
					...
					// Sys-Info
					goto skip;
				if ((registered) && !cpeonly && !strcmp_pnt(m->menu[i][j],
									    "register.asp")) // jump
					// over
					// register.asp
					goto skip;
				if ((!*(nvram_safe_get("mypage_scripts"))) && !strcmp_pnt(m->menu[i][j],
											  "MyPage.asp")) // jump
					// over
					// MyPage.asp
					goto skip;
				...
				if (!strcmp(m->menu[i][j], submenu) &&
				    (*(m->menu[i][j]) && !strcmp_pnt(m->menu[i][j], "Wireless_Basic.asp") && !wifi && wimaxwifi)) {
					websWrite(
						wp,
						"<li><span><script type=\"text/javascript\">Capture(bmenu.wimax)</script></span></li>\n");
				}
#endif
				else if (!strcmp(m->menu[i][j], submenu) && (*(m->menu[i][j]))) {
					websWrite(
						wp,
...
#endif /* < */
#ifdef HAVE_MADWIFI
				else if (*(m->menu[i][j]) && !strcmp_pnt(m->menu[i][j], "Wireless_Basic.asp") && !wifi &&
					 wimaxwifi) {
					websWrite(
						wp,
