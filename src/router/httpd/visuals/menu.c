...
static struct menucontext *init_menu(webs_t wp)
{
	...
	static char *menu_t[8][13] = {
		{ "index.asp", "IPV6.asp", "DDNS.asp", "WanMAC.asp",
		  "Routing.asp", "Vlan.asp", "Networking.asp", "eop-tunnel.asp",
		  "", "", "", "", "" }, //
		{ "Wireless_Basic.asp", "SuperChannel.asp", "WiMAX.asp",
		  "Wireless_radauth.asp", "WL_WPATable.asp", "Roaming.asp",
		  "Wireless_MAC.asp", "Wireless_Advanced.asp",
		  "Wireless_WDS.asp", "", "", "" }, //
		{ "Services.asp", "FreeRadius.asp", "PPPoE_Server.asp",
		  "PPTP.asp", "USB.asp", "NAS.asp", "Hotspot.asp",
		  "Nintendo.asp", "Milkfish.asp", "Privoxy.asp",
		  "Speedchecker.asp", "", "" }, //
		{ "Firewall.asp", "VPN.asp", "", "", "", "", "", "", "", "", "",
		  "", "" }, //
		{ "Filters.asp", "", "", "", "", "", "", "", "", "", "", "",
		  "" }, //
		{ "ForwardSpec.asp", "Forward.asp", "ForwardIP.asp",
		  "Triggering.asp", "UPnP.asp", "DMZ.asp", "QoS.asp", "P2P.asp",
		  "", "", "", "", "" }, //
		{ "Management.asp", "Alive.asp", "Sysctl.asp",
		  "Diagnostics.asp", "Wol.asp", "Factory_Defaults.asp",
		  "Upgrade.asp", "config.asp", "", "", "", "", "" }, //
		{ "Status_Router.asp", "Status_Internet.asp", "Status_Lan.asp",
		  "Status_Wireless.asp", "Status_SputnikAPD.asp",
		  "Status_OpenVPN.asp", "Status_Bandwidth.asp", "Syslog.asp",
	...
	static char *menuname_t[8][14] = {
		{ "setup", "setupbasic", "setupipv6", "setupddns",
		  "setupmacclone", "setuprouting", "setupvlan", "networking",
		  "setupeop", "", "", "", "", "" }, //
		{ "wireless", "wirelessBasic", "wirelessSuperchannel", "wimax",
		  "wirelessRadius", "wirelessSecurity", "wirelessRoaming",
		...
		{ "services", "servicesServices", "servicesRadius",
		  ...
		  "servicesMilkfish", "servicesPrivoxy", "servicesSpeedchecker",
		  "", "" }, //
		...
		{ "accrestriction", "webaccess", "", "", "", "", "", "", "", "",
		  "", "", "", "" }, //
		{ "applications", "applicationspforwarding",
		  "applicationsprforwarding", "applicationsipforwarding",
		  "applicationsptriggering", "applicationsUpnp",
		  "applicationsDMZ", "applicationsQoS", "applicationsP2P", "",
		  "", "", "", "" }, //
		{ "admin", "adminManagement", "adminAlive", "adminSysctl",
		  "adminDiag", "adminWol", "adminFactory", "adminUpgrade",
		  "adminBackup", "", "", "", "", "" }, //
		{ "statu", "statuRouter", "statuInet", "statuLAN", "statuWLAN",
		  "statuSputnik", "statuVPN", "statuBand", "statuSyslog",
		  "statuSysInfo", "statuActivate", "statuMyPage", "statuGpio",
		  "statuCWMP" } //
	};
	int x, y;
	for (x = 0; x < 8; x++) {
...
#endif
#ifdef HAVE_MADWIFI
	static char *wdsmenu[4] = { "Wireless_WDS-wlan0.asp",
				    "Wireless_WDS-wlan1.asp",
				    "Wireless_WDS-wlan2.asp",
				    "Wireless_WDS-wlan3.asp" };
	static char *tran_wdsmenu[4] = { "wirelessWds0", "wirelessWds1",
					 "wirelessWds2", "wirelessWds3" };
	...
	if (wp->userid) {
		...
		int a;
		static char *wdsmenu[4] = { "Wireless_WDS-wl0.asp",
					    "Wireless_WDS-wl1.asp",
					    "Wireless_WDS-wl2.asp",
					    "Wireless_WDS-wl3.asp" };
		static char *tran_wdsmenu[4] = { "wirelessWdswl0",
						 "wirelessWdswl1",
						 "wirelessWdswl2",
						 "wirelessWdswl3" };

		static char *advmenu[4] = { "Wireless_Advanced-wl0.asp",
					    "Wireless_Advanced-wl1.asp",
					    "Wireless_Advanced-wl2.asp",
					    "Wireless_Advanced-wl3.asp" };
		static char *tran_advmenu[4] = { "wirelessAdvancedwl0",
						 "wirelessAdvancedwl1",
						 "wirelessAdvancedwl2",
						 "wirelessAdvancedwl3" };
		...
		for (a = 0; a < ifcount; a++) {
			...
			if (ifcount == 1) {
				m->menuname[MENU_WIRELESS][a * 2 + 8] =
					"wirelessAdvanced";
				m->menuname[MENU_WIRELESS][a * 2 + 9] =
					"wirelessWds";
			} else {
				m->menuname[MENU_WIRELESS][a * 2 + 8] =
					tran_advmenu[a];
...
EJ_VISIBLE void ej_do_menu(webs_t wp, int argc, char_t **argv)
{
	...
	int sputnik = 0;
#endif
	int openvpn = nvram_matchi("openvpn_enable", 1) |
		      nvram_matchi("openvpncl_enable", 1);
...
#ifdef HAVE_MADWIFI
		if (!wifi && !wimaxwifi &&
		    !strcmp_pnt(m->menu[i][0], "Wireless_Basic.asp"))
			i++;
#endif
#ifdef HAVE_CORENET
		if (!strcmp_pnt(m->menu[i][0], "Firewall.asp") ||
		    !strcmp_pnt(m->menu[i][0], "Filters.asp") ||
		    !strcmp_pnt(m->menu[i][0], "ForwardSpec.asp")) // jump over
			// Corenet
			i++;
#endif
		if (i >= MAXMENU)
...
//fprintf(stderr,"%s->%d\n",__func__,__LINE__);
			...
			for (j = 0; j < MAXSUBMENU; j++) {
				...
				if (!wifi && !strncmp(m->menu[i][j],
						      "Wireless_MAC.asp", 8))
					goto skip;
				if (!strncmp(m->menu[i][j], "Wireless_Advanced",
					     17))
					goto skip;
				if ((!wifi || cpeonly) &&
...
#ifdef HAVE_MICRO
				if (!strcmp_pnt(m->menu[i][j],
						"PPTP.asp")) // jump over PPTP in
					// micro build
					goto skip;
#endif
#ifndef HAVE_USB
				if (!strcmp_pnt(m->menu[i][j],
						"USB.asp")) // jump over USB
					goto skip;
#endif
#ifndef HAVE_SYSCTL_EDIT
				if (!strcmp_pnt(
					    m->menu[i][j],
					    "Sysctl.asp")) // jump over sysctl editor
					goto skip;
#endif
#ifndef HAVE_NAS_SERVER
				if (!strcmp_pnt(m->menu[i][j],
						"NAS.asp")) // jump over NAS
					goto skip;
#endif
#ifdef HAVE_GLAUCO
				if (!strcmp_pnt(m->menu[i][j],
...
//#endif
				...
				if ((!sputnik) &&
				    !strcmp_pnt(m->menu[i][j],
						"Status_SputnikAPD.asp")) // jump
					// over
					// Sputnik
					goto skip;
				if ((!openvpn) &&
				    !strcmp_pnt(m->menu[i][j],
						"Status_OpenVPN.asp")) // jump
					// over
					// OpenVPN
					goto skip;
				if ((!auth) && !strcmp_pnt(m->menu[i][j],
							   "Info.htm")) // jump
					// over
					// Sys-Info
					goto skip;
				if ((registered) && !cpeonly &&
				    !strcmp_pnt(m->menu[i][j],
						"register.asp")) // jump
					// over
					// register.asp
					goto skip;
				if ((!*(nvram_safe_get("mypage_scripts"))) &&
				    !strcmp_pnt(m->menu[i][j],
						"MyPage.asp")) // jump
					// over
					// MyPage.asp
...
#ifdef HAVE_MADWIFI
		else if (!strcmp_pnt(m->menu[i][0], "Wireless_Basic.asp") &&
			 !wifi && wimaxwifi) {
			websWrite(
				wp,
				"<li><a href=\"WiMAX.asp\"><script type=\"text/javascript\">Capture(bmenu.wimax)</script></a></li>\n");
