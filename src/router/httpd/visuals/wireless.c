...
void get_hostname_ip( char *type, char *filename )
{
	...
	while( fgets( line, sizeof( line ), fp ) != NULL )
	{
	    ...
	    if( sscanf( line, "%s %s %s", leases[0], leases[1], leases[2] ) !=
		3 )
		continue;
...
void save_hostname_ip( void )
{
	...
	while( fgets( line, sizeof( line ), fp ) != NULL )
	{
		...
		3 )
		continue;
	    snprintf( wl_clients[i].hwaddr, sizeof( wl_clients[i].hwaddr ),
		      "%s", leases[0] );
	    snprintf( wl_clients[i].ipaddr, sizeof( wl_clients[i].ipaddr ),
		      "%s", leases[1] );
	    snprintf( wl_clients[i].hostname,
		      sizeof( wl_clients[i].hostname ), "%s", leases[2] );
	    i++;
	}
	fclose( fp );
	...
	{			// online && have ip address
		...
		match = 0;
		if( !strcmp
		    ( wl_clients[j].hwaddr, wl_client_macs[i].hwaddr ) )
		{
