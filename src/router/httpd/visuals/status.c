...
void ej_show_status_setting(webs_t wp, int argc, char_t ** argv)
{
	...
	do_ej(METHOD_GET, NULL, "Status_Router1.asp", wp, NULL);
...
void ej_show_status(webs_t wp, int argc, char_t ** argv)
{
	...
	if (!strcmp(wan_proto, "pppoe")
		...
		if (!strcmp(type, "init")) {
			...
			if (retry_count == 0 || (!submit_type && !wan_link && nvram_matchi("gozila_action", 1))) {	// After refresh 2 times, if the status is
