...
void show_roaming(webs_t wp, char *var)
{
	...
	if ((v_show_preshared || v_show_owe || v_show_wparadius) &&
		...
		websWrite(
			wp,
			"<fieldset><legend><script type=\"text/javascript\">Capture(roaming.ft)</script></legend>");
		...
		websWrite(
			...
			vvar, var,
			nvram_default_matchi(bssft, 1, 0) ?
				"checked=\"checked\"" :
				"");
		websWrite(
			wp,
			...
			vvar, var,
			nvram_default_matchi(bssft, 0, 0) ?
				"checked=\"checked\"" :
				"");
		websWrite(wp, "</div>\n");
		char wnm[64];
		...
		websWrite(wp, "<div id=\"%s_iddomain\">\n", vvar);
		{
			if (nvram_nmatch("ap", "%s_mode", var) ||
				...
				websWrite(
					wp,
					"<input id=\"%s_nas\" name=\"%s_nas\" maxlength=\"48\" size=\"32\" value=\"%s\" />\n",
					var, var,
					nvram_default_get(temp,
							  "ap.example.com"));
				websWrite(wp, "</div>\n");
				websWrite(wp, "<div class=\"setting\">\n");
				show_caption(wp, "label", "roaming.domain",
					     NULL);
				sprintf(temp, "%s_domain", var);
				websWrite(
					wp,
					"<input id=\"%s_domain\" name=\"%s_domain\" maxlength=\"4\" size=\"6\" onblur=\"valid_domain(this)\" value=\"%s\" />\n",
					var, var,
					nvram_default_get(temp, "0000"));
				websWrite(wp, "</div>\n");

				websWrite(wp, "<div class=\"setting\">\n");
				show_caption(wp, "label",
					     "roaming.reassociation_deadline",
					     NULL);
				sprintf(temp, "%s_deadline", var);
				websWrite(
					wp,
					"<input id=\"%s_domain\" name=\"%s_deadline\" maxlength=\"6\" size=\"6\" onblur=\"valid_range(this,1000,65535,roaming.reassociation_deadline)\" value=\"%s\" />\n",
					var, var,
				...
				websWrite(
					wp,
					"<div class=\"label\"><script type=\"text/javascript\">Capture(roaming.ft_protocol)</script></div>\n");

				sprintf(wnm, "%s_ft_over_ds", var);
				showOptions_trans(
					wp, wnm, "0 1",
					(char *[]){ "roaming.ft_over_air",
						    "roaming.ft_over_ds" },
		...
		websWrite(
			wp,
			"show_layer_ext(document.getElementsByName(\"%s_ft\"), \"%s_iddomain\", %s);\n",
	...
	websWrite(
		wp,
		"<fieldset><legend><script type=\"text/javascript\">Capture(roaming.s80211v)</script></legend>");
	websWrite(
		wp,
	...
	websWrite(
		...
		var, var, var,
		nvram_default_matchi(s80211v, 1, 0) ? "checked=\"checked\"" :
						      "");
	websWrite(
		wp,
		...
		var, var, var,
		nvram_default_matchi(s80211v, 0, 0) ? "checked=\"checked\"" :
						      "");
	websWrite(wp, "</div>\n");
	char wnm[64];
	char adv[64];
	websWrite(wp, "<div id=\"%s_id80211v\">\n", var);
	{
		...
		for (i = 0; (allTimezones[i].tz_name != NULL); i++) {
			websWrite(wp, "<option value=\"%s\" %s>%s</option>\n",
				  ...
				  !strcmp(allTimezones[i].tz_name, tz) ?
					  "selected=\"selected\"" :
					  "",
				  allTimezones[i].tz_name);
		}
		...
		websWrite(
			wp,
			"show_layer_ext(document.getElementsByName(\"%s_time_advertisement\"), \"%s_id_time_zone\", %s);\n",
			var, var, nvram_matchi(adv, 1) ? "true" : "false");
		websWrite(wp, "//]]>\n</script>\n");

		showRadioPrefix(wp, "roaming.wnm_sleep_mode", "wnm_sleep_mode",
				var);
		showRadioPrefix(wp, "roaming.wnm_sleep_mode_no_keys",
				"wnm_sleep_mode_no_keys", var);
		showRadioPrefix(wp, "roaming.bss_transition", "bss_transition",
				var);
	...
	websWrite(
		wp,
		"<fieldset><legend><script type=\"text/javascript\">Capture(roaming.s80211k)</script></legend>");
	...
	websWrite(
		...
		var, var, var,
		nvram_default_matchi(s80211k, 1, 0) ? "checked=\"checked\"" :
						      "");
	websWrite(
		wp,
		...
		var, var, var,
		nvram_default_matchi(s80211k, 0, 0) ? "checked=\"checked\"" :
						      "");
	websWrite(wp, "</div>\n");
	websWrite(wp, "<div id=\"%s_id80211k\">\n", var);
	{
		showRadioPrefix(wp, "roaming.rrm_neighbor_report",
				"rrm_neighbor_report", var);
		showRadioPrefix(wp, "roaming.rrm_beacon_report",
				"rrm_beacon_report", var);
	}
	websWrite(wp, "</div>\n");
	websWrite(wp, "</fieldset> <br />\n");
	...
	if (v_show_preshared || v_show_owe || v_show_wparadius) {
		websWrite(
			wp,
			"<fieldset><legend><script type=\"text/javascript\">Capture(roaming.mbo)</script></legend>");
		websWrite(
			wp,
		...
		websWrite(
			...
			vvar, var,
			nvram_default_matchi(mbo, 1, has_ax(var) ? 1 : 0) ?
				"checked=\"checked\"" :
				"");
		websWrite(
			wp,
			...
			vvar, var,
			nvram_default_matchi(mbo, 0, has_ax(var) ? 1 : 0) ?
				"checked=\"checked\"" :
				"");
		websWrite(wp, "</div>\n");
		...
		websWrite(wp, "<div id=\"%s_idmbo\">\n", vvar);
		{
			...
			websWrite(
				wp,
				"<div class=\"label\"><script type=\"text/javascript\">Capture(roaming.mbo_cell_data_conn_pref)</script></div>\n");
			showOptions_trans(wp, wnm, "0 1 255",
					  (char *[]){ "share.excluded",
						      "share.not_prefered",
						      "share.prefered" },
					  nvram_default_get(wnm, "0"));
	...
	websWrite(wp, "<div id=\"%s_id80211k2\">\n", var);
	{
		websWrite(wp, "<div id=\"%s_id80211v2\">\n", var);
		{
			websWrite(
				wp,
				"<fieldset><legend><script type=\"text/javascript\">Capture(roaming.usteer)</script></legend>");
	...
	websWrite(
		wp,
		"show_layer_ext(document.getElementsByName(\"%s_80211v\"), \"%s_id80211v\", %s);\n",
		var, var, nvram_matchi(s80211v, 1) ? "true" : "false");
	websWrite(
		wp,
		"show_layer_ext(document.getElementsByName(\"%s_80211k\"), \"%s_id80211k\", %s);\n",
		var, var, nvram_matchi(s80211k, 1) ? "true" : "false");
	websWrite(
		wp,
		"show_layer_ext(document.getElementsByName(\"%s_80211v\"), \"%s_id80211v2\", %s);\n",
		var, var, nvram_matchi(s80211v, 1) ? "true" : "false");
	websWrite(
		wp,
		"show_layer_ext(document.getElementsByName(\"%s_80211k\"), \"%s_id80211k2\", %s);\n",
		var, var, nvram_matchi(s80211k, 1) ? "true" : "false");
	websWrite(
		wp,
		"show_layer_ext(document.getElementsByName(\"%s_usteer\"), \"%s_idusteer\", %s);\n",
		var, var, nvram_matchi(usteer, 1) ? "true" : "false");
	websWrite(
		wp,
		"show_layer_ext(document.getElementsByName(\"%s_mbo\"), \"%s_idmbo\", %s);\n",
		var, var, nvram_matchi(mbo, 1) ? "true" : "false");
	websWrite(wp, "//]]>\n</script>\n");
}
...
static void ej_show_roaming_single(webs_t wp, int argc, char_t **argv,
				   char *prefix)
{
	char *next;
	char var[80];
	...
	if (!nvram_nmatch("disabled", "%s_net_mode", prefix) &&
		...
		websWrite(
			wp,
			"<h2><script type=\"text/javascript\">Capture(roaming.roaming)</script> %s</h2>\n",
			prefix);
...
EJ_VISIBLE void ej_show_roaming(webs_t wp, int argc, char_t **argv)
{
	...
	websWrite(
		wp,
		"<h2><script type=\"text/javascript\">Capture(roaming.usteer_options)</script></h2>\n");
	websWrite(wp, "<fieldset>");
	...
	showInputNum(wp, "roaming.sta_block_timeout",
		     "usteer_sta_block_timeout", 6, 6, 30000);
	showInputNum(wp, "roaming.local_sta_timeout",
		     "usteer_local_sta_timeout", 6, 6, 120000);
	showInputNum(wp, "roaming.local_sta_update", "usteer_local_sta_update",
		     6, 6, 1000);
	showInputNum(wp, "roaming.max_neighbor_reports",
		     "usteer_max_neighbor_reports", 4, 4, 6);
	showInputNum(wp, "roaming.max_retry_band", "usteer_max_retry_band", 2,
		     2, 6);
	showInputNum(wp, "roaming.seen_policy_timeout",
		     "usteer_seen_policy_timeout", 6, 6, 30000);
	showInputNum(wp, "roaming.measurement_report_timeout",
		     "usteer_measurement_report_timeout", 6, 6, 120000);
	showInputNum(wp, "roaming.load_balancing_threshold",
		     "usteer_load_balancing_threshold", 4, 4, 0);
	showInputNum(wp, "roaming.band_steering_threshold",
		     "usteer_band_steering_threshold", 4, 4, 0);
	showInputNum(wp, "roaming.remote_update_interval",
		     "usteer_remote_update_interval", 6, 6, 1000);
	showInputNum(wp, "roaming.remote_node_timeout",
		     "usteer_remote_node_timeout", 6, 6, 50);
	...
	showInputNum(wp, "roaming.min_connect_snr", "usteer_min_connect_snr", 4,
		     4, 0);
	showInputNum(wp, "roaming.min_snr", "usteer_min_snr", 4, 4, 15);
	showInputNum(wp, "roaming.min_snr_kick_delay",
		     "usteer_min_snr_kick_delay", 6, 6, 5000);
	showInputNum(wp, "roaming.steer_reject_timeout",
		     "usteer_steer_reject_timeout", 6, 6, 60000);
	showInputNum(wp, "roaming.roam_process_timeout",
		     "usteer_roam_process_timeout", 6, 6, 5000);
	showInputNum(wp, "roaming.roam_scan_snr", "usteer_roam_scan_snr", 4, 4,
		     20);
	showInputNum(wp, "roaming.roam_scan_tries", "usteer_roam_scan_tries", 2,
		     2, 6);
	showInputNum(wp, "roaming.roam_scan_timeout",
		     "usteer_roam_scan_timeout", 6, 6, 60000);
	showInputNum(wp, "roaming.roam_scan_interval",
		     "usteer_roam_scan_interval", 6, 6, 15000);
	showInputNum(wp, "roaming.roam_trigger_snr", "usteer_roam_trigger_snr",
		     4, 4, 15);
	showInputNum(wp, "roaming.roam_trigger_interval",
		     "usteer_roam_trigger_interval", 6, 6, 180000);
	showInputNum(wp, "roaming.roam_kick_delay", "usteer_roam_kick_delay", 6,
		     6, 100);
	showInputNum(wp, "roaming.signal_diff_threshold",
		     "usteer_signal_diff_threshold", 4, 4, 12);
	showInputNum(wp, "roaming.initial_connect_delay",
		     "usteer_initial_connect_delay", 6, 6, 0);

	showInputNum(wp, "roaming.band_steering_interval",
		     "usteer_band_steering_interval", 6, 6, 120000);
	showInputNum(wp, "roaming.band_steering_min_snr",
		     "usteer_band_steering_min_snr", 4, 4, 20);
	showInputNum(wp, "roaming.link_measurement_interval",
		     "usteer_link_measurement_interval", 6, 6, 30000);
	...
	websWrite(
		...
		nvram_default_matchi("usteer_load_kick_enabled", 1, 0) ?
			"checked=\"checked\"" :
			"");
	websWrite(
		wp,
		...
		nvram_default_matchi("usteer_load_kick_enabled", 0, 0) ?
			"checked=\"checked\"" :
			"");
	websWrite(wp, "</div>\n");
	...
	{
		showInputNum(wp, "roaming.load_kick_threshold",
			     "usteer_load_kick_threshold", 4, 4, 75);
		showInputNum(wp, "roaming.load_kick_delay",
			     "usteer_load_kick_delay", 7, 6, 10000);
		showInputNum(wp, "roaming.load_kick_min_clients",
			     "usteer_load_kick_min_clients", 4, 4, 10);
		showInputNum(wp, "roaming.load_kick_reason_code",
			     "usteer_load_kick_reason_code", 2, 2, 5);
	}
	websWrite(wp, "</div>\n");
	websWrite(wp, "</fieldset> <br />\n");
	...
	websWrite(
		wp,
		"show_layer_ext(document.getElementsByName(\"usteer_load_kick_enabled\"), \"id_load_kick\", %s);\n",
