...
void show_roaming(webs_t wp, char *var)
{
	...
	if (!nvram_nmatch("ap", "%s_mode", var) && !nvram_nmatch("wdsap", "%s_mode", var) && !nvram_nmatch("wdsap_mtik", "%s_mode", var) && !nvram_nmatch("apup", "%s_mode", var))
		return;
...
static int ej_show_roaming_single(webs_t wp, int argc, char_t **argv, char *prefix)
{
	...
	if (!nvram_nmatch("disabled", "%s_net_mode", prefix) && !nvram_nmatch("disabled", "%s_mode", prefix)) {
		...
		websWrite(wp, "</fieldset>\n<br />\n");
		if (nvram_nmatch("ap", "%s_mode", prefix) || nvram_nmatch("wdsap", "%s_mode", prefix) || nvram_nmatch("wdsap_mtik", "%s_mode", prefix) ||
