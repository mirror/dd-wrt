	...
	if ((v_show_preshared || v_show_owe || v_show_wparadius) && !is_brcmfmac(var)) {
		...
		websWrite(wp, "<div id=\"%s_iddomain\">\n", vvar);
		{
			if (nvram_nmatch("ap", "%s_mode", var) || nvram_nmatch("wdsap", "%s_mode", var) ||
			    nvram_nmatch("wdsap_mtik", "%s_mode", var) || nvram_nmatch("apup", "%s_mode", var)) {
				...
				websWrite(
					...
					var, var, nvram_default_get(temp, "1000"));
				websWrite(wp, "</div>\n");
				...
				showOptions_trans(wp, wnm, "0 1", (char *[]){ "roaming.ft_over_air", "roaming.ft_over_ds" },
		...
		websWrite(wp, "show_layer_ext(document.getElementsByName(\"%s_ft\"), \"%s_iddomain\", %s);\n", var, vvar,
			  nvram_matchi(bssft, 1) ? "true" : "false");
		websWrite(wp, "//]]>\n</script>\n");
	...
	websWrite(wp, "<div id=\"%s_id80211v\">\n", var);
	{
		...
		for (i = 0; (allTimezones[i].tz_name != NULL); i++) {
			websWrite(wp, "<option value=\"%s\" %s>%s</option>\n", allTimezones[i].tz_name,
				  !strcmp(allTimezones[i].tz_name, tz) ? "selected=\"selected\"" : "", allTimezones[i].tz_name);
		}
		websWrite(wp, "</select>\n");
		websWrite(wp, "</div>\n");
		websWrite(wp, "</div>\n");
	...
	if ((v_show_preshared || v_show_owe || v_show_wparadius) && !is_brcmfmac(var)) {
		...
		websWrite(wp, "<div id=\"%s_idmbo\">\n", vvar);
		{
			...
			showOptions_trans(wp, wnm, "0 1 255",
					  (char *[]){ "share.excluded", "share.not_prefered", "share.prefered" },
					  nvram_default_get(wnm, "0"));
			websWrite(wp, "</div>\n");
		}
		websWrite(wp, "</div>\n");
...
static int ej_show_roaming_single(webs_t wp, int argc, char_t **argv, char *prefix)
{
	...
	if (!nvram_nmatch("disabled", "%s_net_mode", prefix) && !nvram_nmatch("disabled", "%s_mode", prefix)) {
		...
		if (nvram_nmatch("ap", "%s_mode", prefix) || nvram_nmatch("wdsap", "%s_mode", prefix) ||
		    nvram_nmatch("wdsap_mtik", "%s_mode", prefix) || nvram_nmatch("apup", "%s_mode", prefix))
			cnt++;
	}
...
EJ_VISIBLE void ej_show_roaming(webs_t wp, int argc, char_t **argv)
{
	...
	if (cnt) {
		...
		showInputNum(wp, "roaming.budget_6ghz", "usteer_budget_6ghz", 2, 2, 5);
		showRadio(wp, "roaming.prefer_6ghz", "usteer_prefer_6ghz");
		showRadio(wp, "roaming.prefer_he", "usteer_prefer_he");
		showRadio(wp, "roaming.prefer_eht", "usteer_prefer_eht");
