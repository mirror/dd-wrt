...
void showInputNum(webs_t wp, char *propname, char *nvname, int size, int maxsize, int def)
{
	...
	show_caption(wp, "label", propname, NULL);
	websWrite(wp,
		  "<input class=\"num\" aria-labeledby=\"Capture(%s)\" name=\"%s\" maxlength=\"%d\" size=\"%d\" value=\"%d\" />\n",
		  propname, nvname, maxsize, size, nvram_default_geti(nvname, def));
	websWrite(wp, "</div>\n");
}
...
void showInput(webs_t wp, char *propname, char *nvname, int size, int maxsize, char *def)
{
	...
	show_caption(wp, "label", propname, NULL);
	websWrite(wp,
		  "<input class=\"num\" aria-labeledby=\"Capture(%s)\" name=\"%s\" maxlength=\"%d\" size=\"%d\" value=\"%s\" />\n",
		  propname, nvname, maxsize, size, nvram_default_get(nvname, def));
	websWrite(wp, "</div>\n");
}
