		...
		foreach(word, wordlist, next)
		{
			...
			foreach(tword, twordlist, tnext)
			{
				...
				websWrite(wp, "document.write(\"<option value=\\\"%s\\\" %s >%s</option>\");\n",
					  matchname ? matchname : "",
					  (matchname && !strcmp(matchname, ipvsname)) ? "selected=\\\"selected\\\"" : "",
					  matchname);
			}
			websWrite(wp, "//]]>\n</script>\n</select>\n");
			websWrite(wp, "</td>");
			...
			websWrite(wp, "<td><input type=\"checkbox\" name=\"%s\" value=\"1\" %s/></td>\n", ipvs_name,
				  !strcmp(targetnat, "1") ? "checked=\"checked\"" : "");
			websWrite(
				wp,
...
EJ_VISIBLE void ej_show_ipvs(webs_t wp, int argc, char_t **argv)
{
	...
	foreach(word, wordlist, next)
	{
		...
		while (schedulers[scount] != NULL) {
			...
			websWrite(wp, "document.write(\"<option value=\\\"%s\\\" %s >%s</option>\");\n", sched,
				  !strcmp(sched, scheduler) ? "selected=\\\"selected\\\"" : "",
	...
	for (i = count; i < realcount; i++) {
		...
		while (schedulers[scount] != NULL) {
			...
			sprintf(translate, "networking.%s", sched);
			websWrite(wp, "document.write(\"<option value=\\\"%s\\\" %s >%s</option>\");\n", sched,
				  !strcmp(sched, "wrr") ? "selected=\\\"selected\\\"" : "",
				  tran_string(buf, sizeof(buf), translate));
		}
		websWrite(wp, "//]]>\n</script>\n</select>\n");
		websWrite(wp, "</td>");
