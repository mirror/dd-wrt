...
int
matrixssl_free_session (FILE * fp)
{
  matrixssl_freebuf ((int) fp);
...
do_matrixssl_recv (FILE * fp)
{
  ...
  while (1)
    ...
    decodeMore:
      ...
      alertDescription = 0;
      rc =
	matrixSslDecode (pbuf->ssl, &in, &out, &error, &alertLevel,
			 &alertDescription);
#ifdef DEBUG_MATRIXSSL
	  ...
	  while (more > 0)
	      ...
	      if (in_buf_size - (in.end - in.buf) <= more)
		{
		  in.start = in.buf = in_buf =
		    (char *) realloc (in_buf, in_buf_size + more);
		  in.end = in.buf + in.size;
		  in_buf_size += more;
		}
	      ret =
		recv ((int) fp, in.end, in_buf_size - (in.end - in.buf), 0);
  ...
  do
    {
      ...
      if (matrixSslEncode (pbuf->ssl, pbuf->ssl_send_buf + len, bytes, &out) <
	  0)
	return -1;
