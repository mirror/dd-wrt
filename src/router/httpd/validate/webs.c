...
void tunnel_save(webs_t wp)
{
	...
	for (i = 1; i < tunnels + 1; i++) {
		...
		for (peer = 0; peer < peers; peer++) {
			...
			copytonv(wp, "oet%d_namep%d", i,
				 peer); //for naming legend peer
...
void save_networking(webs_t wp)
{
	...
	for (i = 0; i < bridgescount; i++) {
		...
		if (has_vlan) {
			...
			sprintf(var, "bridgenoll%d", i);
			vlan = websGetVar(wp, var, NULL);
			sprintf(n, "%s_noll", ifname);
			if (!vlan || !*vlan) {
				nvram_seti(n, 1);
			} else {
				nvram_seti(n, 0);
...
void port_vlan_table_save(webs_t wp)
{
	...
	char portid[32], portvlan[64], buff[32] = { 0 }, *c;
	const char *next;
	char br0vlans[64], br1vlans[64], br2vlans[64];
	int portval;
