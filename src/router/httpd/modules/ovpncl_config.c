...
static int make_ovpncl_config(char *ovnpcl_fname)
{
	...
	if (!f1) {
		...
		fprintf(f1,
			"#OpenVPN client config generated, check if settings are correct see: %s, made by %s\n",
		...
		if (nvram_match("openvpn_proto", "tcp4-server") ||
		    nvram_match("openvpn_proto", "tcp-server")) {
			fprintf(f1, "proto %s\n", "tcp4");
		...
		if (nvram_invmatch("openvpn_dc1", "")) {
			sprintf(dataciphers, "%s",
				nvram_safe_get("openvpn_dc1"));
		}
		if (dataciphers[0] != '\0' &&
		    nvram_invmatch("openvpn_dc2", "")) {
			sprintf(dataciphers, "%s:%s", dataciphers,
				nvram_safe_get("openvpn_dc2"));
		} else if (nvram_invmatch("openvpn_dc2", "")) {
			sprintf(dataciphers, "%s",
				nvram_safe_get("openvpn_dc2"));
		}
		if (dataciphers[0] != '\0' &&
		    nvram_invmatch("openvpn_dc3", "")) {
			sprintf(dataciphers, "%s:%s", dataciphers,
				nvram_safe_get("openvpn_dc3"));
...
static int download_ovpncl_config(unsigned char method,
