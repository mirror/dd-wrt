...
validate_filter_tod (webs_t wp)
{
  ...
  if (atoi (time_all) == 1)
    ...
    {
      _start_hour =
	atoi (start_time) ? 12 + atoi (start_hour) : atoi (start_hour);
      ...
      websWrite (wp, " \
		<tr align=\"center\" bgcolor=\"#CCCCCC\" >\n\
			...
			<table width=\"150\" height=\"30\" border=\"1\" cellspacing=\"1\" bordercolor=\"#000000\" bgcolor=\"#FFFFFF\" style=\"border-collapse:collapse\" >\n\
				<tr>\n", i + 1, name);
	websWrite (wp, " \
			<td align=\"center\" width=\"14%\" bgcolor=\"%s\" ><script type=\"text/javascript\">Capture(%s)</script></td>\n\
			<td align=\"center\" width=\"14%\" bgcolor=\"%s\" ><script type=\"text/javascript\">Capture(%s)</script></td>\n\
			<td align=\"center\" width=\"14%\" bgcolor=\"%s\" ><script type=\"text/javascript\">Capture(%s)</script></td>\n\
			<td align=\"center\" width=\"14%\" bgcolor=\"%s\" ><script type=\"text/javascript\">Capture(%s)</script></td>\n",
	  ...
	  if (time_all == 1)
	  ...
	  else
	    {
	      snprintf (time_buf, sizeof (time_buf),
			"%02d:%02d %s - %02d:%02d %s",
			start_hour == 0 ? 12 : start_hour, start_min,
			start_time == 0 ? am : pm,
			end_hour == 0 ? 12 : end_hour, end_min,
			end_time == 0 ? am : pm);
	    }
	}
      websWrite (wp, " \
