...
void
gen_key (char *genstr, int weptype)
#ifdef TEST
     void main ()
#endif
{
...
#endif
  len = strlen (genstr);
  if (len == 0)			// 2001/05/16 Edison: Don't allow zero length passphrase.
    return;
  ...
  if (len)
      ...
      for (i = 0; i < 4; i++)
	{
	  for (j = 0; j < 5; j++)
	    {
	      /* Note that these three lines are equivalent to the Microsoft rand() function. */
	      randNumber *= 0x343fd;
	      randNumber += 0x269ec3;
