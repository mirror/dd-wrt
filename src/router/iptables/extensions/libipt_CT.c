...
//		xtables_param_act(XTF_ONLY_ONCE, "CT", "--zone", *flags & CT_OPT_ZONE);
		if (string_to_number(optarg, 0, UINT16_MAX, 
				     &zone))
...
static void
ct_print(const struct ipt_ip *ip,
...
static void
ct_save(const struct ipt_ip *ip, const struct ipt_entry_target *target)
{
	const struct xt_ct_target_info *info =
		(const struct xt_ct_target_info *)target->data;
...
static struct iptables_target ct_target = {
	.name		= "CT",
	.version	= IPTABLES_VERSION,
	.size		= IPT_ALIGN(sizeof(struct xt_ct_target_info)),
	.userspacesize		= IPT_ALIGN(sizeof(struct xt_ct_target_info)),
