...
static void 
opendpi_mt4_save(const struct ipt_ip *ip, const struct xt_entry_match *match)
{
...
static void opendpi_mt4_print(const struct ipt_ip *ip, const struct ipt_entry_match *match, int numeric)
{
	const struct xt_opendpi_mtinfo *info = (const void *)match->data;
	int i;
...
static void 
opendpi_mt_init (struct ipt_entry_match *match)
{
	struct xt_opendpi_mtinfo *info = (void *)match->data;
...
void _init(void)
{
	...
	register_match(&opendpi_mt4_reg);
}
