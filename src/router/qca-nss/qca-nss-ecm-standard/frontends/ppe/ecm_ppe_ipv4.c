 ...
 * 4 = 3 + TRACE
 */
#define DEBUG_LEVEL ECM_PPE_IPV4_DEBUG_LEVEL
...
#define ECM_PPE_IPV4_STATS_SYNC_COUNT 128	/* Sync max of 128 connections from ppe */

int ecm_ppe_ipv4_no_action_limit_default = 250;		/* Default no-action limit. */
int ecm_ppe_ipv4_driver_fail_limit_default = 250;		/* Default driver fail limit. */
int ecm_ppe_ipv4_nack_limit_default = 1;			/* Default nack limit. */
 ...
 */
uint32_t ecm_ppe_ipv4_accel_limit_mode = ECM_FRONT_END_ACCEL_LIMIT_MODE_UNLIMITED;
 ...
 */
DEFINE_SPINLOCK(ecm_ppe_ipv4_lock);			/* Protect against SMP access between netfilter, events and private threaded function. */

/*
 * PPE driver linkage
 */
static unsigned long ecm_ppe_ipv4_accel_cmd_time_avg_samples = 0;	/* Sum of time taken for the set of accel command samples, used to compute average time for an accel command to complete */
static unsigned long ecm_ppe_ipv4_accel_cmd_time_avg_set = 1;	/* How many samples in the set */
static unsigned long ecm_ppe_ipv4_decel_cmd_time_avg_samples = 0;	/* Sum of time taken for the set of accel command samples, used to compute average time for an accel command to complete */
static unsigned long ecm_ppe_ipv4_decel_cmd_time_avg_set = 1;	/* How many samples in the set */

/*
 * Debugfs dentry object.
 */
static struct dentry *ecm_ppe_ipv4_dentry;
...
/*
 * ecm_ppe_ipv4_accel_done_time_update()
 *	Record how long the command took to complete, updating average samples
 */
void ecm_ppe_ipv4_accel_done_time_update(struct ecm_front_end_connection_instance *feci)
{
	unsigned long delta;
	...
	ecm_ppe_ipv4_accel_cmd_time_avg_samples += delta;
	ecm_ppe_ipv4_accel_cmd_time_avg_set++;
	spin_unlock_bh(&ecm_ppe_ipv4_lock);
}

/*
 * ecm_ppe_ipv4_deccel_done_time_update()
 *	Record how long the command took to complete, updating average samples
 */
void ecm_ppe_ipv4_decel_done_time_update(struct ecm_front_end_connection_instance *feci)
{
	unsigned long delta;
	...
	spin_lock_bh(&ecm_ppe_ipv4_lock);
	ecm_ppe_ipv4_decel_cmd_time_avg_samples += delta;
	ecm_ppe_ipv4_decel_cmd_time_avg_set++;
	spin_unlock_bh(&ecm_ppe_ipv4_lock);
}
 ...
 */
static inline void ecm_ppe_ipv4_process_one_conn_sync_msg(struct ppe_drv_v4_conn_sync *sync)
{
...
/*
 * ecm_ppe_ipv4_stats_callback()
 *	Callback handler from the PPE.
 */
static void ecm_ppe_ipv4_stats_callback(void *app_data, struct ppe_drv_v4_conn_sync *conn_sync)
{
	ecm_ppe_ipv4_process_one_conn_sync_msg(conn_sync);
}

/*
 * ecm_ppe_ipv4_stats_sync_req_work()
 *	Schedule delayed work to process connection stats and request next sync
 */
static void ecm_ppe_ipv4_stats_sync_req_work(struct work_struct *work)
{
	/*
	...
	spin_lock_bh(&ecm_ppe_ipv4_lock);
	if (ecm_ppe_ipv4_accelerated_count == 0) {
		spin_unlock_bh(&ecm_ppe_ipv4_lock);
...
reschedule:
	...
	queue_delayed_work(ecm_ppe_ipv4_workqueue, &ecm_ppe_ipv4_work, ECM_PPE_IPV4_STATS_SYNC_PERIOD);
}

/*
 * ecm_ppe_ipv4_get_accel_limit_mode()
 */
static int ecm_ppe_ipv4_get_accel_limit_mode(void *data, u64 *val)
{
	*val = ecm_ppe_ipv4_accel_limit_mode;

	return 0;
}

/*
 * ecm_ppe_ipv4_set_accel_limit_mode()
 */
static int ecm_ppe_ipv4_set_accel_limit_mode(void *data, u64 val)
{
	DEBUG_TRACE("ecm_ppe_ipv4_accel_limit_mode = %x\n", (int)val);
	...
	if (val && (val ^ (ECM_FRONT_END_ACCEL_LIMIT_MODE_FIXED | ECM_FRONT_END_ACCEL_LIMIT_MODE_UNLIMITED))) {
		DEBUG_WARN("ecm_ppe_ipv4_accel_limit_mode = %x bad\n", (int)val);
		return -EINVAL;
	}

	ecm_ppe_ipv4_accel_limit_mode = (int)val;
 ...
 */
DEFINE_SIMPLE_ATTRIBUTE(ecm_ppe_ipv4_accel_limit_mode_fops, ecm_ppe_ipv4_get_accel_limit_mode, ecm_ppe_ipv4_set_accel_limit_mode, "%llu\n");
/*
 * ecm_ppe_ipv4_get_accel_cmd_avg_millis()
 */
static ssize_t ecm_ppe_ipv4_get_accel_cmd_avg_millis(struct file *file,
								...
								size_t sz, loff_t *ppos)
{
	 ...
	 */
	spin_lock_bh(&ecm_ppe_ipv4_lock);
	samples = ecm_ppe_ipv4_accel_cmd_time_avg_samples;
	set = ecm_ppe_ipv4_accel_cmd_time_avg_set;
	ecm_ppe_ipv4_accel_cmd_time_avg_samples /= ecm_ppe_ipv4_accel_cmd_time_avg_set;
	ecm_ppe_ipv4_accel_cmd_time_avg_set = 1;
	avg = ecm_ppe_ipv4_accel_cmd_time_avg_samples;
	spin_unlock_bh(&ecm_ppe_ipv4_lock);
 ...
 */
static struct file_operations ecm_ppe_ipv4_accel_cmd_avg_millis_fops = {
	.read = ecm_ppe_ipv4_get_accel_cmd_avg_millis,
};

/*
 * ecm_ppe_ipv4_get_decel_average_millis()
 */
static ssize_t ecm_ppe_ipv4_get_decel_cmd_avg_millis(struct file *file,
								char __user *user_buf,
								size_t sz, loff_t *ppos)
{
	unsigned long set;
	 ...
	 */
	spin_lock_bh(&ecm_ppe_ipv4_lock);
	samples = ecm_ppe_ipv4_decel_cmd_time_avg_samples;
	set = ecm_ppe_ipv4_decel_cmd_time_avg_set;
	ecm_ppe_ipv4_decel_cmd_time_avg_samples /= ecm_ppe_ipv4_decel_cmd_time_avg_set;
	ecm_ppe_ipv4_decel_cmd_time_avg_set = 1;
	avg = ecm_ppe_ipv4_decel_cmd_time_avg_samples;
	spin_unlock_bh(&ecm_ppe_ipv4_lock);
 ...
 */
static struct file_operations ecm_ppe_ipv4_decel_cmd_avg_millis_fops = {
	.read = ecm_ppe_ipv4_get_decel_cmd_avg_millis,
};

/*
 * ecm_ppe_ipv4_get_stats_request_counter()
 */
static ssize_t ecm_ppe_ipv4_get_stats_request_counter(struct file *file,
								char __user *user_buf,
								size_t sz, loff_t *ppos)
{
	char *buf;
	...
	ret = snprintf(buf, (ssize_t)PAGE_SIZE, "success=%lu\tfail=%lu\tnack=%lu\t\n",
			ecm_ppe_ipv4_stats_request_success, ecm_ppe_ipv4_stats_request_fail,
			ecm_ppe_ipv4_stats_request_nack);
	if (ret < 0) {
		kfree(buf);
		return -EFAULT;
	}
 ...
 * File operations for decel command average time.
 */
static struct file_operations ecm_ppe_ipv4_stats_request_counter_fops = {
	.read = ecm_ppe_ipv4_get_stats_request_counter,
};

/*
 * ecm_ppe_ipv4_sync_queue_init
...
static bool ecm_ppe_ipv4_sync_queue_init(void)
{
	...
	INIT_DELAYED_WORK(&ecm_ppe_ipv4_work, ecm_ppe_ipv4_stats_sync_req_work);
	queue_delayed_work(ecm_ppe_ipv4_workqueue, &ecm_ppe_ipv4_work, ECM_PPE_IPV4_STATS_SYNC_PERIOD);
...
/*
 * ecm_ppe_ipv4_sync_queue_exit
 *	 the workqueue for ipv4 stats sync
 */
static void ecm_ppe_ipv4_sync_queue_exit(void)
{
	/*
...
int ecm_ppe_ipv4_init(struct dentry *dentry)
{
	...
	if (!debugfs_create_u32("no_action_limit_default", S_IRUGO | S_IWUSR, ecm_ppe_ipv4_dentry,
					(u32 *)&ecm_ppe_ipv4_no_action_limit_default)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 no_action_limit_default file in debugfs\n");
		goto task_cleanup_1;
	}

	if (!debugfs_create_u32("driver_fail_limit_default", S_IRUGO | S_IWUSR, ecm_ppe_ipv4_dentry,
					(u32 *)&ecm_ppe_ipv4_driver_fail_limit_default)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 driver_fail_limit_default file in debugfs\n");
		goto task_cleanup_1;
	}

	if (!debugfs_create_u32("nack_limit_default", S_IRUGO | S_IWUSR, ecm_ppe_ipv4_dentry,
					(u32 *)&ecm_ppe_ipv4_nack_limit_default)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 nack_limit_default file in debugfs\n");
		goto task_cleanup_1;
	}

	if (!debugfs_create_u32("accelerated_count", S_IRUGO, ecm_ppe_ipv4_dentry,
					(u32 *)&ecm_ppe_ipv4_accelerated_count)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 accelerated_count file in debugfs\n");
		goto task_cleanup_1;
	}

	if (!debugfs_create_u32("pending_accel_count", S_IRUGO, ecm_ppe_ipv4_dentry,
					(u32 *)&ecm_ppe_ipv4_pending_accel_count)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 pending_accel_count file in debugfs\n");
		goto task_cleanup_1;
	}

	if (!debugfs_create_u32("pending_decel_count", S_IRUGO, ecm_ppe_ipv4_dentry,
					(u32 *)&ecm_ppe_ipv4_pending_decel_count)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 pending_decel_count file in debugfs\n");
		goto task_cleanup_1;
	}

	if (!debugfs_create_file("accel_limit_mode", S_IRUGO | S_IWUSR, ecm_ppe_ipv4_dentry,
					NULL, &ecm_ppe_ipv4_accel_limit_mode_fops)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 accel_limit_mode file in debugfs\n");
		goto task_cleanup_1;
	}
	 ...
	 */
	if (!ppe_drv_v4_stats_callback_register(ecm_ppe_ipv4_stats_callback, NULL)) {
		DEBUG_ERROR("Failed to register stats callback\n");
		goto task_cleanup_1;
	}

	if (!ecm_ppe_ipv4_sync_queue_init()) {
...
EXPORT_SYMBOL(ecm_ppe_ipv4_init);

/*
 * ecm_ppe_ipv4_exit()
 */
void ecm_ppe_ipv4_exit(void)
{
	if (!ecm_front_end_is_feature_supported(ECM_FE_FEATURE_PPE)) {
		DEBUG_INFO("PPE IPv4 is disabled\n");
		return;
	...
	DEBUG_INFO("ECM PPE IPv4 Module exit\n");
...
EXPORT_SYMBOL(ecm_ppe_ipv4_exit);
