...
unsigned int ecm_ported_ipv6_process(struct net_device *out_dev, struct net_device *out_dev_nat,
	...
	if (protocol == IPPROTO_TCP) {
		if (likely(ct)) {
			...
			if (!nf_ct_is_confirmed(ct)) {
				...
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TCP_NOT_CONFIRM);
				return NF_ACCEPT;
			}
			...
			if (!test_bit(IPS_ASSURED_BIT, &ct->status)) {
				...
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TCP_NOT_ESTAB);
				return NF_ACCEPT;
			}
		}
		...
		if (unlikely(!tcp_hdr)) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TCP_MALFORMED_HEADER);
			return NF_ACCEPT;
		}
		...
		if (likely(ct) && !nf_ct_is_confirmed(ct)) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_UDP_CONN_NOT_CONFIRM);
			return NF_ACCEPT;
		}
		...
		if (unlikely(!udp_hdr)) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_MALFORMED_UDP_HEADER);
			return NF_ACCEPT;
		}
		...
		ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_PROTOCOL_UNSUPPORTED);
		return NF_ACCEPT;
	}
	...
	if (unlikely(!ci)) {
		...
		if (ecm_ipv6_terminate_pending) {
			 ...
			 */
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_ECM_IN_TERMINATING_STATE);
			return NF_ACCEPT;
		}
		spin_unlock_bh(&ecm_ipv6_lock);
		...
		if (ct) {
			if (protocol == IPPROTO_TCP) {
				...
				if (ct->proto.tcp.state >= TCP_CONNTRACK_FIN_WAIT && ct->proto.tcp.state <= TCP_CONNTRACK_CLOSE) {
					...
					ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TCP_IN_TERMINATING_STATE);
					return NF_ACCEPT;
				}
				spin_unlock_bh(&ct->lock);
			}
		...
		case ECM_AE_CLASSIFIER_RESULT_NSS:
			if ((!ecm_nss_feature_check(skb, iph)) || (ecm_dir == ECM_DB_DIRECTION_EGRESS_NAT) || (ecm_dir == ECM_DB_DIRECTION_INGRESS_NAT)) {
				...
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_NSS_ACCEL_NOT_SUPPORTED);
				return NF_ACCEPT;
			}
		...
		case ECM_AE_CLASSIFIER_RESULT_SFE:
			if (!ecm_sfe_feature_check(skb, iph, is_routed)) {
				...
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_SFE_ACCEL_NOT_SUPPORTED);
				return NF_ACCEPT;
			}
		...
		case ECM_AE_CLASSIFIER_RESULT_PPE_DS:
			if ((!ecm_ppe_feature_check(skb, iph)) || (ecm_dir == ECM_DB_DIRECTION_EGRESS_NAT) || (ecm_dir == ECM_DB_DIRECTION_INGRESS_NAT)) {
				...
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_PPE_DS_ACCEL_NOT_SUPPORTED);
				return NF_ACCEPT;
			}
		...
		case ECM_AE_CLASSIFIER_RESULT_PPE_VP:
			if ((!ecm_ppe_feature_check(skb, iph)) || (ecm_dir == ECM_DB_DIRECTION_EGRESS_NAT) || (ecm_dir == ECM_DB_DIRECTION_INGRESS_NAT)) {
				DEBUG_WARN("Unsupported feature found for PPE acceleration\n");
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_PPE_VP_ACCEL_NOT_SUPPORTED);
				return NF_ACCEPT;
			}
		...
		case ECM_AE_CLASSIFIER_RESULT_NOT_YET:
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_AE_TYPE_NOT_ASSIGNED);
			return NF_ACCEPT;
		...
		default:
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_UNKNOWN_AE_TYPE);
			return NF_ACCEPT;
		}
...
precedence_alloc:
		for (i = 0; i <= ECM_AE_PRECEDENCE_MAX; i++) {
			if (ae_precedence[i].ae_type == ECM_FRONT_END_ENGINE_MAX) {
				...
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_PRECEDENCE_ALLOC_FAIL);
				return NF_ACCEPT;
			}
...
feci_alloc_done:
		...
		if (!ecm_front_end_ipv6_interface_construct_set_and_hold(skb, sender, ecm_dir, is_routed,
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_FRONTEND_CONSTRUCT_FAIL);
			goto fail_1;
		}
		...
		if (from_list_first == ECM_DB_IFACE_HEIRARCHY_MAX) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_FROM_HIERARCHY_CREATION_FAIL);
			goto fail_2;
		}
		...
		if (!ni[ECM_DB_OBJ_DIR_FROM]) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_FROM_NODE_FAIL);
			goto fail_2;
		}
		...
		if (!mi[ECM_DB_OBJ_DIR_FROM]) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_FROM_MAPPING_FAIL);
			goto fail_3;
		}
		...
		if (to_list_first == ECM_DB_IFACE_HEIRARCHY_MAX) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TO_HIERARCHY_CREATION_FAIL);
			goto fail_4;
		}
		...
		if (!ni[ECM_DB_OBJ_DIR_TO]) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TO_NODE_FAIL);
			goto fail_4;
		}
		...
		if (!mi[ECM_DB_OBJ_DIR_TO]) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TO_MAPPING_FAIL);
			goto fail_5;
		}
		...
		if (from_nat_list_first == ECM_DB_IFACE_HEIRARCHY_MAX) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_FROM_NAT_HIERARCHY_CREATION_FAIL);
			goto fail_6;
		}
		...
		if (!ni[ECM_DB_OBJ_DIR_FROM_NAT]) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_FROM_NAT_NODE_FAIL);
			goto fail_6;
		}
		...
		if (!mi[ECM_DB_OBJ_DIR_FROM_NAT]) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_FROM_NAT_MAPPING_FAIL);
			goto fail_7;
		}
		...
		if (to_nat_list_first == ECM_DB_IFACE_HEIRARCHY_MAX) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TO_NAT_HIERARCHY_CREATION_FAIL);
			goto fail_8;
		}
		...
		if (!ni[ECM_DB_OBJ_DIR_TO_NAT]) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TO_NAT_NODE_FAIL);
			goto fail_8;
		}
		...
		if (!mi[ECM_DB_OBJ_DIR_TO_NAT]) {
			DEBUG_WARN("Failed to establish dest mapping\n");
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TO_NAT_MAPPING_FAIL);
			goto fail_9;
		}
		...
		if (!ecm_db_connection_add_vlan_filter(nci, ni, skb, ECM_DB_OBJ_DIR_FROM, ECM_DB_OBJ_DIR_TO, is_routed)) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_VLAN_FILTER_ADD_FAIL);
			goto fail_10;
		}
#endif
		...
		if (!dci) {
			...
			ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_DEFAULT_CLASSIFIER_ALLOC_FAIL);
			goto fail_10;
		}
		...
		for (classifier_type = ECM_CLASSIFIER_TYPE_DEFAULT + 1; classifier_type < ECM_CLASSIFIER_TYPES; ++classifier_type) {
			...
			if (aci) {
				...
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_CLASSIFIER_ASSIGN_FAIL);
				goto fail_11;
			}
		}
...
fail_1:
	...
	if (!ecm_db_connection_is_routed_get(ci) && (ecm_dir == ECM_DB_DIRECTION_NON_NAT)) {
		...
		ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_IGNORE_BRIDGE_PACKETS);
		return NF_ACCEPT;
	}
	...
	if (unlikely(ecm_interface_igs_enabled)) {
		...
		if (feci->accel_engine == ECM_FRONT_END_ENGINE_NSS) {
			if (!ecm_nss_common_igs_acceleration_is_allowed(feci, skb)) {
				...
				ecm_front_end_connection_deref(feci);
				ecm_db_connection_deref(ci);
				ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_IGS_ACCEL_DENIED);
				return NF_ACCEPT;
			}
		}
		ecm_front_end_connection_deref(feci);
	...
	if (!ECM_IP_ADDR_MATCH(ip_src_addr_nat, match_addr) && ct && (sender == ECM_TRACKER_SENDER_TYPE_SRC)) {
		...
		ecm_db_connection_deref(ci);
		ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_SRC_NAT_IP_CHANGED);
		return NF_ACCEPT;
	}
	...
	if (ecm_db_connection_defunct_timer_no_touch_get(ci)) {
		...
		ecm_db_connection_deref(ci);
		ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_TOUCH_TIMER_NOT_SET);
		return NF_ACCEPT;
	}
	...
	if (!ecm_db_connection_defunct_timer_touch(ci)) {
		ecm_db_connection_deref(ci);
		ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_DB_CONN_TIMER_EXPIRED);
		return NF_ACCEPT;
	}
	...
	if (prevalent_pr.drop) {
		...
		ecm_db_connection_deref(ci);
		ecm_stats_v6_inc(ECM_STATS_V6_EXCEPTION_PORTED, ECM_STATS_V6_EXCEPTION_PORTED_DROP_BY_CLASSIFIER);
		return NF_ACCEPT;
	}
