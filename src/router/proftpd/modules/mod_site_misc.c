 ...
 *
 * $Id: mod_site_misc.c,v 1.1 2006/04/24 11:39:28 honor Exp $
 */
...
static time_t site_misc_mktime(unsigned int year, unsigned int month,
    unsigned int mday, unsigned int hour, unsigned int min, unsigned sec) {
  struct tm tm;
...
MODRET site_misc_utime(cmd_rec *cmd) {
  ...
  if (strcasecmp(cmd->argv[1], "UTIME") == 0) {
    ...
    if (strlen(cmd->argv[2]) != 12 && strlen(cmd->argv[2]) != 14) {
    ...
    tmbuf.actime = tmbuf.modtime = site_misc_mktime(year, month, day, hour, min, sec);
