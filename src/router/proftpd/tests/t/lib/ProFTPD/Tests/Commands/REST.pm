...
sub rest_2gb_last_byte {
  ...
  # (IMHO) seeking to just before the last byte of the file on the server, and
  # expecting just a single byte.
  #
  ...
  if ($pid) {
    eval {
      ...
      ($resp_code, $resp_msg) = $client->rest($rest_len);
      ...
      unless ($conn) {
        die("RETR $test_file failed: " . $client->response_code() . " " .
          $client->response_msg());
      }
...
sub rest_4gb_last_byte {
  ...
  if (open(my $fh, "> $test_file")) {
    ...
    # Seek to the $GB limit, then fill the rest with 'A'
