package ProFTPD::Tests::Commands::MLST;
...
my $TESTS = {
  ...
  mlst_path_with_spaces_ok => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  mlst_nonascii_chars_bug3032 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },
...
sub mlst_fails_enoent {
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg);
      eval { ($resp_code, $resp_msg) = $client->mlst($sub_dir) };
      unless ($@) {
      ...
      $expected = "'$sub_dir' cannot be listed";
...
sub mlst_fails_eperm {
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg);
      eval { ($resp_code, $resp_msg) = $client->mlst($test_file) };
      unless ($@) {
        die("MLST succeeded unexpectedly ($resp_code $resp_msg)");
      ...
      $expected = "'$test_file' cannot be listed";
      $self->assert($expected eq $resp_msg,
        test_msg("Expected '$expected', got '$resp_msg'"));
    };
...
sub mlst_hidden_file_ok {
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg);
      ($resp_code, $resp_msg) = $client->mlst('test.foo');
      ...
      # Bug#3190 occurred because mod_facts' handling of MLST was not checking
      # to see if the requested file has been hidden (e.g. via HideFiles).
