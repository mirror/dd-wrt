package ProFTPD::Tests::Config::MaxClientsPerHost;
...
my $TESTS = {
  maxclientsperhost_one => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  maxclientsperhost_one_multi_conns => {
    order => ++$order,
    test_class => [qw(forking)],
  },
...
sub maxclientsperhost_one_multi_conns {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
