package ProFTPD::Tests::Config::MaxClientsPerUser;
...
my $TESTS = {
  maxclientsperuser_one => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  maxclientsperuser_one_multi_conns => {
    order => ++$order,
    test_class => [qw(forking)],
  },
...
sub maxclientsperuser_one_multi_conns {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
