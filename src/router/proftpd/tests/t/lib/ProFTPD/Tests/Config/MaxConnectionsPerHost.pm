package ProFTPD::Tests::Config::MaxConnectionsPerHost;
...
my $TESTS = {
  maxconnsperhost_one_multi_conns => {
    order => ++$order,
...
sub maxconnsperhost_one_multi_conns {
  my $self = shift;
  ...
  if ($pid) {
    eval {
      ...
      # ...but the next clients shouldn't be able to connect at all.
      for (my $i = 0; $i < 10; $i++) {
        eval {
