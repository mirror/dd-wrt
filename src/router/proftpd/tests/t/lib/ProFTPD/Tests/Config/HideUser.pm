...
my $TESTS = {
  hideuser_explicit_user => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },

  hideuser_session_user => {
    order => ++$order,
    test_class => [qw(forking rootprivs)],
  },
...
sub hideuser_virtual_user_bug3934 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
