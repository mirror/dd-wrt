package ProFTPD::Tests::Modules::mod_tls_redis;
...
my $TESTS = {
  tls_sess_cache_redis => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  tls_stapling_on_redis => {
    order => ++$order,
    test_class => [qw(forking)],
  },
...
sub set_up {
  ...
  # Clear the Redis server before each unit test
  my $redis_server = $ENV{REDIS_SERVER} ? $ENV{REDIS_SERVER} : "127.0.0.1:6379";
  unless ($redis_server =~ /:/) {
    $redis_server .= ':6379';
  }
  ...
  # First, make sure that a Redis is running
  my $pong = $redis->ping();
  unless ($pong) {
    die("Can't ping Redis server '$redis_server'");
  }
...
sub tls_sess_cache_redis {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  my $setup = test_setup($tmpdir, 'tls_redis');
  ...
  my $config = {
    ...
    Trace => 'tls:20 redis:30 tls.redis:20',
...
sub tls_stapling_on_redis {
  ...
  my $config = {
    ...
    SystemLog => $setup->{log_file},
    TraceLog => $setup->{log_file},
    Trace => 'tls:20 redis:20 tls.redis:20',
