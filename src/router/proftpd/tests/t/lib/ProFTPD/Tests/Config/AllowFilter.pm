package ProFTPD::Tests::Config::AllowFilter;
...
my $TESTS = {
  allowfilter_dele_allowed => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_dele_denied => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_mkd_allowed => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_mkd_denied => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_rmd_allowed => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_rmd_denied => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_rnfr_allowed => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_rnfr_denied => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_rnto_allowed => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_rnto_denied => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_stor_allowed => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_stor_denied => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  allowfilter_stor_denied_nocase_bug3592 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  allowfilter_stor_denied_nocase_bug3609 => {
    order => ++$order,
...
sub allowfilter_mkd_denied {
  ...
  if ($pid) {
    eval {
      ...
      $expected = "$dirname: Permission denied";
      $self->assert($expected eq $resp_msg,
        test_msg("Expected '$expected', got '$resp_msg'"));
...
sub allowfilter_stor_denied_nocase_bug3592 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
...
sub allowfilter_stor_denied_nocase_bug3609 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
