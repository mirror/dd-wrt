package ProFTPD::Tests::Config::Limit::Filters;
...
my $TESTS = {
  filter_dele_allow_bug2067 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  filter_dele_deny_bug2067 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  filter_stor_allow_bug2067 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },
