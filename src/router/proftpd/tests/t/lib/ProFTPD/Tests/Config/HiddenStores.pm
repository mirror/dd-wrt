...
my $TESTS = {
  ...
  hiddenstores_bug3156 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  hiddenstores_prefix_bug3294 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  hiddenstores_bool_prefix_bug3294 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  hiddenstores_suffix_bug3872 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },
...
sub hiddenstores_bug3156 {
  ...
  if ($pid) {
    eval {
      ...
      # Make sure we're using binary transfers
      $client->type('binary');

      # Bug#3156 is a request to allow resumed downloads even if HiddenStores
      # is in effect.
      $client->rest(1);
...
sub hiddenstores_prefix_bug3294 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  ...
  my $hiddenstore_prefix = "foobar";

  my $hidden_file = File::Spec->rel2abs("$tmpdir/." . $hiddenstore_prefix . ".test.txt.");
  my $test_file = File::Spec->rel2abs("$tmpdir/test.txt");
...
sub hiddenstores_bool_prefix_bug3294 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
...
sub hiddenstores_suffix_bug3872 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  ...
  my $hiddenstore_prefix = "foobar-";
  my $hiddenstore_suffix = "-quxx-quzz";

  my $hidden_file = File::Spec->rel2abs("$tmpdir/" . $hiddenstore_prefix . "test.txt" . $hiddenstore_suffix);
  my $test_file = File::Spec->rel2abs("$tmpdir/test.txt");
