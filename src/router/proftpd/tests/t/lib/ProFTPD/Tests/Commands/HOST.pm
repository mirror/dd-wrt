package ProFTPD::Tests::Commands::HOST;
...
my $TESTS = {
  ...
  host_literal_ipv6_with_port_fails => {
    order => ++$order,
    test_class => [qw(feature_ipv6 forking)],
  },

  host_invalid_ipv4_fails => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  host_ipv4_with_port_fails => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  host_unknown_host_fails => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  host_known_ipv4_same_host_ok => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  host_known_ipv6_same_host_ok => {
    order => ++$order,
    test_class => [qw(feature_ipv6 forking)],
  },

  host_known_ipv4_different_host_fails => {
    order => ++$order,
    test_class => [qw(forking)],
  },
...
sub host_literal_ipv6_fails_useipv6_off {
  ...
  if ($pid) {
    eval {
      ...
      $expected = "$host: Invalid hostname provided";
      $self->assert($expected eq $resp_msg,
        test_msg("Expected response message '$expected', got '$resp_msg'"));
    };
...
sub host_literal_ipv6_with_port_fails {
  ...
  if ($pid) {
    eval {
      ...
      $expected = "$host: Invalid IPv6 address provided";
      $self->assert($expected eq $resp_msg,
        test_msg("Expected response message '$expected', got '$resp_msg'"));
    };
...
sub host_unknown_host_fails {
  ...
  if ($pid) {
    eval {
      my $client = ProFTPD::TestSuite::FTP->new('127.0.0.1', $port);
      my $host = "nosuchname.proftpd.org";
...
sub host_known_ipv4_same_host_ok {
  ...
  if ($pid) {
    eval {
      ...
      $expected = "HOST command successful";
      $self->assert($expected eq $resp_msg,
        test_msg("Expected response message '$expected', got '$resp_msg'"));
    };
