...
sub eprt_ipv6_ok {
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg);
      ($resp_code, $resp_msg) = $client->eprt('|2|::ffff:127.0.0.1|4856|');
...
sub eprt_fails_bad_values {
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg);
      eval { ($resp_code, $resp_msg) = $client->eprt('|1|100.200.300.400|0|') };
      unless ($@) {
        die("EPRT succeeded unexpectedly");
...
sub eprt_during_xfer_bug3487 {
  ...
  if ($pid) {
    eval {
      ...
      eval { $client->eprt('|1|127.0.0.1|4856|') };
      unless ($@) {
        die("EPRT succeeded unexpectedly");
      }
      ...
      $expected = 'EPRT: data transfer in progress';
      $self->assert($expected eq $resp_msg,
        test_msg("Expected '$expected', got '$resp_msg'"));
