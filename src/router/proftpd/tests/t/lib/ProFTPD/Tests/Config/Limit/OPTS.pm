package ProFTPD::Tests::Config::Limit::OPTS;
...
my $TESTS = {
  opts_limit_utf8_bug3438 => {
    order => ++$order,
    test_class => [qw(bug forking mod_lang)],
  },

  opts_limit_all_bug3438 => {
    order => ++$order,
    test_class => [qw(bug forking mod_lang)],
  },
