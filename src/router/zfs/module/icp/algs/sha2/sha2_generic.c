...
#define	WU(j) (W[j & 15] += sigma1(W[(j + 14) & 15]) + W[(j + 9) & 15] \
	+ sigma0(W[(j + 1) & 15]))
...
static void sha256_update(sha256_ctx *ctx, const uint8_t *data, size_t len)
{
	...
	if (len >= 64) {
		...
		ops->tf256(ctx->state, data, blocks);
		len -= bytes;
		total += (bytes) * 8;
		data += bytes;
	}
...
static void sha512_update(sha512_ctx *ctx, const uint8_t *data, size_t len)
{
	...
	if (len >= 128) {
		...
		ops->tf512(ctx->state, data, blocks);
		len -= bytes;
		total += (bytes) * 8;
		data += bytes;
	}
...
static void sha256_final(sha256_ctx *ctx, uint8_t *result)
{
...
static void sha384_final(sha512_ctx *ctx, uint8_t *result)
{
	uint64_t mlen, pos = ctx->count[0];
	uint8_t *m = ctx->wbuf;
...
static void sha512_final(sha512_ctx *ctx, uint8_t *result)
{
	uint64_t mlen, pos = ctx->count[0];
	uint8_t *m = ctx->wbuf;
	uint64_t *R = (uint64_t *)result;
...
static void sha512_256_final(sha512_ctx *ctx, uint8_t *result)
{
	uint64_t mlen, pos = ctx->count[0];
	uint8_t *m = ctx->wbuf;
	uint64_t *R = (uint64_t *)result;
...
/* SHA2 Init function */
void
SHA2Init(SHA2_CTX *ctx, int algotype)
{
	ctx->algotype = algotype;
	switch (ctx->algotype) {
		case SHA256:
			ctx->sha256.state[0] = 0x6a09e667;
			ctx->sha256.state[1] = 0xbb67ae85;
			ctx->sha256.state[2] = 0x3c6ef372;
			ctx->sha256.state[3] = 0xa54ff53a;
			ctx->sha256.state[4] = 0x510e527f;
			ctx->sha256.state[5] = 0x9b05688c;
			ctx->sha256.state[6] = 0x1f83d9ab;
			ctx->sha256.state[7] = 0x5be0cd19;
			ctx->sha256.count[0] = 0;
			ctx->sha256.ops = sha256_get_ops();
			break;
		case SHA384:
			ctx->sha512.state[0] = 0xcbbb9d5dc1059ed8ULL;
			ctx->sha512.state[1] = 0x629a292a367cd507ULL;
			ctx->sha512.state[2] = 0x9159015a3070dd17ULL;
			ctx->sha512.state[3] = 0x152fecd8f70e5939ULL;
			ctx->sha512.state[4] = 0x67332667ffc00b31ULL;
			ctx->sha512.state[5] = 0x8eb44a8768581511ULL;
			ctx->sha512.state[6] = 0xdb0c2e0d64f98fa7ULL;
			ctx->sha512.state[7] = 0x47b5481dbefa4fa4ULL;
			ctx->sha512.count[0] = 0;
			ctx->sha512.count[1] = 0;
			ctx->sha512.ops = sha512_get_ops();
			break;
		case SHA512:
			ctx->sha512.state[0] = 0x6a09e667f3bcc908ULL;
			ctx->sha512.state[1] = 0xbb67ae8584caa73bULL;
			ctx->sha512.state[2] = 0x3c6ef372fe94f82bULL;
			ctx->sha512.state[3] = 0xa54ff53a5f1d36f1ULL;
			ctx->sha512.state[4] = 0x510e527fade682d1ULL;
			ctx->sha512.state[5] = 0x9b05688c2b3e6c1fULL;
			ctx->sha512.state[6] = 0x1f83d9abfb41bd6bULL;
			ctx->sha512.state[7] = 0x5be0cd19137e2179ULL;
			ctx->sha512.count[0] = 0;
			ctx->sha512.count[1] = 0;
			ctx->sha512.ops = sha512_get_ops();
			break;
		case SHA512_256:
			ctx->sha512.state[0] = 0x22312194fc2bf72cULL;
			ctx->sha512.state[1] = 0x9f555fa3c84c64c2ULL;
			ctx->sha512.state[2] = 0x2393b86b6f53b151ULL;
			ctx->sha512.state[3] = 0x963877195940eabdULL;
			ctx->sha512.state[4] = 0x96283ee2a88effe3ULL;
			ctx->sha512.state[5] = 0xbe5e1e2553863992ULL;
			ctx->sha512.state[6] = 0x2b0199fc2c85b8aaULL;
			ctx->sha512.state[7] = 0x0eb72ddc81c52ca2ULL;
			ctx->sha512.count[0] = 0;
			ctx->sha512.count[1] = 0;
			ctx->sha512.ops = sha512_get_ops();
			break;
...
/* SHA2Final function */
void
SHA2Final(SHA2_CTX *ctx, void *digest)
{
	switch (ctx->algotype) {
		case SHA256:
		...
		case SHA512:
			...
			break;
		case SHA512_256:
			sha512_256_final(&ctx->sha512, digest);
			break;
	}
}
