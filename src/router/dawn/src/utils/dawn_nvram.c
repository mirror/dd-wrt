...
void uci_get_hostname(char* hostname)
{
    ...
    snprintf(hostname, HOST_NAME_MAX, "%s", nvram_safe_get("hostname"));

}


static void set_if_present_time_t(time_t *ret, const char* option) {
    const char *str = nvram_nget("dawn_%s", option);;
    if (*str)
        *ret = atoi(str);
}
...
struct local_config_s uci_get_local_config() {
    ...
    DAWN_SET_CONFIG_INT(ret, loglevel);
...
struct network_config_s uci_get_dawn_network() {
    ...
    uci_foreach_element(&uci_pkg->sections, e)
    {
        ...
        if (strcmp(s->type, "network") == 0) {
            ...
            DAWN_SET_CONFIG_INT(ret, broadcast_port);
