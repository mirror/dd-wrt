...
[ -n "$DO_UPDATE" ] && svn update
cd ../../../
cp .config_xscale_vilim .config
make -f Makefile.armbe kernel clean all install
mkdir -p ~/GruppenLW/releases/$DATE/gateworks-vilim
mkdir -p ~/GruppenLW/releases/$DATE/compex-wp188
cd ../../../
cp xscale/src/router/armeb-uclibc/gateworks-firmware-squashfs.bin ~/GruppenLW/releases/$DATE/gateworks-vilim
#cp xscale/src/router/armeb-uclibc/gateworx-firmware-jffs.bin ~/GruppenLW/releases/$DATE/gateworks
cp xscale/src/router/armeb-uclibc/gateworx-firmware.raw2 ~/GruppenLW/releases/$DATE/gateworks-vilim/linux.bin
exit
