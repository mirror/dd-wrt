...
cat .config_laguna-small | grep -v "^CONFIG_WIKAR\|^CONFIG_KROMO\|^CONFIG_XIRIAN\|^CONFIG_BRAINSLAYER\|^CONFIG_ROUTERSTYLE\|^CONFIG_IPTRAF\|^CONFIG_SNORT" > .config
...
mkdir -p ~/GruppenLW/releases/CUSTOMER/$DATE/wircom/gateworks_gw2388-16M
cd ../../../

cp laguna/src/router/arm-uclibc/laguna-firmware-squashfs.bin ~/GruppenLW/releases/CUSTOMER/$DATE/wircom/gateworks_gw2388-16M
cp laguna/src/router/arm-uclibc/laguna-firmware.raw2 ~/GruppenLW/releases/CUSTOMER/$DATE/wircom/gateworks_gw2388-16M/uImage.bin
