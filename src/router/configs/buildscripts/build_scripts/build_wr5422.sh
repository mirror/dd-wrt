...
[ -n "$DO_UPDATE" ] && svn update
cd ../../../
cp .config_wr5422 .config
#echo "CONFIG_KERNELLTO=y" >> .config
make -f Makefile.rt2880 kernel clean all install
mkdir -p ~/GruppenLW/releases/$DATE/Repotec_RP-WR5422
cd ../../../
#cp rt2880/src/router/mipsel-uclibc/aligned.uimage ~/GruppenLW/releases/$DATE/WHR-G300N/firmware.bin
cp rt2880/src/router/mipsel-uclibc/rt2880-webflash.bin ~/GruppenLW/releases/$DATE/Repotec_RP-WR5422/RP-WR5422-webflash.bin
cp rt2880/src/router/mipsel-uclibc/root.uimage ~/GruppenLW/releases/$DATE/Repotec_RP-WR5422/factory-to-ddwrt.dat
