...
cat .config_laguna-small | grep -v "^CONFIG_WIKAR\|^CONFIG_KROMO\|^CONFIG_XIRIAN\|^CONFIG_BRAINSLAYER\^CONFIG_ROUTERSTYLE\^CONFIG_BLUE\^CONFIG_YELLOW\^CONFIG_CYAN\^CONFIG_RED\^CONFIG_GREEN" > .config
...
mkdir -p ~/GruppenLW/releases/CUSTOMER/$DATE/hdwifi/gateworks_gw2388-16M
cd ../../../
cp laguna/src/router/arm-uclibc/laguna-firmware-squashfs.bin ~/GruppenLW/releases/CUSTOMER/$DATE/hdwifi/gateworks_gw2388-16M
cp laguna/src/router/arm-uclibc/laguna-firmware.raw2 ~/GruppenLW/releases/CUSTOMER/$DATE/hdwifi/gateworks_gw2388-16M/uImage.bin

cp notes/laguna/* ~/GruppenLW/releases/$DATE/hdwifi/gateworks_gw2388-16M
