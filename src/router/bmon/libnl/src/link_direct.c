	...
	if (!strcasecmp(argv[1], "list")) {
		if (argc > argin) {
			if (!strcasecmp(argv[argin], "brief")) {
				argin++;
				p.dp_type = NL_DUMP_BRIEF;
			} else if (!strcasecmp(argv[argin], "detailed")) {
				argin++;
				p.dp_type = NL_DUMP_FULL;
			} else if (!strcasecmp(argv[argin], "stats")) {
				argin++;
				p.dp_type = NL_DUMP_STATS;
			}

			get_filter(&l);
		}

		nl_cache_dump_filter(&lc, stdout, &p, (struct nl_common *) &l);

	} else if (!strcasecmp(argv[1], "set")) {
		char *odev;
		struct rtnl_link cr = RTNL_INIT_LINK();
		...
		rtnl_link_set_name(&l, odev);

		if (rtnl_link_change(&h, &cr, &l) < 0) {
			fprintf(stderr, "%s\n", nl_geterror());
			return 1;
		}
	} else
