...
static void copy(FILE *out, size_t inoff, size_t outoff, int len)
{
	...
	if (inoff >= outoff) {
		for (i = 0; i < (len / 65536); i++) {
			fprintf(stderr, "copy from %zu to %zu\r",
				inoff + (i * 65536), outoff + (i * 65536));
		...
		long o = len - 65536;
		for (i = 0; i < (len / 65536); i++) {
			fprintf(stderr, "copy from %lu to %lu\r", inoff + o,
				outoff + o);
...
int main_mbr(int argc, char *argv[])
{
	...
	for (i = 0; i < 4; i++) {
		fprintf(stderr, "p[%d]: start %d end %d active %X type %X\n", i,
			le32_to_cpu(presentlayout[i].start),
			le32_to_cpu(presentlayout[i].start) +
				le32_to_cpu(presentlayout[i].length) - 1,
			presentlayout[i].active, presentlayout[i].type);
	}
	fprintf(stderr, "new layout\n");
	for (i = 0; i < 4; i++) {
		fprintf(stderr, "p[%d]: start %d end %d active %X type %X\n", i,
			le32_to_cpu(newlayout[i].start),
			le32_to_cpu(newlayout[i].start) +
				le32_to_cpu(newlayout[i].length) - 1,
			newlayout[i].active, newlayout[i].type);
	}
	struct pte *nvram = &presentlayout[2];
	fseek(out, le32_to_cpu(nvram->start) * 512, SEEK_SET);
	uint32_t nvlen = nvram->length * 512;
	if (nvlen) {
		if (le32_to_cpu(nvram->start) ==
		    le32_to_cpu(newlayout[2].start)) {
			if (le32_to_cpu(nvram->length) >
			    le32_to_cpu(newlayout[2].length)) {
				// if old nvram partition size is bigger than the new partition to be written, we keep the old partition entry as is
				memcpy(&newlayout[2], &nvram,
				       sizeof(struct pte));
				fseek(out, MBR_PARTITION_ENTRY_OFFSET,
				      SEEK_SET);
				fseek(in, MBR_PARTITION_ENTRY_OFFSET, SEEK_SET);
				fwrite(&newlayout, sizeof(struct pte),
				       MBR_ENTRY_MAX, in);
				fwrite(&newlayout, sizeof(struct pte),
				       MBR_ENTRY_MAX, out);
			}
			...
			fprintf(stderr,
				"read nvram from old offset %d with len %d and write to offset %d\n",
				le32_to_cpu(nvram->start) * 512, nvlen,
				le32_to_cpu(newlayout[2].start) * 512);
			copy(out, le32_to_cpu(nvram->start) * 512,
			     le32_to_cpu(newlayout[2].start) * 512, nvlen);
		}
	}
	fseek(in, 0, SEEK_END);
...
int main_gpt(int argc, char *argv[])
{
	...
	buf = malloc(le32_to_cpu(header.n_parts) *
		     le32_to_cpu(header.part_entry_len));
	fread(buf,
	      le32_to_cpu(header.n_parts) * le32_to_cpu(header.part_entry_len),
	      1, in);

	buf2 = malloc(le32_to_cpu(header2.n_parts) *
		      le32_to_cpu(header2.part_entry_len));
	fread(buf2,
	      le32_to_cpu(header2.n_parts) *
		      le32_to_cpu(header2.part_entry_len),
	      1, out);
	...
	for (i = 0; i < 4; i++) {
		part = (gpt_partition
				*)&buf2[i * le32_to_cpu(header2.part_entry_len)];
		fprintf(stderr,
			"p[%d]: start %lld end %lld flags %llX name %s\n", i,
			le64_to_cpu(part->lba_start),
			le64_to_cpu(part->lba_end), le64_to_cpu(part->flags),
			part->name36);
	}
	fprintf(stderr, "new layout\n");
	for (i = 0; i < 4; i++) {
		part = (gpt_partition
				*)&buf[i * le32_to_cpu(header.part_entry_len)];
		fprintf(stderr,
			"p[%d]: start %lld end %lld flags %llX name %s\n", i,
			le64_to_cpu(part->lba_start),
			le64_to_cpu(part->lba_end), le64_to_cpu(part->flags),
	...
	gpt_partition *nvram =
		(gpt_partition *)&buf2[2 * le32_to_cpu(header2.part_entry_len)];
	gpt_partition *newnvram =
		(gpt_partition *)&buf[2 * le32_to_cpu(header.part_entry_len)];
	fseek(out, le64_to_cpu(nvram->lba_start) * 512, SEEK_SET);
	int64_t nvlen =
		(le64_to_cpu(nvram->lba_end) - le64_to_cpu(nvram->lba_start)) *
		512;
	if (nvlen > 0) {
		if (nvram->lba_start == newnvram->lba_start) {
			if (le64_to_cpu(nvram->lba_end) >
			    le64_to_cpu(newnvram->lba_end)) {
				// if old nvram partition size is bigger than the new partition to be written, we keep the old partition entry as is
				memcpy(newnvram, nvram,
				       le32_to_cpu(header.part_entry_len));
				fseek(out,
				      le32_to_cpu(header.part_entry_len) * 2,
				      SEEK_SET);
				fwrite(newnvram,
				       le32_to_cpu(header.part_entry_len), 1,
				       out);
			}
		} else {
			fprintf(stderr,
				"read nvram from old offset %lld with len %lld and write to offset %lld\n",
				le64_to_cpu(nvram->lba_start) * 512, nvlen,
				le64_to_cpu(newnvram->lba_start) * 512);
			copy(out, le64_to_cpu(nvram->lba_start) * 512,
			     le64_to_cpu(newnvram->lba_start) * 512, nvlen);
		}
	}
	fseek(in, 0, SEEK_END);
	uint32_t len = ftell(in);
