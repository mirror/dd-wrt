...
static void copy(FILE *out, size_t inoff, size_t outoff, int len)
{
	...
	if (inoff >= outoff) {
		for (i = 0; i < (len / 65536); i++) {
			...
			fseeko(out, inoff + (i * 65536), SEEK_SET);
			fread(mem, 65536, 1, out);
			fseeko(out, outoff + (i * 65536), SEEK_SET);
		...
		for (i = 0; i < (len / 65536); i++) {
			...
			fseeko(out, inoff + o, SEEK_SET);
			fread(mem, 65536, 1, out);
			fseeko(out, outoff + o, SEEK_SET);
			fwrite(mem, 65536, 1, out);
			fflush(out);
			fsync(fileno(out));
			o -= 65536;
		...
		if (len % 65536) {
			fseeko(out, inoff, SEEK_SET);
			fread(mem, len % 65536, 1, out);
			fseeko(out, outoff, SEEK_SET);
			fwrite(mem, len % 65536, 1, out);
			fflush(out);
			fsync(fileno(out));
		}
...
int main_mbr(int argc, char *argv[])
{
	...
	fseeko(out, MBR_PARTITION_ENTRY_OFFSET, SEEK_SET);
	fseeko(in, MBR_PARTITION_ENTRY_OFFSET, SEEK_SET);
	// read old mbr
	...
	fseeko(out, le32_to_cpu(nvram->start) * 512, SEEK_SET);
	...
	int count = len / 65536;
	fprintf(stderr, "write image len = %ld, count %d\n", len, count);
	for (i = 0; i < count; i++) {
...
int main_gpt(int argc, char *argv[])
{
	...
	fseeko(out, le64_to_cpu(nvram->lba_start) * 512, SEEK_SET);
	unsigned long long nvlen = (le64_to_cpu(nvram->lba_end) - le64_to_cpu(nvram->lba_start)) * 512;
	if (nvlen > 0) {
		if (nvram->lba_start == newnvram->lba_start) {
			if (le64_to_cpu(nvram->lba_end) > le64_to_cpu(newnvram->lba_end)) {
				...
				fseeko(out, le32_to_cpu(header.part_entry_len) * 2, SEEK_SET);
