...
#if 0
int write_boot(const char *path, const char *mtd)
{
	int mtd_fd = -1;
	mtd_info_t mtd_info;
 ...
 * write mac. The mac filename must be mac.bin. mac.bin format : new format:
 * byte 0 - 5 : mac old format: (no use) byte 0 : increment count byte 1 - 4 : 
 * ip address byte 5 : start position byte 6 - 11 : start mac byte 12 - 15: no 
 * used byte 16 - 21: end mac 
 */

int write_mac(const char *path)
{
...
int write_eou_key(const char *path)
{
	char string[1024];
	char buf[1024];
	FILE *fp;
 ...
 */
int mtd_erase_sector(const char *mtd, unsigned long start, unsigned long end)
{
	int mtd_fd;
	mtd_info_t mtd_info;
	erase_info_t erase_info;
