...
rewrite:;
	...
	size_t rootfslen = ftell(f_rootfs);
	rewind(f_rootfs);
	...
	// #ifndef HAVE_WRK54G
	if (kernellen + rootfslen < trx.len) {
...
again:;
	...
	for (i = 0; i < kernellen; i++)
		getc(f_kernel);
	dd_loginfoverbose("flash", "reread rootfs partition");
	for (i = 0; i < rootfslen; i++)
		getc(f_rootfs);
	fclose(f_rootfs);
	fclose(f_kernel);
