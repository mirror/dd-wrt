...
"reference board",
		    ...
		    BCM_WAN_PORT,
		    BCM_LAN_MIN_PORT,
		    BCM_LAN_MAX_PORT, 0, BCM_DEF_WAN_VLAN, BCM_DEF_LAN_VLAN},};
...
static int vlan_configure(void)
{
	...
	if (strlen(restore_wan_ifname)) {
		if (nvram_set
		    (strcat_r(prefix, "ifname", tmp), restore_wan_ifname))
			return -1;
	...
	if (strlen(restore_wan_hwaddr)) {
		if (nvram_set
		    (strcat_r(prefix, "wan_hwaddr", tmp), restore_wan_hwaddr))
			return -1;
	...
	for (i = vlan_boards[board_index].lan_port_start;
		...
		if (i == vlan_boards[board_index].lan_port_start) {
			...
			l2addr.pbmp =
			    PBMP_PORT(i - 1) | PBMP_PORT(BCM_MII_PORT - 1);
			if (BCM_RET_SUCCESS !=
			    (retval = bcm_l2_addr_add(0, &l2addr)))
				dprintf("failure writing lan l2 addr: %d\n",
					retval);
	...
	if (strlen(restore_wan_hwaddr)) {
		if (nvram_set
		    (strcat_r(prefix, "hwaddr", tmp), restore_wan_hwaddr))
			return -1;
		if (nvram_unset("restore_wan_hwaddr"))
			return -1;
	}
...
static int vlan_deconfigure(void)
{
	...
	if (vlan_configured) {
		...
		if (!wan_ifname) {
			if (bcm_is_robo()) {
				dprintf
				    ("No active, configured WAN interface found\n");
			}
			return -1;
		}
