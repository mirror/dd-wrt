		       		...
		       		struct switch_val *val)
{
	...
	if (priv->version == QCA_VER_AR8227)
		rv = fal_fdb_first(dev_id, &entry);
	else
		rv = fal_fdb_extend_first(dev_id, &option, &entry);
	while (!rv)
    {
...
//			snprintf(buf+len, 2048-len, "Buffer not enough!\n");
		...
		if (priv->version == QCA_VER_AR8227)
			rv = fal_fdb_iterate(dev_id, &i, &entry);
		else
			rv = fal_fdb_extend_next(dev_id, &option, &entry);
    }
