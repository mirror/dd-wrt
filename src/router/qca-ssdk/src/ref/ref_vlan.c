...
a_uint8_t qca_devname_2_devid( char *devname )
{
	if( devname == NULL )
		return 0;
	if(strcmp(devname, "switch1") == 0)
		return 1;
	else
		return 0;
...
qca_ar8327_sw_hw_apply(struct switch_dev *dev)
{
    ...
    if (!priv->init) {
        ...
        if (priv->vlan_id[0] == 0 && priv->vlan_table[0] == 0) {
            qca_ar8327_sw_enable_vlan0(dev_id, A_FALSE, 0);
        }
        ...
        for (j = 0; j < AR8327_MAX_VLANS; j++) {
            ...
            if (!vp) {
                fal_vlan_delete(dev_id, priv->vlan_id[j]);
                continue;
            }
            fal_vlan_delete(dev_id, priv->vlan_id[j]);
            fal_vlan_create(dev_id, priv->vlan_id[j]);

            for (i = 0; i < dev->ports; i++) {
                u8 mask = (1 << i);
                if (vp & mask) {
                    fal_vlan_member_add(dev_id, priv->vlan_id[j], i,
                           (mask & priv->vlan_tagged[j])? FAL_EG_TAGGED : FAL_EG_UNTAGGED);
                    portmask[i] |= vp & ~mask;
                }
            }
	    	if (SSDK_CURRENT_CHIP_TYPE == CHIP_SHIVA)
				fal_vlan_member_update(dev_id, priv->vlan_id[j],vp,0);
        }

        /*Hanlde VLAN 0 entry*/
        if (priv->vlan_id[0] == 0 && priv->vlan_table[0]) {
            qca_ar8327_sw_enable_vlan0(dev_id, A_TRUE, priv->vlan_table[0]);
        }
    ...
    for (i = 0; i < dev->ports; i++) {
        ...
        fal_port_1qmode_set(dev_id, i, ingressMode);
        fal_port_egvlanmode_set(dev_id, i, egressMode);
        fal_port_default_cvid_set(dev_id, i, pvid);
        fal_portvlan_member_update(dev_id, i, portmask[i]);
    }
