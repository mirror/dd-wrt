			     ...
			     u8 type, const u8 * data, size_t datalen )
{
	...
	wpa_printf( MSG_ERROR, "malloc() failed for "
		    "ieee802_1x_send(len=%lu)", ( unsigned long )len );
	return;
    }
				   ...
				   u8 * key_data, size_t key_len )
{
    ...
    ieee802_1x_send( hapd, sta, IEEE802_1X_TYPE_EAPOL_KEY, ( u8 * ) key,
		     len );
    if( sta->eapol_sm )
...
static struct hostapd_wep_keys *ieee802_1x_group_alloc( struct hostapd_data
							*hapd,
							const char *ifname )
{
...
static struct hostapd_wep_keys *ieee802_1x_get_group( struct hostapd_data
						      *hapd,
						      struct hostapd_ssid
						      *ssid, size_t vlan_id )
{
    ...
    wpa_printf( MSG_DEBUG, "IEEE 802.1X: Creating new group "
		"state machine for VLAN ID %lu", ( unsigned long )vlan_id );
...
void ieee802_1x_tx_key( struct hostapd_data *hapd, struct sta_info *sta )
{
	...
	if( hostapd_set_encryption( hapd->conf->iface, hapd, "WEP",
				    sta->addr, 0, ikey,
				    hapd->conf->individual_wep_key_len, 1 ) )
	{
...
void ieee802_1x_receive( struct hostapd_data *hapd, const u8 * sa,
			 const u8 * buf, size_t len )
{
    ...
    if( !hapd->conf->ieee802_1x || wpa_key_mgmt_wpa_psk(wpa_auth_sta_key_mgmt(sta->wpa_sm)))
	return;
...
void ieee802_1x_new_station( struct hostapd_data *hapd, struct sta_info *sta )
{
    ...
    if( ( !force_1x && !hapd->conf->ieee802_1x ) || wpa_key_mgmt_wpa_psk(wpa_auth_sta_key_mgmt(sta->wpa_sm)))
	return;
				  ...
				  const struct radius_class_data *src )
{
	...
	if( dst->attr[i].data == NULL )
	    break;
	dst->count++;
	os_memcpy( dst->attr[i].data, src->attr[i].data, src->attr[i].len );
...
static struct eapol_state_machine *ieee802_1x_search_radius_identifier( struct
									hostapd_data
									*hapd,
									u8
									identifier )
			 ...
			 void *data )
{
	...
	case RADIUS_CODE_ACCESS_ACCEPT:
	    ...
	    if( sta->vlan_id > 0 &&
		hostapd_get_vlan_id_ifname( hapd->conf->vlan, sta->vlan_id ) )
	    {
		hostapd_logger( hapd, sta->addr,
...
void ieee802_1x_dump_state( FILE * f, const char *prefix,
			    struct sta_info *sta )
{
    struct eapol_state_machine *sm = sta->eapol_sm;
...
static int ieee802_1x_rekey_broadcast( struct hostapd_data *hapd )
{
    ...
    wpa_hexdump_key( MSG_DEBUG, "IEEE 802.1X: New default WEP key",
		     hapd->default_wep_key, hapd->conf->default_wep_key_len );
