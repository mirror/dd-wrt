...
int main(int argc, char **argv)
{
	...
	if (((fd = open(fn_loader, O_RDWR)) < 0) || ((hdrlen = lseek(fd, 0, SEEK_END)) < 0) ||
	...
	if (((fd = open(fn_openixp, O_RDWR)) < 0) || ((openixplen = lseek(fd, 0, SEEK_END)) < 0) ||
	...
	if (((fd = open(fn_linux, O_RDWR)) < 0) || ((linuxlen = lseek(fd, 0, SEEK_END)) < 0) ||
	...
	if (((fd = open(fd_openixp, O_RDWR)) < 0) || (ioctl(fd, MEMGETINFO, &mtdInfo))) {
