...
#define CRC_INNER_LOOP(n, c, x) (c) = ((c) >> 8) ^ crc##n##_table[((c) ^ (x)) & 0xff]

uint32 crc32(uint8 *pdata, /* pointer to array of data to process */
...
int main(int argc, char **argv)
{
	...
	while (fis->name[0] != 0xff) {
		if (!strncmp(fis->name, "linux", 5) || !strncmp(fis->name, "vmlinux", 7) || !strcmp(fis->name, "kernel") ||
		    !strncmp(fis->name, "rootfs", 6)) {
			lfis = fis;
			if (fis->file_cksum != 0)
				flash = 1;
			fis->file_cksum = 0;
