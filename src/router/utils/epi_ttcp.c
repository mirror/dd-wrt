...
void prusage(struct rusage *, struct rusage *, struct timeval *, struct timeval *);
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "ttcp%s: buflen=%d, nbuf=%d, align=%d/%d, port=%d\n", trans ? "-t" : "-r", buflen, nbuf, bufalign,
		bufoffset, port);
	if (log_cnt)
		fprintf(fplog, " %6d %6d %6d %6d %4d", buflen, nbuf, bufalign, bufoffset, port);
	...
	if (sockbufsize) {
		...
		if (trans) {
			if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, (char *)&sockbufsize, sizeof sockbufsize) < 0)
				sockets_err("setsockopt: sndbuf");
		} else {
			if (setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (char *)&sockbufsize, sizeof sockbufsize) < 0)
...
#if defined(__lynx__) || defined(__sun__) || defined(__NTVIS__)
		getsockopt(fd, SOL_SOCKET, SO_SNDBUF, (char *)&sockbufsndsize, &sockbuflen);
		sockbuflen = sizeof sockbufrcvsize;
		getsockopt(fd, SOL_SOCKET, SO_RCVBUF, (char *)&sockbufrcvsize, &sockbuflen);
#else
...
#if defined(__lynx__) || defined(__sun__) || defined(__NTVIS__)
				if (p && setsockopt(fd, p->p_proto, TCP_NODELAY, (char *)&one, sizeof(one)) < 0)
#else
...

	{
		...
		if (trans) {
			...
			if (udp) {
				...
				err = setsockopt(fd, IPPROTO_UDP, UDP_NOCHECKSUM, (char *)&nochecksum_opt, sizeof(nochecksum_opt));
				if (err < 0)
					sockets_err("setsockopt");
			}
#endif
...
#if defined(__NTVIS__)
						rbuflen = (((unsigned long)rand() * ((unsigned long)(buflen - bufmin + 1))) >> 15) +
							  bufmin;
#else /* ! __NTVIS__ */
						rbuflen = ((((unsigned long)random() >> 15) *
							    ((unsigned long)(buflen - bufmin + 1))) >>
							   16) +
							  bufmin;
#endif /* __NTVIS__ */
					}
            ...
            fit the calculation in 32 bits */
#if defined(__NTVIS__)
					rbuflen = (((unsigned long)rand() * ((unsigned long)(buflen - bufmin + 1))) >> 15) + bufmin;
#else /* ! __NTVIS__ */
					rbuflen =
						((((unsigned long)random() >> 15) * ((unsigned long)(buflen - bufmin + 1))) >> 16) +
						bufmin;
#endif /* __NTVIS__ */
				}
			...
			if (!errno) {
				...
				if (udp && !no_end) {
					...
					net_byte[3] = data_count & 0xFF;
					data_count >>= 8;
					net_byte[2] = data_count & 0xFF;
					data_count >>= 8;
					net_byte[1] = data_count & 0xFF;
					data_count >>= 8;
			...
			if (udp) {
				udp_rcv_test();
			} else { /* not udp received transfer */
				while ((cnt = Nread(fd, buf, buflen)) > 0) {
					if (debug)
						fprintf(stdout, "ttcp%s: %5d | %d Bytes Read\n", trans ? "-t" : "-r", ++nb, cnt);
					nbytes += cnt;
					if (!sinkmode) {
#if !defined(__NTVIS__) /* else case specific to WINNT */
...
void udp_rcv_test(void)
{
	...
	while (rcv_state != END_TEST && (cnt = Nread(fd, buf, buflen)) > 0) {
		...
		if (!memcmp(buf, start_pattern, START_PATTERN_LEN)) {
			...
			expected_pkt_cnt = ((net_byte[0] << 24) + (net_byte[1] << 16) + (net_byte[2] << 8) + net_byte[3]);
	...
	if (expected_pkt_cnt != 0) {
		...
		fprintf(stderr, "ttcp-r: %g%% (%u/%u) packet loss\n", 100.0 * (double)lost_cnt / (double)expected_pkt_cnt, lost_cnt,
			expected_pkt_cnt);
	}
}
...
void result_summary(void)
{
	...
	if (fplog) {
		...
		fprintf(fplog, " %10.0f %4.06f %4.3f %6ld", nbytes, realt / 1e6, bytes_per_sec / (1024.0 * 1024.0), numCalls);
	}
}
/*--------------------------------------------------------------------------*/
void sockets_err(char *s)
{
#if defined(__NTVIS__)
...
register struct rusage *ru;
{
	...
	if (GetThreadTimes(phd, &CreateTime, &ExitTime, &KernelTime, &UserTime) != TRUE)
#else
 ...
 */
void prusage(struct rusage *r0, struct rusage *r1, struct timeval *t0, struct timeval *t1)
{
	struct timeval tdiff;
	int t, ms;
	register char *cp;
	...
	if (debug) {
		...
		printf("user (sec) : %9ld  %9ld\n", r1->ru_utime.tv_sec, r0->ru_utime.tv_sec);
   ...
   */
	...
	for (; *cp; cp++) {
		if (*cp != '%') {
			...
			case 'D':
				fprintf(stderr, "%ld",
					t == 0 ? 0 : (r1->ru_idrss + r1->ru_isrss - (r0->ru_idrss + r0->ru_isrss)) / t);
				break;
			...
			case 'C':
				fprintf(stderr, "%ld+%ld", r1->ru_nvcsw - r0->ru_nvcsw, r1->ru_nivcsw - r0->ru_nivcsw);
				break;
#endif /* !SYSV || __osf__ */
			default:
				putc(*cp, stderr);
...
void tvadd(tsum, t0, t1) struct timeval *tsum, *t0, *t1;
{
...
void tvsub(tdiff, t1, t0) struct timeval *tdiff, *t1, *t0;
{
...
int Nread(SOCKET s, void *bufp, int count)
{
...
int Nwrite(int s, void *bufp, int count)
{
	register int cnt;
	if (udp) {
again:
		cnt = sendto(s, bufp, count, 0, SOCKADDR_CAST & sinhim, sizeof(sinhim));
 ...
 */
int mread(int s, char *bufp, unsigned n)
{
...
void open_log()
{
...

	{
		if ((fplog = fopen(logfile, "a+")) == NULL) {
			...
			fprintf(fplog, "%02d-%02d-%02d %02d:%02d\n", tms->tm_mday, tms->tm_mon, tms->tm_year, tms->tm_hour,
				tms->tm_min);
#endif
...
void do_help()
{
	char More_help[] = " Details about the reply: \n"
			   "   Example: \n"
...
void do_Usage()
{
	char Usage[] = "  Usage: ttcp -t [-options] host [ < in ]    ttcp -r [-options > out]\n"
