...
OPENSSL_OPTIONS:= $(ENABLE_TLS) no-err threads no-ssl2 enable-ssl3-method no-ec2m no-heartbeats no-egd no-nextprotoneg no-brotli no-argon2 no-winstore no-docs enable-ecx no-unstable-qlog no-sm2-precomp no-zstd enable-quic
