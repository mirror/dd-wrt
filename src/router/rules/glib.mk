...
glib20-configure: libffi-configure libffi zlib-configure zlib util-linux-configure util-linux
	...
	echo "c_args = '$(CFLAGS) $(COPTS)  $(THUMB) -I$(TOP)/libffi/$(ARCH)-$(SUBARCH)-linux-gnu/include -I$(TOP)/zlib -DNEED_PRINTF'" >> $(TOP)/glib20/libglib/cross.txt
...
ifneq ($(CONFIG_NFS),y)
	...
	cd glib20/gettext && ./configure --enable-static --disable-shared --host=$(ARCH)-linux  LDFLAGS="$(COPTS) $(LTO) -std=gnu89 $(THUMB) -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc " CFLAGS="$(COPTS) $(THUMB)  -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc" CXXFLAGS="$(COPTS) $(THUMB) -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc"
	make -C glib20/gettext clean all
	rm -rf $(TOP)/glib20/libglib/build
	export CPPFLAGS="$(COPTS) $(THUMB)  -I$(TOP)/libffi/$(ARCH)-$(SUBARCH)-linux-gnu/include -I$(TOP)/zlib -DNEED_PRINTF" && \
	export CFLAGS="$(COPTS) $(THUMB)  -I$(TOP)/libffi/$(ARCH)-$(SUBARCH)-linux-gnu/include -I$(TOP)/zlib -DNEED_PRINTF" && \
	...
	export CPPFLAGS="$(COPTS) $(LTO) $(THUMB) -I$(TOP)/libffi/$(ARCH)-$(SUBARCH)-linux-gnu/include -I$(TOP)/zlib -DNEED_PRINTF -ffunction-sections -fdata-sections -Wl,--gc-sections" && \
	export CFLAGS="$(COPTS) $(LTO) $(THUMB) -I$(TOP)/libffi/$(ARCH)-$(SUBARCH)-linux-gnu/include -I$(TOP)/zlib -DNEED_PRINTF -ffunction-sections -fdata-sections -Wl,--gc-sections" && \
