curl-install:
	$(MAKE) -C $(CURLPATH)/build install DESTDIR=$(INSTALLDIR)/curl
	...
	cp $(CURLPATH)/build/lib/ca-bundle.crt $(INSTALLDIR)/curl/etc/ssl/certs/ca-certificates.crt

curl: openssl zlib
	$(MAKE) -C $(CURLPATH)/build

curl-clean:
	$(MAKE) -C $(CURLPATH)/build clean
...
curl-configure: zlib openssl
	...
	$(MAKE) -C $(CURLPATH)/build
	$(MAKE) -C $(CURLPATH)/build ca-bundle
