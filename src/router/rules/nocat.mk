nocat-configure:
	...
	cd glib && ./config.sh "$(CC)" "$(COPTS) $(MIPS16_OPT) $(LTO) -std=gnu89 -ffunction-sections -fdata-sections -Wl,--gc-sections -ldl" "$(LTOPLUGIN)" ac_cv_host=$(ARCH)-uclibc-linux --target=$(ARCH)-linux --host=$(ARCH)
	cd nocat && ./configure \
	    --with-remote-splash \
	    CC="$(CC)" \
