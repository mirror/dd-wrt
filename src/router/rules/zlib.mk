...
ZLIB_CMAKE_OPTIONS=-DZLIB_COMPAT=ON -DZLIB_ENABLE_TESTS=OFF -DZLIBNG_ENABLE_TESTS=OFF -DBUILD_SHARED_LIBS=ON
...
ZLIB_CMAKE_OPTIONS+=-DARMv8=ON -DWITH_NEON=ON -DWITH_ACLE=OFF
else
...
ZLIB_CMAKE_OPTIONS+=-DWITH_ARMV6=ON -DWITH_NEON=OFF -DWITH_ACLE=OFF
else
ifeq ($(ARCHITECTURE),thunderx)
ZLIB_CMAKE_OPTIONS+=-DARMv8=ON -DWITH_NEON=ON -DWITH_ACLE=ON
else
ifeq ($(ARCHITECTURE),ipq807x)
ZLIB_CMAKE_OPTIONS+=-DARMv8=ON -DWITH_NEON=ON -DWITH_ACLE=ON
else
