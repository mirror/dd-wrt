...
MRP_CMAKE_OPTIONS=-DCMAKE_BUILD_TYPE=release -DLibNL_INCLUDE_DIR="$(TOP)/kernel_headers/$(KERNELRELEASE)/include" -DLibNL_LIBRARY="$(TOP)/libnl-tiny/libnl-tiny.so" -DLibEV_LIBRARY="$(TOP)/mrp/ev/.libs/libev.a"

MRP_STAGING_DIR=$(TOP)/_staging/usr
MRP_EXTRA_CFLAGS=$(COPTS) $(MIPS16_OPT) -I$(TOP)/kernel_headers/$(KERNELRELEASE)/include -I$(TOP)/libnl-tiny/include -DNEED_PRINTF -D_GNU_SOURCE $(LTO)
