...
openssl:
	...
	$(MAKE) -C openssl build-shared CC="$(CC)  -fPIC" MAKEDEPPROG=$(ARCH)-linux-uclibc-gcc $(OPENSSL_MAKEFLAGS)
	$(MAKE)  -C openssl build_apps CC="$(CC)  -fPIC" MAKEDEPPROG=$(ARCH)-linux-uclibc-gcc $(OPENSSL_MAKEFLAGS)
...
OPENSSL_OPTIONS:= no-err no-hw threads no-sse2 no-perlasm -DHAVE_CRYPTODEV
else
OPENSSL_OPTIONS:= no-err no-hw threads no-ssl2 no-ssl3 no-ec2m
endif
