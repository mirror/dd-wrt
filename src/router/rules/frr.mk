frr-configure: ncurses json-c
	...
	rm -rf frr/build
	-mkdir -p frr/build
	cd frr/build && ../configure CC="" CFLAGS="" LDFLAGS="" --disable-doc --enable-clippy-only --disable-zeromq --enable-opaque-lsa --disable-nhrpd --enable-ospf-te --disable-ospfclient --enable-multipath=32  --enable-ipv6 --prefix=/usr --sysconfdir=/tmp --disable-ospf6d  --enable-vtysh --enable-user=root --enable-group=root --disable-ospfapi --disable-isisd --disable-pimd --disable-nhrpd --enable-pie=no
	make -C frr/build lib/clippy	
	-mkdir -p frr/build/hosttools/lib
	cd frr/build && cp -vR lib/* hosttools/lib
	cd frr/build && ../configure --host=$(ARCH)-uclibc-linux --localstatedir=/var/run  --libdir=/usr/tmp --disable-doc --disable-zeromq --enable-opaque-lsa --disable-nhrpd --enable-ospf-te --disable-ospfclient --enable-multipath=32  --enable-ipv6 --prefix=/usr --sysconfdir=/tmp --disable-ospf6d  --enable-vtysh --enable-user=root --enable-group=root --disable-ospfapi --disable-isisd --disable-pimd --disable-nhrpd --enable-pie=no --with-libreadline=$(TOP)/frr/readline CFLAGS="-fno-strict-aliasing $(COPTS) $(MIPS16_OPT) -I$(TOP)/frr -Drpl_malloc=malloc -Drpl_realloc=realloc -I$(TOP)/_staging/usr/include -I$(TOP)/frr/build" LDFLAGS="-L$(TOP)/frr/readline -L$(TOP)/ncurses/lib -lncurses -L$(TOP)/json-c/.libs -ljson-c" 
