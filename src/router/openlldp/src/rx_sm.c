...
int rxProcessFrame(struct lldp_port *lldp_port) {
    ...
    struct lldp_flat_tlv *current_tlv = NULL;
...
#warning Write a unit test to stress this
	...
	if(tlv_type == CHASSIS_ID_TLV) {
	  ...
	  msap_id = malloc(msap_tlv1->length - 1  + msap_tlv2->length - 1);
...
//************************************
		...
		if (tlv->info_string[3] == 3)
		  {
			...
			strncpy(elin, &tlv->info_string[5], tlv->length - 5);
			elin[tlv->length - 5] = '\0';
	...
	msap_cache = malloc(sizeof(struct lldp_msap));
	memset(msap_cache, 0x0, sizeof(struct lldp_msap));
...
uint8_t mibDeleteObjects(struct lldp_port *lldp_port) {
  struct lldp_msap *head    = lldp_port->msap_cache;
  struct lldp_msap *current = lldp_port->msap_cache;
  struct lldp_msap *prev    = NULL;
...
void rx_display_timers(struct lldp_port *lldp_port) {
  ...
  while(msap_cache != NULL) {
	...
	debug_printf(DEBUG_STATE, ") rxInfoTTL: %d\n", msap_cache->rxInfoTTL);
