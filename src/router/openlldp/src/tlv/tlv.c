  ...
  char *title  = NULL;
    ...
    switch(tlv->type)
    {
        case CHASSIS_ID_TLV:
            {
			  ...
			  debug_hex_strcat(result, chassis_id, (tlv->length - 1));
                        }break;
		...
		//	strncat(result, "Port ID: ", 2048);
			  ...
			  debug_hex_strcat(result, port_id, tlv->length -1);
                        }break;
			  ...
			  sprintf(tmp, "\tInterface Name - %s", tlv->info_string);
                    ...
                    default:
                        {
                          ...
                          sprintf(tmp, "\tReserved (%02X)", (uint8_t)tlv->info_string);
        ...
        case SYSTEM_NAME_TLV:
	      ...
	      snprintf(tmp, tlv->length + 2, "\t%s", tlv->info_string);
        ...
        case SYSTEM_DESCRIPTION_TLV:
            {
	      strncat(result, tlv->info_string, 2048);
...
char *decode_network_address(uint8_t *network_address) {
  ...
  debug_printf(DEBUG_NORMAL, "In %s\n", __FUNCTION__);
  ...
  switch(subtype)
	...
	if(addr_len == 5) {
	  ...
	  debug_hex_strcat(result, addr, addr_len - 1);
	}
      }break;
    default:
      debug_hex_strcat(result, addr, addr_len - 1);
  ...
  debug_printf(DEBUG_NORMAL, "Leaving %s\n", __FUNCTION__);
...
char *decode_management_address(struct lldp_tlv *tlv){
  ...
  uint8_t oid_length          = tlv->info_string[addr_len + 7];
...
struct lldp_tlv *initialize_tlv() {
    struct lldp_tlv *tlv = calloc(1, sizeof(struct lldp_tlv));

    if(tlv != NULL) {
		memset(tlv, 0x0, sizeof(struct lldp_tlv));
