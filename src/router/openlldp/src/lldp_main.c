...
/*	ServiceStatus.dwCurrentState = SERVICE_STOPPED; 
    ...
    strcpy(local.sun_path, "/tmp/lldpd.sock");
		...
		if(result > 0) {
		  if(FD_ISSET(neighbor_local_sd, &unixfds)) {
		    ...
		    if(neighbor_remote_sd < 0) {
		      debug_printf(DEBUG_NORMAL, "Couldn't accpet remote client socket!\n");
		    } else {		    
...
void cleanupLLDP(struct lldp_port *lldp_port) {
  struct lldp_msap *msap_cache = NULL;
	...
	if(lldp_port->msap_cache->tlv_list != NULL) {
	  destroy_tlv_list(&lldp_port->msap_cache->tlv_list);
	}
