...
function getOUIFromMAC(mac) {
	...
	var win = window.open("http://oidsearch.s.dd-wrt.com/search/" + tab[0] + ':' + tab[1] + ':' + tab[2], 'DDWRT_OUI_Search', 'top=' + top + ',left=' + left + ',width=' + width + ',height=' + height + ",resizable=yes,scrollbars=no,statusbar=no");
	addEvent(window, "unload", function() { if(!win.closed) win.close(); });
	win.focus();
}
