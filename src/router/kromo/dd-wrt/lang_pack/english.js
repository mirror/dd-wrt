...
share.state="Status";
share.first_seen="First seen";
share.blocked_until="Blocked until";
share.remember_till="Remember till";
...
share.not_prefered="Not preferred";
share.prefered="Preferred";
share.normal="Normal";
share.high="High";
...
errmsg.err4="The end time must be after the start time.";
...
hdiag.right2="You can run commands via the Web interface. Fill the input area with the desired command and click the <em>" + sbutton.runcmd + "</em> button to execute.<br/><b>Recommended</b>: a terminal connection via SSH/Telnet is more suitable, flexible, faster and reliable for some commands.";

//help page
hdiag.page1="<dd><b>Recommended</b>: A terminal connection via SSH/Telnet is more suitable, flexible, faster and reliable for some commands.</dd>";
...
hdiag.page4="<dd>Each time the firewall is started, custom firewall rules can be added to the chain. Fill the input area with additional iptables/ip6tables commands (only one command per row) and click the <em>" + sbutton.firewall + "</em> button.</dd>";
...
filter.mess3="The selected end time must be after the start time.";
filter.none="";
...
hfirewall.page15="<dd>Tarpit tries to switch an incoming connection into persist state to keep it alive as long as possible without allowing any data to be sent. The main objective here is to annoy brute force attackers and slow them down by stealing resources from them. Such a connection freeze can last between 12 and 24 minutes. Consider, if you enable that feature all ports may look open from outside even if they are not in fact. It's basically a trap for attackers. For real fun with annoying such attackers, keep port scan detection disabled.</dd>";
...
hfirewall.page17="<dd>Block UDP floods by rate limiting incoming UDP packets to the configured value per second. Do not enable it if you are running any UDP-based server in your network like WireGuard</dd>";
...
nas.rsync="Remote Synchronization";
...
// help page
hstatus_hots.page1="<dd>You can use the router as a Hotspot gateway (CoovaChilli solution) with authentication and accounting (RADIUS). CoovaChilli is an open-source captive portal or wireless LAN access point controller. It is used for authenticating users of a wireless LAN. It supports Web-based login while also supporting Wireless Protected Access (WPA). Authentication, authorization and accounting (AAA) is handled by your RADIUS server.</dd>";
...
hidx.page9="<dd>The PPP Compression provides a method to negotiate and utilize compression protocols over PPP encapsulated links. It is based on the Microsoft Point-to-Point Compression (MPPC) protocol. A protocol which is designed for transferring compressed datagrams over point-to-point links.</dd>";
...
hmanagement.page10="<dd>The TCP Congestion Control algorithm availability is router-dependent and provided by the kernel. When the router is the endpoint, and if enabled the Transmission BitTorrent client, Webserver (lighttpd) or VPN connections e.g. PPPoE etc..., can be affected by the selected congestion control algorithm.<br /><br /><div class=\"note\"><h4>Note:</h4><div>The selected congestion control algorithm will benefit mostly slow links or slower low-end routers to prevent network congestion occurring in case of traffic overloading and it will <b>not</b> increase link speed.</div></div><br />For peer-to-peer (P2P) applications running on the network/router, depending on your router hardware capabilities, either increase or decrease the maximum connections number and lower the TCP / UDP timeout. This may help maintain router stability since peer-to-peer applications open many connections and do not close them properly. Consider using these for old low-end routers:<ul><li>Maximum Connections: 4096/32768</li><li>TCP Timeout: 300&nbsp;seconds</li><li>UDP Timeout: 60&nbsp;seconds</li></ul></dd><dd>Check all values and click the <em>" + sbutton.save +"</em> button to save your settings. Click the <em>" + sbutton.cancel + "</em> button to cancel your unsaved changes. Click the <em>" + sbutton.reboot + "</em> button to reboot your router immediately.</dd>";
...
//help page
hnetworking.page1="<dd>Allows you to transfer different independent network streams by using just one interface. This is done by inserting a small TAG within the Ethernet header. By identifying this tag these Ethernet packets can be split up again on the other side to create new interfaces out of it. Using this option together with the bridging option allows you to create different transfer networks which can be bridged to a Wireless Interface to separate the Router Management network from the network accessible by the User. This is useful for large ISP networks.</dd>";
...
service.ssh_keyalert="Be patient, key generation can take up to 10 minutes or more.\nAfter successful generation the " + service.ssh_download + " button will become available to allow you to download the private key file.";
...
freeradius.organisation="Organization / Company";
...
freeradius.gencerdone="Certificate generation completed!";
...
hwl_adv.page12="<dd>Values are <i>Auto</i>, <i>Left</i>, <i>Right</i>, default value is <i>Auto</i>. This is used in conjunction with external antennas to give them optimum performance. On some router models left and right antennas may be reversed depending on your point of view.</dd>";
...
hwl_adv.page16="<dd>The default value is <i>Enabled</i>. The setting allows access to the router's setup Web interface from wireless clients. Disable this if you wish to block all wireless clients from accessing the setup pages.</dd>";
...
wl_basic.bexacn="BE / AX / AC / N Mixed";
...
//help page
hwl_basic.page1="<dd>The wireless part of your router can run in different modes:<ul class=\"wide\"><li>AP mode – This is the default mode, also called Infrastructure mode. Your router acts as a central connection point, which wireless clients can connect to.</li><li>AP (WDS) mode - Same as AP mode, but does allow special stations (See also station (WDS) mode) to connect which does allow bridging. Mixed operation mit stations are permitted.</li><li>AP (Mikrotik WDS) mode - Implements Mikrotik's proprietary variant of WDS AP which works exclusively on Mikrotik WDS stations. See also Station (Mikrotik WDS)</li><li>Station mode – The radio interface is used to connect the Internet-facing side of the router (i.e., the WAN) as a client to a remote access point. NAT or routing are performed between WAN and LAN, like in \"normal\" gateway or router mode. Use this mode, e.g. if your Internet connection is provided by a remote access point, and you want to connect a subnet of your own to it. </li><li>Station Bridged (Routed) mode – The radio interface is used to connect the LAN side of the router to a remote access point. The LAN and the remote AP will be in the same subnet (This is called a \"bridge\" between two network segments). The WAN side of the router is unused and can be disabled. Use this mode, e.g., to make the router act as a \"WLAN adapter\" for a device connected to one of the LAN Ethernet ports.</li><li>Station WDS - mode. This special station mode connects only to AP WDS operating access points and allows 802.3 compliant bridging</li><li>Station (Mikrotik WDS) - mode. Same as Station WDS but implements Mikrotik's proprietary WDS protocol. See also AP (Mikrotik WDS)</li><li>Mesh / 802.11s - mode. Is a wireless local area network standard and an IEEE 802.11 amendment for mesh networking</li><li>Ad-Hoc mode – This is for peer-to-peer wireless connections. Clients running in Ad-Hoc mode can connect to each other as required without involving central access points.</li></ul><br /><div class=\"note\"><h4>Note:</h4><div>Note that <a href=\"HWDS.asp\">WDS</a> is only available in AP mode.</div></div></dd>";
