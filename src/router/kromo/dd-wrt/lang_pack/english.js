...
hmanagement.page10="<dd>The TCP Congestion Control algorithm availability is router-dependent and provided by the kernel. When the router is the endpoint, and if enabled the Transmission Bitorrent client, Webserver (lighttpd) or VPN connections e.g. PPPoE etc..., can be affected by the selected congestion control algorithm.<br /><br /><div class=\"note\"><h4>Note:</h4><div>The selected congestion control algorithm will benefit mostly slow links or slower low end routers to prevent network congestion occurring in case of traffic overloading and it will <b>not</b> increase link speed.</div></div><br />For peer-to-peer (P2P) applications running on the network/router, dependent on your router hardware capabilities, either increase or decrease the maximum connections number and lower the TCP / UDP timeout. This may help maintain router stability since peer-to-peer applications open many connections and do not close them properly. Consider using these for old low end routers:<ul><li>Maximum Connections: 4096/32768</li><li>TCP Timeout: 300&nbsp;seconds</li><li>UDP Timeout: 60&nbsp;seconds</li></ul></dd><dd>Check all values and click the <em>" + sbutton.save +"</em> button to save your settings. Click the <em>" + sbutton.cancel + "</em> button to cancel your unsaved changes. Click the <em>" + sbutton.reboot + "</em> button to reboot your router immediately.</dd>";
