...
share.cant_parse_no_path=" Can't parse request. (no path given)";
share.cant_parse_no_proto=" Can't parse request. (no protocol given)";
...
share.no_wifi_access=" Can not use wireless interface to access GUI.";
...
errmsg.err51="The router is currently set to the default username and password. As a security precaution, you must enter a new username and password before the Remote Management feature can be enabled. Click the <em>OK</em> button to change your username/password. Click the <em>Cancel</em> button to leave the Remote Management feature disabled.";
...
halive.page2="<dd></dd><dd>Check all values and click <em>" + sbutton.save + "</em> button to save your settings. Click the <em>" + sbutton.cancel + "</em> button to cancel your unsaved changes. Click <em>" + sbutton.reboot +"</em> to reboot the router immediately.</dd>";
...
hconfig.right2="You may backup your current configuration in case you need to reset the router back to it's factory default settings.<br /><br />Click the <em>" + sbutton.backup + "</em> button to backup your current configuration to a file saved to disk.<br /> Tip: Over terminal type <b>nvram show > /tmp/mybackup.txt</b> and grab that file to desktop for a human readable backup of your current configuration.";
...
hconfig.page3="<dd>Click the <i>Browse...</i> button to search for a backup configuration file that you have previously saved to disk.<br>Click <em>" + sbutton.restore + "</em> to overwrite <b>all current configuration settings</b> with the values in the backup file.<br /><br /><div class=\"note\"><h4>Note:</h4><div>Only upload a backup file generated with DD-WRT's firmware and from the same model of router.</dd></div></dd>";
...
hdiag.right2="You can run command lines via the web interface. Fill the text area with your command and click the <em>" + sbutton.runcmd + "</em> button to submit.";

//help page
hdiag.page1="<dd>You can run commands directly via the web interface.</dd>";
hdiag.page2="<dd>Fill the text area with your command and click the <em>"+ sbutton.runcmd + "</em> button to run them.</dd>";
hdiag.page3="<dd>You can specify certain commands to be executed during router startup. Fill the text area with commands (only one command per row) and click the <em>" + sbutton.startup + "</em> button.</dd>";
hdiag.page4="<dd>Each time the firewall is started custom firewall rules can added to the chain. Fill the text area with additional iptables/ip6tables commands (only one command per row) and click the <em>" + sbutton.firewall + "</em> button.<br /><br />";
...
//help page
hdmz.page1="<dd>The DMZ (Demilitarized Zone) hosting feature allows one local user to be exposed to the Internet for use of a special-purpose service such as Internet gaming or videoconferencing. DMZ hosting forwards all the ports at the same time to one PC. The Port Forwarding feature is more secure because it only opens the ports you want to have opened, while DMZ hosting opens all the ports of one computer, exposing the computer so the Internet can see it.<br /><br /><div class=\"note\"><h4>Note:</h4><div>Any PC whose port is being forwarded must should have a new static IP address assigned to it because it's IP address may change when using the DHCP function.</div></div></dd>";
hdmz.page2="<dd>To expose one PC to the Internet, select <em>" + share.enable + "</em> and enter the computer's IP address in the <i>DMZ Host IP Address</i> field.<br /><br />To disable the DMZ, keep the default setting, <em>" + share.disable + "</em>.</dd><dd>Click <em>" + sbutton.save + "</em> button to save your settings or click the <em>" + sbutton.cancel + "</em> button to cancel your unsaved changes.</dd>";
...
hfactdef.page2="<dd>Click the <em>" + share.yes + "</em> button to reset all configuration settings to their default values. Then click the <em>" + sbutton.apply + "</em> button.<br /><br /><div class=\"note\"><h4>Note:</h4><div>Any settings you have saved will be lost when the default settings are restored. After resetting to defaults the will reboot and will become accessible via the default IP address 192.168.1.1 and entering a new username and password is required before proceeding with configuration.</div></div></dd>";
...
hfilter.right2="You may define up to 20 access policies. Click <em>" + sbutton.del + "</em> to delete a policy or <em>" + sbutton.summary + "</em> to see a summary of the policy.";
...
hfirewall.page13="<dd>To see a temporary log of the router's most recent outgoing traffic, click the <i>Outgoing Log</i> button.</dd><dd>Check all values and click the <em>" + sbutton.save + "</em> button to save your settings. Click <em>" + sbutton.cancel + "</em> to cancel your unsaved changes.</dd>";
...
hpforward.page2="</a>.<br /><br /><div class=\"note\"><h4>Note:</h4><div>Any PC whose port is being forwarded must have a static IP address assigned to it because it's IP address may change when using the DHCP function.</div></div></dd><dd>To add a new Port Forwarding rule, click the <em>" + sbutton.add + "</em> and fill in the fields below. To remove the last rule, click <i>Remove</i>.</dd>";
...
husb.page6="<dd><ul><li>Displays disk info e.g. partition size, volume name if set, as well as UUID ( e.g. B965FA66-CC65-4DK8-1254-DD0A78D19A90) for all connected drives</li></ul></dd><div class=\"note\"><h4>Note:</h4><div>Automount by default mounts all drives to /mnt/<devname> e.g. /mnt/sda1 <br />If your volume name is opt the partition will be mounted to /opt, if set to jffs -> /jffs.<br />You can override this by entering a partitions UUID in the option fields.</div></div><br />";
...
//help page
hprivoxy.page1="<dd>Privoxy enables you to filter common ads. <br /></dd>";
...
hnetworking.page4="<dd>Allows you to assign any valid interface to a network bridge. Consider setting the Wireless Interface options to Bridged if you want to assign any Wireless Interface here. Any system specific bridge setting can be overridden here in this field. </dd><dd>Click <em>" + sbutton.save + "</em> to save your settings. Click the <em>" + sbutton.cancel + "</em> button to cancel your unsaved changes.</dd>";
...
hqos.right10="You may specify priority for all traffic from a device on your network by giving the device a device name, specifying priority and entering it's MAC address.";
...
//help page
hqos.page1="<dd>Bandwidth management prioritizes the traffic on your router. Interactive traffic (telephony, browsing, telnet, etc.) gets priority and bulk traffic (file transfer, P2P) gets low priority. The main goal is to allow both types to live side-by side without unimportant traffic disturbing more critical things. All of this is more or less automatic.<br /><br />QoS allows control of the bandwidth allocation to different services, netmasks, MAC addresses and the four LAN ports, LAN port availability will vary by router, if it's not displayed, your hardware does not support it. QoS is divided into five bandwidth classes called Maximum, Premium, Express, Standard, and Bulk. Unclassified services will use the Standard bandwidth class.</dd>";
...
//help page
hpppoesrv.page1="<dd>This is a PPP over Ethernet redirector for PPPD.<br /><b>rp-pppoe</b> It's a user-space redirector which permits the use of PPPoE (Point-to-Point Protocol over Ethernet) with Linux.<br />PPPoE is used by many DSL service providers.<br /><div class=\"note\"><h4>Note:</h4><div>Notes to be added</div></div></dd>";
hpppoesrv.page2="<dd>Click <em>" + sbutton.save + "</em> button to save your settings or click <em>" + sbutton.cancel + "</em> to cancel your unsaved changes.</dd>";
...
//help page
hupgrad.page1="<dd>New firmware versions are posted at <a href=\"https:\/\/dd-wrt.com/support/other-downloads/?path=betas\" target=\"_new\">https:\/\/dd-wrt.com/downloads/betas</a> and can be downloaded for free.<br />If the router is not exibiting any issues, there is no need to download a more recent firmware version, <b>unless that version has a new feature, bug or security fixes</b>.<br /><br /><div class=\"note\"><h4>Note:</h4><div>Ensure you have a backup of your current settings before you upgrade the router's firmware, in case you lose it's configuration settings you can restore the backup.</div></div></dd><dd>To upgrade the router's firmware:<ol class=\"wide\"><li>Download the firmware upgrade file from the website.</li><li>Click the <i>Browse...</i> button and choose the firmware upgrade file form the directory you saved the upgrade file to.</li><li>Click the <em>" + sbutton.upgrade + "</em> button and wait until the upgrade is finished.</li></ol><br /><div class=\"note\"><h4>Note:</h4><div>DO NOT POWER OFF THE ROUTER OR PRESS THE RESET BUTTON WHILE THE FIRMWARE IS BEING UPGRADED.</div></div></dd><dt>Reset Options</dt><dd>If you want to reset the router to the default settings for the firmware version you are upgrading to, select from the dropdown menu the <em>" + upgrad.resetOn + "</em> option.<br /><br /></dd>";
...
//help page
hupnp.page1="<dd>Also know as UPnP is a set of computer network protocols developed by Microsoft technology used for the automatic configuration of devices. The goals of UPnP are to allow devices to connect seamlessly and to simplify the implementation of home or corporate environment networks. UPnP achieves this by defining and publishing UPnP device control protocols built upon open, Internet-based communication standards.</dd>";
...
hvpn.page2="<dd>Internet Protocol Security (IPSec) is a suite of protocols used to implement secure exchange of packets at the IP layer. To allow IPSec tunnels to pass through the router, IPSec Passthrough is enabled by default. To disable IPSec Passthrough, select <em>" + share.disable + "</em>.</dd>";
hvpn.page3="<dd>Point-to-Point Tunneling Protocol is the method used to enable VPN sessions to PPTP VPN servers. To allow PPTP tunnels to pass through the router, PPTP Passthrough is enabled by default. To disable PPTP Passthrough, select <em>" + share.disable + "</em>.</dd>";
hvpn.page4="<dd>Layer 2 Tunneling Protocol, an extension to the PPP protocol that enables ISPs to operate VPNs. L2TP merges the best features of two other tunneling protocols: PPTP from Microsoft and L2F from Cisco Systems. To allow L2TP tunnels to pass through the router, L2TP Passthrough is enabled by default. To disable L2TP Passthrough, select <em>" + share.disable + "</em>.</dd>";
...
hwanmac.page2="<dd>To clone your network adapter's MAC address onto the router, select <em>" + share.enable + "</em> and enter your adapter's MAC address in the <i>Clone WAN MAC</i> field or click <i>Get Current PC MAC Address</i> to fill in the MAC address of the computer you are using. Then save your changes by clicking on the <em>" + sbutton.save + "</em> button.<br /><br />To disable MAC address cloning, keep the default setting, <em>" + share.disable + "</em>.</dd><dd>Find your adapter's MAC address by following the instructions for your PC's operating system.<br /><br /><b>Microsoft Windows:</b><ol class=\"wide\"><li>Click the Start button, and select Run or the<em>Windows key</em>+<em>R</em> shortcut.</li><li>Type <tt>cmd</tt> in the field provided, and press the <em>OK</em> button.</li><li>At the command prompt, run <tt>ipconfig /all</tt>, and look at your adapter's physical address.</li><li>Write down your adapter's MAC address.</li></ol><br /><b>Linux:</b><ol class=\"wide\"><li>Click <em>CTRL</em>+<em>ALT</em>+<em>T</em>, to open a terminal.</li><li>Type <tt>ifconfig -a</tt>, and press Enter</li><li>Look for the Ethernet adapter you are using, the MAC address is in this format <b>xx:xx:xx:xx:xx:xx</b>.</li><li>Write down your adapter's MAC address.</li></ol><br /></dd><dd>Check all values and click the <em>" + sbutton.save + "</em> button to save your settings. Click the <em>" + sbutton.cancel + "</em> button to cancel your unsaved changes.</dd>";
...
hwpa.page6="<dd>RADIUS utilizes either a RADIUS server for authentication or WEP for data encryption. To utilize RADIUS, enter the IP address of the RADIUS server and it's shared secret. Select the desired encryption bit (64 or 128) for WEP and enter either a passphrase or a manual WEP key.</dd>";
