...
networking.vlan_forwarding="VLAN Filtering";
...
roaming.remote_node_timeout="Remote Node Timeout";
...
//help page
hwl_basic.page1="<dd>The wireless part of your router can run in different modes:<ul class=\"wide\"><li>AP mode – This is the default mode, also called Infrastructure mode. Your router acts as an central connection point, which wireless clients can connect to.</li><li>AP (WDS) mode - Same as AP mode, but does allow special stations (See also station (WDS) mode) to connect which does allow bridging. Mixed operation mit stations are permitted</li><li>AP (Mikrotik WDS) mode - Implements Mikrotiks proprietary variant of WDS AP which works exklusive only for mikrotik wds stations. See also Station (Mikrotik WDS)</li><li>Station mode – The radio interface is used to connect the Internet-facing side of the router (i.e., the WAN) as a client to a remote access point. NAT or routing are performed between WAN and LAN, like in \"normal\" gateway or router mode. Use this mode, e.g. if your Internet connection is provided by a remote access point, and you want to connect a subnet of your own to it. </li><li>Station Bridged (Routed) mode – The radio interface is used to connect the LAN side of the router to a remote access point. The LAN and the remote AP will be in the same subnet (This is called a \"bridge\" between two network segments). The WAN side of the router is unused and can be disabled. Use this mode, e.g., to make the router act as a \"WLAN adapter\" for a device connected to one of the LAN Ethernet ports.</li><li>Station WDS - mode. This special station mode connects only to AP WDS operating accesspoints and allows 802.3 compliant bridging</li><li>Station (Mikrotik WDS) - mode. Same as Station WDS but implements Mikrotiks proprietary WDS protocol. See also AP (Mikrotik WDS)</li><li>Mesh / 802.11s - mode. Is a wireless local area network standard and an IEEE 802.11 amendment for mesh networking</li><li>Ad-Hoc mode – This is for peer to peer wireless connections. Clients running in Ad-Hoc mode can connect to each other as required without involving central access points.</li></ul><br /><div class=\"note\"><h4>Note:</h4><div>Note that <a href=\"HWDS.asp\">WDS</a> is only available in AP mode.</div></div></dd>";
