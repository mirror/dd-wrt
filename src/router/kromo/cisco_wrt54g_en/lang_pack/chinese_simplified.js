...
halive.right2 =
  "选择何时重新启动路由器。在普通管理标签中的Cron选项必须被开启。";
halive.right4 =
  "最多允许3个IP地址，之间使用<em>空格</em> 分割。<BR/>IP地址格式为：xxx.xxx.xxx.xxx。";
...
config.legend = "备份设置";
config.mess1 =
  "点击 \"" + sbutton.backup + "\" 按钮将配置备份文件下载到您的个人电脑。";
config.h22 = "恢复配置";
config.legend2 = "恢复设置";
config.mess2 = "请选择一个用来恢复的文件";
config.mess3 = " [ 警 ] [ 告 ] ";
config.mess4 =
  "请仅上传使用此（版本）固件并且从相同型号路由器上备份的文件。<br />请勿上传任何不是使用本界面创建的文件！";

//help container

hconfig.right2 =
  "您可以备份您当前配置以备您将来需要将路由器复位到出厂设置后使用。<br /><br />点击 <em>备份</em> 按钮备份您的当前设置。";
hconfig.right4 =
  "点击 <em>浏览...</em> 按钮浏览到一个当前存储在您个人电脑上的配置文件。<br /><br />点击 <em>"
  + sbutton.restore + "</em> 按钮使用配置文件覆盖当前配置。";



// ******************************************* DDNS.asp *******************************************//

ddns.titl = " - 动态DNS" ddns.h2 = "动态域名系统(DDNS)";
ddns.legend = "DDNS";
ddns.srv = "DDNS服务";
...
hdiag.right2 =
  "您可以通过Web界面运行命令行。将您的命令填入文本区域并且点击 <em>" +
  sbutton.runcmd + "</em> 按钮进行提交。";
...
hdmz.right2 =
  "启用这个选项将使特定的主机被暴露给Internet。所有的端口都可以被从Internet访问。";
...
factdef.mess1 =
  "警告！如果您点击确定，此设备将会被复位到出厂默认设置，所有以往的设置都会被清除。";

//help container

hfactdef.right1 =
  "此操作将会将所有设置复位回出厂默认设置。您所做的所有设置都将被清除。";
...
hfilter.right2 =
  "您可以最多定义10条访问策略。点击 <em>" + sbutton.del +
  "</em> 按钮删除一条策略，或者点击 <em>" + sbutton.summary +
  "</em> 按钮察看策略综述。";
...
hfilter.right12 =
  "您可以选择封禁特定的服务。点击 <em>" + sbutton.filterSer +
  "</em> 按钮更改这些设置。";
hfilter.right14 = "您可以通过输入特定Web站点的URL来封禁对其访问。";
hfilter.right16 =
  "您可以通过输入包含在特定Web站点页面中的关键字来封禁对其访问。";;
...
idx.legend3 = "时间设置";
idx.timeset = "时区 / 夏令时 (DST)";
idx.localtime = "使用当地时间";
idx.static_ip = "静态IP";
idx.dhcp = "自动配置 - DHCP";
...
hidx.right18 =
  "请选择您所在的时区和夏令时（DST）期间。本路由器可以使用本地时间或者UTC时间。";
...
management.lang_chinese_simplified =
  "simpchinese（简体中文，Simplified Chinese）";
...
hqos.right10 = "您可以通过为一个设备指定设备名，制定优先级并输入MAC地址， \
			从而为该设备的所有流量指定优先级。";
hqos.right12 =
  "您可以根据您的设备连接到哪一个无力的LAN口来控制它的数据传输速率。 \
...
hroute.right2 =
  "如果本路由器正主控您的Internet连接，选择 <em>网关</em> 模式。如果您的网络中存在另外一个路由器，选择 <em>路由</em> 模式。";
...
hstatus_lan.right8 =
  "如果您将您的路由器当作DHCP服务器使用，那将被显示在这里。";
hstatus_lan.right10 =
  "点击任意MAC地址，您将能获取该网络接口的“唯一组织识别码”（IEEE标准OUI数据库搜索）。";
...
hstatus_router.right12 =
  "这里给出的三个数字表示系统在过去1、5、15分钟时间内的系统负载。";
...
hstatus_wireless.right4 =
  "正如您在在无线标签里所设置的，这里将显示网络所使用的无线模式（混合，仅G，仅B或者禁用）。";
...
htrforward.right4 =
  "为每一个应用程序列出被激发的端口范围。请参考Internet应用程序文档来查找所需要的端口号。";
htrforward.right6 =
  "为每一个应用程序列出被转发（映射）的端口范围。请参考Internet应用程序文档来查找所需要的端口号。";
...
upgrad.warning = " [警] [告] ";
upgrad.mess1 =
  "升级固件可能需要花费数分钟。<br />请不要关闭电源或者按复位按钮！";

//help container

hupgrad.right2 =
  "点击 <em>浏览...</em> 按钮选择将要上载到路由器的固件文件。<br /><br /> \
...
hvpn.right1 =
  "您可以选择启用IPSec，PPTP 和/或 L2TP 穿透，从而允许您的网络设备通过VPN进行通讯。";
...
hwol.right2 =
  "本页面允许您 <em>唤醒</em> 在您本地网络上的主机（例如，本地的连接到您的路由器上的）。";
hwol.right4 =
  "MAC地址使用如下格式输入 xx:xx:xx:xx:xx:xx （例如 01:23:45:67:89:AB）";
hwol.right6 =
  "IP地址通常是本地网络的广播地址，但是如果目标主机并为连接到路由器的本地网络，则应该设置为远程地址。"
// ******************************************* WanMAC.asp *******************************************//
  wanmac.titl = " - MAC地址克隆";
wanmac.h2 = "MAC地址克隆";
wanmac.legend = "MAC克隆";
...
hwpa.right1 = "安全模式：";
hwpa.right2 =
  "您可以从禁用、WEP、WPA预共享秘钥、WPA RADIUS或者RADIUS中选取一种。在您的网络上的所有设备必须使用相同的安全模式。";
...
wl_filter.h3 =
  "使用如下格式输入MAC地址&nbsp;：&nbsp;&nbsp;&nbsp;xx:xx:xx:xx:xx:xx";
...
hwl_basic.right2 =
  "如果您想剔除无线-G客户端，选择 <em>仅B</em> 模式。如果您想禁用无线访问，选择 <em>禁用</em>。";
...
hwl_adv.right2 =
  "您可以从自动或者共享秘钥中选择一个。共享秘钥授权更加安全，但是所有在您网络中的设备必须支持共享秘钥授权。";
...
success.success_reboot =
  success.success_noreboot + "<br />设备正在重新启动。请稍候……";

success.alert_reset = "所有配置已经被恢复到它们的默认值。<br /><br />";
success.alert1 = "请在再次连接前检查以下内容：";
success.alert2 =
  "如果您更改了您的路由器的IP地址，请注意您必须release并且renew您网络上客户端的地址。";
