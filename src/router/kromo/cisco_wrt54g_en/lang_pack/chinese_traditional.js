...
halive.right2 =
  "選擇何時重新啟動路由器。在普通管理標籤中的Cron選項必須被開啟。";
halive.right4 =
  "最多允許3個IP位址，之間使用<em>空格</em> 分割。<BR/>IP地址格式為：xxx.xxx.xxx.xxx。";
...
config.legend = "備份設置";
config.mess1 =
  "點擊 \"" + sbutton.backup + "\" 按鈕將配置備份檔案下載到您的個人電腦。";
config.h22 = "恢復配置";
config.legend2 = "恢復設置";
config.mess2 = "請選擇一個用來恢復的檔";
config.mess3 = " [ 警 ] [ 告 ] ";
config.mess4 =
  "請僅上傳使用此（版本）固件並且從相同型號路由器上備份的檔。<br />請勿上傳任何不是使用本介面創建的檔！";

//help container

hconfig.right2 =
  "您可以備份您當前配置以備您將來需要將路由器重定到出廠設置後使用。<br /><br />點擊 <em>備份</em> 按鈕備份您的當前設置。";
hconfig.right4 =
  "點擊 <em>流覽...</em> 按鈕流覽到一個當前存儲在您個人電腦上的配置檔。<br /><br />點擊 <em>"
  + sbutton.restore + "</em> 按鈕使用配置檔覆蓋當前配置。";



// ******************************************* DDNS.asp *******************************************//

ddns.titl = " - 動態DNS" ddns.h2 = "動態功能變數名稱系統(DDNS)";
ddns.legend = "DDNS";
ddns.srv = "DDNS服務";
...
hdiag.right2 =
  "您可以通過Web介面運行命令行。將您的命令填入文本區域並且點擊 <em>" +
  sbutton.runcmd + "</em> 按鈕進行提交。";
...
hdmz.right2 =
  "啟用這個選項將使特定的主機被暴露給Internet。所有的埠都可以被從Internet訪問。";
...
factdef.mess1 =
  "警告！如果您點擊確定，此設備將會被重定到出廠默認設置，所有以往的設置都會被清除。";

//help container

hfactdef.right1 =
  "此操作將會將所有設置復位回出廠默認設置。您所做的所有設置都將被清除。";
...
hfilter.right2 =
  "您可以最多定義10條訪問策略。點擊 <em>" + sbutton.del +
  "</em> 按鈕刪除一條策略，或者點擊 <em>" + sbutton.summary +
  "</em> 按鈕察看策略綜述。";
...
hfilter.right12 =
  "您可以選擇封禁特定的服務。點擊 <em>" + sbutton.filterSer +
  "</em> 按鈕更改這些設置。";
hfilter.right14 = "您可以通過輸入特定Web站點的URL來封禁對其訪問。";
hfilter.right16 =
  "您可以通過輸入包含在特定Web站點頁面中的關鍵字來封禁對其訪問。";;
...
hidx.right18 =
  "請選擇您所在的時區和夏令時（DST）期間。本路由器可以使用本地時間或者UTC時間。";
...
management.lang_chinese_simplified =
  "simpchinese（簡體中文，Simplified Chinese）";
...
hqos.right10 = "您可以通過為一個設備指定設備名，制定優先順序並輸入MAC位址， \
			從而為該設備的所有流量指定優先順序。";
hqos.right12 =
  "您可以根據您的設備連接到哪一個無力的LAN口來控制它的資料傳輸速率。 \
...
hroute.right2 =
  "如果本路由器正主控您的Internet連接，選擇 <em>閘道</em> 模式。如果您的網路中存在另外一個路由器，選擇 <em>路由</em> 模式。";
...
hstatus_lan.right8 =
  "如果您將您的路由器當作DHCP伺服器使用，那將被顯示在這裏。";
hstatus_lan.right10 =
  "點擊任意MAC位址，您將能獲取該網路介面的“唯一組織識別碼”（IEEE標準OUI資料庫搜索）。";
...
hstatus_router.right12 =
  "這裏給出的三個數位表示系統在過去1、5、15分鐘時間內的系統負載。";
...
hstatus_wireless.right4 =
  "正如您在在無線標籤裏所設置的，這裏將顯示網路所使用的無線模式（混合，僅G，僅B或者禁用）。";
...
htrforward.right4 =
  "為每一個應用程式列出被激發的埠範圍。請參考Internet應用程式文檔來查找所需要的埠號。";
htrforward.right6 =
  "為每一個應用程式列出被轉發（映射）的埠範圍。請參考Internet應用程式文檔來查找所需要的埠號。";
...
upgrad.warning = " [警] [告] ";
upgrad.mess1 =
  "升級固件可能需要花費數分鐘。<br />請不要關閉電源或者按重定按鈕！";

//help container

hupgrad.right2 =
  "點擊 <em>流覽...</em> 按鈕選擇將要上載到路由器的固件檔。<br /><br /> \
...
hvpn.right1 =
  "您可以選擇啟用IPSec，PPTP 和/或 L2TP 穿透，從而允許您的網路設備通過VPN進行通訊。";
...
hwol.right2 =
  "本頁面允許您 <em>喚醒</em> 在您本地網路上的主機（例如，本地的連接到您的路由器上的）。";
hwol.right4 =
  "MAC位址使用如下格式輸入 xx:xx:xx:xx:xx:xx （例如 01:23:45:67:89:AB）";
hwol.right6 =
  "IP位址通常是本地網路的廣播位址，但是如果目標主機並為連接到路由器的本地網路，則應該設置為遠端位址。"
// ******************************************* WanMAC.asp *******************************************//
  wanmac.titl = " - MAC地址克隆";
wanmac.h2 = "MAC地址克隆";
wanmac.legend = "MAC克隆";
...
hwpa.right1 = "安全模式：";
hwpa.right2 =
  "您可以從禁用、WEP、WPA預共用秘鑰、WPA RADIUS或者RADIUS中選取一種。在您的網路上的所有設備必須使用相同的安全模式。";
...
wl_filter.h3 =
  "使用如下格式輸入MAC位址&nbsp;：&nbsp;&nbsp;&nbsp;xx:xx:xx:xx:xx:xx";
...
hwl_basic.right2 =
  "如果您想剔除無線-G用戶端，選擇 <em>僅B</em> 模式。如果您想禁用無線訪問，選擇 <em>禁用</em>。";
...
hwl_adv.right2 =
  "您可以從自動或者共用秘鑰中選擇一個。共用秘鑰授權更加安全，但是所有在您網路中的設備必須支援共用秘鑰授權。";
...
success.success_reboot =
  success.success_noreboot + "<br />設備正在重新啟動。請稍候……";

success.alert_reset = "所有配置已經被恢復到它們的預設值。<br /><br />";
success.alert1 = "請在再次連接前檢查以下內容：";
success.alert2 =
  "如果您更改了您的路由器的IP位址，請注意您必須release並且renew您網路上用戶端的位址。";
