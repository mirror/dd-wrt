...
share.right="右";
share.left="左";
share.share_key="Shared Key（共用金鑰）";
...
errmsg.err22="WAN端MAC位址不正確。";
errmsg.err23="無效16進制數值 ";
errmsg.err24=" 在如下MAC地址中出現 ";
errmsg.err25="金鑰值不正確。";
errmsg.err26="金鑰長度不正確。";
...
errmsg.err38="請輸入一個Shared Key（共用金鑰）！";
errmsg.err39="無效金鑰，必須是長度為8到63個ASCII字元或者64個十六進位數位";
...
config.mess3=" [ 警 ] [ 告 ] ";
config.mess4="請僅上傳使用此（版本）韌體並且從相同型號路由器上備份的檔。<br />請勿上傳任何不是使用本介面創建的檔！";

//help container

hconfig.right2="您可以備份您目前配置以備您將來需要將路由器重定到出廠設置後使用。<br /><br />點擊 <em>備份</em> 按鈕備份您的目前設置。";
hconfig.right4="點擊 <em>流覽...</em> 按鈕流覽到一個目前存儲在您個人電腦上的配置檔。<br /><br />點擊 <em>" + sbutton.restore + "</em> 按鈕使用配置檔覆蓋目前配置。";
...
factdef.mess1="警告！如果您點擊確定，此設備將會被重定到出廠預設值，所有以往的設置都會被清除。";

//help container

hfactdef.right1="此操作將會將所有設置復位回出廠預設值。您所做的所有設置都將被清除。";
...
hfilter.right12="您可以選擇禁止特定的服務。點擊 <em>" + sbutton.filterSer + "</em> 按鈕更改這些設置。";
...
firewall.ping="禁止來自Internet的匿名請求";
...
management.lang_portuguese_braz="brazilian（巴西語）";
...
upgrad.warning=" [警] [告] ";
upgrad.mess1="升級韌體可能需要花費數分鐘。<br />請不要關閉電源或者按重定按鈕！";
...
wpa.wpa="WPA";
wpa.radius="Radius";
wpa.gtk_rekey="WPA組重新獲取金鑰間隔";
wpa.rekey="金鑰更新間歇（以秒為單位）";
...
wpa.rekeyInt="重新獲取金鑰間隔";

//help container

hwpa.right1="安全模式：";
hwpa.right2="您可以從禁用、WEP、WPA預共用金鑰、WPA RADIUS或者RADIUS中選取一種。在您的網路上的所有設備必須使用相同的安全模式。";
