...
{   aes_32t        locals(b0, b1);
    ...
    case 10:
        ...
        round(fwd_lrnd, b0, b1, kp + 9 * N_COLS);
    default:
        ;
    }
#else
...
{   aes_32t        locals(b0, b1);
    const aes_32t  *kp = cx->k_sch + N_COLS * cx->n_rnd;
#ifdef dec_imvars
    ...
    kp = cx->k_sch + 9 * N_COLS;

    /*lint -e{616} control flows into case/default */
    switch(cx->n_rnd)
    {
    case 14:
        round(inv_rnd,  b1, b0, kp + 4 * N_COLS);
        round(inv_rnd,  b0, b1, kp + 3 * N_COLS);
    case 12:
        round(inv_rnd,  b1, b0, kp + 2 * N_COLS);
        round(inv_rnd,  b0, b1, kp + N_COLS    );
    case 10:
        ...
        round(inv_rnd,  b0, b1, kp - 3 * N_COLS);
        round(inv_rnd,  b1, b0, kp - 4 * N_COLS);
        round(inv_rnd,  b0, b1, kp - 5 * N_COLS);
        round(inv_rnd,  b1, b0, kp - 6 * N_COLS);
        round(inv_rnd,  b0, b1, kp - 7 * N_COLS);
        round(inv_rnd,  b1, b0, kp - 8 * N_COLS);
        round(inv_lrnd, b0, b1, kp - 9 * N_COLS);
    default:
        ;
    }
#else
