...
struct netr_CIPHER_VALUE {
	uint32_t len;
	uint32_t maxlen;
	uint8_t *cipher_data;/* [size_is(maxlen),unique,length_is(len)] */
};
...
union netr_CONTROL_DATA_INFORMATION {
	const char *domain;/* [charset(UTF16),case(NETLOGON_CONTROL_REDISCOVER),unique] */
	const char *user;/* [charset(UTF16),case(NETLOGON_CONTROL_FIND_USER),unique] */
