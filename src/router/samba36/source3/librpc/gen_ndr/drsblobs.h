...
struct package_PrimaryKerberosString {
	...
	const char * string;/* [flag(LIBNDR_FLAG_STR_NOTERM|LIBNDR_FLAG_REMAINING),relative,subcontext_size(size),subcontext(0)] */
};
...
struct package_PrimaryKerberosKey3 {
	...
	DATA_BLOB *value;/* [subcontext_size(value_len),subcontext(0),flag(LIBNDR_FLAG_REMAINING),relative] */
};
...
struct package_PrimaryKerberosKey4 {
	...
	uint32_t keytype;
	uint32_t value_len;/* [value((value?value->length:0))] */
	DATA_BLOB *value;/* [flag(LIBNDR_FLAG_REMAINING),relative,subcontext_size(value_len),subcontext(0)] */
};
