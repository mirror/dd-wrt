...
static struct ndpi_id_struct *ndpi_id_search_or_insert(struct ndpi_net *n, union nf_inet_addr *ip)
{
	int res;
...
#endif
#else
	return __nf_ct_ext_add_length(ct, nf_ct_ext_id_ndpi, sizeof(struct nf_ct_ext_ndpi) - sizeof(struct nf_conn_labels), GFP_ATOMIC);
#endif
}
...
static int __ndpi_free_flow(struct nf_conn *ct, void *data)
{
...
static void nf_ndpi_free_flow(struct nf_conn *ct)
{
...
static struct ndpi_flow_struct *ndpi_alloc_flow(struct nf_ct_ext_ndpi *ct_ndpi)
{
	struct ndpi_flow_struct *flow;
...
#ifndef NF_CT_CUSTOM
static void (*ndpi_nf_ct_destroy) (struct nf_conntrack *)
__rcu __read_mostly;
...
static void ndpi_enable_protocols(struct ndpi_net *n, const struct xt_ndpi_mtinfo *info)
{
	int i;
...
static void ndpi_disable_protocols(struct ndpi_net *n, const struct xt_ndpi_mtinfo *info)
{
	int i;
...
static u32 ndpi_process_packet(struct ndpi_net *n, struct nf_conn *ct, struct nf_ct_ext_ndpi *ct_ndpi, const uint64_t time, const struct sk_buff *skb, int dir)
{
...
#define MAX_EXTRA_PACKETS_TO_CHECK  7
	...
	if (flow->detection_completed && (flow->detected_protocol != NDPI_PROTOCOL_UNKNOWN)) {
		...
		if (flow != NULL && flow->check_extra_packets) {
			if (flow->num_extra_packets_checked == 0 && flow->max_extra_packets_to_check == 0) {
				/* Protocols can set this, but we set it here in case they didn't */
				flow->max_extra_packets_to_check = MAX_EXTRA_PACKETS_TO_CHECK;
			}
			if (flow->num_extra_packets_checked < flow->max_extra_packets_to_check) {
				ndpi_process_extra_packet(n->ndpi_struct, flow, (uint8_t *) iph, skb->len, time, src, dst);
			}
		}
	...
	src = ct_ndpi->src;
	if (!src) {
		src = ndpi_id_search_or_insert(n, &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3);
		if (!src)
			return proto.protocol;
		ct_ndpi->src = src;
	}
	dst = ct_ndpi->dst;
	if (!dst) {
		dst = ndpi_id_search_or_insert(n, &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3);
		if (!dst)
			return proto.protocol;
		ct_ndpi->dst = dst;
	}
...
//#endif
					      (uint8_t *) iph, skb->len, time, src, dst, dir);
	...
	proto = ndpi_guess_undetected_protocol(n->ndpi_struct, protocol, low_ip, low_port, up_ip, up_port);
	flow->detected_protocol = proto.protocol;
...
static inline int can_handle(const struct sk_buff *skb, u8 *l4_proto)
{
	...
	*l4_proto = proto = iph->protocol;
	...
	if (proto == IPPROTO_ESP || proto == IPPROTO_GRE || proto == IPPROTO_ICMP || proto == IPPROTO_IGMP || proto == IPPROTO_OSPF || proto == IPPROTO_IPIP) {
		ndpi_p7++;
		return 1;
	}
#endif
...
#endif
	if (ct_ndpi->proto == NDPI_PROTOCOL_UNKNOWN || (ct_ndpi->flow)) {
		struct ndpi_net *n;
		...
		time = ((uint64_t) tv.tv_sec) * detection_tick_resolution + tv.tv_usec / (1000000 / detection_tick_resolution);
		ndpi_p6++;
		n = ndpi_pernet(nf_ct_net(ct));
		proto = ndpi_process_packet(n, ct, ct_ndpi, time, skb_use, CTINFO2DIR(ctinfo) != IP_CT_DIR_ORIGINAL);
...
#ifdef NF_CT_CUSTOM
static unsigned int seq_print_ndpi(struct seq_file *s, const struct nf_conn *ct, int dir)
{
...
static unsigned int ndpi_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
...
static int inet_ntop(int ipv6, void *ip, u_int16_t port, char *lbuf, size_t bufsize)
{
	...
	return ipv6 ? snprintf(lbuf, bufsize - 1, "%x:%x:%x:%x:%x:%x:%x:%x.%d", htons(ip6p[0]), htons(ip6p[1]), htons(ip6p[2]), htons(ip6p[3]), htons(ip6p[4]), htons(ip6p[5]), htons(ip6p[6]), htons(ip6p[7]), htons(port))
	    : snprintf(lbuf, bufsize - 1, "%d.%d.%d.%d:%d", ipp[0], ipp[1], ipp[2], ipp[3], htons(port));
}

static ssize_t _ninfo_proc_read(struct file *file, char __user * buf, size_t count, loff_t * ppos, int ipv6)
{
...
#define BSS1 144
#define BSS2 12
		if (*ppos * BSS1 >= bt_hash_size * 1024)
			return 0;
		...
		for (i = 0; i < BSS1; i++, t++) {
			...
			l = snprintf(lbuf, sizeof(lbuf) - 1, "%5zu%c", t->len, (i % BSS2) == (BSS2 - 1) ? '\n' : ' ');
...
static ssize_t ninfo_proc_read(struct file *file, char __user * buf, size_t count, loff_t * ppos)
{
	return _ninfo_proc_read(file, buf, count, ppos, 0);
}

static ssize_t ninfo6_proc_read(struct file *file, char __user * buf, size_t count, loff_t * ppos)
{
	return _ninfo_proc_read(file, buf, count, ppos, 1);
}

static ssize_t ninfo_proc_write(struct file *file, const char __user * buffer, size_t length, loff_t * loff)
{
	struct ndpi_net *n = PDE_DATA(file_inode(file));
	char buf[32];
	int idx;
	...
	if (length > 0) {
		...
		if (!(access_ok(VERIFY_READ, buffer, length) && !__copy_from_user(&buf[0], buffer, min(length, sizeof(buf) - 1))))
			return -EFAULT;
...
#ifdef BT_ANNOUNCE
static ssize_t nann_proc_read(struct file *file, char __user * buf, size_t count, loff_t * ppos)
{
	struct ndpi_net *n = PDE_DATA(file_inode(file));
	struct ndpi_detection_module_struct *ndpi_struct = n->ndpi_struct;
	...
	for (i = 0, p = 0; i < bt_len; i++, b++) {
		...
		l = snprintf(lbuf, sizeof(lbuf) - 1, "%08x%08x%08x%08x%08x %s %u '%.*s'\n",
			     htonl(b->hash[0]), htonl(b->hash[1]), htonl(b->hash[2]), htonl(b->hash[3]), htonl(b->hash[4]), ipbuf, b->time, b->name_len, b->name);
...
//                              cmd,id,mark,m);
		...
		for (i = 0; i < NDPI_LAST_IMPLEMENTED_PROTOCOL + 1; i++) {
			if (!any && !all && strcmp(cmd, prot_short_str[i]))
				continue;
			if (any && !i)
				continue;
			n->mark[i].mark = mark;
...
static ssize_t nproto_proc_write(struct file *file, const char __user * buffer, size_t length, loff_t * loff)
{
	struct ndpi_net *n = PDE_DATA(file_inode(file));
	...
	while (pos < length) {
		...
		memset(buf, 0, sizeof(buf));
		if (!(access_ok(VERIFY_READ, buffer + pos, l) && !__copy_from_user(&buf[0], buffer + pos, l)))
			return -EFAULT;
		for (bpos = 0; bpos < l; bpos++) {
			if (buf[bpos] == '\n') {
				if (parse_ndpi_mark(cmd, n))
					return -EINVAL;
				cpos = 0;
				memset(cmd, 0, sizeof(cmd));
				continue;
...
static int __net_init ndpi_net_init(struct net *net)
{
	...
	/* init global detection structure */
	n->ndpi_struct = ndpi_init_detection_module(detection_tick_resolution, malloc_wrapper, free_wrapper, debug_printf);
	...
	n->n_hash = -1;
	n->pe_info = proc_create_data(info_name, S_IRUGO | S_IWUSR, n->pde, &ninfo_proc_fops, n);
	...
	n->pe_proto = proc_create_data(proto_name, S_IRUGO | S_IWUSR, n->pde, &nproto_proc_fops, n);
...
#ifdef BT_ANNOUNCE
	n->pe_ann = proc_create_data(ann_name, S_IRUGO, n->pde, &nann_proc_fops, n);
	if (!n->pe_ann) {
		remove_proc_entry(proto_name, n->pde);
		remove_proc_entry(info_name, n->pde);
		PROC_REMOVE(n->pde, net);
...
#endif
#ifdef NDPI_DETECTION_SUPPORT_IPV6
	n->pe_info6 = proc_create_data(info6_name, S_IRUGO | S_IWUSR, n->pde, &ninfo6_proc_fops, n);
	if (!n->pe_info6) {
		remove_proc_entry(proto_name, n->pde);
		remove_proc_entry(info_name, n->pde);
#ifdef BT_ANNOUNCE
...
/*	size_id_struct = ndpi_detection_get_sizeof_ndpi_id_struct() + sizeof(struct osdpi_id_node); */
	...
	osdpi_flow_cache = kmem_cache_create("ndpi_flows", size_flow_struct, 0, 0, NULL);
	if (!osdpi_flow_cache) {
		pr_err("xt_ndpi: error creating flow cache.\n");
		goto unreg_target;
	}

	osdpi_id_cache = kmem_cache_create("ndpi_ids", size_id_struct, 0, 0, NULL);
	if (!osdpi_id_cache) {
		pr_err("xt_ndpi: error creating id cache.\n");
		goto free_flow;
	}

	bt_port_cache = kmem_cache_create("ndpi_btport", sizeof(struct hash_ip4p_node)
#ifdef NDPI_DETECTION_SUPPORT_IPV6
					  + 12
#endif
					  , 0, 0, NULL);
	...
	if (bt_hash_size && bt_hash_size > 32)
		bt_hash_size = 32;
	if (!bt_hash_tmo)
		bt_hash_tmo = 900;
	if (bt_hash_tmo > 3600)
	...
	pr_info("xt_ndpi v1.2 ndpi %s %s\n"
		...
		"without IPv6",
#endif
		bt_hash_size, bt_hash_tmo, sizeof(struct hash_ip4p_node)
#ifdef NDPI_DETECTION_SUPPORT_IPV6
		+ 12
#endif
		, size_id_struct, size_flow_struct,
		sizeof(struct ndpi_packet_struct), sizeof(struct ndpi_flow_tcp_struct), sizeof(struct ndpi_flow_udp_struct), sizeof(struct ndpi_int_one_line_struct), sizeof(ndpi_ip_addr_t), nf_ct_ext_id_ndpi);
	return 0;
