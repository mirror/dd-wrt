...
#define DO_HTTP_PROTO(_type) do { \
		if (IPOQUE_COMPARE_PROTOCOL_TO_BITMASK(*detection_bitmask, IPOQUE_PROTOCOL_ ## _type) != 0) { \
			IPOQUE_ADD_PROTOCOL_TO_BITMASK(ipoque_struct->callback_buffer[*a].detection_bitmask, IPOQUE_PROTOCOL_ ## _type); \
			http = true; \
		} \
	} while (0)

static void proto_common_detection_mask(struct ipoque_detection_module_struct *ipoque_struct,
