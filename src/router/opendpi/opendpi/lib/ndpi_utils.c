...
static int ndpi_check_punycode_string(char * buffer , int len) {
  int i = 0;
...
static void * ndpi_tsearch(const void *vkey, void **vrootp,
...
static void * ndpi_tdelete(const void *vkey, void **vrootp,
		    int (*compar)(const void *, const void *))
{
  ndpi_node **rootp = (ndpi_node **)vrootp;
  char *key = (char *)vkey;
...
/* Walk the nodes of a tree */
static void ndpi_twalk(const void *vroot, void (*action)(const void *, ndpi_VISIT, int, void *), void *user_data)
{
...
static u_int8_t ndpi_is_safe_ssl_cipher(u_int32_t cipher) {
...
static const char* ndpi_cipher2str(u_int32_t cipher) {
  switch(cipher) {
...
static int ndpi_has_human_readeable_string(struct ndpi_detection_module_struct *ndpi_struct,
...
static char* ndpi_ssl_version2str(u_int16_t version) {
  static char v[8];
