...
static void ndpi_search_nest_log_sink(
...
//    NDPI_LOG_DBG(ndpi_struct, "search nest_log_sink\n");
	...
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_NEST_LOG_SINK);
        return;
    }
...
static void init_nest_log_sink_dissector(
        struct ndpi_detection_module_struct *ndpi_struct,
