...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (flow->thunder_stage == 0 && packet->payload_packet_len > 17 && ndpi_mem_cmp(packet->payload, "POST / HTTP/1.1\r\n", 17) == 0) {
		...
		if (packet->empty_line_position_set != 0 &&
		    ...
		    packet->content_line.len == 24 && ndpi_mem_cmp(packet->content_line.ptr, "application/octet-stream", 24) == 0 && packet->empty_line_position_set < (packet->payload_packet_len - 8)
				  ...
				  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->payload_packet_len > 5 && memcmp(packet->payload, "GET /", 5) == 0 && NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_PROTOCOL_THUNDER)) {
		...
		if (packet->parsed_lines > 7
		    ...
		    && packet->user_agent_line.ptr != NULL && packet->user_agent_line.len > 49 && ndpi_mem_cmp(packet->user_agent_line.ptr, "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.0)", 50) == 0) {
