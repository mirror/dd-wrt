...
static void ndpi_int_oscar_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow /* , ndpi_protocol_type_t protocol_type */ )
{
					  ...
					  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->payload_packet_len >= 10 && packet->payload[0] == 0x2a) {
		 ...
		 * */
		if (packet->payload[1] == 0x02 && ntohs(get_u_int16_t(packet->payload, 4)) >= packet->payload_packet_len - 6 && get_u_int16_t(packet->payload, 6) == htons(0x0004)
