...
{
	snprintf(s, 41, "%08x%08x%08x%08x%08x", htonl(*(u_int32_t *)b), htonl(*(u_int32_t *)(b + 4)), htonl(*(u_int32_t *)(b + 8)), htonl(*(u_int32_t *)(b + 12)), htonl(*(u_int32_t *)(b + 16)));
	return s;
}
...
static char *print_id_ip_p(char *s, const struct bt_nodes_data *b)
{
	u_int8_t *p = (void *)b;
	print20b(s, b->id);
	snprintf(s + 40, 39, " %d.%d.%d.%d:%u", p[20], p[21], p[22], p[23], htons(b->port));
	return s;
}
...
static char *print_ip_p(char *s, const struct bt_ipv4p *b, int np)
{
	...
	snprintf(s, 39, !np ? "%d.%d.%d.%d:%u" : "%d.%d.%d.%d", p[0], p[1], p[2], p[3], htons(b->port));
	return s;
}
...
{
	u_int16_t *p = (void *)b;
	snprintf(s, 79, !np ? "%x:%x:%x:%x:%x:%x:%x:%x.%u" : "%x:%x:%x:%x:%x:%x:%x:%x", htons(p[0]), htons(p[1]), htons(p[2]), htons(p[3]), htons(p[4]), htons(p[5]), htons(p[6]), htons(p[7]), htons(b->port));
	return s;
}
...
static void cb_data(bt_parse_data_cb_t * cbd, int *ret)
{
	...
	if (cbd->t == 1) {
		...
		if (STREQ(cbd->buf, "a.implied_port") || STREQ(cbd->buf, "a.noseed") || STREQ(cbd->buf, "a.scrape") || STREQ(cbd->buf, "a.seed") || STREQ(cbd->buf, "a.vote")
		    ) {
			return;
		}
	...
	if ((*cbd->buf == 't' || *cbd->buf == 'v') && !cbd->buf[1]) {
		...
		case 6:
			d &= 0xffffffffffffllu;
			d = (htonl(d & 0xffffffff) << 16) | (htons(d >> 32) & 0xffff);
			break;
		case 8:
			d = ((u_int64_t)htonl(d & 0xffffffff) << 32) | htonl(d >> 32);
			break;
		default:
			d = 0;
		}
