...
static void ndpi_search_upnp(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  if(packet->udp
	 ...
	 ||
	 (packet->iphv6 && ntohl(packet->iphv6->daddr.ndpi_v6_u.u6_addr32[0]) == 0xFF020000)
#endif
	 )
	...
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_UPNP);
  }
}


static void init_upnp_dissector(struct ndpi_detection_module_struct *ndpi_struct, u_int32_t *id,
