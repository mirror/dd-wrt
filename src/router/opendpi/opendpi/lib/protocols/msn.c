			...
			if (
				...
				if (packet->content_line.offs != 0xffff &&
				    ((packet->content_line.len == NDPI_STATICSTRING_LEN("application/x-msn-messenger") &&
				     ...
				     (packet->content_line.len >= NDPI_STATICSTRING_LEN("text/x-msnmsgr") && memcmp(packet_hdr(content_line), "text/x-msnmsgr", NDPI_STATICSTRING_LEN("text/x-msnmsgr")) == 0))) {
			...
			if (status) {
				...
				if (packet->content_line.offs != 0xffff && ((packet->content_line.len == 23 && memcmp(packet_hdr(content_line), "text/xml; charset=utf-8", 23) == 0)
					...
					for (a = 0; a < packet->parsed_lines; a++) {
						if (packet->line[a].len >= 4 && (memcmp(packet_line(a), "CVR ", 4) == 0 || memcmp(packet_line(a), "VER ", 4) == 0 || memcmp(packet_line(a), "ANS ", 4) == 0)) {
