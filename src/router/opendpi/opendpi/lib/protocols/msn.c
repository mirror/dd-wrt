...
static u_int8_t ndpi_int_find_xmsn(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->parsed_lines > 3) {
		...
		for (i = 2; i < packet->parsed_lines; i++) {
			if (packet->line[i].ptr != NULL && packet->line[i].len > NDPI_STATICSTRING_LEN("X-MSN") && memcmp(packet->line[i].ptr, "X-MSN", NDPI_STATICSTRING_LEN("X-MSN")) == 0) {
				return 1;
			}
		}
	}
...
static void ndpi_search_msn_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_SSL) {
		...
		if (flow->packet_counter < 10) {
			if (flow->packet_counter == 7 && packet->payload_packet_len > 300) {
				if (memcmp(packet->payload + 24, "MSNSLP", 6) == 0 || (get_u_int32_t(packet->payload, 0) == htonl(0x30000000) && get_u_int32_t(packet->payload, 4) == 0x00000000)) {
			...
			if (flow->packet_counter >= 5 && flow->packet_counter <= 10 && (get_u_int32_t(packet->payload, 0) == htonl(0x18000000)
				...
				NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE, "increased msn ft ssl stage to: %u at packet nr: %u\n", flow->l4.tcp.msn_ssl_ft, flow->packet_counter);
				if (flow->l4.tcp.msn_ssl_ft == 2) {
					NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE, "detected MSN File Transfer, ifdef ssl 2.\n");
					ndpi_int_msn_add_connection(ndpi_struct, flow);
				}
				return;
			}
	...
	if (flow->packet_counter == 1
		...
		if (packet->payload_packet_len > 32 && (packet->payload[0] == 0x02 || packet->payload[0] == 0x00)
			...
			|| (ntohl(get_u_int32_t(packet->payload, 20)) == 0x000f0004 && ntohl(get_u_int32_t(packet->payload, 24)) == 0x72c64bc6))) {
			NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE, "found MSN in packets that also contain voice.messenger.live.com.\n");
		...
		if (
			...
			if (packet->user_agent_line.ptr != NULL &&
			    packet->user_agent_line.len > NDPI_STATICSTRING_LEN("Messenger/") && memcmp(packet->user_agent_line.ptr, "Messenger/", NDPI_STATICSTRING_LEN("Messenger/")) == 0) {
				ndpi_int_msn_add_connection(ndpi_struct, flow);
				return;
			}
		}
...
/* #endif */
		...
		if (packet->payload_packet_len > 100) {
			if (
				...
				if (packet->content_line.ptr != NULL &&
					...
					NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE, "found MSN by pattern POST http:// .... application/x-msn-messenger.\n");
					ndpi_int_msn_add_connection(ndpi_struct, flow);
					return;
				}
			}
		...
		if (packet->payload_packet_len > 400) {
			...
			if (status) {
				...
				if (packet->content_line.ptr != NULL && ((packet->content_line.len == 23 && memcmp(packet->content_line.ptr, "text/xml; charset=utf-8", 23) == 0)
									 || (packet->content_line.len == 24 && memcmp(packet->content_line.ptr, "text/html; charset=utf-8", 24) == 0)
									 || (packet->content_line.len == 33 && memcmp(packet->content_line.ptr, "application/x-www-form-urlencoded", 33) == 0))) {
					...
					for (a = 0; a < packet->parsed_lines; a++) {
						if (packet->line[a].len >= 4 && (memcmp(packet->line[a].ptr, "CVR ", 4) == 0 || memcmp(packet->line[a].ptr, "VER ", 4) == 0 || memcmp(packet->line[a].ptr, "ANS ", 4) == 0)) {
							...
							NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE, "found MSN with pattern text/sml; charset0utf-8.\n");
		...
		if (flow->packet_counter <= 10 && (flow->packet_direction_counter[0] <= 2 || flow->packet_direction_counter[1] <= 2)
		...
		if (packet->payload_packet_len >= 18) {
			...
			if (memcmp(packet->payload, "USR ", 4) == 0 || memcmp(packet->payload, "ANS ", 4) == 0) {
				...
				while (plen < endlen) {
					...
					if (packet->payload_packet_len > plen + 1 && (packet->payload[plen] < 20 || packet->payload[plen] > 128)) {
						goto ndpi_msn_exclude;
					}
					plen++;
					if (plen >= endlen) {
	...
	if ((src != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_PROTOCOL_MSN) != 0)
	    || (dst != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask, NDPI_PROTOCOL_MSN) != 0)) {
		if (flow->packet_counter == 1 && packet->payload_packet_len > 12 && memcmp(packet->payload, "recipientid=", 12) == 0) {
	...
	if (flow->l4.tcp.msn_stage == 0) {
		...
		if ((packet->payload_packet_len == 4) && (get_u_int32_t(packet->payload, 0) == htonl(0x10000000) || get_u_int32_t(packet->payload, 0) == htonl(0x30000000))) {
			ndpi_int_msn_add_connection(ndpi_struct, flow);
	...
	if (flow->packet_counter <= MAX_PACKETS_FOR_MSN) {
		if (memcmp(&packet->payload[0], "MSG ", 4) == 0
		    ...
		    || memcmp(&packet->payload[0], "RNG ", 4) == 0 || memcmp(&packet->payload[0], "NLN ", 4) == 0 || memcmp(&packet->payload[0], "UBX ", 4) == 0 || memcmp(&packet->payload[0], "XFR ", 4) == 0) {
			ndpi_int_msn_add_connection(ndpi_struct, flow);
