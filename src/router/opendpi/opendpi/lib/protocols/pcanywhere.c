...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->udp != NULL && packet->udp->dest == htons(5632)
	    && packet->payload_packet_len == 2 && (ndpi_mem_cmp(packet->payload, "NQ", 2) == 0 || ndpi_mem_cmp(packet->payload, "ST", 2) == 0)) {
		NDPI_LOG(NDPI_PROTOCOL_PCANYWHERE, ndpi_struct, NDPI_LOG_DEBUG, "PC Anywhere name or status query detected.\n");
