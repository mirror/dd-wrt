...
static void ndpi_int_pplive_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow, ndpi_protocol_type_t protocol_type)
{
				       ...
				       *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->udp != NULL) {
		...
		if (src != NULL && src->pplive_vod_cli_port == packet->udp->source && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_PROTOCOL_PPLIVE)) {
		...
		if (dst != NULL && dst->pplive_vod_cli_port == packet->udp->dest && NDPI_COMPARE_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask, NDPI_PROTOCOL_PPLIVE)) {
		...
		if (packet->payload_packet_len > 50 && packet->payload[0] == 0xe9 && packet->payload[1] == 0x03 && (packet->payload[3] == 0x00 || packet->payload[3] == 0x01)
		    && packet->payload[4] == 0x98 && packet->payload[5] == 0xab && packet->payload[6] == 0x01 && packet->payload[7] == 0x02) {
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive.\n");
			ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
			return;
		}

		if (packet->payload_packet_len == 94 && packet->payload[8] == 0x00 && ((get_u_int32_t(packet->payload, 9) == ntohl(0x03010000))
		...
		if ((packet->payload_packet_len >= 90 && packet->payload_packet_len <= 110)
			...
			for (i = 54; i < 68; i += 2) {
				if (((get_u_int32_t(packet->payload, i) == ntohl(0x4fde7e7f))
					...
					NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive through " "bitpatterns either 4f de 7e 7f 00 00 or 7a a6 09 0d 00 00.\n");
					ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
					return;
				}
			}
		...
		if (flow->packet_counter < 5 && !flow->pplive_stage) {	/* With in 1st 4 packets */
			if (((packet->payload_packet_len >= 90 && packet->payload_packet_len <= 110)
				...
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "Maybe found pplive packet. Now start looking for size(28 | 30).\n");
			}
			if (68 == packet->payload_packet_len && get_l16(packet->payload, 0) == 0x21 && packet->payload[19] == packet->payload[20]
		...
		if (flow->pplive_stage == (4 - packet->packet_direction)
		    && packet->payload_packet_len > 67 && (get_l16(packet->payload, 0) == 0x21 || (get_l16(packet->payload, 0) == 0x22 && !get_l16(packet->payload, 28)))) {
			if (dst != NULL) {
				dst->pplive_vod_cli_port = packet->udp->dest;
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: VOD Port marked %u.\n", ntohs(packet->udp->dest));
				dst->pplive_last_packet_time = packet->tick_timestamp;
				dst->pplive_last_packet_time_set = 1;
			}
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive.\n");
		...
		NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG,
			 ...
			 packet->payload_packet_len, flow->pplive_stage, packet->payload[0], packet->payload[1], packet->payload[2], packet->payload[3]);

		if (src != NULL && src->pplive_vod_cli_port == packet->tcp->source && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_PROTOCOL_PPLIVE)) {
			if (src->pplive_last_packet_time_set == 1 && (u_int32_t)
			    (packet->tick_timestamp - src->pplive_last_packet_time) < ndpi_struct->pplive_connection_timeout) {
				ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
				src->pplive_last_packet_time_set = 1;
		...
		if (dst != NULL && dst->pplive_vod_cli_port == packet->tcp->dest && NDPI_COMPARE_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask, NDPI_PROTOCOL_PPLIVE)) {
			if (dst->pplive_last_packet_time_set == 1 && (u_int32_t)
			    (packet->tick_timestamp - dst->pplive_last_packet_time) < ndpi_struct->pplive_connection_timeout) {
				ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
				dst->pplive_last_packet_time_set = 1;
		...
		if (packet->payload_packet_len > 4 && memcmp(packet->payload, "GET /", 5) == 0) {
			...
			if ((packet->parsed_lines == 8 || packet->parsed_lines == 9)
			    ...
			    && memcmp(packet->user_agent_line.ptr, "Mozilla/4.0", 11) == 0 && packet->line[6].ptr != NULL && packet->line[6].len >= 21 && memcmp(packet->line[6].ptr, "Pragma: Client=PPLive", 21) == 0) {
				...
				return;
				   ...
				   && packet->line[0].ptr != NULL && packet->line[0].len >= 8
				   && memcmp(&packet->payload[packet->line[0].len - 8], "HTTP/1.", 7) == 0 && (((packet->user_agent_line.ptr != NULL && packet->user_agent_line.len >= 10)
				...
				ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
				return;
			} else if (packet->referer_line.ptr != NULL && packet->referer_line.len >= NDPI_STATICSTRING_LEN("http://player.pplive")
				   && memcmp(packet->referer_line.ptr, "http://player.pplive", NDPI_STATICSTRING_LEN("http://player.pplive")) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: found via Referer header.\n");
				ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
				return;
			} else if (packet->parsed_lines >= 8 && packet->line[0].ptr != NULL && packet->line[0].len >= 8 && memcmp(&packet->payload[packet->line[0].len - 8], "HTTP/1.", 7) == 0) {
				...
				for (i = 0; i < packet->parsed_lines && i < 10 && flag < 2; i++) {
					if (packet->line[i].ptr != NULL && packet->line[i].len >= NDPI_STATICSTRING_LEN("x-flash-version:")
					    && memcmp(packet->line[i].ptr, "x-flash-version:", NDPI_STATICSTRING_LEN("x-flash-version:")) == 0) {
						flag++;
					} else if (packet->line[i].ptr != NULL && packet->line[i].len >= NDPI_STATICSTRING_LEN("Pragma: Client=PPLive")
						   && memcmp(packet->line[i].ptr, "Pragma: Client=PPLive", NDPI_STATICSTRING_LEN("Pragma: Client=PPLive")) == 0) {
						flag++;
					}
				}
				if (flag == 2) {
		...
		if (packet->payload_packet_len > 20 && ntohl(get_u_int32_t(packet->payload, 0)) == packet->payload_packet_len - 4) {
			if (packet->payload[4] == 0x21 && packet->payload[5] == 0x00) {
				if ((packet->payload[9] == packet->payload[10]) && (packet->payload[9] == packet->payload[11])) {
					if ((packet->payload[16] == packet->payload[17]) && (packet->payload[16] == packet->payload[18]) && (packet->payload[16] == packet->payload[19])) {
						NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: direct server request or response found\n");
						ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
						return;
					}
				}
		...
		if (packet->payload_packet_len > 11 && ntohl(get_u_int32_t(packet->payload, 0)) == packet->payload_packet_len - 4) {
			if (packet->payload[4] == 0xe9 && packet->payload[5] == 0x03 && ((packet->payload[7] == packet->payload[10]) || (packet->payload[7] == packet->payload[11]))) {
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: direct server request or response found\n");
				ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
				return;
			}
		...
		if (packet->payload_packet_len > 10 && flow->pplive_stage) {
			if (packet->payload[0] == 0xe9 && packet->payload[1] == 0x03 && ((packet->payload[3] == packet->payload[6]) || (packet->payload[3] == packet->payload[7]))) {
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: direct server request or response found\n");
				ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
				return;
			}
		...
		if (packet->payload_packet_len > 11 && ntohl(get_u_int32_t(packet->payload, 0)) == packet->payload_packet_len - 4) {
			if (packet->payload[4] == 0x21 && packet->payload[5] == 0x00 && ((packet->payload[8] == 0x98 && packet->payload[9] == 0xab && packet->payload[10] == 0x01 && packet->payload[11] == 0x02)
			    )) {
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: direct server request or response found\n");
				ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
				return;
			...
			if (packet->payload_packet_len > 20) {
				...
				while (a < 20) {
					if (packet->payload[a] >= '0' && packet->payload[a] <= '9') {
						if (a == 19) {
							NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: direct new header format found\n");
							ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
							return;
						} else {
							a++;
		...
		if (flow->pplive_stage == 0) {
			if (packet->payload_packet_len == 4 && packet->payload[0] > 0x04 && packet->payload[1] == 0x00 && packet->payload[2] == 0x00 && packet->payload[3] == 0x00) {
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: 4Byte TCP Packet Request found \n");
			...
			if (packet->payload_packet_len == 4 && packet->payload[0] > 0x04 && packet->payload[1] == 0x00 && packet->payload[2] == 0x00 && packet->payload[3] == 0x00) {
				NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: 4Byte TCP Packet Response found \n");
			...
			if (packet->payload_packet_len > 7 && flow->l4.tcp.pplive_next_packet_size[packet->packet_direction] >= 4) {
				if (packet->payload_packet_len == flow->l4.tcp.pplive_next_packet_size[packet->packet_direction]) {
					...
					if (packet->payload[0] == 0xe9 && packet->payload[1] == 0x03
						...
						NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: two packet response found\n");

						NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive over tcp with pattern iii.\n");
						ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
						return;
					}
					// packet 4 to 19 have a hex representation from 0x30 to 0x39
					if (packet->payload_packet_len > 16) {
						...
						while (a < 16) {
							if (packet->payload[a] >= '0' && packet->payload[a] <= '9') {
								if (a == 15) {
									NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: new header format found\n");
									NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive over tcp with pattern v.\n");
									ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
									return;
								} else {
									a++;
					...
					if (packet->payload_packet_len > 79
					    && get_u_int32_t(packet->payload, packet->payload_packet_len - 9) == 0x00000000 && get_u_int32_t(packet->payload, packet->payload_packet_len - 5) == 0x00000000) {
						NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "PPLIVE: Last 8 NULL bytes found.\n");
						NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive over tcp with pattern vi.\n");
						ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
						return;
					}
				}
				if (packet->payload_packet_len > flow->l4.tcp.pplive_next_packet_size[packet->packet_direction]) {
					if (packet->payload[0] == 0xe9 && packet->payload[1] == 0x03
						...
						if (packet->payload_packet_len > a + 4 && packet->payload[a + 2] == 0x00 && packet->payload[a + 3] == 0x00 && packet->payload[a] != 0) {
							a += ((packet->payload[a + 1] << 8) + packet->payload[a] + 4);
							NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "a=%u.\n", a);
							if (packet->payload_packet_len == a) {
								NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive over tcp with pattern vii.\n");
								ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
								return;
							}
							if (packet->payload_packet_len > a + 4 && packet->payload[a + 2] == 0x00 && packet->payload[a + 3] == 0x00 && packet->payload[a] != 0) {
								a += ((packet->payload[a + 1] << 8) + packet->payload[a] + 4);
								if (packet->payload_packet_len == a) {
									NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive over tcp with pattern viii.\n");
									ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
									return;
								}
							}
