...
static void ndpi_int_secondlife_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_SECONDLIFE, NDPI_PROTOCOL_UNKNOWN);
}
				   ...
				   *ndpi_struct, struct ndpi_flow_struct *flow)
{
					...
					if (packet_hdr(host_line)[packet->host_line.len - (1 + x)] == ':') {
						if ((1 + x + NDPI_STATICSTRING_LEN(".agni.lindenlab.com")) < packet->host_line.len
						    && memcmp(&packet_hdr(host_line)[packet->host_line.len -
										     (1 + x + NDPI_STATICSTRING_LEN(".agni.lindenlab.com"))], ".agni.lindenlab.com", NDPI_STATICSTRING_LEN(".agni.lindenlab.com")) == 0) {
							NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct, NDPI_LOG_DEBUG, "Second Life TCP HTTP Host detected.\n");
							ndpi_int_secondlife_add_connection(ndpi_struct, flow);
							return;
						}
						break;
	...
	if (packet->udp != NULL) {
		if (packet->payload_packet_len == 46 && memcmp(packet->payload, "\x40\x00\x00\x00\x01\x00\xff\xff\x00\x03", 10) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct, NDPI_LOG_DEBUG, "Second Life 0xffff0003 detected.\n");
			ndpi_int_secondlife_add_connection(ndpi_struct, flow);
			return;
		}
		if (packet->payload_packet_len == 54 && memcmp(packet->payload, "\x40\x00\x00\x00\x01\x00\xff\xff\x00\x52", 10) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct, NDPI_LOG_DEBUG, "Second Life 0xffff0052 detected.\n");
			ndpi_int_secondlife_add_connection(ndpi_struct, flow);
			return;
		}
		if (packet->payload_packet_len == 58 && memcmp(packet->payload, "\x40\x00\x00\x00\x01\x00\xff\xff\x00\xa9", 10) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct, NDPI_LOG_DEBUG, "Second Life 0xffff00a9 detected.\n");
			ndpi_int_secondlife_add_connection(ndpi_struct, flow);
			return;
		}
		if (packet->payload_packet_len > 54 && memcmp(packet->payload, "\x40\x00\x00\x00\x01\x00\x08", 7) == 0 && get_u_int32_t(packet->payload, packet->payload_packet_len - 4) == 0) {
...
static void init_secondlife_dissector(struct ndpi_detection_module_struct *ndpi_struct, u_int32_t *id, NDPI_PROTOCOL_BITMASK * detection_bitmask)
{
	ndpi_set_bitmask_protocol_detection("SECONDLIFE", ndpi_struct, detection_bitmask, *id,
