...
static void ndpi_int_smpp_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
...
static void ndpi_search_smpp_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (flow->packet.detected_protocol_stack[0] != NDPI_PROTOCOL_SMPP) {
		...
		NDPI_LOG(NDPI_PROTOCOL_SMPP, ndpi_struct, NDPI_LOG_DEBUG, "calculated PDU Length: %d, received PDU Length: %d\n", pdu_l, packet->payload_packet_len);
		...
		if (pdu_l != packet->payload_packet_len) {
			...
			while (total_pdu_l < packet->payload_packet_len) {
				...
				if (tmp_pdu_l == 0 || ndpi_check_overflow(tmp_pdu_l, total_pdu_l))
					return;
			...
			NDPI_LOG(NDPI_PROTOCOL_SMPP,
				 ndpi_struct, NDPI_LOG_DEBUG, "multiple PDUs included, calculated total PDU Length: %d, PDU count: %d, TCP payload length: %d\n", total_pdu_l, pdu_c, packet->payload_packet_len);
		...
		if ((pdu_req > 0x00000000 && pdu_req <= 0x00000009) || (pdu_req == 0x0000000B || pdu_req == 0x00000015 || pdu_req == 0x00000021 || pdu_req == 0x00000102 || pdu_req == 0x00000103)) {

			NDPI_LOG(NDPI_PROTOCOL_SMPP, ndpi_struct, NDPI_LOG_DEBUG, "PDU type: %x, Request PDU type = %x\n", pdu_type, pdu_req);
...
static void init_smpp_dissector(struct ndpi_detection_module_struct *ndpi_struct, u_int32_t *id, NDPI_PROTOCOL_BITMASK * detection_bitmask)
{
	ndpi_set_bitmask_protocol_detection("SMPP", ndpi_struct, detection_bitmask, *id,
					    NDPI_PROTOCOL_SMPP,
					    ndpi_search_smpp_tcp, NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, SAVE_DETECTION_BITMASK_AS_UNKNOWN, ADD_TO_DETECTION_BITMASK);
