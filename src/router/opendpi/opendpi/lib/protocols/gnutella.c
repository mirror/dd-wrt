...
void ipoque_search_gnutella(struct ipoque_detection_module_struct *ipoque_struct)
{
	...
	if (packet->tcp != NULL) {
		...
		} else if (packet->payload_packet_len == 46) {
			/* might be super tricky new ssl gnutella transmission, but the certificate is strange... */
			if (get_u32(packet->payload, 0) == htonl(0x802c0103)
				&& get_u32(packet->payload, 4) == htonl(0x01000300)
				&& get_u32(packet->payload, 8) == htonl(0x00002000)
...
//neonet detection follows
	...
	if (packet->tcp != NULL && ntohs(packet->tcp->source) >= 1024 && ntohs(packet->tcp->dest) >= 1024) {
		if (flow->gnutella_stage == 0) {
			if (flow->packet_counter == 1
				&& (packet->payload_packet_len == 11
					|| packet->payload_packet_len == 33 || packet->payload_packet_len == 37)) {
				flow->gnutella_msg_id[0] = packet->payload[4];
				flow->gnutella_msg_id[1] = packet->payload[6];
				flow->gnutella_msg_id[2] = packet->payload[8];
				flow->gnutella_stage = 1 + packet->packet_direction;
				return;
			}
		} else if (flow->gnutella_stage == 1 + packet->packet_direction) {
			if (flow->packet_counter == 2 && (packet->payload_packet_len == 33 || packet->payload_packet_len == 22)
				&& flow->gnutella_msg_id[0] == packet->payload[0]
				&& flow->gnutella_msg_id[1] == packet->payload[2]
				&& flow->gnutella_msg_id[2] == packet->payload[4]
		...
		} else if (flow->gnutella_stage == 2 - packet->packet_direction) {
