...
static void ndpi_search_battlefield(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_BATTLEFIELD) {
		if (src != NULL && ((u_int32_t)
			...
			NDPI_LOG(NDPI_PROTOCOL_BATTLEFIELD, ndpi_struct, NDPI_LOG_DEBUG, "battlefield : save src connection packet detected\n");
			src->battlefield_ts = packet->tick_timestamp;
		} else if (dst != NULL && ((u_int32_t)
					   (packet->tick_timestamp - dst->battlefield_ts) < ndpi_struct->battlefield_timeout)) {
			NDPI_LOG(NDPI_PROTOCOL_BATTLEFIELD, ndpi_struct, NDPI_LOG_DEBUG, "battlefield : save dst connection packet detected\n");
			dst->battlefield_ts = packet->tick_timestamp;
		}
		return;
	}
	...
	if (NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_PROTOCOL_BATTLEFIELD)) {
		if (flow->l4.udp.battlefield_stage == 0 || flow->l4.udp.battlefield_stage == 1 + packet->packet_direction) {
			...
			if (packet->payload_packet_len > 8 && get_u_int32_t(packet->payload, 0) == flow->l4.udp.battlefield_msg_id) {
				NDPI_LOG(NDPI_PROTOCOL_BATTLEFIELD, ndpi_struct, NDPI_LOG_DEBUG, "Battlefield message and reply detected.\n");
	...
	if (flow->l4.udp.battlefield_stage == 0) {
		if (packet->payload_packet_len == 46 && packet->payload[2] == 0 && packet->payload[4] == 0 && get_u_int32_t(packet->payload, 7) == htonl(0x98001100)) {
			flow->l4.udp.battlefield_stage = 3 + packet->packet_direction;
			return;
		}
	} else if (flow->l4.udp.battlefield_stage == 4 - packet->packet_direction) {
		if (packet->payload_packet_len == 7 && (packet->payload[0] == 0x02 || packet->payload[packet->payload_packet_len - 1] == 0xe0)) {
			NDPI_LOG(NDPI_PROTOCOL_BATTLEFIELD, ndpi_struct, NDPI_LOG_DEBUG, "Battlefield message and reply detected.\n");
			ndpi_int_battlefield_add_connection(ndpi_struct, flow);
			return;
		}
	...
	if (packet->payload_packet_len == 18 && ndpi_mem_cmp(&packet->payload[5], "battlefield2\x00", 13) == 0) {
		...
		return;
		   ...
		   (ndpi_mem_cmp(packet->payload, "\x11\x20\x00\x01\x00\x00\x50\xb9\x10\x11", 10) == 0
		    || ndpi_mem_cmp(packet->payload, "\x11\x20\x00\x01\x00\x00\x30\xb9\x10\x11", 10) == 0 || ndpi_mem_cmp(packet->payload, "\x11\x20\x00\x01\x00\x00\xa0\x98\x00\x11", 10) == 0)) {
