...
static void ndpi_int_feidian_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow, ndpi_protocol_type_t protocol_type)
{
...
static void ndpi_search_feidian(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		if (packet->tcp->dest == htons(8080) && packet->payload_packet_len == 4 && packet->payload[0] == 0x29 && packet->payload[1] == 0x1c && packet->payload[2] == 0x32 && packet->payload[3] == 0x01) {
			...
			if (packet->host_line.ptr != NULL && packet->host_line.len == 18 && memcmp(packet->host_line.ptr, "config.feidian.com", 18) == 0) {
