...
static void ndpi_int_tvuplayer_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow	/* , */
	...
	if (packet->tcp != NULL) {
		...
		if (packet->payload_packet_len >= 50) {
			...
			if (memcmp(packet->payload, "POST", 4) || memcmp(packet->payload, "GET", 3)) {
				...
				if (packet->user_agent_line.offs != 0xffff && packet->user_agent_line.len >= 8 && (memcmp(packet_hdr(user_agent_line), "MacTVUP", 7) == 0)) {
