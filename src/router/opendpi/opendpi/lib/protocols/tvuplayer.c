												...
												*ipoque_struct)
{
	...
	flow->detected_protocol = IPOQUE_PROTOCOL_TVUPLAYER;
	packet->detected_protocol = IPOQUE_PROTOCOL_TVUPLAYER;


	if (src != NULL) {
		IPOQUE_ADD_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, IPOQUE_PROTOCOL_TVUPLAYER);
	}
	if (dst != NULL) {
		IPOQUE_ADD_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask, IPOQUE_PROTOCOL_TVUPLAYER);
	}
}
...
//      struct ipoque_id_struct         *dst=ipoque_struct->dst;
	...
	if (packet->tcp != NULL) {
		if ((packet->payload_packet_len == 36 || packet->payload_packet_len == 24)
			...
			ipoque_int_tvuplayer_add_connection(ipoque_struct);
			return;
		}
