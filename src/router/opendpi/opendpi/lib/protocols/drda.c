...
static void ndpi_search_drda(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		...
		if (payload_len >= sizeof(struct ndpi_drda_hdr)) {
			...
			/* check first header */
			if (len != ntohs(drda->length2) + 6 || drda->magic != 0xd0)
				goto no_drda;
...
static void init_drda_dissector(struct ndpi_detection_module_struct *ndpi_struct, u_int32_t *id, NDPI_PROTOCOL_BITMASK * detection_bitmask)
{
	ndpi_set_bitmask_protocol_detection("DRDA", ndpi_struct, detection_bitmask, *id,
					    NDPI_PROTOCOL_DRDA, ndpi_search_drda, NDPI_SELECTION_BITMASK_PROTOCOL_TCP_WITH_PAYLOAD, SAVE_DETECTION_BITMASK_AS_UNKNOWN, ADD_TO_DETECTION_BITMASK);
