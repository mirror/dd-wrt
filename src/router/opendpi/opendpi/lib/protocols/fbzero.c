...
PACK_ON struct fbzero_tag {
	char tag[4];
	u_int32_t tag_offset_len;
} PACK_OFF;

PACK_ON struct fbzero_header {
	u_int8_t flags;
	char version[3];
	u_int8_t unknown;
	u_int32_t len;
...
static void ndpi_search_fbzero(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
...
#ifdef DEBUG
				printf("[FBZERO] SNI [@%u][len: %u][%s]\n", tag_len, t->tag_offset_len - data_prev_offset, flow->host_server_name);
#endif

				ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_FBZERO, NDPI_PROTOCOL_UNKNOWN);

				ndpi_match_host_subprotocol(ndpi_struct, flow, (char *)flow->host_server_name, strlen((const char *)flow->host_server_name), NDPI_PROTOCOL_FBZERO);
				return;
			}
...
fbzero_not_found:
	NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_FBZERO);
}
...
static void init_fbzero_dissector(struct ndpi_detection_module_struct *ndpi_struct, u_int32_t *id, NDPI_PROTOCOL_BITMASK * detection_bitmask)
{
	ndpi_set_bitmask_protocol_detection("FacebookZero", ndpi_struct, detection_bitmask, *id,
					    ...
					    ndpi_search_fbzero, NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_WITH_PAYLOAD_WITHOUT_RETRANSMISSION, SAVE_DETECTION_BITMASK_AS_UNKNOWN, ADD_TO_DETECTION_BITMASK);
	*id += 1;
}
