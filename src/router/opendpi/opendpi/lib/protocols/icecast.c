...
static void ndpi_search_icecast_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if ((packet->payload_packet_len < 500 && packet->payload_packet_len >= 7 && memcmp(packet->payload, "SOURCE ", 7) == 0)
		...
		for (i = 0; i < packet->parsed_lines; i++) {
			if (packet->line[i].offs != 0xffff && packet->line[i].len > 4 && memcmp(packet_line(i), "ice-", 4) == 0) {
	...
	if (packet->packet_direction != flow->setup_packet_direction) {
		...
		if (packet->server_line.offs != 0xffff && packet->server_line.len > NDPI_STATICSTRING_LEN("Icecast") && memcmp(packet_hdr(server_line), "Icecast", NDPI_STATICSTRING_LEN("Icecast")) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_ICECAST, ndpi_struct, NDPI_LOG_DEBUG, "Icecast detected.\n");
