...
static int is_diameter(struct ndpi_packet_struct *packet, int size_payload)
{
	...
	if (diameter->version == 0x01 && (diameter->flags == REQUEST || diameter->flags == PROXYABLE || diameter->flags == ERROR || diameter->flags == RETRASM)) {

		u_int16_t com_code = diameter->com_code[2] + (diameter->com_code[1] << 8) + (diameter->com_code[0] << 8);

		if (com_code == AC || com_code == AS || com_code == CC || com_code == CE || com_code == DW || com_code == DP || com_code == RA || com_code == ST)
			return 0;	// OK
	}
	// wrong packet
	return -2;
