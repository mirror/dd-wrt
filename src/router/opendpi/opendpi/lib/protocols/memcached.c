					      ...
					      *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_MEMCACHED, NDPI_PROTOCOL_UNKNOWN);
}
...
static void ndpi_search_memcached(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		if (packet->payload_packet_len < MEMCACHED_MIN_LEN) {
			NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_MEMCACHED);
			return;
		}
		...
		if ((offset[4] == 0x00 && offset[5] == 0x00) || offset[6] != 0x00 || offset[7] != 0x00) {
			NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_MEMCACHED);
			return;
		}
...
static void init_memcached_dissector(struct ndpi_detection_module_struct *ndpi_struct, u_int32_t *id, NDPI_PROTOCOL_BITMASK * detection_bitmask)
{
	ndpi_set_bitmask_protocol_detection("MEMCACHED",
					    ...
					    NDPI_PROTOCOL_MEMCACHED, ndpi_search_memcached, NDPI_SELECTION_BITMASK_PROTOCOL_V4_V6_TCP_OR_UDP_WITH_PAYLOAD, SAVE_DETECTION_BITMASK_AS_UNKNOWN, ADD_TO_DETECTION_BITMASK);
