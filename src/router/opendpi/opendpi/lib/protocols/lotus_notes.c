...
static void ndpi_check_lotus_notes(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		...
		printf("[lotus_notes_packet_id: %u][len=%u][%02X %02X %02X %02X %02X %02X %02X %02X]\n",
		       ...
		       packet->payload[7] & 0xFF, packet->payload[8] & 0xFF, packet->payload[9] & 0xFF, packet->payload[10] & 0xFF, packet->payload[11] & 0xFF, packet->payload[12] & 0xFF, packet->payload[13] & 0xFF);
#endif
