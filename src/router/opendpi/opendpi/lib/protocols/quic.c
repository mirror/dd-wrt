...
{
	if ((sport == 443 || dport == 443 || sport == 80 || dport == 80) && (sport != 123 && dport != 123))
		return 1;
...
static void ndpi_search_quic(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->udp != NULL && (udp_len > (quic_hlen + 4 /* QXXX */ ))
	    ...
	    u_int32_t prev_offset = le32toh(*((u_int32_t*)&packet->payload[i-4]));
						...
						if ((sni_offset + len) < udp_len) {
							...
							ndpi_match_host_subprotocol(ndpi_struct, flow, (char *)flow->host_server_name, strlen((const char *)flow->host_server_name), NDPI_PROTOCOL_QUIC);
