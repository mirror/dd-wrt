	...
	if (packet->udp != NULL) {
		...
		case 3:
			if (payload_len > 11 && memcmp(packet_payload, "1 ", 2) == 0 && packet_payload[2] != ' ') {
				...
				while ((packet_payload[i] >= '0' && packet_payload[i] <= '9') || (packet_payload[i] >= 'A' && packet_payload[i] <= 'Z')) {
					i++;
				}
...
static void init_tinc_dissector(struct ndpi_detection_module_struct *ndpi_struct, u_int32_t *id, NDPI_PROTOCOL_BITMASK * detection_bitmask)
{
	ndpi_set_bitmask_protocol_detection("TINC", ndpi_struct, detection_bitmask, *id,
					    NDPI_PROTOCOL_TINC, ndpi_search_tinc, NDPI_SELECTION_BITMASK_PROTOCOL_TCP_OR_UDP, SAVE_DETECTION_BITMASK_AS_UNKNOWN, ADD_TO_DETECTION_BITMASK);
