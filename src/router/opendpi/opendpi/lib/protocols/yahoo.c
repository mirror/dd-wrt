	...
	if (ndpi_struct->yahoo_detect_http_connections != 0 && packet->payload_packet_len > 100) {
		...
		if (memcmp(packet->payload, "POST ", 5) == 0) {
			...
			if (NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_PROTOCOL_YAHOO)
			    ...
			    && ndpi_mem_cmp(packet->line[4].ptr, "User-Agent: Mozilla/5.0", 23) == 0 && packet->line[5].len >= 23 && ndpi_mem_cmp(packet->line[5].ptr, "Cache-Control: no-cache", 23) == 0) {
			...
			if (packet->host_line.ptr != NULL && packet->host_line.len >= 26 && ndpi_mem_cmp(packet->host_line.ptr, "filetransfer.msg.yahoo.com", 26) == 0) {
		...
		if (flow->l4.tcp.yahoo_http_proxy_stage == 2 - packet->packet_direction) {
			...
			if (packet->parsed_unix_lines >= 9) {
				...
				if (packet->unix_line[4].ptr != NULL && packet->unix_line[4].len >= 9 &&
				    packet->unix_line[8].ptr != NULL && packet->unix_line[8].len >= 6 && memcmp(packet->unix_line[4].ptr, "<Session ", 9) == 0 && memcmp(packet->unix_line[8].ptr, "<Ymsg ", 6) == 0) {
