...
static void ndpi_int_yahoo_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow /* , ndpi_protocol_type_t protocol_type */ )
{
...
static void ndpi_search_yahoo_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->payload_packet_len >= 20 && memcmp(yahoo->YMSG_str, "YMSG", 4) == 0 && ((packet->payload_packet_len - 20) == ntohs(yahoo->len)
		...
		return;
	} else if (flow->l4.tcp.yahoo_sip_comm && packet->detected_protocol_stack[0] == NDPI_PROTOCOL_UNKNOWN && flow->packet_counter < 3) {
		return;
	}
	...
	if (ndpi_struct->yahoo_detect_http_connections != 0 && packet->payload_packet_len > 100) {
		if (memcmp(packet->payload, "POST /relay?token=", 18) == 0
		    || memcmp(packet->payload, "GET /relay?token=", 17) == 0 || memcmp(packet->payload, "GET /?token=", 12) == 0 || memcmp(packet->payload, "HEAD /relay?token=", 18) == 0) {
			if ((src != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_PROTOCOL_YAHOO)
			     != 0) || (dst != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask, NDPI_PROTOCOL_YAHOO)
				       != 0)) {
		...
		if (memcmp(packet->payload, "POST ", 5) == 0) {
			...
			if (NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_PROTOCOL_YAHOO)
			    ...
			    && packet->line[4].len >= 23 && memcmp(packet_line(4), "User-Agent: Mozilla/5.0", 23) == 0 && packet->line[5].len >= 23 && memcmp(packet_line(5), "Cache-Control: no-cache", 23) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_YAHOO, ndpi_struct, NDPI_LOG_TRACE, "YAHOO HTTP POST P2P FILETRANSFER FOUND\n");
				NDPI_LOG(NDPI_PROTOCOL_YAHOO, ndpi_struct, NDPI_LOG_DEBUG, "found YAHOO");
				ndpi_int_yahoo_add_connection(ndpi_struct, flow);
				return;
			}

			if (packet->host_line.offs != 0xffff && packet->host_line.len >= 26 && memcmp(packet_hdr(host_line), "filetransfer.msg.yahoo.com", 26) == 0) {
			...
			for (a = 0; a < packet->parsed_lines; a++) {
				if (packet->line[a].len >= 4 && memcmp(packet_line(a), "YMSG", 4) == 0) {
					NDPI_LOG(NDPI_PROTOCOL_YAHOO, ndpi_struct, NDPI_LOG_TRACE, "YAHOO HTTP POST FOUND, line is: %.*s\n", packet->line[a].len, packet_line(a));
					NDPI_LOG(NDPI_PROTOCOL_YAHOO, ndpi_struct, NDPI_LOG_DEBUG, "found YAHOO");
					ndpi_int_yahoo_add_connection(ndpi_struct, flow);
					return;
				}
		...
		if ((memcmp(packet->payload, "GET /", 5) == 0)) {
			ndpi_parse_packet_line_info(ndpi_struct, flow);
			if ((packet->user_agent_line.offs != 0xffff && packet->user_agent_line.len >= NDPI_STATICSTRING_LEN("YahooMobileMessenger/")
			     && memcmp(packet_hdr(user_agent_line), "YahooMobileMessenger/", NDPI_STATICSTRING_LEN("YahooMobileMessenger/")) == 0)
			    || (packet->user_agent_line.len >= 15 && (memcmp(packet_hdr(user_agent_line), "Y!%20Messenger/", 15) == 0))) {
				NDPI_LOG(NDPI_PROTOCOL_YAHOO, ndpi_struct, NDPI_LOG_DEBUG, "found YAHOO(Mobile)");
				ndpi_int_yahoo_add_connection(ndpi_struct, flow);
				return;
			}
			if (packet->host_line.offs != 0xffff && packet->host_line.len >= NDPI_STATICSTRING_LEN("msg.yahoo.com") &&
			    memcmp(&packet_hdr(host_line)[packet->host_line.len - NDPI_STATICSTRING_LEN("msg.yahoo.com")], "msg.yahoo.com", NDPI_STATICSTRING_LEN("msg.yahoo.com")) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_YAHOO, ndpi_struct, NDPI_LOG_DEBUG, "found YAHOO");
				ndpi_int_yahoo_add_connection(ndpi_struct, flow);
				return;
			}
	...
	if ((src != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_PROTOCOL_YAHOO) != 0)
		...
		if (packet->payload_packet_len == 8
		    && (memcmp(packet->payload, "<SNDIMG>", 8) == 0 || memcmp(packet->payload, "<REQIMG>", 8) == 0 || memcmp(packet->payload, "<RVWCFG>", 8) == 0 || memcmp(packet->payload, "<RUPCFG>", 8) == 0)) {
...
#endif
	{
		...
		if (flow->l4.tcp.yahoo_http_proxy_stage == 0) {
			NDPI_LOG(NDPI_PROTOCOL_YAHOO, ndpi_struct, NDPI_LOG_DEBUG, "YAHOO maybe HTTP proxy packet 1 => need next packet\n");
		...
		if (flow->l4.tcp.yahoo_http_proxy_stage == 2 - packet->packet_direction) {
			...
			if (packet->parsed_lines >= 9) {
				...
				if (packet->line[4].len >= 9 && memcmp(packet_line(4), "<Session ", 9) == 0 && packet->line[8].len >= 6 && memcmp(packet_line(8), "<Ymsg ", 6) == 0) {
