...
#define REFRESH_RATE 1000000  /* delay in us between updates */
...
int abg_chans[] =		/* 8 FCC channel: 52, 56, 60, 64, 36, 40, 44, 48 */
{ 
	4500,
	4505,
	4510,
...
int a_chans[] =		/* 8 FCC channel: 52, 56, 60, 64, 36, 40, 44, 48 */
{ 
4900,
4905,
4910,
...
void dump_print( int ws_row, int ws_col , FILE *out)
{
    time_t tt;
    struct tm *lt;
    int nlines, i, n;
    ...
    while( ap_cur != NULL )
    {
        ...
        fprintf( out, "AP %02X:%02X:%02X:%02X:%02X:%02X",
                ap_cur->bssid[0], ap_cur->bssid[1],
                ap_cur->bssid[2], ap_cur->bssid[3],
                ap_cur->bssid[4], ap_cur->bssid[5] );

        fprintf( out, "  %3d %8ld %8ld",
                 ap_cur->avg_power,
                 ap_cur->nb_bcn,
                 ap_cur->nb_data );
        ...
        {
            case  0: fprintf( out, "OPN " ); break;
            case  1: fprintf( out, "WEP?" ); break;
            case  2: fprintf( out, "WEP " ); break;
            case  3: fprintf( out, "WPA " ); break;
    ...
    while( ap_cur != NULL )
    {
        ...
        while( st_cur != NULL )
        {
            ...
            if( ! memcmp( ap_cur->bssid, BROADCAST_ADDR, 6 ) )
                fprintf( out, "STA (not associated) " );
            else
                fprintf( out, "STA %02X:%02X:%02X:%02X:%02X:%02X",
                        ap_cur->bssid[0], ap_cur->bssid[1],
                        ap_cur->bssid[2], ap_cur->bssid[3],
                        ap_cur->bssid[4], ap_cur->bssid[5] );
            ...
            fprintf( out, "  %3d", st_cur->power );
            fprintf( out, " %8ld", st_cur->nb_pkt );
...
//fprintf(stderr,"set channel %d\n",channel);
    ...
    memset( &wrq, 0, sizeof( struct iwreq ) );
    strncpy( wrq.ifr_name, interface, IFNAMSIZ );
    wrq.u.freq.m = (double) channel * 100000;
...
//    fprintf( stderr, "\33[?25l\33[2J\n" );
    ...
    int chidx=0;
    int lastchannel=0;
    while( lastchannel!=-1 && chidx!=-1 && lastchannel<=chidx )
    {
    lastchannel=chidx;
    chidx=G.channel;
//    fprintf(stderr,"channel %d %d\n",chidx,lastchannel);
	    ...
	    FILE *out = fopen("/tmp/scan.txt","wb");
    //        fprintf( stderr, "\33[1;1H" );
            dump_print( ws.ws_row, ws.ws_col ,out );
    //        fprintf( stderr, "\33[J" );
            fclose(out);
        }

        FD_ISSET( fd_raw, &rfds );
