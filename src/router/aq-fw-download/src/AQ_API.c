...
#define AQ_API_DECLARE_UP_BUSY_VARS                              \
	AQ_API_Variable(AQ_GlobalGeneralStatus) uint16_t uPbusy; \
	clock_t startTime;                                       \
	AQ_boolean uPBusyTimeoutOccurred = False;

#define AQ_API_CHECK_UP_NOT_BUSY                                                                                      \
	AQ_API_Wait(1, port);                                                                                         \
	startTime = clock();                                                                                          \
	do {                                                                                                          \
...
const uint16_t AQ_CRC16Table[256] = {
	...
	unsigned int gangLoadAddress)
{
   ...
   * until AQ_API_DisableGangLoadMode is called. */
	if (AQ_DEVICE_APPIA == port->device) {
		...
		AQ_API_Set_DeviceRestricted(APPIA, port->PHY_ID, AQ_GlobalGeneralProvisioning, gangLoadMdioAddress,
					    gangLoadAddress);
	} else if (AQ_DEVICE_HHD == port->device) {
		/* REGDOC: Read-Modify-Write bitfield (HHD: 1E.C447.4:0) */
		AQ_API_Set_DeviceRestricted(HHD, port->PHY_ID, AQ_GlobalGeneralProvisioning, mdioBroadcastAddressConfiguration,
					    gangLoadAddress);
		/* REGDOC: Read-Modify-Write bitfield (HHD: 1E.C441.E) */
		AQ_API_Set_DeviceRestricted(HHD, port->PHY_ID, AQ_GlobalGeneralProvisioning, mdioBroadcastModeEnable, 1);
	}
	...
	uint16_t origVal_GGP1)
{
   ...
   * reset to the value indicated by the pins. */
	...
	if (AQ_DEVICE_APPIA == port->device) {
		...
		AQ_API_AssignBitfieldOfLocalStruct_DeviceRestricted(APPIA, AQ_GlobalGeneralProvisioning, globalGeneralProvisioning,
								    mdioAddressReset, 1);
	} else if (AQ_DEVICE_HHD == port->device) {
		/* REGDOC: Assign to local representation of bitfield (HHD: 1E.C441.E) */
		AQ_API_AssignBitfieldOfLocalStruct_DeviceRestricted(HHD, AQ_GlobalGeneralProvisioning, globalGeneralProvisioning,
								    mdioBroadcastModeEnable, 0);
	}
	/* REGDOC: Write register (HHD/APPIA: 1E.C440 + 1) */
	AQ_API_WriteRegister(port->PHY_ID, AQ_GlobalGeneralProvisioning, 1, AQ_API_WordOfLocalStruct(globalGeneralProvisioning, 1));

	/* The PHY has now exited gang-load mode. */
	return;
}

AQ_Retcode AQ_API_WriteBootLoadImageWithProvTable(AQ_API_Port **ports, unsigned int numPorts, unsigned int *provisioningAddresses,
						  AQ_Retcode *resultCodes, uint32_t *imageSizePointer, uint8_t *image,
						  uint8_t gangload_MDIO_address, AQ_API_Port *gangloadPort,
						  uint32_t *provTableSizePointer, uint8_t *provTableImage)
{
	/*------------------------------------- NOTE!!!!!!!!!! ----------------------------------------------------------*/
	...
	if (provTableSizePointer != NULL && provTableImage != NULL) {
		...
		provTableFileCRC = provTableImage[provTableImageSize + 1] << 8 | provTableImage[provTableImageSize];
   ...
   * gangload mode. */
	...
	AQ_API_WriteRegister(gangloadPort->PHY_ID, AQ_GlobalControl, 1, AQ_API_WordOfLocalStruct(globalControl, 1));
	...
	if (provTableSizePointer != NULL && provTableImage != NULL) {
		...
		terminatorPtr = primaryDramPtr + ((image[primaryDramPtr + AQ_PHY_IMAGE_PROVTABLE_TERM_OFFSET + 1] << 8) |
     ...
     * space. */
		if (terminatorPtr - (primaryDramPtr + AQ_PHY_IMAGE_PROVTABLE_OFFSET) + provTableImageSize >
		    AQ_PHY_IMAGE_PROVTABLE_MAXSIZE) {
			for (j = 0; j < numPorts; j++) {
	...
	AQ_API_AssignBitfieldOfLocalStruct(AQ_GlobalMailboxInterface, globalMailboxInterface, upMailboxAddressLSW,
					   (AQ_IRAM_BASE_ADDRESS & 0xFFFF) >> 2);
	...
	for (j = 0; j < numPorts; j++) {
		...
		/* REGDOC: Write register (HHD/APPIA: 1E.C000 + 1) */
		AQ_API_WriteRegister(ports[j]->PHY_ID, AQ_GlobalControl, 1, AQ_API_WordOfLocalStruct(globalControl, 1));
		/* REGDOC: Assign to local representation of bitfield (HHD/APPIA: 1E.C001.F) */
...
AQ_Retcode AQ_API_WriteBootLoadImage(AQ_API_Port **ports, unsigned int numPorts, unsigned int *provisioningAddresses,
				     AQ_Retcode *resultCodes, uint32_t *imageSizePointer, uint8_t *image,
				     uint8_t gangload_MDIO_address, AQ_API_Port *gangloadPort)
{
	return AQ_API_WriteBootLoadImageWithProvTable(ports, numPorts, provisioningAddresses, resultCodes, imageSizePointer, image,
	...
	AQ_API_Port *port)
{
	...
	AQ_API_Variable(AQ_GlobalNvrProvisioning) AQ_API_Variable(AQ_GlobalReservedProvisioning)
