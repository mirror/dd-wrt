...
#define AQ_API_ReadRegister_Devs_APPIA(id, reg, wd)                                                     \
	((port->device == AQ_DEVICE_APPIA) ?                                                            \
		 AQ_API_MDIO_Read(id, reg##_APPIA_mmdAddress, (reg##_APPIA_baseRegisterAddress + wd)) : \
		 (0))

#define AQ_API_ReadRegister_Devs_HHD(id, reg, wd) \
	((port->device == AQ_DEVICE_HHD) ? AQ_API_MDIO_Read(id, reg##_HHD_mmdAddress, (reg##_HHD_baseRegisterAddress + wd)) : (0))

#define AQ_API_ReadRegister_Devs_APPIA_HHD(id, reg, wd)                                                          \
	((port->device == AQ_DEVICE_HHD) ?                                                                       \
		 AQ_API_MDIO_Read(id, reg##_HHD_mmdAddress, (reg##_HHD_baseRegisterAddress + wd)) :              \
		 ((port->device == AQ_DEVICE_APPIA) ?                                                            \
			  AQ_API_MDIO_Read(id, reg##_APPIA_mmdAddress, (reg##_APPIA_baseRegisterAddress + wd)) : \
			  (0)))
...
#define AQ_API_WriteRegister_Devs_APPIA(id, reg, wd, value)                                                     \
	((port->device == AQ_DEVICE_APPIA) ?                                                                    \
		 AQ_API_MDIO_Write(id, reg##_APPIA_mmdAddress, (reg##_APPIA_baseRegisterAddress + wd), value) : \
		 ((void)0))

#define AQ_API_WriteRegister_Devs_HHD(id, reg, wd, value)                                                   \
	((port->device == AQ_DEVICE_HHD) ?                                                                  \
		 AQ_API_MDIO_Write(id, reg##_HHD_mmdAddress, (reg##_HHD_baseRegisterAddress + wd), value) : \
		 ((void)0))
...
#define AQ_API_BlockReadRegister_Devs_APPIA(id, reg, wd)                                                     \
	((port->device == AQ_DEVICE_APPIA) ?                                                                 \
		 AQ_API_MDIO_BlockRead(id, reg##_APPIA_mmdAddress, (reg##_APPIA_baseRegisterAddress + wd)) : \
		 ((void)0))
...
#define AQ_API_BlockReadRegister_Devs_APPIA_HHD(id, reg, wd)                                                          \
	((port->device == AQ_DEVICE_HHD) ?                                                                            \
		 AQ_API_MDIO_BlockRead(id, reg##_HHD_mmdAddress, (reg##_HHD_baseRegisterAddress + wd)) :              \
		 ((port->device == AQ_DEVICE_APPIA) ?                                                                 \
			  AQ_API_MDIO_BlockRead(id, reg##_APPIA_mmdAddress, (reg##_APPIA_baseRegisterAddress + wd)) : \
			  ((void)0)))
...
#define AQ_API_BlockWriteRegister_DeviceRestricted(devices, id, reg, wd, value) \
	AQ_API_BlockWriteRegister_Devs_##devices(id, reg, wd, value)

#define AQ_API_BlockWriteRegister_Devs_APPIA(id, reg, wd, value)                                                     \
	((port->device == AQ_DEVICE_APPIA) ?                                                                         \
		 AQ_API_MDIO_BlockWrite(id, reg##_APPIA_mmdAddress, (reg##_APPIA_baseRegisterAddress + wd), value) : \
		 ((void)0))

#define AQ_API_BlockWriteRegister_Devs_HHD(id, reg, wd, value)                                                   \
	((port->device == AQ_DEVICE_HHD) ?                                                                       \
		 AQ_API_MDIO_BlockWrite(id, reg##_HHD_mmdAddress, (reg##_HHD_baseRegisterAddress + wd), value) : \
		 ((void)0))
...
#define AQ_API_Variable_Devs_APPIA(reg)                            \
	uint8_t _local##reg##_space[sizeof(reg##_BiggestVersion)]; \
	reg##_APPIA *_local##reg##_APPIA = (reg##_APPIA *)_local##reg##_space;

#define AQ_API_Variable_Devs_HHD(reg)                              \
	uint8_t _local##reg##_space[sizeof(reg##_BiggestVersion)]; \
	reg##_HHD *_local##reg##_HHD = (reg##_HHD *)_local##reg##_space;

#define AQ_API_Variable_Devs_APPIA_HHD(reg)                                    \
	uint8_t _local##reg##_space[sizeof(reg##_BiggestVersion)];             \
	reg##_APPIA *_local##reg##_APPIA = (reg##_APPIA *)_local##reg##_space; \
	reg##_HHD *_local##reg##_HHD = (reg##_HHD *)_local##reg##_space;
...
#define AQ_API_DeclareLocalStruct_Devs_APPIA(reg, localvar)     \
	uint8_t localvar##_space[sizeof(reg##_BiggestVersion)]; \
	reg##_APPIA *localvar##_APPIA = (reg##_APPIA *)localvar##_space;

#define AQ_API_DeclareLocalStruct_Devs_HHD(reg, localvar)       \
	uint8_t localvar##_space[sizeof(reg##_BiggestVersion)]; \
	reg##_HHD *localvar##_HHD = (reg##_HHD *)localvar##_space;

#define AQ_API_DeclareLocalStruct_Devs_APPIA_HHD(reg, localvar)          \
	uint8_t localvar##_space[sizeof(reg##_BiggestVersion)];          \
	reg##_APPIA *localvar##_APPIA = (reg##_APPIA *)localvar##_space; \
	reg##_HHD *localvar##_HHD = (reg##_HHD *)localvar##_space;
...
#define AQ_API_Set_Devs_APPIA(id, reg, field, value)                                                              \
	{                                                                                                         \
		...
		case AQ_DEVICE_APPIA:                                                                             \
			_local##reg##_APPIA->word_##reg##_APPIA_##field =                                         \
				AQ_API_ReadRegister_Devs_APPIA(id, reg, reg##_APPIA_##field);                     \
			if (_local##reg##_APPIA->bits_##reg##_APPIA_##field.field != value) {                     \
				_local##reg##_APPIA->bits_##reg##_APPIA_##field.field = value;                    \
				AQ_API_WriteRegister_Devs_APPIA(id, reg, reg##_APPIA_##field,                     \
								_local##reg##_APPIA->word_##reg##_APPIA_##field); \
			}                                                                                         \
			break;                                                                                    \
		default:                                                                                          \
	...
	{                                                                                                                       \
		...
		case AQ_DEVICE_HHD:                                                                                             \
			...
			if (_local##reg##_HHD->bits_##reg##_HHD_##field.field != value) {                                       \
				...
				AQ_API_WriteRegister_Devs_HHD(id, reg, reg##_HHD_##field,                                       \
							      _local##reg##_HHD->word_##reg##_HHD_##field);                     \
			}                                                                                                       \
			break;                                                                                                  \
		default:                                                                                                        \
			break;                                                                                                  \
...
#define AQ_API_Set_Devs_APPIA_HHD(id, reg, field, value)                                                              \
	{                                                                                                             \
		...
		case AQ_DEVICE_APPIA:                                                                                 \
			_local##reg##_APPIA->word_##reg##_APPIA_##field =                                             \
				AQ_API_ReadRegister_Devs_APPIA_HHD(id, reg, reg##_APPIA_##field);                     \
			if (_local##reg##_APPIA->bits_##reg##_APPIA_##field.field != value) {                         \
				_local##reg##_APPIA->bits_##reg##_APPIA_##field.field = value;                        \
				AQ_API_WriteRegister_Devs_APPIA_HHD(id, reg, reg##_APPIA_##field,                     \
								    _local##reg##_APPIA->word_##reg##_APPIA_##field); \
			}                                                                                             \
			break;                                                                                        \
		case AQ_DEVICE_HHD:                                                                                   \
			_local##reg##_HHD->word_##reg##_HHD_##field =                                                 \
				AQ_API_ReadRegister_Devs_APPIA_HHD(id, reg, reg##_HHD_##field);                       \
			if (_local##reg##_HHD->bits_##reg##_HHD_##field.field != value) {                             \
				_local##reg##_HHD->bits_##reg##_HHD_##field.field = value;                            \
				AQ_API_WriteRegister_Devs_APPIA_HHD(id, reg, reg##_HHD_##field,                       \
								    _local##reg##_HHD->word_##reg##_HHD_##field);     \
			}                                                                                             \
			break;                                                                                        \
		default:                                                                                              \
...
#define AQ_API_BitfieldOfLocalStruct(reg, localvar, field) \
	AQ_API_BitfieldOfLocalStruct_DeviceRestricted(APPIA_HHD, reg, localvar, field)

#define AQ_API_BitfieldOfLocalStruct_DeviceRestricted(devices, reg, localvar, field) \
	AQ_API_BitfieldOfLocalStruct_Devs_##devices(reg, localvar, field)

#define AQ_API_BitfieldOfLocalStruct_Devs_APPIA(reg, localvar, field) \
	((port->device == AQ_DEVICE_APPIA) ? ((localvar##_APPIA)->bits_##reg##_APPIA##_##field.field) : (0))

#define AQ_API_BitfieldOfLocalStruct_Devs_HHD(reg, localvar, field) \
	((port->device == AQ_DEVICE_HHD) ? ((localvar##_HHD)->bits_##reg##_HHD##_##field.field) : (0))

#define AQ_API_BitfieldOfLocalStruct_Devs_APPIA_HHD(reg, localvar, field) \
	((port->device == AQ_DEVICE_HHD) ?                                \
		 ((localvar##_HHD)->bits_##reg##_HHD##_##field.field) :   \
		 ((port->device == AQ_DEVICE_APPIA) ? ((localvar##_APPIA)->bits_##reg##_APPIA##_##field.field) : (0)))

#define AQ_API_BitfieldOfLocalStruct_Devs_HHD_APPIA(reg, localvar, field) \
	AQ_API_BitfieldOfLocalStruct_Devs_APPIA_HHD(reg, localvar, field)

#define AQ_API_AssignBitfieldOfLocalStruct(reg, localvar, field, value) \
	AQ_API_AssignBitfieldOfLocalStruct_DeviceRestricted(APPIA_HHD, reg, localvar, field, value)

#define AQ_API_AssignBitfieldOfLocalStruct_DeviceRestricted(devices, reg, localvar, field, value) \
	AQ_API_AssignBitfieldOfLocalStruct_Devs_##devices(reg, localvar, field, value)

#define AQ_API_AssignBitfieldOfLocalStruct_Devs_APPIA(reg, localvar, field, value) \
	((port->device == AQ_DEVICE_APPIA) ? ((localvar##_APPIA)->bits_##reg##_APPIA##_##field.field = value) : (0))

#define AQ_API_AssignBitfieldOfLocalStruct_Devs_HHD(reg, localvar, field, value) \
	((port->device == AQ_DEVICE_HHD) ? ((localvar##_HHD)->bits_##reg##_HHD##_##field.field = value) : (0))

#define AQ_API_AssignBitfieldOfLocalStruct_Devs_APPIA_HHD(reg, localvar, field, value) \
	((port->device == AQ_DEVICE_HHD) ?                                             \
		 ((localvar##_HHD)->bits_##reg##_HHD##_##field.field = value) :        \
		 ((port->device == AQ_DEVICE_APPIA) ? ((localvar##_APPIA)->bits_##reg##_APPIA##_##field.field = value) : (0)))

#define AQ_API_AssignBitfieldOfLocalStruct_Devs_HHD_APPIA(reg, localvar, field, value) \
	AQ_API_AssignBitfieldOfLocalStruct_Devs_APPIA_HHD(reg, localvar, field, value)
...
#define AQ_API_WordOfLocalStruct_Devs_APPIA(localvar, wd) \
	((port->device == AQ_DEVICE_APPIA) ? ((localvar##_APPIA)->u##wd.word_##wd) : (0))

#define AQ_API_WordOfLocalStruct_Devs_HHD(localvar, wd) \
	((port->device == AQ_DEVICE_HHD) ? ((localvar##_HHD)->u##wd.word_##wd) : (0))
...
#define AQ_API_AssignWordOfLocalStruct(localvar, wd, value) \
	AQ_API_AssignWordOfLocalStruct_DeviceRestricted(APPIA_HHD, localvar, wd, value)

#define AQ_API_AssignWordOfLocalStruct_DeviceRestricted(devices, localvar, wd, value) \
	AQ_API_AssignWordOfLocalStruct_Devs_##devices(localvar, wd, value)

#define AQ_API_AssignWordOfLocalStruct_Devs_APPIA(localvar, wd, value) \
	((port->device == AQ_DEVICE_APPIA) ? ((localvar##_APPIA)->u##wd.word_##wd = value) : (0))

#define AQ_API_AssignWordOfLocalStruct_Devs_HHD(localvar, wd, value) \
	((port->device == AQ_DEVICE_HHD) ? ((localvar##_HHD)->u##wd.word_##wd = value) : (0))

#define AQ_API_AssignWordOfLocalStruct_Devs_APPIA_HHD(localvar, wd, value) \
	((port->device == AQ_DEVICE_HHD) ?                                 \
		 ((localvar##_HHD)->u##wd.word_##wd = value) :             \
		 ((port->device == AQ_DEVICE_APPIA) ? ((localvar##_APPIA)->u##wd.word_##wd = value) : (0)))

#define AQ_API_AssignWordOfLocalStruct_Devs_HHD_APPIA(localvar, wd, value) \
	AQ_API_AssignWordOfLocalStruct_Devs_APPIA_HHD(localvar, wd, value)
