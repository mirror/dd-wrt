...
*/
Rational parse_rational_time( const char * const arg, const char * const pn,
  ...
  show_option_error( arg, "Bad value in time interval argument of", pn );
  std::exit( 1 );
  }
...
bool about_to_copy( const Rescuebook & rescuebook, const char * const iname,
                    const char * const oname, const long long insize,
                    const int cluster, const int ides, const int o_direct_out,
                    const int o_trunc, const bool ask )
  {
  if( ask || verbosity >= 0 )
    std::printf( "%s %s\n", Program_name, PROGVERSION );
  if( ask || verbosity >= 1 )
  ...
  if( ask )
    ...
    if( !safe_fflush( stdout ) || std::tolower( std::fgetc( stdin ) ) != 'y' )
      return false;
    }
  if( ask || verbosity >= 1 ) std::fputc( '\n', stdout );
  return true;
  }
...
int do_rescue( const long long offset, Domain & domain,
  ...
  if( check_input_size )
    ...
    if( !rescuebook.mapfile_exists() || insize <= 0 ||
      ...
      { show_error( "Can't check input file size.\n          Mapfile is "
                    "unfinished or missing, or size is invalid." ); return 1; }
  ...
  if( rescuebook.domain().empty() )
    ...
    if( rescuebook.complete_only && !rescuebook.mapfile_exists() )
      { show_error( "Nothing to complete; mapfile is missing or empty.", 0,
                    true ); return 1; }
    return empty_domain();
    }
  if( o_trunc && !rescuebook.blank() )
    { show_error( "Outfile truncation and mapfile input are incompatible.", 0,
                  true ); return 1; }
...
void parse_cpass( const char * const arg, const char * const pn,
                  Rb_options & rb_opts )
  {
  const char * p = arg;
  ...
  show_option_error( arg, "Invalid pass or range of passes in", pn );
  std::exit( 1 );
  }
...
void parse_errno_vector( const char * const arg, const char * const pn,
                         Rb_options & rb_opts )
  {
  const char * p = arg;
  while( std::isdigit( *(const unsigned char *)p ) )
    {
    const char * tail;
    rb_opts.errno_vector.push_back( getnum( p, pn, 0, 1, INT_MAX, &tail ) );
    if( *tail == 0 ) return;
    if( *tail == ',' ) p = tail + 1; else break;
    }
  show_option_error( arg, "Invalid errno value in", pn );
  std::exit( 1 );
  }


void parse_pause_on_error( const char * const arg, const char * const pn,
                           Rb_options & rb_opts )
  {
  rb_opts.simulated_poe = *arg == 's';
  if( rb_opts.simulated_poe )
    rb_opts.pause_on_error = parse_rational_time( arg + 1, pn );
  else
    rb_opts.pause_on_error = parse_time_interval( arg, pn );
  }
...
void show_final_msg( const char * const msg, const int errcode )
  ...
  if( errcode > 0 )
    std::fprintf( stderr, "%s: %s: %s (errno=%u)\n", program_name, msg,
...
int main( const int argc, const char * const argv[] )
  ...
  for( ; argind < parser.arguments(); ++argind )
    ...
    switch( code )
      ...
      case 'e': rb_opts.new_bad_areas_only = *arg == '+';
