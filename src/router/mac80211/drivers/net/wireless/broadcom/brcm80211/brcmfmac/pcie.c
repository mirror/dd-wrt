...
static const struct brcmf_firmware_mapping brcmf_pcie_fwnames[] = {
	...
	BRCMF_FW_ENTRY(BRCM_CC_43666_CHIP_ID, 0xFFFFFFF4, 4366C),
	BRCMF_FW_ENTRY(BRCM_CC_4371_CHIP_ID, 0xFFFFFFFF, 4371),
};

#define BRCMF_PCIE_FW_UP_TIMEOUT		10000 /* msec */
...
static void brcmf_pcie_bus_console_init(struct brcmf_pciedev_info *devinfo)
{
	...
	printk(KERN_INFO "Console: base %x, buf %x, size %d\n",
...
//	printk(KERN_INFO "newidx %d, read_idx %d bufsize %d\n",newidx, console->read_idx, console->bufsize);
			  ...
			  u32 data_len)
{
	...
	printk(KERN_INFO "Found ramsize info in FW, adjusting to 0x%x\n",
		  newsize);
					...
					u32 nvram_len)
{
	...
	printk(KERN_INFO "Halt ARM.\n");
	err = brcmf_pcie_enter_download_state(devinfo);
	if (err)
		return err;

	printk(KERN_INFO "Download FW %s\n", devinfo->fw_name);
	...
	if (nvram) {
		printk(KERN_INFO "Download NVRAM %s\n", devinfo->nvram_name);
		address = devinfo->ci->rambase + devinfo->ci->ramsize -
			  nvram_len;
		brcmf_pcie_copy_mem_todev(devinfo, address, nvram, nvram_len);
		brcmf_fw_nvram_free(nvram);
	} else {
		printk(KERN_INFO "No matching NVRAM file found %s\n",
			  devinfo->nvram_name);
	}
	...
	sharedram_addr_written = brcmf_pcie_read_ram32(devinfo,
						       devinfo->ci->ramsize -
						       4);
	printk(KERN_INFO "Bring ARM in running state\n");
	err = brcmf_pcie_exit_download_state(devinfo, resetintr);
	if (err)
		return err;

	printk(KERN_INFO "Wait for FW init\n");
	...
	if (sharedram_addr == sharedram_addr_written) {
		printk(KERN_INFO "FW failed to initialize\n");
		return -ENODEV;
	}
	...
	printk(KERN_INFO "Shared RAM addr: 0x%08x\n", sharedram_addr);
...
static const struct pci_device_id brcmf_pcie_devid_table[] = {
	...
	BRCMF_PCIE_DEVICE(BRCM_PCIE_43666_DEVICE_ID),
