 ...
 *
 * $Id: d11.h 530150 2015-01-29 08:43:40Z $
 */
...
typedef volatile struct _d11regs {
	...
	uint32	gptimer_psmx;		/* 0x1b0 */
	uint32	maccontrol_psmx;	/* 0x1b4 */
	uint32	maccontrol1_psmx;	/* 0x1b8 */
	uint32	maccommand_psmx;	/* 0x1bc */
	uint32	macintstatus_psmx;	/* 0x1c0 */
	uint32	macintmask_psmx;	/* 0x1c4 */
	uint32	altmacintstatus_psmx;	/* 0x1c8 */
	uint32	altmacintmask_psmx;	/* 0x1cc */
	uint32	psmdebug_psmx;		/* 0x1d0 */
	uint32	phydebug_psmx;		/* 0x1d4 */
...
#define	OBJADDR_PSMXUCM_SEL	0x00080000
#define	OBJADDR_PSMXSHM_SEL	0x00090000
#define	OBJADDR_PSMXSCR_SEL	0x000a0000
#define	OBJADDR_PSMXIHR_SEL	0x000b0000
...
#define	WSEC_MAX_TKMIC_ENGINE_KEYS	12 /* 8 + 4 default - 2 mic keys 8 bytes each */

/* max keys in M_WAPIMICKEYS_BLK - 64 * sizeof(uint16) */
#define	WSEC_MAX_SMS4MIC_ENGINE_KEYS	8 /* 4 + 4 default  - 16 bytes each */
...
#define	T_P2P_NULL_TPL_BASE	(0x334 * 2)
...
#define M_RSSI_GAINVALRESP 		(76*2)
...
BWL_PRE_PACKED_STRUCT struct d11rxhdrshort {
	uint16 RxFrameSize;	/**< Actual byte length of the frame data received */

	/* Microcode writes entire structure as a sequence of 16-bit words in little-endian.
	 * The driver converts the entire buffer to host endian in one shot.
	 * The following word is sliced into two fields in the driver. To keep these fields
	 * in the proper order after the endian conversion, reverse the order depending on
	 * the endianness of the processor.
	 */
#ifdef IL_BIGENDIAN
	uint8 fifo;        /**< rx fifo number */
	uint8 dma_flags;   /**< bit 0 indicates whether rx status is short or long. 1 == short. */
#else
	uint8 dma_flags;
	uint8 fifo;
#endif /* IL_BIGENDIAN */
...
#define SRXHDR_LEN  12	/**< sizeof d11rxhdrshort_t */
#define	RXHDR_LEN   28	/**< sizeof d11rxhdr_t */
...
#define	WRXHDR_LEN		39 	/**< sizeof wlc_d11rxhdr_t */
...
#define M_PAPDOFF_MCS   (M_PSM_SOFT_REGS + (0x12 * 2))	/**< ucode WAR which disable PAPD in mcs */
...
/** BT RSSI in 5 dB step (1: -15, .. 15: -85 dBm) */
#define M_BTCX_RSSI			(120 * 2)
...
typedef enum
{
	BTC_FW_RX_REAGG_AFTER_SCO = 0,		/* Set to 1 to allow rx-agg to be re-enabled
						 *  after SCO or A2DP session completes
						 */
...
#define ADDR_BMP_BSSID		(1 << 2)	/**< BSSID. For corerev >= 40, BSSID-only */
...
#define M_COREMASK_BTRESP_PRE40	((D11_PRE40_M_COREMASK_BLK + 5) * 2)
#define TXCOREMASK		0x0F
#define SPATIAL_SHIFT		8
#define MAX_COREMASK_BLK	5

/*  bit fields in M_COREMASK_BPHY  */
...
/** PMQ Interrupt Threshold for IBSS */
#define SHM_PMQ_INTR_THRESH		(0x2A)
...
/* Power-save related */
#define M_AID_NBIT	(0x062 * 2)	/**< The station's AID bit position in AP's TIM bitmap */
...
#define M_WOWL_TMR_L        (0x09d * 2)		/**< lower TSF LSB to be passed to ucode */
#define M_WOWL_TMR_ML       (0x09e * 2)		/**< Lower TSF MSB to be passed to ucode */
...
#define M_TXFL_BMAP                (0x3f*2) /* selectively flushing of transmit fifo's */
...
#define M_LTECX_BLK_PTR				M_BTCX_BLK_PTR
...
/** Beacon trim feature definition */
#define M_BCNTRIM_BLK_PTR		(M_PSM_SOFT_REGS_EXT + 0x1e)
