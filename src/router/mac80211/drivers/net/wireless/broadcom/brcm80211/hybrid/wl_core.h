...
#define WL_PM_WAKE_LOCK_TIMEOUT(cfg, val) DHD_PM_WAKE_LOCK_TIMEOUT(cfg->pub, val)
#define WL_PM_WAKE_UNLOCK(cfg) DHD_PM_WAKE_UNLOCK(cfg->pub)
#define WL_OS_SCAN_WAKE_LOCK_TIMEOUT(cfg, val) DHD_OS_SCAN_WAKE_LOCK_TIMEOUT(cfg->pub, val)
#define WL_OS_SCAN_WAKE_UNLOCK(cfg) DHD_OS_SCAN_WAKE_UNLOCK(cfg->pub)
#define WL_DISABLE_RUNTIME_PM(cfg) DHD_DISABLE_RUNTIME_PM(cfg->pub)
#define WL_ENABLE_RUNTIME_PM(cfg) DHD_ENABLE_RUNTIME_PM(cfg->pub)
#define WL_EVENT_WAKE_LOCK(cfg) DHD_EVENT_WAKE_LOCK(cfg->pub)
#define WL_EVENT_WAKE_UNLOCK(cfg) DHD_EVENT_WAKE_UNLOCK(cfg->pub)
#define WL_OS_WAKE_LOCK_TIMEOUT(cfg) DHD_OS_WAKE_LOCK_TIMEOUT(cfg->pub)
#define WL_OS_WAKE_UNLOCK(cfg) DHD_OS_WAKE_UNLOCK(cfg->pub)
#define WL_OS_WAKE_LOCK_CTRL_TIMEOUT_ENABLE(cfg, val) \
			DHD_OS_WAKE_LOCK_CTRL_TIMEOUT_ENABLE(cfg->pub, val)
#define WL_OS_WAKE_LOCK_CTRL_TIMEOUT_CANCEL(cfg) \
		DHD_OS_WAKE_LOCK_CTRL_TIMEOUT_CANCEL(cfg->pub)
#endif /* MEDIA_CFG) || ROUTER_CFG */
#ifdef PKT_FILTER_SUPPORT
#define WL_ARP_FILTER_NUM              5
#endif /* PKT_FILTER_SUPPORT */
bool wl_is_associated(struct net_device *dev, int *retval);
int wl_register_interface(void *pub, int ifidx, struct net_device *dev, bool pre_locked);
