 ...
 *
 * $Id: wl_cfg80211.c 766802 2018-08-15 02:54:59Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
 ...
 */
static s32 __wl_cfg80211_up(struct net_device *ndev, struct bcm_cfg80211 *cfg);
...
s32
wl_get_vif_macaddress(struct bcm_cfg80211 *cfg, u16 wl_iftype, u8 *mac_addr)
{
	...
	struct vif_params *params)
{
	...
	if (wl_get_vif_macaddress(cfg, wl_iftype, mac_addr) != BCME_OK) {
		err = -EINVAL;
		goto fail;
	}
...
#if defined(BCMDONGLEHOST) && defined(OEM_ANDROID)
		WL_OS_SCAN_WAKE_LOCK_TIMEOUT(cfg,
			SCAN_WAKE_LOCK_TIMEOUT);
		WL_DISABLE_RUNTIME_PM(cfg);
#endif // endif
	}
...
#if defined(BCM4350_CHIP_ID) && defined(BCM4355_CHIP_ID) && defined(BCM4345_CHIP_ID)
	u32 chipnum;
...
#endif /* BCM4350_CHIP_ID && BCM4355_CHIP_ID && BCM4345_CHIP_ID */
...
static void wl_reset_drv_status(struct bcm_cfg80211 *cfg,
		struct net_device *ndev)
{
...
static bool wl_get_chan_isvht80(struct net_device *net)
{
	u32 chanspec = 0;
	bool isvht80 = 0;
...
static void wl_fils_add_hlp_container(struct bcm_cfg80211 *cfg, struct net_device *dev,
...
#endif // endif
{
	...
	WL_ERR(("SCB_AUTHORIZE mac_addr:"MACDBG" sta_flags_mask:0x%x "
...
static s32 __wl_update_wiphybands(struct bcm_cfg80211 *cfg, bool notify)
{
	...
	if (band_5ghz -> n_channels == 0)
		wl_cfg80211_deinit_5g_band(cfg);
	if (band_2ghz -> n_channels == 0)
		wl_cfg80211_deinit_2g_band(cfg);
...
s32 wl_cfg80211_up(struct net_device *net)
{
	...
	if (!(cfg->wlcore->op_mode & DHD_FLAG_HOSTAP_MODE)) {
		...
		err = wl_cfg80211_attach_post(net);
		if (unlikely(err)) {
			mutex_unlock(&cfg->usr_sync);
			return err;
		}
...
int wl_cfg80211_get_the_vif_name(struct net_device *ndev, char *name)
{
	struct bcm_cfg80211 *cfg = wl_get_cfg(ndev);
...
bool wl_cfg80211_support_sta_mode(struct bcm_cfg80211 *cfg)
{
...
static s32 wl_cfg80211_alloc_2g_channel_bands(struct wireless_dev *wdev)
{
	...
	u16 kflags = in_atomic() ? GFP_ATOMIC : GFP_KERNEL;
	wl_band_2ghz = kmalloc(sizeof(struct ieee80211_supported_band), kflags);
	if (!wl_band_2ghz) {
...
static s32 wl_cfg80211_alloc_5g_channel_bands(struct wireless_dev *wdev)
{
	...
	u16 kflags = in_atomic() ? GFP_ATOMIC : GFP_KERNEL;
	wl_band_5ghz = kmalloc(sizeof(struct ieee80211_supported_band), kflags);
	if (!wl_band_5ghz) {
...
extern s32 (*hook_wl_cfg80211_attach)(struct net_device *ndev, void *context);
extern void (*hook_wl_cfg80211_detach)(struct bcm_cfg80211 *cfg);

extern void (*hook_wl_cfg80211_event)(struct net_device *ndev, const wl_event_msg_t *e, void *data);
extern s32 (*hook_wl_cfg80211_up)(struct net_device *ndev);
extern s32 (*hook_wl_cfg80211_down)(struct net_device *ndev);

extern int __init wl_module_init(void);

static int __init wl_cfg80211_module_init(void)
{
	hook_wl_cfg80211_attach = &wl_cfg80211_attach;
	hook_wl_cfg80211_detach = &wl_cfg80211_detach;
	hook_wl_cfg80211_event = &wl_cfg80211_event;
	hook_wl_cfg80211_up = &wl_cfg80211_up;
	hook_wl_cfg80211_down = &wl_cfg80211_down;
	wl_module_init();
	return 0;
}
 ...
 *
 */
static void __exit wl_cfg80211_module_exit(void)
{

}

module_init(wl_cfg80211_module_init);
module_exit(wl_cfg80211_module_exit);
