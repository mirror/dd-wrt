					   ...
					   enum nl80211_iftype type)
{
	...
	if (type == NL80211_IFTYPE_AP &&
	    (brcmf_feat_is_enabled(ifp, BRCMF_FEAT_MBSS4) || brcmf_feat_is_enabled(ifp, BRCMF_FEAT_MBSS8) || brcmf_feat_is_enabled(ifp, BRCMF_FEAT_MBSS16))) {
		mbss = false;
...
//	band->ht_cap.cap |= WLAN_HT_CAP_SM_PS_DISABLED << IEEE80211_HT_CAP_SM_PS_SHIFT;
...
static void brcmf_update_vht_cap(struct brcmf_if *ifp, struct ieee80211_supported_band *band,
				 u32 bw_cap[2], u32 nchain, u32 txstreams,
				 u32 txbf_bfe_cap, u32 txbf_bfr_cap, bool qam256, int vhtmode)
{
	__le16 mcs_map;

	/* not allowed in 2.4G band */
	if ((band->band == NL80211_BAND_2GHZ && !qam256)) {
		band->vht_cap.vht_supported = false;
		band->vht_cap.cap = 0;
		brcmf_fil_iovar_int_set(ifp, "vhtmode", 0);
		return;
	} else 
		brcmf_fil_iovar_int_set(ifp, "vhtmode", vhtmode);
...
int brcmf_setup_wiphybands(struct brcmf_cfg80211_info *cfg, bool qam256)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_if *ifp = brcmf_get_ifp(drvr, 0);
	struct wiphy *wiphy = cfg_to_wiphy(cfg);
...
//		  bw_cap[NL80211_BAND_5GHZ]);
	brcmf_info("nmode=%d, vhtmode=%d, vhtmode_init=%d , bw_cap=(%d, %d)\n",
		  nmode, vhtmode, drvr->vhtmode, bw_cap[NL80211_BAND_2GHZ],
		  bw_cap[NL80211_BAND_5GHZ]);
	vhtmode = drvr->vhtmode;
	...
	for (i = 0; i < ARRAY_SIZE(wiphy->bands); i++) {
		...
		brcmf_update_vht_cap(ifp, band, bw_cap, nchain, txstreams,
				     txbf_bfe_cap, txbf_bfr_cap, qam256, vhtmode);
...
static int brcmf_setup_ifmodes(struct wiphy *wiphy, struct brcmf_if *ifp)
{
	...
	mon_flag = true; //brcmf_feat_is_enabled(ifp, BRCMF_FEAT_MONITOR_FLAG);
	...
	if (mbss) {
		...
		combo[c].num_different_channels = 1;
		combo[c].max_interfaces = maxbss + mon_flag;
		combo[c].n_limits = i;
...
//	if (!country_codes) {
//		printk(KERN_INFO "No country codes configured for device, use default\n");
//	}

	if ((alpha2[0] == ccreq->country_abbrev[0]) &&
	    (alpha2[1] == ccreq->country_abbrev[1])) {
//		printk(KERN_INFO "Country code already set\n");
		return -EAGAIN;
	}
					...
					struct regulatory_request *req)
{
	...
	printk(KERN_INFO "Current Country %s, ccode %s, rev %d\n", &ccreq.country_abbrev[0], &ccreq.ccode[0], ccreq.rev);
	err = brcmf_translate_country_code(ifp->drvr, req->alpha2, &ccreq);
	if (err)
		return;

	printk(KERN_INFO "New Country %s\n", &ccreq.country_abbrev[0]);
	err = brcmf_fil_iovar_data_set(ifp, "country", &ccreq, sizeof(ccreq));
	if (err) {
		bphy_err(drvr, "Firmware rejected country setting\n");
		return;
	}
	brcmf_setup_wiphybands(cfg, false);
}
...
#if 0
static void brcmf_reg_notifier(struct wiphy *wiphy,
			     struct regulatory_request *request)
{
struct brcmf_if *ifp = netdev_priv(ndev);
char *alpha2 = &request->alpha2[0];
char abbrev[4];
s32 err;
memcpy(abbrev, alpha2, 3);
abbrev[3] = 0;
err = brcmf_fil_cmd_data_set(ifp, BRCMF_C_SET_COUNTRY, abbrev, strlen(abbrev));
	if (err) {
						  ...
						  bool p2pdev_forced)
{
...
#if 0
	wiphy->reg_notifier = brcmf_reg_notifier;
#endif
	cfg->wiphy = wiphy;
	cfg->pub = drvr;
	cfg->num_softap = 0;
	...
	err = brcmf_setup_wiphybands(cfg, false);
	if (err) {
