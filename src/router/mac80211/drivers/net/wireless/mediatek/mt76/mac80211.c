...
{
	if (phy->cap.has_2ghz)
		mt76_init_stream_cap(phy, &phy->sband_2g.sband, turboqam);
	if (phy->cap.has_5ghz)
...
EXPORT_SYMBOL(mt76_init_sband_2g);
		      ...
		      struct ieee80211_rate *rates, int n_rates)
{
	...
	if (phy->dev->disable_2ghz)
	    phy->cap.has_2ghz = false;
	if (phy->dev->disable_5ghz)
	    phy->cap.has_5ghz = false;
	if (phy->cap.has_2ghz) {
		    ...
		    struct survey_info *survey)
{
	...
	int nstream = hweight8(phy->antenna_mask);
	...
	survey->filled = SURVEY_INFO_TIME | SURVEY_INFO_TIME_BUSY | SURVEY_INFO_NOISE_DBM;
