...
#include "../mac80211/tdma/pnwext.h"
#endif
...
u32 ieee80211_channel_to_freq_khz(int chan, enum nl80211_band band)
{
	...
	case NL80211_BAND_2GHZ:
		if (chan == 14)
			...
			return MHZ_TO_KHZ(2407);
		}
		else if (chan > 14 && chan < 27)
			return MHZ_TO_KHZ(2512 + ((chan-15)*20));
		else if (chan > 14 && chan < 65) {
			return MHZ_TO_KHZ(2414 + (chan * 5));
		}
...
int ieee80211_channel_to_closest_freq_khz(struct ieee80211_supported_band *sband,
					   int channel, int cfreq)
{
	return MHZ_TO_KHZ(ieee80211_channel_to_closest_freq(sband, channel, cfreq));
}
EXPORT_SYMBOL(ieee80211_channel_to_closest_freq_khz);

int ieee80211_channel_to_closest_freq(struct ieee80211_supported_band *sband,
					   int channel, int cfreq)
{
	...
	for (i = 0; i < sband->n_channels; i++) {
		...
		if (ieee80211_frequency_to_channel(chan->center_freq) != channel)
			continue;

		if (abs(freq - cfreq) > abs(chan->center_freq - cfreq))
			freq = chan->center_freq;
	}

	return freq;
}
EXPORT_SYMBOL(ieee80211_channel_to_closest_freq);
...
int ieee80211_freq_khz_to_channel(u32 freq)
{
	...
	// boes hack...
		...
		return (fcast - 2407) / 5 + 256;
	else if (fcast < 2484)
		return (fcast - 2407) / 5;
	else if (fcast < 2502 && fcast > 2484)
		return 14;
	else if (fcast < 2512 && fcast > 2484)
		return 15;
	else if (fcast > 2484 && fcast < 4000 )
		return (15 + ((fcast - 2512) / 20)) & 0xff;
	else if (fcast < 4990 && fcast > 4940)
		return ((fcast * 10) + (((fcast % 5) == 2) ? 5 : 0) - 49400) / 5;
	else if (fcast > 4800 && fcast < 5005)
		return (fcast - 4000) / 5;
	else if (fcast < 7000)
		return ((fcast - 5000) / 5) % 255;
	else if (fcast <= 45000) /* DMG band lower limit */
		return ((fcast - 5940) / 5) % 0xff;
	else if (fcast >= 58320 && fcast <= 70200)
		return (fcast - 56160) / 2160;
	else
		return 0;
}
				  ...
				  u8 data_offset, bool is_amsdu)
{
	...
	case cpu_to_le16(IEEE80211_FCTL_TODS | IEEE80211_FCTL_FROMDS):
		if (unlikely(iftype != NL80211_IFTYPE_WDS &&
			     ...
			     iftype != NL80211_IFTYPE_AP_VLAN &&
			     iftype != NL80211_IFTYPE_TDMA &&
			     iftype != NL80211_IFTYPE_STATION))
			return -1;
#ifdef CPTCFG_MAC80211_TDMA
		if (iftype == NL80211_IFTYPE_TDMA) {
		    hdrlen += tdma_hdr_len();
