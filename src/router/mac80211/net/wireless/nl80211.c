...
static int nl80211_set_key(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (key.def) {
		...
		if (err) {
			printk(KERN_WARNING "%s: failed %d (set_default_key)\n", __func__, err);
			goto out;
		}
		...
		if (err) {
			printk(KERN_WARNING "%s: failed %d (set_default_mgmt_key)\n", __func__, err);
			goto out; 
		}
				 ...
				 int attr, struct ieee80211_vht_operation *vht_operation)
{
	...
	case RATE_INFO_BW_160:
		if (vht_operation->chan_width == IEEE80211_VHT_CHANWIDTH_80P80MHZ)
...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[NL80211_ATTR_SCAN_FREQUENCIES]) {
		...
		nla_for_each_nested(attr, info->attrs[NL80211_ATTR_SCAN_FREQUENCIES], tmp) {
			...
			if (!chan) {
				continue; // ignore invalid channel
			}
