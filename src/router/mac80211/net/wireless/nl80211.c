...
static const struct nla_policy nl80211_policy[NUM_NL80211_ATTR] = {
	...
	[NL80211_ATTR_MTIKWDS] = { .type = NLA_U8 },
	...
	[NL80211_ATTR_TDMA_VERSION] = { .type = NLA_U8 },
	[NL80211_ATTR_TDMA_NODES] = { .type = NLA_U8 },
	[NL80211_ATTR_TDMA_BWMODE] = { .type = NLA_U8 },
	[NL80211_ATTR_TDMA_EXT_TSF] = { .type = NLA_BINARY,
				       .len = IEEE80211_MAX_DATA_LEN },
	[NL80211_ATTR_COMPR] = { .type = NLA_U8 },
	[NL80211_ATTR_COMPR_THRESHOLD] = { .type = NLA_U32 },
	[NL80211_ATTR_TDMA_SLOT_SIZE] = { .type = NLA_U8 },
	[NL80211_ATTR_TDMA_TX_RATIO] = { .type = NLA_U8 },
	[NL80211_ATTR_TDMA_RX_RATIO] = { .type = NLA_U8 },
	[NL80211_ATTR_TDMA_AGGREGATION] = { .type = NLA_U8 },
	[NL80211_ATTR_TDMA_GACK] = { .type = NLA_FLAG },
	[NL80211_ATTR_TDMA_REORDER] = { .type = NLA_FLAG },
	[NL80211_ATTR_TDMA_POLLING] = { .type = NLA_FLAG },
};
...
static int nl80211_get_interface(struct sk_buff *skb, struct genl_info *info)
{
	...
	msg = nlmsg_new(NLMSG_DEFAULT_SIZE + 4096, GFP_KERNEL);
	if (!msg)
		return -ENOMEM;
...
static int nl80211_set_interface(struct sk_buff *skb, struct genl_info *info)
{
	...
	u32 fsize = 512;
	u8 use_compr;
	char *comprstrings[] = {"Enable LZO", "Enable LZMA", "Enable LZ4", "Enable ZSTD" };
	...
	if (info->attrs[NL80211_ATTR_COMPR]) {
		params.set_compr = 1;
		use_compr = nla_get_u8(info->attrs[NL80211_ATTR_COMPR]);
		if (use_compr > (sizeof(comprstrings) + 1))
			printk(KERN_INFO "%s: Invalid Compression Type %d\n",dev->name, use_compr);
		if (info->attrs[NL80211_ATTR_COMPR_THRESHOLD])
		    fsize = nla_get_u32(info->attrs[NL80211_ATTR_COMPR_THRESHOLD]);
		if (dev->ieee80211_ptr->use_compr != use_compr) {	
			params.set_compr = 1;
			params.use_compr = use_compr;
			params.compr_threshold = fsize;	
			printk(KERN_INFO "%s: %s Compression (Threshold %d)\n", dev->name, use_compr ? comprstrings[use_compr - 1] : "Disable", fsize);
			dev->ieee80211_ptr->use_compr = params.use_compr;
			dev->ieee80211_ptr->compr_threshold = params.compr_threshold;
		}
	}
#endif
...
bool nl80211_put_sta_rate(struct sk_buff *msg, struct rate_info *info,
				 int attr, struct ieee80211_vht_operation *vht_operation)
{
	struct nlattr *rate;
	u32 bitrate;
	u16 bitrate_compat;
	...
	case RATE_INFO_BW_160:
		if (vht_operation && vht_operation->chan_width == IEEE80211_VHT_CHANWIDTH_80P80MHZ)
			rate_flg = NL80211_RATE_INFO_80P80_MHZ_WIDTH;
		else
...
#define PUT_SINFO_STRING(attr, memb) do {				\
	if (sinfo->filled & (1ULL << NL80211_STA_INFO_ ## attr) &&	\
	    nla_put_string(msg, NL80211_STA_INFO_ ## attr,		\
			     &sinfo->memb[0]))				\
		goto nla_put_failure;					\
	} while (0)
	...
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_TX_BITRATE)) {
		if (!nl80211_put_sta_rate(msg, &sinfo->txrate,
					  NL80211_STA_INFO_TX_BITRATE, &sinfo->vht_operation))
			goto nla_put_failure;
	}
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_RX_BITRATE)) {
		if (!nl80211_put_sta_rate(msg, &sinfo->rxrate,
					  NL80211_STA_INFO_RX_BITRATE, &sinfo->vht_operation))
			goto nla_put_failure;
	}
...
#if IS_ENABLED(CPTCFG_MAC80211_COMPRESS)
	PUT_SINFO(RX_COMPRESSED, rx_compressed, u32);
	PUT_SINFO(TX_COMPRESSED, tx_compressed, u32);
	PUT_SINFO_U64(TX_COMPRESSED_BYTES64, tx_compressed_bytes);
	PUT_SINFO_U64(RX_COMPRESSED_BYTES64, rx_compressed_bytes);
#endif
	PUT_SINFO_STRING(RADIONAME, radioname);
...
static int nl80211_set_mcast_rate(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (dev->ieee80211_ptr->iftype != NL80211_IFTYPE_ADHOC &&
	    dev->ieee80211_ptr->iftype != NL80211_IFTYPE_TDMA &&
...
#ifdef CPTCFG_MAC80211_TDMA
	{
		.cmd = NL80211_CMD_JOIN_TDMA,
		.doit = nl80211_join_tdma,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = IFLAGS(NL80211_FLAG_NEED_NETDEV),
	},
	{
		.cmd = NL80211_CMD_LEAVE_TDMA,
		.doit = nl80211_leave_tdma,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = IFLAGS(NL80211_FLAG_NEED_NETDEV_UP),
	},
#endif
...
static struct genl_family nl80211_fam __genl_ro_after_init = {
	...
	.resv_start_op = NL80211_CMD_QUERY_SYNC_TDMA + 1,
#endif
				  ...
				  void *_notify)
{
	...
	list_for_each_entry_rcu(rdev, &cfg80211_rdev_list, list) {
		...
		list_for_each_entry_rcu(wdev, &rdev->wiphy.wdev_list, list) {
			...
			cfg80211_release_pmsr(wdev, netlink_notify_portid(notify));
		}
