...
void cfg80211_unregister_wdev(struct wireless_dev *wdev)
{
	...
	case NL80211_IFTYPE_TDMA:
		rdev->ops->leave_tdma(&rdev->wiphy, wdev->netdev);
		rdev->opencount--;
		break;
#endif
	case NL80211_IFTYPE_P2P_DEVICE:
		      ...
		      struct wireless_dev *wdev)
{
	...
	case NL80211_IFTYPE_TDMA:
#ifdef CPTCFG_MAC80211_TDMA
		__cfg80211_leave_tdma(rdev, dev, false);
#endif
		break;
					 ...
					 unsigned long state, void *ptr)
{
	...
	case NETDEV_REGISTER:
		...
		if ((wdev->wiphy->flags & WIPHY_FLAG_PS_ON_BY_DEFAULT) && (wdev->iftype != NL80211_IFTYPE_TDMA))
			wdev->ps = true;
		else
			wdev->ps = false;
