...
#ifdef CPTCFG_MAC80211_TDMA
	if ((sdata->vif.type == NL80211_IFTYPE_TDMA) &&
	    rcu_access_pointer(sdata->u.tdma.sta) == sta)
		RCU_INIT_POINTER(sdata->u.tdma.sta, NULL);
#endif
		   ...
		   bool tidstats)
{
	...
	memcpy(&sinfo->vht_operation, &sta->sta.vht_operation, sizeof(sinfo->vht_operation));
	...
	if (!(sinfo->filled & (BIT_ULL(NL80211_STA_INFO_TX_COMPRESSED)))) {
		sinfo->tx_compressed = sdata->compressed;
		sinfo->filled |= BIT_ULL(NL80211_STA_INFO_TX_COMPRESSED);
	}

	if (!(sinfo->filled & (BIT_ULL(NL80211_STA_INFO_RX_COMPRESSED)))) {
		sinfo->rx_compressed = sdata->decompressed;
		sinfo->filled |= BIT_ULL(NL80211_STA_INFO_RX_COMPRESSED);
	}

	if (!(sinfo->filled & (BIT_ULL(NL80211_STA_INFO_TX_COMPRESSED_BYTES64)))) {
		sinfo->tx_compressed_bytes = sdata->tx_compressed_bytes;
		sinfo->filled |= BIT_ULL(NL80211_STA_INFO_TX_COMPRESSED_BYTES64);
	}

	if (!(sinfo->filled & (BIT_ULL(NL80211_STA_INFO_RX_COMPRESSED_BYTES64)))) {
		sinfo->rx_compressed_bytes = sdata->rx_compressed_bytes;
		sinfo->filled |= BIT_ULL(NL80211_STA_INFO_RX_COMPRESSED_BYTES64);
	}

#endif
	if (!(sinfo->filled & BIT_ULL(NL80211_STA_INFO_RADIONAME))) {
		strncpy(&sinfo->radioname[0], &sta->radioname[0], 15);
		sinfo->filled |= BIT_ULL(NL80211_STA_INFO_RADIONAME);
	}
	...
	if (test_sta_flag(sta, WLAN_STA_PS_STA))
		sinfo->sta_flags.set |= BIT(NL80211_STA_FLAG_PM);
	if (test_sta_flag(sta, WLAN_STA_PS_DRIVER))
		sinfo->sta_flags.set |= BIT(NL80211_STA_FLAG_PM);
