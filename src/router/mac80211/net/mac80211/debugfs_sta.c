...
#define STA_READ_UL(name, field) STA_READ(name, field, "%lu\n")
STA_FILE(tdma_aid, n.aid, D);
STA_FILE(tdma_see_us, n.voted, D);
STA_FILE(tdma_last_own_beacon_missed, n.miss_own_beacon, D);
STA_FILE(tdma_rcvd_between_gacks, n.num_seqs, D);
STA_FILE(tdma_created, n.created, UL);
STA_FILE(tdma_last_seen, n.last_seen, UL);

static ssize_t sta_tdma_queues_read(struct file *file,
					  char __user *userbuf,
					  size_t count, loff_t *ppos)
{
	struct sta_info *sta = file->private_data;
	...
	res = scnprintf(buf, sizeof(buf), "ACTIVE %d, PENDED %d\n", skb_queue_len(&sta->n.tx), skb_queue_len(&sta->n.pending));
...
#ifdef CPTCFG_MAC80211_TDMA
	DEBUGFS_ADD(tdma_aid);
	DEBUGFS_ADD(tdma_see_us);
	DEBUGFS_ADD(tdma_last_own_beacon_missed);
	DEBUGFS_ADD(tdma_rcvd_between_gacks);
	DEBUGFS_ADD(tdma_created);
	DEBUGFS_ADD(tdma_last_seen);
