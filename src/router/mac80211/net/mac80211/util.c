			       ...
			       u64 filter, u32 crc)
{
	...
	while (left >= 2) {
		...
		case WLAN_EID_VENDOR_SPECIFIC:
			...
			if (pos[0] == 0x00 && pos[1] == 0x0c && pos[2] == 0x42) {
				/* Mikrotik Shit OUI (00:0C:42) */
				if ((elen-6) >= sizeof(struct ieee80211_mtik_ie_data)) {
					elems->mtik = (struct ieee80211_mtik_ie_data *)&pos[6];
				}else {
					printk(KERN_EMERG "mikrotik element length is too small %d\n",elen);
				}
			    }
			       ...
			       bool bss_notify, bool enable_qos)
{
	...
	for (ac = 0; ac < IEEE80211_NUM_ACS; ac++) {
		...
		if (sdata->vif.type == NL80211_IFTYPE_TDMA)
		{
			...
			/* Confiure TDMA access rules. */
			qparam.cw_max = 1;
			qparam.cw_min = 1;
			qparam.txop = 0;
			qparam.aifs = ((TDMA_CFG_VERSION(tdma) == 0) || TDMA_CFG_GACK(tdma)) ? 0 : 1;
		}
#endif
...
int ieee80211_build_preq_ies(struct ieee80211_sub_if_data *sdata, struct ieee80211_local *local, u8 *buffer,
			     size_t buffer_len,
					  ...
					  u32 flags)
{
	...
	ies_len = ieee80211_build_preq_ies(sdata, local, skb_tail_pointer(skb),
			       ...
			       struct cfg80211_chan_def *chandef)
{
	...
	case IEEE80211_HT_PARAM_CHA_SEC_BELOW:
		...
		break;
/*
	case NL80211_CHAN_WIDTH_40_NOHT:
		WARN_ON_ONCE(1);
		c->width = NL80211_CHAN_WIDTH_40_NOHT;
...
static struct ieee80211_ddwrt_ie_data ddwrtie;

u32 ieee80211_get_ddwrt_ie_len(void)
{
	u32 len;

	if (!ddwrtie.os_version) {
		char *os = nvram_get("os_version");
		kstrtouint(os, 10, &ddwrtie.os_version);
	}
	if (!ddwrtie.boardname_len) {
		char *board = nvram_get("DD_BOARD");
		ddwrtie.boardname_len = strlen(board);
		memcpy(&ddwrtie.boardname[0], board, ddwrtie.boardname_len);
	}

	if (!ddwrtie.os_version || !ddwrtie.boardname_len)
		return 0;

	len = sizeof(struct ieee80211_ddwrt_ie);
	len += sizeof(struct ieee80211_ddwrt_ie_data);
	len += ddwrtie.boardname_len;
...
{
	static const u_int8_t ddwrtoui[3] = { 0x00, 0xdd, 0xdd };
	if (!ddwrtie.os_version || !ddwrtie.boardname_len)
		return frm;
	*frm++ = WLAN_EID_VENDOR_SPECIFIC;
	*frm++ = sizeof(struct ieee80211_ddwrt_ie_data) + ddwrtie.boardname_len + 6; /* ddwrt ie + oui + {version, type, subtype} */
	memcpy(frm, &ddwrtoui, sizeof(ddwrtoui));
	frm += sizeof(ddwrtoui);
	*frm++ = 0;
	*frm++ = 0;
	*frm++ = 0;
	memcpy(frm, &ddwrtie, sizeof(ddwrtie));
	frm += sizeof(ddwrtie);
	memcpy(frm, ddwrtie.boardname, ddwrtie.boardname_len);
	frm += ddwrtie.boardname_len;
	return frm;
}
...
u8 *ieee80211_add_mtik_ie(u8 *frm, int wds)
{
	...
	if (0) {
		...
		*frm++ = sizeof(struct ieee80211_mtik_ie_data) + 6; /* mtik ie + oui + {version, type, subtype} */
		memcpy(frm, mtikoui, sizeof(mtikoui));	  /* mtik OUI, type, subtype, and version for adv capabilities */
		frm += sizeof(mtikoui);
		*frm++ = 0;
		*frm++ = 0;
		*frm++ = 0;
	...
	strncpy(&mtikie->radioname[0], system_utsname.nodename, mtikie->namelen - 1);
#else
	strncpy(&mtikie->radioname[0], init_utsname()->nodename, mtikie->namelen - 1);
#endif
	mtikie->namelen = strlen(&mtikie->radioname[0]);
	frm += sizeof(struct ieee80211_mtik_ie_data);
