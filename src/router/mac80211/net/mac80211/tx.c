...
ieee80211_tx_h_rate_ctrl(struct ieee80211_tx_data *tx)
{
	...
	txrc.bss = (tx->sdata->vif.type == NL80211_IFTYPE_AP ||
		    ...
		    tx->sdata->vif.type == NL80211_IFTYPE_ADHOC || tx->sdata->vif.type == NL80211_IFTYPE_TDMA ||
		    tx->sdata->vif.type == NL80211_IFTYPE_OCB);

	/* set up RTS protection if desired */
	if ((len > tx->local->hw.wiphy->rts_threshold) && (tx->sdata->vif.type != NL80211_IFTYPE_TDMA)) {
		txrc.rts = true;
	}
...
#define CALL_TXH(txh) \
	...
	if (ieee80211_tx_h_compress) {
		CALL_TXH(ieee80211_tx_h_compress);
	}
#endif
		    ...
		    struct sta_info *sta, struct sk_buff *skb)
{
	...
	if (sdata->vif.type == NL80211_IFTYPE_TDMA)
		tdma_sched_tx(sdata, skb, info->band);
	else
#endif
					   ...
					   struct sta_info *sta)
{
	...
	struct ieee80211_tx_info *info;
	int head_need;
	u16 ethertype, hdrlen = 24,  meshhdrlen = 0;
	__le16 fc;
	struct ieee80211_hdr hdr;
	...
	case NL80211_IFTYPE_STATION:
		...
		if (tdls_peer) {
			...
			hdrlen = 24;
			    ...
			    (cpu_to_be16(ethertype) != sdata->control_port_protocol || sdata->u.mgd.use_mtikwds)) {
			fc |= cpu_to_le16(IEEE80211_FCTL_FROMDS |
					  IEEE80211_FCTL_TODS);
			/* RA TA DA SA */
	...
	case NL80211_IFTYPE_TDMA:
		...
		memcpy(hdr.addr3, skb->data, ETH_ALEN);
		memcpy(hdr.addr4, skb->data + ETH_ALEN, ETH_ALEN);
		sta = rcu_dereference(sdata->u.tdma.sta);
		if (sta) {
			memcpy(hdr.addr1, sta->sta.addr, ETH_ALEN);
			authorized = test_sta_flag(sta, WLAN_STA_AUTHORIZED);
			wme_sta = sta->sta.wme;
			...
			if (!ether_addr_equal(hdr.addr4, sdata->vif.addr))
			    tdma_originator_update_rx(&sdata->u.tdma, hdr.addr4, NULL, 0, 13);
			if (TDMA_CFG_VERSION_P(sdata->u.tdma.cfg) == 2)
			    tdma_originator_get(&sdata->u.tdma, skb->data, hdr.addr1);
			else
			    eth_broadcast_addr(hdr.addr1);
		}
		meshhdrlen = tdma_hdr_len();
		chanctx_conf = rcu_dereference(sdata->vif.chanctx_conf);
		if (!chanctx_conf)
			goto free;
		band = chanctx_conf->def.chan->band;
	...
	if (encaps_data)
		memcpy((void*)skb_push(skb, encaps_len), encaps_data, encaps_len);

#ifdef CPTCFG_MAC80211_MESH
	if ((meshhdrlen > 0) && (sdata->vif.type == NL80211_IFTYPE_MESH_POINT)) {
		memcpy((void*)skb_push(skb, meshhdrlen), &mesh_hdr, meshhdrlen);
	}
#endif
#ifdef CPTCFG_MAC80211_TDMA
	if ((meshhdrlen > 0) && (sdata->vif.type == NL80211_IFTYPE_TDMA)) {
		tdma_update_skb_hdr(&sdata->u.tdma, skb, sta, meshhdrlen);
	}
#endif

	if (padsize)
		memset((void*)skb_push(skb, padsize), 0, padsize);

	if (ieee80211_is_data_qos(fc)) {
		__le16 *qos_control;

		qos_control = (void*)skb_push(skb, 2);
		memcpy((void*)skb_push(skb, hdrlen - 2), &hdr, hdrlen - 2);
		/*
		...
		*qos_control = 0;
		...
		memcpy((void*)skb_push(skb, hdrlen), &hdr, hdrlen);
					 ...
					 struct sk_buff *skb)
{
	...
	data = (void*)skb_push(skb, sizeof(*amsdu_hdr));
				      ...
				      struct sk_buff *skb)
{
	...
	ret = true;
	data = (void*)skb_push(skb, ETH_ALEN + 2);
...
//	skb_queue_walk_safe(&tx.skbs, skb, tmp) {
	...
	if (sdata->vif.type == NL80211_IFTYPE_AP) {
		...
		if (beacon) {
			...
			skb = dev_alloc_skb(local->tx_headroom +
					    beacon->head_len +
					    beacon->tail_len + 256 + sizeof(struct ieee80211_mtik_ie) +
