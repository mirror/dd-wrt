...
#include "wme.h"
#include "rate.h"
#if IS_ENABLED(CPTCFG_MAC80211_COMPRESS) || defined(CPTCFG_MAC80211_TDMA)
				 ...
				 int rtap_len, bool has_fcs)
{
	...
	rthdr = (void*)skb_push(skb, rtap_len);
...
ieee80211_rx_h_check(struct ieee80211_rx_data *rx)
{
	...
	if (unlikely((ieee80211_is_data(hdr->frame_control) ||
		     ...
		     rx->sdata->vif.type != NL80211_IFTYPE_TDMA &&
...
void ieee80211_free_radionames(struct ieee80211_sub_if_data *sdata)
{
	...
	struct radioname_data *last;
	while(current_radio){
	    last = current_radio;
	    current_radio = current_radio->next;
	    kfree(last);
	}
}
...
char *add_radioname(struct ieee80211_sub_if_data *sdata, char *mac, char *radioname, int wds)
{
	char *temp = NULL;
	struct radioname_data *current_radio = sdata->radionames;
	struct radioname_data *last = NULL;
	while (current_radio) {
		if (!memcmp(&current_radio->mac[0], mac, 6)) {
			if (radioname && radioname[0])
				memcpy(&current_radio->radioname[0], &radioname[0], 16);
			if (wds != -1)
			    current_radio->wds = wds;
			temp = &current_radio->radioname[0];
		}
		last = current_radio;
		current_radio = current_radio->next;
	}
	...
	if (radioname) {	// add new
		if (!radioname[0]) { 
			/* ignore empty names. we dont need to store things which dont exist */
			return radioname;
		}
		
		current_radio = kzalloc(sizeof(struct radioname_data), GFP_ATOMIC);
		if (!current_radio) {
			printk(KERN_INFO "alloc for radioname failed\n");
			return NULL;
		}
		memcpy(&current_radio->mac[0], &mac[0], 6);
		memcpy(&current_radio->radioname[0], &radioname[0], 16);
		if (wds != -1)
		       current_radio->wds = wds;
		if (!sdata->radionames)
			sdata->radionames = current_radio;
		else if (last)
			last->next = current_radio;
		return radioname;
	}
	return NULL;
...
ieee80211_rx_h_mgmt_check(struct ieee80211_rx_data *rx)
{
	...
	case cpu_to_le16(IEEE80211_STYPE_ACTION):
		...
		if (sta) {
			...
			if (radio) {
				memcpy(&sta->radioname[0], radio, 16);
			}
		}
...
ieee80211_rx_h_action(struct ieee80211_rx_data *rx)
{
	...
	case WLAN_CATEGORY_HT:
		...
		if (sdata->vif.type != NL80211_IFTYPE_STATION &&
		    ...
		    sdata->vif.type != NL80211_IFTYPE_AP_VLAN &&
		    sdata->vif.type != NL80211_IFTYPE_AP &&
		    sdata->vif.type != NL80211_IFTYPE_TDMA &&
...
#ifdef CPTCFG_MAC80211_TDMA
static ieee80211_rx_result debug_noinline
ieee80211_rx_h_tdma(struct ieee80211_rx_data *rx)
{
	struct ieee80211_sub_if_data *sdata = rx->sdata;
	struct ieee80211_mgmt *mgmt = (void *)rx->skb->data;
	__le16 stype;
	...
	if (stype != cpu_to_le16(IEEE80211_STYPE_BEACON))
		return RX_CONTINUE;
...
static ieee80211_rx_result debug_noinline
ieee80211_rx_h_tdma_amsdu(struct ieee80211_rx_data *rx)
{
	struct ieee80211_sub_if_data *sdata = rx->sdata;
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)rx->skb->data;
	struct net_device *dev = rx->sdata->dev;
	...
	if (ieee80211_is_data_present(hdr->frame_control)) {
	    if (!tdma_process_hdr(rx, hdr, &amsdu, &len, &to_skip))
		return RX_DROP_UNUSABLE;
	}
	if (amsdu) {
	    struct sk_buff_head frame_list;
	    ...
	    tdma_amsdu_to_8023s(sdata, skb, &frame_list, rx->local->hw.extra_tx_headroom, len, to_skip);

	    while (!skb_queue_empty(&frame_list)) {
		rx->skb = __skb_dequeue(&frame_list);

		if (!ieee80211_frame_allowed(rx, hdr->frame_control)) {
		    dev_kfree_skb(rx->skb);
		    continue;
		}
...
#endif
#ifdef CPTCFG_MAC80211_TDMA
		CALL_RXH(ieee80211_rx_h_tdma_amsdu);
#endif
...
//#ifdef CPTCFG_MAC80211_COMPRESS
//	ieee80211_rx_h_decompress(rx);
//#endif
	...
	memcpy((void*)skb_push(skb, sizeof(addrs)), &addrs, sizeof(addrs));
					    ...
					    struct sk_buff *skb, bool consume)
{
	...
	if (consume && rx->sta
#if IS_ENABLED(CPTCFG_MAC80211_COMPRESS)
	&& !sdata->dev->ieee80211_ptr->use_compr
#endif
	) {
		struct ieee80211_fast_rx *fast_rx;
