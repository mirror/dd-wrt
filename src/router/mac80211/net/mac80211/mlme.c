			     ...
			     struct cfg80211_chan_def *chandef, bool tracking)
{
	...
	conf_channel = ieee80211_frequency_to_channel(channel->center_freq);
	/* check that channel matches the right operating channel */
	if (!tracking && ht_oper->primary_chan != conf_channel) {
		/*
		 ...
		 */
		sdata_info(sdata,
			   "Wrong control channel: conf_channel: %d ht->primary_chan: %d band: %d - Disabling HT\n",
			   conf_channel, ht_oper->primary_chan, channel->band);
		ret = IEEE80211_STA_DISABLE_HT |
		      IEEE80211_STA_DISABLE_VHT |
		      IEEE80211_STA_DISABLE_HE;
		goto out;
				...
				struct ieee80211_supported_band *sband)
{
	...
	pos = (void *)skb_put(skb, he_cap_size);
...
static void ieee80211_send_assoc(struct ieee80211_sub_if_data *sdata)
{
	...
	sdata_assert_lock(sdata);
	sdata->radioname[0] = 0; // clear radioname
	...
	/* SSID */
	pos = (void *)skb_put(skb, 2 + assoc_data->ssid_len);
	ie_start = pos;
	*pos++ = WLAN_EID_SSID;
	*pos++ = assoc_data->ssid_len;
	...
	pos = (void *)skb_put(skb, supp_rates_len + 2);
	...
	*pos++ = supp_rates_len;
	...
	if (rates_len > count) {
		pos = (void *)skb_put(skb, rates_len - count + 2);
...
skip_rates:
	...
	if (capab & WLAN_CAPABILITY_SPECTRUM_MGMT &&
		...
		pos = (void *)skb_put(skb, 2 * sband->n_channels + 2);
	...
	if (assoc_data->ie_len) {
		...
		noffset = ieee80211_ie_split(assoc_data->ie, assoc_data->ie_len,
					     ...
					     offset);
		pos = (void *)skb_put(skb, noffset - offset);
...
void ieee80211_recalc_ps(struct ieee80211_local *local)
{
	...
	list_for_each_entry(sdata, &local->interfaces, list) {
		if (!ieee80211_sdata_running(sdata))
			continue;
		if ((sdata->vif.type == NL80211_IFTYPE_AP) || (sdata->vif.type == NL80211_IFTYPE_TDMA)) {
				    ...
				    struct ieee802_11_elems *elems)
{
	...
	if (!is_6ghz &&
		...
		if (bss_elems.mtik) {
			memcpy(&radioname[0], &bss_elems.mtik->radioname[0], 15);
			if (bss_elems.mtik->namelen && bss_elems.mtik->namelen < 16)
	...
	if (radio) {
		memcpy(&sdata->radioname[0], radio, 16);
		if (sta)
	...
	if (elems->vht_operation)
	    memcpy(&sta->sta.vht_operation, elems->vht_operation, sizeof(sta->sta.vht_operation));
				     ...
				     struct ieee80211_rx_status *rx_status)
{
	...
	if (ieee80211_rx_status_to_khz(rx_status) != chanctx_conf->def.chan->center_freq) {
		rcu_read_unlock();
		return;
	}
	chan = chanctx_conf->def.chan;
				  ...
				  struct cfg80211_bss *cbss)
{
	...
	bool is_2ghz = cbss->channel->band == NL80211_BAND_2GHZ;
	struct ieee80211_bss *bss = (void *)cbss->priv;
	int ret;
	...
	if (sband->vht_cap.vht_supported && is_2ghz) {
		    if (!local->turboqam) {
