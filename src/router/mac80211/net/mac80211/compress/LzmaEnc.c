...
MY_NO_INLINE static void MY_FAST_CALL LenPriceEnc_UpdateTable(CLenPriceEnc * p, unsigned posState, const CLenEnc * enc, const CProbPrice * ProbPrices)
{
...
static void LenPriceEnc_UpdateTables(CLenPriceEnc * p, unsigned numPosStates, const CLenEnc * enc, const CProbPrice * ProbPrices)
{
	unsigned posState;
...
static unsigned GetOptimum(CLzmaEnc * p, UInt32 position)
{
	...
	for (;;) {
		...
		if (matchByte == curByte)
			...
			if (nextOpt->len < 2 || (nextOpt->dist != 0 && nextOpt->extra <= 1	// 17.old
			    )) {
		...
		if (
			   // litPrice != 0 && // 18.new
			   !nextIsLit && matchByte != curByte && numAvailFull > 2) {
			...
			for (; repIndex < LZMA_NUM_REPS; repIndex++) {
				...
				/* if (_maxMode) */
				{
					...
					if (len2 >= 3) {
						...
						price += p->repLenEnc.prices[posState][(size_t)len - 2]
...
static SRes LzmaEnc_MemPrepare(CLzmaEncHandle pp, const Byte * src, SizeT srcLen, UInt32 keepWindowSize, ISzAllocPtr alloc, ISzAllocPtr allocBig)
{
	CLzmaEnc *p = (CLzmaEnc *) pp;
