				     ...
				     bool acked, bool dropped)
{
	...
	if (info->flags & IEEE80211_TX_INTFL_NL80211_FRAME_TX) {
		...
		if (skb->len >= ETH_HLEN && skb->protocol == cpu_to_be16(ETH_P_802_3))
			skb_copy_bits(skb, 2 * ETH_ALEN, &ethertype, 2);
				  ...
				  int rates_idx, int retry_count)
{
	...
	if (status->sta) {
		...
		if (sta->sdata->vif.type == NL80211_IFTYPE_TDMA) {
		    ...
		    if (sta && ((sta->sdata->u.tdma.node_num > 2) && (TDMA_CFG_VERSION_P(sta->sdata->u.tdma.cfg) < 2)))
			tdma_originator_update_ack(&sta->sdata->u.tdma, hdr->addr1, acked, skb->len, info->status.ack_signal);
#endif
		    if (!(info->flags & IEEE80211_TX_CTL_INJECTED) && ieee80211_is_data_qos(fc)) {
			struct tdma_hdr * thdr = (struct tdma_hdr *)(skb->data + ieee80211_hdrlen(fc));

			if (thdr->sequence != 0) {
			    TDMA_HDR_SET_INFO(thdr, false);
			    if (tdma_store_pending_frame(&sta->n, skb)) {
				rcu_read_unlock();
				return;
			    }
			}
...
#ifdef CPTCFG_MAC80211_TDMA
			ieee80211_tdma_tx_notify( &sta->sdata->vif, false);
#endif
			return;
