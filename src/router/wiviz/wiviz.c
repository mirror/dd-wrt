...
void dealWithPacket (wiviz_cfg * cfg, struct pcap_pkthdr *header,
		      const u_char * packet);
	  ...
	  if (!strcmp (v, "hop"))
	      ...
	      else
		{
		  ...
		  if (cfg->readFromWl)
		    {
		      if (wl_ioctl
			   (WL_DEVICE, WLC_SET_CHANNEL, &cfg->curChannel,
			    4) < 0)
			{
...
////////////////////////////////////////////////////////////////////////////////
  void
dealWithPacket (wiviz_cfg * cfg, struct pcap_pkthdr *header,
		const u_char * packet)
{
  ...
  while ((int) i < (int) hWifi)
    ...
    wfType =
    ((hWifi->frame_control & 0xF0) >> 4) +
    ((hWifi->frame_control & 0xC) << 2);
  switch (wfType)
    {
  ...
  if (type == typeUnknown)
      ...
      while ((u_int) e < (u_int) packet + header->len)
	{
	  ...
	  if (e->tag == tagVendorSpecific)
	    {
	      if (e->length >= 4
		   && memcmp (e + 1, "\x00\x50\xf2\x01", 4) == 0)
		{
...
////////////////////////////////////////////////////////////////////////////////
  void
print_mac (u_char * mac, char *extra)
{
...
////////////////////////////////////////////////////////////////////////////////
  void
fprint_mac (FILE * outf, u_char * mac, char *extra)
{
  fprintf (outf, "%02X:%02X:%02X:%02X:%02X:%02X%s", mac[0] & 0xFF,
  ...
  if (!h->occupied)
    {
      ...
      fprintf (stderr, "%s\n",
		(type ==
		 typeAP) ? "AP" : ((type == typeSta) ? "Sta" : "Unk"));
    }
  h->occupied = 1;
...
////////////////////////////////////////////////////////////////////////////////
  void
print_host (FILE * outf, wiviz_host * host)
{
  int i;
  if (!host->occupied)
    return;
...
readWL (wiviz_cfg * cfg)
{
  ...
  if (ap)
    {
      ...
      host->RSSI = 0;
      wl_ioctl (WL_DEVICE, WLC_GET_CHANNEL, &channel,
		 sizeof (channel_info_t));
      host->apInfo->channel = channel.hw_channel;
      macs =
	(maclist_t *) malloc (4 + MAX_STA_COUNT * sizeof (ether_addr_t));
      macs->count = MAX_STA_COUNT;
      if (wl_ioctl
	   (WL_DEVICE, WLC_GET_ASSOCLIST, macs,
	    4 + MAX_STA_COUNT * sizeof (ether_addr_t)) > -1)
	{
	  for (i = 0; i < macs->count; i++)
	    {
	      ...
	      if (wl_ioctl (WL_DEVICE, WLC_GET_RSSI, &starssi, 12) < 0)
		printf ("rssifail\n");
  ...
  else
    {
      ...
      host->RSSI = 0;
      if (wl_ioctl
	   (WL_DEVICE, WLC_GET_BSSID, &host->staInfo->connectedBSSID, 6) < 0)
	{
	  host->staInfo->state = ssUnassociated;
	}
  ...
  if (wl_ioctl
       (WL_DEVICE, WLC_GET_CHANNEL, &channel, sizeof (channel_info_t)) >= 0)
    {
