...
CONNECTIONFILE="/tmp/conninfo.ini"
if [ -z "$2" ]
then
	if [ ! -f $CONNECTIONFILE ]
	then
		echo "# this file contains the connection information for your subscription" >> $CONNECTIONFILE
		echo "APN=surfo2" >> $CONNECTIONFILE
		echo "USER=internet" >> $CONNECTIONFILE
		echo "PASS=internet" >> $CONNECTIONFILE
		echo "PIN=1234" >> $CONNECTIONFILE
	fi
else
	if [ -f $2 ]
	then
