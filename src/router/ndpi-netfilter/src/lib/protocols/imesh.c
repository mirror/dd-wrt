...
#define NDPI_CURRENT_PROTO NDPI_PROTOCOL_IMESH

#include "ndpi_api.h"

static void ndpi_int_imesh_add_connection(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow /* , ndpi_protocol_type_t protocol_type */ )
{
	ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_IMESH, NDPI_PROTOCOL_UNKNOWN);
}

static void ndpi_search_imesh_tcp_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->udp != NULL) {
		...
		if (packet->payload_packet_len == 28 && (get_u_int32_t(packet->payload, 0)) == htonl(0x02000000) &&
		    get_u_int32_t(packet->payload, 24) == 0 && (packet->udp->dest == htons(1864) || packet->udp->source == htons(1864))) {
		...
		if (packet->payload_packet_len == 32 && get_u_int32_t(packet->payload, 0) == htonl(0x02000000) && get_u_int16_t(packet->payload, 21) == 0 && get_u_int16_t(packet->payload, 26) == htons(0x0100)) {
			if (get_u_int32_t(packet->payload, 4) == htonl(0x00000081) && packet->payload[11] == packet->payload[15] && get_l16(packet->payload, 24) == htons(packet->udp->source)) {
	...
	if (packet->tcp != NULL) {
		if (packet->payload_packet_len == 64 && get_u_int32_t(packet->payload, 0) == htonl(0x40000000) &&
		    ...
		    get_u_int32_t(packet->payload, 12) == htonl(0x04800100) && get_u_int32_t(packet->payload, 45) == htonl(0xff020000) && get_u_int16_t(packet->payload, 49) == htons(0x001a)) {
		...
		if (packet->payload_packet_len == 28 && get_u_int32_t(packet->payload, 0) == htonl(0x1c000000) &&
		    ...
		    (get_u_int16_t(packet->payload, packet->payload_packet_len - 2) == htons(0x1900) || get_u_int16_t(packet->payload, packet->payload_packet_len - 2) == htons(0x1a00))) {
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "found imesh.\n");
			ndpi_int_imesh_add_connection(ndpi_struct, flow);
			return;
		}
		...
		if ((packet->actual_payload_len == 8 || packet->payload_packet_len == 10)	/* PATTERN:: 04 00 00 00 00 00 00 00 [00 00] */
			...
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			   ...
			   && packet->payload[5] == 0x00 && (packet->payload[2] != packet->payload[4])	/* We do not want that the packet is ?? ?? 00 00 00 00 */
		    ) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			...
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			   ...
			   && packet->payload[0] == 0x06 && packet->payload[1] == 0x00 && (packet->payload[2] == 0x04 || packet->payload[2] == 0x00)
			   ...
			   && packet->payload[7] == 0x00 && packet->payload[9] == 0x00 && (packet->payload[2] || packet->payload[4] || packet->payload[6])	/* We do not want that the packet is all 06 00 00 ... */
		    ) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			...
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			   ...
			   && packet->payload[1] == 0x00
			   && packet->payload[2] == 0x02 && packet->payload[3] == 0x00 && packet->payload[4] == 0x00 && packet->payload[5] == 0x00 && packet->payload[6] == 0x33 && packet->payload[7] == 0x00) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
		} else if (packet->payload_packet_len == 6	/* PATTERN:: 02 00 00 00 33 00 */
			   && packet->payload[0] == 0x02 && packet->payload[1] == 0x00 && packet->payload[2] == 0x00 && packet->payload[3] == 0x00 && packet->payload[4] == 0x33 && packet->payload[5] == 0x00) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
		} else if (packet->actual_payload_len == 12 && packet->payload[0] == 0x06	// PATTERN : 06 00 06 00 00 00 64 00
			   && packet->payload[1] == 0x00
			   && packet->payload[2] == 0x06 && packet->payload[3] == 0x00 && packet->payload[4] == 0x00 && packet->payload[5] == 0x00 && packet->payload[6] == 0x64 && packet->payload[7] == 0x00) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
		} else if (packet->actual_payload_len == 10	/* PATTERN:: 06 00 04|01 00 00 00 01|00 00 ?? 00 */
			   && packet->payload[0] == 0x06 && packet->payload[1] == 0x00 && (packet->payload[2] == 0x04 || packet->payload[2] == 0x01)
			   && packet->payload[3] == 0x00 && packet->payload[4] == 0x00 && packet->payload[5] == 0x00 && (packet->payload[6] == 0x01 || packet->payload[6] == 0x00)
			...
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			   ...
			   && packet->payload[1] == 0x00 && packet->payload[2] == 0x00 && packet->payload[3] == 0x00 && packet->payload[4] == 0x00) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			...
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
			   ...
			   && get_u_int16_t(packet->payload, 0) == (packet->actual_payload_len)
			   && packet->payload[2] == 0xee && packet->payload[3] == 0x00 && packet->payload[4] == 0x00 && packet->payload[5] == 0x00) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
		} else if (packet->actual_payload_len == 10	/* PATTERN:: 06 00 00 00 00 00 00 00 */
			   && packet->payload[0] == 0x06
			   && packet->payload[1] == 0x00
			   && packet->payload[2] == 0x00 && packet->payload[3] == 0x00 && packet->payload[4] == 0x00 && packet->payload[5] == 0x00 && packet->payload[6] == 0x00 && packet->payload[7] == 0x00) {
			flow->l4.tcp.imesh_stage += 2;
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct, NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n", packet->actual_payload_len);
		}
