...
#elif defined(HAVE_R9000) || defined(HAVE_MVEBU) || defined(HAVE_IPQ806X) || defined(HAVE_XSCALE) || defined(HAVE_X86) || defined(HAVE_MAGICBOX) || defined(HAVE_LAGUNA) || defined(HAVE_VENTANA) || defined(HAVE_NORTHSTAR) || defined(HAVE_RB600) \
...
void start_drivers(void)
{
	...
	if (nvram_matchi("usb_enable", 1)) {
		...
		insmod
		    ("nls_base usb-common usbcore ehci-hcd ehci-platform ehci-fsl ehci-pci usb-uhci uhci-hcd usb-ohci ohci-hcd ohci-pci xhci-hcd xhci-pci xhci-plat-hcd dwc_otg usb-libusual fsl-mph-dr-of phy-mxs-usb extcon ci_hdrc ci13xxx_imx usbmisc_imx ci_hdrc_imx dwc3 dwc3-qcom phy-qcom-hsusb phy-qcom-ssusb");
...
/* ntfs-3g */
	...
	if (brand == ROUTER_WRT_1200AC) {
		set_led_usbport("pca963x\\:caiman\\:white\\:usb3_2", "usb1-port1");
		set_led_usbport("pca963x\\:caiman\\:white\\:usb3_1", "usb2-port1 usb1-port1");
		set_led_usbport("pca963x\\:caiman\\:white\\:usb2", "usb3-port1");
	}

	if (brand == ROUTER_WRT_1900ACV2) {

		set_led_usbport("pca963x\\:cobra\\:white\\:usb3_2", "usb1-port1");
	...
	if (brand == ROUTER_WRT_1900ACS) {
		...
		set_led_usbport("pca963x\\:shelby\\:white\\:usb3_2", "usb1-port1");
	...
	if (brand == ROUTER_WRT_3200ACM) {
		set_led_usbport("pca963x\\:rango\\:white\\:usb3_2", "usb1-port1");
