...
void fscheck_main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if (!strcmp(type, "md")) {
			...
			sysprintf("echo check > /sys/block/md%d/md/sync_action",
				  i);
		}
		i++;
	}
...
void start_raid(void)
{
	...
	if (zfs) {
		...
		dd_loginfo("raid",
			   "ZFS modules successfully loaded (%d threads)\n",
			   cpucount);
	}
	if (btrfs) {
	...
	while (1) {
		...
		if (strcmp(done, "1")) {
			...
			if (!strcmp(type, "md")) {
				dd_loginfo("raid", "creating MD Raid /dev/md%d",
					   i);
				sysprintf(
					"mdadm --create --assume-clean /dev/md%d --level=%s --raid-devices=%d --run %s",
					i, level, drives, raid);
				if (nvram_nmatch("ext4", "raidfs%d", i)) {
					sysprintf(
						"mkfs.ext4 -F -E lazy_itable_init=1 -L \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("ext2", "raidfs%d", i)) {
					sysprintf(
						"mkfs.ext2 -F -E lazy_itable_init=1 -L \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("ext3", "raidfs%d", i)) {
					sysprintf(
						"mkfs.ext3 -F -E lazy_itable_init=1 -L \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("xfs", "raidfs%d", i)) {
					sysprintf(
						"mkfs.xfs -f -L \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("apfs", "raidfs%d", i)) {
					sysprintf(
						"mkfs.apfs -s -L \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("btrfs", "raidfs%d", i)) {
					sysprintf(
						"mkfs.btrfs -f -L \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("exfat", "raidfs%d", i)) {
					sysprintf(
						"mkfs.exfat -n \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("fat32", "raidfs%d", i)) {
					sysprintf(
						"mkfs.fat -F 32 -n \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("ntfs", "raidfs%d", i)) {
					sysprintf(
						"mkfs.ntfs -Q -F -L \"%s\" /dev/md%d",
						poolname, i);
				}
				if (nvram_nmatch("zfs", "raidfs%d", i)) {
					sysprintf(
						"zpool create -f -m \"/tmp/mnt/%s\" \"%s\" /dev/md%d",
						poolname, poolname, i);
				}
			}
			if (!strcmp(type, "btrfs")) {
				if (!strcmp(level, "0"))
					sysprintf("mkfs.btrfs -f -d raid0 %s",
						  raid);
				if (!strcmp(level, "1"))
					sysprintf("mkfs.btrfs -f -d raid1 %s",
						  raid);
				if (!strcmp(level, "5"))
					sysprintf("mkfs.btrfs -f -d raid5 %s",
						  raid);
				if (!strcmp(level, "6"))
					sysprintf("mkfs.btrfs -f -d raid6 %s",
						  raid);
				if (!strcmp(level, "10"))
					sysprintf("mkfs.btrfs -f -d raid10 %s",
						  raid);
			}
			if (!strcmp(type, "zfs")) {
				dd_loginfo("raid", "creating ZFS Pool %s",
					   poolname);
				sysprintf("mkdir -p /tmp/mnt/%s", poolname);
				if (!strcmp(level, "1"))
					sysprintf(
						"zpool create -f -m \"/tmp/mnt/%s\" \"%s\" mirror %s",
						poolname, poolname, raid);
				if (!strcmp(level, "5"))
					sysprintf(
						"zpool create -f -m \"/tmp/mnt/%s\" \"%s\" raidz1 %s",
						poolname, poolname, raid);
				if (!strcmp(level, "6"))
					sysprintf(
						"zpool create -f -m \"/tmp/mnt/%s\" \"%s\" raidz2 %s",
						poolname, poolname, raid);
				if (!strcmp(level, "z3"))
					sysprintf(
						"zpool create -f -m \"/tmp/mnt/%s\" \"%s\" raidz3 %s",
						poolname, poolname, raid);
				if (!strcmp(level, "0"))
					sysprintf(
						"zpool create -f -m \"/tmp/mnt/%s\" \"%s\" %s",
						poolname, poolname, raid);
			}
		...
		if (!strcmp(type, "zfs")) {
			...
			if (nvram_nmatch("zle", "raidlz%d", i))
				sysprintf("zfs set compression=zle %s",
					  poolname);
			else if (nvram_nmatch("lz4", "raidlz%d", i))
				sysprintf("zfs set compression=lz4 %s",
					  poolname);
			else if (nvram_nmatch("gzip", "raidlz%d", i)) {
				if (nvram_nmatch("0", "raidlzlevel%d", i))
					sysprintf("zfs set compression=gzip %s",
						  poolname);
				else
					sysprintf(
						"zfs set compression=gzip-%s %s",
						nvram_nget("raidlzlevel%d", i),
						poolname);
			} else if (nvram_nmatch("lzjb", "raidlz%d", i))
				sysprintf("zfs set compression=lzjb %s",
					  poolname);
			else if (nvram_nmatch("zstd", "raidlz%d", i)) {
				if (nvram_nmatch("0", "raidlzlevel%d", i))
					sysprintf("zfs set compression=zstd %s",
						  poolname);
				else
					sysprintf(
						"zfs set compression=zstd-%s %s",
						nvram_nget("raidlzlevel%d", i),
						poolname);
			} else
				sysprintf("zfs set compression=off %s",
					  poolname);
		...
		if (!strcmp(type, "md")) {
			...
			{
				sysprintf(
					"echo 1 > /sys/block/%s/device/queue_depth",
					drive);
			}
			sysprintf("mdadm --assemble /dev/md%d %s", i, raid);
			sysprintf(
				"echo 32768 > /sys/block/md%d/md/stripe_cache_size",
				i);
			writeprocsys(
				"dev/raid/speed_limit_max",
				nvram_default_get("dev.raid.speed_limit_max",
						  "10000000"));
			sysprintf("mkdir -p \"/tmp/mnt/%s\"", poolname);
			if (nvram_nmatch("ext4", "raidfs%d", i)) {
				sysprintf("fsck.ext4 -p -y /dev/md%d", i);
				sysprintf(
					"mount -t ext4 /dev/md%d -o init_itable=0,nobarrier,noatime,nobh,nodiratime,barrier=0 \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("ext2", "raidfs%d", i)) {
				sysprintf("fsck.ext2 -p -y /dev/md%d", i);
				sysprintf(
					"mount -t ext2 /dev/md%d -o nobarrier,noatime,nobh,nodiratime,barrier=0 \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("ext3", "raidfs%d", i)) {
				sysprintf("fsck.ext3 -p -y /dev/md%d", i);
				sysprintf(
					"mount -t ext3 /dev/md%d -o nobarrier,noatime,nobh,nodiratime,barrier=0 \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("xfs", "raidfs%d", i)) {
				sysprintf(
					"mount -t xfs /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("btrfs", "raidfs%d", i)) {
				sysprintf(
					"mount -t btrfs /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("exfat", "raidfs%d", i)) {
				sysprintf(
					"mount -t exfat -o iocharset=utf8 /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("apfs", "raidfs%d", i)) {
				sysprintf(
					"mount -t apfs /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("fat32", "raidfs%d", i)) {
				sysprintf(
					"mount -t vfat -o iocharset=utf8 /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
			}
			if (nvram_nmatch("ntfs", "raidfs%d", i)) {
#ifdef HAVE_LEGACY_KERNEL
				sysprintf(
					"ntfs-3g -o compression,direct_io,big_writes /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
#else
#ifdef HAVE_NTFS3
				sysprintf(
					"mount -t ntfs3 -o nls=utf8,noatime /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
#else
				sysprintf(
					"mount -t antfs -o utf8 /dev/md%d \"/tmp/mnt/%s\"",
					i, poolname);
#endif
#endif
			}
		...
		if (!strcmp(type, "btrfs")) {
			...
			if (nvram_nmatch("lzo", "raidlz%d", i))
				sysprintf(
					"mount -t btrfs -o compression=lzo %s \"/tmp/mnt/%s\"",
					r, poolname);
			else if (nvram_nmatch("zstd", "raidlz%d", i))
				sysprintf(
					"mount -t btrfs -o compression=zstd %s \"/tmp/mnt/%s\"",
					r, poolname);
			else if (nvram_nmatch("gzip", "raidlz%d", i)) {
				if (nvram_nmatch("0", "raidlzlevel%d", i))
					sysprintf(
						"mount -t btrfs -o compression=gzip %s \"/tmp/mnt/%s\"",
						r, poolname);
				else
					sysprintf(
						"mount -t btrfs -o compression=gzip:%s %s \"/tmp/mnt/%s\"",
						nvram_nget("raidlzlevel%d", i),
						r, poolname);
			} else
				sysprintf("mount -t btrfs %s \"/tmp/mnt/%s\"",
					  r, poolname);
			free(r);
