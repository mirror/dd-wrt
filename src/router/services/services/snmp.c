...
{
	char path[64];
	char *snmpd_argv[] = { "snmpd", "-c", getdefaultconfig("snmp", path, sizeof(path), "snmpd/snmpd.conf"), NULL };
	FILE *fp = NULL;
	...
	fp = fopencreate(getdefaultconfig("snmp", path, sizeof(path), "snmpd/snmpd.conf"), "w");
	if (NULL == fp)
		return;
