...
void start_sshd(int force)
{
	int state = nvram_states("sshd_enable http_username http_passwd " NVRAM_RSA_KEY_NAME " sshd_authorized_keys sshd_port sshd_passwd_auth sshd_forwarding");
	if (!force && !state)
		return;
	stop_sshd();
