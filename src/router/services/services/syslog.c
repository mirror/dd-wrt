...
void start_syslog(void)
{
	...
	if (nvram_matchi("syslogd_jffs2", 1) && !nvram_matchi("flash_active", 1)) {
		...
		if (*(nvram_safe_get("syslogd_rem_ip")))
			log_eval("syslogd", "-t", "-Z", "-b", "5", "-L", "-R", nvram_safe_get("syslogd_rem_ip"), "-O",
				 "/jffs/log/messages");
		else
			log_eval("syslogd", "-t", "-Z", "-b", "5", "-L", "-O", "/jffs/log/messages");
	} else {
fallback:;
		if (*(nvram_safe_get("syslogd_rem_ip")))
			log_eval("syslogd", "-t", "-Z", "-L", "-R", nvram_safe_get("syslogd_rem_ip"));
		else
			log_eval("syslogd", "-t", "-Z", "-L");
	}
