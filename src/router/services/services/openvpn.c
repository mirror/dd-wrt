...
int cleanup_pbr(char *tablenr)
{
	...
	//system(cmd);
	while (limit > 0 && !eval("ip", "rule", "delete", "from", "0/0", "to",
				  "0/0", "table", tablenr)) {
		limit--;
	}
	return 0;
...
void setroute_pbr(char *tablenr)
{
	...
	if (nvram_matchi("openvpncl_killswitch", 1) &&
	    nvram_matchi("openvpncl_spbr", 1)) {
		eval("ip", "route", "add", "prohibit", "default", "table",
		     tablenr);
		dd_loginfo("openvpn", "PBR killswitch is active using: %s",
			   curline);
		...
		eval("ip", "route", "add", "default", "via",
		     nvram_safe_get("wan_gateway"), "table", tablenr);
		dd_loginfo("openvpn", "PBR is active but NO killwitch: %s",
			   curline);
	}
	...
	while (curline) {
		...
		size_t curlinelen = nextLine ?
					    ((unsigned)(nextLine - curline)) :
					    strlen(curline);
		...
		if (tempstr) {
			...
			if (tempstr[0] == '#') {
				...
				if (*iprule) {
					if (test_ipv4(iprule)) {
						sysprintf(
							"ip rule add %s table %s",
							iprule, tablenr);
					} else {
						dd_loginfo(
							"openvpn",
							"ip rule %s has NO valid IP address, not added to table %s",
							iprule, tablenr);
					}
				}
			}
...
void create_openvpnrules(FILE *fp)
{
	...
	if (nvram_matchi("openvpncl_fw", 0)) {
		if (nvram_match("openvpncl_tuntap", "tun")) {
			fprintf(fp,
				"iptables -D INPUT -i $dev -m state --state NEW -j ACCEPT 2> /dev/null\n"
	...
	if (nvram_match("openvpncl_tuntap", "tun")) {
		...
		if (nvram_matchi("openvpncl_splitdns", 1) &&
		    nvram_invmatch("openvpncl_spbr", "0")) {
			fprintf(fp,
				"if [[ $set -eq 0 ]]; then\n"
		...
		if (nvram_matchi("openvpncl_killswitch", 1) &&
		    nvram_matchi("openvpncl_spbr", 2)) {
			fprintf(fp,
				"logger -p user.info \"OpenVPN firewall PBR via WAN escape\"\n");
			fprintf(fp,
				"echo \"iptables -N vpn-pbr\" >> /tmp/openvpncl_fw.sh\n");
			fprintf(fp,
				"sed '/^[[:blank:]]*#/d;s/#.*//' \"/tmp/openvpncl/policy_ips\" | while read pbrip; do\n"
				"case $pbrip in\n"
				"[0-9]*)\n"
			...
			fprintf(fp,
				"echo \"iptables -D FORWARD %s -o $(get_wanface) -j vpn-pbr\" >> /tmp/openvpncl_fw.sh\n",
				IN_IF);
			fprintf(fp,
				"echo \"iptables -I FORWARD %s -o $(get_wanface) -j vpn-pbr\" >> /tmp/openvpncl_fw.sh\n",
				IN_IF);
		}
	}
	...
	if (*(nvram_safe_get("openvpncl_route")) &&
	    nvram_invmatch("openvpncl_spbr", "0")) { //policy based routing
		...
		fprintf(fp,
			"for i in $(ip rule | grep 'from all lookup 10' | cut -d: -f1); do ip rule del prior $i; done\n");
		...
		if (!nvram_match("openvpncl_tuntap", "tap")) {
			...
			if (nvram_matchi("openvpncl_spbr",
				...
				fprintf(fp,
					"ip route add 0.0.0.0/1 via $route_vpn_gateway table 10\n");
				fprintf(fp,
					"ip route add 128.0.0.0/1 via $route_vpn_gateway table 10\n");
				if (nvram_matchi("openvpncl_killswitch", 1)) {
					fprintf(fp,
						"ip route add prohibit default table 10 >/dev/null 2>&1\n");
				}
			} else {
				fprintf(fp,
					"ip route add default via %s table 10 >/dev/null 2>&1\n",
...
void start_openvpnserver(void)
{
	...
	if (jffs_mounted())
		jffs = 1;
	dd_loginfo("openvpn",
		   "OpenVPN daemon (Server) starting/restarting...\n");
	...
	if (jffs == 1) {
		...
		if (*(nvram_safe_get("openvpn_ccddef"))) {
			write_nvram("/jffs/etc/openvpn/ccd/DEFAULT",
				    "openvpn_ccddef");
	...
	if (fp == NULL)
		return;
	if (nvram_matchi("openvpn_tls_btn", 2) &&
	    nvram_invmatch("openvpn_static", "")) { //static key
		fprintf(fp, "secret /tmp/openvpn/static.key\n");
	} else if (nvram_matchi("openvpn_pkcs", 1) &&
		   nvram_invmatch("openvpn_pkcs12", "")) {
		if (nvram_invmatch("openvpn_dh", "") &&
		    nvram_matchi("openvpn_dh_btn", 0))
			fprintf(fp, "dh /tmp/openvpn/dh.pem\n");
		fprintf(fp, "pkcs12 /tmp/openvpn/cert.p12\n");
	} else {
		if (nvram_invmatch("openvpn_dh", "") &&
		    nvram_matchi("openvpn_dh_btn",
	...
	if (nvram_matchi("openvpn_switch", 1)) {
		...
		sprintf(dcbuffer, "%s%s%s%s%s", dc1, *dc2 ? ":" : "", dc2,
			*dc3 ? ":" : "", dc3);
		if (dcbuffer[0])
		...
		if (nvram_matchi("openvpn_redirgate", 1)) {
			...
			snprintf(lanaddr, sizeof(lanaddr), "%s",
				 nvram_safe_get("lan_ipaddr"));
			//truncate and replace with .0
			mypoint = strrchr(lanaddr, '.');
			if (mypoint != NULL) {
				*mypoint = '\0';
				snprintf(srvnetwork, sizeof(srvnetwork), "%s%s",
					 lanaddr, ".0");
				fprintf(fp,
					"push \"route %s %s vpn_gateway\"\n",
					srvnetwork,
		...
		if (nvram_matchi("openvpn_enuserpass", 1) &&
		    nvram_invmatchi("openvpn_userpassnum", 0)) {
			...
			fprintf(fp,
				"auth-user-pass-verify /tmp/openvpn/quickAuth.sh via-env\n");
		}
	...
	if (nvram_matchi("openvpn_enuserpass", 1) &&
		...
		if (!fp) {
			dd_loginfo(
				"openvpn",
				"ERROR: Could not open %s to store usernames and passwords\n",
		...
		if (!fp) {
			dd_loginfo(
				"openvpn",
				"ERROR: Could not open username/password authentication script %s \n",
				"/tmp/openvpn/quickAuth.sh");
		} else {
			fprintf(fp, "#!/bin/sh\n");
			fprintf(fp,
				"pass=\"$(awk -v usr=$username '$1==usr { print $2 }' /tmp/openvpn/openvpn-auth)\"\n");
			fprintf(fp,
				"[[ -n $pass && $pass == $password ]] && exit 0\n");
			fprintf(fp, "exit 1\n");
			fclose(fp);
...
void stop_openvpnserver(void)
{
	...
	if (stop_process("openvpnserver", "OpenVPN daemon (Server)")) {
		...
		eval("ebtables", "-t", "nat", "-D", "POSTROUTING", "-o", "tap2",
		     "-p", "ipv4", "--ip-proto", "udp", "--ip-sport", "67:68",
		     "--ip-dport", "67:68", "-j", "DROP");
		eval("ebtables", "-t", "nat", "-D", "PREROUTING", "-i", "tap2",
		     "-p", "ipv4", "--ip-proto", "udp", "--ip-sport", "67:68",
		     "--ip-dport", "67:68", "-j", "DROP");
...
void start_openvpn(void)
{
	...
	insmod("tun");
	dd_loginfo("openvpn",
		   "OpenVPN daemon (Client) starting/restarting...\n");
	...
	if (*(nvram_safe_get("openvpncl_route")) &&
	    nvram_invmatch("openvpncl_spbr", "0")) {
		dd_loginfo("openvpn",
			   "PBR via tunnel now using setroute_pbr(): %s\n",
	...
	if (nvram_matchi("openvpncl_killswitch", 1) &&
		...
		dd_loginfo(
			"openvpn",
			"General Killswitch for OpenVPN enabled from OpenVPN ");
		...
		eval("iptables", "-D", "FORWARD", "-o",
		     safe_get_wan_face(wan_if_buffer), "-j", "DROP");
		eval("iptables", "-I", "FORWARD", "-o",
		     safe_get_wan_face(wan_if_buffer), "-j", "DROP");
	...
	if (fp == NULL)
		return;
	if (nvram_matchi("openvpncl_tls_btn", 2) &&
	    nvram_invmatch("openvpncl_static", "")) { //static key
		fprintf(fp, "secret /tmp/openvpncl/static.key\n");
	} else if (nvram_matchi("openvpncl_pkcs", 1) &&
		   nvram_invmatch("openvpncl_pkcs12", "")) {
...
*/
	...
	if (nvram_matchi("openvpncl_multirem", 1)) {
		...
		for (i = 2; i < 6; i++) {
			...
			if (nvram_invmatch(tempip, "") &&
			    nvram_invmatch(tempport, ""))
				fprintf(fp, "remote %s %s\n",
					nvram_safe_get(tempip),
					nvram_safe_get(tempport));
		}
	}
	if (nvram_invmatch("openvpncl_scramble", "off")) {
		...
		if (nvram_match("openvpncl_scramble", "obfuscate"))
			fprintf(fp, "scramble %s %s\n",
				...
				nvram_safe_get("openvpncl_scrmblpw"));
		else
	...
	if (nvram_matchi("openvpncl_fw", 0)) {
		if (nvram_match("openvpncl_tuntap", "tun")) {
			fprintf(fp,
				"iptables -D INPUT -i $dev -m state --state NEW -j ACCEPT\n"
	...
	if (nvram_matchi("openvpncl_killswitch", 1) &&
	    nvram_matchi("openvpncl_spbr", 2)) {
		fprintf(fp,
			"iptables -D FORWARD %s -o $(get_wanface) -j vpn-pbr\n",
			IN_IF);
