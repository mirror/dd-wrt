...
static void wl_rsn_ie_dump(bcm_tlv_t *ie, char *sum)
{
	...
	if (rsn_info.mcast) {
		if (!wlu_bcmp(rsn_info.mcast->oui, std_oui, 3)) {
			...
			sprintf(sum, "%s Unknown-%02X:%02X:%02X(#%d) ", sum, rsn_info.mcast->oui[0] & 0xff,
...
#endif
		site_survey_lists[i].frequency = ieee80211_ieee2mhz(dev, site_survey_lists[i].channel & 0xff);
