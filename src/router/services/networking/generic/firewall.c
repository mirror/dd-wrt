...
{
	return (nvram_nmatch("0", "%s_bridged", name) || isbridge(name)) ? 1 :
									   0;
}
...
static void parse_port_forward(char *wan_iface, char *wanaddr, char *lan_cclass,
			       char *wordlist, int dmzenable)
{
	char var[256], *next;
	...
	foreach(var, wordlist, next)
	{
		...
		if (!strcmp(proto, "tcp") || !strcmp(proto, "both")) {
			...
			if (flag_dis == 0) {
				save2file_A_prerouting(
					"-p tcp -d %s --dport %s:%s -j DNAT --to-destination %s",
					wanaddr, from, to, ip);
				if (!strcmp(wan_proto, "pppoe_dual") ||
				    (!strcmp(wan_proto, "pptp") &&
				     nvram_matchi("wan_dualaccess", 1)) ||
				    (!strcmp(wan_proto, "l2tp") &&
				     nvram_matchi("wan_dualaccess", 1)))
					save2file_A_prerouting(
						"-i %s -p tcp --dport %s:%s -j DNAT --to-destination %s",
						wan_iface, from, to, ip);
				snprintf(
					buff, sizeof(buff),
					"-A FORWARD -i %s -p tcp -m tcp -d %s --dport %s:%s -j %s\n",
		...
		if (!strcmp(proto, "udp") || !strcmp(proto, "both")) {
			...
			if (flag_dis == 0) {
				save2file_A_prerouting(
					"-p udp -d %s --dport %s:%s -j DNAT --to-destination %s",
					wanaddr, from, to, ip);
				if (!strcmp(wan_proto, "pppoe_dual") ||
				    (!strcmp(wan_proto, "pptp") &&
				     nvram_matchi("wan_dualaccess", 1)) ||
				    (!strcmp(wan_proto, "l2tp") &&
				     nvram_matchi("wan_dualaccess", 1)))
					save2file_A_prerouting(
						"-i %s -p udp -m udp --dport %s:%s -j DNAT --to-destination %s",
						wan_iface, from, to, ip);
				snprintf(
					buff, sizeof(buff),
					"-A FORWARD -i %s -p udp -m udp -d %s --dport %s:%s -j %s\n",
					wan_iface, ip, from, to, log_accept);
			} else {
				if ((!dmzenable) ||
				    (dmzenable &&
...
static void parse_upnp_forward(char *wanface, char *wanaddr, char *lan_cclass)
{
	...
	for (i = 0; i < 50; i++) {
		...
		if (!strcmp(proto, "tcp") || !strcmp(proto, "both")) {
			if (flag_dis == 0) {
				save2file_A_prerouting(
					"-i %s -p tcp -d %s --dport %s -j DNAT --to-destination %s%d:%s",
					wanface, wanaddr, wan_port0, lan_cclass,
					get_single_ip(lan_ipaddr, 3),
					lan_port0);
				snprintf(
					buff, sizeof(buff),
					"-A upnp -i %s -p tcp -m tcp -d %s%d --dport %s -j %s\n",
					wanface, lan_cclass,
					get_single_ip(lan_ipaddr, 3), lan_port0,
					log_accept);
			} else
				snprintf(
					buff, sizeof(buff),
		...
		if (!strcmp(proto, "udp") || !strcmp(proto, "both")) {
			if (flag_dis == 0) {
				save2file_A_prerouting(
					"-i %s -p udp -d %s --dport %s -j DNAT --to-destination %s%d:%s",
					wanface, wanaddr, wan_port0, lan_cclass,
					get_single_ip(lan_ipaddr, 3),
					lan_port0);
				snprintf(
					buff, sizeof(buff),
					"-A upnp -i %s -p udp -m udp -d %s%d --dport %s -j %s\n",
					wanface, lan_cclass,
					get_single_ip(lan_ipaddr, 3), lan_port0,
					log_accept);
			} else
...
#endif
static void create_spec_forward(char *wan_iface, char *proto, char *src,
				char *wanaddr, char *from, char *ip, char *to,
				int disabled)
{
	char buff[256];
	char *wan_proto = nvram_safe_get("wan_proto");
	if (src && *src) {
		if (disabled == 0) {
			save2file_A_prerouting(
				"-p %s -m %s -s %s -d %s --dport %s -j DNAT --to-destination %s:%s",
			...
			if (!strcmp(wan_proto, "pppoe_dual") ||
			    ...
			    (!strcmp(wan_proto, "l2tp") &&
			     nvram_matchi("wan_dualaccess", 1)))
				save2file_A_prerouting(
					"-i %s -p %s -m %s --dport %s -j DNAT --to-destination %s:%s",
					wan_iface, proto, proto, from, ip, to);

			if (!strcmp(nvram_safe_get("lan_ipaddr"), ip)) {
				snprintf(
					buff, sizeof(buff),
					"-I INPUT -p %s -m %s -s %s -d %s --dport %s -j %s\n",
		...
		if (disabled == 0) {
			save2file_A_prerouting(
				"-p %s -m %s -d %s --dport %s -j DNAT --to-destination %s:%s",
			...
			if (!strcmp(nvram_safe_get("lan_ipaddr"), ip)) {
				snprintf(
					buff, sizeof(buff),
					"-I INPUT -i %s -p %s -m %s -d %s --dport %s -j %s\n",
					wan_iface, proto, proto, ip, to,
					log_accept);
			} else {
				snprintf(
					buff, sizeof(buff),
					"-A FORWARD -i %s -p %s -m %s -d %s --dport %s -j %s\n",
					wan_iface, proto, proto, ip, to,
					log_accept);
			}
		} else {
...
static void parse_spec_forward(char *wan_iface, char *wanaddr, char *wordlist)
{
	...
	foreach(var, wordlist, next)
	{
		...
		if (!strcmp(proto, "tcp") || !strcmp(proto, "both")) {
			create_spec_forward(wan_iface, "tcp", src, wanaddr,
					    from, ip, to, flag_dis);
		}
		if (!strcmp(proto, "udp") || !strcmp(proto, "both")) {
			create_spec_forward(wan_iface, "udp", src, wanaddr,
					    from, ip, to, flag_dis);
		}
	}
}
...
static void create_ip_forward(int mode, char *wan_iface, char *src_ip,
			      char *dest_ip, int cnt, int disabled)
{
	char buff[256];
	...
	if (disabled == 0) {
		if (mode == ANT_IPF_PREROUTING) {
			...
			eval("ifconfig", buff, src_ip, "netmask",
			     "255.255.255.255", "up");

			save2file_A_prerouting(
				"-i %s -d %s -j DNAT --to-destination %s",
				wan_iface, src_ip, dest_ip);

			snprintf(buff, sizeof(buff),
				 "-A FORWARD -i %s -d %s -j %s\n", wan_iface,
				 dest_ip, log_accept);
			count += strlen(buff) + 1;
			suspense = realloc(suspense, count);
			strcat(suspense, buff);
		...
		if (mode == ANT_IPF_POSTROUTING) {
			save2file_A_postrouting(
				"-o %s -s %s -j SNAT --to-source %s", wan_iface,
				dest_ip, src_ip);
		}
	} else {
		if (mode == ANT_IPF_PREROUTING) {
...
static void nat_prerouting_bridged(char *wanface, char *vifs)
{
	...
	if (nvram_matchi("tor_enable", 1)) {
		if (nvram_matchi("tor_transparent", 1)) {
			save2file_A_prerouting(
				"-i %s -p udp --dport 53 -j DNAT --to %s:5353",
				"br0", nvram_safe_get("lan_ipaddr"));
			save2file_A_prerouting(
				"-i %s -p udp --dport 5353 -j DNAT --to %s:5353",
				"br0", nvram_safe_get("lan_ipaddr"));
			save2file_A_prerouting(
				"-i %s -p tcp --syn -j DNAT --to %s:9040",
				"br0", nvram_safe_get("lan_ipaddr"));
		}
	...
	if (nvram_matchi("dnsmasq_enable", 1)) {
		if (nvram_matchi("dns_redirectdot", 1)) {
			save2file_A_prerouting(
				"-i %s -p udp --dport 853 -j DNAT --to %s:53",
				nvram_safe_get("lan_ifname"),
				nvram_safe_get("lan_ipaddr"));
			save2file_A_prerouting(
				"-i %s -p tcp --dport 853 -j DNAT --to %s:53",
				nvram_safe_get("lan_ifname"),
				nvram_safe_get("lan_ipaddr"));
		}
		if (nvram_matchi("dns_redirect", 1)) {
			save2file_A_prerouting(
				"-i %s -p udp --dport 53 -j DNAT --to %s",
				nvram_safe_get("lan_ifname"),
				nvram_safe_get("lan_ipaddr"));
			save2file_A_prerouting(
				"-i %s -p tcp --dport 53 -j DNAT --to %s",
				nvram_safe_get("lan_ifname"),
				nvram_safe_get("lan_ipaddr"));
		}
	}
	foreach(var, vifs, next)
	{
		if ((!wanface || strcmp(wanface, var)) &&
			...
			if (nvram_nmatch("1", "%s_dns_redirect", var)) {
				...
				if (*target && sv_valid_ipaddr(target) &&
				    strcmp(target, "0.0.0.0")) {
					save2file_A_prerouting(
						"-i %s -p udp --dport 53 -j DNAT --to %s",
						var, target);
					save2file_A_prerouting(
...
static void nat_prerouting(char *wanface, char *wanaddr, char *lan_cclass,
			   int dmzenable, int remotessh, int remotetelnet,
			   int remotemanage, char *vifs)
{
	...
	if (nvram_matchi("privoxy_transp_enable", 1) &&
		...
		foreach(var, vifs, next)
		{
			if ((!wanface || strcmp(wanface, var)) &&
				...
				if (nvram_nmatch("1", "%s_isolation", var)) {
					save2file_A_prerouting(
						"-i %s -d %s/%s -j RETURN", var,
						lan_ip,
						nvram_safe_get("lan_netmask"));
					sprintf(vif_ip, "%s_ipaddr", var);
					save2file_A_prerouting(
						"-i %s -d %s -j RETURN", var,
		...
		if (*(nvram_safe_get("privoxy_transp_exclude"))) {
			save2file_A_prerouting(
				"-p tcp -s %s --dport 80 -j ACCEPT",
				nvram_safe_get("privoxy_transp_exclude"));
		}
		/* block access from privoxy to webif */
		save2file_A_prerouting("-p tcp -s %s -d %s --dport %d -j DROP",
				       lan_ip, lan_ip, web_lanport);
		/* do not filter access to the webif from lan */
		save2file_A_prerouting(
			"-p tcp -s %s/%s -d %s --dport %d -j ACCEPT", lan_ip,
			nvram_safe_get("lan_netmask"), lan_ip, web_lanport);
		/* go through proxy */
		save2file_A_prerouting(
			"-p tcp ! -d %s --dport 80 -j DNAT --to %s:8118",
			wanaddr, lan_ip);
	}
#endif
	...
	if (remotemanage) {
		if (remote_any) {
			save2file_A_prerouting(
				"-p tcp -d %s --dport %s -j DNAT --to-destination %s:%d",
				wanaddr, nvram_safe_get("http_wanport"), lan_ip,
				web_lanport);
		} else {
			...
			{
				save2file_A_prerouting(
					"-p tcp -s %s -d %s --dport %s -j DNAT --to-destination %s:%d",
					var, wanaddr,
					nvram_safe_get("http_wanport"), lan_ip,
					web_lanport);
			}
		}
	}
	...
	if (remotessh) {
		if (remote_any) {
			save2file_A_prerouting(
				"-p tcp -d %s --dport %s -j DNAT --to-destination %s:%s",
				wanaddr, nvram_safe_get("sshd_wanport"), lan_ip,
			...
			{
				save2file_A_prerouting(
					"-p tcp -s %s -d %s --dport %s -j DNAT --to-destination %s:%s",
					var, wanaddr,
					nvram_safe_get("sshd_wanport"), lan_ip,
					nvram_safe_get("sshd_port"));
			}
		}
	}
	...
	if (remotetelnet) {
		if (remote_any) {
			save2file_A_prerouting(
				"-p tcp -d %s --dport %s -j DNAT --to-destination %s:23",
				wanaddr, nvram_safe_get("telnet_wanport"),
				lan_ip);
		} else {
			sscanf(remote_ip, "%s %s", from, to);
			...
			{
				save2file_A_prerouting(
					"-p tcp -s %s -d %s --dport %s -j DNAT --to-destination %s:23",
					var, wanaddr,
	 ...
	 */
	save2file_A_prerouting("-p icmp -d %s -j DNAT --to-destination %s",
			       wanaddr, lan_ip);
	...
	if (nvram_matchi("remote_upgrade", 1))
		save2file_A_prerouting(
			"-p udp -d %s --dport %d -j DNAT --to-destination %s",
	...
	*suspense = 0;
...
#endif
		parse_spec_forward(wanface, wanaddr,
				   nvram_safe_get("forward_spec"));
		parse_port_forward(wanface, wanaddr, lan_cclass,
				   nvram_safe_get("forward_port"), dmzenable);
		/*
		 * DD-WRT addition by Eric Sauvageau 
		 */
		save2file_A_prerouting("-d %s -j TRIGGER --trigger-type dnat",
				       wanaddr);
		/*
		 * DD-WRT addition end 
		 */
	 ...
	 */
	if (dmzenable)
		save2file_A_prerouting("-d %s -j DNAT --to-destination %s%s",
				       wanaddr, lan_cclass,
				       nvram_safe_get("dmz_ipaddr"));
}
...
static void nat_postrouting(char *wanface, char *wanaddr, char *vifs)
{
	...
	if (has_gateway()) {
		...
		if (nvram_matchi("dtag_vlan8", 1) &&
			...
			save2file_A_postrouting("-o %s -j SNAT --to-source %s",
		...
		if (*wanface && wanactive(wanaddr) &&
			...
			save2file_A_postrouting(
				"-s %s/%d -o %s -j SNAT --to-source %s",
				nvram_safe_get("lan_ipaddr"), loopmask, wanface,
				wanaddr);
...
*/
		...
		if ((nvram_matchi("block_loopback", 0) ||
			...
			if (strcmp(wanface, var) &&
				...
				if (isstandalone(var)) {
					...
					if (nvram_matchi(nat, 1)) {
						...
						char *block_loopback = nvram_default_get(
							bl,
							nvram_safe_get(
								"block_loopback"));
						/* todo: block/allow loopback per interface */
						if (!strcmp(block_loopback,
							    "0") ||
						    nvram_match("filter",
								"off")) {
							save2file_A_postrouting(
								"-o %s -m pkttype --pkt-type broadcast -j RETURN",
								var);
							save2file_A_postrouting(
								"-o %s -s %s/%d -d %s/%d -j MASQUERADE",
								var,
								nvram_nget(
									"%s_ipaddr",
									var),
...
//              }
		...
		if (*wanface && wanactive(wanaddr))
			if (nvram_matchi("wl_br1_enable", 1))
				save2file_A_postrouting(
					"-o %s -j SNAT --to-source %s", wanface,
					wanaddr);
	}
}
...
static void parse_port_filter(char *lanface, char *wordlist)
{
	...
	foreach(var, wordlist, next)
	{
		...
		if (!strcmp(protocol, "tcp") || !strcmp(protocol, "both")) {
			save2file_A_forward("-i %s -p tcp --dport %s:%s -j %s",
					    lanface, lan_port0, lan_port1,
					    log_drop);
		}
		if (!strcmp(protocol, "udp") || !strcmp(protocol, "both")) {
			save2file_A_forward("-i %s -p udp --dport %s:%s -j %s",
					    lanface, lan_port0, lan_port1,
					    log_drop);
		}
	}
...
static int schedule_by_tod(FILE *cfd, int seq)
{
	...
	if (strcmp(todvalue, "0:0 23:59 0-0") == 0 ||
		...
		if (strcmp(todvalue, "0:0 23:59") == 0)
			allday = 1;
		if (sscanf(todvalue, "%d:%d %d:%d %s", &hr_st, &mi_st, &hr_end,
			   &mi_end, wday) != 5)
	...
	if (allday) { /* 24-hour, but not everyday */
		...
		while (token != NULL) {
			...
			if (rotate == 1 && st == 0)
				snprintf(wday_end + strlen(wday_end),
					 sizeof(wday_end) - strlen(wday_end),
					 ",%d", end);
			else if (rotate == 1 && end == 6)
				snprintf(wday_st + strlen(wday_st),
					 sizeof(wday_st) - strlen(wday_st),
					 ",%d", st);
			else {
				snprintf(wday_st + strlen(wday_st),
					 sizeof(wday_st) - strlen(wday_st),
...
static void macgrp_chain(int seq, int urlenable, char *iflist, char *target)
{
	...
	foreach(var, wordlist, next)
	{
		...
		if (iflist) {
			...
			{
				save2file_A(
					"grp_%d -i %s -m mac --mac-source %s -j %s",
					seq, ifname, var, target);
				save2file_A(
					"grp_%d -i %s -m mac --mac-destination %s -j %s",
					seq, ifname, var, target);
			}
		} else {
			save2file_A("grp_%d -m mac --mac-source %s -j %s", seq,
				    var, target);
			save2file_A("grp_%d -m mac --mac-destination %s -j %s",
				    seq, var, target);
		}
	}
}

static void ipgrp_chain(char *lan_cclass, int seq, int urlenable, char *iflist,
			char *target)
{
	char buf[256];
	...
	foreach(var1, wordlist1, next1)
	{
		...
		foreach(var2, wordlist2, next2)
		{
			...
			if (iflist) {
				foreach(ifname, iflist, nextif)
				{
					save2file_A("grp_%d -i %s -s %s -j %s",
						    seq, ifname, var2, target);
					save2file_A("grp_%d -i %s -d %s -j %s",
						    seq, ifname, var2, target);
				}

			} else {
				save2file_A("grp_%d -s %s -j %s", seq, var2,
					    target);
				save2file_A("grp_%d -d %s -j %s", seq, var2,
					    target);
			}
		}
	}
		...
		if (iflist) {
			foreach(ifname, iflist, nextif)
			{
				if (!strcmp(protocol, "tcp") ||
					...
					save2file_A(
						"grp_%d -i %s -p tcp --dport %s:%s -j %s",
						seq, ifname, lan_port0,
						lan_port1, target);
				}
				if (!strcmp(protocol, "udp") ||
				    !strcmp(protocol, "both")) {
					save2file_A(
						"grp_%d -i %s -p udp --dport %s:%s -j %s",
						seq, ifname, lan_port0,
						lan_port1, target);
				}
			}
			...
			if (!strcmp(protocol, "tcp") ||
			    !strcmp(protocol, "both")) {
				save2file_A("grp_%d -p tcp --dport %s:%s -j %s",
					    seq, lan_port0, lan_port1, target);
			}
			if (!strcmp(protocol, "udp") ||
			    !strcmp(protocol, "both")) {
				save2file_A("grp_%d -p udp --dport %s:%s -j %s",
					    seq, lan_port0, lan_port1, target);
			}
		}
	}
...
static struct TELEMETRY ubnt_telemetry[] = {
	{ 34, 212, 233, 218, 32 }, { 34, 215, 113, 32, 32 },
	{ 34, 216, 161, 237, 32 }, { 35, 155, 204, 51, 32 },
	{ 44, 231, 80, 1, 32 },	   { 44, 235, 182, 105, 32 },
	{ 44, 241, 144, 120, 32 }, { 52, 24, 128, 121, 32 },
	{ 52, 35, 87, 198, 32 },   { 52, 34, 106, 102, 32 },
	{ 52, 43, 249, 150, 32 },  { 54, 69, 31, 183, 32 },
	{ 54, 187, 15, 73, 32 },   { 54, 201, 77, 117, 32 },
	{ 54, 202, 181, 132, 32 },
};
...
static void advgrp_chain(int seq, int urlenable, char *ifname)
{
	...
	split(word, wordlist, next, delim)
	{
		split(srv, services, next2, delim)
		{
			...
			if (sscanf(name, "$NAME:%3d:", &len) != 1 ||
			    strlen(word) != len)
				continue;
			if (memcmp(name + sizeof("$NAME:nnn:") - 1, word,
				   len) != 0)
				continue;
			...
			if (!strcmp(protocol, "tcp") ||
			    !strcmp(protocol, "both"))
				save2file_A("advgrp_%d -p tcp --dport %s -j %s",
					    seq, ports, log_drop);
			if (!strcmp(protocol, "udp") ||
			    !strcmp(protocol, "both"))
				save2file_A("advgrp_%d -p udp --dport %s -j %s",
					    seq, ports, log_drop);
			if (!strcmp(protocol, "icmp"))
				save2file_A("advgrp_%d -p icmp -j %s", seq,
					    log_drop);
			nvram_seti("dnsmasq_telemetry", 0);
			if (!strcmp(realname, "windows-telemetry")) {
				nvram_seti("dnsmasq_ms_telemetry", 1);
				int i;
				for (i = 0; i < sizeof(ms_telemetry) /
							sizeof(ms_telemetry[0]);
				     i++)
					save2file_A(
						"advgrp_%d -d %d.%d.%d.%d/%d -j %s",
						seq, ms_telemetry[i].ip1,
						ms_telemetry[i].ip2,
						ms_telemetry[i].ip3,
						ms_telemetry[i].ip4,
				...
				int i;
				for (i = 0;
				     i < sizeof(ubnt_telemetry) /
						 sizeof(ubnt_telemetry[0]);
				     i++)
					save2file_A(
						"advgrp_%d -d %d.%d.%d.%d -j %s",
						seq, ubnt_telemetry[i].ip1,
						ubnt_telemetry[i].ip2,
						ubnt_telemetry[i].ip3,
						ubnt_telemetry[i].ip4,
						log_drop);
				...
				save2file_A(
					"advgrp_%d -m layer7 --l7proto %s -j %s",
				...
				if (risk && dep) {
					save2file_A(
						"advgrp_%d -m ndpi --proto %s --risk %d -j %s",
						seq, dep, risk, log_drop);
				}
			}
#endif
				...
				if (proto) //avoid null pointer, if realname isnt matched
				{
					...
					if (!strcmp(proto, "bit")) {
						...
						insmod("xt_ndpi");
						save2file_A(
							"advgrp_%d -m ndpi --proto bittorrent -j %s",
							seq, log_drop);
#else
						insmod("ipt_layer7 xt_layer7");
#ifdef HAVE_MICRO
						save2file_A(
							"advgrp_%d -m layer7 --l7proto bt -j %s",
							seq, log_drop);
#else
						save2file_A(
							"advgrp_%d -m length --length 0:550 -m layer7 --l7proto bt -j %s",
							seq, log_drop);
#endif
						save2file_A(
							"advgrp_%d -m layer7 --l7proto bt1 -j %s",
							seq, log_drop);
						save2file_A(
							"advgrp_%d -m layer7 --l7proto bt2 -j %s",
							seq, log_drop);
#endif
					}
				}
	...
	if (dpi_collect) {
		insmod("xt_ndpi");
		save2file_A("advgrp_%d -m ndpi --proto %s -j %s", seq,
			    dpi_collect, log_drop);
		free(dpi_collect);
	}
#endif
	/*
	...
	if (nvram_nmatch("1", "filter_p2p_grp%d", seq)) {
		...
		insmod("xt_ndpi");
		/*commonly used protocols, decending */
		save2file_A("advgrp_%d -m ndpi --proto bittorrent -j %s", seq,
			    log_drop);
		save2file_A("advgrp_%d -m ndpi --proto edonkey -j %s", seq,
			    log_drop);
		/*atm rarly used protocols */
		...
		save2file_A("advgrp_%d -m ndpi --proto gnutella -j %s", seq,
			    log_drop);
...
//              save2file_A("advgrp_%d -p tcp -m ndpi --winmx -j %s", seq, log_drop);
#else
#ifdef HAVE_MICRO
		save2file_A("advgrp_%d -m layer7 --l7proto bt -j %s", seq,
			    log_drop);
#else
		save2file_A(
			"advgrp_%d -m length --length 0:550 -m layer7 --l7proto bt -j %s",
		...
		save2file_A("advgrp_%d  -p tcp -m layer7 --l7proto ares -j %s",
			    seq, log_drop);
#ifndef HAVE_OPENDPI
		save2file_A("advgrp_%d -m layer7 --l7proto bt4 -j %s", seq,
			    log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto bt1 -j %s", seq,
			    log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto bittorrent -j %s",
			    seq, log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto bt2 -j %s", seq,
			    log_drop);
		save2file_A(
			"advgrp_%d -p tcp -m layer7 --l7proto gnutella -j %s",
			seq, log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto applejuice -j %s",
			    seq, log_drop);
		save2file_A(
			"advgrp_%d -p tcp -m layer7 --l7proto directconnect -j %s",
			seq, log_drop);
		save2file_A(
			"advgrp_%d -p tcp -m layer7 --l7proto soulseek -j %s",
			seq, log_drop);
		save2file_A("advgrp_%d -p tcp -m layer7 --l7proto openft -j %s",
			    seq, log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto fasttrack -j %s",
			    seq, log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto imesh -j %s", seq,
			    log_drop);
#endif
		save2file_A("advgrp_%d -m layer7 --l7proto audiogalaxy -j %s",
			    seq, log_drop);
		save2file_A(
			"advgrp_%d -p tcp -m layer7 --l7proto bearshare -j %s",
			seq, log_drop);
		/* atm rarly used protocols */
		save2file_A("advgrp_%d -m layer7 --l7proto edonkey -j %s", seq,
			    log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto freenet -j %s", seq,
			    log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto gnucleuslan -j %s",
			    seq, log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto goboogy -j %s", seq,
			    log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto hotline -j %s", seq,
			    log_drop);
		/*	 	save2file_A("advgrp_%d -m layer7 --l7proto kugoo -j %s", seq, log_drop);// xunlei, kugoo, winmx block websurfing */
		save2file_A("advgrp_%d -p tcp -m layer7 --l7proto mute -j %s",
			    seq, log_drop);
		save2file_A(
			"advgrp_%d -p tcp -m layer7 --l7proto napster -j %s",
			seq, log_drop);
		save2file_A(
			"advgrp_%d -p tcp -m layer7 --l7proto soribada -j %s",
			seq, log_drop);
		save2file_A("advgrp_%d -m layer7 --l7proto tesla -j %s", seq,
			    log_drop);
	...
	if (wordlist && strcmp(wordlist, "")) {
		insmod("ipt_webstr");
		save2file_A("advgrp_%d -p tcp -m webstr --host \"%s\" -j %s",
		...
		while ((wordlist - pos) < len &&
		       (next = strstr(wordlist, "<&nbsp;>"))) {
			bzero(var, 256);
			...
			if (strstr(var, "https://"))
				offset = 8;
			save2file_A(
				"advgrp_%d -p tcp -m string --string \"%s\" --algo bm --from 1 --to 600 -j %s",
	...
	if (wordlist && strcmp(wordlist, "")) {
		insmod("ipt_webstr");
		save2file_A("advgrp_%d -p tcp -m webstr --url \"%s\" -j %s",
			    seq, wordlist, log_reject);
...
static void parse_trigger_out(char *wordlist)
{
	...
	foreach(var, wordlist, next)
	{
		...
		if (!strcmp(proto, "tcp") || !strcmp(proto, "udp")) {
			save2file_A(
				"trigger_out -p %s -m %s --dport %s:%s "
				"-j TRIGGER --trigger-type out --trigger-proto %s --trigger-match %s-%s --trigger-relate %s-%s",
				proto, proto, wport0, wport1, proto, wport0,
				wport1, lport0, lport1);
	...
	foreach(word, wordlist, next)
	{
		...
		if (ifexists(tag)) {
			if (nvram_exists(ipaddr) && nvram_exists(netmask) &&
				...
				eval("ifconfig", tag, nvram_safe_get(ipaddr),
				     "netmask", nvram_safe_get(netmask), "up");
			} else {
				eval("ifconfig", tag, "up");
			}
			if (forward && wanface && *wanface) {
				save2file_A_forward("-i %s -o %s -j %s", tag,
						    wanface, log_accept);
			} else {
				if (!strcmp(chain, "OUTPUT")) {
					save2file_A("%s -o %s -j %s", chain,
						    tag, log_accept);
				} else {
					save2file_A("%s -i %s -j %s", chain,
						    tag, log_accept);
				}
			}
		}
...
static void filter_input(char *wanface, char *lanface, char *wanaddr,
			 int remotessh, int remotetelnet, int remotemanage,
			 char *vifs)
{
	 ...
	 */
	save2file_A_input("-m state --state RELATED,ESTABLISHED -j %s",
			  log_accept);
	...
	if (nvram_matchi("dtag_vlan8", 1) && nvram_matchi("wan_vdsl", 1)) {
		save2file_A_input("-i %s -j %s", nvram_safe_get("tvnicfrom"),
				  log_accept);
	}
#ifdef HAVE_PPTP
	/*
	...
	if (nvram_matchi("pptpd_enable", 1)) {
		if (nvram_matchi("limit_pptp", 1)) {
			save2file_A_input("-i %s -p tcp --dport %d -j logbrute",
					  wanface, PPTP_PORT);
			/*
	...
	if (nvram_match("wan_proto", "dhcp") ||
	    nvram_match("wan_proto", "dhcp_auth"))
		save2file_A_input("-i %s -p udp --sport 67 --dport 68 -j %s",
				  wanface, log_accept);
	if (nvram_matchi("pptpd_enable", 1) ||
	    nvram_matchi("pptpd_client_enable", 1) ||
	    nvram_match("wan_proto", "pptp")) {
		save2file_A_input("-i %s -p tcp --dport %d -j %s", wanface,
				  PPTP_PORT, log_accept);
		save2file_A_input("-i %s -p 47 -j %s", wanface, log_accept);
		if (nvram_matchi("pptpd_lockdown", 1)) {
			save2file_A_input(
				"-i %s -p udp --sport 67 --dport 68 -j %s",
				lanface, log_accept);
	...
	if (nvram_matchi("proftpd_enable", 1) &&
		...
		if (nvram_matchi("limit_ftp", 1)) {
			save2file_A_input("-i %s -p tcp --dport %s -j logbrute",
					  wanface,
					  nvram_safe_get("proftpd_port"));
			save2file_A_input("-i %s -p tcp --dport %s -j %s",
					  wanface,
					  nvram_safe_get("proftpd_port"),
					  log_accept);
		} else {
	...
	if (nvram_matchi("lighttpd_enable", 1) &&
	    nvram_matchi("lighttpd_wan", 1)) {
		save2file_A_input("-i %s -p tcp --dport %s -j %s", wanface,
				  nvram_safe_get("lighttpd_port"), log_accept);
		save2file_A_input("-i %s -p tcp --dport %s -j %s", wanface,
	...
	if (nvram_matchi("openvpn_enable", 1)) {
		...
		if (nvram_match("openvpn_proto", "udp") ||
		    nvram_match("openvpn_proto", "udp4") ||
		    nvram_match("openvpn_proto", "udp6")) {
			//if (nvhas("openvpn_proto", "udp")) {
			save2file_A_input("-i %s -p udp --dport %s -j %s",
					  wanface,
					  nvram_safe_get("openvpn_port"),
					  log_accept);
		}
		if (nvram_match("openvpn_proto", "tcp-server") ||
		    nvram_match("openvpn_proto", "tcp4-server") ||
		...
		if (nvram_match("openvpn_tuntap", "tun")) {
			save2file_A_input("-i %s2 -j %s",
					  nvram_safe_get("openvpn_tuntap"),
					  log_accept);
			save2file_A_forward("-i %s2 -j %s",
					    nvram_safe_get("openvpn_tuntap"),
					    log_accept);
		}
	}
	...
	if (wanactive(wanaddr)) {
		if (nvram_invmatchi("dr_wan_rx", 0))
			save2file_A_input("-p udp -i %s --dport %d -j %s",
	...
	if (nvram_invmatchi("dr_lan_rx", 0))
		save2file_A_input("-p udp -i %s --dport %d -j %s", lanface,
				  RIP_PORT, log_accept);
	/*else
	...
	{
		save2file_A_input("-p udp -i %s --dport %d -j %s", buff,
				  RIP_PORT, log_drop);
	}
	...
	if (nvram_match("wk_mode", "bgp"))
		save2file_A_input("-i %s -p tcp --dport 179 -j %s", wanface,
				  log_accept);
#ifdef HAVE_OLSRD
	if (nvram_match("wk_mode", "olsr"))
		save2file_A_input("-i %s -p udp --dport 698 -j %s", wanface,
				  log_accept);
#endif
	/*
	 ...
	 */
	if (remotemanage) {
		save2file_A_input("-i %s -p tcp -d %s --dport %d -j %s",
				  wanface, nvram_safe_get("lan_ipaddr"),
				  web_lanport, log_accept);
	}
#ifdef HAVE_SSHD
	/*
	...
	if (remotessh) {
		if (nvram_matchi("limit_ssh", 1))
			save2file_A_input(
				"-i %s -p tcp -d %s --dport %s -j logbrute",
				wanface, nvram_safe_get("lan_ipaddr"),
				nvram_safe_get("sshd_port"));
	}
#endif
	 ...
	 */
	if (remotessh) {
		save2file_A_input("-i %s -p tcp -d %s --dport %s -j %s",
				  wanface, nvram_safe_get("lan_ipaddr"),
	...
	if (remotetelnet) {
		if (nvram_matchi("limit_telnet", 1))
			save2file_A_input(
				"-i %s -p tcp -d %s --dport 23 -j logbrute",
				wanface, nvram_safe_get("lan_ipaddr"));
	}
#endif
	if (remotetelnet) {
		save2file_A_input("-i %s -p tcp -d %s --dport 23 -j %s",
				  wanface, nvram_safe_get("lan_ipaddr"),
				  log_accept);
	}
#endif
	...
	if (wanactive(wanaddr)) {
		if (nvram_invmatch("filter", "off"))
			save2file_A_input("-i %s -p icmp -j %s", wanface,
					  nvram_matchi("block_wan", 1) ?
						  log_drop :
						  log_accept);
		else
			save2file_A_input("-i %s -p icmp -j %s", wanface,
	 ...
	 */
	save2file_A_input("-i %s -p igmp -j %s", wanface,
			  doMultiCast() == 0 ? log_drop : log_accept);
#ifdef HAVE_UDPXY
	if (wanactive(wanaddr) && nvram_matchi("udpxy_enable", 1) &&
	    nvram_exists("tvnicfrom"))
		save2file_A_input("-i %s -p udp -d %s -j %s",
				  nvram_safe_get("tvnicfrom"), IP_MULTICAST,
				  log_accept);
#endif
#ifndef HAVE_MICRO
	/*
	 ...
	 */
	if (nvram_matchi("snmpd_enable", 1) &&
	    (nvram_matchi("block_snmp", 0) || nvram_match("filter", "off"))) {
		save2file_A_input("-i %s -p udp --dport 161 -j %s", wanface,
				  log_accept);
	}
#endif
	 ...
	 */
	if (nvram_matchi("remote_upgrade", 1))
		save2file_A_input("-i %s -p udp --dport %d -j %s", wanface,
				  TFTP_PORT, log_accept);
#endif
#ifdef HAVE_MILKFISH
	if (*wanface && nvram_matchi("milkfish_enabled", 1))
		save2file_A_input("-p udp -i %s --dport 5060 -j %s", wanface,
				  log_accept);
	...
	if (nvram_matchi("vncr_enable", 1) && *wanface) {
		save2file_A_input("-p tcp -i %s --dport 5900 -j %s", wanface,
				  log_accept);
		save2file_A_input("-p tcp -i %s --dport 5500 -j %s", wanface,
				  log_accept);
	}
#endif
#ifdef HAVE_TOR
	if (nvram_matchi("tor_enable", 1)) {
		if (nvram_matchi("tor_relay", 1))
			save2file_A_input("-p tcp -i %s --dport 9001 -j %s",
					  wanface, log_accept);
		if (nvram_matchi("tor_dir", 1))
			save2file_A_input("-p tcp -i %s --dport 9030 -j %s",
					  wanface, log_accept);
	}
#endif
	/*
	...
	save2file_A_input("-i %s -m state --state NEW -j %s", lanface,
			  log_accept);
	/*
	...
	foreach(buff, iflist, next)
	{
		save2file_A_input("-i %s -m state --state NEW -j %s", buff,
				  log_accept);
	}
	char var[80];
	// char *vifs = nvram_safe_get ("lan_ifnames");
	...
	foreach(var, vifs, next)
	{
		if (strcmp(safe_get_wan_face(wan_if_buffer), var) &&
			...
			if (nvram_nmatch("1", "%s_isolation", var)) {
				save2file_A_input(
					"-i %s -p udp --dport 67 -j %s", var,
					log_accept);
				save2file_A_input(
					"-i %s -p udp --dport 53 -j %s", var,
					log_accept);
				save2file_A_input(
					"-i %s -p tcp --dport 53 -j %s", var,
					log_accept);
				save2file_A_input(
					"-i %s -m state --state NEW -j %s", var,
					log_drop);
			}
			if (isstandalone(var)) {
				save2file_A_input("-i %s -j %s", var,
						  log_accept);
			}
		}
	}
...
static void filter_forward(char *wanface, char *lanface, char *lan_cclass,
			   int dmzenable, int webfilter, char *vifs)
{
	char wan_if_buffer[33];
	...
	if (nvram_matchi("openvpncl_enable", 1)) {
		...
		if (nvram_matchi("openvpncl_killswitch", 1) &&
		    nvram_invmatch("openvpncl_spbr", "1")) {
			...
			dd_loginfo(
				"openvpn",
				"General Killswitch for OpenVPN enabled via Firewall");
		}
	}
#endif
	...
	foreach(var, vifs, next)
	{
		if (strcmp(safe_get_wan_face(wan_if_buffer), var) &&
			...
			if (nvram_nmatch("1", "%s_isolation", var)) {
				save2file_A_forward(
					"-i %s -d %s/%s -m state --state NEW -j %s",
	...
	if (!filter_host_url)
		save2file_A_forward(
			"-m state --state RELATED,ESTABLISHED -j %s",
			log_accept);
	...
	if (nvram_matchi("filter_invalid", 1)) {
		save2file_A_forward(
			"! -s %s -o %s -p tcp -m state --state INVALID -j %s",
			nvram_safe_get("wan_ipaddr"), wanface, log_drop);
	}

	save2file_A_forward("-j upnp");
	if (nvram_matchi("dtag_vlan8", 1) && nvram_matchi("wan_vdsl", 1)) {
		save2file_A_forward("-i %s -j %s", nvram_safe_get("tvnicfrom"),
				    log_accept);
		save2file_A_forward("-o %s -j %s", nvram_safe_get("tvnicfrom"),
				    log_accept);
	}
	...
	if (nvram_invmatch("filter", "off") && webfilter && *wanface) {
		insmod("ipt_webstr");
		save2file_A_forward(
			"-i %s -o %s -p tcp -m webstr --content %d -j %s",
	...
	if (nvram_matchi("wl0_br1_enable", 1)) {
		if (nvram_matchi("wl0_br1_nat", 1)) {
			...
			save2file_A_forward(
				"-o br0 -i br1 -m state --state ESTABLISHED,RELATED -j %s",
				log_accept);
		}
		 ...
		 */
			...
			save2file_A_forward(
				"-i br0 -o br1 -m state --state ESTABLISHED,RELATED -j %s",
				log_accept);
		}
		/*
		 * Sveasoft mod - default for br1/separate subnet WDS type 
		...
		if (wan && *wan)
			save2file_A_forward("-i br1 -o %s -j %s", wan,
					    log_accept);
	}
	...
	if (nvram_invmatch("filter", "off") && *wanface) {
		if (nvram_matchi("pptp_pass", 1)) {
			if (*wanface) {
				save2file_A_forward(
					"-o %s -s %s/%d -p tcp --dport %d -j %s",
					wanface, nvram_safe_get("lan_ipaddr"),
					getmask(nvram_safe_get("lan_netmask")),
					PPTP_PORT, log_accept);
				save2file_A_forward(
					"-o %s -s %s/%d -p gre -j %s", wanface,
					nvram_safe_get("lan_ipaddr"),
					getmask(nvram_safe_get("lan_netmask")),
					log_accept);
			}
	...
	if (nvram_matchi("dtag_vlan8", 1) && nvram_matchi("wan_vdsl", 1)) {
		if (doMultiCast() > 0)
			save2file_A_forward(
				"-i %s -p udp --destination %s -j %s",
				nvram_safe_get("tvnicfrom"), IP_MULTICAST,
				log_accept);
#ifdef HAVE_PPTP
	} else if (nvram_match("wan_proto", "pptp") &&
		   nvram_matchi("pptp_iptv", 1) && nvram_exists("tvnicfrom")) {
		if (doMultiCast() > 0)
			save2file_A_forward(
				"-i %s -p udp --destination %s -j %s",
				nvram_safe_get("tvnicfrom"), IP_MULTICAST,
		...
		if (doMultiCast() > 0 && *wanface)
			save2file_A_forward(
				"-i %s -p udp --destination %s -j %s", wanface,
				IP_MULTICAST, log_accept);
	}
	/*
	 ...
	 */
	if (*wanface) {
		save2file_A_forward("-i %s -o %s -j TRIGGER --trigger-type in",
				    wanface, lanface);
	 ...
	 */
	if (dmzenable)
		save2file_A_forward("-o %s -d %s%s -j %s", lanface, lan_cclass,
	...
	foreach(var, vifs, next)
	{
		if (strcmp(safe_get_wan_face(wan_if_buffer), var) &&
			...
			if (nvram_nmatch("1", "%s_isolation", var)) {
				save2file_A_forward(
					"-i br0 -o %s -m state --state NEW -j %s",
					var, log_drop);
				if (nvram_matchi("privoxy_transp_enable", 1)) {
					save2file(
						"-I INPUT -i %s -d %s/%s -p tcp --dport 8118 -j %s",
						var,
						nvram_safe_get("lan_ipaddr"),
						nvram_safe_get("lan_netmask"),
						log_accept);
			...
			if (*wanface) {
				save2file_A_forward(
					"-i %s -o %s -j TRIGGER --trigger-type in",
					wanface, var);
				save2file_A_forward("-i %s -j trigger_out",
						    var);
				save2file_A_forward(
					"-i %s -m state --state NEW -j %s", var,
					log_accept);
	 ...
	 */
	save2file_A_forward("-i %s -m state --state NEW -j %s", lanface,
			    log_accept);
...
static void mangle_table(char *wanface, char *wanaddr, char *vifs)
{
	...
	if (nvram_matchi("wan_priority", 1) && isvlan(wanface)) {
		...
		save2file_A_prerouting("-i %s -j MARK --set-mark 0x100000",
				       wanface);
		save2file_A_postrouting("-o %s -j MARK --set-mark 0x100000",
					wanface);
		save2file_A_postrouting(
			"-m mark --mark 0x100000 -j CLASSIFY --set-class 0:1");
		save2file_A_postrouting(
			"-o %s -p udp --dport 67 -j CLASSIFY --set-class 0:0",
			wanface);

		eval_silenceip6("ip6tables", "-t", "mangle", "-D", "PREROUTING",
				"-i", wanface, "-j", "MARK", "--set-mark",
				"0x100000");
		eval_silenceip6("ip6tables", "-t", "mangle", "-A", "PREROUTING",
				"-i", wanface, "-j", "MARK", "--set-mark",
				"0x100000");
		eval_silenceip6("ip6tables", "-t", "mangle", "-D",
				"POSTROUTING", "-o", wanface, "-j", "MARK",
				"--set-mark", "0x100000");
		eval_silenceip6("ip6tables", "-t", "mangle", "-A",
				"POSTROUTING", "-o", wanface, "-j", "MARK",
				"--set-mark", "0x100000");
		eval_silenceip6("ip6tables", "-t", "mangle", "-D",
				"POSTROUTING", "-m", "mark", "--mark",
				"0x100000", "-j", "CLASSIFY", "--set-class",
				"0:1");
		eval_silenceip6("ip6tables", "-t", "mangle", "-A",
				"POSTROUTING", "-m", "mark", "--mark",
				"0x100000", "-j", "CLASSIFY", "--set-class",
				"0:1");
		eval_silenceip6("ip6tables", "-t", "mangle", "-D",
				"POSTROUTING", "-o", wanface, "-p", "udp",
				"--dport", "547", "-j", "CLASSIFY",
				"--set-class", "0:0");
		eval_silenceip6("ip6tables", "-t", "mangle", "-A",
				"POSTROUTING", "-o", wanface, "-p", "udp",
				"--dport", "547", "-j", "CLASSIFY",
	...
	if (nvram_matchi("filter_tos", 1)) {
		...
		if (!nvram_matchi("wan_priority", 1) || !isvlan(wanface)) {
			save2file_A_prerouting(
				"-i %s -j MARK --set-mark 0x100000", wanface);
			save2file_A_postrouting(
				"-o %s -j MARK --set-mark 0x100000", wanface);
		}
		save2file_A_postrouting(
			"-m mark --mark 0x100000 -j TOS --set-tos 0x00");
		eval_silenceip6("ip6tables", "-t", "mangle", "-D",
				"POSTROUTING", "-m", "mark", "--mark",
				"0x100000", "-j", "TOS", "--set-tos", "0x00");
		eval_silenceip6("ip6tables", "-t", "mangle", "-A",
				"POSTROUTING", "-m", "mark", "--mark",
				"0x100000", "-j", "TOS", "--set-tos", "0x00");
	}
	...
	if ((nvram_matchi("privoxy_enable", 1)) &&
	    (nvram_matchi("wshaper_enable", 1))) {
		save2file("-I OUTPUT -p tcp --sport 8118 -j IMQ --todev 0");
		eval_silenceip6("ip6tables", "-t", "mangle", "-D", "OUTPUT",
				"-p", "tcp", "--sport", "8118", "-j", "IMQ",
				"--todev", "0");
		eval_silenceip6("ip6tables", "-t", "mangle", "-I", "OUTPUT",
				"-p", "tcp", "--sport", "8118", "-j", "IMQ",
				"--todev", "0");
	}
#endif
		      ...
		      int remotemanage, char *vifs)
{
	...
	if (wanactive(wanaddr)) {
		nat_prerouting(wanface, wanaddr, lan_cclass, dmzenable,
			       remotessh, remotetelnet, remotemanage, vifs);
 ...
 *      Filter table
 */
static void filter_table(char *wanface, char *lanface, char *wanaddr,
			 char *lan_cclass, int dmzenable, int webfilter,
			 int remotessh, int remotetelnet, int remotemanage,
			 char *vifs)
{
	char wan_if_buffer[33];
	int log_level =
		nvram_matchi("log_enable", 1) ? nvram_geti("log_level") : 0;
...
#ifndef HAVE_MICRO
	if ((nvram_matchi("limit_telnet", 1)) ||
	    (nvram_matchi("limit_pptp", 1)) || (nvram_matchi("limit_ssh", 1)) ||
	    (nvram_matchi("limit_ftp", 1))) {
		save2file(":logbrute - [0:0]");
		save2file_A(
			"logbrute -m recent --set --name BRUTEFORCE --rsource");
		save2file_A(
			"logbrute -m recent ! --update --seconds 60 --hitcount 4 --name BRUTEFORCE --rsource -j RETURN");
		// -m limit rule is a fallback in case -m recent isn't included in a build
		save2file_A(
			"logbrute -m limit --limit 1/min --limit-burst 1 -j RETURN");
		if ((nvram_matchi("log_enable", 1)) &&
		    (nvram_matchi("log_dropped", 1)))
			save2file_A(
				"logbrute -j LOG --log-prefix \"[DROP BRUTEFORCE] : \" --log-tcp-options --log-ip-options");
	...
	if (wanactive(wanaddr)) {
		...
		if (nvram_match("filter", "off") || !has_gateway()) {
			...
			if (!remotemanage && *wanface) {
				save2file_A_input(
					"-p tcp -i %s --dport %s -j %s",
					wanface, nvram_safe_get("http_wanport"),
					log_drop);
				save2file_A_input(
					"-p tcp -i %s --dport 80 -j %s",
					wanface, log_drop);
				save2file_A_input(
					"-p tcp -i %s --dport 443 -j %s",
					wanface, log_drop);
				save2file_A_input(
					"-p tcp -i %s --dport 69 -j %s",
					wanface, log_drop);
			}
			/*
			...
			if (!remotetelnet && *wanface) {
				save2file_A_input(
					"-p tcp -i %s --dport 23 -j %s",
					wanface, log_drop);
			}
#endif
			filter_forward(wanface, lanface, lan_cclass, dmzenable,
				       webfilter, vifs);
		} else {
			filter_input(wanface, lanface, wanaddr, remotessh,
				     remotetelnet, remotemanage, vifs);
		...
		foreach(var, vifs, next)
		{
			if (strcmp(safe_get_wan_face(wan_if_buffer), var) &&
				...
				if (nvram_nmatch("1", "%s_isolation", var)) {
					save2file_A_input(
						"-i %s -p udp --dport 67 -j %s",
						var, log_accept);
					save2file_A_input(
						"-i %s -p udp --dport 53 -j %s",
						var, log_accept);
					save2file_A_input(
						"-i %s -p tcp --dport 53 -j %s",
						var, log_accept);
					save2file_A_input(
						"-i %s -m state --state NEW -j %s",
						var, log_drop);
				}
				if (isstandalone(var)) {
					save2file_A_input("-i %s -j %s", var,
	...
	if (log_level > 0) {
		...
		if (nvram_matchi("log_accepted", 1))
			save2file_A(
				...
				wanface, FLOOD_RATE);
		save2file_A(
			"logaccept -i %s -m state --state NEW -m limit --limit %s -j %s",
...
#endif
		save2file_A(
			"logreject -p tcp -j REJECT --reject-with tcp-reset");
#ifdef FLOOD_PROTECT
		/*
		 * limaccept chain 
		 */
...
#endif
		save2file_A(
			"limaccept -i %s -m state --state NEW -m limit --limit %s -j %s\n-A limaccept -j ACCEPT",
			wanface, FLOOD_RATE, log_drop);
#endif
	}
	save2file("COMMIT");
...
static void create_restore_file(char *wanface, char *lanface, char *wanaddr,
				char *lan_cclass, int dmzenable, int webfilter,
				int remotessh, int remotetelnet,
				int remotemanage, char *vifs)
{
	mangle_table(wanface, wanaddr, vifs);
	nat_table(wanface, wanaddr, lan_cclass, dmzenable, remotessh,
		  remotetelnet, remotemanage, vifs);
	filter_table(wanface, lanface, wanaddr, lan_cclass, dmzenable,
		     webfilter, remotessh, remotetelnet, remotemanage, vifs);
}
...
void set_gprules(char *iface)
{
	...
	if (nvram_exists(gport)) {
		...
		sysprintf(
			"iptables -I INPUT -i %s -d %s/%s -m state --state NEW -j DROP",
			giface, nvram_safe_get("lan_ipaddr"),
			nvram_safe_get("lan_netmask"));
		sysprintf(
			"iptables -I INPUT -i %s -d %s/255.255.255.255 -m state --state NEW -j DROP",
			giface, gipaddr);
		sysprintf(
			"iptables -I INPUT -i %s -d %s/255.255.255.255 -p udp --dport 67 -j %s",
			giface, gipaddr, "ACCEPT");
		sysprintf(
			"iptables -I INPUT -i %s -d %s/255.255.255.255 -p udp --dport 53 -j %s",
			giface, gipaddr, "ACCEPT");
		sysprintf(
			"iptables -I INPUT -i %s -d %s/255.255.255.255 -p tcp --dport 53 -j %s",
			giface, gipaddr, "ACCEPT");
		sysprintf(
			"iptables -I FORWARD -i %s -m state --state NEW -j %s",
			giface, TARG_PASS);
		sysprintf(
			"iptables -I FORWARD -i %s -o br0 -m state --state NEW -j DROP",
			giface);
		sysprintf(
			"iptables -I FORWARD -i br0 -o %s -m state --state NEW -j DROP",
			giface);
	}
}
#endif
...
static void run_firewall6(char *vifs)
{
	...
	if (nvram_matchi("remote_management", 1) &&
	    nvram_invmatch("http_wanport", "") &&
	    nvram_invmatchi("http_wanport", 0))
		remotemanage = 1;
	else
		remotemanage = 0;
#ifdef HAVE_SSHD
	if (nvram_matchi("remote_mgt_ssh", 1) &&
	    nvram_invmatch("sshd_wanport", "") &&
	    nvram_invmatchi("sshd_wanport", 0) &&
...
#endif
#ifdef HAVE_TELNET
	if (nvram_matchi("remote_mgt_telnet", 1) &&
	    nvram_invmatch("telnet_wanport", "") &&
	    nvram_invmatchi("telnet_wanport", 0) &&
	...
	eval("ip6tables", "-A", "INPUT", "-m", "rt", "--rt-type", "0", "-j",
	     log_drop);
	eval("ip6tables", "-A", "FORWARD", "-m", "rt", "--rt-type", "0", "-j",
	     log_drop);
	eval("ip6tables", "-A", "OUTPUT", "-m", "rt", "--rt-type", "0", "-j",
	     log_drop);
	/* Filter INVALID packets */
	eval("ip6tables", "-A", "INPUT", "-m", "conntrack", "--ctstate",
	     "RELATED,ESTABLISHED", "-j", log_accept);
	if (nvram_matchi("filter_invalid", 1)) {
		eval("ip6tables", "-A", "INPUT", "-m", "conntrack", "--ctstate",
		     "INVALID", "-j", log_drop);
	}
	...
	if (remotemanage) {
		sysprintf("ip6tables -A INPUT -i %s -p tcp --dport %d -j %s",
			  wanface, web_lanport, log_accept);
	}
#ifdef HAVE_SSHD
	if (remotessh) {
		sysprintf("ip6tables -A INPUT -i %s -p tcp --dport %s -j %s",
			  wanface, nvram_safe_get("sshd_port"), log_accept);
	}
#endif

#ifdef HAVE_TELNET
	if (remotetelnet) {
		sysprintf("ip6tables -A INPUT -i %s -p tcp --dport 23 -j %s",
			  wanface, log_accept);
	}
#endif
	...
	/* Anti-spoofing */
	eval("ip6tables", "-A", "INPUT", "!", "-i", "lo", "-s", "::1/128", "-j",
	     log_drop);
	eval("ip6tables", "-A", "FORWARD", "-s", "::1/128", "-j", log_drop);
	eval("ip6tables", "-A", "INPUT", "-i", wanface, "-s", "fc00::/7", "-j",
	     log_drop);
	eval("ip6tables", "-A", "FORWARD", "-i", wanface, "-s", "fc00::/7",
	     "-j", log_drop);
	/* Enable stateful inspection */
	eval("ip6tables", "-A", "FORWARD", "-m", "conntrack", "--ctstate",
	     "RELATED,ESTABLISHED", "-j", log_accept);
	...
	eval("ip6tables", "-A", "INPUT", "-p", "udp", "--sport", "547",
	     "--dport", "546", "-m", "conntrack", "--ctstate", "NEW", "-j",
	     log_accept);
	/* Allow the localnet access us */
	eval("ip6tables", "-A", "INPUT", "-i", nvram_safe_get("lan_ifname"),
	     "-j", log_accept);
	...
	eval("ip6tables", "-A", "FORWARD", "-m", "conntrack", "--ctstate",
	     "NEW", "-i", nvram_safe_get("lan_ifname"), "-j", log_accept);
	...
	foreach(var, vifs, next)
	{
		if (strcmp(wanface, var) &&
		    strcmp(nvram_safe_get("lan_ifname"), var)) {
			if (isstandalone(var)) {
				eval("ip6tables", "-A", "FORWARD", "-m",
				     "conntrack", "--ctstate", "NEW", "-i", var,
				     "-j", log_accept);
				eval("ip6tables", "-A", "INPUT", "-i", var,
				     "-j", log_accept);
			}
		}
	}
	/* Use the technique of TCP MSS Clamping to correct weird browsers behaviour */
	eval("ip6tables", "-t", "mangle", "-D", "FORWARD", "-p", "tcp", "-m",
	     "tcp", "--tcp-flags", "SYN,RST", "SYN", "-j", "TCPMSS",
	     "--clamp-mss-to-pmtu");
	eval("ip6tables", "-t", "mangle", "-A", "FORWARD", "-p", "tcp", "-m",
	     "tcp", "--tcp-flags", "SYN,RST", "SYN", "-j", "TCPMSS",
	     "--clamp-mss-to-pmtu");
	/* Permit IMCPv6 echo requests (ping) but use but ratelimit it for preventing ping flooding */
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "--icmpv6-type",
	     "128", "-j", log_accept, "-m", "limit", "--limit", "30/minute");
	/* Allow dedicated  ICMPv6 packettypes */
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "1", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "2", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "3", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "4", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "128", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "129", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "133", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "134", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "135", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "136", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "141", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "142", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-s", "fe80::/10", "-p", "ipv6-icmp",
	     "-m", "icmp6", "--icmpv6-type", "130", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-s", "fe80::/10", "-p", "ipv6-icmp",
	     "-m", "icmp6", "--icmpv6-type", "131", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-s", "fe80::/10", "-p", "ipv6-icmp",
	     "-m", "icmp6", "--icmpv6-type", "132", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-s", "fe80::/10", "-p", "ipv6-icmp",
	     "-m", "icmp6", "--icmpv6-type", "143", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "148", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "149", "-m", "hl", "--hl-eq", "255", "-j",
	     log_accept);
	eval("ip6tables", "-A", "INPUT", "-s", "fe80::/10", "-p", "ipv6-icmp",
	     "-m", "icmp6", "--icmpv6-type", "151", "-m", "hl", "--hl-eq", "1",
	     "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-s", "fe80::/10", "-p", "ipv6-icmp",
	     "-m", "icmp6", "--icmpv6-type", "152", "-m", "hl", "--hl-eq", "1",
	     "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-s", "fe80::/10", "-p", "ipv6-icmp",
	     "-m", "icmp6", "--icmpv6-type", "153", "-m", "hl", "--hl-eq", "1",
	     "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "144", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "145", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "146", "-j", log_accept);
	eval("ip6tables", "-A", "INPUT", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "147", "-j", log_accept);
	eval("ip6tables", "-A", "FORWARD", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "1", "-j", log_accept);
	eval("ip6tables", "-A", "FORWARD", "-p", "ipv6-icmp", "-m", "icmp6",
	     "--icmpv6-type", "2", "-j", log_accept);
	eval("ip6tables", "-A", "FORWARD", "-p", "ipv6-icmp", "-m", "icmp6",
	...
	if (nvram_match("ipv6_typ", "ipv6in4")) {
		...
		if (*ip && strcmp(ip, "0.0.0.0") != 0)
			eval("iptables", "-I", "INPUT", "-p", "icmp", "-s", ip,
			     "-j", log_accept);
	}
	...
	if (nvram_match("ipv6_typ", "ipv6in4")) {
		eval("ip6tables", "-A", "FORWARD", "-o", "ip6tun", "-j",
		     log_accept);
	}
	...
	if (nvram_match("filter", "off") ||
		...
		if (log_level > 0) {
			...
			eval("ip6tables", "-A", "INPUT", "-j", "--reject-with",
			     "icmp6-adm-prohibited");
			eval("ip6tables", "-A", "FORWARD", "-j",
	...
	if (log_level > 0) {
#ifdef FLOOD_PROTECT
		if (nvram_matchi("log_accepted", 1))
			eval("ip6tables", "-A", "logaccept", "-i", wanface,
			     "-m", "state", "--state", "NEW", "-m", "limit",
			     "--limit", FLOOD_RATE, "-j", "LOG", "--log-prefix",
			     "FLOOD ", "--log-tcp-sequence",
			     "--log-tcp-options", "--log-ip-options");
		eval("ip6tables", "-A", "logaccept", "-i", wanface, "-m",
		     "state", "--state", "NEW", "-m", "limit", "--limit",
		     FLOOD_RATE, "-j", log_drop);
#endif
		if (nvram_matchi("log_accepted", 1))
			eval("ip6tables", "-A", "logaccept", "-m", "state",
			     "--state", "NEW", "-j", "LOG", "--log-prefix",
			     "ACCEPT ", "--log-tcp-sequence",
			     "--log-tcp-options", "--log-ip-options");
		 ...
		 */
		if (nvram_matchi("log_dropped", 1)) {
			eval("ip6tables", "-A", "logdrop", "-m", "state",
			     "--state", "NEW", "-j", "LOG", "--log-prefix",
			     "DROP ", "--log-tcp-sequence", "--log-tcp-options",
			     "--log-ip-options");
			if (has_gateway() &&
			    nvram_matchi("filter_invalid", 1)) {
				eval("ip6tables", "-A", "logdrop", "-m",
		 ...
		 */
		if (nvram_matchi("log_rejected", 1))
			eval("ip6tables", "-A", "logreject", "-j", "LOG",
			     "--log-prefix", "WEBDROP ", "--log-tcp-sequence",
			     "--log-tcp-options", "--log-ip-options");
		eval("ip6tables", "-A", "logreject", "-p", "tcp", "-j",
		 ...
		 */
		if (nvram_matchi("log_accepted", 1))
			eval("ip6tables", "-A", "limaccept", "-i", wanface,
			     "-m", "state", "--state", "NEW", "-m", "limit",
			     "--limit", FLOOD_RATE, "-j", "LOG", "--log-prefix",
			     "FLOOD ", "--log-tcp-sequence",
			     "--log-tcp-options", "--log-ip-options");
		eval("ip6tables", "-A", "limaccept", "-i", wanface, "-m",
		     "state", "--state", "NEW", "-m", "limit", "--limit",
		     FLOOD_RATE, "-j", log_drop);
...
#ifdef HAVE_OPENVPN
	if (nvram_matchi("openvpn_enable", 1)) {
		if (nvram_invmatch("openvpn_v6netmask", "") &&
		    nvram_matchi("ipv6_enable", 1)) {
			if (nvram_match("openvpn_proto", "udp") ||
			    nvram_match("openvpn_proto", "udp6")) {
				eval("ip6tables", "-I", "INPUT", "-p", "udp",
				     "--dport", nvram_safe_get("openvpn_port"),
				     "-i", wanface, "-j", log_accept);
			}
			if (nvram_match("openvpn_proto", "tcp-server") ||
			    nvram_match("openvpn_proto", "tcp6-server")) {
				eval("ip6tables", "-I", "INPUT", "-p", "tcp",
				     "--dport", nvram_safe_get("openvpn_port"),
				     "-i", wanface, "-j", log_accept);
			}
			if (nvram_match("openvpn_tuntap", "tun")) {
				eval("ip6tables", "-I", "INPUT", "-i", "tun2",
				     "-j", log_accept);
				eval("ip6tables", "-I", "FORWARD", "-i", "tun2",
				     "-j", log_accept);
			}
		}
	}
...
static void setsysctl(char *path, char *nvname, char *name, char *sysval,
		      void *priv)
{
...
#endif
{
	 ...
	 */
	writeprocsysnet("core/netdev_max_backlog",
			nvram_default_get("net.core.netdev_max_backlog",
					  "120"));
#else
...
#endif
#endif
#if defined(HAVE_X86) || defined(HAVE_VENTANA) || defined(HAVE_IPQ806X) || \
	defined(HAVE_LAGUNA) || defined(HAVE_CAMBRIA) ||                   \
	defined(HAVE_NEWPORT) || defined(HAVE_NORTHSTAR) ||                \
	defined(HAVE_OCTEON) || defined(HAVE_80211AC)
	writeprocsysnet("core/somaxconn",
			nvram_default_get("net.core.somaxconn", "1024"));
	writeprocsysnet("ipv4/tcp_max_syn_backlog",
			nvram_default_get("net.ipv4.tcp_max_syn_backlog",
					  "1024"));
	writeprocsysnet("core/rmem_default",
			nvram_default_get("net.core.rmem_default", "262144"));
	writeprocsysnet("core/rmem_max",
			nvram_default_get("net.core.rmem_max", "262144"));
	writeprocsysnet("core/wmem_default",
			nvram_default_get("net.core.wmem_default", "262144"));
	writeprocsysnet("core/wmem_max",
			nvram_default_get("net.core.wmem_max", "262144"));
	writeprocsysnet("ipv4/tcp_rmem",
			nvram_default_get("net.ipv4.tcp_rmem",
					  "8192 131072 262144"));
	writeprocsysnet("ipv4/tcp_wmem",
			nvram_default_get("net.ipv4.tcp_wmem",
					  "8192 131072 262144"));
	writeprocsysnet("ipv4/neigh/default/gc_thresh1",
			nvram_default_get("net.ipv4.neigh.default.gc_thresh1",
					  "1"));
	writeprocsysnet("ipv4/neigh/default/gc_thresh2",
			nvram_default_get("net.ipv4.neigh.default.gc_thresh2",
					  "2048"));
	writeprocsysnet("ipv4/neigh/default/gc_thresh3",
			nvram_default_get("net.ipv4.neigh.default.gc_thresh3",
					  "4096"));
	writeprocsysnet("ipv6/neigh/default/gc_thresh1",
			nvram_default_get("net.ipv6.neigh.default.gc_thresh1",
					  "1"));
	writeprocsysnet("ipv6/neigh/default/gc_thresh2",
			nvram_default_get("net.ipv6.neigh.default.gc_thresh2",
					  "2048"));
	writeprocsysnet("ipv6/neigh/default/gc_thresh3",
			nvram_default_get("net.ipv6.neigh.default.gc_thresh3",
					  "4096"));
#endif
	writeprocsysnet("ipv4/tcp_fin_timeout",
			nvram_default_get("net.ipv4.tcp_fin_timeout", "40"));
	writeprocsysnet("ipv4/tcp_keepalive_intvl",
			nvram_default_get("net.ipv4.tcp_keepalive_intvl",
					  "30"));
	writeprocsysnet("ipv4/tcp_keepalive_probes",
			nvram_default_get("net.ipv4.tcp_keepalive_probes",
					  "5"));
	writeprocsysnet("ipv4/tcp_keepalive_time",
			nvram_default_get("net.ipv4.tcp_keepalive_time",
					  "120"));
	writeprocsysnet("ipv4/tcp_retries2",
			nvram_default_get("net.ipv4.tcp_retries2", "5"));
	writeprocsysnet("ipv4/tcp_syn_retries",
			nvram_default_get("net.ipv4.tcp_syn_retries", "3"));
	writeprocsysnet("ipv4/tcp_synack_retries",
			nvram_default_get("net.ipv4.tcp_synack_retries", "3"));
#ifdef HAVE_MADWIFI
	writeprocsysnet("ipv4/tcp_tw_recycle",
			nvram_default_get("net.ipv4.tcp_tw_recycle", "0"));
#endif
	writeprocsysnet("ipv4/tcp_tw_reuse",
			nvram_default_get("net.ipv4.tcp_tw_reuse", "1"));
	writeprocsysnet(
		"ipv4/icmp_ignore_bogus_error_responses",
		nvram_default_get("net.ipv4.icmp_ignore_bogus_error_responses",
				  "1"));
	writeprocsysnet(
		"ipv4/icmp_echo_ignore_broadcasts",
		nvram_default_get("net.ipv4.icmp_echo_ignore_broadcasts", "1"));
	writeprocsysnet("ipv4/tcp_rfc1337",
			nvram_default_get("net.ipv4.tcp_rfc1337", "1"));
	writeprocsysnet("ipv4/tcp_syncookies",
			nvram_default_get("net.ipv4.tcp_syncookies", "1"));
	writeprocsysnet("netfilter/nf_conntrack_checksum",
			nvram_default_get("net.netfilter.nf_conntrack_checksum",
					  "0"));
	writeprocsysnet("ipv4/conf/default/arp_ignore",
			nvram_default_get("net.ipv4.conf.default.arp_ignore",
					  "1"));
	writeprocsysnet("ipv4/conf/all/arp_ignore",
			nvram_default_get("net.ipv4.conf.all.arp_ignore", "1"));
	 ...
	 * Run DMZ forwarding ? 
	 */
	if (has_gateway() && nvram_matchi("dmz_enable", 1) &&
	...
	if (nvram_matchi("remote_mgt_https", 1))
		web_lanport = nvram_geti("https_lanport") > 0 ?
				      nvram_geti("https_lanport") :
				      HTTPS_PORT;
	else
#endif
		web_lanport = nvram_geti("http_lanport") > 0 ?
				      nvram_geti("http_lanport") :
				      HTTP_PORT;
	/*
	...
	create_restore_file(wanface, lanface, wanaddr, lan_cclass, dmzenable,
			    webfilter, remotessh, remotetelnet, remotemanage,
			    vifs);
#ifndef DEVELOPE_ENV
	/*
	...
	if (nvram_matchi("ipv6_enable", 1)) {
		writeprocsysnet(
			...
			nvram_default_get("net.ipv6.conf.all.forwarding", "1"));
	}
#endif
#ifdef HAVE_GGEW
	...
	if (nvram_matchi("ias_startup", 3))
		sysprintf(
			"iptables -t nat -I PREROUTING -i br0 -p udp --dport 53 -j DNAT --to 192.168.11.1:55300");
#endif
#ifdef HAVE_GUESTPORT
	set_gprules("wlan0");
...
void stop_firewall(void)
{
	...
	unlock();
#if !defined( \
	HAVE_MICRO) //&& !(defined(ARCH_broadcom) && !defined(HAVE_BCMMODERN))
...
void start_test_ndpi(void)
{
	...
	while (filters[cnt].name) {
		if (filters[cnt].protocol == 2) {
			if (eval("iptables", "-A", "testchain", "-m", "ndpi",
				 "--proto", filters[cnt].name, "-j", "DROP"))
				fprintf(stderr, "error in %s\n",
					filters[cnt].name);
			eval("iptables", "-D", "testchain", "-m", "ndpi",
			     "--proto", filters[cnt].name, "-j", "DROP");
		}
		cnt++;
	}
