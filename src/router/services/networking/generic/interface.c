...
void start_setup_vlans(void)
{
	...
	if (nvram_match("switch_leds", "0")) {
		eval("ssdk_sh_id", "1", "debug", "reg", "set", "0x50",
		     "0x00000000", "4");
		eval("ssdk_sh_id", "1", "debug", "reg", "set", "0x54",
		     "0x00000000", "4");
		eval("ssdk_sh_id", "1", "debug", "reg", "set", "0x58",
		     "0x00000000", "4");
		eval("ssdk_sh_id", "1", "debug", "reg", "set", "0x5c",
		     "0x00000000", "4");

		eval("ssdk_sh_id", "0", "debug", "reg", "set", "0x50",
		     "0x00000000", "4");
		eval("ssdk_sh_id", "0", "debug", "reg", "set", "0x54",
		     "0x00000000", "4");
		eval("ssdk_sh_id", "0", "debug", "reg", "set", "0x58",
		     "0x00000000", "4");
		eval("ssdk_sh_id", "0", "debug", "reg", "set", "0x5c",
		     "0x00000000", "4");
	...
	for (i = 0; i < lanports; i++) {
		...
		foreach(vlan, vlans, next)
		{
			...
			if (tmp >= 16000) {
				...
				if (nvram_exists("sw_wancpuport") &&
				    nvram_match("wan_default", "eth0")) {
					lanphy = "eth1";
					wanphy = "eth0";
				}
				if (nvram_exists("sw_wancpuport") &&
				    nvram_match("wan_default", "eth1")) {
					lanphy = "eth0";
					wanphy = "eth1";
				}
				 ...
				 */
				if (tagged[i] &&
				    (i == cpuportidx || i == wancpuportidx ||
				     i == lancpuportidx)) {
					if (i == wancpuportidx)
						eval("vconfig", "add", wanphy,
						     buff);
					else if (i == lancpuportidx)
						eval("vconfig", "add", lanphy,
						     buff);
					else
						eval("vconfig", "add", lanphy,
						     buff);
					snprintf(buff, 9, "vlan%d",
						 vlanlist[vlan_number]);
					if (strcmp(nvram_safe_get("wan_ifname"),
						   buff)) {
						char hwaddr[32];
						sprintf(hwaddr, "%s_hwaddr",
							buff);
		...
		if (i != cpuportidx && i != lancpuportidx &&
		    i != wancpuportidx) {
			char linkstr[128] = { 0 };
			if (mask & 4) {
				if (mask & 2)
				...
				if (mask & 16) {
					if (mask & 1)
						sprintf(linkstr, "%s speed 10",
							linkstr);
					else
						sprintf(linkstr, "%s speed 100",
							linkstr);

					sprintf(linkstr, "%s autoneg off",
						linkstr);
					if (mask & 32) {
						sprintf(linkstr,
							"%s rxflow txflow",
					...
					sprintf(linkstr, "%s speed 1000",
						linkstr);
					sprintf(linkstr, "%s autoneg on",
						linkstr);
				}
			...
			if (i == 0) {
				eval("swconfig", "dev", "switch0", "port",
				     nvram_safe_get("sw_wan"), "set", "link",
				     linkstr);
			} else {
				eval("swconfig", "dev", "switch0", "port",
				     nvram_nget("sw_lan%d", i), "set", "link",
				     linkstr);
			}
			if (mask & 8) {
				if (i == 0) {
					eval("swconfig", "dev", "switch0",
					     "port", nvram_safe_get("sw_wan"),
					     "set", "disable", "1");
				} else {
					eval("swconfig", "dev", "switch0",
					     "port", nvram_nget("sw_lan%d", i),
					     "set", "disable", "1");
				}
			} else {
				if (i == 0) {
	...
	for (vlan_number = 0; vlan_number < 16; vlan_number++) {
		...
		if (strlen(ports)) {
			...
			sysprintf(
				". /usr/sbin/config_vlan.sh \"%d\" \"%s\" \"0\" \"0\" \"0\" \"normal_lan\"");
		}
	}

	sysprintf(
		". /usr/sbin/config_vlan.sh \"2\" \"\" \"0\" \"1\" \"0\"  \"wan\"");
	...
	eval("swconfig", "dev", "switch0", "set", "enable_vlan",
	     vlan_enable ? "1" : "0");
		...
		if (vlans) {
			...
			foreach(vlan, vlans, next)
			{
				...
				if (tmp < 16000) {
					...
					sprintf((char *)portsettings[tmp],
						"%s %d%s",
						(char *)portsettings[tmp], use,
						tagged[i] ? "t" : "");
				} else {
	...
	for (i = 0; i < blen; i++) {
		...
		foreach(vlan, port, next)
		{
			...
			} else if ((vlan_number == 5 || vlan_number == 8 ||
				    vlan_number == 7) &&
				   !ast) {
				strspcattach(portsettings[i], vlan);
			} else if ((vlan_number == 5 || vlan_number == 8 ||
	...
	for (i = 0; i < blen; i++) {
		writevaproc(" ", "/proc/switch/%s/vlan/%d/ports", phy,
			    vlanlist[i]);
	}
	for (i = 0; i < blen; i++) {
