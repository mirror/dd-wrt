...
/*  disable till pattern works
		save2file
		    ("-A advgrp_%d -m ndpi --edonkey -j %s",
		     seq, log_drop); */
		/*atm rarly used protocols */
		save2file_A("advgrp_%d -m ndpi --apple -j %s", seq, log_drop);
		...
		save2file_A("advgrp_%d -p tcp -m ndpi --pando -j %s", seq, log_drop);
...
static void mangle_table(char *wanface, char *wanaddr, char *vifs)
{
	...
	if (nvram_match("wan_priority", "1") && isvlan(wanface)) {
		...
		save2file_A_prerouting("-i %s -j CLASSIFY --set-class 0:1", wanface);
		...
		eval("ip6tables", "-t", "mangle", "-D", "PREROUTING", "-i", wanface, "-j", "CLASSIFY", "--set-class", "0:1");
		eval("ip6tables", "-t", "mangle", "-A", "PREROUTING", "-i", wanface, "-j", "CLASSIFY", "--set-class", "0:1");
