...
static void parse_port_forward(char *wordlist)
{
	...
	foreach(var, wordlist, next) {
		...
		if (!strcmp(proto, "tcp") || !strcmp(proto, "both")) {
			...
			if (flag_dis == 0) {
				save2file
				    ("-A PREROUTING -p tcp -m tcp -d %s --dport %s "
		...
		if (!strcmp(proto, "udp") || !strcmp(proto, "both")) {
			...
			if (flag_dis == 0) {
				save2file
				    ("-A PREROUTING -p udp -m udp -d %s --dport %s "
				     "-j DNAT --to-destination %s\n", wanaddr,
				     port, ip);
...
static void parse_upnp_forward()
{
	...
	for (i = 0; i < 50; i++) {
		...
		if (!strcmp(proto, "tcp") || !strcmp(proto, "both")) {
			save2file
			    ("-A PREROUTING -i %s -p tcp -m tcp -d %s --dport %s "
		...
		if (!strcmp(proto, "udp") || !strcmp(proto, "both")) {
			save2file
			    ("-A PREROUTING -i %s -p udp -m udp -d %s --dport %s "
			     "-j DNAT --to-destination %s%d:%s\n", wan, wanaddr,
			     wan_port0, lan_cclass, get_single_ip(lan_ipaddr,
								  3),
			     lan_port0);
...
static void nat_prerouting(void)
{
		...
		if (remote_any) {
			...
			foreach(var, wordlist, next) {
				save2file
				    ("-A PREROUTING -p tcp -m tcp -s %s -d %s --dport %s "
				     "-j DNAT --to-destination %s:%s\n", var,
				     wanaddr, nvram_safe_get("sshd_wanport"),
				     nvram_safe_get("lan_ipaddr"),
				     nvram_safe_get("sshd_port"));
			}
		}
	...
	*suspense = 0;
	...
	if (has_gateway()) {
		 ...
		 */
		save2file
		    ("-A PREROUTING -d %s -j TRIGGER --trigger-type dnat\n",
		     wanaddr);
		/*
		 * DD-WRT addition end 
		 */
...

		{
			...
			foreach(var, vifs, next) {
				if (strcmp(get_wan_face(), var)
				    && strcmp(nvram_safe_get("lan_ifname"),
					      var)) {
					if (nvram_nmatch
					    ("0", "%s_bridged", var)) {
						if (nvram_match
...
static int schedule_by_tod(int seq)
{
	...
	if (allday) {		/* 24-hour, but not everyday */
		...
		while (token != NULL) {
			...
			if (rotate == 1 && st == 0)
				sprintf(wday_end + strlen(wday_end), ",%d",
					end);
	...
	if (mark == MARK_DROP) {
		foreach(var, wordlist, next) {
			save2file("-A grp_%d -m mac --mac-source %s -j %s\n",
				  seq, var, log_drop);
			save2file
			    ("-A grp_%d -m mac --mac-destination %s -j %s\n",
			     seq, var, log_drop);
		}
	} else {
		foreach(var, wordlist, next) {
			save2file
			    ("-A grp_%d -m mac --mac-source %s -j advgrp_%d\n",
			     seq, var, seq);
			save2file
			    ("-A grp_%d -m mac --mac-destination %s -j advgrp_%d\n",
			     seq, var, seq);
...
static void portgrp_chain(int seq, unsigned int mark, int urlenable)
{
	...
	foreach(var, wordlist, next) {
		...
		if (!strcmp(protocol, "tcp") || !strcmp(protocol, "both")) {
			save2file
			    ("-A grp_%d -p tcp -m tcp --dport %s:%s -j %s\n",
			     seq, lan_port0, lan_port1, target);
		}
		if (!strcmp(protocol, "udp") || !strcmp(protocol, "both")) {
			save2file
			    ("-A grp_%d -p udp -m udp --dport %s:%s -j %s\n",
			     seq, lan_port0, lan_port1, target);
		}
	}
}
	...
	split(word, wordlist, next, delim) {
		...
		split(srv, services, next2, delim) {
			...
			if (!strcmp(protocol, "tcp")
				...
				save2file
				    ("-A advgrp_%d -p tcp -m tcp --dport %s -j %s\n",
				     seq, ports, log_drop);
			if (!strcmp(protocol, "udp")
			    || !strcmp(protocol, "both"))
				save2file
				    ("-A advgrp_%d -p udp -m udp --dport %s -j %s\n",
				     seq, ports, log_drop);
	...
	foreach(var, wordlist, next) {
		...
		if (!strcmp(proto, "tcp") || !strcmp(proto, "udp")) {
			...
			save2file("-A trigger_out -p tcp -m tcp --dport %s:%s "
				  ...
				  "--trigger-match %s-%s --trigger-relate %s-%s\n",
				  wport0, wport1, wport0, wport1, lport0,
				  lport1);
...
/*#ifdef HAVE_AP_SERV
	...
	if (remotessh && nvram_match("limit_ssh", "1")) {
		save2file
		    ("-A INPUT -p tcp -m tcp -d %s --dport %s -m state --state NEW -m limit --limit 3/min --limit-burst 3 -j ACCEPT\n",
		     nvram_safe_get("lan_ipaddr"), nvram_safe_get("sshd_port"));
		save2file("-A INPUT -p tcp -d %s --dport %s -j DROP\n", 
		     nvram_safe_get("lan_ipaddr"), nvram_safe_get("sshd_port"));
	}
#endif
	/*
	...
	if (remotetelnet && nvram_match("limit_telnet", "1")) {
		save2file
		    ("-A INPUT -p tcp -m tcp -d %s --dport 23 -m state --state NEW -m limit --limit 3/min --limit-burst 3 -j ACCEPT\n",
		     nvram_safe_get("lan_ipaddr"));
		save2file("-A INPUT -p tcp -d %s --dport 23 -j DROP\n",
		     nvram_safe_get("lan_ipaddr"));
	}
#endif
	if (remotetelnet) {
...
//            atoi( nvram_safe_get( "wan_mtu" ) ) - 40 );
	...
	if (nvram_invmatch("filter", "off") && strlen(wanface)) {
		...
		if (nvram_match("pptp_pass", "0"))
			save2file
			    ("-A FORWARD -o %s -p tcp -m tcp --dport %d -j %s\n",
		...
		if (nvram_match("l2tp_pass", "0"))
			save2file
			    ("-A FORWARD -o %s -p udp -m udp --dport %d -j %s\n",
	...
	if (strlen(wanface)) {
		save2file
		    ("-A FORWARD -i %s -o %s -j TRIGGER --trigger-type in\n",
		     wanface, lanface);
	...
	if (dmzenable)
		save2file("-A FORWARD -o %s -d %s%s -j %s\n", lanface,
			  lan_cclass, nvram_safe_get("dmz_ipaddr"), log_accept);
	...
	if (!strcmp(nvram_safe_get("QoS"), "1")) {
		...
		if (nvram_match("enable_game", "1")) {
			for (i = 0; i < STRUCT_LEN(gamming_app_table); i++) {
				int j = 0;
				struct game_port_t *udp_item =
				    gamming_app_table[i].udp_port;
