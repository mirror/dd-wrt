	...
	if (nvram_nmatch("1", "%s_gcmp", prefix) || iswpa3_128) {
		...
		if (grpstring) {
			if (has_ad(prefix))
				...
				strspcattach(temp_grpstring, sizeof(temp_grpstring), "GCMP CCMP");
			}
		}
	}
	...
	if (grpstring) {
		...
		foreach(var, temp_grpstring, next) {
			if (!strhas(grpstring, var)) {
				strspcattach(grpstring, grpstrlen, var);
			}
		}
	}
}
	...
	for (i = 0; i < c; i++) {
		...
		if (nvram_nmatch("ap", "%s_mode", ath) || nvram_nmatch("wdsap", "%s_mode", ath) ||
			...
			if (nvhas(akm, "psk") || nvhas(akm, "psk2") || nvhas(akm, "psk3") || nvhas(akm, "owe") ||
			    ...
			    nvhas(akm, "wpa3-128") || nvhas(akm, "wpa3-192") || nvhas(akm, "wpa2-sha256") ||
			    nvhas(akm, "wpa2-sha384") || nvram_match(akm, "wep")) {
...
void setupHostAPPSK(FILE *fp, const char *prefix, int isfirst)
{
	...
	if (ispsk || iswpa)
		wpamask |= 1;
	if (ispsk2 || ispsk3 || isowe || iswpa2 || iswpa3 || iswpa3_192 || iswpa3_128 || iswpa2sha256 || iswpa2sha384 ||
	    ispsk2sha256)
		wpamask |= 2;
	fprintf(fp, "wpa=%d\n", wpamask);
	if (ispsk)
	...
	if (_has_wpa3 && ispsk3)
		fprintf(fp, "sae_groups=19 20 21\n");
#ifdef HAVE_80211R
	if (nvram_matchi(ft, 1) && (ispsk3 || ispsk || ispsk2 || ispsk2sha256 || iswpa || iswpa2 || iswpa3 || iswpa2sha256 ||
				    iswpa2sha384 || iswpa3_128 || iswpa3_192)) {
		char dl[32];
		char temp[128];
	...
	get_pairwise(prefix, pwstring, sizeof(pwstring), NULL, 0, 0, 0);
...
static void configure_single(int count, char **configs, int *configidx)
{
	...
	if (!strcmp(apm, "ap") || !strcmp(apm, "wdsap") || !strcmp(apm, "wdsap_mtik")) {
	...
	if (vifs != NULL)
		foreach(var, vifs, next) {
			...
			if (*mode) {
				...
				else if (!strcmp(vapm, "ap") || !strcmp(vapm, "wdsap") || !strcmp(vapm, "wdsap_mtik"))
	...
	if (!strcmp(apm, "wdssta") || !strcmp(apm, "wdsap") || !strcmp(apm, "wdsap_mtik") || !strcmp(apm, "wdssta_mtik"))
		eval("iwpriv", dev, "wds", "1");

	if (!strcmp(apm, "wdsap") || !strcmp(apm, "wdsap_mtik"))
...
//              fprintf(stderr, "waiting for %d interfaces, %d finished\n", c, cnf);
	...
	if (is_ath12k("wlan0"))
		sysprintf("echo 1 > /sys/kernel/debug/ieee80211/phy0/ath12k/ext_rx_stats");
	if (is_ath12k("wlan1"))
		sysprintf("echo 1 > /sys/kernel/debug/ieee80211/phy1/ath12k/ext_rx_stats");
	if (is_ath12k("wlan2"))
