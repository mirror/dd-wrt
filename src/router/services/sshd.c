...
//cprintf("start sshd %s\n",sshd_argv);
  ...
  ret =
    eval ("dropbear", "-r", RSA_HOST_KEY_FILE, "-d", DSS_HOST_KEY_FILE, "-p",
	  port, passwd_ok);
 ...
 */
static int
write_key_file (char *keyname, char *keyfile, int chmodval)
{
  FILE *fd = NULL;
  char *host_key = NULL;
  int i = 0;
...
static int
generate_dropbear_rsa_host_key (void)
{
  FILE *fp = (void *) 0;
  char buf[4096] = "";
  int ret = -1;
...
static int
generate_dropbear_dss_host_key (void)
{
  FILE *fp = (void *) 0;
  char buf[4096] = "";
  int ret = -1;
