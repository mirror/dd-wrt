 ...
 * @opc: OPc = 128-bit value derived from OP and K with milenage_opc()
 * @k: K = 128-bit subscriber key
 ...
 */
static void milenage_f1(const u8 *opc, const u8 *k, const u8 *rand,
 ...
 */
static void milenage_f2345(const u8 *opc, const u8 *k, const u8 *rand, u8 *res,
 ...
 */
void milenage_generate(const u8 *op, const u8 *amf, const u8 *k, const u8 *sqn,
		       const u8 *rand, u8 *autn, u8 *ik, u8 *ck, u8 *res,
		       size_t *res_len)
{
	int i;
	u8 opc[16], mac_a[16], ak[6];
	...
	*res_len = 8;
	milenage_opc(op, k, opc);
	milenage_f1(opc, k, rand, sqn, amf, mac_a, NULL);
	milenage_f2345(opc, k, rand, res, ck, ik, ak, NULL);
 ...
 */
int milenage_auts(const u8 *op, const u8 *k, const u8 *rand, const u8 *auts,
		  u8 *sqn)
{
	u8 amf[2] = { 0x00, 0x00 }; /* TS 33.102 v7.0.0, 6.3.3 */
	u8 ak[6], mac_s[8], opc[16];
	int i;
	...
	for (i = 0; i < 6; i++)
		sqn[i] = auts[i] ^ ak[i];
	milenage_f1(opc, k, rand, sqn, amf, NULL, mac_s);
...
int main(int argc, char *argv[])
{
	...
	memcpy(rand, "\x12\x69\xb8\x23\x41\x39\x35\x66\xfb\x99\x41\xe9\x84"
	       "\x4f\xe6\x2f", 16);
	res_len = 8;
	milenage_generate(t->op, t->amf, t->k, sqn, rand, buf, buf2, buf3,
			  buf4, &res_len);
	wpa_hexdump(MSG_DEBUG, "SQN", sqn, 6);
	wpa_hexdump(MSG_DEBUG, "RAND", rand, 16);
