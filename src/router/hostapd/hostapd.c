...
void hostapd_reassoc_sta(hostapd *hapd, struct sta_info *sta)
{
	/* IEEE 802.11F (IAPP) */
	if (hapd->conf->ieee802_11f)
		iapp_new_station(hapd->iapp, sta);
	...
	/* Let 802.1x decide wether it should run or not */ 
	ieee802_1x_new_station(hapd, sta);
}
...
#ifdef JUMPSTART
static hostapd * hostapd_init(const char *config_file, char *js_passwd)
#else
