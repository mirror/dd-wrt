...
void ieee802_1x_new_station(hostapd *hapd, struct sta_info *sta)
{
	...
	if (sta->eapol_sm && !sta->pmksa) {
		...
		/* 
		 * XXX Divy. The STA might have left without our knowledge
		 * and is trying to come back. Since there is no PMKSA,
		 * force a 802.1x reauthentication.
		 */
		sta->eapol_sm->keyRun = TRUE; /* To get to INIPMK */
		sta->eapol_sm->keyAvailable = FALSE; /* then move to DISCONNECT */
		eapol_sm_step(sta->eapol_sm);
		return;
	}
