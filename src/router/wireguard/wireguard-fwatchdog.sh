...
while sleep "$SLEEP"; do
	...
	while ! ping -qc1 -W6 -n "$PINGIP" -I "oet${i}" &> /dev/null; do
		...
		if ! ping -qc1 -W6 -n $PINGIP -I oet${i} &> /dev/null; then
			...
			[[ "$FAILGRP" = "1" ]] && { nvram set oet${i}_failstate=1; logger -p user.warning "WireGuard watchdog: oet${i} set to fail"; }
			#if [[ $RESET -eq 1 && $REBOOT -eq 1 && $FAILGRP -eq 1 ]]; then
			#needs testing
			if [[ "$REBOOT" = "1" ]] && [[ "$RESET" = "1" || "$FAILGRP" = "0" ]]; then
