...
foreach ($data['templategroup_rights'] as $groupid => $group_right) {
	...
	if ($groupid == 0) {
		...
		$form_vars[] = (new CVar('templategroup_rights['.$groupid.'][permission]', $group_right['permission']))
			->removeId();
		...
		if (array_key_exists('grouped', $group_right) && $group_right['grouped']) {
			$form_vars[] = (new CVar('templategroup_rights['.$groupid.'][grouped]', $group_right['grouped']))
				->removeId();
			$form_data_json[$groupid]['grouped'] = $group_right['grouped'];
