...
class RestrictedCompileMode(AbstractCompileMode):
    ...
    def __init__(self, source, filename, elevated=False):
        if source:                                  
            source = '\n'.join(source.splitlines()) + '\n'
        self.rm = RestrictionMutator(elevated)
...
def compile_restricted_function(p, body, name, filename, globalize=None, elevated=False):
    ...
    """
    gen = RFunction(p, body, name, filename, globalize, elevated)
    return compileAndTuplize(gen)

def compile_restricted_exec(s, filename='<string>', elevated=False):
    """Compiles a restricted code suite."""
    gen = RModule(s, filename, elevated)
    return compileAndTuplize(gen)
...
def compile_restricted(source, filename, mode, elevated=False):
    """Replacement for the builtin compile() function."""
    if mode == "single":
        gen = RInteractive(source, filename)
    elif mode == "exec":
        gen = RModule(source, filename, elevated)
    elif mode == "eval":
...
class RFunction(RModule):
    ...
    def __init__(self, p, body, name, filename, globals, elevated=False):
        self.params = p
        if body:
        ...
        RModule.__init__(self, None, filename, elevated)
