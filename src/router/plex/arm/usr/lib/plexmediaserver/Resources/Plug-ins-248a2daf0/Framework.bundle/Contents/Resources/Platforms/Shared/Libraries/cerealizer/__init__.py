...
def _convert_type(obj):
  if obj.__class__.__name__ in string_types:
    obj = str(obj)
  elif obj.__class__.__name__ in unicode_types:
    obj = unicode(obj)
  return obj
...
class Dumper(object):
  ...
  def dump(self, root_obj, s):
    root_obj = _convert_type(root_obj)
...
class DatetimeHandler(Handler):
  ...
  def dump_obj(self, obj, dumper, s):
    assert issubclass(obj.__class__, datetime)
  ...
  def undump_obj(self, dumper, s):
    ...
    return datetime(line.decode('string_escape'))
...
register(datetime  , DatetimeHandler ())
