...
batadv_vis_hash_find(struct batadv_priv *bat_priv, const void *data)
{
	...
	rcu_read_lock();
	bat_hlist_for_each_entry_rcu(vis_info, node, head, hash_entry) {
					     ...
					     bool primary)
{
	...
	bat_hlist_for_each_entry(entry, pos, if_list, list) {
					 ...
					 struct batadv_vis_info_entry *entries)
{
	...
	bat_hlist_for_each_entry(entry, pos, list, list) {
					     ...
					     const struct hlist_head *head)
{
	...
	bat_hlist_for_each_entry_rcu(info, node, head, hash_entry) {
		...
		bat_hlist_for_each_entry_safe(entry, pos, n, &vis_if_list, list) {
...
next:
	...
	for (i = 0; i < hash->size; i++) {
		...
		rcu_read_lock();
		bat_hlist_for_each_entry_rcu(tt_common_entry, node, head,
					 hash_entry) {
			packet_pos = skb_put(info->skb_packet, sizeof(*entry));
			entry = (struct batadv_vis_info_entry *)packet_pos;
...
static void batadv_purge_vis_packets(struct batadv_priv *bat_priv)
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		bat_hlist_for_each_entry_safe(info, node, node_tmp,
					  head, hash_entry) {
