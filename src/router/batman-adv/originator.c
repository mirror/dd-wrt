...
static void batadv_orig_node_free_rcu(struct rcu_head *rcu)
{
	...
	bat_hlist_for_each_entry_safe(neigh_node, node, node_tmp,
...
void batadv_originator_free(struct batadv_priv *bat_priv)
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		bat_hlist_for_each_entry_safe(orig_node, node, node_tmp,
					  head, hash_entry) {
			hlist_del_rcu(node);
...
int batadv_orig_seq_print_text(struct seq_file *seq, void *offset)
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		bat_hlist_for_each_entry_rcu(orig_node, node, head, hash_entry) {
			...
			bat_hlist_for_each_entry_rcu(neigh_node_tmp, node_tmp,
						 &orig_node->neigh_list, list) {
