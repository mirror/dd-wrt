			...
			struct batadv_bla_claim *data)
{
	...
	rcu_read_lock();
	bat_hlist_for_each_entry_rcu(claim, node, head, hash_entry) {
			  ...
			  uint8_t *addr, short vid)
{
	...
	rcu_read_lock();
	bat_hlist_for_each_entry_rcu(backbone_gw, node, head, hash_entry) {
...
batadv_bla_del_backbone_claims(struct batadv_bla_backbone_gw *backbone_gw)
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		bat_hlist_for_each_entry_safe(claim, node, node_tmp,
					  head, hash_entry) {
...
static void batadv_bla_purge_backbone_gw(struct batadv_priv *bat_priv, int now)
{
	...
	for (i = 0; i < hash->size; i++) {
		...
		spin_lock_bh(list_lock);
		bat_hlist_for_each_entry_safe(backbone_gw, node, node_tmp,
					  head, hash_entry) {
			if (now)
				goto purge_now;
