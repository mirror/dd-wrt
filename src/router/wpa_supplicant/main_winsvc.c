...
#define SERVICE_NAME "wpasvc"
#define DISPLAY_NAME "wpa_supplicant service"
...
static int wpa_supplicant_thread(void)
{
	...
	buflen = sizeof(val);
	ret = RegQueryValueEx(hk, "debug_level", NULL, NULL,
			      (LPBYTE) &val, &buflen);
	...
	buflen = sizeof(val);
	ret = RegQueryValueEx(hk, "debug_show_keys", NULL, NULL,
			      (LPBYTE) &val, &buflen);
	if (ret == ERROR_SUCCESS && buflen == sizeof(val)) {
	...
	ret = RegOpenKeyEx(hk, "interfaces", 0, KEY_ENUMERATE_SUB_KEYS, &ihk);
	RegCloseKey(hk);
	if (ret != ERROR_SUCCESS) {
...
static int register_service(const char *exe)
{
	SC_HANDLE svc, scm;

	printf("Registering service: " SERVICE_NAME "\n");
...
static int unregister_service(void)
{
	...
	printf("Unregistering service: " SERVICE_NAME "\n");
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		if (strcmp(argv[1], "reg") == 0) {
			if (argc < 3) {
				printf("wpasvc reg <path to wpasvc.exe>\n");
				return -1;
			}
			return register_service(argv[2]);
