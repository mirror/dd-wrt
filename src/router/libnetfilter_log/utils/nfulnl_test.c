...
int main(int argc, char **argv)
{
	...
	printf("unbinding existing nf_log handler for AF_BRIDGE (if any)\n");
	if (nflog_unbind_pf(h, AF_BRIDGE) < 0) {
		fprintf(stderr, "error nflog_unbind_pf()\n");
		exit(1);
	}

	printf("binding nfnetlink_log to AF_BRIDGE\n");
	if (nflog_bind_pf(h, AF_BRIDGE) < 0) {
