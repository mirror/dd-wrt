		...
		for (i = 0; i < ZEBRA_ROUTE_MAX; i++) {
			if (i != babel_zclient->redist_default &&
				...
				vty_out(vty, " redistribute %s %s\n",
					(afi == AFI_IP) ? "ipv4" : "ipv6", zebra_route_string(i));
				lines++;
			}
		}
	}
...
static int babel_create_routing_process(void)
{
...
static void babel_main_loop(struct event *thread)
{
	...
	while (1) {
		...
		FOR_ALL_INTERFACES (vrf, ifp) {
			...
			if (timeval_compare(&babel_now, &babel_ifp->update_flush_timeout) >= 0)
				flushupdates(ifp);
		}
...
static void printIfMin(struct timeval *tv, int cmd, const char *tag, const char *ifname)
{
...
static void babel_distribute_update(struct distribute_ctx *ctx __attribute__((__unused__)),
				    struct distribute *dist)
{
	...
	for (type = 0; type < DISTRIBUTE_MAX; type++) {
		family = type == DISTRIBUTE_V4_IN || type == DISTRIBUTE_V4_OUT ? AFI_IP : AFI_IP6;
		if (dist->list[type])
			babel_ifp->list[type] = access_list_lookup(family, dist->list[type]);
		else
			babel_ifp->list[type] = NULL;
		if (dist->prefix[type])
			babel_ifp->prefix[type] = prefix_list_lookup(family, dist->prefix[type]);
		else
...
static void babel_distribute_update_all(struct prefix_list *notused)
{
	struct vrf *vrf = vrf_lookup_by_id(VRF_DEFAULT);
	struct interface *ifp;
...
static void babel_distribute_update_all_wrapper(struct access_list *notused
...
{
	change_smoothing_half_life(no ? BABEL_DEFAULT_SMOOTHING_HALF_LIFE : seconds);
	return CMD_SUCCESS;
}
       ...
       "Interface name\n")
{
	...
	return distribute_list_parser(babel_routing_process->distribute_ctx, prefix, true,
				      argv[2 + prefix]->text, argv[1 + prefix]->arg, ifname);
}

ALIAS(babel_distribute_list, babel_distribute_list_prefix_cmd,
       ...
       "Interface name\n")
{
	...
	return distribute_list_no_parser(babel_routing_process->distribute_ctx, vty, prefix, true,
					 argv[3 + prefix]->text, argv[2 + prefix]->arg, ifname);
}

ALIAS(babel_no_distribute_list, babel_no_distribute_list_prefix_cmd,
       ...
       "Interface name\n")
{
	...
	return distribute_list_parser(babel_routing_process->distribute_ctx, prefix, false,
				      argv[3 + prefix]->text, argv[2 + prefix]->arg, ifname);
}

ALIAS(babel_ipv6_distribute_list, babel_ipv6_distribute_list_prefix_cmd,
       ...
       "Interface name\n")
{
	...
	return distribute_list_no_parser(babel_routing_process->distribute_ctx, vty, prefix, false,
					 argv[4 + prefix]->text, argv[3 + prefix]->arg, ifname);
}

ALIAS(babel_no_ipv6_distribute_list, babel_no_ipv6_distribute_list_prefix_cmd,
...
int input_filter(const unsigned char *id __attribute__((__unused__)), const unsigned char *prefix,
		 unsigned short plen, const unsigned char *neigh __attribute__((__unused__)),
		 unsigned int ifindex)
{
	return babel_filter(0, prefix, plen, ifindex);
}

int output_filter(const unsigned char *id __attribute__((__unused__)), const unsigned char *prefix,
...
/* There's no redistribute filter in Quagga -- the zebra daemon does its
   own filtering. */
int redistribute_filter(const unsigned char *prefix, unsigned short plen, unsigned int ifindex,
			int proto)
{
	return 0;
}
