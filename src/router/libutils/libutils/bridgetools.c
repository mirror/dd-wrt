...
int br_set_vlan_filtering(const char *br, int on)
{
	...
	if (on) {
		eval("ip", "link", "set", "dev", br, "type", "bridge", "no_linklocal_learn",
		     nvram_default_nget("0", "%s_noll", br));
		eval("ip", "link", "set", "dev", br, "type", "bridge", "fdb_max_learned",
		     nvram_default_nget("4096", "%s_fdbmax", br));
