...
static int ifcompare(const void *a, const void *b)
{
	...
	if (!strncmp(src, "lan", 3) && !strncmp(dst, "lan", 3)) {
		if (strlen(src) == 3 && strlen(dst) > 3)
			return -1;
		if (strlen(dst) == 3 && strlen(src) > 3)
			return 1;
	}
...
void set_smp_affinity(int irq, int mask)
{
	if (nvram_match("console_debug", "1"))
		dd_loginfo(__func__, "set smp_affinity mask %x for irq %d", mask, irq);
	char s_cpu[32];
			...
			if (type) {
				...
				if (d && *d) {
					...
					if (!strncmp(cmp, "sd", 2))
						...
						cmp[3] = 0;
					else if (!strncmp(cmp, "loop", 4) && strncmp(cmp, "loop-", 5))
						cmp[4] = 0;
					else if (!strncmp(cmp, "mmcblk", 6))
						cmp[7] = 0;
					else if (!strncmp(cmp, "nvme", 4) && strncmp(cmp, "nvme-", 4))
						cmp[7] = 0;
				}
				char grep[128];
