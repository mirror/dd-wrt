...
typedef struct _OID_BA_ORI_ENTRY {
	unsigned char MACAddr[6];
	unsigned char
		BaBitmap; // if (BaBitmap&(1<<TID)), this session with{MACAddr, TID}exists, so read BufSize[TID] for BufferSize, read ORI_BA_Status[TID] for status
	unsigned char rsv;
	unsigned char BufSize[8];
...
typedef struct _OID_BACAP_STRUC {
	...
	unsigned char
		Policy; // 0: DELAY_BA 1:IMMED_BA  (//BA Policy subfiled value in ADDBA frame)   2:BA-not use. other value invalid
	unsigned char
...
} RT_WPA_SUPPLICANT_KEY_MGMT,
	*PRT_WPA_SUPPLICANT_KEY_MGMT;
...
} RT_WPA_SUPPLICANT_EAP,
	*PRT_WPA_SUPPLICANT_EAP;
...
} RT_WPA_SUPPLICANT_TUNNEL,
	*PRT_WPA_SUPPLICANT_TUNNEL;
#endif
