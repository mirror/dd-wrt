...
static int internal_eval_va(int space, const char *cmd, va_list args)
{
	...
	int ret = _evalpid((char *const *)s_args, silence ? ">/dev/null" : ">/dev/console", 0, NULL);
	i = 1;
	while (1) {
		if (!space || !s_args[i])
			break;
...
static char *psname_argv0(int pid, char *buffer, int maxlen)
{
	...
	*buffer = 0;
	...
	if (f_read_string(path, buf, sizeof(buf)) > 0) {
		strlcpy_compat(buffer, buf, maxlen);
	}
	return buffer;
}
...
static int _pidof(const char *name, pid_t **pids)
{
	...
	if ((dir = opendir("/proc")) != NULL) {
		while ((de = readdir(dir)) != NULL) {
			...
			if (*e != 0)
				continue;
			if (strncmp(altname, psname(i, buf, sizeof(buf)), 15) == 0 ||
			    strcmp(name, psname_argv0(i, buf, sizeof(buf))) == 0) {
				if (getpsstate(i) == 'Z')
					return -1;
				if (pids) {
	...
	while (entry) {
		...
		if (entry->blocked == 1 && entry->end && entry->end < cur) {
			...
			change = 1;
			   ...
			   (entry->blocked == 0 && entry->seen && entry->seen + (7 * 24 * 60 * 60) < cur)) {
			last->next = entry->next;
			free(entry);
			entry = last->next;
			change = 1;
