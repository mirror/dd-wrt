...
static int bias_2g_ht40[] = { 50, 50, 100, 50,	50, 50, 50,
			      50, 50, 50,  100, 50, 50 };
#else
static int bias_2g[] = {
	100, 50, 75, 50, 100, 50, 75, 50, 100, 50, 75, 50, 100
};
static int bias_2g_ht40[] = { 50, 50, 100, 50,	50, 50, 50,
			      50, 50, 50,  100, 50, 50 };
#endif
...
static int freq_list(struct unl *unl, int phy, const char *freq_range,
		     struct dd_list_head *frequencies)
{
...
static int freq_add_stats(struct nl_msg *msg, void *data)
{
	...
	if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_RX]) {
		time = (unsigned long long)nla_get_u64(
...
struct wifi_channels *get_chan(struct wifi_channels *wifi_channels, int freq,
			       const char *interface)
{
	...
	if (chan && chan->freq != CHANNEL_EOF) {
		if (freq >= 4000 &&
		    (nvram_nmatch("ng-only", "%s_net_mode", interface) ||
		...
		if (freq < 4000 &&
		    (nvram_nmatch("a-only", "%s_net_mode", interface) ||
		...
		if ((!strcmp(getUEnv("region"), "AP") ||
		    ...
		    ieee80211_mhz2ieee(freq) > 11 &&
		    ieee80211_mhz2ieee(freq) < 14 &&
...
static int freq_quality(struct wifi_channels *wifi_channels, int _max_eirp,
			int _htflags, struct frequency *f, struct sort_data *s,
			const char *interface)
{
	int c;
	int idx;
	...
	struct wifi_channels *chan =
		get_chan(wifi_channels, f->freq, interface);
	...
	if (f->active && f->active_count && f->busy_count) {
		...
		if (f->freq >= 2412 && f->freq <= 2484 &&
		    idx < ARRAY_SIZE(bias_2g))
			c = (c * bias[idx]) / 100;
	} else {
		c = 100;
	}
...
int getsurveystats(struct dd_list_head *frequencies,
		   struct wifi_channels **channels, const char *interface,
		   char *freq_range, int scans, int bw)
{
	struct frequency *f;
	int verbose = 0;
	struct unl unl;
	...
	if (!country)
		country = "DE";
	wifi_channels =
		mac80211_get_channels(&unl, interface, country, bw, 0xff, 1);
	if (channels)
...
struct mac80211_ac *mac80211autochannel(const char *interface, char *freq_range,
					int scans, int enable_passive,
					int htflags)
{
	...
	if (getsurveystats(&frequencies, &wifi_channels, interface, freq_range,
			   scans, bw))
		goto out;
	...
	dd_list_for_each_entry(f, &frequencies, list)
	{
		...
		if (!f->noise)
			f->noise = -95;
		f->quality = freq_quality(wifi_channels, _max_eirp, _htflags, f,
					  &sdata, interface);
	}
			...
			if (freq) {
				dd_list_for_each_entry(f, &frequencies, list)
				{
					if (f->freq == freq) {
						dd_loginfo(
							"autochannel",
							"%s: %d already in use by %s, reduce quality\n",
							interface, freq, dev);
						f->quality /= 2;
					}
				}
	...
	dd_list_for_each_entry(f, &frequencies, list)
	{
		dd_loginfo("autochannel",
			   "%s: freq:%d qual:%d noise:%d eirp: %d\n", interface,
	...
	dd_list_for_each_entry(f, &frequencies, list)
	{
		...
		case 40:
			if (chan->luu) {
				...
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq - 20, &frequencies),
					&sdata, interface);
				acs->quality /= 2;
				acs->noise = f->noise;
				acs->luu = 1;
		...
		case 80:
			if (chan->ull) {
				...
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq + 40, &frequencies),
					&sdata, interface);
				acs->quality /= 4;
				acs->noise = f->noise;
				acs->ull = 1;
			...
			if (chan->ulu) {
				...
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq + 60, &frequencies),
					&sdata, interface);
				acs->quality /= 4;
				acs->noise = f->noise;
				acs->ulu = 1;
		...
		case 160:
			if (chan->luu) {
				...
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq - 80, &frequencies),
					&sdata, interface);
				acs->quality /= 5;
				acs->noise = f->noise;
				acs->luu = 1;
			...
			if (chan->lll) {
				...
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq - 140, &frequencies),
					&sdata, interface);
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq - 20, &frequencies),
				...
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq - 120, &frequencies),
					&sdata, interface);
				acs->quality /= 7;
				acs->noise = f->noise;
				acs->lll = 1;
			...
			if (chan->uuu) {
				...
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq + 100, &frequencies),
					&sdata, interface);
				acs->quality += freq_quality(
					wifi_channels, _max_eirp, _htflags,
					get_freq(f->freq + 120, &frequencies),
	...
	while (acs) {
		...
		if (acs->quality > lastq) {
			if (bw > 20) {
				if (!acs->ulu && !acs->lul && !acs->uul &&
				    !acs->llu && !acs->uuu && !acs->lll &&
				    !acs->ull && !acs->luu)
...
static struct mac80211_ac *add_to_mac80211_ac(struct mac80211_ac *list_root)
{
	...
	if (new == NULL) {
		fprintf(stderr,
			"mac80211_autochannel add_to_mac80211_ac: Out of memory!\n");
		return (NULL);
	} else {
		new->next = list_root;
		return (new);
