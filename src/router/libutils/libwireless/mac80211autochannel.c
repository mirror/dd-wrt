	...
	dd_list_for_each_entry(f, &frequencies, list) {
		...
		case 40:
			if (chan->luu) {
				...
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 20, &frequencies), &sdata)) / 2;
				acs->noise = f->noise;
				acs->luu = 1;
		...
		case 80:
			if (chan->ull) {
				...
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 20, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 20, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 40, &frequencies), &sdata)) / 2;
				acs->noise = f->noise;
				acs->ull = 1;
			...
			if (chan->ulu) {
				...
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 20, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 40, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 60, &frequencies), &sdata)) / 2;
				acs->noise = f->noise;
				acs->ulu = 1;
		...
		case 160:
			if (chan->luu) {
				...
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 40, &frequencies), &sdata)) / 2;
//				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 60, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 80, &frequencies), &sdata)) / 2;
				acs->noise = f->noise;
				acs->luu = 1;
			...
			if (chan->lll) {
				...
				acs->freq = f->freq;
				acs->quality = f->quality;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 140, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 20, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 40, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 60, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 80, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq - 120, &frequencies), &sdata)) / 2;
				acs->noise = f->noise;
				acs->lll = 1;
			...
			if (chan->uuu) {
				...
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 60, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 80, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 100, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 120, &frequencies), &sdata)) / 2;
				acs->quality = (acs->quality + freq_quality(wifi_channels, _max_eirp, _htflags, get_freq(f->freq + 140, &frequencies), &sdata)) / 2;
				acs->noise = f->noise;
				acs->uuu = 1;
