...
static int scan(struct unl *unl, int wdev, struct dd_list_head *frequencies)
{
	...
	if ((i = unl_genl_request(unl, msg, NULL, NULL)) < 0) {
		dd_loginfo("survey", "Scan request failed: %s", strerror(-i));
		ret = -1;
		goto out;
	}
	...
	if (chan && chan->freq != CHANNEL_EOF) {
		if (freq >= 4000 && freq < 5925 &&
		    (nvram_nmatch("ng-only", "%s_net_mode", interface) || nvram_nmatch("n2-only", "%s_net_mode", interface) ||
		     ...
		     nvram_nmatch("ax6-only", "%s_net_mode", interface) || nvram_nmatch("be6-only", "%s_net_mode", interface) ||
		     nvram_nmatch("beax6-only", "%s_net_mode", interface) || nvram_nmatch("b-only", "%s_net_mode", interface) ||
		...
		if (freq < 4000 &&
		    (nvram_nmatch("a-only", "%s_net_mode", interface) || nvram_nmatch("na-only", "%s_net_mode", interface) ||
		     ...
		     nvram_nmatch("ax5-only", "%s_net_mode", interface) || nvram_nmatch("be5-only", "%s_net_mode", interface) ||
		     nvram_nmatch("be6-only", "%s_net_mode", interface) || nvram_nmatch("beax5-only", "%s_net_mode", interface) ||
		     nvram_nmatch("beax6-only", "%s_net_mode", interface) || nvram_nmatch("xacn-mixed", "%s_net_mode", interface) ||
		...
		if ((!strcmp(getUEnv("region"), "AP") || !strcmp(getUEnv("region"), "US")) &&
		    ieee80211_mhz2ieee(interface, freq) > 11 && ieee80211_mhz2ieee(interface, freq) < 14 &&
...
static int freq_quality(struct wifi_channels *wifi_channels, int _max_eirp, int _htflags, struct frequency *f, struct sort_data *s,
			const char *interface, struct site_survey_list *list)
{
	int i;
	int c;
	int idx;
...
struct mac80211_ac *mac80211autochannel(const char *interface, char *freq_range, int scans, int enable_passive, int htflags, int band)
{
	...
	int _max_eirp;
	struct wifi_channels *wifi_channels;
	struct site_survey_list *list = NULL;
	...
	dd_list_for_each_entry(f, &frequencies, list)
	{
		...
		if (!f->noise)
			f->noise = -95;
		f->quality = freq_quality(wifi_channels, _max_eirp, _htflags, f, &sdata, interface, list);
	}
	...
	dd_list_for_each_entry(f, &frequencies, list)
	{
		dd_loginfo("autochannel", "%s: freq:%d qual:%d noise:%d eirp: %d", interface, f->freq, f->quality, f->noise,
			   f->eirp);
	}
	...
	dd_list_for_each_entry(f, &frequencies, list)
	{
		...
		struct wifi_channels *chan = get_chan(wifi_channels, f->freq, interface);
		if (!chan) {
			dd_loginfo("autochannel", "chan %d not found, curious", f->freq);
			continue;
		}
		if (chan->freq < 0)
			continue;
		...
		case 40:
			if (chan->luu) {
				...
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq - 20, &frequencies), &sdata, interface, list);
				acs->quality /= 2;
				acs->noise = f->noise;
				acs->luu = 1;
		...
		case 80:
			if (chan->ull) {
				...
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq + 20, &frequencies), &sdata, interface, list);
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq + 40, &frequencies), &sdata, interface, list);
				acs->quality /= 4;
				acs->noise = f->noise;
				acs->ull = 1;
				dd_loginfo("autochannel", "%s: freq: %d VHT80 [UL] quality %d", interface, f->freq, acs->quality);
			}
			if (chan->luu) {
				acs = add_to_mac80211_ac(acs);
				acs->freq = f->freq;
				...
				acs->noise = f->noise;
				acs->luu = 1;
				dd_loginfo("autochannel", "%s: freq: %d VHT80 [LU] quality %d", interface, f->freq, acs->quality);
			}
			if (chan->ulu) {
				acs = add_to_mac80211_ac(acs);
				acs->freq = f->freq;
				...
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq + 40, &frequencies), &sdata, interface, list);
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq + 60, &frequencies), &sdata, interface, list);
				acs->quality /= 4;
				acs->noise = f->noise;
				acs->ulu = 1;
				dd_loginfo("autochannel", "%s: freq: %d VHT80 [UU] quality %d", interface, f->freq, acs->quality);
			}
			if (chan->lul) {
				acs = add_to_mac80211_ac(acs);
				acs->freq = f->freq;
				...
				acs->noise = f->noise;
				acs->lul = 1;
				dd_loginfo("autochannel", "%s: freq: %d VHT80 [LL] quality %d", interface, f->freq, acs->quality);
			}
			break;
		case 160:
			if (chan->luu) {
				...
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq - 60, &frequencies), &sdata, interface, list);
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq - 80, &frequencies), &sdata, interface, list);
				acs->quality /= 8;
				acs->noise = f->noise;
				acs->luu = 1;
			...
			if (chan->lll) {
				...
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq - 80, &frequencies), &sdata, interface, list);
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq - 100, &frequencies), &sdata, interface, list);
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq - 120, &frequencies), &sdata, interface, list);
				acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
							     get_freq(f->freq - 140, &frequencies), &sdata, interface, list);
				acs->quality /= 8;
				acs->noise = f->noise;
				acs->lll = 1;
			...
			break;
		case 320:
			int chan = ieee80211_mhz2ieee(interface, f->freq);
			acs = add_to_mac80211_ac(acs);
			acs->freq = f->freq;
			acs->quality = f->quality;
			acs->noise = f->noise;

			unsigned int methods[16][16] = {
				{ +20, +40, +60, +80, +100, +120, +140, +160, +180, +200, +220, +240, +260, +280, +300 },
				{ -20, +20, +40, +60, +80, +100, +120, +140, +160, +180, +200, +220, +240, +260, +280 },
				{ -40, -20, +20, +40, +60, +80, +100, +120, +140, +160, +180, +200, +220, +240, +260 },
				{ -60, -40, -20, +20, +40, +60, +80, +100, +120, +140, +160, +180, +200, +220, +240 },
				{ -80, -60, -40, -20, +20, +40, +60, +80, +100, +120, +140, +160, +180, +200, +220 },
				{ -100, -80, -60, -40, -20, +20, +40, +60, +80, +100, +120, +140, +160, +180, +200 },
				{ -120, -100, -80, -60, -40, -20, +20, +40, +60, +80, +100, +120, +140, +160, +180 },
				{ -140, -120, -100, -80, -60, -40, -20, +20, +40, +60, +80, +100, +120, +140, +160 },
				{ -160, -140, -120, -100, -80, -60, -40, -20, +20, +40, +60, +80, +100, +120, +140 },
				{ -180, -160, -140, -120, -100, -80, -60, -40, -20, +20, +40, +60, +80, +100, +120 },
				{ -200, -180, -160, -140, -120, -100, -80, -60, -40, -20, +20, +40, +60, +80, +100 },
				{ -220, -200, -180, -160, -140, -120, -100, -80, -60, -40, -20, +20, +40, +60, +80 },
				{ -240, -220, -200, -180, -160, -140, -120, -100, -80, -60, -40, -20, +20, +40, +60 },
				{ -260, -240, -220, -200, -180, -160, -140, -120, -100, -80, -60, -40, -20, +20, +40 },
				{ -280, -260, -240, -220, -200, -180, -160, -140, -120, -100, -80, -60, -40, -20, +20 },
			...
			if (calcidx < 16) {
				...
				for (i = 0; i < 16; i++) {
					acs->quality += freq_quality(wifi_channels, _max_eirp, _htflags,
								     get_freq(f->freq + methods[calcidx][i], &frequencies), &sdata,
								     interface, list);
				}
				acs->quality /= 16;
			}
		...
		default:
			...
			acs->quality = f->quality;
			acs->noise = f->noise;
			dd_loginfo("autochannel", "%s: freq: %d default quality %d", interface, f->freq, acs->quality);
			break;
		}
	}
