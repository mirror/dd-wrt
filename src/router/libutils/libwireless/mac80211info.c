...
{
	return (phy_lookup_by_number(mac80211_get_phy_ifname(vif)));
}
...
int has_n(const char *interface)
{
	INITVALUECACHEi(interface);
	char *htcaps = mac80211_get_caps(interface, 1, 1, 1, 1, 0);
	if (strlen(htcaps)) {
...
{
	mac80211_init();
	int phy = mac80211_get_phy_ifname(prefix);
	struct nlattr *tb[NL80211_ATTR_MAX + 1];
	struct nl_msg *msg;
	struct genlmsghdr *gnlh;
	void *ret;
...
static struct wifi_channels ghz60channels[] = {
	...
	//      {.channel = 5,.freq = 66960,.max_eirp = 40,.hw_eirp = 40},
	//      {.channel = 6,.freq = 69120,.max_eirp = 40,.hw_eirp = 40},
	//      {.channel = 7,.freq = 71280,.max_eirp = 40,.hw_eirp = 40},
	//      {.channel = 8,.freq = 73440,.max_eirp = 40,.hw_eirp = 40},
	{ .channel = -1, .freq = -1, .max_eirp = -1, .hw_eirp = -1 },
};

static void sort_channels(struct wifi_channels *list, int count)
{
	int i, j;
	for (j = 0; j < count; j++) {
		for (i = 1; i < count; i++) {
...
static MICRO_MAP us_chans[] = {
	{ 902000, 5660, 40 },  { 903000, 5660, 20 },

	{ 904000, 5180, 160 }, { 905000, 5200, 40 }, { 906000, 5220, 80 }, { 907000, 5240, 80 },
	{ 908000, 5260, 80 },  { 909000, 5280, 80 }, { 910000, 5300, 40 }, { 911000, 5320, 20 },

	{ 912000, 5500, 160 }, { 913000, 5520, 40 }, { 914000, 5540, 80 }, { 915000, 5560, 80 },
	{ 916000, 5580, 80 },  { 917000, 5600, 80 }, { 918000, 5610, 40 }, { 919000, 5620, 20 },

	{ 920000, 5745, 160 }, { 921000, 5765, 40 }, { 922000, 5785, 80 }, { 923000, 5805, 80 },
	{ 924000, 5825, 80 },  { 925000, 5845, 80 }, { 926000, 5865, 40 }, { 927000, 5885, 20 },
};

static MICRO_MAP nz_chans[] = {
	{ 915000, 5560, 20 },  { 916000, 5580, 80 }, { 917000, 5600, 40 }, { 918000, 5620, 40 }, { 919000, 5640, 20 },

	{ 920000, 5745, 160 }, { 921000, 5765, 40 }, { 922000, 5785, 80 }, { 923000, 5805, 80 }, { 924000, 5825, 80 },
	{ 925000, 5845, 80 },  { 926000, 5865, 40 }, { 927000, 5885, 20 },
};

static MICRO_MAP kr_chans[] = {
	{ 926000, 5310, 20 },

	{ 917500, 5660, 40 }, { 918500, 5680, 20 },

	{ 921500, 5220, 40 }, { 922500, 5240, 20 },

	{ 926000, 5310, 40 }, { 927000, 5330, 20 },

	{ 928000, 5510, 40 }, { 929000, 5530, 20 },

	{ 919500, 5180, 80 }, { 920500, 5200, 40 }, { 921500, 5220, 40 }, { 922500, 5240, 20 },
};

static MICRO_MAP jp_chans[] = {
	{ 920500, 5540, 20 },

	{ 922500, 5180, 80 }, { 923500, 5200, 40 }, { 924500, 5220, 40 }, { 925500, 5240, 20 },

	{ 923500, 5260, 80 }, { 924500, 5280, 40 }, { 925500, 5300, 40 }, { 926500, 5320, 20 },
};

static MICRO_MAP gb_chans[] = {
	{ 867000, 5220, 20 },

	{ 863000, 5660, 40 }, { 864000, 5680, 20 },

	{ 865000, 5180, 40 }, { 866000, 5200, 20 },
...
static MICRO_MAP au_chans[] = {
	...
	{ 916000, 5580, 80 },  { 917000, 5600, 40 }, { 918000, 5620, 40 }, { 919000, 5640, 20 },
