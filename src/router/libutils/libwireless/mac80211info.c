...
static struct wifi_client_info *
add_to_wifi_clients(struct wifi_client_info *list_root);
...
static int mac80211_cb_survey(struct nl_msg *msg, void *data)
{
	...
	if (sinfo[NL80211_SURVEY_INFO_IN_USE]) {
		...
		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME] &&
			...
			mac80211_info->channel_active_time = nla_get_u64(
				sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME]);
			mac80211_info->channel_busy_time = nla_get_u64(
				sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_BUSY]);
		}

		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_RX])
			mac80211_info->channel_receive_time = nla_get_u64(
				sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_RX]);

		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_TX])
			mac80211_info->channel_transmit_time = nla_get_u64(
				sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_TX]);

		if (sinfo[NL80211_SURVEY_INFO_CHANNEL_TIME_EXT_BUSY])
			mac80211_info->extension_channel_busy_time = nla_get_u64(
...
static void getNoise_mac80211_internal(char *interface,
...
getcurrentsurvey_mac80211(const char *interface,
			  struct mac80211_info *mac80211_info)
{
	mac80211_init();
	lock();
...
int mac80211_get_coverageclass(char *interface)
{
	...
	if (nvram_nmatch("0", "%s_distance", ifname)) {
		char str[64];
		sprintf(str, "/sys/kernel/debug/ieee80211/phy%d/ath9k/ack_to",
			phy);
		...
		sprintf(str, "/sys/kernel/debug/ieee80211/phy%d/ath5k/ack_to",
			phy);
		fp = fopen(str, "rb");
		if (fp) {
			int ack;
...
int mac80211_get_maxpower(char *interface)
{
	...
	if (tb[NL80211_ATTR_WIPHY_BANDS]) {
		nla_for_each_nested(nl_band, tb[NL80211_ATTR_WIPHY_BANDS],
				    rem_band) {
...
static int mac80211_cb_stations(struct nl_msg *msg, void *data)
{
	...
	memcpy(mac80211_info->wci->etheraddr, nla_data(tb[NL80211_ATTR_MAC]),
	       6);
	...
	if (sinfo[NL80211_STA_INFO_INACTIVE_TIME]) {
		mac80211_info->wci->inactive_time =
			nla_get_u32(sinfo[NL80211_STA_INFO_INACTIVE_TIME]);
	}
	if (sinfo[NL80211_STA_INFO_RX_BYTES]) {
		mac80211_info->wci->rx_bytes =
			nla_get_u32(sinfo[NL80211_STA_INFO_RX_BYTES]);
	}
	if (sinfo[NL80211_STA_INFO_RX_PACKETS]) {
		mac80211_info->wci->rx_packets =
	...
	if (sinfo[NL80211_STA_INFO_TX_BYTES]) {
		mac80211_info->wci->tx_bytes =
			nla_get_u32(sinfo[NL80211_STA_INFO_TX_BYTES]);
	}
	if (sinfo[NL80211_STA_INFO_TX_PACKETS]) {
		mac80211_info->wci->tx_packets =
			nla_get_u32(sinfo[NL80211_STA_INFO_TX_PACKETS]);
	}
	if (sinfo[NL80211_STA_INFO_TX_COMPRESSED]) {
		mac80211_info->wci->tx_compressed =
			nla_get_u32(sinfo[NL80211_STA_INFO_TX_COMPRESSED]);
	}
	get_chain_signal(sinfo[NL80211_STA_INFO_CHAIN_SIGNAL],
			 mac80211_info->wci->chaininfo);
	if (sinfo[NL80211_STA_INFO_SIGNAL_AVG]) {
		mac80211_info->wci->signal =
			(int8_t)nla_get_u8(sinfo[NL80211_STA_INFO_SIGNAL_AVG]);
	}
	get_chain_signal(sinfo[NL80211_STA_INFO_CHAIN_SIGNAL_AVG],
			 mac80211_info->wci->chaininfo_avg);

	if (sinfo[NL80211_STA_INFO_DATA_ACK_SIGNAL_AVG]) {
		mac80211_info->wci->signal_avg = (int8_t)nla_get_u8(
			sinfo[NL80211_STA_INFO_DATA_ACK_SIGNAL_AVG]);
	...
	if (sinfo[NL80211_STA_INFO_CONNECTED_TIME]) {
		mac80211_info->wci->uptime =
			nla_get_u32(sinfo[NL80211_STA_INFO_CONNECTED_TIME]);
	}
	if (sinfo[NL80211_STA_INFO_RADIONAME]) {
		strcpy(mac80211_info->wci->radioname,
	...
	if (sinfo[NL80211_STA_INFO_TX_BITRATE]) {
		if (nla_parse_nested(rinfo, NL80211_RATE_INFO_MAX,
				     ...
				     rate_policy)) {
			fprintf(stderr,
				"failed to parse nested tx rate attributes!\n");
		} else {
			if (rinfo[NL80211_RATE_INFO_BITRATE]) {
				mac80211_info->wci->txrate = nla_get_u16(
					rinfo[NL80211_RATE_INFO_BITRATE]);
			}

			if (rinfo[NL80211_RATE_INFO_MCS]) {
				mac80211_info->wci->mcs = nla_get_u8(
					rinfo[NL80211_RATE_INFO_MCS]);
	...
	if (d->iftype && sinfo[NL80211_STA_INFO_EXPECTED_THROUGHPUT]) {
		unsigned int tx = nla_get_u32(
			sinfo[NL80211_STA_INFO_EXPECTED_THROUGHPUT]);
		tx = tx * 1000;
		tx = tx / 1024;
	...
	if (sinfo[NL80211_STA_INFO_RX_BITRATE]) {
		if (nla_parse_nested(rinfo, NL80211_RATE_INFO_MAX,
				     ...
				     rate_policy)) {
			fprintf(stderr,
				"failed to parse nested rx rate attributes!\n");
		} else {
			if (rinfo[NL80211_RATE_INFO_BITRATE]) {
				mac80211_info->wci->rxrate = nla_get_u16(
					rinfo[NL80211_RATE_INFO_BITRATE]);
			}

			if (rinfo[NL80211_RATE_INFO_MCS]) {
				mac80211_info->wci->rx_mcs = nla_get_u8(
					rinfo[NL80211_RATE_INFO_MCS]);
...
struct mac80211_info *mac80211_assoclist(char *interface)
{
	...
	if (interface)
		asprintf(&globstring,
			 "/sys/class/ieee80211/phy*/device/net/%s*", interface);
	else
			...
			int ht40, int ldpc, int smps)
{
	...
	nla_for_each_nested(band, bands, rem) {
		caps = nla_find(nla_data(band), nla_len(band),
				NL80211_BAND_ATTR_HT_CAPA);
		if (!caps)
			continue;
		cap = nla_get_u16(caps);
		asprintf(&capstring, "%s%s%s%s%s%s%s%s%s%s%s%s%s",
			 ((cap & HT_CAP_INFO_LDPC_CODING_CAP) && ldpc ?
				  "[LDPC]" :
				  ""),
			 (((cap & HT_CAP_INFO_SHORT_GI20MHZ) && shortgi) ?
				  "[SHORT-GI-20]" :
				  ""),
			 (((cap & HT_CAP_INFO_SHORT_GI40MHZ) && shortgi) ?
				  "[SHORT-GI-40]" :
				  ""),
			 ...
			 ((cap & HT_CAP_INFO_DSSS_CCK40MHZ) ? "[DSSS_CCK-40]" :
							      ""),
			 ((cap & HT_CAP_INFO_GREEN_FIELD && greenfield) ?
				  "[GF]" :
				  ""),
			 (cap & HT_CAP_INFO_DELAYED_BA ? "[DELAYED-BA]" : ""),
			 ((((cap >> 2) & 0x3) == 0 && !is_brcmfmac(interface)) ?
				  smps == 1 ? "[SMPS-STATIC]" : "" :
				  ""),
			 (((cap >> 2) & 0x3) == 1 ?
				  smps == 2 ? "[SMPS-DYNAMIC]" : "" :
				  ""),
			 (cap & HT_CAP_INFO_MAX_AMSDU_SIZE ?
				  "[MAX-AMSDU-7935]" :
				  ""));
	}
out:
...
char *mac80211_get_vhtcaps(const char *interface, int shortgi, int vht80,
			   int vht160, int vht8080, int su_bf, int mu_bf)
{
	mac80211_init();
	struct nl_msg *msg;
	struct nlattr *caps, *bands, *band;
	...
	nla_for_each_nested(band, bands, rem) {
		...
		asprintf(
			&capstring,
			"%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s[MAX-A-MPDU-LEN-EXP%d]%s%s%s%s%s%s",
			(cap & VHT_CAP_RXLDPC ? "[RXLDPC]" : ""),
			(((cap & VHT_CAP_SHORT_GI_80) && shortgi && has5ghz &&
			  (vht80 || vht160)) ?
				 "[SHORT-GI-80]" :
				 ""),
			(((cap & VHT_CAP_SHORT_GI_160) && shortgi && has5ghz &&
			  vht160) ?
				 "[SHORT-GI-160]" :
				 ""),
			...
			(((cap & VHT_CAP_SU_BEAMFORMER_CAPABLE) && su_bf) ?
				 "[SU-BEAMFORMER]" :
				 ""),
			(((cap & VHT_CAP_MU_BEAMFORMER_CAPABLE) && mu_bf) ?
				 "[MU-BEAMFORMER]" :
				 ""),
			...
			(cap & VHT_CAP_RX_ANTENNA_PATTERN ?
				 "[RX-ANTENNA-PATTERN]" :
				 ""),
			(cap & VHT_CAP_TX_ANTENNA_PATTERN ?
				 "[TX-ANTENNA-PATTERN]" :
				 ""),
			((cap & 3) == 1 ? "[MAX-MPDU-7991]" : ""),
			((cap & 3) == 2 ? "[MAX-MPDU-11454]" : ""),
			(((cap & VHT_CAP_SUPP_CHAN_WIDTH_160MHZ) && has5ghz &&
			  vht160) ?
				 "[VHT160]" :
				 ""),
			(((cap & VHT_CAP_SUPP_CHAN_WIDTH_160_80PLUS80MHZ) &&
			  has5ghz && (vht8080 || vht160)) ?
				 "[VHT160-80PLUS80]" :
				 ""),
			((cap & VHT_CAP_HTC_VHT) ?
				 ((cap &
			...
			(((cap & VHT_CAP_SU_BEAMFORMEE_CAPABLE) &&
			  su_bf) || ((cap & VHT_CAP_MU_BEAMFORMEE_CAPABLE) &&
				     mu_bf) && (bfantenna & 1) ?
				 "[BF-ANTENNA-2]" :
				 ""),
			(((cap & VHT_CAP_SU_BEAMFORMER_CAPABLE) &&
			  su_bf) || ((cap & VHT_CAP_MU_BEAMFORMER_CAPABLE) &&
				     mu_bf) && (sodimension & 1) ?
...
static void *iftype_worker(struct nlattr **tb, void *priv)
{
	...
	if (tb[NL80211_ATTR_SUPPORTED_IFTYPES]) {
		nla_for_each_nested(nl_mode, tb[NL80211_ATTR_SUPPORTED_IFTYPES],
				    rem_mode)
...
static void *feature_worker(struct nlattr **tb, void *priv)
{
	...
	if (tb[NL80211_ATTR_FEATURE_FLAGS]) {
		unsigned int features =
			nla_get_u32(tb[NL80211_ATTR_FEATURE_FLAGS]);
	...
	if (tb[NL80211_ATTR_CIPHER_SUITES]) {
		*num = nla_len(tb[NL80211_ATTR_CIPHER_SUITES]) / sizeof(__u32);
		if (*num > 0) {
			...
			memcpy(ciphers,
			       nla_data(tb[NL80211_ATTR_CIPHER_SUITES]),
			       *num * sizeof(__u32));
		}
	}
	return ciphers;
...
static void *
mac80211_has_worker(const char *prefix,
		    void *(*worker)(struct nlattr **tb, void *priv), void *priv)
{
	mac80211_init();
...
int mac80211_check_band(const char *interface, int checkband)
{
	...
	nla_for_each_nested(band, bands, rem) {
		...
		nla_for_each_nested(freq, freqlist, rem2) {
			nla_parse_nested(tb, NL80211_FREQUENCY_ATTR_MAX, freq,
					 freq_policy);
...
static int isinlist(struct wifi_channels *list, struct wifi_channels *freq,
		    int range, int bw)
{
	int i = 0;
	struct wifi_channels *chan;
	while ((chan = &list[i++])->freq > 0) {
		if ((bw == 40 && !chan->ht40) || (bw == 80 && !chan->vht80) ||
		    (bw == 160 && !chan->vht160))
			continue;
		if (chan->freq == freq->freq + range &&
		    chan->band == freq->band) {
			return 1;
		}
	}
...
static int check_ranges(char *name, struct wifi_channels *list,
			struct wifi_channels *chan, int *ranges, int mhz)
{
	int i = 0;
	int range;
...
#define VHT160RANGE(offset)                                                    \
	(int[])                                                                \
	{                                                                      \
		offset + 70, offset + 50, offset - 50, offset - 70,            \
...
#define VHTRANGE(width, offset) \
	width == 160 ? VHT160RANGE(offset) : VHT80RANGE(offset)
				...
				int nooverlap)
{
	...
	while (1) {
		...
		if (bw == 40) {
			if (check_ranges("LOWER", list, chan,
					 (int[]){ -20, -1 }, 40)) {
				chan->luu = 1;
			}
			if (check_ranges("UPPER", list, chan, (int[]){ 20, -1 },
					 40)) {
				chan->ull = 1;
			}
		...
		if (bw == 80) {
			if (check_ranges("LL", list, chan, VHTRANGE(80, -30),
					 80)) {
				chan->lul = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("UU", list, chan, VHTRANGE(80, 30),
					 80)) {
				chan->ulu = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("UL", list, chan, VHTRANGE(80, -10),
					 80)) {
				chan->luu = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("LU", list, chan, VHTRANGE(80, 10),
					 80)) {
				chan->ull = 1;
				if (nooverlap)
					goto next;
		...
		if (bw == 160) {
			if (check_ranges("LLL", list, chan, VHTRANGE(160, -70),
					 160)) {
				chan->lll = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("UUU", list, chan, VHTRANGE(160, 70),
					 160)) {
				chan->uuu = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("LLU", list, chan, VHTRANGE(160, -50),
					 160)) {
				chan->llu = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("UUL", list, chan, VHTRANGE(160, 50),
					 160)) {
				chan->uul = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("LUL", list, chan, VHTRANGE(160, -30),
					 160)) {
				chan->lul = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("ULU", list, chan, VHTRANGE(160, 30),
					 160)) {
				chan->ulu = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("LUU", list, chan, VHTRANGE(160, -10),
					 160)) {
				chan->luu = 1;
				if (nooverlap)
					goto next;
			}
			if (check_ranges("ULL", list, chan, VHTRANGE(160, 10),
					 160)) {
				chan->ull = 1;
				if (nooverlap)
					goto next;
...
mac80211_get_channels(struct unl *local_unl, const char *interface,
		      const char *country, int max_bandwidth_khz,
		      unsigned char checkband, int nocache)
{
	struct nlattr *tb[NL80211_FREQUENCY_ATTR_MAX + 1];
	struct nlattr *tb_band[NL80211_BAND_ATTR_MAX + 1];
	struct nl_msg *msg;
	...
	for (run = 0; run < 2; run++) {
		...
		if (run == 1) {
			list = (struct wifi_channels *)calloc(
				sizeof(struct wifi_channels) * (chancount + 1),
				1);
		}
		nla_for_each_nested(band, bands, rem) {
			nla_parse(tb_band, NL80211_BAND_ATTR_MAX,
			...
			nla_for_each_nested(freq, freqlist, rem2) {
				...
				//for (rrc = 0; rrc < rrdcount; rrc++)
				{
					...
					if (super) {
						...
						int ccidx = 0;
						for (cc = 0; cc < rrdcount;
						     cc++) {
							regfreq =
								rd->reg_rules[cc]
									.freq_range;
							if (cc &&
							    regfreq.start_freq_khz ==
								    rd->reg_rules[cc -
										  1]
									    .freq_range
									    .end_freq_khz)
							...
							if (regfreq.end_freq_khz <=
								    stophighbound &&
							    regfreq.end_freq_khz >
								    stoplowbound) {
								stopfreq =
									regfreq.end_freq_khz /
									1000;
							...
							if (freq_mhz >
								...
								band = ccidx +
								       bandcounter;
								flags = rd->reg_rules[cc]
										.flags;
								regpower =
									rd->reg_rules[cc]
										.power_rule;
								regmaxbw =
									regfreq.max_bandwidth_khz /
									1000;
								if (ccidx !=
								    lastband) {
									firstchan =
					...
					//                                              regmaxbw = 40;
					if (!skip ||
					    ((freq_mhz - range) >= startfreq &&
					     (freq_mhz + range) <= stopfreq)) {
						if (run == 1) {
...
struct wifi_channels *mac80211_get_channels_simple(const char *interface,
						   const char *country,
						   int max_bandwidth_khz,
						   unsigned char checkband)
{
	lock();
	struct wifi_channels *chan = mac80211_get_channels(
		&unl, interface, country, max_bandwidth_khz, checkband, 1);
	unlock();
	return chan;
}
...
int can_ht40(const char *interface)
{
	...
	lock();
	chan = mac80211_get_channels(&unl, interface, getIsoName(country), 40,
				     0xff, 1);
	unlock();
	if (chan) {
		while (chan[i].freq != -1) {
...
int can_vht80(const char *interface)
{
	...
	country = nvram_default_get(regdomain, "UNITED_STATES");
	lock();
	chan = mac80211_get_channels(&unl, interface, getIsoName(country), 80,
				     0xff, 1);
	unlock();
	if (chan) {
		while (chan[i].freq != -1) {
...
int can_vht160(const char *interface)
{
	...
	country = nvram_default_get(regdomain, "UNITED_STATES");
	lock();
	chan = mac80211_get_channels(&unl, interface, getIsoName(country), 160,
				     0xff, 1);
	unlock();
	if (chan) {
		while (chan[i].freq != -1) {
...
int mac80211_check_valid_frequency(const char *interface, char *country,
				   int freq)
{
	struct wifi_channels *chan;
	int found = 0;
...
add_to_wifi_clients(struct wifi_client_info *list_root)
{
	struct wifi_client_info *new =
		calloc(1, sizeof(struct wifi_client_info));
	if (new == NULL) {
...
int mac80211_get_maxrate(char *interface)
{
	...
	nla_for_each_nested(band, bands, rem) {
		...
		nla_for_each_nested(rate, ratelist, rem2) {
			nla_parse_nested(tb, NL80211_BITRATE_ATTR_MAX, rate,
					 rate_policy);
			if (!tb[NL80211_BITRATE_ATTR_RATE])
				continue;
			maxrate = 0.1 *
				  nla_get_u32(tb[NL80211_BITRATE_ATTR_RATE]);
		}
	}
	nlmsg_free(msg);
	unlock();
...
int mac80211_get_maxmcs(char *interface)
{
	...
	nla_for_each_nested(band, bands, rem) {
		...
		if (tb[NL80211_BAND_ATTR_HT_MCS_SET] &&
			...
			maxmcs = get_ht_mcs(
				nla_data(tb[NL80211_BAND_ATTR_HT_MCS_SET]));
	}
	nlmsg_free(msg);
	unlock();
	return maxmcs;
...
int mac80211_get_maxvhtmcs(char *interface)
{
	...
	nla_for_each_nested(band, bands, rem) {
		...
		if (tb[NL80211_BAND_ATTR_VHT_CAPA] &&
			...
			maxmcs = get_vht_mcs(
				nla_get_u32(tb[NL80211_BAND_ATTR_VHT_CAPA]),
...
static int mac80211_get_antennas(char *prefix, int which, int direction)
{
	...
	if (which == 0 && direction == 0) {
		if (tb[NL80211_ATTR_WIPHY_ANTENNA_AVAIL_TX])
			ret = ((int)nla_get_u32(
				tb[NL80211_ATTR_WIPHY_ANTENNA_AVAIL_TX]));
	}
	...
	if (which == 1 && direction == 0) {
		if (tb[NL80211_ATTR_WIPHY_ANTENNA_TX])
			ret = ((int)nla_get_u32(
				tb[NL80211_ATTR_WIPHY_ANTENNA_TX]));
	}
...
struct wifi_interface *mac80211_get_interface(char *dev)
{
	...
	if (tb_msg[NL80211_ATTR_WIPHY_FREQ]) {
		interface = (struct wifi_interface *)malloc(
		...
		if (tb_msg[NL80211_ATTR_CHANNEL_WIDTH]) {
			if (tb_msg[NL80211_ATTR_CENTER_FREQ1])
				interface->center1 = nla_get_u32(
					tb_msg[NL80211_ATTR_CENTER_FREQ1]);
			if (tb_msg[NL80211_ATTR_CENTER_FREQ2])
				interface->center2 = nla_get_u32(
			...
			channel_type = nla_get_u32(
				tb_msg[NL80211_ATTR_WIPHY_CHANNEL_TYPE]);
			switch (channel_type) {
...
void main(int argc, char *argv[])
{
	...
	mac80211_get_channels("wlan1", "DE", 20, 255);
	mac80211_get_channels("wlan0", "US", 20, 255);
	fprintf(stderr, "phy0 %d %d %d %d\n", mac80211_get_avail_tx_antenna(0),
		mac80211_get_avail_rx_antenna(0),
		mac80211_get_configured_tx_antenna(0),
		mac80211_get_configured_rx_antenna(0));
	fprintf(stderr, "phy1 %d %d %d %d\n", mac80211_get_avail_tx_antenna(1),
		mac80211_get_avail_rx_antenna(1),
		mac80211_get_configured_tx_antenna(1),
		mac80211_get_configured_rx_antenna(1));
}
#endif
