...
StartContinueTx (char *value)
{
  ...
  dprintf
    ("gmode=[%s](%d), channel=[%s](%d), rate=[%s](%d), rates=(%f), txant=[%s](%d)\n",
     ...
     tx_rate ? tx_rate : "NULL", tx_rate ? rate : -1, tx_rate ? rates : -1,
     tx_ant ? tx_ant : "NULL", tx_ant ? txant : -1);
  ...
  switch (atoi (value))
      ...
      else
	{
      ...
      mysystem ("wl rate 54");
      mysystem ("wl txpwr1 -d -o 16.5");	// 2005-03-04, Set tx power to 16 dbm
      mysystem ("wl curpower > /tmp/curpower");

      if ((fp = fopen ("/tmp/curpower", "r")))
	{			// Get real value
	  char line[254];
	  char string[254];
	  char value[254];
