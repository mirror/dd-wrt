...
static void WLMD5Transform (UINT4[4], unsigned char[64]);
  ...
  if (inputLen >= partLen)
    {
      MD5_memcpy ((POINTER) & context->buffer[index], (POINTER) input,
		  partLen);
      WLMD5Transform (context->state, context->buffer);

      for (i = partLen; i + 63 < inputLen; i += 64)
	WLMD5Transform (context->state, &input[i]);
...
static unsigned char __md5__magic[] = MD5_MAGIC_STR;
...
//	if (!strncmp(sp, __md5__magic, MD5_MAGIC_LEN))
	...
	MD5Update(&ctx, __md5__magic, MD5_MAGIC_LEN);
...
//TODO: the above comment seems to be wrong. final is used later.
	...
	for (i = pw_len; i; i >>= 1) {
		MD5Update(&ctx, ((i & 1) ? final : (unsigned char *) pw), 1);
	}
