...
#define ROUTER_LINKSYS_MX4200V1 0x114134
...
#define ROUTER_ZYXEL_GS1900 0x12011b
...
extern char *get_lan_gateway(void);
extern char *get_lan_dns(void);
...
char *sha256_string(char *str, char *hashbuf, size_t len);
...
extern int morse_bwsupport(int bw);
...
static inline int morse_bwsupport(int bw)
{
	return 1;
}
#endif
...
#endif
#ifdef HAVE_ATH12K
extern int is_ath12k(const char *prefix);
#else
static inline int is_ath12k(const char *prefix)
{
	return 0;
}
#endif
...
char *getgptpartitionbyname_safe(char *retbuf, const char *dev, const char *name);
