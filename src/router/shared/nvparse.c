...
get_forward_port (int which, netconf_nat_t * nat)
{
  ...
  if (get_single_ip (lan_ipaddr, 0) !=
      ...
      || get_single_ip (lan_ipaddr,
			1) != get_single_ip (nvram_safe_get ("lan_ipaddr"), 1)
      || get_single_ip (lan_ipaddr,
			2) != get_single_ip (nvram_safe_get ("lan_ipaddr"),
					     2))
    {
...
bool
valid_filter_client (const netconf_filter_t * start,
		     const netconf_filter_t * end)
{
  ...
  if (start->match.src.netmask.s_addr)
	...
	return FALSE;
      if (ntohl (start->match.src.ipaddr.s_addr) >
...
bool
get_filter_client (int which, netconf_filter_t * start,
		   netconf_filter_t * end)
{
  ...
  if (*lan_ipaddr0 == '*')
    {
      ...
      start->match.src.ipaddr.s_addr = end->match.src.ipaddr.s_addr =
	htonl (0);
      start->match.src.netmask.s_addr = end->match.src.netmask.s_addr =
	htonl (0);
    }
  ...
  start->match.dst.ports[0] = end->match.dst.ports[0] =
    htons (atoi (lan_port0));
  start->match.dst.ports[1] = end->match.dst.ports[1] =
    htons (atoi (lan_port1));
...
bool
set_filter_client (int which, const netconf_filter_t * start,
		   const netconf_filter_t * end)
{
...
convert_filter_port (void)
{
  ...
  for (i = 1; i <= 6; i++)
	...
	goto fail;
      ...
      (void) inet_aton (nvram_safe_get ("lan_ipaddr"),
			&start.match.src.ipaddr);
	...
	htonl ((ntohl (start.match.src.ipaddr.s_addr) & ~0xff) |
      ...
      if (!(colon = strchr (value, ':')))
	goto fail;
      end.match.src.ipaddr.s_addr =
	htonl ((ntohl (end.match.src.ipaddr.s_addr) & ~0xff) |
	       strtoul (value, NULL, 0));
      value = colon + 1;
...
convert_static_route (void)
{
  ...
  foreach (word, nvram_safe_get ("static_route"), next)
  {
    ...
    if (strcmp (ifname, "lan") == 0)
      {
	lan_cur +=
	  snprintf (lan_cur, lan_route + sizeof (lan_route) - lan_cur,
		    "%s%s:%s:%s:%s", lan_cur == lan_route ? "" : " ", ipaddr,
      ...
      {
	wan_cur +=
	  snprintf (wan_cur, wan_route + sizeof (wan_route) - wan_cur,
		    "%s%s:%s:%s:%s", wan_cur == wan_route ? "" : " ", ipaddr,
		    netmask, gateway, metric);
      }
#endif /* __CONFIG_NAT__ */
    /* what to do? */
...
convert_deprecated (void)
{
  ...
  if (nvram_invmatch ("pppoe_ifname", "")
