 ...
 *
 * $Id: netconf.h,v 1.1 2005/09/26 11:09:08 seg Exp $
 */
...
#define NETCONF_ESTABLISHED \
	0x02 /* Packet is related to an existing
					 * connection */
#define NETCONF_RELATED \
	0x04 /* Packet is part of an established
					 * connection */
#define NETCONF_NEW \
	0x08 /* Packet is trying to establish a
					 * new connection */

/*
 * Supported match flags 
 */
#define NETCONF_INV_SRCIP \
	0x01 /* Invert the sense of source IP
					 * address */
#define NETCONF_INV_DSTIP \
	0x02 /* Invert the sense of destination IP 
					 * address */
#define NETCONF_INV_SRCPT \
	0x04 /* Invert the sense of source port
					 * range */
#define NETCONF_INV_DSTPT \
	0x08 /* Invert the sense of destination
					 * port range */
#define NETCONF_INV_MAC \
	0x10 /* Invert the sense of source MAC
					 * address */
#define NETCONF_INV_IN \
	0x20 /* Invert the sense of inbound
					 * interface */
#define NETCONF_INV_OUT \
	0x40 /* Invert the sense of outbound
					 * interface */
#define NETCONF_INV_STATE 0x80 /* Invert the sense of state */
#define NETCONF_INV_DAYS \
	0x100 /* Invert the sense of day of the
					 * week */
...
typedef struct _netconf_match_t {
	...
	uint days[2]; /* Match by day of the week (local time) (0
				 * == Sunday) */
	uint secs[2]; /* Match by time of day (local time) (0 ==
				 * 12:00 AM) */
	struct _netconf_match_t *next, *prev;
} netconf_match_t;

#define netconf_valid_ipproto(ipproto) ((ipproto == 0) || (ipproto) == IPPROTO_TCP || (ipproto) == IPPROTO_UDP)
...
#define netconf_valid_dir(dir) ((dir) >= 0 && (dir) < NETCONF_DIR_MAX)
...
typedef struct _netconf_app_t {
	...
	uint16 dport[2]; /* Related destination port(s) (network
				 * order) */
	uint16 to[2]; /* Port(s) to map related destination port to 
				 * (network order) */
} netconf_app_t;
...
#define netconf_list_add(new, head)         \
	do {                                \
		(head)->next->prev = (new); \
		(new)->next = (head)->next; \
		(new)->prev = (head);       \
...
#define netconf_list_del(old)                    \
	do {                                     \
		(old)->next->prev = (old)->prev; \
		(old)->prev->next = (old)->next; \
	} while (0)

#define netconf_list_for_each(pos, head) for ((pos) = (head)->next; (pos) != (head); (pos) = (pos)->next)

#define netconf_list_free(head)                                             \
	do {                                                                \
