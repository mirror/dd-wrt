...
int parse_cfe_partitions( struct mtd_info *master, struct mtd_partition **pparts)
{
       ...
       curpart++;
       parts[curpart].name = "linux"; //root partition needed for web flashing
       parts[curpart].offset = master->erasesize;
       if (kerneladdr>rootfsaddr)
    	    parts[curpart].size = spareaddr+sparelen-master->erasesize;
       else
    	    parts[curpart].size = spareaddr+sparelen-master->erasesize;
       curpart++;
       if (kernellen > 0){
...
static int bcm963xx_detect_cfe(struct mtd_info *master)
{
       ...
       buf[8]=0; // BS: fix for missing zero termination. strcmp will fail otherwise in some compiling conditions
