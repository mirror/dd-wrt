...
static int dev_max_count = 1; /* only NPEB is used */
      ...
      || defined (CONFIG_ARCH_ADI_COYOTE) || defined (CONFIG_MACH_GTWX5715)
 ...
 * - Adjust ixpdev_max_backlog=n in the driver's command line
...
#elif defined(CONFIG_ARCH_ADI_COYOTE) || defined (CONFIG_MACH_GTWX5715)
...
{
    IX_ETH_NPE_B_IMAGE_ID,  /* Npe firmware for EthAcc Port 0  */
    IX_ETH_NPE_C_IMAGE_ID,  /* Npe firmware for EthAcc Port 1  */
#if defined (CONFIG_ARCH_IXDP465) || defined(CONFIG_MACH_IXDP465)
    IX_ETH_NPE_A_IMAGE_ID   /* Npe firmware for EthAcc Port 2  */
#endif
};
...
#if IS_KERNEL26
static struct workqueue_struct *ethdb_maintenance_workq = NULL;
#endif
...
static int dev_media_check_thread (void* arg)
{
    ...
    */
	...
	if (default_phy_cfg[priv->port_id].linkMonitor)
	{
		...
		{
       P_ERROR("%s PHY:%d link down->up\n",dev->name,phyNum); //phj		    /* inform the kernel of a change in link state */
	...
	else
	{
	    ...
	    if (!netif_carrier_ok(dev))
	    {
       P_ERROR("%s PHY:%d link not monitored,up\n",dev->name,phyNum); //phj
       		netif_carrier_on(dev);
	    }
	}
...
#if IS_KERNEL26
static irqreturn_t dev_pmu_timer_os_isr( int irg, void *dev_id, struct pt_regs
                                        *regs)
#else
static void dev_pmu_timer_os_isr(int irg, void *dev_id, struct pt_regs *regs)
#endif
{
...
{
#if IS_KERNEL26
    queue_delayed_work(ethdb_maintenance_workq, &ethdb_maintenance_work,
	DB_MAINTENANCE_TIME);
#else
...
static void maintenance_timer_clear(void)
{
    ...
    flush_workqueue(ethdb_maintenance_workq);
#else
...
int ixpdev_set_mac_address(struct net_device *dev, void *addr)
{
    int res;
    IxEthAccMacAddr npeMacAddr;
...
#endif
{
   ...
   /* initialise the MII register access mutex */
    ...
    ndev->set_mac_address = ixpdev_set_mac_address;
...
static int __init ixp400_eth_init(void)
{
    ...
    ethdb_maintenance_workq = create_singlethread_workqueue(MODULE_NAME);
    BUG_ON(!ethdb_maintenance_workq);
#endif
...
void __exit ixp400_eth_exit(void)
{
    ...
    if (ethdb_maintenance_workq)
    {
	destroy_workqueue(ethdb_maintenance_workq);
	ethdb_maintenance_workq = NULL;
    }
#endif
