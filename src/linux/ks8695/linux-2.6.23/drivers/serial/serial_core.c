...
{
	struct uart_port *uartport = (struct uart_port*)data;
	struct epld_struct epld;
	int i, j, ret = 0, port = uartport->line;

	//printk(KERN_INFO "proc_epld_read (/proc/%s%d) called\n", PROC_NAME_EPLD, port);
	if (!offset)
	{
		ret = sprintf(buffer, "inactive\n");
	   ...
	   unsigned long arg)
{
	...
	case TIOCGEPLD:
	case TIOCSEPLD:
		return ioctl_epld(state->port, (struct epld_struct*)arg, cmd);
		break;
#endif
	}
...
#if defined(CONFIG_ARCH_KS8695_VSOPENRISC)
	if(port->line != 0)
		clear_dev_open_status(port->line, DRV_IN_USE_CAN);
#endif
	mutex_lock(&state->mutex);
