		       ...
		       struct ktermios *old)
{
	...
	if (up->port.type == PORT_16C950)
	{
		...
		serial_outp(up, UART_LCR, 0xBF);
		temp_EFR = serial_inp(up, UART_EFR);
		temp_EFR |= UART_EFR_ECB;
		serial_outp(up, UART_EFR, temp_EFR);
		serial_outp(up, UART_LCR, 0);
		up->mcr |= 0x80;
...
static void __init serial8250_isa_init_ports(void)
{
	...
	for (i = 0, up = serial8250_ports;
		...
		up->port.mapbase  = (resource_size_t)old_serial_port[i].iomem_base;
		up->port.iotype   = old_serial_port[i].io_type;
		up->port.regshift = old_serial_port[i].iomem_reg_shift;
#if defined(CONFIG_SERIAL_NETCOM_EPLD)
		up->port.epld_capabilities = old_serial_port[i].epld_capabilities;
#endif
		if (share_irqs)
			up->port.flags |= UPF_SHARE_IRQ;

#if defined(CONFIG_SERIAL_NETCOM_EPLD)
		if(up->port.flags & ASYNC_VSOPENRISC_EPLD)
		{
			...
			proc_epld = create_proc_entry(proc_name_epld, 0644, NULL);
			if (proc_epld == NULL)
			{
			...
			{
				proc_epld->read_proc = proc_epld_read;
				proc_epld->write_proc = proc_epld_write;
				proc_epld->owner = THIS_MODULE;
				proc_epld->mode = S_IFREG | S_IRUGO;
				proc_epld->uid = 0;
				proc_epld->gid = 0;
				proc_epld->size = 0;
				proc_epld->data = (void*)&up->port;
