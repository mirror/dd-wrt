...
struct mtd_info *cfi_cmdset_0002(struct map_info *map, int primary)
{
	...
	struct mtd_info *mtd;
	int i;
	printk(KERN_INFO "MTD: cfi_cmdset_0002\n");
	...
	if (cfi->cfi_mode==CFI_MODE_CFI){
		...
		// FIXME: we do only extra testing, if we're not a SST one
		struct cfi_pri_amdstd myext;
		
		if (cfi->cfiq->P_ID == 0x0701)
		{
			memset(&myext, 0, sizeof(memset));
			myext.TopBottom = 255;
			extp = &myext;
		}
		else
...
//**************************
		...
		{
			printk(KERN_INFO "Modify the unlock address\n");
