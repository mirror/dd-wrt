 ...
 *   SST & Fujitsu Standard Vendor Command Set (ID 0x0701)
 *
...
struct mtd_info *cfi_cmdset_0701(struct map_info *map, int primary)
{
	...
	return cfi_sststd_setup(mtd);
}
EXPORT_SYMBOL_GPL(cfi_cmdset_0701);

static struct mtd_info *cfi_sststd_setup(struct mtd_info *mtd)
{
	struct map_info *map = mtd->priv;
	struct cfi_private *cfi = map->fldrv_priv;
...
int cfi_sststd_erase_varsize(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if (ret){
		printk("**cfi_sststd_erase_varsize fail \n");
		return ret;
	}
...
MODULE_DESCRIPTION("MTD chip driver for SST/Fujitsu flash chips");
