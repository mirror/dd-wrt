...
#define SYSTEM_DANUBE_CHIPID3	0x30129083 // i think its more usefull to check just the first 24 bits, for now we just hardcode this one
...
unsigned char ifxmips_mii_mac[6];
...
static struct ifxmips_board boards[] =
{
	...
	},
	{
		.type = EASY50712,
		.name = "EASY50712",
		.system_type = SYSTEM_DANUBE_CHIPID3,
		.devs = easy50712_devs,
		.reset_resource = {.name = "reset", .start = 1, .end = 15,},
		.gpiodev_resource =	{.name = "gpio", .start = (1 << 0) | (1 << 1),
			.end = (1 << 0) | (1 << 1)},
