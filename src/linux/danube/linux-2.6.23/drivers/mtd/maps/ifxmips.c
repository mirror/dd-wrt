 ...
 * Copyright (C) 2008 Sebastian Gottschall <brainslayer@dd-wrt.com>
 */
...
int
find_uImage_size(unsigned long *start_offset,unsigned long end_offset,unsigned long erasesize){
	unsigned long temp;
	while(*start_offset<=end_offset)
	    ...
	    if (temp==IH_MAGIC)
		{
		printk(KERN_EMERG "found uImage at %08lX\n",*start_offset);
		break;
		}
	    *start_offset+=erasesize;
	    }
	if (*start_offset==end_offset)
	    {
	    printk(KERN_EMERG "can't find uImage between %X and %X\n",*start_offset,end_offset);
	    // we should raise a oops here, but however it will crash in any way
	    }
...
ifxmips_mtd_probe(struct platform_device *dev)
{
	...
	uimage_size = find_uImage_size(&ifxmips_partitions[3].offset,ifxmips_partitions[3].offset+0x50000,ifxmips_mtd->erasesize);

	if(detect_squashfs_partition(ifxmips_partitions[3].offset + uimage_size))
	{
	...
	ifxmips_partitions[2].offset = ifxmips_partitions[1].offset+ifxmips_partitions[1].size;
	ifxmips_partitions[2].size = ifxmips_partitions[3].offset - ifxmips_partitions[2].offset; 
	ifxmips_partitions[3].size = uimage_size;
	ifxmips_partitions[4].offset = ifxmips_partitions[3].offset + ifxmips_partitions[3].size;
	ifxmips_partitions[4].size = ((ifxmips_mtd->size >> 20) * 1024 * 1024) - ifxmips_partitions[4].offset;
		ifxmips_partitions[4].size -= ifxmips_mtd->erasesize*2;
		ifxmips_partitions[5].offset = ifxmips_mtd->size - ifxmips_mtd->erasesize*2;
		ifxmips_partitions[5].size = ifxmips_mtd->erasesize;		
		ifxmips_partitions[6].offset = ifxmips_mtd->size - ifxmips_mtd->erasesize;
		ifxmips_partitions[6].size = ifxmips_mtd->erasesize;
		ifxmips_partitions[7].offset = 0;
		ifxmips_partitions[7].size = ifxmips_mtd->size;
		add_mtd_partitions(ifxmips_mtd, parts, 8);
