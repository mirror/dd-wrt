...
//	t = ipgre_tunnel_lookup(iph->daddr, iph->saddr, (flags&GRE_KEY) ? get_unaligned((u32*)p + (grehlen>>2) - 1) : 0);
...
static int ipgre_rcv(struct sk_buff *skb)
{
	...
	if (flags&(GRE_CSUM|GRE_KEY|GRE_ROUTING|GRE_SEQ|GRE_VERSION)) {
		...
		if (flags&GRE_KEY) {
//			key = get_unaligned((u32*)(h + offset));
			key = *(u32*)(h + offset);
			offset += 4;
		}
		if (flags&GRE_SEQ) {
//			seqno = get_unaligned((u32*)(h + offset));
