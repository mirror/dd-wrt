...
#define IRQ_CFRDY	(8 + 4 * 32 + CF_GPIO_NUM)	/* 149 */

#define GPIO_BADDR	(void*)0xb8050000
...
unsigned long cfdev_base = 0x18A10000;
// 
unsigned cf_head = 0;
unsigned cf_cyl = 0;
unsigned cf_spt = 0;
...
extern inline int get_gpio_bit(unsigned long offset)
{
	return (readl(GPIO_BADDR + offset) >> CF_GPIO_NUM) & 1;
}

extern inline void set_gpio_bit(int bit, unsigned long offset)
{
	unsigned long val = readl(GPIO_BADDR + offset);
	if (bit)
		val |= (1 << CF_GPIO_NUM);
	else
		val &= ~(1 << CF_GPIO_NUM);
	writel(val, GPIO_BADDR + offset);
}

extern inline int cfrdy(void)
{
	return get_gpio_bit(GPIO_DATA);
}

extern inline void prepare_cf_irq(void)
{
	set_gpio_bit(1, GPIO_ILEVEL);	/* interrupt on cf ready (not busy) */
	set_gpio_bit(0, GPIO_ISTAT);	/* clear interrupt status */
}

static int fix_dc_cfg(struct cf_mips_dev* dev)
{
	unsigned mask;
	unsigned devc;
	unsigned devtc;

	mask = readl(CFG_DC_DEV1 + CFG_DC_DEVMASK);
	if (!mask) {
		printk(KERN_ERR "DEV1 in Device Controller is not mapped anywhere!\n");
		return 0;
	}
	dev->base = (void*) readl(CFG_DC_DEV1 + CFG_DC_DEVBASE);
	devc = readl(CFG_DC_DEV1 + CFG_DC_DEVC);
	devtc = readl(CFG_DC_DEV1 + CFG_DC_DEVTC);
	printk(KERN_INFO "DC DEV1 base 0x%08lx ctrl 0x%08x tctrl 0x%08x cfrdy %d\n",
	       (unsigned long)dev->base, devc, devtc, cfrdy());

	return 1;
}

static int cf_present(struct cf_mips_dev* dev)
{
	...
	set_gpio_bit(0, GPIO_FUNC);
	set_gpio_bit(0, GPIO_CFG);
...
inline int is_busy(struct cf_mips_dev *dev)
{
	return !cfrdy();
}
...
static irqreturn_t cf_irq_handler(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	set_gpio_bit(0, GPIO_ILEVEL);
	set_gpio_bit(0, GPIO_ISTAT);
...
#if 0
	//not using APM
static int set_apm_level(unsigned level, struct cf_mips_dev* dev)
{
	if (level == ATA_APM_DISABLED) {
		wareg(ATA_FEATURE_DISABLE_APM, ATA_REG_FEAT,dev);
	} else {
		wareg(ATA_FEATURE_ENABLE_APM, ATA_REG_FEAT,dev);
		wareg(level, ATA_REG_SC,dev);
	}
	wareg(ATA_REG_DH_BASE | ATA_REG_DH_LBA, ATA_REG_DH,dev);
	wareg(ATA_CMD_SET_FEATURES, ATA_REG_CMD,dev);

	if (wait_not_busy(10 * SECS, 1,dev) != CF_TRANS_OK) {
		printk(KERN_ERR
	...
	if ((rareg(ATA_REG_ST,dev) & ATA_REG_ST_ERR) == 0) {
		...
		printk(KERN_WARNING "cf-mips: failed to change APM level to %u"
		       ...
		       level, (unsigned) rareg(ATA_REG_ST,dev),
		       (unsigned) rareg(ATA_REG_ERR,dev));
		return 1;
	}
}
#endif
...
//	dev->base= (void*) 0x18A10000;
	tasklet_init(&dev->tasklet,cf_do_tasklet,(unsigned long)dev);
#if REQUEST_MEM_REGION
	if (!request_mem_region((unsigned long)dev->base, CFDEV_BUF_SIZE, "cf-mips")) {
	     printk(KERN_WARNING "cf-mips: cf_init: cf mem region 0x%lx/0x%x"
...
/*	set_apm_level(ATA_APM_WITH_STANDBY); */
	...
	prepare_cf_irq();
	if (request_irq(IRQ_CFRDY, cf_irq_handler, 0, "CF Mips", dev)) {
