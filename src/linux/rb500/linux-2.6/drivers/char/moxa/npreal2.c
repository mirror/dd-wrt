...
static void npreal_flush_to_ldisc(void *private_)
{
	...
	if ( tty && (info->flags & ASYNC_INITIALIZED))  {
		...
		if (tty->buf.head.buf_num) {
			cp = tty->buf.head.char_buf + TTY_FLIPBUF_SIZE;
			fp = tty->buf.head.flag_buf + TTY_FLIPBUF_SIZE;
			tty->buf.head.buf_num = 0;
			tty->buf.head.char_buf_ptr = tty->buf.head.char_buf;
			tty->buf.head.flag_buf_ptr = tty->buf.head.flag_buf;
		} else {
			cp = tty->buf.head.char_buf;
			fp = tty->buf.head.flag_buf;
			tty->buf.head.buf_num = 1;

			tty->buf.head.char_buf_ptr = tty->buf.head.char_buf + TTY_FLIPBUF_SIZE;
			tty->buf.head.flag_buf_ptr = tty->buf.head.flag_buf + TTY_FLIPBUF_SIZE;
		}
