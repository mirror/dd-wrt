...
#if 0
	/* I don't know what to do here: calling breakpoint() makes the compiler
	 generate an inline instance, which in turns defines the 'breakinst' asm
	 symbol twice.. */
void kgdb_check(void)
{
	if (poll && isDebugReady()) {
		poll = 0;
		breakpoint();
	}
}
