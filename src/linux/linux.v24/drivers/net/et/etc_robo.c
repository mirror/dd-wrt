...
#define BCM5365_DEVICE_ID 0x5365
/*
...
#define PAGE_STATUS	0x01	/* Status page, added by Joey */
...
#define REG_STATUS_SUMMARY	0x00 /* Status page summary, added by Joey */
...
uint robo_get_ports(robo_info_t *robo);
#endif
...
#if defined(ETROBO) // Added by Joey
uint
robo_get_ports(robo_info_t *robo)
{
	uint portstatus=0;

	ET_TRACE(("ROBO PORTS: %x", robo));
	if(robo)
	{
		/* port status */
		robo->ops->read_reg(robo, PAGE_STATUS, REG_STATUS_SUMMARY, &portstatus, sizeof(portstatus));
	}
	ET_TRACE(("ROBO PORTS RET: %x", portstatus));
	return portstatus;
}
#endif
