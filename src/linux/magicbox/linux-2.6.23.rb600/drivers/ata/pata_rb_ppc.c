...
static struct ata_port_operations rbppc_port_ops = {
	...
	.tf_load		= ata_tf_load,
	.tf_read		= ata_tf_read,
	.check_status		= rbppc_check_status,
	.check_altstatus	= rbppc_check_altstatus,
...
static int init_rbppc_info(struct of_device *pdev, struct rbppc_info *info) {
	...
	u32ptr = get_property(pdev->node, "lbc_extra_divider", NULL);
	if (u32ptr && *u32ptr) {
	...
	u32ptr = get_property(np, "clock-frequency", NULL);
	...
	u32ptr = get_property(pdev->node, "lb-timings", NULL);
	if (u32ptr) {
			  ...
			  const struct of_device_id *match)
{
	...
	u32ptr = get_property(pdev->node, "interrupt-at-level", NULL);
	if (u32ptr) {
		irq_level = *u32ptr;
	...
	err = ata_host_activate(
		host,
		irq_of_parse_and_map(pdev->node, 0), ata_interrupt,
