...
/*
 * Set ASPM state.
 * Enable/disable L0s/L1 depend on link state and aspm_flags.
 */
static void atl1c_set_aspm(struct atl1c_hw *hw, u16 aspm_flags)
{
	u32 pm_ctrl_data;

	pm_ctrl_data = AT_READ_REG(hw, REG_PM_CTRL);
	/* clear L1 entry timer */
	pm_ctrl_data &= ~(PM_CTRL_L1_ENTRY_TIMER_MASK <<
			PM_CTRL_L1_ENTRY_TIMER_SHIFT);
	pm_ctrl_data &= ~PM_CTRL_CLK_SWH_L1;
	pm_ctrl_data &= ~PM_CTRL_SERDES_PD_EX_L1;

	if (hw->adapter->link_speed == SPEED_0) {
		if (aspm_flags & ATL1C_ASPM_L1_ENABLE)
			pm_ctrl_data |= PM_CTRL_CLK_SWH_L1;
		pm_ctrl_data &= ~PM_CTRL_SERDES_PLL_L1_EN;
	} else
		pm_ctrl_data |= PM_CTRL_SERDES_PLL_L1_EN;
	...
	if (aspm_flags != 0) {
		...
		if (aspm_flags & ATL1C_ASPM_L0s_ENABLE)
			pm_ctrl_data |= PM_CTRL_ASPM_L0S_EN;
		else
			pm_ctrl_data &= ~PM_CTRL_ASPM_L0S_EN;
