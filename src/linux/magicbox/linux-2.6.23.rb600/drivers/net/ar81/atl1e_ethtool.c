...
static int
atl1e_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
    struct atl1e_adapter *adapter = netdev_priv(netdev);
    struct atl1e_hw *hw = &adapter->hw;
    ...
    if (atl1e_phy_ctrl_link_chg_pre(hw) != 0)
        printk(KERN_WARNING"phy ctrl link change prefunction failed\n");

    hw->mii_autoneg_adv_reg = 0;
    if (atl1e_phy_setup_autoneg_adv(hw) != 0) {
        printk(KERN_WARNING"invalid ethtool speed/duplex setting\n");
        clear_bit(__AT_RESETTING, &adapter->flags);
        return -EINVAL;
    }
    
    if (atl1e_restart_autoneg(hw) != 0) {
...
static u32
atl1e_get_tx_csum(struct net_device *netdev)
{
    return (netdev->features & NETIF_F_HW_CSUM) != 0;
}

static u32
atl1e_get_msglevel(struct net_device *netdev)
{
#if DBG
    return 1;
#else
...
static void
atl1e_set_msglevel(struct net_device *netdev, u32 data)
{
}

static int
atl1e_get_regs_len(struct net_device *netdev)
{
...
static int
atl1e_get_eeprom_len(struct net_device *netdev)
{
    struct atl1e_adapter *adapter = netdev_priv(netdev);
...
static int
atl1e_get_eeprom(struct net_device *netdev,
...
static void
atl1e_get_drvinfo(struct net_device *netdev,
...
static void
atl1e_get_wol(struct net_device *netdev, struct ethtool_wolinfo *wol)
{
    struct atl1e_adapter *adapter = netdev_priv(netdev);
...
static int
atl1e_nway_reset(struct net_device *netdev)
{
    struct atl1e_adapter *adapter = netdev_priv(netdev);
