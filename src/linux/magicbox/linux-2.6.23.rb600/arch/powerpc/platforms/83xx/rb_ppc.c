...
static void __init rbppc_setup_arch(void)
{
	...
	if (np) {
		timer_freq =
		    *(unsigned *) get_property(np, "clock-frequency", NULL);
		of_node_put(np);
	}
...
static void __init rbppc_beeper_init(struct device_node *beeper)
{
	...
	pin = get_property(beeper, "gpio", NULL);
	if (pin) {
		...
		gpio_id = get_property(gpio, "device-id", NULL);
		if (!gpio_id) {
...
static void rb_restart(char *cmd)
{
    ...
    if (root) {
	const char *prop = (char *) get_property(root, "model", &size);
