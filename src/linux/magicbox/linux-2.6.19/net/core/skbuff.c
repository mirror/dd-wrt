...
static void skb_release_data(struct sk_buff *skb)
{
	if (!skb->cloned ||
		 ...
		 * If skb->retfreeq_data_prealloc is 1, that means the data buffer was pre-allocated
	 	 ...
	 	 */
		if (skb->retfreeq_cb && skb->retfreeq_data_prealloc == 1) {
			(*skb->retfreeq_cb)(skb->retfreeq_context, skb->head, FREE_DATA);
...
void kfree_skbmem(struct sk_buff *skb)
{
	 ...
	 * If skb->retfreeq_skb_prealloc is 1, that means the skb was pre-allocated
 	 * by our network driver. (songw)
 	 */
	if (skb->retfreeq_cb && skb->retfreeq_skb_prealloc == 1)
...
#endif
#if defined(CONFIG_MIPS_BRCM)
	n->retfreeq_skb_prealloc = 0;
	C(retfreeq_data_prealloc);
	C(retfreeq_cb);
	C(retfreeq_context);
	C(rcvfrom);
...
static void copy_skb_header(struct sk_buff *new, const struct sk_buff *old)
{
	...
	new->tc_index	= old->tc_index;
 	...
 	new->retfreeq_skb_prealloc = 0;
 	new->retfreeq_data_prealloc = 0;
#endif
