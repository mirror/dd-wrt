...
#define DRIVER_VERSION			"1.9-Option"
...
#define HSO_SERIAL_TTY_MAJOR		190
...
/* module parameters */
static int debug;
static int tty_major = HSO_SERIAL_TTY_MAJOR;
...
static void hso_log_port(char *portinfo, struct hso_device *hso_dev)
{
	char *port_type;
	char port_dev[20];
	...
	if (portinfo)
		sprintf(portinfo, "%s:%s", port_dev, port_type);
...
static int hso_read_proc(char *page, char **start, off_t off, int count,
			 int *eof, void *data)
{
	int i, len = 0;
	char portinfo[40];
	...
	for (i = 0; i < HSO_SERIAL_TTY_MINORS; i++) {
		if (serial_table[i]) {
			...
			hso_log_port(portinfo, serial->parent);
...
static int hso_write_proc(struct file *file, const char *buffer,
...
static void hso_init_proc(void)
{
	struct proc_dir_entry* entry;
	entry = create_proc_entry("hso_port", 0644, proc_root_driver);
	entry->read_proc = hso_read_proc;
	entry->write_proc = hso_write_proc;
}

static void hso_cleanup_proc(void)
{
	remove_proc_entry("hso_port", proc_root_driver);
}
...
static int __init hso_init(void)
{
	...
	tty_drv->devfs_name=tty_filename;
