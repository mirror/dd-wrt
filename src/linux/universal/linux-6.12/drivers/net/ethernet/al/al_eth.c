...
static const struct pci_device_id al_eth_pci_tbl[] = {
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS,
	  ...
	  ALPINE_INTEGRATED },
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS,
	  PCI_DEVICE_ID_AMAZON_ANNAPURNA_LABS_ETH_ADVANCED, PCI_ANY_ID,
	  PCI_ANY_ID, 0, 0, ALPINE_INTEGRATED },
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS,
	  PCI_DEVICE_ID_AMAZON_ANNAPURNA_LABS_ETH_NIC, PCI_ANY_ID, PCI_ANY_ID,
	  0, 0, ALPINE_NIC },
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS,
	  PCI_DEVICE_ID_AMAZON_ANNAPURNA_LABS_ETH_FPGA_NIC, PCI_ANY_ID,
	  PCI_ANY_ID, 0, 0, ALPINE_FPGA_NIC },
	{
		0,
	}
};
...
static int al_mdio_read(struct mii_bus *bp, int mii_id, int reg)
{
	...
	while (timeout > 0) {
		if (reg & MII_ADDR_C45) {
			...
			rc = al_eth_mdio_read(&adapter->hal_adapter,
					      adapter->phy_addr,
					      (mii_id & 0x1f), reg, &value);
		} else {
			rc = al_eth_mdio_read(&adapter->hal_adapter,
...
static int al_mdio_write(struct mii_bus *bp, int mii_id, int reg, u16 val)
{
	...
	while (timeout > 0) {
		if (reg & MII_ADDR_C45) {
			...
			al_dbg("%s [c45]: dev %x reg %x val %x\n", __func__,
			       (mii_id & 0x1f), reg, val);
			rc = al_eth_mdio_write(&adapter->hal_adapter,
					       adapter->phy_addr,
					       (mii_id & 0x1f), reg, val);
		} else {
			rc = al_eth_mdio_write(&adapter->hal_adapter,
...
static void al_eth_adjust_link(struct net_device *dev)
{
	...
	if (phydev->link) {
		...
		if (new_state) {
			...
			if (adapter->mac_mode != AL_ETH_MAC_MODE_10GbE_Serial) {
				...
				rc = al_eth_mac_link_config(
					&adapter->hal_adapter,
					force_1000_base_x, link_config->autoneg,
					link_config->active_speed,
					link_config->active_duplex ? AL_TRUE :
								     AL_FALSE);
				if (rc) {
					netdev_warn(
						adapter->netdev,
		...
		if (link_config->flow_ctrl_supported &
...
static int al_eth_phy_init(struct al_eth_adapter *adapter)
{
	...
	phydev = phy_connect(adapter->netdev, phydev_name(phydev),
			     al_eth_adjust_link, PHY_INTERFACE_MODE_RGMII);
#else
	...
	ethtool_convert_legacy_u32_to_link_mode(phydev->advertising,
						advertising);
...
static int al_eth_function_reset(struct al_eth_adapter *adapter)
{
	...
	if (adapter->board_type == ALPINE_INTEGRATED)
		rc = al_eth_flr_rmn(&al_eth_read_pci_config,
				    &al_eth_write_pci_config, adapter->pdev,
				    adapter->mac_base);
	else
...
static int al_eth_board_params_init(struct al_eth_adapter *adapter)
{
	if (adapter->board_type == ALPINE_NIC) {
		...
		adapter->link_config.autoneg =
			(adapter->phy_exist) ? (params.an_mode ==
						AL_ETH_BOARD_AUTONEG_IN_BAND) :
					       (!params.an_disable);
		...
		dev_info(
			&adapter->pdev->dev,
			"Board info: board type %d ext phy exists %s, ext phy addr %x, mdio freq %u Khz, SFP connected %s, media %d\n",
			adapter->board_type,
...
static uint8_t al_eth_flow_ctrl_mutual_cap_get(struct al_eth_adapter *adapter)
{
	...
	if (phydev->pause)
		peer_flow_ctrl |=
			(AL_ETH_FLOW_CTRL_TX_PAUSE | AL_ETH_FLOW_CTRL_RX_PAUSE);
...
static void al_eth_init_rings(struct al_eth_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		...
		ring->netdev = adapter->netdev;
		ring->napi =
			&adapter->al_napi[AL_ETH_RXQ_NAPI_IDX(adapter, i)].napi;
...
static void al_eth_free_tx_resources(struct al_eth_adapter *adapter, int qid)
{
	...
	dma_free_coherent(tx_ring->dev, tx_ring->descs_size,
			  q_params->desc_base, q_params->desc_phy_base);
				     ...
				     unsigned int qid)
{
	...
	dma_free_coherent(rx_ring->dev, rx_ring->cdescs_size,
			  q_params->cdesc_base, q_params->cdesc_phy_base);
				 ...
				 unsigned int qid, unsigned int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (unlikely(al_eth_alloc_rx_page(adapter, rx_info,
						  __GFP_COLD | GFP_ATOMIC |
							  __GFP_COMP) < 0)) {
#elif defined(CONFIG_AL_ETH_ALLOC_FRAG)
		if (unlikely(al_eth_alloc_rx_frag(adapter, rx_ring, rx_info) <
			     0)) {
#elif defined(CONFIG_AL_ETH_ALLOC_SKB)
		if (unlikely(al_eth_alloc_rx_skb(adapter, rx_ring, rx_info) <
			     0)) {
#endif
			netdev_warn(adapter->netdev,
				...
				unsigned int qid)
{
	...
	for (i = 0; i < AL_ETH_DEFAULT_TX_SW_DESCS; i++) {
		...
		if (!udma_debug_printed) {
			al_udma_regs_print(tx_ring->dma_q->udma,
					   AL_UDMA_DEBUG_QUEUE(qid));
...
{
	struct al_eth_napi *al_napi =
		container_of(napi, struct al_eth_napi, napi);
	...
	while (total_done) {
		...
		if ((skb_queue_len(rx_rc) < AL_ETH_DEFAULT_RX_DESCS) &&
		    skb_recycle_check(skb, tx_ring->netdev->mtu + ETH_HLEN +
	...
	if (unlikely(netif_tx_queue_stopped(txq) &&
		     (al_udma_available_get(tx_ring->dma_q) >
				     ...
				     unsigned int descs, u16 *next_to_clean)
{
	...
	if (len <= adapter->small_copy_len) {
		...
		*next_to_clean =
			AL_ETH_RX_RING_IDX_ADD(rx_ring, *next_to_clean, descs);
		return skb;
	}
	...
	do {
		...
		rx_info->page = NULL;
		*next_to_clean =
			AL_ETH_RX_RING_IDX_NEXT(rx_ring, *next_to_clean);
		if (likely(--descs == 0))
			break;
				      ...
				      struct sk_buff *skb)
{
	...
	if (unlikely(!(adapter->netdev->features & NETIF_F_RXCSUM))) {
		netdev_dbg(adapter->netdev,
			   "hw checksum offloading disabled\n");
		return;
	}
...
static int al_eth_rx_poll(struct napi_struct *napi, int budget)
{
	...
	do {
		...
		netdev_dbg(adapter->netdev,
			   "rx_poll: q %d got packet from hal. descs %d\n", qid,
			   descs);
		netdev_dbg(adapter->netdev,
		...
		if (hal_pkt->flags & (AL_ETH_RX_ERROR | AL_UDMA_CDESC_ERROR)) {
			netdev_dbg(adapter->netdev,
				   "receive packet with error. flags = 0x%x\n",
				   hal_pkt->flags);
			next_to_clean = AL_ETH_RX_RING_IDX_ADD(
				rx_ring, next_to_clean, descs);
			goto next;
		}
		...
		if (likely(adapter->netdev->features & NETIF_F_RXHASH)) {
			...
			if (likely((hal_pkt->l4_proto_idx ==
				    AL_ETH_PROTO_ID_TCP) ||
				   (hal_pkt->l4_proto_idx ==
				    AL_ETH_PROTO_ID_UDP)))
				skb->l4_hash = 1;
...
next:
	...
	if (unlikely(refill_actual < refill_required)) {
		...
		dev_dbg(&adapter->pdev->dev,
			...
			next_to_clean);
		napi_complete(napi);
		al_reg_write32_relaxed(rx_ring->unmask_reg_offset,
				       rx_ring->unmask_val);
	}
	...
	if (reg & AL_INT_GROUP_A_GROUP_B_SUM) {
		...
		for (qid = 0; qid < adapter->num_rx_queues; qid++) {
			if (cause_b & (1 << qid)) {
				...
				napi_schedule(
					&adapter->al_napi[AL_ETH_RXQ_NAPI_IDX(
								  adapter, qid)]
						 .napi);
			}
		}
	...
	if (reg & AL_INT_GROUP_A_GROUP_C_SUM) {
		...
		for (qid = 0; qid < adapter->num_tx_queues; qid++) {
			if (cause_c & (1 << qid)) {
				...
				napi_schedule(
					&adapter->al_napi[AL_ETH_TXQ_NAPI_IDX(
								  adapter, qid)]
						 .napi);
			}
		}
...
static void al_eth_enable_msix(struct al_eth_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_tx_queues; i++) {
		...
		adapter->msix_entries[irq_idx].entry =
			3 + AL_ETH_MAX_HW_QUEUES + i;
		adapter->msix_entries[irq_idx].vector = 0;
	}

	rc = -ENOSPC;
	while (msix_vecs >= 1) {
		rc = pci_enable_msix_exact(adapter->pdev, adapter->msix_entries,
					   msix_vecs);
		if (rc <= 0)
			break;
		if (rc > 0)
			msix_vecs =
				1; /* if we can't allocate all, then try only 1; */
	}
	...
	if ((adapter->board_type == ALPINE_NIC) ||
		...
		writew(PCI_MSIX_FLAGS_ENABLE,
		       adapter->internal_pcie_base + 0x92);
...
static int al_eth_setup_int_mode(struct al_eth_adapter *adapter, int dis_msi)
{
	...
	if (adapter->msix_vecs == 0) {
		...
		cpumask_set_cpu(cpu, &adapter->irq_tbl[AL_ETH_MGMT_IRQ_IDX]
					      .affinity_hint_mask);
	...
	cpu = cpumask_first(cpu_online_mask);
	cpumask_set_cpu(
		cpu, &adapter->irq_tbl[AL_ETH_MGMT_IRQ_IDX].affinity_hint_mask);
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		...
		cpu = cpumask_next((i % num_online_cpus() - 1),
				   cpu_online_mask);
		cpumask_set_cpu(cpu,
...
static int al_eth_request_irq(struct al_eth_adapter *adapter)
{
	...
	for (i = 0; i < adapter->irq_vecs; i++) {
		...
		if (rc) {
			netdev_err(adapter->netdev,
				   "failed to request irq %d. index %d rc %d\n",
				   irq->vector, i, rc);
			break;
		}
		irq->requested = 1;
...
static int al_init_rx_cpu_rmap(struct al_eth_adapter *adapter)
{
	...
	adapter->netdev->rx_cpu_rmap =
		alloc_irq_cpu_rmap(adapter->num_rx_queues);
	...
	for (i = 0; i < AL_ETH_RX_FSM_TABLE_SIZE; i++) {
		...
		case AL_ETH_FSM_ENTRY_NOT_IP:
			if (AL_ETH_FSM_ENTRY_TUNNELED(i) &&
			    !AL_ETH_FSM_ENTRY_INNER(i)) /*PPPoE*/
				val = AL_ETH_FSM_DATA_INNER_4_TUPLE |
				      AL_ETH_FSM_DATA_HASH_SEL;
			else
				val = (0 << AL_ETH_FSM_DATA_DEFAULT_Q_SHIFT |
				       ((1 << 0)
					<< AL_ETH_FSM_DATA_DEFAULT_UDMA_SHIFT));
			break;
		default:
			val = (0 << AL_ETH_FSM_DATA_DEFAULT_Q_SHIFT |
			       ((1 << 0)
...
static void al_eth_up_complete(struct al_eth_adapter *adapter)
{
	...
	if ((adapter->board_type == ALPINE_FPGA_NIC) ||
	    (adapter->board_type == ALPINE_NIC))
		writel(0x1FFFF,
		       adapter->internal_pcie_base + 0x1800000 + 0x1210);
			     ...
			     u32 flow_id)
{
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		...
		netdev_info(
			...
			&ip->saddr, ntohs(ports[0]), &ip->daddr,
		...
		netdev_info(
			...
			&ip6h->saddr, ntohs(ports[0]), &ip6h->daddr,
			ntohs(ports[1]), rxq_index, flow_id, rc);
	}
...
static void al_eth_serdes_mode_set(struct al_eth_adapter *adapter)
{
	...
	if (alpine_serdes_eth_mode_set(adapter->serdes_grp, mode))
		netdev_err(adapter->netdev,
			   ...
			   __func__, adapter->serdes_grp, mode);
...
static int al_eth_open(struct net_device *netdev)
{
	...
	if (adapter->phy_exist == false) {
		...
		if (adapter->phy_fixup_needed) {
			rc = phy_register_fixup_for_uid(AQUANTIA_AQR105_ID,
							0xffffffff,
							al_eth_aq_phy_fixup);
			if (rc)
				netdev_warn(adapter->netdev,
					    "failed to register PHY fixup\n");
		}
	...
	if (adapter->mdio_bus) {
		...
		if ((adapter->board_type == ALPINE_INTEGRATED) &&
			...
			delay = msecs_to_jiffies(
				AL_ETH_FIRST_LINK_POLL_INTERVAL);

			INIT_DELAYED_WORK(&adapter->link_status_task,
					  al_eth_link_status_task);
			schedule_delayed_work(&adapter->link_status_task,
					      delay);
		} else {
			netif_carrier_on(adapter->netdev);
		}
...
static int al_eth_set_link_ksettings(struct net_device *netdev,
				 ...
				 struct ethtool_pauseparam *pause)
{
	...
	if (pause->rx_pause) {
		...
		if (pause->tx_pause) {
			link_config->flow_ctrl_supported |=
				AL_ETH_FLOW_CTRL_TX_PAUSE;
			   ...
			   struct al_eth_pkt *hal_pkt, struct sk_buff *skb)
{
	...
	if ((skb->ip_summed == CHECKSUM_PARTIAL) || mss) {
		...
		meta->l4_header_len =
			tcp_hdr(skb)->doff; /* this param needed only
		for TSO */
...
static int al_eth_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (ent->driver_data == ALPINE_INTEGRATED)
		...
		rc = pcim_iomap_regions(pdev,
					(1 << board_info[ent->driver_data].bar),
					DRV_MODULE_NAME);
	...
	if (adapter->board_type == ALPINE_INTEGRATED) {
		 ...
		 */
		adapter->internal_pcie_base =
			iomap[board_info[ent->driver_data].bar];
		adapter->udma_base =
			iomap[board_info[ent->driver_data].bar] + 128 * 0x400;
		adapter->ec_base =
			iomap[board_info[ent->driver_data].bar] + 20 * 0x400;
		adapter->mac_base =
			iomap[board_info[ent->driver_data].bar] + 36 * 0x400;
		/* enable master/slave in the adapter conf */
		adapter_pci_cmd =
			readw(adapter->internal_pcie_base + PCI_COMMAND);
		adapter_pci_cmd |= PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER;
		adapter_pci_cmd &= ~PCI_COMMAND_INTX_DISABLE;

		writew(adapter_pci_cmd,
		       adapter->internal_pcie_base + PCI_COMMAND);
	}
...
static int al_eth_resume(struct pci_dev *pdev)
{
	...
	if (adapter->rev_id > AL_ETH_REV_ID_0) {
		...
		al_eth_mac_table_broadcast_add(
			adapter, AL_ETH_MAC_TABLE_BROADCAST_IDX, 1);
		al_eth_mac_table_promiscuous_set(adapter, false);
