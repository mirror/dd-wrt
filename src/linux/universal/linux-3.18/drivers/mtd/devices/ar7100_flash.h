...
#define ar7100_spi_bit_banger(_byte)                                                                       \
	do {                                                                                               \
		int _i;                                                                                    \
		for (_i = 0; _i < 8; _i++) {                                                               \
			ar7100_reg_wr_nf(AR7100_SPI_WRITE, AR7100_SPI_CE_LOW | ar7100_be_msb(_byte, _i));  \
			ar7100_reg_wr_nf(AR7100_SPI_WRITE, AR7100_SPI_CE_HIGH | ar7100_be_msb(_byte, _i)); \
		}                                                                                          \
	} while (0);

#define ar7100_spi_go()                                                \
	do {                                                           \
		ar7100_reg_wr_nf(AR7100_SPI_WRITE, AR7100_SPI_CE_LOW); \
		ar7100_reg_wr_nf(AR7100_SPI_WRITE, AR7100_SPI_CS_DIS); \
	} while (0);

#define ar7100_spi_send_addr(_addr)                               \
	do {                                                      \
