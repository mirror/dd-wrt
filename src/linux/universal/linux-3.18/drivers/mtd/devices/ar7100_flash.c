...
static int ar7100_flash_read(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
...
static int ar7100_flash_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
		...
		while ((offset + mtd->erasesize) < mtd->size) {
			...
			if (*((__u32 *)buf) == SQUASHFS_MAGIC_SWAP) {
				...
				if (compex) {
					...
					p = (unsigned char *)(0xbf000000 + dir_parts[5].offset);
					...
					while (1) {
						...
						if (!strncmp(fis->name, "linux", 5) || !strncmp(fis->name, "vmlinux", 7) ||
						    !strncmp(fis->name, "kernel", 6)) {
