...
#include <mach/hardware.h>	/* Pick up IXP4xx-specific bits */
...
#define I2C_HW_B_IXP4XX		0x010017 /* GPIO on IXP4XX systems */
	...
	drv_data->adapter.nr = I2C_HW_B_IXP4XX;
	...
	printk(KERN_ERR "install %s\n", dev_name(&plat_dev->dev));

	drv_data->adapter.nr = 0;
	err = i2c_bit_add_numbered_bus(&drv_data->adapter);
