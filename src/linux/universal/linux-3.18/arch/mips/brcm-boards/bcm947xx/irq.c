...
static INLINE void
enable_brcm_irq(struct irq_data *d)
{
	set_c0_status(SR_IM(d->irq));
	irq_enable_hazard();
}

static INLINE void
disable_brcm_irq(struct irq_data *d)
{
	clear_c0_status(SR_IM(d->irq));
	irq_disable_hazard();
}

static void
ack_brcm_irq(struct irq_data *d)
{
	/* Done in plat_irq_dispatch()! */
}

static void
end_brcm_irq(struct irq_data *d)
{
	/* Done in plat_irq_dispatch()! */
}
...
static INLINE void
enable_brcm_irq2(struct irq_data *d)
{
	ASSERT(irq2en >= 0);
	if (irq2en++)
		return;
...
static INLINE void
disable_brcm_irq2(struct irq_data *d)
{
	ASSERT(irq2en > 0);
	if (--irq2en)
		return;
...
static void
ack_brcm_irq2(struct irq_data *d)
{
	/* Already done in plat_irq_dispatch()! */
}

static void
end_brcm_irq2(struct irq_data *d)
{
	/* Already done in plat_irq_dispatch()! */
}
