...
static int squashfs_readdir(struct file *, struct dir_context *ctx);
...
static struct dentry *squashfs_get_sb(struct file_system_type *, int,
			const char *, void *);
...
allocate_root:
	...
	if ((s->s_root = d_make_root(root)) == NULL) {
...
static struct dentry *squashfs_get_sb(struct file_system_type *fs_type,
