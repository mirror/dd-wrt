...
BCMINITFN(si_cpu_clock)(si_t *sih)
{
	...
	*/
	...
	if (BCM53573_CHIP(CHIPID(sih->chip))) {
		...
		W_REG(osh, PMUREG(sih, pllcontrol_addr), 13);
		mdiv = R_REG(osh, PMUREG(sih, pllcontrol_data)) & 0xff;
		ASSERT(mdiv);
		pdiv = (R_REG(osh, PMUREG(sih, pllcontrol_data)) >> 24) & 0x7;
		ASSERT(pdiv);

		/* Read ndiv[29:20], ndiv_frac[19:0] from pllcontrol[14] */
		W_REG(osh, PMUREG(sih, pllcontrol_addr), 14);
		ndiv_nfrac = R_REG(osh, PMUREG(sih, pllcontrol_data)) & 0x3fffffff;

		/* Read pll_ctrl_20 from pllcontrol[15] */
		W_REG(osh, PMUREG(sih, pllcontrol_addr), 15);
		pll_ctrl_20 = 1 << ((R_REG(osh, PMUREG(sih, pllcontrol_data)) >> 20) & 0x1);
...
BCMINITFN(si_mem_clock)(si_t *sih)
{
	...
	*/
	...
	if (BCM53573_CHIP(CHIPID(sih->chip))) {
		...
		W_REG(osh, PMUREG(sih, pllcontrol_addr), 25);
		mdiv = (R_REG(osh, PMUREG(sih, pllcontrol_data)) >> 16) & 0xff;
		ASSERT(mdiv);

		/* Read pdiv[18:16], ndiv[29:20], ndiv_frac[31] from pllcontrol[26] */
		W_REG(osh, PMUREG(sih, pllcontrol_addr), 26);
		pdiv = (R_REG(osh, PMUREG(sih, pllcontrol_data)) >> 16) & 0x7;
		ndiv_nfrac = R_REG(osh, PMUREG(sih, pllcontrol_data)) & 0x3ff00000;
		ndiv_nfrac |= (R_REG(osh, PMUREG(sih, pllcontrol_data)) & 0x80000000)? 1: 0;
		ASSERT(pdiv);

		/* Read ndiv_frac[19:1] from pllcontrol[27] */
		W_REG(osh, PMUREG(sih, pllcontrol_addr), 27);
		ndiv_nfrac |= ((R_REG(osh, PMUREG(sih, pllcontrol_data)) << 1) & 0xffffe);

		/* Read pll_ctrl_20 from pllcontrol[28] */
		W_REG(osh, PMUREG(sih, pllcontrol_addr), 28);
		pll_ctrl_20 = 1 << ((R_REG(osh, PMUREG(sih, pllcontrol_data)) >> 20) & 0x1);
