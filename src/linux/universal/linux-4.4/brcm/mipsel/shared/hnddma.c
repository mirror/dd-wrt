...
next_frame:
	...
	DMA_TRACE(("%s: dma_rx skb %p data %p len %d\n", di->name, head, data, len));
	...
	if (resid <= 0) {
		...
		while ((resid > 0) && (p = _dma_getnextrxp(di, FALSE))) {
			DMA_TRACE(("%s: dma_rx: multi-buffer packet resid %d skb %p\n", di->name, resid, p));
...
_dma_rxfill(dma_info_t *di)
{
	...
	for (i = 0; i < n; i++) {
		...
		*(uint16 *)(PKTDATA(di->osh, p)) = 0;
		...
		DMA_TRACE(("%s: dma_rxfill: Index %d skb %p data %p\n", di->name, rxout, p, PKTDATA(di->osh, p) ));
...
dma64_txfast(dma_info_t *di, void *p0, bool commit)
{
	...
	for (p = p0; p; p = next) {
		...
		DMA_TRACE(("%s: dma_txfast Index %d skb %p data %p len %d\n", di->name, txout, p, data, len));
...
dma64_getnexttxp(dma_info_t *di, txd_range_t range)
{
	...
	defined(__NetBSD__)) || defined(BCM_SECURE_DMA)
		...
		for (j = nsegs; j > 0; j--) {
			...
			DMA_TRACE(("%s: dma_getnexttxp Index %d skb %p\n", di->name, i, txp ));
...
nextframe:
	...
	// Need to always UNMAP since we just RX'd from the DMA.
	{
		dmaaddr_t pa;
		PHYSADDRLOSET(pa, (BUS_SWAP32(R_SM(&di->rxd64[i].addrlow)) - di->dataoffsetlow));
		PHYSADDRHISET(pa, (BUS_SWAP32(R_SM(&di->rxd64[i].addrhigh)) - di->dataoffsethigh));
