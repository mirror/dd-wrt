...
NDIS_STATUS MlmeInit(RTMP_ADAPTER *pAd)
{
	...
	do 
	{
		...
		RTMPInitTimer(pAd, &pAd->Mlme.RxAntEvalTimer, GET_TIMER_FUNCTION(AsicRxAntEvalTimeout), pAd, FALSE);
			...
			P2PDiscoverySMInit(pAd, &pAd->P2pCfg.P2PDiscMachine, pAd->P2pCfg.P2PDiscFunc);
	...
	IN MAC_TABLE_ENTRY *pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		if (pEntry && IS_P2P_GO_ENTRY(pEntry))
			result = pAd->ApCfg.MBSSID[pEntry->apidx].wdev.bAutoTxRateSwitch;
		else if (pEntry && IS_ENTRY_APCLI(pEntry))
			result = pAd->ApCfg.ApCliTab[pEntry->wdev_idx].wdev.bAutoTxRateSwitch;
		else
#endif /* P2P_SUPPORT */
