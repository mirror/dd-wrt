...
#ifdef APCLI_WPA_SUPPLICANT_SUPPORT
		apcli_entry->AssocInfo.Length = sizeof(NDIS_802_11_ASSOCIATION_INFORMATION); 
		apcli_entry->AssocInfo.AvailableRequestFixedIEs =
			NDIS_802_11_AI_REQFI_CAPABILITIES | NDIS_802_11_AI_REQFI_LISTENINTERVAL;
		apcli_entry->AssocInfo.RequestFixedIEs.Capabilities = CapabilityInfo;
		apcli_entry->AssocInfo.RequestFixedIEs.ListenInterval = ListenIntv;		
		apcli_entry->AssocInfo.OffsetRequestIEs = sizeof(NDIS_802_11_ASSOCIATION_INFORMATION);
		
        	NdisZeroMemory(apcli_entry->ReqVarIEs, MAX_VIE_LEN);
		/*First add SSID*/
		VarIesOffset = 0;
		NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, &SsidIe, 1);
		VarIesOffset += 1;
		NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, &pAd->MlmeAux.SsidLen, 1);
		VarIesOffset += 1;
		NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen);
		VarIesOffset += pAd->MlmeAux.SsidLen;

		/*Second add Supported rates*/
		NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, &SupRateIe, 1);
		VarIesOffset += 1;
		NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, &pAd->MlmeAux.SupRateLen, 1);
		VarIesOffset += 1;
		NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, pAd->MlmeAux.SupRate, pAd->MlmeAux.SupRateLen);
		...
		*/
...
#ifdef SIOCSIWGENIE
			if ((apcli_entry->WpaSupplicantUP & WPA_SUPPLICANT_ENABLE) &&
				(apcli_entry->bRSN_IE_FromWpaSupplicant == TRUE))			
			{
				;
			}
			else
...
#ifdef APCLI_WPA_SUPPLICANT_SUPPORT
#ifdef SIOCSIWGENIE
			if (((apcli_entry->WpaSupplicantUP & 0x7F) != WPA_SUPPLICANT_ENABLE) ||
				(apcli_entry->bRSN_IE_FromWpaSupplicant == FALSE))
#endif
			{
			            // Append Variable IE
			            NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, &RSNIe, 1);
			            VarIesOffset += 1;
			            NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, &apcli_entry->RSNIE_Len, 1);
			            VarIesOffset += 1;

				NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, apcli_entry->RSN_IE, apcli_entry->RSNIE_Len);
				VarIesOffset += apcli_entry->RSNIE_Len;
			...
			(apcli_entry->bRSN_IE_FromWpaSupplicant == TRUE))			
		{
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpWpaAssocIeLen,
	                        	apcli_entry->WpaAssocIeLen, apcli_entry->pWpaAssocIe,
	                        	END_OF_ARGS);

			FrameLen += TmpWpaAssocIeLen;

			NdisMoveMemory(apcli_entry->ReqVarIEs + VarIesOffset, apcli_entry->pWpaAssocIe, apcli_entry->WpaAssocIeLen);
			VarIesOffset += apcli_entry->WpaAssocIeLen;
