								  ...
								  u8 channel, BOOLEAN scan)
{
	...
	//mt76x2_set_ed_cca(ad, TRUE);
		...
		if (channel > 14) {			
			...
			ad->DefaultTargetPwr = ee_val & 0x00FF;
#ifdef DOT11_VHT_AC
			if (ad->CommonCfg.BBPCurrentBW == BW_80)
			...
			RT28xx_EEPROM_READ16(ad, 0xF6, ee_val);
			ad->DefaultTargetPwr = ee_val & 0xFF00;
		...
		*/
		...
		DBGPRINT(RT_DEBUG_ERROR, ("DefaultTargetPwr = 0x%x, delta_power = 0x%x\n", 
			ad->DefaultTargetPwr, delta_power));
		...
		RTMP_IO_READ32(ad, TX_ALC_CFG_1, &RegValue);
		RegValue = (RegValue & ~0x3F) | (ch_pwr_adj & 0x3F);
		RTMP_IO_WRITE32(ad, TX_ALC_CFG_1, RegValue);

		RTMP_IO_READ32(ad, TX_ALC_CFG_2, &RegValue);
		RegValue = (RegValue & ~0x3F) | (ch_pwr_adj & 0x3F);
		RTMP_IO_WRITE32(ad, TX_ALC_CFG_2, RegValue);	

		ad->tc_init_val = ch_pwr_adj & 0x3F;

		DBGPRINT(RT_DEBUG_ERROR, ("SkuBasePwr = 0x%x,  DefaultTargetPwr = 0x%x, ChannelPwrAdj = 0x%x(%d), 0x13B4: 0x%x\n", 
			sku_base_pwr, ad->DefaultTargetPwr, ch_pwr_adj, ch_pwr_adj, RegValue));
...
UCHAR mt76x2_update_sku_pwr(RTMP_ADAPTER *ad, u8 channel)
{
	...
	UCHAR sku_base_pwr = 0, start_ch = 0;
	...
	*/
	RTMP_IO_READ32(ad, TX_ALC_CFG_0, &reg_val);
