...
/*NetDevInit============================================== */
	...
	os_alloc_mem(NULL,(UCHAR **)&((POS_COOKIE)handle)->pci_dev,sizeof(struct pci_dev));
	NdisZeroMemory(((POS_COOKIE)handle)->pci_dev, sizeof(struct pci_dev));
	memcpy(&((POS_COOKIE)handle)->pci_dev->dev,&net_dev->dev,sizeof(net_dev->dev));
