...
static UINT8 NICGetBandSupported(
...
#ifdef CONFIG_RT15N
	{LED_CFG,		0x77031e46}, // ASUS, 2008-03-06
#else
...
BOOLEAN RTMPCheckPhyMode(
	...
	INOUT 	PUCHAR 			pPhyMode)
{
	BOOLEAN RetVal = TRUE;

	if (BandSupported == RFIC_24GHZ)
	{
		if (!PHY_MODE_IS_2G_ONLY(*pPhyMode))
		{
	...
	else if (BandSupported == RFIC_5GHZ)
	{
		if (!PHY_MODE_IS_5G_ONLY(*pPhyMode))
		{
			DBGPRINT(RT_DEBUG_TRACE, 
	...
	else if (BandSupported == RFIC_DUAL_BAND)
	{
		RetVal = TRUE;
	}
	else
...
/*	if (Antenna.word == 0xFFFF)*/
	...
	pAd->RFICType = NICGetBandSupported(pAd);

	if (RTMPCheckPhyMode(pAd, pAd->RFICType, &pAd->CommonCfg.PhyMode) == FALSE)
	{
		STRING PhyMode[10];
		sprintf(PhyMode, "%u", pAd->CommonCfg.PhyMode);
		Set_WirelessMode_Proc(pAd, PhyMode);
	}
	...
	if ((value & 0x00FF) != 0x00FF)
		pAd->RfFreqOffset = (value & 0x00FF);
	else
		pAd->RfFreqOffset = 0;

	DBGPRINT(RT_DEBUG_TRACE, ("E2PROM: RF FreqOffset=0x%02x \n", pAd->RfFreqOffset));
...
#ifdef CONFIG_AP_SUPPORT
{
	...
	if (pAd->chipCap.FlgHwStreamMode)
	{
		...
		NdisMoveMemory(
					&pAd->CommonCfg.StreamModeMac[0][0],
					BROADCAST_ADDR,
					MAC_ADDR_LEN);
	}
#endif /* STREAM_MODE_SUPPORT */
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		pAd->ApCfg.DataFloodThreshold = 5;	/* 5 malicious data frame per sec */
	...
	IN INT			infType)
{
	...
	/* The original has moved to rt28xx_init, and doing after RTMPSetProfileParameters */
#ifdef MULTIPLE_CARD_SUPPORT 
	pAd->E2pAccessMode = RtmpEepromGetDefault(pAd);
