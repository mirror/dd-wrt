...
#ifdef RA_MTD_RW_BY_NUM
#ifdef CONFIG_MTD_AR670W
	ra_mtd_read(MTD_NUM_FACTORY, 0x2000, (size_t)b, p);
#else
 	ra_mtd_read(MTD_NUM_FACTORY, 0x8000, (size_t)b, p);
#endif
#else
...
#ifdef RA_MTD_RW_BY_NUM
#ifdef CONFIG_MTD_AR670W
	ra_mtd_write(MTD_NUM_FACTORY, 0x2000, (size_t)b, p);
#else
 	ra_mtd_write(MTD_NUM_FACTORY, 0x8000, (size_t)b, p);
#endif
#else
...
#if (WIRELESS_EXT < 20) && (WIRELESS_EXT >= 12)
...
USHORT RtmpDevPrivFlagsGet(IN VOID *pDev) {
	...
	pDevInfo = (DEV_PRIV_INFO *) _RTMP_OS_NETDEV_GET_PRIV((PNET_DEV) pDev);
	if (pDevInfo != NULL)
		return pDevInfo->priv_flags & INT_RTMASK;
	return 0;
}
