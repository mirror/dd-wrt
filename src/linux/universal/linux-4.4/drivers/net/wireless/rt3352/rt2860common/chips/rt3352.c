	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	pChipCap->TxPowerMaxCompenStep = 8; /* default 4dB */ /* ralink debug */
	pChipCap->TxPowerTableMaxIdx = 0; /* default 4dB */ /* ralink debug */
	TxPowerTuningTable = RT3352_TxPowerTuningTable;
#endif // RTMP_INTERNAL_TX_ALC //
	...
	pChipOps->AsicEeBufferInit = RT3352_AsicEeBufferInit;
	...
	pChipOps->ChipResumeMsduTransmission = RT3352_ChipResumeMsduTransmission;
#ifdef CONFIG_STA_SUPPORT
	pChipOps->ChipStaBBPAdjust = RT3352_ChipStaBBPAdjust;
#endif // CONFIG_STA_SUPPORT //
	pChipOps->ChipBBPAdjust = RT3352_ChipBBPAdjust;
	pChipOps->ChipSwitchChannel = RT3352_ChipSwitchChannel;
	pChipOps->RTMPSetAGCInitValue = RT3352_RTMPSetAGCInitValue;
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	for(IdReg=0; IdReg<RT3352_NUM_MAC_REG_PARMS; IdReg++)
	{
		...
		{
			if ((RT3352_MACRegTable[IdReg].Register == PWR_PIN_CFG) ||
				(RT3352_MACRegTable[IdReg].Register == TX_PIN_CFG))
				RtmpOsMsDelay(100);
		}
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	RT30xxReadRFRegister(pAd, RF_R17, (PUCHAR)&RfReg);
	RfReg = (RfReg & 0x80) | pAd->RfFreqOffset;
	RT30xxWriteRFRegister(pAd, RF_R17, (UCHAR)RfReg);
	...
	for (i = 0; i < RT3352_NUM_RF_REG_PARMS; i++)
	{
		...
		if ((pAd->CommonCfg.CID & 0x0000000f) < 0x00000006)
		{
			if ((RT3352_RFRegTable[i].Register == RF_R18) ||
				(RT3352_RFRegTable[i].Register == RF_R20))
				RtmpOsMsDelay(100);
		}
	}
...
========================================================================
Routine Description:
	Initialize default flash content for RT3352.
...
========================================================================
*/
VOID RT3352_AsicEeBufferInit(
	IN	RTMP_ADAPTER *pAd)
{
	extern UCHAR *EeBuffer;


	EeBuffer = RT3352_EeBuffer;
}
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	DBGPRINT(RT_DEBUG_INFO, ("RT3352_EnableAPMIMOPS, changes the # of antenna to 1\n"));
}
	...
	IN PRTMP_ADAPTER			pAd)
{
	...
	DBGPRINT(RT_DEBUG_INFO, ("RT3352_DisableAPMIMOPS\n"));
}
#endif // GREENAP_SUPPORT //
...
VOID RT3352_ChipResumeMsduTransmission(
	IN	PRTMP_ADAPTER		pAd)
{
	UCHAR	byteValue = 0;
	...
	)
	{
		...
		{	// request by Gary 20070208 for middle and long range G Band
			if (IS_RT3352(pAd))
			{
				...
				/* Chain 0 */
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R27, byteValue);
//				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R66, 0x3A);
