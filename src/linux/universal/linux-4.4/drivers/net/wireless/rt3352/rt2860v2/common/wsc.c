...
//						P2P_GoStop(pAdapter);
			...
			if (CurOpMode == STA_MODE)
			{
				...
				{
					WscBuildBeaconIE(pAdapter, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, BSS0, NULL, 0, CurOpMode);
					WscBuildProbeRespIE(pAdapter, WSC_MSGTYPE_REGISTRAR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, BSS0, NULL, 0, CurOpMode);
					WscLinkDown(pAdapter);
				}
...
/*	UCHAR			OutMsgBuf[512];		// buffer to create message contents */
	...
	if (pAd->StaCfg.WscControl.WscV2Info.bEnableWpsV2)
	{
		if (pAd->StaCfg.WscControl.bWscTrigger)
		{
			...
			{
				tempVal = (pAd->StaCfg.WscControl.WscConfigMethods & 0x200F);
			}
			else
			{
				tempVal = (pAd->StaCfg.WscControl.WscConfigMethods & 0x02F0);
			}
		}
		else
		{
...
#ifdef CONFIG_STA_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
			pWscEnList = &pAd->StaCfg.WscControl.WscPeerList;
#endif // CONFIG_STA_SUPPORT //
