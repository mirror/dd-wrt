	...
	IN PVOID SystemSpecific3) 
{
	...
	if (pP2PCtrl->P2pCounter.ListenInterval > 0)
	...
	if (pAd->P2pCfg.P2pCounter.CounterAftrScanButton > 0)
		pAd->P2pCfg.P2pCounter.CounterAftrScanButton--; /* update Device Discovery period */

	if (pAd->P2pCfg.P2pCounter.CounterAftrSetEvent != 0xffffffff)
		pAd->P2pCfg.P2pCounter.CounterAftrSetEvent++;

	if (pAd->P2pCfg.P2pCounter.ClientConnectedCounter > 0)
		pAd->P2pCfg.P2pCounter.ClientConnectedCounter--;

	if (pAd->P2pCfg.P2pCounter.ManageAPEnReconCounter > 0)
		pAd->P2pCfg.P2pCounter.ManageAPEnReconCounter--;
...
VOID P2pStopConnectAction(
	IN PRTMP_ADAPTER pAd)
{
	DBGPRINT(RT_DEBUG_ERROR, ("P2pStopConnectAction =  %s \n",  decodeP2PState(pAd->P2pCfg.P2PConnectState)));
	pAd->P2pCfg.ConnectingIndex = MAX_P2P_GROUP_SIZE;
}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/* pAd->MlmeAux.Channel = pP2PCtrl->P2pProprietary.ListenChanel[pP2PCtrl->P2pProprietary.ListenChanelCount-1]; */
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	/* pAd->P2pCfg.P2PDiscoProvState = P2P_IDLE; */
	...
	if (P2P_GO_ON(pAd))
	{
		if (pP2PCtrl->GONoASchedule.bValid == TRUE)
		{
			...
			if (Value <= pP2PCtrl->GONoASchedule.NextTargetTimePoint)
			{
				...
				DBGPRINT(RT_DEBUG_INFO,(" NextTargetTimePoint = %d. \n", pAd->P2pCfg.GONoASchedule.NextTargetTimePoint));
				DBGPRINT(RT_DEBUG_INFO,("  Value = %d.	GPDiff = %d.\n", Value, GPDiff));
			}
			else
			{
	...
	IN UINT32 ConnType)
{
	...
	if (p2pindex < MAX_P2P_GROUP_SIZE)
	{
		...
		if (IS_P2P_SEARCH(pAd) || IS_P2P_SCAN(pAd))
			OS_WAIT(200);			
		/* Change Discovery State Machine State. */
		pAd->P2pCfg.DiscCurrentState = P2P_DISC_SEARCH;
		pAd->P2pCfg.GoFormCurrentState = P2P_GO_FORM_IDLE;
	...
	IN UCHAR		idx)
{
	...
	if (pAd->P2pTable.Client[GrpIndex].P2pClientState == P2PSTATE_DISCOVERY)
	{
		...
		{
			index = P2pPerstTabSearch(pAd, pAd->P2pTable.Client[GrpIndex].addr, pAd->P2pTable.Client[GrpIndex].bssid, pAd->P2pTable.Client[GrpIndex].InterfaceAddr);
	...
	IN UCHAR		index)
{
	...
	{		
			...
			|| (pAd->P2pTable.Client[index].P2pClientState == P2PSTATE_INVITE_COMMAND))
		{
			...
			/*if ((pP2PCtrl->PortSubtype == PORTSUBTYPE_P2PGO) ||(IS_P2P_CLIENT_OP(pAd))) */
...
{ 
	...
	if ((pP2PCtrl->Rule == P2P_IS_GO) && (P2P_GO_ON(pAd)))
	{
		...
		if (pP2PCtrl->WscMode == WSC_PIN_MODE)
		{
			...
			{
				Set_AP_WscConfMode_Proc(pAd, "7"); /* Registrar / Proxy / Enrollee  */
	...
	else if ((pP2PCtrl->Rule == P2P_IS_CLIENT) && (P2P_CLI_ON(pAd)))
	{
		...
		/*COPY_MAC_ADDR(pAd->ApCfg.ApCliTab[0].CfgApCliBssid, pP2pEntry->InterfaceAddr);*/
		/* CLIENT WPS trigger. */
		if (pP2PCtrl->WscMode == WSC_PIN_MODE)
		{
			if (pP2PCtrl->ConfigMethod == WSC_CONFMET_KEYPAD)
...
/*	
	==========================================================================
	Description: 
		when timeout or error case happend, need to update P2PConnectState to correct state.
	Parameters: 
...
/*	
	==========================================================================
	Description: 
		Decide P2P's Default HT capability parameters. If concurrent mode and infra is connected, HT should follow
	...
	OUT 		UCHAR	*TempPid)
{
		...
		if ((pFrame->Subtype == GO_NEGOCIATION_CONFIRM) && (NdisEqualMemory(pP2pOUI, P2POUIBYTE, 4)) && (pFrame->Action == ACTION_WIFI_DIRECT) && (pFrame->Category == CATEGORY_PUBLIC))
		{
		...
		if ((pFrame->Subtype == P2P_INVITE_RSP) && (NdisEqualMemory(pP2pOUI, P2POUIBYTE, 4)) && (pFrame->Action == ACTION_WIFI_DIRECT) && (pFrame->Category == CATEGORY_PUBLIC))
		{
	...
	UINT32	value)
{
	...
	if (pP2PCtrl->P2pCounter.bStartScan == FALSE)
	{
		...
		pP2PCtrl->P2pCounter.CounterAftrScanButton = 200;//P2P_SCAN_PERIOD;
	}
}
	...
	PUSHORT PeerWscMethod)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("My P2P Wsc Configuration:\n"));
	DBGPRINT(RT_DEBUG_ERROR, ("WscMode = %d.    WscConfigMethod = %s.    WscDpid = %s.\n", pP2PCtrl->WscMode, decodeConfigMethod(pP2PCtrl->ConfigMethod), decodeDpid(pP2PCtrl->Dpid)));
	...
	DBGPRINT(RT_DEBUG_ERROR, ("Peer P2P Wsc Configuration:\n"));
...
{
	DBGPRINT(RT_DEBUG_ERROR, ("                         DeviceCapability = %x.\n", State));
	if (State & DEVCAP_SD)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [0] bServiceDiscovery\n"));
	if (State & DEVCAP_CLIENT_DISCOVER)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [1] bP2PClientDiscoverability\n"));
	if (State & DEVCAP_CLIENT_CONCURRENT)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [2] bConcurrentOperation\n"));
	if (State & DEVCAP_INFRA_MANAGED)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [3] bP2PInfrastructureManaged\n"));
	if (State & DEVCAP_DEVICE_LIMIT)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [4] bP2PDeviceLimit\n"));
	if (State & DEVCAP_INVITE)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [5] bP2PInvitationProcedure\n"));
}

VOID decodeGroupCap (UCHAR State)
{
	DBGPRINT(RT_DEBUG_ERROR, ("                         GroupCapability = %x.\n", State));
	if (State & GRPCAP_OWNER)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [0] bP2PGroupOwner\n"));
	if (State & GRPCAP_PERSISTENT)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [1] bPersistentP2PGroup\n"));
	if (State & GRPCAP_LIMIT)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [2] bP2PGroupLimit\n"));
	if (State & GRPCAP_INTRA_BSS)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [3] bIntra-BssDistribution\n"));
	if (State & GRPCAP_CROSS_CONNECT)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [4] bCrossConnection\n"));
	if (State & GRPCAP_PERSISTENT_RECONNECT)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [5] bPersistentReconnect\n"));
	if (State & GRPCAP_GROUP_FORMING)
		DBGPRINT(RT_DEBUG_ERROR, ("                             [6] bGroupFormation\n"));
}
	...
	IN UCHAR		p2pindex)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s:: p2p Entry idx = %d\n", __FUNCTION__, p2pindex));
	DBGPRINT(RT_DEBUG_ERROR, ("====================================================>\n"));
	pClient = &pAd->P2pTable.Client[p2pindex];
	DBGPRINT(RT_DEBUG_ERROR, ("P2pClientState = %s  \n", decodeP2PClientState(pClient->P2pClientState)));
	DBGPRINT(RT_DEBUG_ERROR, ("ConfigMethod = %x.    %s \n", pClient->ConfigMethod, decodeConfigMethod(pClient->ConfigMethod)));
	DBGPRINT(RT_DEBUG_ERROR, ("Opchannel = %d.    Listenchannel = %d.", pClient->OpChannel, pClient->ListenChannel));
	DBGPRINT(RT_DEBUG_ERROR, ("MyGOIndex = %d. rule = %s  , addr = %02x:%02x:%02x:%02x:%02x:%02x\n", pClient->MyGOIndex, decodeMyRule(pClient->Rule), PRINT_MAC(pClient->addr)));
	...
	DBGPRINT(RT_DEBUG_ERROR, ("DeviceType = %02x %02x %02x %02x %02x %02x %02x %02x\n", pClient->PrimaryDevType[0], pClient->PrimaryDevType[1], pClient->PrimaryDevType[2],pClient->PrimaryDevType[3],pClient->PrimaryDevType[4],pClient->PrimaryDevType[5],pClient->PrimaryDevType[6],pClient->PrimaryDevType[7]));
	DBGPRINT(RT_DEBUG_ERROR, ("NumSecondaryType = %x.    RegClass = %x.    ConfigTimeOut = %x. \n", pClient->NumSecondaryType, pClient->RegClass, pClient->ConfigTimeOut));
	DBGPRINT(RT_DEBUG_ERROR, ("DeviceName[%ld] = ", pClient->DeviceNameLen));
	for (j=0; j<pAd->P2pTable.Client[p2pindex].DeviceNameLen;j++)
		DBGPRINT(RT_DEBUG_ERROR, ("%c ", pAd->P2pTable.Client[p2pindex].DeviceName[j]));
	DBGPRINT(RT_DEBUG_ERROR, ("\n"));
		...
		if (pMbss->WscControl.WscV2Info.bEnableWpsV2)
		{
			...
			*/
			if ((pMbss->WepStatus == Ndis802_11WEPEnabled) || (pMbss->WepStatus == Ndis802_11Encryption2Enabled))
				WscOnOff(pAd, MAIN_MBSSID, TRUE);
			else
				WscOnOff(pAd, MAIN_MBSSID, FALSE);
		}
	...
	IN PRTMP_ADAPTER 	pAd)
{
	...
	for(ifIndex = 0; ifIndex < MAX_APCLI_NUM; ifIndex++)
	{
		...
		/*WscInit(pAd, TRUE, ifIndex); */
		RTMP_AP_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_WSC_INIT, 0, (VOID *)pApCliEntry, 0);
...
*/
    ...
    if ((pAd->CommonCfg.CentralChannel > pAd->CommonCfg.Channel) && (pAd->ApCliMlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_40))
	{	
		/* Must using 40MHz. */
		...
		DBGPRINT(RT_DEBUG_TRACE, ("!!!40MHz Lower !!! ApCli Control Channel at Below. Central = %d \n", pAd->CommonCfg.CentralChannel ));
	}
	else if ((pAd->CommonCfg.CentralChannel < pAd->CommonCfg.Channel) && (pAd->ApCliMlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_40))
    {	
	    /* Must using 40MHz. */
		pAd->CommonCfg.BBPCurrentBW = BW_40;
		AsicSwitchChannel(pAd, pAd->CommonCfg.CentralChannel, FALSE);
	    ...
	    DBGPRINT(RT_DEBUG_TRACE, ("!!! 40MHz Upper !!! ApCli Control Channel at UpperCentral = %d \n", pAd->CommonCfg.CentralChannel ));
    }
    else
#endif /* DOT11_N_SUPPORT */
    {
	    ...
	    DBGPRINT(RT_DEBUG_TRACE, ("!!! ApCli 20MHz !!! \n" ));
	}
	...
	IN RTMP_OS_NETDEV_OP_HOOK		*pNetDevOps)
{
	...
	NdisMoveMemory(pAd->ApCfg.MBSSID[MAIN_MBSSID].Ssid, "DIRECT-qa", 9);
	pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen = 9;
