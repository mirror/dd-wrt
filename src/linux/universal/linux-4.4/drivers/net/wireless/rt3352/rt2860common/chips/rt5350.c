	...
	IN PUCHAR					pBbpR49)
{
	...
	if (bKeepRF == FALSE)
	{
	  ...
	  TotalDeltaPower = pAd->TxPowerCtrl.MAC_PowerDelta;
		...
		DBGPRINT(RT_DEBUG_TRACE, ("Current index of tuning table = %d {RF_TX_ALC = 0x%X, MAC_PowerDelta = %d}\n", \
	...
	IN PRTMP_ADAPTER			pAd)
{
	...
	*/
	if (((E2PValue >> 8) & 0x04) == 0x04)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s : slope of TSSI is 0.7\n", __FUNCTION__));
	}
	else
	{
		pChipOps->TSSIRatio = TSSIRatioDot85;
		DBGPRINT(RT_DEBUG_TRACE, ("%s : slope of TSSI is 0.85\n", __FUNCTION__));
	}	
