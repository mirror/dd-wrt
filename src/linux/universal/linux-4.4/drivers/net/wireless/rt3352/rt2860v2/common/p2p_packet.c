...
/*	MLME_P2P_ACTION_STRUCT	P2PActReq; */	
	...
	if (((pP2PCtrl->bSigmaEnabled == TRUE) && (DevDiscCnt == 0))||
			((pMacEntry != NULL) && (pEntry->P2pClientState == P2PSTATE_CLIENT_WPS_DONE)))
	{
		MLME_P2P_ACTION_STRUCT	P2PActReq;	
	...
	{
			DBGPRINT(RT_DEBUG_TRACE, ("%s:: p2p client[%02x:%02x:%02x:%02x:%02x:%02x]. has leaving Status fail", __FUNCTION__, PRINT_MAC(pEntry->InterfaceAddr)));
		P2PSendDevDisRsp(pAd, P2PSTATUS_IMCOMPA_PARM, pFrame->Token, pFrame->p80211Header.Addr2, &TotalFrameLen);
	}
		return;
	}

	DBGPRINT(RT_DEBUG_TRACE, ("%s:: can't find p2p client[%02x:%02x:%02x:%02x:%02x:%02x].Status fail", __FUNCTION__, PRINT_MAC(devAddr)));
	P2PSendDevDisRsp(pAd, P2PSTATUS_IMCOMPA_PARM, pFrame->Token, pFrame->p80211Header.Addr2, &TotalFrameLen);
}
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	if (bReinvoke == TRUE)
	{
		...
		P2pCheckInviteReq(pAd, (pAd->P2pTable.PerstEntry[index].MyRule == P2P_IS_GO), index, ChannelList, BssidAddr/*DevAddr*/, OpChannel, Ssid, SsidLen, &RspStatus);
		if (RspStatus == P2PSTATUS_SUCCESS)
		{
	...
	OUT PULONG		pTotalFrameLen)
{
	...
	*pTotalFrameLen += Length;
	...
	/*
		The Tie breaker bit in a GO Negotiation Response frame shall be toggled from the corresponding GO Negotiation Request frame
	*/
	...
	OUT PULONG		pTotalFrameLen)
{
	...
	*pTotalFrameLen += Length;
	...
	if (INFRA_ON(pAd))
		Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_LISTEN_CHANNEL, &pAd->CommonCfg.Channel, pDest);
	else
