...
#ifdef CONFIG_AP_SUPPORT
#ifdef AP_DLS_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
				APPeerDlsReqAction(pAd, Elem);
#endif /* AP_DLS_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
...
#ifdef RT_BIG_ENDIAN
			if (SWAP32(*(PULONG)&Elem->Msg[LENGTH_802_11+2]) == P2P_OUI)
#else
			if (*(PULONG)&Elem->Msg[LENGTH_802_11+2] == P2P_OUI)
#endif /* RT_BIG_ENDIAN */
			{
				...
				switch(Subtype)
				{
					case GO_NEGOCIATION_REQ:
						...
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_GO_NEGO_REQ_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case GO_NEGOCIATION_RSP:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Group Nego. Rsp\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_GO_NEGO_RSP_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case GO_NEGOCIATION_CONFIRM:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Group Nego. Confirm\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_GO_NEGO_CONFIRM_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case P2P_INVITE_REQ:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Invite Req\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_INVITE_REQ_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case P2P_INVITE_RSP:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Invite Rsp\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_INVITE_RSP_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case P2P_DEV_DIS_REQ:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Device Discovery Req\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_DEV_DISC_REQ_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case P2P_DEV_DIS_RSP:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Device Discovery Rsp\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_DEV_DISC_RSP_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case P2P_PROVISION_REQ:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Provision Req\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_GO_NEGO_PROV_REQ_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					case P2P_PROVISION_RSP:
						DBGPRINT(RT_DEBUG_ERROR, ("%s : recv Provision Rsp\n", __FUNCTION__));
						MlmeEnqueue(pAd, P2P_GO_FORM_STATE_MACHINE, P2P_PEER_GO_NEGO_PROV_RSP_EVT, Elem->MsgLen, Elem->Msg, Subtype);
						break;
					default:
