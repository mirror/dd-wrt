/*******************************************************************************
	DualDemo.c Copyright 2002 Silicon Laboratories

This file simulates the PBX demo.  The below consists of two parts.  The state machine 
and the interrupt service routine.  This code is meant for demo purposes only.  
************************************************************************************/
//#include "stdlib.h"
//#include "stdio.h"
#include "dual_io.h"
...
//void slic_demo_main(char **argv, int argc)
void slic_demo_main()

{
	...
	int i=0, j=0  ; 
	int startTime = 0;
//	register unsigned short ramTemp ;//  16 bit data read once but used several times;
	register unsigned char chipType,version, regTemp ; // 8 bit value for temporary used
//	unsigned char freq;
	...
	printf("\n running on a %I64u machine\n",	calibratePrecisionClock()); 



	TicksPerSecond= calibratePrecisionClock();

	reset(); /* sets the reset of the proslic */

	//sleep( (CLOCKS_PER_SEC )/4); /* hold it for a quarter of a second */
	slic_sleep(1000);
	unReset(); 
	/* Release the Reset */

	//sleep( (CLOCKS_PER_SEC )/4);
	slic_sleep(1000);
	...
	chipType= 0x0f & (regTemp >>4 );
	version = (0x0f & regTemp) -1;
    
	printf("\n Dual Demo 0.11 [ID=0x%X]\n\n", regTemp);
	
	printf("\n%s  Revision %s  \n",  TYPE[chipType],REVISION[version]); 

	for (currentChannel=0; currentChannel<1; currentChannel++)
	{
		printf("\n Intitializing Channel %i\n",currentChannel);

		//logfile[currentChannel]=fopen(filename[currentChannel],"w+");		
		changeCID(currentChannel);
	
		pCCData= &channelData[currentChannel];
	
		pCCData->version = 0xf & regTemp; 
		pCCData->type  = (0x70 & regTemp) >> 4;
		
	
		setState(ONHOOK);  /* This state is assumed here but it will be tested later. */
	
		pCCData->country=USA; //defaults to USA
		pCCData->complexCadence = false;
		pCCData->toneType = DIALTONEtype;
		pCCData->currentPulsedigit =0;
		if(dualProSLIC_channelInit())//intializes each channel
		{
			printf("\n\n Halting....");
			exit(0);  /* initialize one Dual ProSLIC */
		}
		printf("dualProSLIC_channelInit(ch=%d)..done\n",currentChannel);
		pCCData->connectionWith = currentChannel;
		connectionTable[currentChannel].source=connectionTable[currentChannel].destination = currentChannel ;
		printf("setState[ONHOOK]..ch=%d\n",currentChannel);
		setState(ONHOOK);  /* This state is assumed here but it will be tested later. */
		printf("goActive..\n");
		goActive();
	...
	//printf("\nPress 'D' to enter diagnostics mode,\nPress 'P' to enter PBX mode.\n");
	//scanf("%s", select);
	//if (select[0] == 'D' || select[0] == 'd')
	//printf("diagnostics...\n");
	//	runDiag();

    /**********************************************************************
	The system loops through each channel and checks for interrupts.  If one is
	found it changes the state machine as necessary.  This is accomplished via polling
	the interrupt status register in the Si3220/3225.
	***********************************************************************/

	while (1)  // Loop through both the channels run the demo per channel
	...
	*/
	...
	for ( interruptCause=OSC1_T1 ; interruptCause <= CMBALS ; interruptCause++)
	{
		if (shiftMask & pCCData->interrupt)
		{
			...
			//fprintf(logfile[0],(( interruptCause >=10) && (interruptCause<=11))?"\n %s""\t\t\t\t Channel #%d  ":"\n(%s)  " "\t\t\t\t Channel #%d  ", icause[interruptCause], currentChannel);
			//fflush(logfile[0]);
...
//		checkForGroundShorts(); /* Check for grounded Tip or Ring Leads*/
	...
	case PQ6:
		...
		{int lcv;		for (lcv=0;lcv<8;lcv++)lastEventNumber[lcv] =1;  }
...
{
	//fprintf(logfile[0],"\nState = %s  Channel %i",szStates[pCCData->state],currentChannel);
	//fflush(logfile[0]);
