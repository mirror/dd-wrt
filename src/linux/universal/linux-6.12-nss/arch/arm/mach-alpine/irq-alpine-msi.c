				    ...
				    struct device_node *node)
{
	...
	if (!gic_node) {
		printk(KERN_EMERG "Failed to find the GIC node\n");
		return -ENODEV;
	}

	gic_domain = irq_find_host(gic_node);
	if (!gic_domain) {
		printk(KERN_EMERG "Failed to find the GIC domain\n");
		return -ENXIO;
	}
	...
	middle_domain = irq_domain_add_tree(NULL,
					    ...
					    priv);
	if (!middle_domain) {
		printk(KERN_EMERG "Failed to create the MSIX middle domain\n");
		return -ENOMEM;
	}
	...
	msi_domain = pci_msi_create_irq_domain(of_node_to_fwnode(node),
					       ...
					       middle_domain);
	if (!msi_domain) {
		printk(KERN_EMERG "Failed to create MSI domain\n");
...
int al_msix_init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "annapurna-labs,al-msix");
	BUG_ON(!node);
	...
	if (ret) {
		printk(KERN_EMERG "Failed to allocate resource\n");
		goto err_priv;
	}
...
//	}
	...
	printk(KERN_EMERG "Registering %d msixs, starting at %d\n",
...
//IRQCHIP_DECLARE(alpine_msix, "annapurna-labs,al-msix", alpine_msix_init);
