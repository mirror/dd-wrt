...
static void br_cfm_notify(int event, struct net_bridge_port *port)
{
...

{
	...
	/* Ethernet header */
	eth_hdr = (void *)skb_put(skb, sizeof(*eth_hdr));
	...
	/* Common CFM Header */
	common_hdr = (void *)skb_put(skb, sizeof(*common_hdr));
	...
	/* Sequence number */
	snumber = (void *)skb_put(skb, sizeof(*snumber));
	...
	*mepid = cpu_to_be16((u16)mep->config.mepid);
	...
	if (tx_info->port_tlv) {
		status_tlv = (void *)skb_put(skb, sizeof(*status_tlv));
	...
	/* End TLV */
	e_tlv = (void *)skb_put(skb, sizeof(*e_tlv));
