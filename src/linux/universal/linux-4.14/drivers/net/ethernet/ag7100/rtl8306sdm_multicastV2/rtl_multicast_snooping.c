...
/*gateway infomation*/
uint8 rtl_gatewayMac[6]={0,0,0,0,0,0};
static uint32 rtl_gatewayIpv4Addr={0};
static uint32 rtl_gatewayIpv6Addr[4]={0,0,0,0};
...
static void rtl_checkGroupEntryTimer(struct rtl_groupEntry * groupEntry)
{
	...
	if(oldFwdPortMask!=newFwdPortMask)
	{
		...
		{
			rtl_set8306LookupTable(groupEntry);  /*set rtl8306sdm lookup table*/
		}
	}
	...
	if(groupEntry->aggregatorFlag==TRUE)
	{
		...
		if(aggregatorPortMask!=0) /*need to modify aggregator multicast entry*/
		{
			if(groupEntry->lookupTableFlag==TRUE) /*means in the lookup table*/
			{
                            ...
                            #ifndef RTL_MULTICAST_SNOOPING_TEST			
				if(rtl8306_addMuticastMacAddress(multicastMacAddress,1,LookupTablePortMask,&entryaddr)==SUCCESS) /*then modify existing entry*/
				#else
				if(rtl8306_addMuticastMacAddress(multicastMacAddress,1,LookupTablePortMask)==SUCCESS) /*then modify existing entry*/
				#endif
				...
				{
					rtl_gluePrintf("hareware failure\n");
					return FAILED;
				}
			}
			else /*means not in the lookup table*/
			{
				...
				#ifndef RTL_MULTICAST_SNOOPING_TEST
				if(rtl8306_addMuticastMacAddress(multicastMacAddress, 1, LookupTablePortMask,&entryaddr)==SUCCESS)  /*always try to write lookup table*/
				#else
				if(rtl8306_addMuticastMacAddress(multicastMacAddress,1,LookupTablePortMask)==SUCCESS) /*then modify existing entry*/
				#endif
                            {
		...
		else /*means all aggregator entries expired at the same time*/
		{
			if(aggregatorLookupTableFlag!=0)
			{
				...
				#else
				if(rtl8306_deleteMacAddress(multicastMacAddress)==SUCCESS) 
				#endif
				{
	...
	else /*here means non-aggregator entry*/
	{
		...
		{	
			...
			if(groupEntry->lookupTableFlag==TRUE)
			{
                            ...
                            #ifndef RTL_MULTICAST_SNOOPING_TEST
				if(rtl8306_addMuticastMacAddress(multicastMacAddress, 1, LookupTablePortMask,&entryaddr)!=SUCCESS)
				#else
				if(rtl8306_addMuticastMacAddress(multicastMacAddress, 1, LookupTablePortMask)!=SUCCESS)
				#endif
				{
					rtl_gluePrintf("hareware failure\n");
					return FAILED;
			...
			else
			{
				...
				{	
						if(rtl_compareMacAddr(macFrame, rtl_gatewayMac)==TRUE) 
				       	{
								if(((struct ipv4Pkt *)(macInfo->ipBuf))->destinationIp==htonl(rtl_gatewayIpv4Addr))
								{
				...
				case ICMP_PROTOCOL:
					...
					if((ptr[0]==MLD_QUERY) ||(ptr[0]==MLDV1_REPORT) ||(ptr[0]==MLDV1_DONE) ||(ptr[0]==MLDV2_REPORT))
					{
						...
						{	
								...
								ipAddr[0]=htonl(rtl_gatewayIpv6Addr[0]);
								ipAddr[1]=htonl(rtl_gatewayIpv6Addr[1]);
								ipAddr[2]=htonl(rtl_gatewayIpv6Addr[2]);
								ipAddr[3]=htonl(rtl_gatewayIpv6Addr[3]);
								if(	(rtl_compareMacAddr(macFrame, rtl_gatewayMac)==TRUE) &&\
...
@parm uint32	| delPortMaskRevLeave		| TRUE to delete group member port which send leave or TO_IN{} records.  FALSE to lower group member port timer .
	...
	if(rtl_multicastStatus==DISABLE)
	{
		...
		rtl_totalMaxGroupCnt=multicastConfiguration.maxGroupNum;
	      ...
	      else
	      {
			 ...
			 rtl_gatewayIpv4Addr=0;
			 for(j=0; j<4; j++)
	      		{
		      		rtl_gatewayIpv6Addr[j]=0;
	     		 }
...
void rtl_setMulticastParameters(struct rtl_mCastTimerParameters mCastTimerParameters, uint8 *gatewayMac, uint32 gatewayIpv4Addr, uint32 *gatewayIpv6Addr)
{
	...
	if(gatewayMac!=NULL)
	{
		if((rtl_gatewayMac[0]|rtl_gatewayMac[1]|rtl_gatewayMac[2]|rtl_gatewayMac[3]|rtl_gatewayMac[4]|rtl_gatewayMac[5])==0)
		{
			...
			#else
			rtl8306_deleteMacAddress(rtl_gatewayMac);
			#endif
		}
	...
	{
			rtl_gatewayIpv4Addr=gatewayIpv4Addr;
	...
	{
				rtl_gatewayIpv6Addr[0]=gatewayIpv6Addr[0];
				rtl_gatewayIpv6Addr[1]=gatewayIpv6Addr[1];
				rtl_gatewayIpv6Addr[2]=gatewayIpv6Addr[2];
				rtl_gatewayIpv6Addr[3]=gatewayIpv6Addr[3];
