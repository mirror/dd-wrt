...
static void qcom_pcie_prog_viewport_cfg0(struct qcom_pcie *pcie, u32 busdev)
{
	...
	writel(PCIE20_PLR_IATU_REGION_OUTBOUND |
	       ...
	       pci->dbi_base + PCIE20_PLR_IATU_VIEWPORT);

	writel(PCIE20_PLR_IATU_TYPE_CFG0, pci->dbi_base + PCIE20_PLR_IATU_CTRL1);
	writel(PCIE20_PLR_IATU_ENABLE, pci->dbi_base + PCIE20_PLR_IATU_CTRL2);
	writel(pp->cfg0_base, pci->dbi_base + PCIE20_PLR_IATU_LBAR);
	writel((pp->cfg0_base >> 32), pci->dbi_base + PCIE20_PLR_IATU_UBAR);
	writel((pp->cfg0_base + pp->cfg0_size - 1),
	       pci->dbi_base + PCIE20_PLR_IATU_LAR);
	writel(busdev, pci->dbi_base + PCIE20_PLR_IATU_LTAR);
	writel(0, pci->dbi_base + PCIE20_PLR_IATU_UTAR);
}
...
static void qcom_pcie_prog_viewport_mem2_outbound(struct qcom_pcie *pcie)
{
	...
	writel(PCIE20_PLR_IATU_TYPE_MEM, pci->dbi_base + PCIE20_PLR_IATU_CTRL1);
	writel(PCIE20_PLR_IATU_ENABLE, pci->dbi_base + PCIE20_PLR_IATU_CTRL2);
	writel(pp->mem_base, pci->dbi_base + PCIE20_PLR_IATU_LBAR);
	writel((pp->mem_base >> 32), pci->dbi_base + PCIE20_PLR_IATU_UBAR);
	writel(pp->mem_base + pp->mem_size - 1,
	       pci->dbi_base + PCIE20_PLR_IATU_LAR);
	writel(pp->mem_bus_addr, pci->dbi_base + PCIE20_PLR_IATU_LTAR);
	writel(upper_32_bits(pp->mem_bus_addr),
	       pci->dbi_base + PCIE20_PLR_IATU_UTAR);

	/* 256B PCIE buffer setting */
	writel(0x1, pci->dbi_base + PCIE20_AXI_MSTR_RESP_COMP_CTRL0);
	writel(0x1, pci->dbi_base + PCIE20_AXI_MSTR_RESP_COMP_CTRL1);
}
