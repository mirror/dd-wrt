...
static irqreturn_t octeon_mmc_interrupt(int irq, void *dev_id)
{
	...
	printk(KERN_INFO "Got interrupt: EMM_INT = 0x%llx\n", emm_int.u64);

	if (!req)
		goto out;

	rsp_sts.u64 = cvmx_read_csr(host->base + OCT_MIO_EMM_RSP_STS);
	printk(KERN_INFO "octeon_mmc_interrupt  MIO_EMM_RSP_STS 0x%llx\n", rsp_sts.u64);

	if (host->dma_err_pending) {
	printk(KERN_INFO "dma error pending\n");
	...
	if (host_done && req->done) {
		...
		if (rsp_sts.s.rsp_val) {
			...
			u64 rsp_lo = 0;
			int cnt = 1000;
			rsp_lo = cvmx_read_csr(host->base + OCT_MIO_EMM_RSP_LO);
			...
			case 2:
				...
				break;
			default:
				printk(KERN_INFO "octeon_mmc_interrupt unhandled rsp_val %d\n",
					       rsp_sts.s.rsp_type);
				break;
			}
			printk(KERN_INFO "octeon_mmc_interrupt  resp %08x %08x %08x %08x\n",
...
static int octeon_mmc_probe(struct platform_device *pdev)
{
	...
	while (node) {
		...
		if (!r) {
			...
			printk(KERN_INFO "init slot %d, ret = %d\n", slot, ret);
			if (ret)
				goto err;
		}
