...
static irqreturn_t gsw_interrupt_mt7620(int irq, void *_priv)
{
	...
	if (status & PORT_IRQ_ST_CHG)
		for (i = 0; i <= max; i++) {
			...
			if (link != priv->link[i])
				mt7620_print_link_state(priv, i, link, (status >> 2) & 3, (status & 0x2));
...
void mt7620_hw_init(struct mt7620_gsw *gsw, struct device_node *np)
{
	...
	if (of_property_read_bool(np, "mediatek,mt7530")) {
		...
		mtk_switch_w32(gsw, mtk_switch_r32(gsw, GSW_REG_GPC1) | (0x1f << 24) | (0xc << 16), GSW_REG_GPC1);
...
int mtk_gsw_init(struct fe_priv *priv)
{
	...
	printk(KERN_EMERG "soc is %X\n",priv->soc);
	printk(KERN_EMERG "gsw pointer is %p\n",gsw);
	priv->soc->swpriv = gsw;
