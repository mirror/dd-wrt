...
static inline int
ar7100_set_wd_timer_action(uint32_t val)
{
...
#ifdef AR7100_WDT_TEST_CODE
static inline uint32_t
ar7100_get_wd_timer_action(void)
{
	return (uint32_t)(ar7100_reg_rd(AR7100_WATCHDOG_TMR_CONTROL) &
			AR7100_WD_ACT_MASK);
}

static inline uint32_t
ar7100_get_wd_timer_last(void)
{
	return ((uint32_t)(ar7100_reg_rd(AR7100_WATCHDOG_TMR_CONTROL) &
				AR7100_WD_LAST_MASK) >> AR7100_WD_LAST_SHIFT);
}
#endif /* AR7100_WDT_TEST_CODE */
...
static __init int
ar7100wdt_open(struct inode *inode, struct file *file)
{
	wddbg("%s: called\n", __func__);
...
static __init int
ar7100wdt_close(struct inode *inode, struct file *file)
{
	wddbg("%s: called\n", __func__);
...
static __init ssize_t
ar7100wdt_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	wddbg("%s: called\n", __func__);
...
static __init int
ar7100wdt_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
		unsigned long arg)
{
	int ret = 0;
...
static __init ssize_t
ar7100wdt_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
	int i;
	char c;
