...
static int soc_pci_read_config(struct pci_bus *bus, unsigned int devfn, int where, int size, u32 *val);
static int soc_pci_write_config(struct pci_bus *bus, unsigned int devfn, int where, int size, u32 val);
...
static int sbpci_read_config_reg(struct pci_bus *bus, u32 devfn, int where, int size, u32 *value)
{
	...
	ret = hndpci_read_config(sih, bus->number, PCI_SLOT(devfn), PCI_FUNC(devfn), where, value, size);
...
static int sbpci_write_config_reg(struct pci_bus *bus, u32 devfn, int where, int size, u32 value)
{
	...
	spin_lock_irqsave(&sih_lock, flags);
	ret = hndpci_write_config(sih, bus->number, PCI_SLOT(devfn), PCI_FUNC(devfn), where, &value, size);
	spin_unlock_irqrestore(&sih_lock, flags);
	return ret ? PCIBIOS_DEVICE_NOT_FOUND : PCIBIOS_SUCCESSFUL;
}
...
void __iomem *soc_pci_cfg_base(struct pci_bus *bus, unsigned int devfn, int where)
{
	...
	if (busno == 0) {
		...
		if (fn > 1)
			return NULL;
		type = 1;
		addr_reg = (busno & 0xff) << 20 | (slot << 15) | (fn << 12) | (where & 0xffc) | (type & 0x3);
...
static void asmedia_pcie_switch_init(struct pci_bus *bus, unsigned int devfn)
{
	struct soc_pcie_port *port = soc_pcie_bus2port(bus);
	u32 dRead = 0;
	u32 bm = 0;
...
static int soc_pci_read_config(struct pci_bus *bus, unsigned int devfn, int where, int size, u32 *val)
{
	...
	if (port->isswitch == 1) {
		if (bus->number == (bus_inc + 2)) {
			if (port->switch_id == PLX_SWITCH_ID && !((devfn == 0x8) || (devfn == 0x10))) {
				*val = ~0UL;
				return PCIBIOS_SUCCESSFUL;
			} else if (port->switch_id == ASMEDIA_SWITCH_ID && !((devfn == 0x18) || (devfn == 0x38))) {
				*val = ~0UL;
				return PCIBIOS_SUCCESSFUL;
			}
	...
	if ((bus->number == (bus_inc + 1)) && (port->isswitch == 0) && (where == 0) && (((data_reg >> 16) & 0xFFFF) == PLX_SWITCH_ID)) {
		plx_pcie_switch_init(bus, devfn);
		port->switch_id = PLX_SWITCH_ID;
		port->isswitch = 1;
	} else if ((bus->number == (bus_inc + 1)) && (port->isswitch == 0) && (where == 0) && (((data_reg >> 16) & 0xFFFF) == ASMEDIA_SWITCH_ID)) {
...
static void bcm5301x_usb_power_on(int coreid)
{
	int enable_usb;
...
static void bcm5301x_usb_idm_ioctrl(int coreid)
{
	uint32 usb3_idm_idm_base;
...
static void __init bcm5301x_3rd_pcie_init(void)
{
	uint32 cru_straps_ctrl;
	uint32 ccb_mii_base;
	uint32 dmu_base;
	...
	/* Check Chip ID */
	if (!BCM4707_CHIP(CHIPID(sih->chip)) || (sih->chippkg != BCM4708_PKG_ID && sih->chippkg != BCM4709_PKG_ID))
		return;
...
static void __init bcm5301x_pcie_phy_init(void)
{
	uint32 ccb_mii_base;
	uint32 *ccb_mii_mng_ctrl_addr;
	uint32 *ccb_mii_mng_cmd_data_addr;
	...
	for (i = 0; i < (pcie_ports_sz - 1); i++) {
		...
		SPINWAIT(((readl(ccb_mii_mng_ctrl_addr) >> 8 & 1) == 1), 1000);
		regaddr = 0x13;
		val = (sb << 30) | (op_w << 28) | (pa[i] << 23) | (regaddr << 18) | (ta << 16) | 0x0190;
		writel(val, ccb_mii_mng_cmd_data_addr);
