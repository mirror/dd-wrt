...
#if 0
typedef void gpioFn(unsigned short bpGpio, GPIO_STATE_t state);
extern gpioFn *gpioFnPtr;
#endif
...
#if 0
static int proc_get_boardid(char *page, char **start, off_t off, int cnt, int *eof, void *data);
static int proc_set_boardid(struct file *f, const char *buf, unsigned long cnt, void *data);
#endif
    ...
    int *pnoReboot, int partition)
{
    ...
    if (NULL != (pNvramDataOrig = readNvramData()))
    {
        ...
        else
        {
            ...
            ret = kerSysBcmNandImageSet(rootfs_part, string, size, 1);
			/*
                       ...
                       unsigned int command, unsigned long arg )
{
    ...
    case BOARD_IOCTL_FLASH_WRITE:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case FOXCNN_NVRAM:
                ret = kerSysFoxconnNvramSet(ctrlParms.string, ctrlParms.strLen);
                break;
            case FOXCNN_NVS:
                ret = kerSysNvDataSet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
            case FOXCNN_SP:
                ret = kerSysSPSet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
            case FOXCNN_BD:
                ret = kerSysBDSet(ctrlParms.string, ctrlParms.strLen);
                break;
            case FOXCNN_DPF:
                ret = kerSysDPFSet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
    ...
    case BOARD_IOCTL_FLASH_READ:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case FOXCNN_NVRAM:
                ret = kerSysFoxconnNvramGet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
            case FOXCNN_NVS:
                ret = kerSysNvDataGet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
            case FOXCNN_SP:
                ret = kerSysSPGet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
            case FOXCNN_BD:
                ret = kerSysBDGet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
            case FOXCNN_DPF:
                ret = kerSysDPFGet(ctrlParms.string, ctrlParms.strLen, ctrlParms.offset);
                break;
            ...
            case SET_BOARDID: {
                    ...
                    if (pNvramData == NULL)
                         return -ENOMEM;
                    printk("set_board_id:%s old:%s\n", ctrlParms.string, pNvramData->szFirmwareUpgradeBoardId);
                    //memcpy(pNvramData->szFirmwareUpgradeBoardId, ctrlParms.string, ctrlParms.strLen);
                    __copy_from_user((char*)pNvramData->szFirmwareUpgradeBoardId, ctrlParms.string, UP_BOARD_ID_LEN);
                    printk("set_board_id:%s new:%s\n", ctrlParms.string, pNvramData->szFirmwareUpgradeBoardId);
            ...
            case GET_BOARDID: {
                    ...
                    if (pNvramData == NULL)
                         return -ENOMEM;
                    memcpy(ctrlParms.string, pNvramData->szFirmwareUpgradeBoardId, strlen(pNvramData->szFirmwareUpgradeBoardId));
                    printk("get board id %d %d %s\n", GET_BOARDID, SET_BOARDID, pNvramData->szFirmwareUpgradeBoardId);
                    kfree(pNvramData);
				} break;
    ...
    case BOARD_IOCTL_SET_BASE_MAC_ADDRESS:
    {
        ...
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            if (NULL != (pNvramData = readNvramData()))
            {
                __copy_from_user((char*)pNvramData->ucaBaseMacAddr,ctrlParms.string, NVRAM_MAC_ADDRESS_LEN);
                __copy_from_user((char*)g_pMacInfo->ucaBaseMacAddr, ctrlParms.string, NVRAM_MAC_ADDRESS_LEN);
                writeNvramDataCrcLocked(pNvramData);
                ret = 0;
                kfree(pNvramData);
            }
    ...
    case BOARD_IOCTL_SET_SKU_NUMBER:
    {
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            NVRAM_DATA *pNvramData;
            if (NULL != (pNvramData = readNvramData()))
            {
                __copy_from_user((char*)pNvramData->szRegionNum, ctrlParms.string, 2);
                writeNvramDataCrcLocked(pNvramData);
                kfree(pNvramData);
            }
            else
    ...
    case BOARD_IOCTL_GET_SKU_NUMBER:
    {
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            NVRAM_DATA *pNvramData;
            if (NULL != (pNvramData = readNvramData()))
            {
                memcpy(ctrlParms.string, pNvramData->szRegionNum, 2);
                kfree(pNvramData);
            }
            else
                ret = -EFAULT;
    ...
    case BOARD_IOCTL_SET_DIS_TEST:
    {
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            ...
            if (NULL != (pNvramData = readNvramData()))
            {
                // Use ucUnused3 to be a switch
                __copy_from_user((char*)pNvramData->ucUnused3, ctrlParms.string, 2);
                writeNvramDataCrcLocked(pNvramData);
                kfree(pNvramData);
            }
            else
    ...
    case BOARD_IOCTL_GET_LOAD_DEFAULT:
    {
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            NVRAM_DATA *pNvramData;
            if (NULL != (pNvramData = readNvramData()))
            {
                // Use ucUnused2[0 ~ 1] to be a switch
                memcpy(ctrlParms.string, pNvramData->ucUnused2, 2);
                kfree(pNvramData);
            }
            else
                ret = -EFAULT;
    ...
    case BOARD_IOCTL_SET_LOAD_DEFAULT:
    {
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            NVRAM_DATA *pNvramData;
            ...
            {
                // Use ucUnused2[0 ~ 1] to be a switch
                __copy_from_user((char*)pNvramData->ucUnused2, ctrlParms.string, 2);
                writeNvramDataCrcLocked(pNvramData);
                kfree(pNvramData);
            }
            else
    ...
    case BOARD_IOCTL_SET_RECOVERY_TOKEN:
    {
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            NVRAM_DATA *pNvramData;
            if (NULL != (pNvramData = readNvramData()))
            {
                // Use ucUnused2[2] to be a switch
                __copy_from_user(((char*)pNvramData->ucUnused2) + 2, ctrlParms.string, 1);
                writeNvramDataCrcLocked(pNvramData);
                kfree(pNvramData);
            }
            else
    ...
    case BOARD_IOCTL_SET_GPIO_DIR:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            unsigned short gpio;
            gpio = (unsigned short) ctrlParms.strLen;
            gpio &= BP_GPIO_NUM_MASK;
    ...
    case BOARD_IOCTL_GET_GPIO:
            if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
                ctrlParms.result = kerSysGetGpioValue(ctrlParms.strLen);
                __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
                ret = 0;
            }
            else {
...
static int proc_get_boardid(char *page, char **start, off_t off, int cnt, int *eof, void *data)
{
    ...
    *eof = 1;
    ...
    strcpy(page, pNvramData->szFirmwareUpgradeBoardId);
    r = strlen(pNvramData->szFirmwareUpgradeBoardId);
    kfree(pNvramData);
    return r;
}
#endif
