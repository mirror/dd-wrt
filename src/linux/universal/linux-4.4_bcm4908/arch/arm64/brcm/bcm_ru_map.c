...
void remap_ru_block_addrs(uint32_t block_index, ru_block_rec *ru_blocks[])
{
    ...
    //go over all blocks
    for(blk = 0; ru_blocks[blk] != NULL; blk++  )
    {
        //go over every sub block
        for(addr_itr = 0; addr_itr < ru_blocks[blk]->addr_count; addr_itr++)
        {
            address = ru_blocks[blk]->addr[addr_itr];

            // new_address = old_address + block_virtual_base - block_physical_base
            ru_blocks[blk]->addr[addr_itr] += bcm_io_block_address[bcm_io_blocks[block_index].index]
                                                                   - bcm_io_blocks[block_index].address;
#ifdef DEBUG_RU_IOREMAP
             printk(KERN_INFO "       Remapped Block %s idx = %d Phys Address = 0x%lx to Virtual = 0x%lx\n",
                    ru_blocks[blk]->name, addr_itr, (long unsigned int) address, (long unsigned int) ru_blocks[blk]->addr[addr_itr]);
#endif
        }
    }
}
...
void remap_ru_block_single(uint32_t block_index, ru_block_rec *ru_block, uint32_t phys_base)
{
    ...
    {
        address = ru_block->addr[addr_itr];

        // new_address = old_address + block_virtual_base - block_physical_base
        ru_block->addr[addr_itr] += bcm_io_block_virt_base_get(block_index, phys_base);
#ifdef DEBUG_RU_IOREMAP
         printk(KERN_INFO "       Remapped Block %s idx = %d Phys Address = 0x%lx to Virtual = 0x%lx\n",
                ru_block->name, addr_itr, (long unsigned int) address, (long unsigned int) ru_block->addr[addr_itr]);
#endif
    }
}
