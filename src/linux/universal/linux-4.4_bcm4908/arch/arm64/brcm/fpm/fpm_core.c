...
#define FPM_POOL_SIZE_MAX	SZ_32M
#define FPM_BUFFER_SIZE_MAX	SZ_4K
#else
#define FPM_POOL_SIZE_MAX	SZ_16M
#define FPM_BUFFER_SIZE_MAX	SZ_2K
#endif
...
u32 fpm_alloc_max_size_token_pool(int pool)
{
	...
	token = __fpm_alloc_token_pool(pool, FPM_BUFFER_SIZE_MAX);
	fpm_check_token(token);
...
{
	struct fpmdev *fdev = pdev->dev.platform_data;
	int remaining_size, pool_idx;

	if (fdev->pool_size[0] < (FPM_POOL_SIZE_MAX * FPM_POOL_NUM)) {
		pr_err("FPM fails to initialize %d pool[s], because it requires "
		       "%d MB, and system has only reserved %d MB.\n  "
		       "Initialization will fail!\n", FPM_POOL_NUM, 
		       (FPM_POOL_SIZE_MAX >> 20) * FPM_POOL_NUM,
		       fdev->pool_size[0] >> 20);
		return -EFAULT;
	}

	remaining_size = fdev->pool_size[0] - (FPM_POOL_SIZE_MAX * FPM_POOL_NUM);
	fdev->pool_size[0] = FPM_POOL_SIZE_MAX;

	for (pool_idx = 1; pool_idx < FPM_POOL_NUM; pool_idx++) {
		fdev->pool_size[pool_idx] = FPM_POOL_SIZE_MAX;
		fdev->pool_vbase[pool_idx] = (u32 *)((uintptr_t)fdev->pool_vbase[0] +
					      (FPM_POOL_SIZE_MAX * pool_idx));
		fdev->pool_pbase[pool_idx] = (phys_addr_t)virt_to_phys(
						fdev->pool_vbase[pool_idx]);
	}

	pr_info("FPM will initialize %d pool[s] and require %d MB\n",
		FPM_POOL_NUM, (FPM_POOL_SIZE_MAX * FPM_POOL_NUM) >> 20);
