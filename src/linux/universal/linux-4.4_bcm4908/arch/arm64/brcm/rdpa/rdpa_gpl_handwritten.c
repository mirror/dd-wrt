...
const rdpa_if map_rdd_to_rdpa_if[] __attribute__((aligned(L1_CACHE_BYTES)))= 
...
const rdpa_if map_rdd_to_rdpa_if[] = {rdpa_if_wan0, rdpa_if_lan0, rdpa_if_lan1, rdpa_if_lan2,
...
void (*f_gpon_todd_set_tod_info) (const uint32_t* const sframe_num, const gpon_todd_tstamp_t* const tstamp_n);
EXPORT_SYMBOL(f_gpon_todd_set_tod_info);

void gpon_todd_set_tod_info (const uint32_t* const sframe_num, const gpon_todd_tstamp_t* const tstamp_n)
{
    if (!f_gpon_todd_get_tod_info)
        return;
    f_gpon_todd_set_tod_info(sframe_num, tstamp_n);
}
EXPORT_SYMBOL(gpon_todd_set_tod_info);
