                                 ...
                                 char               enRtpSeqCheck)
{
   ...
   do
   {
#if defined(CVONFIG_BR_IGMP_SNOOP)
      if ( BCM_MCAST_PROTO_IPV4 == proto )
      {
...
#endif
#if defined(CVONFIG_BR_MLD_SNOOP)
      if ( BCM_MCAST_PROTO_IPV6 == proto )
      {
...
static int bcm_mcast_fill_pkt_info(bcm_mcast_ifdata *pif, struct sk_buff *skb, void *pipvx, int proto, t_BCM_MCAST_PKT_INFO *pinfo, int flags)
{
   int queued = 0;
...
static int bcm_mcast_receive_igmp_filter(bcm_mcast_ifdata *pif, struct igmphdr *pigmp, int flags)
{
   if (pigmp != NULL)
   {
      ...
      if (pif->igmp_rate_limit)
      {
         ...
         curTime = ktime_get();
         usPerPacket = (1000000 / pif->igmp_rate_limit);
   ...
   if ( 0 == rv )
   {
...
#endif
      {
         ...
         if ( NULL != skb2 )
         {
            ...
            rv = bcm_mcast_fill_pkt_info(pif, skb_org, pip, BCM_MCAST_PROTO_IPV4, pinfo, flags);
...
static int bcm_mcast_receive_icmp(bcm_mcast_ifdata *pif, struct sk_buff *skb_org, int iph_offset, int icmpv6_offset, int flags)
{
   ...
   if ( 0 == rv )
   {
...
#endif
      {
         ...
         {
            pinfo = (t_BCM_MCAST_PKT_INFO *)skb2->data;
            rv = bcm_mcast_fill_pkt_info(pif, skb_org, pipv6, BCM_MCAST_PROTO_IPV6, pinfo, flags);
...
int bcm_mcast_should_deliver(int ifindex, const struct sk_buff *skb, struct net_device *dst_dev)
{
   unsigned int      is_multicast;
   unsigned int      iph_offset = 0;
   unsigned short    proto = 0;
   ...
   if ( is_multicast || (flags & BCM_MCAST_FLAG_PPP) )
   {
      ...
      do
      {
         ...
         if (proto == htons(ETH_P_IP) )
         {
            ...
            if (pip->protocol == IPPROTO_IGMP)
            {
               ...
               else
               {
                  ...
                  if ( pif->igmp_snooping && 
                      ...
                      ((NULL == destLowerPort) || (0 == destLowerPort->querying_port)) )
                  {
            ...
            {
               rv = bcm_mcast_igmp_should_deliver(pif, pip, skb->dev, dst_dev);
            }
            break;
         }
#endif      
