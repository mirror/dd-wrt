 ...
 *
 * $Id: dhd_linux.c 668759 2016-11-04 21:41:45Z $
 */
...
#if defined(BCM_BLOG) && defined(BCM_WFD)
static uint dhd_if_subunit_get(dhd_pub_t *dhdpub, dhd_if_t *ifp);
#endif /* BCM_BLOG && BCM_WFD */
		...
		if ((ifp->fwdh) && ((ntoh16(eh->ether_type) != ETHER_TYPE_BRCM))) {
			...
			if (ETHER_ISUCAST(da)) {
				...
				if (DHD_IF_ROLE_AP(dhdp, ifidx) && (!ifp->ap_isolate) &&
					...
					void *npkt;
					if ((npkt = PKTDUP(dhdp->osh, pktbuf)) != NULL)
			...
			if (ETHER_ISUCAST(eh->ether_dhost)) {
				if (dhd_find_sta(dhdp, ifidx, (void *)eh->ether_dhost)) {
					...
					if (PKT_DONE ==
						...
						DHD_INFO(("%s: handled inside blog_sinit\n",
								__FUNCTION__));
						continue;
					}
                    PKTCLRFCDONE(pktbuf);
...
#ifdef BCM_BLOG
        PKTSETFCDONE(skb);
		if (PKT_DONE == dhd_handle_blog_sinit(dhdp, ifidx, skb))
			continue;
        PKTCLRFCDONE(skb);
#endif /* BCM_BLOG */
...
#if defined(BCM_WFD)
	{
		uint subunit = dhd_if_subunit_get(dhdpub, ifp);
		DHD_INFO(("%s: ifidx=%d, bssidx=%d, dhdinfo->unit=%d, subunit=%d\n", __FUNCTION__,
			ifidx, bssidx, dhdinfo->unit, subunit));
		netdev_path_set_hw_port(ifp->net, subunit, BLOG_WLANPHY);
	}
#else
	netdev_path_set_hw_port(ifp->net, 0, BLOG_WLANPHY);
#endif /* BCM_WFD */
#endif /* BCM_BLOG */
...
#if defined(BCM_WFD)
		if (wfd_unregisterdevice(dhdinfo->pub.wfd_idx, ifidx) != 0)
			DHD_ERROR(("%s: wfd_unregisterdevice failed wfd_idx %d ifidx %d\n",
				__FUNCTION__, dhdinfo->pub.wfd_idx, ifidx));
		else
			DHD_INFO(("%s: wfd_unregisterdevice successful wfd_idx %d ifidx %d\n",
				__FUNCTION__, dhdinfo->pub.wfd_idx, ifidx));
#endif /* BCM_WFD */
	}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	/* do not increment pcie_probe_unit, yet */
...
#if defined(BCM_WFD)
	wfd_if_idx = netdev_path_get_hw_port(net);
	if (wfd_registerdevice(dhd->pub.wfd_idx, wfd_if_idx, net) != 0)
	{
	   DHD_ERROR(("wfd_registerdevice failed [%s]\n", net->name));
	   goto fail;
	}
#endif /* BCM_WFD */
...
dhd_wmf_fwder_sendup(void *wrapper, void *p)
{
	...
	if (ifp->fwdh && ifp->wmf.wmf_enable && (ETHER_ISMULTI(eh->ether_dhost))) {
		...
		if (DHD_IF_ROLE_AP(dhdp, ifidx) && (!ifp->ap_isolate)) {
			void *npkt;
			if ((npkt = PKTDUP(dhdp->osh, p)) != NULL)
				dhd_sendpkt(dhdp, ifidx, npkt);
		}
...
#if defined(BCM_BLOG) && defined(BCM_WFD)
static uint
dhd_if_subunit_get(dhd_pub_t *dhdpub, dhd_if_t *ifp)
{
	uint subunit = ifp->subunit;
	return subunit;
}
#endif /* BCM_BLOG && BCM_WFD */
...
{
	DHD_INFO(("%s: rxp:%lu,txp:%lu,rxb:%llu,txb:%llu,mc:%lu\r\n", __FUNCTION__,
		blogStats_p->rx_packets, blogStats_p->tx_packets, blogStats_p->rx_bytes,
		blogStats_p->tx_bytes, blogStats_p->multicast));
