 ...
 *
 * $Id: dhd_l2_filter.c 604476 2015-12-07 13:46:20Z $
 *
 */
#include <bcmutils.h>
#include <bcmendian.h>
	...
	uint8 *reply_to_bss, void *pktbuf, bool istx)
{
	...
	if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NA ||
	    ...
	    if (entry == NULL) {
		DHD_INFO(("dhd%d: Add new parp_entry by ICMP6 %s %s\n",
		    ifidx, bcm_ether_ntoa(entry_ea, eabuf),
		    bcm_ipv6_ntoa((void *)entry_ip, ipbuf)));
		bcm_l2_filter_parp_addentry(pub->osh, ptable, entry_ea, entry_ip, IP_VER_6, TRUE,
			time_sec);
	    }
	} else {
		...
		if (entry == NULL)
			entry = bcm_l2_filter_parp_findentry(ptable, entry_ip, IP_VER_6,
				FALSE, time_sec);
		if (entry == NULL) {
			DHD_INFO(("dhd%d: create candidate parp_entry by ICMP6 %s %s\n",
	...
	if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NA) {
		...
		struct ether_addr ea;
		bcopy(&entry->ea, &ea, ETHER_ADDR_LEN);
		DHD_INFO(("dhd%d: withdraw candidate parp_entry IPv6 %s %s\n",
...
dhd_handle_proxyarp_dhcp4(dhd_pub_t *pub, int ifidx, frame_proto_t *fp, bool istx)
{
	...
	if (istx) {
		...
		if (bcmp((void*)cmac_addr, &dhcp[DHCP_CHADDR_OFFSET], ETHER_ADDR_LEN)) {
			...
			DHD_INFO(("dhd%d: Unmatch DHCP Req Client MAC (%s)",
				ifidx, eabuf));
