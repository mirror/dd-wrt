					   ...
					   const struct nf_hook_state *state)
{
	...
	BR_HOOK(NFPROTO_IPV6, NF_INET_PRE_ROUTING, state->sk, skb,
		skb->dev, NULL,
				      ...
				      const struct nf_hook_state *state)
{
	...
	BR_HOOK(NFPROTO_IPV4, NF_INET_PRE_ROUTING, state->sk, skb,
		skb->dev, NULL,
				     ...
				     const struct nf_hook_state *state)
{
	...
	BR_HOOK(pf, NF_INET_FORWARD, NULL, skb,
				      ...
				      const struct nf_hook_state *state)
{
	...
	*d = state->in;
	BR_HOOK(NFPROTO_ARP, NF_ARP_FORWARD, state->sk, skb,
				       ...
				       const struct nf_hook_state *state)
{
	...
	BR_HOOK(pf, NF_INET_POST_ROUTING, state->sk, skb,
		NULL, realoutdev,
