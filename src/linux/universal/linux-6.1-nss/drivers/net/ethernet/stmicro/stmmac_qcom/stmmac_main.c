 ...
 */
static int stmmac_tx_clean(struct stmmac_priv *priv, u32 queue)
{
	struct stmmac_tx_queue *tx_q = &priv->dma_conf.tx_queue[queue];
	...
	if (unlikely(netif_tx_queue_stopped(netdev_get_tx_queue(priv->dev,
		...
		if (netif_tx_queue_stopped(netdev_get_tx_queue(priv->dev,queue)) && stmmac_tx_avail(priv, queue) > STMMAC_TX_THRESH(priv)) {
			netif_info(priv, tx_done, priv->dev, "%s: restart transmit\n", __func__);
...
static int stmmac_napi_check(struct stmmac_priv *priv, u32 chan, u32 dir)
{
	...
	if (((status & handle_rx) || (status & handle_tx)) && (chan < priv->plat->rx_queues_to_use)) {
