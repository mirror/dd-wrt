...
static void gcu_probe_err(enum gcu_err_type err, struct pci_dev *pdev, struct gcu_adapter *adapter);
static void gcu_remove(struct pci_dev *pdev);
static int gcu_notify_reboot(struct notifier_block *, unsigned long event, void *ptr);
...
static int gcu_notify_reboot(struct notifier_block *nb, unsigned long event, void *p)
{
