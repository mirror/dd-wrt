...
static int iegbe_setup_tx_resources(struct iegbe_adapter *adapter, struct iegbe_tx_ring *txdr);
static int iegbe_setup_rx_resources(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rxdr);
static void iegbe_free_tx_resources(struct iegbe_adapter *adapter, struct iegbe_tx_ring *tx_ring);
static void iegbe_free_rx_resources(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring);
...
static void iegbe_clean_tx_ring(struct iegbe_adapter *adapter, struct iegbe_tx_ring *tx_ring);
static void iegbe_clean_rx_ring(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring);
...
static bool iegbe_clean_tx_irq(struct iegbe_adapter *adapter, struct iegbe_tx_ring *tx_ring);
static int iegbe_clean(struct napi_struct *napi, int budget);
static bool iegbe_clean_rx_irq(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring, int *work_done, int work_to_do);
static bool iegbe_clean_rx_irq_ps(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring, int *work_done, int work_to_do);

static void iegbe_alloc_rx_buffers(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring, int cleaned_count);
static void iegbe_alloc_rx_buffers_ps(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring, int cleaned_count);

static int iegbe_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd);
static int iegbe_mii_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd);
...
static inline int iegbe_82547_fifo_workaround(struct iegbe_adapter *adapter, struct sk_buff *skb);
...
static int iegbe_notify_reboot(struct notifier_block *, unsigned long event, void *ptr);
...
static int __init iegbe_init_module(void)
{
	...
	if (copybreak != COPYBREAK_DEFAULT) {
		if (copybreak == 0)
			...
			printk(KERN_INFO "iegbe: copybreak enabled for " "packets <= %u bytes\n", copybreak);
	}
	return ret;
}
...
static inline void iegbe_irq_enable(struct iegbe_adapter *adapter)
{
...
static void iegbe_power_down_phy(struct iegbe_adapter *adapter)
{
	...
	if (!adapter->wol && hw->mac_type >= iegbe_82540 && hw->media_type == iegbe_media_type_copper) {
		u16 mii_reg = 0;
		...
		case iegbe_82572:
		case iegbe_82573:
			if (iegbe_check_mng_mode(hw) || iegbe_check_phy_reset_block(hw))
				goto out;
			break;
		default:
			goto out;
...
void iegbe_reset(struct iegbe_adapter *adapter)
{
	...
	if (legacy_pba_adjust) {
		...
		min_rx_space = adapter->netdev->mtu + ENET_HEADER_SIZE + VLAN_TAG_SIZE;
	...
	if (hw->mac_type >= iegbe_82544 && hw->mac_type <= iegbe_82547_rev_2 && hw->autoneg == 1 && hw->autoneg_advertised == ADVERTISE_1000_FULL) {
	...
	if (!adapter->smart_power_down && (hw->mac_type == iegbe_82571 || hw->mac_type == iegbe_82572)) {
		u16 phy_data = 0;
		/* speed up time to link by disabling smart power down, ignore
		 * the return value of this function because there is nothing
		 * different we would do if it failed */
		iegbe_read_phy_reg(hw, IGP02E1000_PHY_POWER_MGMT, &phy_data);
		phy_data &= ~IGP02E1000_PM_SPD;
		iegbe_write_phy_reg(hw, IGP02E1000_PHY_POWER_MGMT, phy_data);
	}
...
static void iegbe_dump_eeprom(struct iegbe_adapter *adapter)
{
	...
	if (!data) {
		printk(KERN_ERR "Unable to allocate memory to dump EEPROM" " data\n");
		return;
	}
	...
	printk(KERN_ERR "Include this output when contacting your support " "provider.\n");
...
static int iegbe_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	err = -EIO;
	hw->hw_addr = ioremap(pci_resource_start(pdev, BAR_0), pci_resource_len(pdev, BAR_0));
	if (!hw->hw_addr)
		goto err_ioremap;
	...
	if ((hw->mac_type >= iegbe_82544) && (hw->mac_type != iegbe_82547))
	...
	case iegbe_82543:
		break;
	case iegbe_82544:
		iegbe_read_eeprom(&adapter->hw, EEPROM_INIT_CONTROL2_REG, 1, &eeprom_data);
		eeprom_apme_mask = E1000_EEPROM_82544_APM;
		break;
	case iegbe_icp_xxxx:
		iegbe_read_eeprom(&adapter->hw, EEPROM_INIT_CONTROL3_ICP_xxxx(adapter->bd_number), 1, &eeprom_data);
	...
	case iegbe_82546_rev_3:
		if ((E1000_READ_REG(&adapter->hw, STATUS) & E1000_STATUS_FUNC_1)
			...
			iegbe_read_eeprom(&adapter->hw, EEPROM_INIT_CONTROL3_PORT_B, 1, &eeprom_data);
			break;
		}
		/* Fall Through */
	default:
		iegbe_read_eeprom(&adapter->hw, EEPROM_INIT_CONTROL3_PORT_A, 1, &eeprom_data);
		break;
	}
...
static void iegbe_remove(struct pci_dev *pdev)
{
	...
	if (adapter->hw.mac_type >= iegbe_82540 && adapter->hw.mac_type != iegbe_icp_xxxx && adapter->hw.media_type == iegbe_media_type_copper) {
...
static int iegbe_sw_init(struct iegbe_adapter *adapter)
{
	...
	if (hw->media_type == iegbe_media_type_copper || (hw->media_type == iegbe_media_type_oem && iegbe_oem_phy_is_copper(&adapter->hw))) {
...
static int iegbe_alloc_queues(struct iegbe_adapter *adapter)
{
	...
	adapter->tx_ring = kcalloc(adapter->num_tx_queues, sizeof(struct iegbe_tx_ring), GFP_KERNEL);
	if (!adapter->tx_ring)
		return -ENOMEM;

	adapter->rx_ring = kcalloc(adapter->num_rx_queues, sizeof(struct iegbe_rx_ring), GFP_KERNEL);
	if (!adapter->rx_ring) {
		kfree(adapter->tx_ring);
		return -ENOMEM;
	}
...
static int iegbe_close(struct net_device *netdev)
{
	...
	if ((hw->mng_cookie.status & E1000_MNG_DHCP_COOKIE_STATUS_VLAN_SUPPORT) && !test_bit(adapter->mng_vlan_id, adapter->active_vlans)) {
 ...
 **/
static inline boolean_t iegbe_check_64k_bound(struct iegbe_adapter *adapter, void *start, unsigned long len)
{
	...
	if (adapter->hw.mac_type == iegbe_82545 || adapter->hw.mac_type == iegbe_82546) {
...
static int iegbe_setup_tx_resources(struct iegbe_adapter *adapter, struct iegbe_tx_ring *txdr)
{
	...
	if (!iegbe_check_64k_bound(adapter, txdr->desc, txdr->size)) {
		...
		if (!iegbe_check_64k_bound(adapter, txdr->desc, txdr->size)) {
			...
			pci_free_consistent(pdev, txdr->size, olddesc, olddma);
			DPRINTK(PROBE, ERR, "Unable to allocate aligned memory " "for the transmit descriptor ring\n");
			vfree(txdr->buffer_info);
			return -ENOMEM;
		} else {
...
static void iegbe_configure_tx(struct iegbe_adapter *adapter)
{
	...
	case 0x2:
		...
		tdlen = adapter->tx_ring[0x1].count * sizeof(struct iegbe_tx_desc);
	...
	default:
		...
		tdlen = adapter->tx_ring[0x0].count * sizeof(struct iegbe_tx_desc);
	...
	default:
		...
		default:
			tipg = (0xFFFFFFFFUL >> (sizeof(tipg) * 0x8 - E1000_TIPG_IPGR1_SHIFT))
	...
	tctl &= ~E1000_TCTL_CT;
	tctl |= E1000_TCTL_EN | E1000_TCTL_PSP | E1000_TCTL_RTLC | (E1000_COLLISION_THRESHOLD << E1000_CT_SHIFT);
	...
	adapter->txd_cmd = E1000_TXD_CMD_IDE | E1000_TXD_CMD_EOP | E1000_TXD_CMD_IFCS;
	...
	if (hw->mac_type == iegbe_82544 && hw->bus_type == iegbe_bus_type_pcix) {
...
static int iegbe_setup_rx_resources(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rxdr)
{
	...
	rxdr->ps_page = kcalloc(rxdr->count, sizeof(struct iegbe_ps_page), GFP_KERNEL);
	if (!rxdr->ps_page) {
	...
	rxdr->ps_page_dma = kcalloc(rxdr->count, sizeof(struct iegbe_ps_page_dma), GFP_KERNEL);
	if (!rxdr->ps_page_dma) {
		vfree(rxdr->buffer_info);
		kfree(rxdr->ps_page);
		DPRINTK(PROBE, ERR, "Unable to allocate memory for the receive descriptor ring\n");
	...
	if (!iegbe_check_64k_bound(adapter, rxdr->desc, rxdr->size)) {
		...
		DPRINTK(RX_ERR, ERR, "rxdr align check failed: %u bytes " "at %p\n", rxdr->size, rxdr->desc);
		/* Try again, without freeing the previous */
		rxdr->desc = pci_alloc_consistent(pdev, rxdr->size, &rxdr->dma);
		/* Failed allocation, critical failure */
		if (!rxdr->desc) {
...
static void iegbe_setup_rctl(struct iegbe_adapter *adapter)
{
	...
	rctl |= E1000_RCTL_EN | E1000_RCTL_BAM | E1000_RCTL_LBM_NO | E1000_RCTL_RDMTS_HALF | (hw->mc_filter_type << E1000_RCTL_MO_SHIFT);
	...
	if ((hw->mac_type >= iegbe_82571) && (pages <= 3) && PAGE_SIZE <= 16384 && (rctl & E1000_RCTL_LPE))
...
static void iegbe_configure_rx(struct iegbe_adapter *adapter)
{
	...
	if (adapter->rx_ps_pages) {
		rdlen = adapter->rx_ring[0].count * sizeof(union iegbe_rx_desc_packet_split);
		adapter->clean_rx = iegbe_clean_rx_irq_ps;
		adapter->alloc_rx_buf = iegbe_alloc_rx_buffers_ps;
	} else {
		rdlen = adapter->rx_ring[0].count * sizeof(struct iegbe_rx_desc);
	...
	if (hw->mac_type >= iegbe_82543) {
		...
		if (adapter->rx_csum == TRUE) {
			...
			if ((hw->mac_type >= iegbe_82571) && (adapter->rx_ps_pages)) {
...
static inline void iegbe_unmap_and_free_tx_resource(struct iegbe_adapter *adapter, struct iegbe_buffer *buffer_info)
{
	if (buffer_info->dma) {
	...
	for (i = 0; i < rx_ring->count; i++) {
		...
		for (j = 0; j < adapter->rx_ps_pages; j++) {
			if (!ps_page->ps_page[j])
				break;
			pci_unmap_page(pdev, ps_page_dma->ps_page_dma[j], PAGE_SIZE, PCI_DMA_FROMDEVICE);
...
static int iegbe_set_mac(struct net_device *netdev, void *p)
{
	...
	if (adapter->hw.mac_type == iegbe_82571) {
		 ...
		 * RAR[14] */
		iegbe_rar_set(&adapter->hw, adapter->hw.mac_addr, E1000_RAR_ENTRIES - 0x1);
	}
...
static void iegbe_82547_tx_fifo_stall(unsigned long data)
{
	...
	if (atomic_read(&adapter->tx_fifo_stall)) {
		if ((E1000_READ_REG(&adapter->hw, TDT) ==
		     E1000_READ_REG(&adapter->hw, TDH)) && (E1000_READ_REG(&adapter->hw, TDFT) == E1000_READ_REG(&adapter->hw, TDFH)) && (E1000_READ_REG(&adapter->hw, TDFTS) == E1000_READ_REG(&adapter->hw, TDFHS))) {
...
static void iegbe_watchdog(unsigned long data)
{
	...
	if ((adapter->hw.media_type == iegbe_media_type_internal_serdes) && !(E1000_READ_REG(&adapter->hw, TXCW) & E1000_TXCW_ANE)) {
	...
	if (link) {
		if (!netif_carrier_ok(netdev)) {
			u32 ctrl;
			bool txb2b = true;
			iegbe_get_speed_and_duplex(hw, &adapter->link_speed, &adapter->link_duplex);
			...
			DPRINTK(LINK, INFO, "NIC Link is Up %d Mbps %s, "
				...
				"Full Duplex" : "Half Duplex", ((ctrl & E1000_CTRL_TFCE) && (ctrl & E1000_CTRL_RFCE)) ? "RX/TX" : ((ctrl & E1000_CTRL_RFCE) ? "RX" : ((ctrl & E1000_CTRL_TFCE) ? "TX" : "None")));
			...
			if ((hw->mac_type == iegbe_82571 || hw->mac_type == iegbe_82572) && !txb2b) {
				u32 tarc0;
			...
			if (!adapter->tso_force && hw->bus_type == iegbe_bus_type_pci_express) {
				switch (adapter->link_speed) {
				case SPEED_10:
				case SPEED_100:
					DPRINTK(PROBE, INFO, "10/100 speed: disabling TSO\n");
					netdev->features &= ~NETIF_F_TSO;
					netdev->features &= ~NETIF_F_TSO6;
					break;
				case SPEED_1000:
 ...
 **/
static unsigned int iegbe_update_itr(struct iegbe_adapter *adapter, u16 itr_setting, int packets, int bytes)
{
...
static void iegbe_set_itr(struct iegbe_adapter *adapter)
{
	...
	adapter->tx_itr = iegbe_update_itr(adapter, adapter->tx_itr, adapter->total_tx_packets, adapter->total_tx_bytes);
	/* conservative mode (itr 3) eliminates the lowest_latency setting */
	if (adapter->itr_setting == 3 && adapter->tx_itr == lowest_latency)
		adapter->tx_itr = low_latency;

	adapter->rx_itr = iegbe_update_itr(adapter, adapter->rx_itr, adapter->total_rx_packets, adapter->total_rx_bytes);
	/* conservative mode (itr 3) eliminates the lowest_latency setting */
...
static int iegbe_tso(struct iegbe_adapter *adapter, struct iegbe_tx_ring *tx_ring, struct sk_buff *skb)
{
...
static bool iegbe_tx_csum(struct iegbe_adapter *adapter, struct iegbe_tx_ring *tx_ring, struct sk_buff *skb)
{
	struct iegbe_context_desc *context_desc;
	struct iegbe_buffer *buffer_info;
	unsigned int i;
...
static int iegbe_tx_map(struct iegbe_adapter *adapter, struct iegbe_tx_ring *tx_ring, struct sk_buff *skb, unsigned int first, unsigned int max_per_txd, unsigned int nr_frags, unsigned int mss)
{
	struct iegbe_hw *hw = &adapter->hw;
	struct iegbe_buffer *buffer_info;
	...
	while (len) {
		 ...
		 */
		if (unlikely((hw->bus_type == iegbe_bus_type_pcix) && (size > 2015) && count == 0))
			size = 2015;
		...
		buffer_info->dma = pci_map_single(adapter->pdev, skb->data + offset, size, PCI_DMA_TODEVICE);
		buffer_info->time_stamp = jiffies;
		buffer_info->next_to_watch = i;
	...
	for (f = 0; f < nr_frags; f++) {
		...
		while (len) {
			 ...
			 * dwords. */
			if (unlikely(adapter->pcix_82544 && !((unsigned long)(frag->page.p + offset + size - 1) & 4) && size > 4))
				size -= 4;

			buffer_info->length = size;
			buffer_info->dma = pci_map_page(adapter->pdev, frag->page.p, offset, size, PCI_DMA_TODEVICE);
			buffer_info->time_stamp = jiffies;
			buffer_info->next_to_watch = i;
...
static void iegbe_tx_queue(struct iegbe_adapter *adapter, struct iegbe_tx_ring *tx_ring, int tx_flags, int count)
{
	struct iegbe_hw *hw = &adapter->hw;
...
static int iegbe_82547_fifo_workaround(struct iegbe_adapter *adapter, struct sk_buff *skb)
{
...
static int iegbe_transfer_dhcp_info(struct iegbe_adapter *adapter, struct sk_buff *skb)
{
	struct iegbe_hw *hw = &adapter->hw;
	u16 length, offset;
	if (skb_vlan_tag_present(skb)) {
		if (!((skb_vlan_tag_get(skb) == hw->mng_cookie.vlan_id) && (hw->mng_cookie.status & E1000_MNG_DHCP_COOKIE_STATUS_VLAN_SUPPORT)))
			return 0;
	}
		...
		if ((htons(ETH_P_IP) == eth->h_proto)) {
			...
			if (IPPROTO_UDP == ip->protocol) {
				...
				if (ntohs(udp->dest) == 67) {
					...
					return iegbe_mng_write_dhcp_info(hw, (u8 *)udp + 8, length);
				}
			}
		}
	}
...
static int iegbe_maybe_stop_tx(struct net_device *netdev, struct iegbe_tx_ring *tx_ring, int size)
{
	if (likely(E1000_DESC_UNUSED(tx_ring) >= size))
		return 0;
...
static int iegbe_xmit_frame(struct sk_buff *skb, struct net_device *netdev)
{
	 ...
	 */
	if (unlikely((hw->bus_type == iegbe_bus_type_pcix) && (len > 2015)))
		count++;
	...
	if (hw->tx_pkt_filtering && (hw->mac_type == iegbe_82573))
	...
	iegbe_tx_queue(adapter, tx_ring, tx_flags, iegbe_tx_map(adapter, tx_ring, skb, first, max_per_txd, nr_frags, mss));
...
static void iegbe_reset_task(struct work_struct *work)
{
	struct iegbe_adapter *adapter = container_of(work, struct iegbe_adapter, reset_task);
...
#define PHY_IDLE_ERROR_COUNT_MASK 0x00FF
	...
	adapter->net_stats.rx_errors = adapter->stats.rxerrc + adapter->stats.crcerrs + adapter->stats.algnerrc + adapter->stats.rlec + adapter->stats.mpc + adapter->stats.cexterr;
	...
	if (hw->media_type == iegbe_media_type_copper || (hw->media_type == iegbe_media_type_oem && iegbe_oem_phy_is_copper(&adapter->hw))) {
		if ((adapter->link_speed == SPEED_1000) && (!iegbe_read_phy_reg(hw, PHY_1000T_STATUS, &phy_tmp))) {
			phy_tmp &= PHY_IDLE_ERROR_COUNT_MASK;
			adapter->phy_stats.idle_errors += phy_tmp;
		}

		if ((hw->mac_type <= iegbe_82546) && (hw->phy_type == iegbe_phy_m88) && !iegbe_read_phy_reg(hw, M88E1000_RX_ERR_CNTR, &phy_tmp)) {
...
static irqreturn_t iegbe_intr_msi(int irq, void *data)
{
	...
	if (unlikely(icr & (E1000_ICR_RX_DESC_FIFO_PAR | E1000_ICR_TX_DESC_FIFO_PAR | E1000_ICR_PB | E1000_ICR_CPP_TARGET | E1000_ICR_CPP_MASTER))) {
...
static irqreturn_t iegbe_intr(int irq, void *data)
{
	...
	if (unlikely(hw->mac_type >= iegbe_82571 && !(icr & E1000_ICR_INT_ASSERTED)))
		return IRQ_NONE;
...
static int iegbe_clean(struct napi_struct *napi, int budget)
{
	...
	if (spin_trylock(&adapter->tx_queue_lock)) {
		tx_cleaned = iegbe_clean_tx_irq(adapter, &adapter->tx_ring[0]);
...
#define TX_WAKE_THRESHOLD 32
	...
	if (adapter->detect_tx_hung) {
		...
		if (tx_ring->buffer_info[eop].dma && time_after(jiffies, tx_ring->buffer_info[eop].time_stamp + (adapter->tx_timeout_factor * HZ))
			...
			DPRINTK(DRV, ERR, "Detected Tx Unit Hang\n"
				...
				readl(hw->hw_addr + tx_ring->tdt), tx_ring->next_to_use, tx_ring->next_to_clean, tx_ring->buffer_info[eop].time_stamp, eop, jiffies, eop_desc->upper.fields.status);
			netif_stop_queue(netdev);
		}
	}
...
static void iegbe_rx_checksum(struct iegbe_adapter *adapter, u32 status_err, u32 csum, struct sk_buff *skb)
{
	struct iegbe_hw *hw = &adapter->hw;
	...
	/* 82543 or newer only */
	if (unlikely(hw->mac_type < iegbe_82543))
		return;
	...
	while (rx_desc->status & E1000_RXD_STAT_DD) {
		...
		if (unlikely(rx_desc->errors & E1000_RXD_ERR_FRAME_ERR_MASK)) {
			...
			if (TBI_ACCEPT(hw, status, rx_desc->errors, length, last_byte)) {
				spin_lock_irqsave(&adapter->stats_lock, flags);
				iegbe_tbi_adjust_stats(hw, &adapter->stats, length, skb->data);
				spin_unlock_irqrestore(&adapter->stats_lock, flags);
				length--;
			} else {
				buffer_info->skb = skb;
		...
		iegbe_rx_checksum(adapter, (u32)(status) | ((u32)(rx_desc->errors) << 24), le16_to_cpu(rx_desc->csum), skb);

		skb->protocol = eth_type_trans(skb, netdev);

		if (unlikely(iegbe_vlan_used(adapter) && (status & E1000_RXD_STAT_VP))) {
			u16 vid;
...
static bool iegbe_clean_rx_irq_ps(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring, int *work_done, int work_to_do)
{
	...
	while (staterr & E1000_RXD_STAT_DD) {
		...
		if (unlikely(!(staterr & E1000_RXD_STAT_EOP))) {
			E1000_DBG("%s: Packet Split buffers didn't pick up" " the full packet\n", netdev->name);
		...
		if (unlikely(!length)) {
			E1000_DBG("%s: Last part of the packet spanning" " multiple descriptors\n", netdev->name);
			dev_kfree_skb_irq(skb);
			goto next_desc;
		}
...

		{
			...
			if (l1 && (l1 <= copybreak) && ((length + l1) <= adapter->rx_ps_bsize0)) {
				u8 *vaddr;
				pci_dma_sync_single_for_cpu(pdev, ps_page_dma->ps_page_dma[0], PAGE_SIZE, PCI_DMA_FROMDEVICE);
				vaddr = kmap_atomic(ps_page->ps_page[0]);
				memcpy(skb_tail_pointer(skb), vaddr, l1);
				kunmap_atomic(vaddr);
				pci_dma_sync_single_for_device(pdev, ps_page_dma->ps_page_dma[0], PAGE_SIZE, PCI_DMA_FROMDEVICE);
				l1 -= 4;
				skb_put(skb, l1);
				goto copydone;
			}	/* if */
		...
		total_rx_packets++;
		iegbe_rx_checksum(adapter, staterr, le16_to_cpu(rx_desc->wb.lower.hi_dword.csum_ip.csum), skb);
		skb->protocol = eth_type_trans(skb, netdev);

		if (likely(rx_desc->wb.upper.header_status & cpu_to_le16(E1000_RXDPS_HDRSTAT_HDRSP)))
...
static void iegbe_alloc_rx_buffers(struct iegbe_adapter *adapter, struct iegbe_rx_ring *rx_ring, int cleaned_count)
{
	...
	while (cleaned_count--) {
		...
		/* Fix for errata 23, can't cross 64kB boundary */
		if (!iegbe_check_64k_bound(adapter, (void *)(unsigned long)buffer_info->dma, adapter->rx_buffer_len)) {
			DPRINTK(RX_ERR, ERR, "dma align check failed: %u bytes at %p\n", adapter->rx_buffer_len, (void *)(unsigned long)buffer_info->dma);
			dev_kfree_skb(skb);
			buffer_info->skb = NULL;
		...
		for (j = 0; j < PS_PAGE_BUFFERS; j++) {
			if (j < adapter->rx_ps_pages) {
				if (likely(!ps_page->ps_page[j])) {
					ps_page->ps_page[j] = alloc_page(GFP_ATOMIC);
					if (unlikely(!ps_page->ps_page[j])) {
						adapter->alloc_rx_buff_failed++;
						goto no_buffers;
					}
					ps_page_dma->ps_page_dma[j] = pci_map_page(pdev, ps_page->ps_page[j], 0, PAGE_SIZE, PCI_DMA_FROMDEVICE);
				}
				/* Refresh the desc even if buffer_addrs didn't
				 * change because each write-back erases
				 * this info.
				 */
				rx_desc->read.buffer_addr[j + 1] = cpu_to_le64(ps_page_dma->ps_page_dma[j]);
			} else
				rx_desc->read.buffer_addr[j + 1] = ~cpu_to_le64(0);
		}

		skb = netdev_alloc_skb(netdev, adapter->rx_ps_bsize0 + NET_IP_ALIGN);
...
static void iegbe_smartspeed(struct iegbe_adapter *adapter)
{
	...
	if ((adapter->hw.phy_type != iegbe_phy_igp) || !adapter->hw.autoneg || !(adapter->hw.autoneg_advertised & ADVERTISE_1000_FULL)) {
		return;
	}
	if (adapter->smartspeed == 0x0) {
		...
		if (phy_ctrl & CR_1000T_MS_ENABLE) {
			...
			adapter->smartspeed++;
			if (!iegbe_phy_setup_autoneg(&adapter->hw) && !iegbe_read_phy_reg(&adapter->hw, PHY_CTRL, &phy_ctrl)) {
...
static int iegbe_mii_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	...
	if ((adapter->hw.media_type == iegbe_media_type_oem && !iegbe_oem_phy_is_copper(&adapter->hw))
	    || adapter->hw.media_type != iegbe_media_type_copper) {
		return -EOPNOTSUPP;
	}
	switch (cmd) {
	...
	case SIOCGMIIREG:
		...
		spin_lock_irqsave(&adapter->stats_lock, flags);
		if (iegbe_read_phy_reg(&adapter->hw, data->reg_num & 0x1F, &data->val_out)) {
			spin_unlock_irqrestore(&adapter->stats_lock, flags);
			return -EIO;
		}
		spin_unlock_irqrestore(&adapter->stats_lock, flags);
	...
	case SIOCSMIIREG:
		...
		mii_reg = data->val_in;
		spin_lock_irqsave(&adapter->stats_lock, flags);
		if (iegbe_write_phy_reg(&adapter->hw, data->reg_num, mii_reg)) {
			spin_unlock_irqrestore(&adapter->stats_lock, flags);
			return -EIO;
		}
...
static int iegbe_vlan_rx_add_vid(struct net_device *netdev, __be16 proto, u16 vid)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	uint32_t vfta, index;
	if ((adapter->hw.mng_cookie.status & E1000_MNG_DHCP_COOKIE_STATUS_VLAN_SUPPORT) && (vid == adapter->mng_vlan_id)) {
		return 0;
	}
...
int iegbe_set_spd_dplx(struct iegbe_adapter *adapter, u16 spddplx)
{
	...
	if ((adapter->hw.media_type == iegbe_media_type_fiber || (adapter->hw.media_type == iegbe_media_type_oem && !iegbe_oem_phy_is_copper(&adapter->hw)))
...
static int iegbe_notify_reboot(struct notifier_block *nb, unsigned long event, void *p)
{
...
#define E1000_CTRL_EN_PHY_PWR_MGMT 0x00200000
			ctrl |= E1000_CTRL_ADVD3WUC | (adapter->hw.mac_type != iegbe_icp_xxxx ? E1000_CTRL_EN_PHY_PWR_MGMT : 0x0);

			E1000_WRITE_REG(&adapter->hw, CTRL, ctrl);
		}

		if (adapter->hw.media_type == iegbe_media_type_fiber || adapter->hw.media_type == iegbe_media_type_internal_serdes) {
	...
	case iegbe_82572:
		ctrl_ext = E1000_READ_REG(&adapter->hw, CTRL_EXT);
		E1000_WRITE_REG(&adapter->hw, CTRL_EXT, ctrl_ext & ~E1000_CTRL_EXT_DRV_LOAD);
		break;
	case iegbe_82573:
