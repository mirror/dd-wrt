...
static int iegbe_get_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;

	if ((hw->media_type == iegbe_media_type_copper) || (hw->media_type == iegbe_media_type_oem)) {

		ecmd->supported = (SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full | SUPPORTED_100baseT_Half | SUPPORTED_100baseT_Full | SUPPORTED_1000baseT_Full | SUPPORTED_Autoneg | SUPPORTED_TP);
	...
	ecmd->autoneg = ((hw->media_type == iegbe_media_type_fiber) || hw->autoneg) ? AUTONEG_ENABLE : AUTONEG_DISABLE;
	return 0;
}

static int iegbe_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
	...
	if (ecmd->autoneg == AUTONEG_ENABLE) {
		...
		if (hw->media_type == iegbe_media_type_fiber) {
			...
			hw->autoneg_advertised = ADVERTISED_10baseT_Half | ADVERTISED_10baseT_Full | ADVERTISED_100baseT_Half | ADVERTISED_100baseT_Full | ADVERTISED_1000baseT_Full | ADVERTISED_Autoneg | ADVERTISED_TP;
			ecmd->advertising = hw->autoneg_advertised;
		}
	} else if (iegbe_set_spd_dplx(adapter, ecmd->speed + ecmd->duplex)) {
		return -EINVAL;
	}
	/* reset the link */
...
static void iegbe_get_pauseparam(struct net_device *netdev, struct ethtool_pauseparam *pause)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
...
static int iegbe_set_pauseparam(struct net_device *netdev, struct ethtool_pauseparam *pause)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
	int ret_val;
	...
	if (adapter->fc_autoneg == AUTONEG_ENABLE) {
		...
		ret_val = ((hw->media_type == iegbe_media_type_fiber)
			   || (hw->media_type == iegbe_media_type_oem && !iegbe_oem_phy_is_copper(&adapter->hw)) ? iegbe_setup_link(hw) : iegbe_force_mac_fc(hw));
		return ret_val;
	}
...
static uint32_t iegbe_get_rx_csum(struct net_device *netdev)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	return adapter->rx_csum;
}

static int iegbe_set_rx_csum(struct net_device *netdev, uint32_t data)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
...
static uint32_t iegbe_get_tx_csum(struct net_device *netdev)
{
	return (netdev->features & NETIF_F_HW_CSUM) != 0;
}

static int iegbe_set_tx_csum(struct net_device *netdev, uint32_t data)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
...
#ifdef NETIF_F_TSO
static int iegbe_set_tso(struct net_device *netdev, uint32_t data)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	if ((adapter->hw.mac_type < iegbe_82544) || (adapter->hw.mac_type == iegbe_82547)) {
...
static uint32_t iegbe_get_msglevel(struct net_device *netdev)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	return adapter->msg_enable;
}

static void iegbe_set_msglevel(struct net_device *netdev, uint32_t data)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	adapter->msg_enable = data;
}

static int iegbe_get_regs_len(struct net_device *netdev)
{
	return E1000_REGS_LEN * sizeof(uint32_t);
}

static void iegbe_get_regs(struct net_device *netdev, struct ethtool_regs *regs, void *p)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
	...
	case iegbe_phy_igp:
		...
		iegbe_write_phy_reg(hw, IGP01E1000_PHY_PAGE_SELECT, IGP01E1000_PHY_AGC_A);
		iegbe_read_phy_reg(hw, IGP01E1000_PHY_AGC_A & IGP01E1000_PHY_PAGE_SELECT, &phy_data);
		regs_buff[0xd] = (uint32_t) phy_data;	/* cable length */
		iegbe_write_phy_reg(hw, IGP01E1000_PHY_PAGE_SELECT, IGP01E1000_PHY_AGC_B);
		iegbe_read_phy_reg(hw, IGP01E1000_PHY_AGC_B & IGP01E1000_PHY_PAGE_SELECT, &phy_data);
		regs_buff[0xe] = (uint32_t) phy_data;	/* cable length */
		iegbe_write_phy_reg(hw, IGP01E1000_PHY_PAGE_SELECT, IGP01E1000_PHY_AGC_C);
		iegbe_read_phy_reg(hw, IGP01E1000_PHY_AGC_C & IGP01E1000_PHY_PAGE_SELECT, &phy_data);
		regs_buff[0xf] = (uint32_t) phy_data;	/* cable length */
		iegbe_write_phy_reg(hw, IGP01E1000_PHY_PAGE_SELECT, IGP01E1000_PHY_AGC_D);
		iegbe_read_phy_reg(hw, IGP01E1000_PHY_AGC_D & IGP01E1000_PHY_PAGE_SELECT, &phy_data);
		...
		iegbe_read_phy_reg(hw, IGP01E1000_PHY_PORT_STATUS & IGP01E1000_PHY_PAGE_SELECT, &phy_data);
		regs_buff[0x12] = (uint32_t) phy_data;	/* cable polarity */
		iegbe_write_phy_reg(hw, IGP01E1000_PHY_PAGE_SELECT, IGP01E1000_PHY_PCS_INIT_REG);
		iegbe_read_phy_reg(hw, IGP01E1000_PHY_PCS_INIT_REG & IGP01E1000_PHY_PAGE_SELECT, &phy_data);
	...
	if (hw->mac_type >= iegbe_82540 && hw->media_type == iegbe_media_type_copper) {
		regs_buff[0x20] = E1000_READ_REG(hw, MANC);
	}
}

static int iegbe_get_eeprom_len(struct net_device *netdev)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	return adapter->hw.eeprom.word_size * 0x2;
}

static int iegbe_get_eeprom(struct net_device *netdev, struct ethtool_eeprom *eeprom, uint8_t * bytes)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
	...
	eeprom_buff = kmalloc(sizeof(uint16_t) * (last_word - first_word + 1), GFP_KERNEL);
	if (!eeprom_buff) {
		return -ENOMEM;
	}
	if (hw->eeprom.type == iegbe_eeprom_spi) {
		ret_val = iegbe_read_eeprom(hw, first_word, last_word - first_word + 1, eeprom_buff);
	} else {
		for (i = 0; i < last_word - first_word + 1; i++)
			if ((ret_val = iegbe_read_eeprom(hw, first_word + i, 1, &eeprom_buff[i]))) {
				break;
			}
	}
	...
	memcpy(bytes, (uint8_t *) eeprom_buff + (eeprom->offset & 1), eeprom->len);
	kfree(eeprom_buff);

	return ret_val;
}

static int iegbe_set_eeprom(struct net_device *netdev, struct ethtool_eeprom *eeprom, uint8_t * bytes)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
	uint16_t *eeprom_buff;
	...
	if (eeprom->offset & 1) {
		...
		ret_val = iegbe_read_eeprom(hw, first_word, 1, &eeprom_buff[0]);
		ptr++;
	}
	if (((eeprom->offset + eeprom->len) & 1) && (ret_val == 0)) {
		/* need read/modify/write of last changed EEPROM word */
		/* only the first byte of the word is being modified */
		ret_val = iegbe_read_eeprom(hw, last_word, 1, &eeprom_buff[last_word - first_word]);
	}
	...
	ret_val = iegbe_write_eeprom(hw, first_word, last_word - first_word + 1, eeprom_buff);

	/* Update the checksum over the first part of the EEPROM if needed 
	 * and flush shadow RAM for 82573 conrollers */
	if ((ret_val == 0) && ((first_word <= checksum_reg) || (hw->mac_type == iegbe_82573))) {
...
static void iegbe_get_drvinfo(struct net_device *netdev, struct ethtool_drvinfo *drvinfo)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
...
static void iegbe_get_ringparam(struct net_device *netdev, struct ethtool_ringparam *ring)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	...
	ring->rx_max_pending = (mac_type < iegbe_82544) ? E1000_MAX_RXD : E1000_MAX_82544_RXD;
	ring->tx_max_pending = (mac_type < iegbe_82544) ? E1000_MAX_TXD : E1000_MAX_82544_TXD;
...
static int iegbe_set_ringparam(struct net_device *netdev, struct ethtool_ringparam *ring)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	iegbe_mac_type mac_type = adapter->hw.mac_type;
	...
	rxdr->count = min(rxdr->count, (uint32_t) (mac_type < iegbe_82544 ? E1000_MAX_RXD : E1000_MAX_82544_RXD));
	E1000_ROUNDUP(rxdr->count, REQ_RX_DESCRIPTOR_MULTIPLE);

	txdr->count = max(ring->tx_pending, (uint32_t) E1000_MIN_TXD);
	txdr->count = min(txdr->count, (uint32_t) (mac_type < iegbe_82544 ? E1000_MAX_TXD : E1000_MAX_82544_TXD));
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,3,0))
static int iegbe_reg_test(struct iegbe_adapter *adapter, uint64_t * data)
{
	...
	if (value != after) {
		DPRINTK(DRV, ERR, "failed STATUS register test got: " "0x%08X expected: 0x%08X\n", after, value);
		*data = 1;
		return 1;
	}
	...
	if (adapter->hw.mac_type >= iegbe_82543) {
		...
		for (i = 0; i < E1000_RAR_ENTRIES; i++) {
			REG_PATTERN_TEST(RA + ((i << 0x1) << 0x2), 0xFFFFFFFF, 0xFFFFFFFF);
			REG_PATTERN_TEST(RA + (((i << 0x1) + 0x1) << 0x2), 0x8003FFFF, 0xFFFFFFFF);
		}
...
static int iegbe_eeprom_test(struct iegbe_adapter *adapter, uint64_t * data)
{
	uint16_t temp;
	uint16_t checksum = 0;
	uint16_t i;
	uint16_t checksum_reg = adapter->hw.mac_type != iegbe_icp_xxxx ? EEPROM_CHECKSUM_REG : EEPROM_CHECKSUM_REG_ICP_xxxx;
...
static irqreturn_t iegbe_test_intr(int irq, void *data)
{
...
static int iegbe_intr_test(struct iegbe_adapter *adapter, uint64_t * data)
{
	...
	*data = 0;
	...
	if (!request_irq(irq, &iegbe_test_intr, IRQF_PROBE_SHARED, netdev->name, netdev)) {
		shared_int = FALSE;
	} else if (request_irq(irq, &iegbe_test_intr, IRQF_SHARED, netdev->name, netdev)) {
		*data = 1;
		return -1;
	}
...
static void iegbe_free_desc_rings(struct iegbe_adapter *adapter)
{
	...
	if (txdr->desc && txdr->buffer_info) {
		for (i = 0; i < txdr->count; i++) {
			if (txdr->buffer_info[i].dma) {
				pci_unmap_single(pdev, txdr->buffer_info[i].dma, txdr->buffer_info[i].length, PCI_DMA_TODEVICE);
			}
	...
	if (rxdr->desc && rxdr->buffer_info) {
		for (i = 0; i < rxdr->count; i++) {
			if (rxdr->buffer_info[i].dma) {
				pci_unmap_single(pdev, rxdr->buffer_info[i].dma, rxdr->buffer_info[i].length, PCI_DMA_FROMDEVICE);
			}
...
static int iegbe_setup_desc_rings(struct iegbe_adapter *adapter)
{
	struct iegbe_tx_ring *txdr = &adapter->test_tx_ring;
	struct iegbe_rx_ring *rxdr = &adapter->test_rx_ring;
	struct pci_dev *pdev = adapter->pdev;
	...
	E1000_WRITE_REG(&adapter->hw, TDBAL, ((uint64_t) txdr->dma & 0x00000000FFFFFFFF));
	E1000_WRITE_REG(&adapter->hw, TDBAH, ((uint64_t) txdr->dma >> 0x20));
	E1000_WRITE_REG(&adapter->hw, TDLEN, txdr->count * sizeof(struct iegbe_tx_desc));
	E1000_WRITE_REG(&adapter->hw, TDH, 0);
	E1000_WRITE_REG(&adapter->hw, TDT, 0);
	E1000_WRITE_REG(&adapter->hw, TCTL, E1000_TCTL_PSP | E1000_TCTL_EN | E1000_COLLISION_THRESHOLD << E1000_CT_SHIFT | E1000_FDX_COLLISION_DISTANCE << E1000_COLD_SHIFT);
	...
	for (i = 0; i < txdr->count; i++) {
		...
		txdr->buffer_info[i].dma = pci_map_single(pdev, skb->data, skb->len, PCI_DMA_TODEVICE);
	...
	E1000_WRITE_REG(&adapter->hw, RDBAL, ((uint64_t) rxdr->dma & 0xFFFFFFFF));
	E1000_WRITE_REG(&adapter->hw, RDBAH, ((uint64_t) rxdr->dma >> 0x20));
	E1000_WRITE_REG(&adapter->hw, RDLEN, rxdr->size);
	E1000_WRITE_REG(&adapter->hw, RDH, 0);
	E1000_WRITE_REG(&adapter->hw, RDT, 0);
	rctl = E1000_RCTL_EN | E1000_RCTL_BAM | E1000_RCTL_SZ_2048 | E1000_RCTL_LBM_NO | E1000_RCTL_RDMTS_HALF | (adapter->hw.mc_filter_type << E1000_RCTL_MO_SHIFT);
	...
	for (i = 0; i < rxdr->count; i++) {
		...
		if (!(skb = alloc_skb(E1000_RXBUFFER_2048 + NET_IP_ALIGN, GFP_KERNEL))) {
			ret_val = 0x6;
			goto err_nomem;
		}
		...
		rxdr->buffer_info[i].dma = pci_map_single(pdev, skb->data, E1000_RXBUFFER_2048, PCI_DMA_FROMDEVICE);
...
static void iegbe_phy_disable_receiver(struct iegbe_adapter *adapter)
{
...
static void iegbe_phy_reset_clk_and_crs(struct iegbe_adapter *adapter)
{
	uint16_t phy_reg;
	...
	iegbe_write_phy_reg(&adapter->hw, M88E1000_EXT_PHY_SPEC_CTRL, phy_reg);
...
static int iegbe_nonintegrated_phy_loopback(struct iegbe_adapter *adapter)
{
	uint32_t ctrl_reg;
	uint16_t phy_reg;
...
static int iegbe_integrated_phy_loopback(struct iegbe_adapter *adapter)
{
	uint32_t ctrl_reg = 0;
	uint32_t stat_reg = 0;
	...
	if (adapter->hw.phy_type == iegbe_phy_m88) {
		/* Auto-MDI/MDIX Off */
		iegbe_write_phy_reg(&adapter->hw, M88E1000_PHY_SPEC_CTRL, 0x0808);
	...
	if (adapter->hw.media_type == iegbe_media_type_copper && adapter->hw.phy_type == iegbe_phy_m88) {
		ctrl_reg |= E1000_CTRL_ILOS;	/* Invert Loss of Signal */
	} else {
...
static int iegbe_set_phy_loopback(struct iegbe_adapter *adapter)
{
	uint16_t phy_reg = 0;
	uint16_t count = 0;
	...
	case iegbe_82543:
		if (adapter->hw.media_type == iegbe_media_type_copper) {
			 ...
			 */
			while (iegbe_nonintegrated_phy_loopback(adapter) && count++ < 0xa) ;
			if (count < 0xb) {
				return 0;
			}
		}
...
static int iegbe_setup_loopback_test(struct iegbe_adapter *adapter)
{
	uint32_t rctl;

	if (adapter->hw.media_type == iegbe_media_type_fiber || adapter->hw.media_type == iegbe_media_type_internal_serdes) {
		if (adapter->hw.mac_type == iegbe_82545 || adapter->hw.mac_type == iegbe_82546 || adapter->hw.mac_type == iegbe_82545_rev_3 || adapter->hw.mac_type == iegbe_82546_rev_3) {
	...
	} else if (adapter->hw.media_type == iegbe_media_type_copper || (adapter->hw.media_type == iegbe_media_type_oem && iegbe_oem_phy_is_copper(&adapter->hw))) {
		return iegbe_set_phy_loopback(adapter);
	}
	return 0x7;
}

static void iegbe_loopback_cleanup(struct iegbe_adapter *adapter)
{
	uint32_t rctl;
	uint16_t phy_reg;
	...
	if (adapter->hw.media_type == iegbe_media_type_copper ||
	    ((adapter->hw.media_type == iegbe_media_type_fiber ||
	     ...
	     (adapter->hw.mac_type == iegbe_82545 || adapter->hw.mac_type == iegbe_82546 || adapter->hw.mac_type == iegbe_82545_rev_3 || adapter->hw.mac_type == iegbe_82546_rev_3))) {
...
static void iegbe_create_lbtest_frame(struct sk_buff *skb, unsigned int frame_size)
{
...
static int iegbe_check_lbtest_frame(struct sk_buff *skb, unsigned int frame_size)
{
	frame_size = (frame_size % 0x2) ? (frame_size - 1) : frame_size;
	if (*(skb->data + 0x3) == 0xFF) {
		if ((*(skb->data + frame_size / 0xc) == 0xBE) && (*(skb->data + frame_size / 0xe) == 0xAF)) {
			return 0;
		}
	}
	return 0xd;
...
static int iegbe_run_loopback_test(struct iegbe_adapter *adapter)
{
	struct iegbe_tx_ring *txdr = &adapter->test_tx_ring;
	struct iegbe_rx_ring *rxdr = &adapter->test_rx_ring;
	struct pci_dev *pdev = adapter->pdev;
	...
	for (j = 0; j <= lc; j++) {	/* loop count loop */
		for (i = 0; i < 0x40; i++) {	/* send the packets */
			iegbe_create_lbtest_frame(txdr->buffer_info[i].skb, 0x400);
			pci_dma_sync_single_for_device(pdev, txdr->buffer_info[k].dma, txdr->buffer_info[k].length, PCI_DMA_TODEVICE);
		...
		do {		/* receive the sent packets */
			pci_dma_sync_single_for_cpu(pdev, rxdr->buffer_info[l].dma, rxdr->buffer_info[l].length, PCI_DMA_FROMDEVICE);

			ret_val = iegbe_check_lbtest_frame(rxdr->buffer_info[l].skb, 0x400);
			if (!ret_val) {
				good_cnt++;
			}
...
static int iegbe_loopback_test(struct iegbe_adapter *adapter, uint64_t * data)
{

	/* PHY loopback cannot be performed if SoL/IDER
	 * sessions are active */
	if (iegbe_check_phy_reset_block(&adapter->hw)) {
		DPRINTK(DRV, ERR, "Cannot do PHY loopback test " "when SoL/IDER is active.\n");
		*data = 0;
		goto out;
	}
...
static int iegbe_link_test(struct iegbe_adapter *adapter, uint64_t * data)
{
	*data = 0;
...
static int iegbe_diag_test_count(struct net_device *netdev)
{
	return E1000_TEST_LEN;
}

static void iegbe_diag_test(struct net_device *netdev, struct ethtool_test *eth_test, uint64_t * data)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
...
static void iegbe_get_wol(struct net_device *netdev, struct ethtool_wolinfo *wol)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
...
static int iegbe_set_wol(struct net_device *netdev, struct ethtool_wolinfo *wol)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	struct iegbe_hw *hw = &adapter->hw;
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,3,0))
static void iegbe_led_blink_callback(unsigned long data)
{
...
static int iegbe_phys_id(struct net_device *netdev, uint32_t data)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	...
	if (adapter->hw.mac_type < iegbe_82571) {
		...
		E1000_WRITE_REG(&adapter->hw, LEDCTL, (E1000_LEDCTL_LED2_BLINK_RATE |
						       ...
						       (E1000_LEDCTL_MODE_LINK_ACTIVITY << E1000_LEDCTL_LED1_MODE_SHIFT) | (E1000_LEDCTL_MODE_LED_OFF << E1000_LEDCTL_LED0_MODE_SHIFT)));
		msleep_interruptible(data * 0x3e8);
	}
...
static int iegbe_nway_reset(struct net_device *netdev)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	if (netif_running(netdev)) {
		iegbe_down(adapter);
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,3,0))
static int iegbe_get_stats_count(struct net_device *netdev)
{
	return E1000_STATS_LEN;
}
#endif

static void iegbe_get_ethtool_stats(struct net_device *netdev, struct ethtool_stats *stats, uint64_t * data)
{
	struct iegbe_adapter *adapter = netdev_priv(netdev);
	int i;
	...
	for (i = 0; i < E1000_STATS_LEN; i++) {
		...
		data[i] = (iegbe_gstrings_stats[i].sizeof_stat == sizeof(uint64_t)) ? *(uint64_t *) p : *(uint32_t *) p;
	}
}

static void iegbe_get_strings(struct net_device *netdev, uint32_t stringset, uint8_t * data)
{
	int i;

	switch (stringset) {
	case ETH_SS_TEST:
		memcpy(data, *iegbe_gstrings_test, E1000_TEST_LEN * ETH_GSTRING_LEN);
		break;
	case ETH_SS_STATS:
		for (i = 0; i < E1000_STATS_LEN; i++) {
			memcpy(data + i * ETH_GSTRING_LEN, iegbe_gstrings_stats[i].stat_string, ETH_GSTRING_LEN);
		}
		break;
	}
}
