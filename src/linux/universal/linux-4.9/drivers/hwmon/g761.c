/*
 * g761 - Driver for the Global Mixed-mode Technology Inc. fan speed
 *        PWM controller chips from G761 family, i.e. G761 and G763
 *
 ...
 *
 *  http://natisbad.org/NAS/refs/GMT_EDS-761_763-080710-0.2.pdf
 *
 ...
 *
 * g761: minimal datasheet available at:
 *       http://www.gmt.com.tw/product/datasheet/EDS-761_3.pdf
 *
...
MODULE_DEVICE_TABLE(i2c, g761_id);

enum g761_regs {
	G761_REG_SET_CNT  = 0x00,
	G761_REG_ACT_CNT  = 0x01,
	G761_REG_FAN_STA  = 0x02,
	G761_REG_SET_OUT  = 0x03,
	G761_REG_FAN_CMD1 = 0x04,
	G761_REG_FAN_CMD2 = 0x05,
};

struct g761_platform_data {
	u32 fan_startv;
	u32 fan_gear_mode;
	u32 pwm_polarity;
	u32 clk_freq;
...
#define G761_REG_FAN_CMD1_DET_FAN_FAIL  0x80 /* enable fan_fail signal */
#define G761_REG_FAN_CMD1_DET_FAN_OOC   0x40 /* enable fan_out_of_control */
#define G761_REG_FAN_CMD1_OUT_MODE      0x20 /* out mode: PWM or DC */
#define G761_REG_FAN_CMD1_FAN_MODE      0x10 /* fan mode: closed/open-loop */
#define G761_REG_FAN_CMD1_CLK_DIV_ID1   0x08 /* clock divisor value */
#define G761_REG_FAN_CMD1_CLK_DIV_ID0   0x04
#define G761_REG_FAN_CMD1_PWM_POLARITY  0x02 /* PWM polarity */
#define G761_REG_FAN_CMD1_PULSE_PER_REV 0x01 /* pulse per fan revolution */

#define G761_REG_FAN_CMD2_GEAR_MODE_1   0x08 /* fan gear mode */
#define G761_REG_FAN_CMD2_GEAR_MODE_0   0x04
#define G761_REG_FAN_CMD2_FAN_STARTV_1  0x02 /* fan startup voltage */
#define G761_REG_FAN_CMD2_FAN_STARTV_0  0x01

#define G761_REG_FAN_STA_FAIL           0x02 /* fan fail */
#define G761_REG_FAN_STA_OOC            0x01 /* fan out of control */

/* Config register values */
#define G761_OUT_MODE_PWM            1
#define G761_OUT_MODE_DC             0

#define G761_FAN_MODE_CLOSED_LOOP    2
#define G761_FAN_MODE_OPEN_LOOP      1

#define G761_PWM_POLARITY_NEGATIVE   1
#define G761_PWM_POLARITY_POSITIVE   0

/* Register data is read (and cached) at most once per second. */
#define G761_UPDATE_INTERVAL    HZ

/*
 * Extract pulse count per fan revolution value (2 or 4) from given
 * FAN_CMD1 register value.
 */
#define G761_PULSE_FROM_REG(reg) \
	((((reg) & G761_REG_FAN_CMD1_PULSE_PER_REV) + 1) << 1)

/*
 * Extract fan clock divisor (1, 2, 4 or 8) from given FAN_CMD1
 * register value.
 */
#define G761_CLKDIV_FROM_REG(reg) \
	(1 << (((reg) & (G761_REG_FAN_CMD1_CLK_DIV_ID0 |	\
			 G761_REG_FAN_CMD1_CLK_DIV_ID1)) >> 2))

/*
 * Extract fan gear mode multiplier value (0, 2 or 4) from given
 * FAN_CMD2 register value.
 */
#define G761_GEARMULT_FROM_REG(reg) \
	(1 << (((reg) & (G761_REG_FAN_CMD2_GEAR_MODE_0 |	\
			 G761_REG_FAN_CMD2_GEAR_MODE_1)) >> 2))
...
static struct g761_data *g761_update_client(struct device *dev)
{
	struct g761_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	int ret = 0;

	mutex_lock(&data->update_lock);
	if (time_before(jiffies, data->last_updated + G761_UPDATE_INTERVAL) &&
	    likely(data->valid))
		goto out;

	ret = i2c_smbus_read_byte_data(client, G761_REG_SET_CNT);
	if (ret < 0)
		goto out;
	data->set_cnt = ret;

	ret = i2c_smbus_read_byte_data(client, G761_REG_ACT_CNT);
	if (ret < 0)
		goto out;
	data->act_cnt = ret;

	ret = i2c_smbus_read_byte_data(client, G761_REG_FAN_STA);
	if (ret < 0)
		goto out;
	data->fan_sta = ret;

	ret = i2c_smbus_read_byte_data(client, G761_REG_SET_OUT);
	if (ret < 0)
		goto out;
	data->set_out = ret;

	ret = i2c_smbus_read_byte_data(client, G761_REG_FAN_CMD1);
	if (ret < 0)
		goto out;
	data->fan_cmd1 = ret;

	ret = i2c_smbus_read_byte_data(client, G761_REG_FAN_CMD2);
	if (ret < 0)
		goto out;
	data->fan_cmd2 = ret;
...
{
	struct g761_data *data = g761_update_client(dev);
	int ret;
	...
	mutex_lock(&data->update_lock);
	switch (val) {
	case G761_OUT_MODE_PWM:
		data->fan_cmd1 |=  G761_REG_FAN_CMD1_OUT_MODE;
		break;
	case G761_OUT_MODE_DC:
		data->fan_cmd1 &= ~G761_REG_FAN_CMD1_OUT_MODE;
		break;
	default:
		ret = -EINVAL;
	...
	ret = i2c_smbus_write_byte_data(data->client, G761_REG_FAN_CMD1,
					data->fan_cmd1);
	data->valid = false;
 out:
	mutex_unlock(&data->update_lock);
...
static int do_set_fan_div(struct device *dev, unsigned long val)
{
	...
	switch (val) {
	case 1:
		data->fan_cmd1 &= ~G761_REG_FAN_CMD1_CLK_DIV_ID0;
		data->fan_cmd1 &= ~G761_REG_FAN_CMD1_CLK_DIV_ID1;
		break;
	case 2:
		data->fan_cmd1 |=  G761_REG_FAN_CMD1_CLK_DIV_ID0;
		data->fan_cmd1 &= ~G761_REG_FAN_CMD1_CLK_DIV_ID1;
...
static int do_set_fan_gear_mode(struct device *dev, unsigned long val)
{
	...
	switch (val) {
	case 0:
		data->fan_cmd2 &= ~G761_REG_FAN_CMD2_GEAR_MODE_0;
		data->fan_cmd2 &= ~G761_REG_FAN_CMD2_GEAR_MODE_1;
		break;
	case 1:
		data->fan_cmd2 |=  G761_REG_FAN_CMD2_GEAR_MODE_0;
		data->fan_cmd2 &= ~G761_REG_FAN_CMD2_GEAR_MODE_1;
	...
	ret = i2c_smbus_write_byte_data(data->client, G761_REG_FAN_CMD2,
					data->fan_cmd2);
	data->valid = false;
 out:
	mutex_unlock(&data->update_lock);
...
static int do_set_fan_pulses(struct device *dev, unsigned long val)
{
	...
	switch (val) {
	case 2:
		data->fan_cmd1 &= ~G761_REG_FAN_CMD1_PULSE_PER_REV;
		break;
	case 4:
		data->fan_cmd1 |=  G761_REG_FAN_CMD1_PULSE_PER_REV;
		break;
...
static int do_set_pwm_enable(struct device *dev, unsigned long val)
{
	...
	mutex_lock(&data->update_lock);
	switch (val) {
	case G761_FAN_MODE_CLOSED_LOOP:
		data->fan_cmd1 |=  G761_REG_FAN_CMD1_FAN_MODE;
		break;
	case G761_FAN_MODE_OPEN_LOOP:
		data->fan_cmd1 &= ~G761_REG_FAN_CMD1_FAN_MODE;
		/*
...
static int do_set_pwm_polarity(struct device *dev, unsigned long val)
{
	...
	mutex_lock(&data->update_lock);
	switch (val) {
	case G761_PWM_POLARITY_POSITIVE:
		data->fan_cmd1 &= ~G761_REG_FAN_CMD1_PWM_POLARITY;
		break;
	case G761_PWM_POLARITY_NEGATIVE:
		data->fan_cmd1 |=  G761_REG_FAN_CMD1_PWM_POLARITY;
		break;
	default:
		ret = -EINVAL;
...
static int do_set_pwm(struct device *dev, unsigned long val)
{
	...
	mutex_lock(&data->update_lock);
	ret = i2c_smbus_write_byte_data(client, G761_REG_SET_OUT, val);
	data->valid = false;
	mutex_unlock(&data->update_lock);
...
static int do_set_fan_target(struct device *dev, unsigned long val)
{
	...
	data->set_cnt = cnt_from_rpm(val, data->clk_freq,
				     G761_PULSE_FROM_REG(data->fan_cmd1),
				     G761_CLKDIV_FROM_REG(data->fan_cmd1),
				     G761_GEARMULT_FROM_REG(data->fan_cmd2));
	ret = i2c_smbus_write_byte_data(data->client, G761_REG_SET_CNT,
					data->set_cnt);
	data->valid = false;
	mutex_unlock(&data->update_lock);
...
static int do_set_fan_startv(struct device *dev, unsigned long val)
{
	...
	switch (val) {
	case 0:
		data->fan_cmd2 &= ~G761_REG_FAN_CMD2_FAN_STARTV_0;
		data->fan_cmd2 &= ~G761_REG_FAN_CMD2_FAN_STARTV_1;
		break;
	case 1:
		data->fan_cmd2 |=  G761_REG_FAN_CMD2_FAN_STARTV_0;
		data->fan_cmd2 &= ~G761_REG_FAN_CMD2_FAN_STARTV_1;
...
#ifdef CONFIG_OF
static const struct of_device_id g761_dt_match[] = {
	{ .compatible = "gmt,g761" },
	{ .compatible = "gmt,g763" },
	{ },
};
MODULE_DEVICE_TABLE(of, g761_dt_match);
 ...
 * call to g761_of_clock_disable(). Note that a reference to clock is kept
 * in our private data structure to be used in this function.
 */
static int g761_of_clock_enable(struct i2c_client *client)
{
	struct g761_data *data;
...
static void g761_of_clock_disable(struct i2c_client *client)
{
	struct g761_data *data = i2c_get_clientdata(client);
...
static int g761_of_prop_import_one(struct i2c_client *client,
				   const char *pname,
...
static int g761_of_prop_import(struct i2c_client *client)
{
	int ret;
	...
	ret = g761_of_prop_import_one(client, "fan_gear_mode",
				      do_set_fan_gear_mode);
	if (ret)
		return ret;

	ret = g761_of_prop_import_one(client, "pwm_polarity",
				      do_set_pwm_polarity);
	if (ret)
		return ret;

	return g761_of_prop_import_one(client, "fan_startv",
				       do_set_fan_startv);
}
...
static int g761_pdata_prop_import(struct i2c_client *client)
{
	struct g761_platform_data *pdata = dev_get_platdata(&client->dev);
	int ret;
			   ...
			   char *buf)
{
	...
	if (data->fan_sta & G761_REG_FAN_STA_OOC) {
 ...
 * and it is not emulated by g761 driver. -EINVAL is returned in this case.
 */
			      ...
			      struct device_attribute *da, char *buf)
{
	...
	return sprintf(buf, "%d\n",
		       (!!(data->fan_cmd1 & G761_REG_FAN_CMD1_FAN_MODE)) + 1);
}
			       ...
			       char *buf)
{
	...
	return sprintf(buf, "%u\n", !!(data->fan_sta & G761_REG_FAN_STA_FAIL));
}
			   ...
			   char *buf)
{
	...
	return sprintf(buf, "%u\n", !(data->fan_sta & G761_REG_FAN_STA_OOC));
}
...
/* Driver data */
static struct attribute *g761_attrs[] = {
...
//ATTRIBUTE_GROUPS(g761);

static const struct attribute_group g761_group = {
	.attrs = g761_attrs,
};
 ...
 */
static inline int g761_fan_init(struct device *dev)
{
	struct g761_data *data = g761_update_client(dev);
	...
	data->fan_cmd1 |= G761_REG_FAN_CMD1_DET_FAN_FAIL;
	data->fan_cmd1 |= G761_REG_FAN_CMD1_DET_FAN_OOC;
	data->valid = false;

	return i2c_smbus_write_byte_data(data->client, G761_REG_FAN_CMD1,
					 data->fan_cmd1);
}

static int g761_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data = devm_kzalloc(dev, sizeof(struct g761_data), GFP_KERNEL);
	if (!data)
		return -ENOMEM;
	...
	ret = g761_fan_init(dev);
	if (ret)
		return ret;

	/* Get configuration via DT ... */
	ret = g761_of_clock_enable(client);
	if (ret)
		return ret;
	ret = g761_of_prop_import(client);
	if (ret)
		goto clock_dis;
	/* ... or platform_data */
	ret = g761_pdata_prop_import(client);
	if (ret)
		goto clock_dis;

	//data->hwmon_dev = hwmon_device_register_with_groups(dev, client->name,data, g761_groups);
	
	/* Register sysfs hooks */
	err = sysfs_create_group(&client->dev.kobj, &g761_group);
	if (err)
		return err;
...
static struct i2c_driver g761_driver = {
	.driver = {
		.name = DRVNAME,
		.of_match_table = of_match_ptr(g761_dt_match),
	},
	.probe	  = g761_probe,
	.remove	  = g761_remove,
	.id_table = g761_id,
...
module_i2c_driver(g761_driver);

MODULE_AUTHOR("Arnaud EBALARD <arno@natisbad.org>");
MODULE_DESCRIPTION("GMT G761/G763 driver");
MODULE_LICENSE("GPL");
