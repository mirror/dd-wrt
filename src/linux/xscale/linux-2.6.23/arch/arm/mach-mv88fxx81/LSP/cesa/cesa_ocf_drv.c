...
static int			cesa_block = 0;
//static spinlock_t 		cesa_lock;
static u32			cesaReadyMap = 0;

/* static APIs */
static int 		cesa_ocf_process	(void *, struct cryptop *, int);
static int 		cesa_ocf_newsession	(void *, u_int32_t *, struct cryptoini *);
static int 		cesa_ocf_freesession	(void *, u_int64_t);
static void 		cesa_callback		(unsigned long);
static irqreturn_t 	cesa_interrupt_handler	(int, void *, struct pt_regs *);
...
static void cesaTestTraceAdd(int type)
{
    ...
    cesaTestTrace[cesaTestTraceIdx].procMap = mvCesaChanInProcessGet();
    cesaTestTrace[cesaTestTraceIdx].intrMap = cesaReadyMap;
 ...
 */
static int 
cesa_ocf_process(void *arg, struct cryptop *crp, int hint)
{
	...
	p_buf_info += 1; 
		...
		/* Authentication */
			...
			if ((crp->crp_flags & CRYPTO_F_IOV) && crp->crp_mac) {
				
				dprintk("%s,%d: set digest in crp_mac. \n", __FILE__, __LINE__);
				/* append the digest buf to the mbuf */
				p_mbuf_info->numFrags++;
				p_mbuf_info->mbufSize += DIGEST_BUF_SIZE; 
				p_mbuf_info->pFrags = p_buf_info;

				cesa_ocf_cmd->digest_len = cesa_ocf_cur_ses->digestlen;
				memcpy(cesa_ocf_cmd->digest, crp->crp_mac, cesa_ocf_cmd->digest_len);
				p_buf_info->bufVirtPtr = cesa_ocf_cmd->digest;
				p_buf_info->bufSize = DIGEST_BUF_SIZE;
				p_buf_info--;

				/* Offsets */
				cesa_cmd->digestOffset = 0;
				cesa_cmd->macOffset += DIGEST_BUF_SIZE;
 				if(encrypt || decrypt) {
					cesa_cmd->cryptoOffset += DIGEST_BUF_SIZE;
					cesa_cmd->ivOffset += DIGEST_BUF_SIZE; 	
				}
			}
		} 
		else {
	...
	/* make sure we have enough reserved space, don't change this, 	  */
	/* if you want to explore the Q better, simply enlarge the Q size. */
	if(cesaReqResources <= 4){
		cesa_block = 1;
	}

	if(!cesa_block)
		crypto_unblock(cesa_ocf_id, CRYPTO_SYMQ);
...
cesa_callback(unsigned long dummy)
{
	...
	disable_irq(CESA_IRQ);
    	...
    	while(MV_TRUE) {
	    	...
	    	/* Get Ready requests */
	    	status = mvCesaReadyGet(cesaReadyMap, &result[res_idx]);
		cesaTestTraceAdd(2);	
			...
			return;
#endif
			if(status == MV_TERMINATE) { /* last authentication with WA */
				continue;
			}
		    	break;
        	}
		res_idx++;
		if(res_idx > 2) {
			printk("%s,%d: cesa get ready return too many??? \n", __FILE__, __LINE__);
			break;
		}
	}
	...
	for(i = 0; i < res_idx; i++) {
		...
		if(cesa_block && cesa_ocf_cmd->need_cb) {	
			crypto_unblock(cesa_ocf_id, CRYPTO_SYMQ);
			cesa_block = 0;
		}
...
#endif
		if(cesa_ocf_cmd->need_cb) {
			if(cesa_ocf_cmd->digest_len) {
				memcpy(crp->crp_mac ,cesa_ocf_cmd->digest, cesa_ocf_cmd->digest_len);
			}
			if(debug) {
...
cesa_interrupt_polling(void)
{
	...
	procMap = mvCesaChanInProcessGet();

  	/* Read cause register */
	do {
		cause = MV_REG_READ(MV_CESA_ISR_CAUSE_REG);

    		/* Process only if all channels finished */
    		cesaReadyMap |= ((cause & MV_CESA_CAUSE_ACC_IDMA_ALL_MASK) >> MV_CESA_CAUSE_ACC_IDMA_OFFSET);
	
    		dprintk("%s,%d: cause=0x%x, readyMap=%d, procMap=%d, status=0x%x\n", __FILE__, __LINE__, 
        	cause, cesaReadyMap, procMap, MV_REG_READ(MV_CESA_STATUS_REG));

	} while (cesaReadyMap != procMap);
 ...
 */
static irqreturn_t
cesa_interrupt_handler(int irq, void *arg, struct pt_regs *regs)
{
        u32                  	cause, procMap = 0;
	...
	cause = MV_REG_READ(MV_CESA_ISR_CAUSE_REG);
    	...
    	if(cesaReadyMap != procMap) {
        	/* Clear readyMap, exit and wait for next interrupt */
		dprintk("%s,%d: waiting for next interrupt\n", __FILE__, __LINE__);
        	MV_REG_WRITE(MV_CESA_ISR_CAUSE_REG, ~(cause & MV_CESA_CAUSE_ACC_IDMA_ALL_MASK));
        	return IRQ_HANDLED;
    	}
 ...
 */
static int
cesa_ocf_freesession(void *arg, u_int64_t tid)
{
        struct cesa_ocf_data *cesa_ocf_cur_ses;
...
cesa_ocf_init(void)
{
	...
	cesa_ocf_id = crypto_get_driverid(0);
	if (cesa_ocf_id < 0)
		panic("MV CESA crypto device cannot initialize!");

	dprintk("%s,%d: cesa ocf device id is %d \n", __FILE__, __LINE__, cesa_ocf_id);

	if( MV_OK != mvCesaInit(CESA_OCF_MAX_SES*5, CESA_Q_SIZE, (char *)CRYPT_ENG_BASE) ) {
	...
	MV_REG_WRITE( MV_CESA_ISR_CAUSE_REG, 0);
    	...
    	MV_REG_WRITE( MV_CESA_ISR_MASK_REG, MV_CESA_CAUSE_ACC_IDMA_MASK(0) | MV_CESA_CAUSE_ACC_IDMA_MASK(1) );
#endif
#ifdef CESA_OCF_TASKLET
	...
	crypto_register(cesa_ocf_id, CRYPTO_AES_CBC, 0, 0, cesa_ocf_newsession, cesa_ocf_freesession, cesa_ocf_process, NULL);
#define	REGISTER(alg) \
	crypto_register(cesa_ocf_id, alg, 0,0,NULL,NULL,NULL,NULL)
...
void cesa_ocf_debug(void)
{
...
#ifdef CESA_OCF_TRACE_DEBUG
    {
        ...
        for(i=0; i<MV_CESA_TEST_TRACE_SIZE; i++)
        {
            mvOsPrintf("%02d.  %d   0x%04x   0x%04x   0x%02x   0x%02x   %02d   0x%06x  %p  %p  %p\n",
                ...
                cesaTestTrace[j].idmaCause, cesaTestTrace[j].procMap,
                cesaTestTrace[j].intrMap, cesaTestTrace[j].resources, cesaTestTrace[j].timeStamp,
	...
	printk("cesaReadyMap %x ", cesaReadyMap);
	printk("mvCesaChanInProcessGet() %x", mvCesaChanInProcessGet());
}
