...
#define DRIVER_VERSION 		"27 July 2006"
#define DRIVER_AUTHOR 		"Intel"
#define DRIVER_DESC   		"Intel IXP4XX on-chip EHCI USB 2.0 Host Controller Driver"
#define IXP4XX_HCD_DESC		"ixp4xx_ehci_hcd"
#define IXP4XX_EHCI_NAME	"ixp4xx-ehci"

static const char ixp4xx_ehci_name[] = IXP4XX_EHCI_NAME;
...
/*-------------------------------------------------------------------------*/
#define IXP4XX_USB_HOST_IO_LEN      0x200

#define IXP4XX_USB_HOST0_IO_START   (0xcd000000+0x100)
#define IXP4XX_USB_HOST1_IO_START   (0xce000000+0x100)

#define USBMODE      	0xA8
#define USBMODE_BE   	0x4
 ...
 */
static int handshake (struct ehci_hcd *ehci,void __iomem *ptr, u32 mask, u32 done, int usec)
{
	u32	result;
...
#include "ixp4xx-ehci-hub.c"
#include "ehci-mem.c"
#include "ixp4xx-ehci-q.c"
...
/*-------------------------------------------------------------------------*/
static int ehci_ixp4xx_setup(struct usb_hcd *hcd)
{
	struct ehci_hcd		*ehci = hcd_to_ehci(hcd);
	u32			temp;
	int			retval;
...
static int ixp4xx_ehci_hcd_remove(struct device *dev)
{
     /* The following code piece is refered to hcd-pci.c  */
	struct usb_hcd *hcd;
	struct platform_device *pdev = to_platform_device(dev);

	hcd = (struct usb_hcd *)dev_get_drvdata(dev);	
...
/*
 * Driver definition to register with the IXP4XX bus
 */
...
static struct resource ixp4xx_ehci_host0_res[] = {
	{
		.start		= IXP4XX_USB_HOST0_IO_START,
		.end		= IXP4XX_USB_HOST0_IO_START 
					+ IXP4XX_USB_HOST_IO_LEN - 1,
...
static struct resource ixp4xx_ehci_host1_res[] = {
	{
		.start		= IXP4XX_USB_HOST1_IO_START,
		.end		= IXP4XX_USB_HOST1_IO_START 
					+ IXP4XX_USB_HOST_IO_LEN - 1,
		.flags		= IORESOURCE_MEM,
	},
	{
...
static void __exit ixp4xx_ehci_cleanup(void) 
{
	driver_unregister(&ixp4xx_ehci_driver);
}

module_init(ixp4xx_ehci_init);
module_exit(ixp4xx_ehci_cleanup);
