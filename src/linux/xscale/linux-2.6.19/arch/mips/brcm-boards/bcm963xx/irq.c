...
extern void brcm_timer_interrupt(void);
                                ...
                                unsigned int irq)
{
     ...
     */
    irq_desc[irq].chip = &brcm_irq_no_end_type;

#if defined(CONFIG_BCM96348) || defined(CONFIG_BCM96358) 
    if( irq == INTERRUPT_ID_MPI ) {
        nRet = request_irq( irq, pfunc, IRQF_SAMPLE_RANDOM | IRQF_DISABLED | IRQF_SHARED,
            devname, (void *) param );
    ...
    if( irq >= INTERNAL_ISR_TABLE_OFFSET )
    {
        ...
        local_irq_restore(flags);
#endif
        nRet = request_irq( irq, pfunc, IRQF_SAMPLE_RANDOM | IRQF_DISABLED,
            devname, (void *) param );
    ...
    {
        nRet = request_external_irq( irq, pfunc, IRQF_SAMPLE_RANDOM | IRQF_DISABLED,
            devname, (void *) param );
    }
#endif
    else if ((irq == INTERRUPT_ID_SOFTWARE_0) || (irq == INTERRUPT_ID_SOFTWARE_1))
