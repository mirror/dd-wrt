...
//    UINT32       *imageCodePtr  = NULL;
	...
	extern struct platform_device ixp400_npe_devices[];
...
*/
		sprintf(firmware,"ixNpe%c-%1X%02X-%d.%d",
			'A'+npeId,
			deviceId,
        ...
        status = request_firmware(&fw_entry, firmware, &ixp400_npe_devices[npeId].dev);
		printk( KERN_ERR "Loading firmware %s: %d\n", firmware, status );
        ...
        if (IX_SUCCESS == status)
        {
             ...
             */
            status = ixNpeDlNpeMgrImageLoad (npeId, (UINT32*)fw_entry->data, TRUE);
            if (IX_SUCCESS == status)
            {
