   ...
   const void *data, unsigned int datalen)
{
	...
	if (ah->ar_hln == ETH_ALEN && ah->ar_pro == htons(ETH_P_IP) &&
	    ah->ar_pln == 4)
	{
	    arp_sip = (uint32_t*)(((**pskb).nh.raw) + sizeof(struct arphdr) + ((**pskb).nh.arph)->ar_hln);
	    arp_smac = ((**pskb).nh.raw) + sizeof(struct arphdr);
	    arp_dip = (uint32_t*)(((**pskb).nh.raw) + sizeof(struct arphdr) + (2*(((**pskb).nh.arph)->ar_hln)) + ((**pskb).nh.arph)->ar_pln);
	    arp_dmac = ((**pskb).nh.raw) + sizeof(struct arphdr) + ((**pskb).nh.arph)->ar_hln + ((**pskb).nh.arph)->ar_pln;
	}
	else
	{
            ah = NULL;
