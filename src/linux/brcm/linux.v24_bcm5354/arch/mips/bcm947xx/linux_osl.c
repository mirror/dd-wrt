...
{
	uint retry = PCI_CFG_RETRY;	 
	 //printk(KERN_EMERG "write_config\n");
...
{
	void *addr;
	 //printk(KERN_EMERG "osl malloc magic\n");

	/* only ASSERT if osh is defined */
	if (osh)
		ASSERT(osh->magic == OS_HANDLE_MAGIC);
	 //printk(KERN_EMERG "kmalloc\n");
...
{
	 //printk(KERN_EMERG "malloc_failed\n");
...
osl_printf(const char *format, ...)
{
	...
	/* sprintf into a local buffer because there *is* no "printk()".. */
...
{
	struct sk_buff *nskb;
	 //printk(KERN_EMERG "okt_tonative\n");
...
{
	struct sk_buff *nskb;
	 //printk(KERN_EMERG "pkt_frmnative\n");
