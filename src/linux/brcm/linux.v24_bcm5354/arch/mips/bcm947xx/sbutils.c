...
//printk(KERN_EMERG "sb_doattach\n");
	...
	if (bustype == PCI_BUS) {
//printk(KERN_EMERG "sb_doattach->readconfig %X\n",si);
//printk(KERN_EMERG "sb_doattach->readconfig %X\n",si->osh);
		if (OSL_PCI_READ_CONFIG(si->osh, PCI_SPROM_CONTROL, sizeof(uint32)) == 0xffffffff) {
			printk(KERN_EMERG "%s: incoming bus is PCI but it's a lie, switching to SB "
			          "devid:0x%x\n", __FUNCTION__, devid);
			bustype = SB_BUS;
		}
//printk(KERN_EMERG "sb_doattach->done\n");
	}
...
//printk(KERN_EMERG "sb_attach->srom_var_init\n");
	...
	if (si->sb.cccaps & CAP_PMU) {
//printk(KERN_EMERG "sb_doattach->pmuctl_pllinit\n");

		/* Find out Crystal frequency and init PMU/PLL */
		if (!((si->sb.chip == BCM5354_CHIP_ID) && (devid == BCM4710_DEVICE_ID)))
			sb_pmuctl_pllinit((sb_t *)si, getintvar(NULL, "xtalfreq"));
//printk(KERN_EMERG "sb_doattach->pmu_init_resources\n");
...
//printk(KERN_EMERG "sb_doattach->getboardvendor and type\n");
...
//printk(KERN_EMERG "sb_doattach->forcehtwar\n");
	if (FORCEHT_WAR32414(si)) {
		sb_clkctl_init((void *)si);
		sb_war32414_forceHT((void *)si, 1);
	}
//printk(KERN_EMERG "sb_doattach->disable gpiopullup and gpiopulldown\n");
...
sb_coreflags(sb_t *sbh, uint32 mask, uint32 val)
{
	...
	sbconfig_t *sb;
	uint32 w;
//printk(KERN_EMERG "sb_coreflags\n");
...
sb_coreflagshi(sb_t *sbh, uint32 mask, uint32 val)
{
	...
	sbconfig_t *sb;
	uint32 w;
//printk(KERN_EMERG "sb_coreflagshi\n");
...
{
//printk(KERN_EMERG "sb_chip2numcores\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_pcmciarev\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_boardvendor\n");
...
BCMINITFN(sb_boardtype)(sb_t *sbh)
{
	sb_info_t *si;
	char *var;
//printk(KERN_EMERG "sb_boardtype\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_buscoretype\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_buscorerev\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_corelisth\n");
...
sb_core_reset(sb_t *sbh, uint32 bits, uint32 resetbits)
{
	...
	sbconfig_t *sb;
	volatile uint32 dummy;
//printk(KERN_EMERG "sb_core_reset\n");
...
sb_core_tofixup(sb_t *sbh)
{
	sb_info_t *si;
	sbconfig_t *sb;
//printk(KERN_EMERG "sb_core_tofixup\n");
...
sb_core_disable(sb_t *sbh, uint32 bits)
{
	...
	uint32 rej;
	sbconfig_t *sb;
//printk(KERN_EMERG "sb_core_disable\n");
...
sb_pcmcia_init(sb_t *sbh)
{
	sb_info_t *si;
	uint8 cor = 0;
//printk(KERN_EMERG "sb_pcmcia_init\n");
...
BCMINITFN(sb_pci_setup)(sb_t *sbh, uint coremask)
{
	...
	uint idx;
	int reg_val;
//printk(KERN_EMERG "sb_pci_setup\n");
...
//printk(KERN_EMERG "sb_clock_rate\n");
...
BCMINITFN(sb_alp_clock)(sb_t *sbh)
{
	sb_info_t *si;
	uint32 cap;
//printk(KERN_EMERG "sb_alp_clock\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_gpiosetcore\n");
...
sb_gpiocontrol(sb_t *sbh, uint32 mask, uint32 val, uint8 priority)
{
	sb_info_t *si;
	uint regoff;
	//printk(KERN_EMERG "sb_gpiocontrol\n");
...
sb_gpioouten(sb_t *sbh, uint32 mask, uint32 val, uint8 priority)
{
	sb_info_t *si;
	uint regoff;
//printk(KERN_EMERG "sb_gpioouten\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_gpioreserve\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_gpiorelease\n");
...
sb_gpiointpolarity(sb_t *sbh, uint32 mask, uint32 val, uint8 priority)
{
	sb_info_t *si;
	uint regoff;
//printk(KERN_EMERG "sb_gpiointpolarity\n");
...
sb_gpiointmask(sb_t *sbh, uint32 mask, uint32 val, uint8 priority)
{
	sb_info_t *si;
	uint regoff;
//printk(KERN_EMERG "sb_gpiointmask\n");
...
sb_gpiotimerval(sb_t *sbh, uint32 mask, uint32 gpiotimerval)
{
	sb_info_t *si;
	si = SB_INFO(sbh);
//printk(KERN_EMERG "sb_gpiotimerinterval\n");
...
//printk(KERN_EMERG "sb_slowclk_src\n");
...
sb_slowclk_freq(sb_info_t *si, bool max_freq)
{
	...
	uint32 slowclk;
	uint div;
//printk(KERN_EMERG "sb_slowclk_freq\n");
...
//printk(KERN_EMERG "sb_clkctl_Setdelay\n");
...
BCMINITFN(sb_clkctl_init)(sb_t *sbh)
{
	...
	uint origidx;
	chipcregs_t *cc;
//printk(KERN_EMERG "sb_clkctl_init\n");
...
sb_clkctl_fast_pwrup_delay(sb_t *sbh)
{
	...
	uint16 fpdelay;
	uint intr_val = 0;
//printk(KERN_EMERG "sb_fast_pwrup_delay\n");
...
sb_clkctl_xtal(sb_t *sbh, uint what, bool on)
{
	sb_info_t *si;
	uint32 in, out, outen;
//printk(KERN_EMERG "sb_clkctl_xtal\n");
...
sb_clkctl_clk(sb_t *sbh, uint mode)
{
	...
	uint32 scc;
	uint intr_val = 0;
//printk(KERN_EMERG "sb_clkctl_clk\n");
	...
	if (!(R_REG(si->osh, &cc->capabilities) & CAP_PWR_CTL) && (si->sb.ccrev < 20))
		goto done;
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_register_intr_callback\n");
...
sb_d11_devid(sb_t *sbh)
{
	uint chip, chippkg;
	uint16 device;
//printk(KERN_EMERG "sb_d11_devid\n");
             ...
             uint8 *pciheader)
{
	...
	uint8 header;
//printk(KERN_EMERG "sb_corepciid\n");
...
//printk(KERN_EMERG "sb_pcie_mdiowrite\n");
...
sb_pcie_readreg(void *sb, void* arg1, uint offset)
{
	...
	sbpcieregs_t *pcieregs;
	uint addrtype;
//printk(KERN_EMERG "sb_pcie_readreg\n");
...
sb_pcie_writereg(sb_t *sbh, void *arg1,  uint offset, uint val)
{
	...
	sbpcieregs_t *pcieregs;
	uint addrtype;
//printk(KERN_EMERG "sb_pcie_writereg\n");
...
//printk(KERN_EMERG "sb_getdevpathintvar\n");
...
sb_pci_fixcfg(sb_info_t *si)
{
	...
	char devpath[SB_DEVPATH_BUFSZ];
//printk(KERN_EMERG "sb_pci_fixcfg\n");
...
{
	sb_info_t *si;
//printk(KERN_EMERG "sb_backplane64\n");

	si = SB_INFO(sbh);
	return ((si->sb.cccaps & CAP_BKPLN64) != 0);
}
...
sb_btcgpiowar(sb_t *sbh)
{
	...
	chipcregs_t *cc;
	si = SB_INFO(sbh);
//printk(KERN_EMERG "sb_gpiowar\n");

	/* Make sure that there is ChipCommon core present &&
	 * UART_TX is strapped to 1
	 */
	if (!(si->sb.cccaps & CAP_UARTGPIO))
		return;
...
sb_deviceremoved(sb_t *sbh)
{
	uint32 w;
	sb_info_t *si;
//printk(KERN_EMERG "sb_deviceremoved\n");
...
sb_socram_size(sb_t *sbh)
{
	...
	uint origidx;
	uint intr_val = 0;
//printk(KERN_EMERG "sb_socram_size\n");
...
BCMINITFN(sb_set_switcher_voltage)(sb_t *sbh, uint8 bb_voltage, uint8 rf_voltage)
{
	...
	uint origidx;
	osl_t *osh;
//printk(KERN_EMERG "sb_set_switcher_voltage\n");

	ASSERT(sbh->cccaps & CAP_PMU);
...
//printk(KERN_EMERG "sb_pmu_init_resources\n");
...
BCMINITFN(sb_pmu_fast_pwrup_delay)(sb_t *sbh)
{
	sb_info_t *si;
	uint16 delay = 10000;
//printk(KERN_EMERG "sb_pmu_fast_pwrup_delay\n");
...
BCMINITFN(sb_pmuctl_pllinit)(sb_t *sbh, uint32 xtal)
{
	...
	uint origidx;
	osl_t *osh;
//printk(KERN_EMERG "sb_pmuctl_pllinit\n");
...
sb_pmuctl_force_ilp(sb_t *sbh, bool force)
{
	...
	osl_t *osh;
//printk(KERN_EMERG "sb_pmuctl_force_ilp\n");
...
sb_pmu_set_res_mask(sb_t *sbh, uint32 *min_mask, uint32 *max_mask)
{
	...
	uint32 oldmask;
	osl_t *osh;
//printk(KERN_EMERG "sb_pmu_set_res_mask\n");
...
BCMINITFN(sb_pmu_alp_clock)(sb_info_t *si)
{
	uint32 xf;
	const xtaltab_t *xt;
//printk(KERN_EMERG "sb_pmu_alp_clock\n");
