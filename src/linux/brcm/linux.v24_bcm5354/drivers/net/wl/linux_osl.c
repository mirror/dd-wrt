...
osl_attach(void *pdev, uint bustype, bool pkttag)
{
	...
	switch (bustype) {
		...
		case SDIO_BUS:
		case USB_BUS:
			 printk(KERN_EMERG "mmbus = false\n");
		
			osh->pub.mmbus = FALSE;
			break;
		default:
			 printk(KERN_EMERG "bloedsinn\n");
			ASSERT(FALSE);
			break;
	}
...
{
			 printk(KERN_EMERG "trying detach\n");

	if (osh == NULL)
		return;
			 printk(KERN_EMERG "osl detach\n");
...
osl_debug_mfree(osl_t *osh, void *addr, uint size, int line, char* file)
{
	...
	if (p->size == 0) {
		printk("osl_debug_mfree: double free on addr 0x%p size %d at line %d file %s\n",
		       addr, size, line, file);
		ASSERT(p->size);
		return;
	}
	...
	if (p->size != size) {
		printk("osl_debug_mfree: dealloc size %d does not match alloc size %d on addr 0x%p"
		       " at line %d file %s\n",
...
osl_debug_memdump(osl_t *osh, struct bcmstrbuf *b)
{
	...
	for (p = osh->dbgmem_list; p; p = p->next)
		bcm_bprintf(b, "0x%08x\t%5d\t%s:%d\n",
		            (uint8 *)p + sizeof(bcm_mem_link_t), p->size, p->file, p->line);
