...
#endif
{
	...
	if (flags & (LOOKUP_CREATE | LOOKUP_RENAME_TARGET))
		return 0;
#else
    if (nd && nd->flags & (LOOKUP_CREATE | LOOKUP_RENAME_TARGET))
        return 0;
#endif
...
static unsigned int __exfat_striptail_len(const struct qstr* qstr)
{
	return exfat_striptail_len(qstr->len, qstr->name);
}
...
#endif
{
	...
	alen = __exfat_striptail_len(a);
	blen = __exfat_striptail_len(b);
#else
	...
	if (alen != blen)
		return 1;
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,38)
	for (i = 0; i < a->len; i += charlen) {
		charlen = t->char2uni(&a->name[i], alen - i, &c1);
		if (charlen < 0)
			return 1;
		if (charlen != t->char2uni(&b->name[i], blen - i, &c2))
			return 1;
...
#endif
{
	...
	for (i = 0; i < alen; i += charlen) {
		charlen = utf8_to_utf32(&a->name[i], alen - i, &u_a);
		if (charlen < 0)
			return 1;
		if (charlen != utf8_to_utf32(&b->name[i], blen - i, &u_b))
			return 1;
...
#else
            WARN_ON(alias->d_name.hash != dentry->d_name.hash || alias->d_name.len != dentry->d_name.len);
#endif
