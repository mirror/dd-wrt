...
STATIC int bcm5700_poll(struct napi_struct *napi, int budget);
#endif
...
#endif
#ifdef BCM_NAPI_RXPOLL
	netif_napi_add(dev, &pDevice->sc_napi, bcm5700_poll, 64);
...
#ifdef BCM_NAPI_RXPOLL
STATIC int
bcm5700_poll(struct napi_struct *napi, int budget)
{
	int orig_budget = budget;
	int work_done;
	PUM_DEVICE_BLOCK sc = (PUM_DEVICE_BLOCK )container_of(napi, struct _LM_DEVICE_BLOCK, sc_napi);
...
MM_ScheduleRxPoll(LM_DEVICE_BLOCK *pDevice)
{
	...
	if (netif_rx_schedule_prep(dev,&pDevice->sc_napi)) {
		__netif_rx_schedule(dev,&pDevice->sc_napi);
