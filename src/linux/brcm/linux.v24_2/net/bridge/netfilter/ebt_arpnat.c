   ...
   const void *data, unsigned int datalen)
{
	...
	else if (((**pskb).mac.ethernet)->h_proto == __constant_htons(ETH_P_IP))
	{
	...
	if (ah)
	{
		...
		eth_smac = ((**pskb).mac.ethernet)->h_source;
		arp_smac = ((**pskb).nh.raw) + sizeof(struct arphdr);
                if (debug)
	...
	else if (bootpnat && ((**pskb).mac.ethernet)->h_proto == __constant_htons(ETH_P_IP) &&
