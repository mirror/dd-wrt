			      ...
			      unsigned long address)
{
	...
	if (in_interrupt() || !mm)
		goto no_context;
#if 0
	printk(KERN_EMERG "[%s:%d:%08lx:%ld:%08lx]\n", current->comm, current->pid,
...
#if 0
		printk(KERN_EMERG "do_page_fault() #2: sending SIGSEGV to %s for illegal %s\n"
...
no_context:
	...
	if (fixup) {
		...
		if (development_version)
			printk(KERN_EMERG "%s: Exception at [<%lx>] (%lx)\n",
	 ...
	 */
	printk(KERN_EMERG "Unable to handle kernel paging request at virtual "
