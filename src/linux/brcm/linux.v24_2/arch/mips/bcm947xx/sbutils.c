...
/* local prototypes */
static sb_info_t *sb_doattach(sb_info_t * si, uint devid, osl_t * osh, void *regs, uint bustype, void *sdh, char **vars, uint * varsz);
static void sb_scan(sb_info_t * si);
static uint _sb_coreidx(sb_info_t * si);
static uint sb_pcidev2chip(uint pcidev);
static uint sb_chip2numcores(uint chip);
static bool sb_ispcie(sb_info_t * si);
uint8 sb_find_pci_capability(sb_info_t * si, uint8 req_cap_id, uchar * buf, uint32 * buflen);
 ...
 */
sb_t *BCMINITFN(sb_attach) (uint devid, osl_t * osh, void *regs, uint bustype, void *sdh, char **vars, uint * varsz) {
	sb_info_t * si;
...
sb_t *BCMINITFN(sb_kattach) (osl_t * osh) {
	...
	if (!ksi_attached) {
		...
		if (((cid & CID_ID_MASK) == BCM4712_CHIP_ID) && ((cid & CID_PKG_MASK) != BCM4712LARGE_PKG_ID) && ((cid & CID_REV_MASK) <= (3 << CID_REV_SHIFT))) {
			uint32 *scc, val;
		...
		if (sb_doattach(&ksi, BCM4710_DEVICE_ID, osh, (void *)regs, SB_BUS, NULL, NULL, NULL) == NULL) {
			return NULL;
		} else
			ksi_attached = TRUE;
	}
...
static sb_info_t *BCMINITFN(sb_doattach) (sb_info_t * si, uint devid, osl_t * osh, void *regs, uint bustype, void *sdh, char **vars, uint * varsz) {
	uint origidx;
	chipcregs_t *cc;
	sbconfig_t *sb;
	uint32 w;
	...
	//printk(KERN_EMERG "sb_bp version\n");
	...
	if (sb_coreid(&si->sb) != SB_CC)
		cc = NULL;
//printk(KERN_EMERG "sb_get chip id and rev\n");
...
//printk(KERN_EMERG "sb_doattach->numcores\n");
...
//      si->sb.boardtype = (w >> 16) & 0xffff;
	...
	if (PCIE(si) && (si->sb.chip == BCM4321_CHIP_ID))
		si->pcie_war_ovr = ((si->sb.boardvendor == VENDOR_APPLE) &&
				    ((uint8) getintvar(pvars, "sromrev") == 4) && ((uint8) getintvar(pvars, "boardrev") <= 0x71)) || ((uint32) getintvar(pvars, "boardflags2") & BFL2_PCIEWAR_OVR);
	...
	if (PCIE(si) && (((si->sb.chip == BCM4311_CHIP_ID) && (si->sb.chiprev == 2)) || ((si->sb.chip == BCM4312_CHIP_ID) && (si->sb.chiprev == 0))))
...
void BCMINITFN(sb_war42780_clkreq) (sb_t * sbh, bool clkreq) {
	sb_info_t * si;
...
static void BCMINITFN(sb_war43448) (sb_t * sbh) {
	...
	if (si->pcie_polarity != 0)
		sb_pcie_mdiowrite((void *)(uintptr) & si->sb, MDIODATA_DEV_RX, SERDES_RX_CTRL, si->pcie_polarity);
}
...
static void BCMINITFN(sb_war32414_forceHT) (sb_t * sbh, bool forceHT) {
	...
	if (forceHT)
		val = SYCC_HR;
	sb_corereg(sbh, SB_CC_IDX, OFFSETOF(chipcregs_t, system_clk_ctl), SYCC_HR, val);
}
...
//printk(KERN_EMERG "sb_corereg\n");
	...
	if (si->sb.bustype == SB_BUS) {
		...
		if ((si->coreid[coreidx] == SB_CC) && ((si->sb.buscoretype == SB_PCIE) || (si->sb.buscorerev >= 13))) {
			 ...
			 */
			fast = TRUE;
			if ((si->sb.buscoretype == SB_PCIE) || (si->sb.buscorerev >= 13))
				r = (uint32 *) ((char *)si->curmap + PCI_16KB0_PCIREGS_OFFSET + regoff);
			else
				r = (uint32 *) ((char *)si->curmap + ((regoff >= SBCONFIGOFF) ? PCI_BAR0_PCISBR_OFFSET : PCI_BAR0_PCIREGS_OFFSET) + regoff);
		}
	}
#endif				/* notyet */
	...
	if (regoff >= SBCONFIGOFF)
		...
		if ((si->sb.chip == BCM5354_CHIP_ID) && (coreidx == SB_CC_IDX) && (regoff == OFFSETOF(chipcregs_t, watchdog))) {
			w = val;
		} else
			w = R_REG(si->osh, r);
	}
...
uint8 sb_pcieL1plldown(sb_t * sbh)
{
	...
	sb_pcie_writereg((void *)(uintptr) sbh, (void *)PCIE_PCIEREGS, PCIE_DLLP_PCIE11, 0);
...
void *sb_setcoreidx(sb_t * sbh, uint coreidx)
{
	...
	uint32 sbaddr;
	uint8 tmp;
//printk(KERN_EMERG "sb_setcoreidx\n");
	...
	case SB_BUS:
		...
		if (!si->regs[coreidx]) {
			...
			printk(KERN_INFO "map 0x%08X to 0x%08X\n", sbaddr, si->regs[coreidx]);
...
bool sb_taclear(sb_t * sbh)
{
	...
	if (BUSTYPE(si->sb.bustype) == PCI_BUS) {
		...
		/* timeout */
		imstate = sb_corereg(sbh, si->sb.buscoreidx, SBCONFIGOFF + OFFSETOF(sbconfig_t, sbimstate), 0, 0);
		...
		if (inband) {
			...
			if (si->sb.sonicsrev == SONICS_2_2) ;
				...
				if (imerrlog & SBTMEL_EC) {
					imerrloga = sb_corereg(sbh, si->sb.buscoreidx, SBIMERRLOGA, 0, 0);
					/* clear errlog */
...
void sb_core_tofixup(sb_t * sbh)
{
	...
	if (BUSTYPE(si->sb.bustype) == SB_BUS) {
		SET_SBREG(si, &sb->sbimconfiglow, SBIMCL_RTO_MASK | SBIMCL_STO_MASK, (0x5 << SBIMCL_RTO_SHIFT) | 0x3);
	} else {
		if (sb_coreid(sbh) == SB_PCI) {
			SET_SBREG(si, &sb->sbimconfiglow, SBIMCL_RTO_MASK | SBIMCL_STO_MASK, (0x3 << SBIMCL_RTO_SHIFT) | 0x2);
		} else {
...
uint32 sb_set_initiator_to(sb_t * sbh, uint32 to, uint idx)
{
	...
	sbconfig_t *sb;
//printk(KERN_EMERG "sb_set_initiator_to\n");
...
void sb_watchdog(sb_t * sbh, uint ticks)
{
	...
	if (sbh->chip == BCM4328_CHIP_ID && ticks != 0)
		sb_corereg(sbh, SB_CC_IDX, OFFSETOF(chipcregs_t, min_res_mask), PMURES_BIT(RES4328_ROM_SWITCH), PMURES_BIT(RES4328_ROM_SWITCH));
#endif

	/* instant NMI */
	switch (si->gpioid) {
	case SB_CC:
//              printk(KERN_EMERG "set watchdog to %d ticks\n",ticks);
...
void BCMINITFN(sb_pci_setup) (sb_t * sbh, uint coremask) {
	...
	if (PCIE(si)) {
		if ((si->sb.buscorerev == 0) || (si->sb.buscorerev == 1)) {
			w = sb_pcie_readreg((void *)(uintptr) sbh, (void *)(uintptr) PCIE_PCIEREGS, PCIE_TLP_WORKAROUNDSREG);
			w |= 0x8;
			sb_pcie_writereg((void *)(uintptr) sbh, (void *)(uintptr) PCIE_PCIEREGS, PCIE_TLP_WORKAROUNDSREG, w);
		}

		if (si->sb.buscorerev == 1) {
			w = sb_pcie_readreg((void *)(uintptr) sbh, (void *)(uintptr) PCIE_PCIEREGS, PCIE_DLLP_LCREG);
			w |= (0x40);
			sb_pcie_writereg((void *)(uintptr) sbh, (void *)(uintptr) PCIE_PCIEREGS, PCIE_DLLP_LCREG, w);
		}
		...
		if ((si->sb.buscorerev >= 3) && (si->sb.buscorerev <= 5)) {
			w = sb_pcie_readreg((void *)(uintptr) sbh, (void *)(uintptr) PCIE_PCIEREGS, PCIE_DLLP_PMTHRESHREG);
			w &= ~(PCIE_L1THRESHOLDTIME_MASK);
			w |= (PCIE_L1THRESHOLD_WARVAL << PCIE_L1THRESHOLDTIME_SHIFT);
			sb_pcie_writereg((void *)(uintptr) sbh, (void *)(uintptr) PCIE_PCIEREGS, PCIE_DLLP_PMTHRESHREG, w);
...
uint32 sb_gpiocontrol(sb_t * sbh, uint32 mask, uint32 val, uint8 priority)
{
	 ...
	 */
	if ((priority != GPIO_HI_PRIORITY) && (BUSTYPE(si->sb.bustype) == SB_BUS) && (val || mask)) {
		mask = priority ? (sb_gpioreservation & mask) : ((sb_gpioreservation | mask) & ~(sb_gpioreservation));
		val &= mask;
	}
...
uint32 sb_gpiotimerval(sb_t * sbh, uint32 mask, uint32 gpiotimerval)
{
	...
	return (sb_corereg(sbh, SB_CC_IDX, OFFSETOF(chipcregs_t, gpiotimerval), mask, gpiotimerval));
}
...
void *BCMINITFN(sb_gpio_handler_register) (sb_t * sbh, uint32 event, bool level, gpio_handler_t cb, void *arg) {
	sb_info_t * si;
	gpioh_item_t *gi;
...
void sb_dump(sb_t * sbh, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "si %p chip 0x%x chiprev 0x%x boardtype 0x%x boardvendor 0x%x bus %d\n", si, si->sb.chip, si->sb.chiprev, si->sb.boardtype, si->sb.boardvendor, si->sb.bustype);
	bcm_bprintf(b, "osh %p curmap %p\n", si->osh, si->curmap);
	bcm_bprintf(b, "sonicsrev %d ccrev %d buscoretype 0x%x buscorerev %d curidx %d\n", si->sb.sonicsrev, si->sb.ccrev, si->sb.buscoretype, si->sb.buscorerev, si->curidx);
...
void sb_dumpregs(sb_t * sbh, struct bcmstrbuf *b)
{
	...
	for (i = 0; i < si->numcores; i++) {
		...
		bcm_bprintf(b, "sbtmstatelow 0x%x sbtmstatehigh 0x%x sbidhigh 0x%x "
			    ...
			    R_SBREG(si, &sb->sbtmstatelow), R_SBREG(si, &sb->sbtmstatehigh), R_SBREG(si, &sb->sbidhigh), R_SBREG(si, &sb->sbimstate), R_SBREG(si, &sb->sbimconfiglow), R_SBREG(si, &sb->sbimconfighigh));
	}
...
void sb_view(sb_t * sbh)
{
	...
	if (si->sb.sonicsrev > SONICS_2_2)
		SB_ERROR(("sbimerrlog 0x%x sbimerrloga 0x%x\n", sb_corereg(sbh, sb_coreidx(&si->sb), SBIMERRLOG, 0, 0), sb_corereg(sbh, sb_coreidx(&si->sb), SBIMERRLOGA, 0, 0)));
	...
	SB_ERROR(("sbadmatch3 0x%x sbadmatch2 0x%x sbadmatch1 0x%x\n", R_SBREG(si, &sb->sbadmatch3), R_SBREG(si, &sb->sbadmatch2), R_SBREG(si, &sb->sbadmatch1)));
	...
	SB_ERROR(("sbactcnfg 0x%x sbflagst 0x%x sbidlow 0x%x sbidhigh 0x%x\n", R_SBREG(si, &sb->sbactcnfg), R_SBREG(si, &sb->sbflagst), R_SBREG(si, &sb->sbidlow), R_SBREG(si, &sb->sbidhigh)));
}
...
static uint sb_slowclk_src(sb_info_t * si)
{
	...
	if (si->sb.ccrev < 6) {
		if ((BUSTYPE(si->sb.bustype) == PCI_BUS) && (OSL_PCI_READ_CONFIG(si->osh, PCI_GPIO_OUT, sizeof(uint32)) & PCI_CFG_GPIO_SCS))
			return (SCC_SS_PCI);
		else
			return (SCC_SS_XTAL);
...
static uint sb_slowclk_freq(sb_info_t * si, bool max_freq)
{
	...
	if (si->sb.ccrev < 6) {
		...
		div = 4 * (((R_REG(si->osh, &cc->slow_clk_ctl) & SCC_CD_MASK) >> SCC_CD_SHIFT)
			   + 1);
...
void BCMINITFN(sb_clkctl_init) (sb_t * sbh) {
	...
	if ((si->sb.chip == BCM4321_CHIP_ID) && (si->sb.chiprev < 2))
		W_REG(si->osh, &cc->chipcontrol, (si->sb.chiprev == 0) ? CHIPCTRL_4321A0_DEFAULT : CHIPCTRL_4321A1_DEFAULT);
	...
	if (si->sb.ccrev >= 10)
		SET_REG(si->osh, &cc->system_clk_ctl, SYCC_CD_MASK, (ILP_DIV_1MHZ << SYCC_CD_SHIFT));
...
uint16 BCMINITFN(sb_clkctl_fast_pwrup_delay) (sb_t * sbh) {
	...
	fpdelay = (((R_REG(si->osh, &cc->pll_on_delay) + 2) * 1000000) + (slowminfreq - 1)) / slowminfreq;
...
bool sb_clkctl_clk(sb_t * sbh, uint mode)
{
	...
	if (sb_setcore(sbh, SB_MIPS33, 0) && (sb_corerev(&si->sb) <= 7) && (BUSTYPE(si->sb.bustype) == SB_BUS) && (si->sb.ccrev >= 10))
		goto done;
	...
	case CLK_FAST:		/* force fast (pll) clock */
		...
		if (R_REG(si->osh, &cc->capabilities) & CC_CAP_PMU) {
			SPINWAIT(((R_REG(si->osh, &cc->clk_ctl_st) & CCS_HTAVAIL) == 0), PMU_MAX_TRANSITION_DLY);
...
void sb_register_intr_callback(sb_t * sbh, void *intrsoff_fn, void *intrsrestore_fn, void *intrsenabled_fn, void *intr_arg)
{
	sb_info_t *si;
...
void sb_clkctl_dump(sb_t * sbh, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "pll_on_delay 0x%x fref_sel_delay 0x%x ", cc->pll_on_delay, cc->fref_sel_delay);
	...
	if (BUSTYPE(si->sb.bustype) == PCI_BUS)
		bcm_bprintf(b, "gpioout 0x%x gpioouten 0x%x ", OSL_PCI_READ_CONFIG(si->osh, PCI_GPIO_OUT, sizeof(uint32)), OSL_PCI_READ_CONFIG(si->osh, PCI_GPIO_OUTEN, sizeof(uint32)));
	bcm_bprintf(b, "\n");
...
uint16 BCMINITFN(sb_d11_devid) (sb_t * sbh) {
	...
	if (sbh->chip == BCM4328_CHIP_ID && (sbh->chippkg == BCM4328USBDUAL_PKG_ID || sbh->chippkg == BCM4328SDIODUAL_PKG_ID))
...
int BCMINITFN(sb_corepciid) (sb_t * sbh, uint func, uint16 * pcivendor, uint16 * pcidevice, uint8 * pciclass, uint8 * pcisubclass, uint8 * pciprogif, uint8 * pciheader) {
...
static int sb_pcie_mdioread(sb_info_t * si, uint physmedia, uint regaddr, uint * regval)
{
	uint mdiodata;
	uint i = 0;
	sbpcieregs_t *pcieregs;
	...
	mdiodata = MDIODATA_START | MDIODATA_READ | (physmedia << MDIODATA_DEVADDR_SHF) | (regaddr << MDIODATA_REGADDR_SHF) | MDIODATA_TA;
...
static int sb_pcie_mdiowrite(sb_info_t * si, uint physmedia, uint regaddr, uint val)
{
	uint mdiodata;
	uint i = 0;
	sbpcieregs_t *pcieregs;
	...
	mdiodata = MDIODATA_START | MDIODATA_WRITE | (physmedia << MDIODATA_DEVADDR_SHF) | (regaddr << MDIODATA_REGADDR_SHF) | MDIODATA_TA | val;
...
int BCMINITFN(sb_devpath) (sb_t * sbh, char *path, int size) {
	...
	case PCI_BUS:
		...
		slen = snprintf(path, (size_t)size, "pci/%u/%u/", OSL_PCI_BUS((SB_INFO(sbh))->osh), OSL_PCI_SLOT((SB_INFO(sbh))->osh));
		break;
	case PCMCIA_BUS:
 ...
 */
static char *BCMINITFN(sb_devpathvar) (sb_t * sbh, char *var, int len, const char *name) {
	uint path_len;
...
int sb_pci_fixcfg(sb_info_t * si)
{
	...
	if (PCIE_ASPMWARS(si)) {
		w = sb_pcie_readreg((void *)(uintptr) & si->sb, (void *)PCIE_PCIEREGS, PCIE_PLP_STATUSREG);
		...
		if ((w & PCIE_PLP_POLARITYINV_STAT) == 0) {
			...
			si->pcie_polarity = (SERDES_RX_CTRL_FORCE | SERDES_RX_CTRL_POLARITY);
		}
	...
	if ((value = getvar(NULL, name))) {
		OSL_PCI_WRITE_CONFIG(si->osh, PCI_BAR0_WIN, sizeof(uint32), bcm_strtoul(value, NULL, 16));
...
void sb_pcie_war_ovr_disable(sb_t * sih)
{
}

void sb_core_cflags_wo(sb_t * sih, uint32 mask, uint32 val)
{
	sb_info_t *sii;
	sbconfig_t *sb;
	uint32 w;
	...
	/* mask and set */
	w = (R_SBREG(sii, &sb->sbtmstatelow) & ~(mask << SBTML_SICF_SHIFT)) | (val << SBTML_SICF_SHIFT);
	W_SBREG(sii, &sb->sbtmstatelow, w);
}

/* set/clear core-specific control flags */
uint32 sb_core_cflags(sb_t * sih, uint32 mask, uint32 val)
{
	sb_info_t *sii;
	sbconfig_t *sb;
	uint32 w;
...
uint32 sb_core_sflags(sb_t * sih, uint32 mask, uint32 val)
{
	sb_info_t *sii;
	sbconfig_t *sb;
	uint32 w;
	...
	/* mask and set */
	if (mask || val) {
		w = (R_SBREG(sii, &sb->sbtmstatehigh) & ~(mask << SBTMH_SISF_SHIFT)) | (val << SBTMH_SISF_SHIFT);
...
void sb_pmu_spuravoid(sb_t * sbh, osl_t * osh, bool spuravoid)
{
	chipcregs_t *cc;
	uint origidx;
	uint32 tmp;
...
void sb_4329_tweak(sb_t * sih, uint32 mask, uint32 val)
{
	sb_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
void sb_4329_pmu_voltage(sb_t * sih)
{
	sb_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
void sb_4329_vbatmeas_on(sb_t * sih, uint32 * save_reg0, uint32 * save_reg5)
{
	sb_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
void sb_4329_vbatmeas_off(sb_t * sih, uint32 save_reg0, uint32 save_reg5)
{
	sb_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
bool sb_ldo_war(sb_t * sih, uint devid)
{
	...
	rev_id &= 0xff;
	if (!(((devid == BCM4322_CHIP_ID) || (devid == BCM4322_D11N_ID) || (devid == BCM4322_D11N2G_ID) || (devid == BCM4322_D11N5G_ID)) && (rev_id == 0)))
		return TRUE;
...
bool BCMINITFN(sb_pci_war16165) (sb_t * sih) {
	sb_info_t *sii;
...
static volatile uint32 *sb_admatch(sb_info_t * sii, uint asidx)
{
	sbconfig_t *sb;
...
uint32 sb_addrspace(sb_t * sih, uint asidx)
{
	sb_info_t *sii;
...
uint32 sb_addrspacesize(sb_t * sih, uint asidx)
{
	sb_info_t *sii;
