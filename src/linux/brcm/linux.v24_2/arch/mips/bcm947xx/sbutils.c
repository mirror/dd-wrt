 ...
 */
sb_t *BCMINITFN (sb_attach) (uint devid, osl_t * osh, void *regs,
					   ...
					   char **vars, uint * varsz)
{
  ...
  if (bustype == PCI_BUS)
    {
      if (OSL_PCI_READ_CONFIG (si->osh, PCI_SPROM_CONTROL, sizeof (uint32)) ==
	  0xffffffff)
	{
...
//	si->sb.boardtype = (w >> 16) & 0xffff;
  ...
  if (si->sb.ccrev >= 16)
	...
	w = DEFAULT_GPIOTIMERVAL;
      sb_corereg (&si->sb, SB_CC_IDX, OFFSETOF (chipcregs_t, gpiotimerval),
		  ~0, w);
    }
...
static void BCMINITFN (sb_war43448_aspm) (sb_t * sbh)
{
  ...
  w =
    OSL_PCI_READ_CONFIG (si->osh, si->pciecap_lcreg_offset, sizeof (uint32));
  if (!si->pcie_war_ovr)
    w |= PCIE_ASPM_ENAB;
  else
    w &= ~PCIE_ASPM_ENAB;
...
uint32
sb_pcielcreg (sb_t * sbh, uint32 mask, uint32 val)
{
  sb_info_t *si;
  uint32 reg_val;
  uint8 offset;
...
sb_pci_pmecap (sb_t * sbh)
{
  ...
  if (!si->pmecap_offset)
    {
      cap_ptr =
	sb_find_pci_capability (si, PCI_CAP_POWERMGMTCAP_ID, NULL, NULL);
      if (!cap_ptr)
	return FALSE;
      ...
      pmecap =
	OSL_PCI_READ_CONFIG (si->osh, si->pmecap_offset, sizeof (uint32));
...
sb_pci_pmeen (sb_t * sbh)
{
  ...
  w =
    OSL_PCI_READ_CONFIG (si->osh, si->pmecap_offset + PME_CSR_OFFSET,
			 sizeof (uint32));
  w |= (PME_CSR_PME_EN);
...
#endif
	  sb_serr_clear (si);
	  OSL_PCI_WRITE_CONFIG (si->osh, PCI_INT_STATUS, sizeof (uint32),
				stcmd);
	}
...
void BCMINITFN (sb_pci_setup) (sb_t * sbh, uint coremask)
{
  ...
  if (PCIE (si))
      ...
      if (si->sb.buscorerev == 1)
	{
	  ...
	  w |= (0x40);
	  sb_pcie_writereg ((void *) (uintptr) sbh,
			    (void *) (uintptr) PCIE_PCIEREGS, PCIE_DLLP_LCREG,
			    w);
	}
...
#ifdef BCMDBG
void
sb_dump (sb_t * sbh, struct bcmstrbuf *b)
{
  sb_info_t *si;
  uint i;
...
void
sb_dumpregs (sb_t * sbh, struct bcmstrbuf *b)
{
  sb_info_t *si;
  sbconfig_t *sb;
  uint origidx;
  ...
  for (i = 0; i < si->numcores; i++)
    {
      ...
      bcm_bprintf (b, "sbtmstatelow 0x%x sbtmstatehigh 0x%x sbidhigh 0x%x "
		   ...
		   R_SBREG (si, &sb->sbtmstatelow), R_SBREG (si,
							     &sb->
							     sbtmstatehigh),
		   R_SBREG (si, &sb->sbidhigh), R_SBREG (si, &sb->sbimstate),
...
sb_view (sb_t * sbh)
{
  ...
  if (si->sb.sonicsrev > SONICS_2_2)
    SB_ERROR (("sbimerrlog 0x%x sbimerrloga 0x%x\n",
	       sb_corereg (sbh, sb_coreidx (&si->sb), SBIMERRLOG, 0, 0),
	       sb_corereg (sbh, sb_coreidx (&si->sb), SBIMERRLOGA, 0, 0)));
...
sb_slowclk_freq (sb_info_t * si, bool max_freq)
{
  ...
  if (si->sb.ccrev < 6)
  ...
  else if (si->sb.ccrev < 10)
    {
      div =
	4 *
	(((R_REG (si->osh, &cc->slow_clk_ctl) & SCC_CD_MASK) >> SCC_CD_SHIFT)
	 + 1);
...
sb_clkctl_clk (sb_t * sbh, uint mode)
{
  ...
  switch (mode)
      ...
      if (si->sb.ccrev < 10)
	{
	  ...
	  SET_REG (si->osh, &cc->slow_clk_ctl, (SCC_XC | SCC_FS | SCC_IP),
		   SCC_IP);
	}
...
void
sb_clkctl_dump (sb_t * sbh, struct bcmstrbuf *b)
{
  sb_info_t *si;
  chipcregs_t *cc;
  uint origidx;
...
sb_pci_fixcfg (sb_info_t * si)
{
  ...
  if (PCIE_ASPMWARS (si))
    {
      w =
	sb_pcie_readreg ((void *) (uintptr) & si->sb, (void *) PCIE_PCIEREGS,
			 PCIE_PLP_STATUSREG);
      ...
      else
	{
      ...
      w =
	OSL_PCI_READ_CONFIG (si->osh, si->pciecap_lcreg_offset,
			     sizeof (uint32));
