...
config_cmd (sb_t * sbh, uint bus, uint dev, uint func, uint off)
{
  ...
  if (bus == 1)
      ...
      if (dev < PCI_SLOT_MAX)
	{
	  ...
	  win =
	    (SBTOPCI_CFG0 | ((1 << (dev + PCI_SLOTAD_MAP)) & SBTOPCI1_MASK));
	...
	(dev << PCICFG_SLOT_SHIFT) | (func << PCICFG_FUN_SHIFT) | (off & ~3);
    }
 ...
 */
static int
sb_read_config (sb_t * sbh, uint bus, uint dev, uint func, uint off,
		void *buf, int len)
{
  pci_config_regs *cfg;
...
static int
sb_write_config (sb_t * sbh, uint bus, uint dev, uint func, uint off,
		 void *buf, int len)
{
  uint coreidx;
  void *regs;
