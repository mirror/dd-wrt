 ...
 *
 * $Id: 005-diag_led.patch 1969 2005-09-22 01:32:36Z nbd $
 */
...
static void set_gpio(uint32 mask, uint32 value) {
	sb_gpiocontrol(sbh,mask,0,0);
	sb_gpioouten(sbh,mask,mask,0);
	sb_gpioout(sbh,mask,value,0);
}
		...
		void *buffer, size_t *lenp)
{
	...
	if (reset_gpio) {
		sb_gpiocontrol(sbh,reset_gpio,reset_gpio,0);
		sb_gpioouten(sbh,reset_gpio,0,0);
