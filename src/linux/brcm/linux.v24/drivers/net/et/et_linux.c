...
int et_rx_after_tx_prev=0;
int et_tx_after_rx_prev=0;
int et_rx_after_tx=0;
int et_tx_after_rx=0;
int et_rx_monitor=0;
int et_timer=0;
int et_rx_monitor_timer=0;
#endif
...
et_init(et_info_t *et)
{
	...
	{	
		/* added by Joey for monitoring some event */
		et_rx_after_tx_prev=0;
		et_tx_after_rx_prev=0;
		et_rx_after_tx=0;
		et_tx_after_rx=0;
		et_rx_monitor=0;
	}
...
et_watchdog(ulong data)
{
	...
	if (et->etc->unit==1)
	{
		...
		/* rule: if reset, then don't reset for 10 seconds 	*/
		/* rule: if meet sudden rx/tx drop			*/

		et_timer++;

		if (et_timer>1000)
		{
			et_timer=et_rx_monitor_timer=0;
		}

		if(et->etc->linkstate)
		{ 
			if(et_tx_after_rx_prev!=0 && et_tx_after_rx_prev>et_tx_after_rx && et_tx_after_rx<=50)	
			{
				if((et_tx_after_rx_prev-et_tx_after_rx)>10) reset=1;
			}
			if(et_rx_after_tx_prev!=0 && et_rx_after_tx_prev>et_rx_after_tx && et_rx_after_tx<=50)
			{
				if((et_rx_after_tx_prev-et_rx_after_tx)>10) reset = 2;
			}

			if ((et_tx_after_rx>0 && et_rx_after_tx==0))
			{
				et_rx_monitor++;
		...
		et_rx_after_tx_prev=et_rx_after_tx;
		et_tx_after_rx_prev=et_tx_after_rx;
		et_tx_after_rx=et_rx_after_tx=0;
	}	
#endif
...
et_ethtool(et_info_t *et, struct ethtool_cmd *ecmd)
{
	...
	case ETHTOOL_GSET:
		...
		ecmd->port = PORT_TP;
#if defined(ETROBO)
		ecmd->phy_address = etc_get_ports(et->etc);
#else
		ecmd->phy_address = 0;
#endif
