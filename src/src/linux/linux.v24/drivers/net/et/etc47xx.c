...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
...
#ifndef ORIGINAL
	if (etc->chip == BCM5365_DEVICE_ID)
	{
		//printk("5365 flag\n");
		boardflags |= BFL_ENETROBO | BFL_ENETVLAN;
	}
#endif
	...
	if ((ch->di = dma_attach(osh, name, ch->sbh, 
				 ...
				 NTXD, NRXD, RXBUFSZ, NRXBUFPOST, HWRXOFF, &et_msg_level)) == NULL) {
...
chipphyinit(struct bcm4xxx *ch, uint phyaddr)
{
	...
	if( phyid == 0x55210022) 
	{
		// 20030806 added by Joey 		
		// printf("Pagesel: %x\n", pagesel);
		// Select Page 0, set bit 12:13 to 11
		chipphywr( ch, phyaddr, 28, (uint16) (chipphyrd( ch, phyaddr, 28 ) & 0x0fff));
			
		ET_TRACE(("default: %x\n", chipphyrd( ch, phyaddr, 30)));
		...
		/* LED Blinking Rate */
		chipphywr( ch, phyaddr, 29, 0x0001);
		/* LED Action */ 
		chipphywr( ch, phyaddr, 30, 0x0004);		
		chipphywr( ch, phyaddr, 28, (uint16) (chipphyrd( ch, phyaddr, 28 ) & 0x0fff));					
	}
...
#ifndef ORIGNAL
/* 2003/05/26, by Joey */
/* Scan routine for interfacing with Reltek 8139C */
#define LINKSTATUS 0x04
#define ADAPTATION 0xff00

void chipphyupdate(struct bcm4xxx *ch, uint phyaddr)
{
	uint16 tmp;

	if ((chipphyrd(ch, phyaddr, 1)&LINKSTATUS)) /* Link up */
	{
		tmp = chipphyrd(ch, phyaddr, 20);
		ET_TRACE(("Link up %x", tmp));
		//tmp |= 0x0090;
		chipphywr(ch, phyaddr, 20, (tmp&(~ADAPTATION))|0x8100);
		//chipphywr(ch, phyaddr, 20, (tmp&(~ADAPTATION))|0x8100);
		tmp = chipphyrd(ch, phyaddr, 20);
		ET_TRACE((" %x\n", tmp));
	}
	...
	{
		chipphywr(ch, phyaddr, 20, 0xC000);
		tmp = chipphyrd(ch, phyaddr, 20);
		ET_TRACE(("Link down %x\n", tmp));
	}
}

void dbg_chipphyrd(struct bcm4xxx *ch, uint phyaddr, uint reg)
{
	chipphyrd(ch, phyaddr, reg);
}
#endif

#if defined(ETROBO)
uint chipportstatus(struct bcm4xxx *ch)
{
	return(robo_get_ports(ch->robo));
}
#endif
