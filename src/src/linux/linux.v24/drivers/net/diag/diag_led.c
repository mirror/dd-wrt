 ...
 * 2005/10/29 support for latest broadcom sources (BrainSlayer)
 */
...
static void set_gpio(uint32 mask, uint32 value) {
	sb_gpiocontrol(sbh,mask,0, 1);
	sb_gpioouten(sbh,mask,mask, 1);
	sb_gpioout(sbh,mask,value, 1);
}
		...
		void *buffer, size_t *lenp)
{
	...
	if (reset_gpio) {
		sb_gpiocontrol(sbh,reset_gpio,reset_gpio, 1);
		sb_gpioouten(sbh,reset_gpio,0, 1);
