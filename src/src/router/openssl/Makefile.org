...
OPTIONS=no-asm no-ec no-engine no-err no-fips no-hw no-krb5 no-threads
CONFIGURE_ARGS=linux-mipsel no-asm no-ec no-engine no-err no-fips no-hw no-krb5 no-threads
...
CFLAG= $(COPTS) -DDSO_DLFCN -DHAVE_DLFCN_H -DOPENSSL_NO_KRB5 -DOPENSSL_NO_ASM -DOPENSSL_NO_EC -DOPENSSL_NO_ENGINE -DOPENSSL_NO_ERR -DOPENSSL_NO_HW -DL_ENDIAN -DTERMIO $(CCOPTS) -Wall
DEPFLAG= -DOPENSSL_NO_EC -DOPENSSL_NO_ENGINE 
PEX_LIBS= 
EX_LIBS= -ldl
EXE_EXT= 
ARFLAGS= 
