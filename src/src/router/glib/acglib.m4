...
m4_define([AC_TYPE_NAME], m4_translit(glib_sizeof_$3, [[[a-z *]]], [[[A-Z_P]]]))dnl
...
[dnl The name to #define.
m4_define([AC_TYPE_NAME], m4_translit(glib_byte_contents_$3, [[[a-z *]]], [[[A-Z_P]]]))dnl
dnl The cache variable name.
m4_define([AC_CV_NAME], m4_translit(glib_cv_byte_contents_$3, [[[ *]]], [[[_p]]]))dnl
...
_______EOF
  ...
  AC_TRY_RUN([`cat confrun.c`], [glib_macro_sysdefs=done],
	[glib_macro_sysdefs=failed])
fi
case "x$glib_macro_sysdefs" in
xdone)
	;;
*)
	for glib_sysdef_input in $2 ; do
...
AC_MSG_RESULT([$glib_macro_sysdefs])
rm -f confrun.c
])
