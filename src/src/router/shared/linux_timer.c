...
int
clock_gettime (clockid_t clock_id,	/* clock ID (always CLOCK_REALTIME) */
...
int
timer_create (clockid_t clock_id,	/* clock ID (always CLOCK_REALTIME) */
...
int
timer_delete (timer_t timerid	/* timer ID */
  )
{
...
int
timer_connect (timer_t timerid,	/* timer ID */
...
*/
int
timer_change_settime (timer_t timerid,	/* timer ID */
...
int
timer_settime (timer_t timerid,	/* timer ID */
...
static int
count_queue (struct event *event_queue)
{
  struct event *event;
  int i = 0;
  for (event = event_queue; event; event = event->next)
  ...
  for (event = event_queue; event; event = event->next)
    {
      printf ("#%d (0x%x)->0x%x: \t%d sec %d usec\t%p\n",
	      ...
	      (int) event->it_value.tv_sec, (int) event->it_value.tv_usec,
	      event->func);
      if (i > g_maxevents)
	{
	  printf ("...(giving up)\n");
  ...
  do
      ...
      if (!(event->flags & TFLAG_CANCELLED))
	{
	  ...
	  if (timerisset (&event->it_interval))
		...
		{
		  if (timercmp
		      (&(event->it_value), &((*ppevent)->it_value), <))
		    {
		      // if the proposed event will trigger sooner than the next event
		      // in the queue, we will insert the new event just before the next one.
		      //
