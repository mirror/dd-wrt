...
class CInMemoryStream:public IInStream,
    public IStreamGetSize, public CMyUnknownImp {
public:
	CInMemoryStream(const Bytef * data, UInt64 size):m_data(data),
...
class COutMemoryStream: public IOutStream, public CMyUnknownImp {
public:
	...
	STDMETHOD(WritePart) (const void *data, UInt32 size,
			      UInt32 * processedSize) {
				   ...
				   int level, int fb, int lc, int lp, int pb)
{
	...
	if (fb < 0)
		fb = ZLIB_FB;
	UInt32 dictSize = 1 << (level + 14);
	UInt32 reduceSize = sourceLen;
	...
	properties[0].ulVal = UInt32(dictSize);
	...
	if (encoderSpec->SetCoderProperties(propIDs, properties, kNumProps) !=
	    S_OK)
...
//fprintf(stderr,"try method [pb:%d lc:%d lp:%d fb:%d]\n",takepbvalue,takelcvalue,takelpvalue,testfb);
	int ret =
	    compress2_lzma_test(test2, &test3len, testsource, testsourcelen,
				testlevel, testfb, takelcvalue, takelpvalue,
				takepbvalue);
...
ZEXTERN int ZEXPORT uncompress OF((Bytef * dest, uLongf * destLen,
				   const Bytef * source, uLong sourceLen))
{
	CInMemoryStream *inStreamSpec =
	    new CInMemoryStream(source + 4, sourceLen - 4);
	CMyComPtr < ISequentialInStream > inStream = inStreamSpec;
