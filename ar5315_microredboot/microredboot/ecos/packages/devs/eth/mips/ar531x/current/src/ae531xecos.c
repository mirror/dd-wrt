...
#include <pkgconf/devs_eth_mips_mips32_ar531x.h>
...
ae531x_init(struct cyg_netdevtab_entry *tab)
{
	...
	diag_printf("Couldn't find valid MAC address for enet%d. Using default!\n",unit);
             ...
             int flags)
{
    ...
    /* If we are the AR2312 or AR5312, then we need to enable Ethernet
       Phy 1 on the local bus using the local configuration register.
       By default, the 2312 and 5312 have Eth Phy 1 disabled.  By
       default, the 2313 has the Enet phy enabled already. */

    ae531x_enablePhy1(MACInfo);
#endif
    ...
    if (total_len > ETH_MAX_MTU) {
        AE531X_PRINT(AE531X_DEBUG_ERROR,
                  ("eth%d Tx: length %d too long.  mtu=%d, trailer=%d\n",
                   MACInfo->unit, total_len, ETH_MAX_MTU, CYGNUM_PHY_TRAILER_SIZE));
