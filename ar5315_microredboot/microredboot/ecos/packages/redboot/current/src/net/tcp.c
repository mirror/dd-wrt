...
#ifdef BSP_LOG
static char *flags_to_str(octet f)
{
	static char str[7], *p;
 ...
 */
static void tcp_send(tcp_socket_t * s, int flags, int resend)
{
	tcp_header_t *tcp;
	ip_header_t *ip;
	pktbuf_t *pkt = &s->pkt;
	...
	BSPLOG(bsp_log("tcp_send: state[%d] flags[%s] ack[%x] data[%d].\n",
		       s->state, flags_to_str(tcp->flags), s->ack,
		       s->data_bytes));
	...
	} else if ((tcp->flags & (TCP_FLAG_FIN | TCP_FLAG_SYN))
		   || s->data_bytes)
 ...
 */
static void send_reset(pktbuf_t * pkt, ip_route_t * r)
{
 ...
 */
static void unlink_socket(tcp_socket_t * s)
{
...
static void free_rxlist(tcp_socket_t * s)
{
	pktbuf_t *p;
 ...
 */
static void do_reset(tcp_socket_t * s)
{
	/* close connection */
	s->state = _CLOSED;
 ...
 */
static int handle_data(tcp_socket_t * s, pktbuf_t * pkt)
{
	tcp_header_t *tcp = pkt->tcp_hdr;
...
static void handle_ack(tcp_socket_t * s, pktbuf_t * pkt)
{
	tcp_header_t *tcp = pkt->tcp_hdr;
	dword ack;
	int advance;
...
void __tcp_handler(pktbuf_t * pkt, ip_route_t * r)
{
	...
	if (__sum((word *) tcp, pkt->pkt_bytes, __pseudo_sum(ip)) == 0) {
		for (prev = NULL, s = tcp_list; s; prev = s, s = s->next) {
			if (s->our_port == ntohs(tcp->dest_port)) {
				...
				if (s->his_port == ntohs(tcp->src_port) &&
				    !memcmp(r->ip_addr, s->his_addr.ip_addr,
					    sizeof(ip_addr_t)))
					break;
			}
		}
		...
		if (s) {
			...
			memcpy(s->his_addr.enet_addr, r->enet_addr,
			       sizeof(enet_addr_t));
			...
			case _LISTEN:
				if (tcp->flags & TCP_FLAG_SYN) {
					...
					memcpy(s->his_addr.ip_addr, r->ip_addr,
					       sizeof(ip_addr_t));
					s->data_bytes = 0;

					BSPLOG(bsp_log
					       ("SYN from %d.%d.%d.%d:%d (seq %x)\n",
						s->his_addr.ip_addr[0],
						s->his_addr.ip_addr[1],
						s->his_addr.ip_addr[2],
						s->his_addr.ip_addr[3],
						s->his_port,
						ntohl(tcp->seqnum)));

					tcp_send(s, TCP_FLAG_SYN | TCP_FLAG_ACK,
						 0);
				} else
					send_reset(pkt, r);
				break;
			...
			case _CLOSE_WAIT:
				...
				if ((tcp->flags & TCP_FLAG_FIN) &&
					...
					BSPLOG(bsp_log
					       ("FIN received - going to _CLOSE_WAIT\n"));
			...
			case _FIN_WAIT_1:
				...
				if (tcp->flags & TCP_FLAG_FIN) {
					...
					tcp_send(s, TCP_FLAG_FIN | TCP_FLAG_ACK,
						 0);
				}
				break;
			...
			BSPLOG(bsp_log
			       ("Unexpected segment from: %d.%d.%d.%d:%d\n",
 ...
 */
void __tcp_so_reuseaddr(tcp_socket_t * s)
{
 ...
 */
void __tcp_drain(tcp_socket_t * s)
{
...
void __tcp_abort(tcp_socket_t * s, unsigned long delay)
{
 ...
 */
void __tcp_close_wait(tcp_socket_t * s)
{
...
int __tcp_read(tcp_socket_t * s, char *buf, int len)
{
	...
	while (len) {
		if (len < s->rxcnt) {
			...
			BSPLOG(bsp_log
			       ("tcp_read: %d bytes left in rxlist head.\n",
				s->rxcnt));
			...
			BSPLOG(bsp_log
			       ("tcp_read: read %d bytes. pkt[%x] freed.\n",
				s->rxcnt, s->rxlist));
			nread += s->rxcnt;
			buf += s->rxcnt;
			len -= s->rxcnt;
			...
			if ((pkt = s->rxlist) != NULL) {
				...
				BSPLOG(bsp_log
				       ("tcp_read: next pkt[%x] has %d bytes.\n",
					s->rxlist, s->rxcnt));
			} else {
 ...
 */
int __tcp_write_block(tcp_socket_t * s, char *buf, int len)
{
	int total = 0;
	int n;
