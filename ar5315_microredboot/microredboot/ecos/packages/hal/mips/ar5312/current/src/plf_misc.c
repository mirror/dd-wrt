...
static unsigned int ar531x_calculated_cpu_freq = 0;
    ...
    */
    if (ar531x_calculated_cpu_freq == 0) {
        unsigned int clockCtl1, clockCtl2;
        int preDivideSelect;
        int preDivisor;
        int multiplier;

		/* must first check CLOCKCTL2 for PLL bypass */
        HAL_READ_UINT32(AR531X_CLOCKCTL2, clockCtl2);
		if (clockCtl2 & AR531X_CLOCKCTL2_STATUS_PLL_BYPASS) {
			ar531x_calculated_cpu_freq = 40000000;
