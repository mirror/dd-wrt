...
static int
read_byte (unsigned char **buffer, UInt32 * bufferSize)
{
  ...
  while (fis->name[0] != 0xff && count < 10)
	  ...
	  || !strcmp (fis->name, "kernel"))
	{
	  ...
	  memcpy ((unsigned char *) ZCACHEADDR,
		  (unsigned char *) fis->flash_base, 1 * 1024 * 1024);
	  return ZCACHEADDR;
#endif
	}
      p += 256;
...
static int
flash_erase_nvram (unsigned int flashsize, unsigned int blocksize)
{
  unsigned int res;
...
nvram_init (void)
{
  ...
  flashdetect ();
#endif
  header = (struct nvram_header *) (flashbase + flashsize - (sectorsize * 3));
...
ulg
decompress_kernel (ulg output_start, ulg free_mem_ptr_p,
...
#define AR531X_WDC      (AR531X_RESETTMR + 0x0008)
 sysRegWrite (AR531X_WDC, 0);
  ...
  if (resetbutton && !strcmp (resetbutton, "1"))
    puts("reset button manual override detected! (nvram var resetbutton=1)\n");
